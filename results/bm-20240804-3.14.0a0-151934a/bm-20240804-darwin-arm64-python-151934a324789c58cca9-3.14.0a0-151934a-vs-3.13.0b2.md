# Results vs. 3.13.0b2

- fork: python
- ref: 151934a324789c58cca9
- machine: darwin-arm64
- commit hash: 151934a
- commit date: 2024-08-04
- overall geometric mean: 1.00x faster
- HPT reliability: 99.71%
- HPT 99th percentile: 1.00x slower
- Memory change: 0.39x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 161 ms                                                     | 166 ms: 1.03x slower                                                  |
| docutils       | 1.44 sec                                                   | 1.49 sec: 1.03x slower                                                |
| Geometric mean | (ref)                                                      | 1.02x slower                                                          |

Benchmark hidden because not significant (2): html5lib, tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_eager_io              | 766 ms                                                     | 637 ms: 1.20x faster                                                  |
| async_tree_eager_io_tg           | 768 ms                                                     | 674 ms: 1.14x faster                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 505 ms: 1.12x faster                                                  |
| async_tree_none_tg               | 198 ms                                                     | 182 ms: 1.09x faster                                                  |
| async_tree_memoization           | 260 ms                                                     | 239 ms: 1.09x faster                                                  |
| async_tree_none                  | 209 ms                                                     | 194 ms: 1.08x faster                                                  |
| async_tree_memoization_tg        | 240 ms                                                     | 227 ms: 1.05x faster                                                  |
| async_tree_io                    | 563 ms                                                     | 542 ms: 1.04x faster                                                  |
| async_tree_cpu_io_mixed          | 467 ms                                                     | 452 ms: 1.03x faster                                                  |
| async_tree_eager_cpu_io_mixed    | 358 ms                                                     | 361 ms: 1.01x slower                                                  |
| async_tree_eager_tg              | 41.4 ms                                                    | 41.8 ms: 1.01x slower                                                 |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 337 ms: 1.02x slower                                                  |
| async_tree_eager                 | 60.3 ms                                                    | 61.6 ms: 1.02x slower                                                 |
| Geometric mean                   | (ref)                                                      | 1.05x faster                                                          |

Benchmark hidden because not significant (3): async_tree_cpu_io_mixed_tg, async_tree_eager_memoization_tg, async_tree_eager_memoization

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 48.6 ms                                                    | 48.3 ms: 1.01x faster                                                 |
| pidigits       | 282 ms                                                     | 282 ms: 1.00x faster                                                  |
| nbody          | 59.6 ms                                                    | 61.3 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                      | 1.01x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 2.58 ms                                                    | 2.55 ms: 1.01x faster                                                 |
| regex_dna      | 149 ms                                                     | 154 ms: 1.03x slower                                                  |
| Geometric mean | (ref)                                                      | 1.00x slower                                                          |

Benchmark hidden because not significant (2): regex_v8, regex_compile

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| xml_etree_generate   | 52.7 ms                                                    | 53.2 ms: 1.01x slower                                                 |
| unpickle_pure_python | 141 us                                                     | 143 us: 1.01x slower                                                  |
| json_loads           | 16.8 us                                                    | 17.2 us: 1.02x slower                                                 |
| tomli_loads          | 1.47 sec                                                   | 1.50 sec: 1.02x slower                                                |
| xml_etree_process    | 37.1 ms                                                    | 37.9 ms: 1.02x slower                                                 |
| pickle_pure_python   | 179 us                                                     | 184 us: 1.03x slower                                                  |
| xml_etree_parse      | 106 ms                                                     | 109 ms: 1.03x slower                                                  |
| xml_etree_iterparse  | 71.5 ms                                                    | 73.9 ms: 1.03x slower                                                 |
| json_dumps           | 6.23 ms                                                    | 6.56 ms: 1.05x slower                                                 |
| Geometric mean       | (ref)                                                      | 1.03x slower                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 15.2 ms                                                    | 18.0 ms: 1.19x slower                                                 |
| python_startup_no_site | 12.3 ms                                                    | 15.1 ms: 1.23x slower                                                 |
| Geometric mean         | (ref)                                                      | 1.21x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|-----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| genshi_text     | 13.9 ms                                                    | 14.0 ms: 1.01x slower                                                 |
| genshi_xml      | 29.9 ms                                                    | 30.3 ms: 1.01x slower                                                 |
| django_template | 19.4 ms                                                    | 19.8 ms: 1.02x slower                                                 |
| Geometric mean  | (ref)                                                      | 1.01x slower                                                          |

Benchmark hidden because not significant (1): mako

All benchmarks:
===============

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-darwin-arm64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| deepcopy                         | 204 us                                                     | 144 us: 1.41x faster                                                  |
| deepcopy_memo                    | 22.6 us                                                    | 16.8 us: 1.34x faster                                                 |
| deepcopy_reduce                  | 1.82 us                                                    | 1.50 us: 1.22x faster                                                 |
| async_tree_eager_io              | 766 ms                                                     | 637 ms: 1.20x faster                                                  |
| async_tree_eager_io_tg           | 768 ms                                                     | 674 ms: 1.14x faster                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 505 ms: 1.12x faster                                                  |
| generators                       | 22.9 ms                                                    | 20.5 ms: 1.11x faster                                                 |
| async_tree_none_tg               | 198 ms                                                     | 182 ms: 1.09x faster                                                  |
| async_tree_memoization           | 260 ms                                                     | 239 ms: 1.09x faster                                                  |
| async_tree_none                  | 209 ms                                                     | 194 ms: 1.08x faster                                                  |
| mdp                              | 1.53 sec                                                   | 1.44 sec: 1.07x faster                                                |
| async_tree_memoization_tg        | 240 ms                                                     | 227 ms: 1.05x faster                                                  |
| async_tree_io                    | 563 ms                                                     | 542 ms: 1.04x faster                                                  |
| async_tree_cpu_io_mixed          | 467 ms                                                     | 452 ms: 1.03x faster                                                  |
| create_gc_cycles                 | 897 us                                                     | 869 us: 1.03x faster                                                  |
| go                               | 101 ms                                                     | 97.8 ms: 1.03x faster                                                 |
| richards_super                   | 35.2 ms                                                    | 34.4 ms: 1.02x faster                                                 |
| dulwich_log                      | 27.6 ms                                                    | 27.1 ms: 1.02x faster                                                 |
| deltablue                        | 2.14 ms                                                    | 2.10 ms: 1.02x faster                                                 |
| richards                         | 31.8 ms                                                    | 31.4 ms: 1.02x faster                                                 |
| regex_effbot                     | 2.58 ms                                                    | 2.55 ms: 1.01x faster                                                 |
| scimark_sor                      | 94.9 ms                                                    | 94.0 ms: 1.01x faster                                                 |
| float                            | 48.6 ms                                                    | 48.3 ms: 1.01x faster                                                 |
| async_generators                 | 281 ms                                                     | 279 ms: 1.01x faster                                                  |
| coverage                         | 45.0 ms                                                    | 44.8 ms: 1.00x faster                                                 |
| logging_silent                   | 60.1 ns                                                    | 60.0 ns: 1.00x faster                                                 |
| asyncio_websockets               | 409 ms                                                     | 408 ms: 1.00x faster                                                  |
| pidigits                         | 282 ms                                                     | 282 ms: 1.00x faster                                                  |
| telco                            | 4.60 ms                                                    | 4.61 ms: 1.00x slower                                                 |
| gc_traversal                     | 2.45 ms                                                    | 2.45 ms: 1.00x slower                                                 |
| thrift                           | 435 us                                                     | 436 us: 1.00x slower                                                  |
| pyflate                          | 321 ms                                                     | 323 ms: 1.01x slower                                                  |
| logging_simple                   | 3.04 us                                                    | 3.06 us: 1.01x slower                                                 |
| raytrace                         | 147 ms                                                     | 148 ms: 1.01x slower                                                  |
| spectral_norm                    | 66.4 ms                                                    | 66.8 ms: 1.01x slower                                                 |
| pprint_pformat                   | 947 ms                                                     | 954 ms: 1.01x slower                                                  |
| async_tree_eager_cpu_io_mixed    | 358 ms                                                     | 361 ms: 1.01x slower                                                  |
| genshi_text                      | 13.9 ms                                                    | 14.0 ms: 1.01x slower                                                 |
| pprint_safe_repr                 | 465 ms                                                     | 469 ms: 1.01x slower                                                  |
| xml_etree_generate               | 52.7 ms                                                    | 53.2 ms: 1.01x slower                                                 |
| hexiom                           | 4.06 ms                                                    | 4.10 ms: 1.01x slower                                                 |
| sympy_integrate                  | 10.3 ms                                                    | 10.5 ms: 1.01x slower                                                 |
| async_tree_eager_tg              | 41.4 ms                                                    | 41.8 ms: 1.01x slower                                                 |
| scimark_monte_carlo              | 42.5 ms                                                    | 43.0 ms: 1.01x slower                                                 |
| bench_thread_pool                | 447 us                                                     | 452 us: 1.01x slower                                                  |
| coroutines                       | 15.8 ms                                                    | 16.0 ms: 1.01x slower                                                 |
| pycparser                        | 632 ms                                                     | 641 ms: 1.01x slower                                                  |
| unpickle_pure_python             | 141 us                                                     | 143 us: 1.01x slower                                                  |
| genshi_xml                       | 29.9 ms                                                    | 30.3 ms: 1.01x slower                                                 |
| logging_format                   | 3.31 us                                                    | 3.36 us: 1.01x slower                                                 |
| bpe_tokeniser                    | 3.05 sec                                                   | 3.10 sec: 1.02x slower                                                |
| scimark_sparse_mat_mult          | 2.77 ms                                                    | 2.82 ms: 1.02x slower                                                 |
| sympy_sum                        | 69.2 ms                                                    | 70.4 ms: 1.02x slower                                                 |
| json                             | 2.93 ms                                                    | 2.98 ms: 1.02x slower                                                 |
| sqlglot_optimize                 | 30.9 ms                                                    | 31.5 ms: 1.02x slower                                                 |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 337 ms: 1.02x slower                                                  |
| sqlglot_transpile                | 891 us                                                     | 907 us: 1.02x slower                                                  |
| nqueens                          | 52.2 ms                                                    | 53.3 ms: 1.02x slower                                                 |
| sympy_str                        | 131 ms                                                     | 134 ms: 1.02x slower                                                  |
| async_tree_eager                 | 60.3 ms                                                    | 61.6 ms: 1.02x slower                                                 |
| json_loads                       | 16.8 us                                                    | 17.2 us: 1.02x slower                                                 |
| django_template                  | 19.4 ms                                                    | 19.8 ms: 1.02x slower                                                 |
| tomli_loads                      | 1.47 sec                                                   | 1.50 sec: 1.02x slower                                                |
| xml_etree_process                | 37.1 ms                                                    | 37.9 ms: 1.02x slower                                                 |
| scimark_lu                       | 66.9 ms                                                    | 68.5 ms: 1.02x slower                                                 |
| scimark_fft                      | 181 ms                                                     | 185 ms: 1.02x slower                                                  |
| meteor_contest                   | 70.3 ms                                                    | 72.0 ms: 1.02x slower                                                 |
| crypto_pyaes                     | 49.5 ms                                                    | 50.7 ms: 1.02x slower                                                 |
| sqlglot_normalize                | 166 ms                                                     | 170 ms: 1.03x slower                                                  |
| pickle_pure_python               | 179 us                                                     | 184 us: 1.03x slower                                                  |
| sympy_expand                     | 226 ms                                                     | 232 ms: 1.03x slower                                                  |
| nbody                            | 59.6 ms                                                    | 61.3 ms: 1.03x slower                                                 |
| xml_etree_parse                  | 106 ms                                                     | 109 ms: 1.03x slower                                                  |
| 2to3                             | 161 ms                                                     | 166 ms: 1.03x slower                                                  |
| comprehensions                   | 10.2 us                                                    | 10.5 us: 1.03x slower                                                 |
| xml_etree_iterparse              | 71.5 ms                                                    | 73.9 ms: 1.03x slower                                                 |
| regex_dna                        | 149 ms                                                     | 154 ms: 1.03x slower                                                  |
| docutils                         | 1.44 sec                                                   | 1.49 sec: 1.03x slower                                                |
| dask                             | 221 ms                                                     | 230 ms: 1.04x slower                                                  |
| chaos                            | 34.0 ms                                                    | 35.7 ms: 1.05x slower                                                 |
| json_dumps                       | 6.23 ms                                                    | 6.56 ms: 1.05x slower                                                 |
| bench_mp_pool                    | 47.2 ms                                                    | 50.0 ms: 1.06x slower                                                 |
| fannkuch                         | 248 ms                                                     | 266 ms: 1.07x slower                                                  |
| typing_runtime_protocols         | 87.6 us                                                    | 94.9 us: 1.08x slower                                                 |
| python_startup                   | 15.2 ms                                                    | 18.0 ms: 1.19x slower                                                 |
| python_startup_no_site           | 12.3 ms                                                    | 15.1 ms: 1.23x slower                                                 |
| Geometric mean                   | (ref)                                                      | 1.00x faster                                                          |

Benchmark hidden because not significant (13): async_tree_cpu_io_mixed_tg, asyncio_tcp, asyncio_tcp_ssl, async_tree_eager_memoization_tg, async_tree_eager_memoization, regex_v8, regex_compile, mako, pathlib, html5lib, tornado_http, pylint, sqlglot_parse
Ignored benchmarks (11) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 99.71% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 0.39x