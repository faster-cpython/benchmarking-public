
# Results vs. 3.10.4

- fork: python
- ref: 0771d71eea30316020a8
- machine: windows-amd64
- commit hash: 0771d71
- commit date: 2022-07-25
- overall geometric mean: 1.11x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 204 ms: 1.16x faster                                                       |
| chameleon      | 5.92 ms                                                     | 5.23 ms: 1.13x faster                                                      |
| docutils       | 1.89 sec                                                    | 1.59 sec: 1.19x faster                                                     |
| html5lib       | 46.5 ms                                                     | 38.9 ms: 1.19x faster                                                      |
| tornado_http   | 109 ms                                                      | 91.8 ms: 1.19x faster                                                      |
| Geometric mean | (ref)                                                       | 1.17x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 53.5 ms: 1.12x faster                                                      |
| nbody          | 69.3 ms                                                     | 68.4 ms: 1.01x faster                                                      |
| pidigits       | 145 ms                                                      | 147 ms: 1.01x slower                                                       |
| Geometric mean | (ref)                                                       | 1.04x faster                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 90.9 ms: 1.14x faster                                                      |
| regex_dna      | 132 ms                                                      | 121 ms: 1.09x faster                                                       |
| regex_v8       | 15.0 ms                                                     | 14.4 ms: 1.05x faster                                                      |
| regex_effbot   | 1.66 ms                                                     | 1.64 ms: 1.01x faster                                                      |
| Geometric mean | (ref)                                                       | 1.07x faster                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| pickle_pure_python   | 257 us                                                      | 199 us: 1.29x faster                                                       |
| unpickle             | 9.17 us                                                     | 7.80 us: 1.18x faster                                                      |
| xml_etree_process    | 43.4 ms                                                     | 37.2 ms: 1.17x faster                                                      |
| unpickle_pure_python | 171 us                                                      | 151 us: 1.13x faster                                                       |
| json_dumps           | 8.50 ms                                                     | 7.69 ms: 1.11x faster                                                      |
| xml_etree_parse      | 102 ms                                                      | 92.7 ms: 1.10x faster                                                      |
| unpickle_list        | 2.81 us                                                     | 2.60 us: 1.08x faster                                                      |
| json_loads           | 14.2 us                                                     | 13.7 us: 1.03x faster                                                      |
| xml_etree_generate   | 54.5 ms                                                     | 53.1 ms: 1.03x faster                                                      |
| pickle               | 6.80 us                                                     | 6.65 us: 1.02x faster                                                      |
| xml_etree_iterparse  | 63.5 ms                                                     | 62.3 ms: 1.02x faster                                                      |
| pickle_list          | 2.59 us                                                     | 2.75 us: 1.06x slower                                                      |
| pickle_dict          | 16.9 us                                                     | 18.5 us: 1.09x slower                                                      |
| Geometric mean       | (ref)                                                       | 1.07x faster                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.3 ms: 1.09x faster                                                      |
| python_startup_no_site | 15.5 ms                                                     | 15.3 ms: 1.01x faster                                                      |
| Geometric mean         | (ref)                                                       | 1.05x faster                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 7.18 ms: 1.23x faster                                                      |
| django_template | 28.2 ms                                                     | 24.6 ms: 1.15x faster                                                      |
| genshi_text     | 19.0 ms                                                     | 17.3 ms: 1.10x faster                                                      |
| genshi_xml      | 40.5 ms                                                     | 38.1 ms: 1.06x faster                                                      |
| Geometric mean  | (ref)                                                       | 1.13x faster                                                               |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220725-pythonperf1-amd64-python-0771d71eea30316020a8-3.11.0b5-0771d71 |
|-------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.72 ms: 1.54x faster                                                      |
| async_tree_io           | 1.07 sec                                                    | 756 ms: 1.41x faster                                                       |
| scimark_lu              | 85.4 ms                                                     | 62.6 ms: 1.36x faster                                                      |
| go                      | 136 ms                                                      | 102 ms: 1.34x faster                                                       |
| scimark_sor             | 105 ms                                                      | 78.3 ms: 1.34x faster                                                      |
| async_tree_none         | 420 ms                                                      | 316 ms: 1.33x faster                                                       |
| raytrace                | 271 ms                                                      | 204 ms: 1.33x faster                                                       |
| async_tree_memoization  | 497 ms                                                      | 376 ms: 1.32x faster                                                       |
| logging_silent          | 96.4 ns                                                     | 72.9 ns: 1.32x faster                                                      |
| sqlglot_parse           | 1.22 ms                                                     | 925 us: 1.32x faster                                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.12 ms: 1.30x faster                                                      |
| richards                | 41.2 ms                                                     | 32.0 ms: 1.29x faster                                                      |
| pickle_pure_python      | 257 us                                                      | 199 us: 1.29x faster                                                       |
| crypto_pyaes            | 62.3 ms                                                     | 48.6 ms: 1.28x faster                                                      |
| async_generators        | 224 ms                                                      | 177 ms: 1.27x faster                                                       |
| pyflate                 | 387 ms                                                      | 308 ms: 1.26x faster                                                       |
| mypy2                   | 352 ms                                                      | 280 ms: 1.26x faster                                                       |
| scimark_monte_carlo     | 55.9 ms                                                     | 44.6 ms: 1.25x faster                                                      |
| thrift                  | 615 us                                                      | 497 us: 1.24x faster                                                       |
| mako                    | 8.80 ms                                                     | 7.18 ms: 1.23x faster                                                      |
| chaos                   | 58.9 ms                                                     | 48.1 ms: 1.22x faster                                                      |
| pycparser               | 868 ms                                                      | 714 ms: 1.22x faster                                                       |
| hexiom                  | 5.52 ms                                                     | 4.61 ms: 1.20x faster                                                      |
| html5lib                | 46.5 ms                                                     | 38.9 ms: 1.19x faster                                                      |
| async_tree_cpu_io_mixed | 609 ms                                                      | 511 ms: 1.19x faster                                                       |
| tornado_http            | 109 ms                                                      | 91.8 ms: 1.19x faster                                                      |
| docutils                | 1.89 sec                                                    | 1.59 sec: 1.19x faster                                                     |
| unpickle                | 9.17 us                                                     | 7.80 us: 1.18x faster                                                      |
| create_gc_cycles        | 782 us                                                      | 665 us: 1.18x faster                                                       |
| aiohttp                 | 1.01 ms                                                     | 862 us: 1.17x faster                                                       |
| xml_etree_process       | 43.4 ms                                                     | 37.2 ms: 1.17x faster                                                      |
| 2to3                    | 237 ms                                                      | 204 ms: 1.16x faster                                                       |
| sqlalchemy_declarative  | 95.4 ms                                                     | 82.9 ms: 1.15x faster                                                      |
| django_template         | 28.2 ms                                                     | 24.6 ms: 1.15x faster                                                      |
| regex_compile           | 103 ms                                                      | 90.9 ms: 1.14x faster                                                      |
| chameleon               | 5.92 ms                                                     | 5.23 ms: 1.13x faster                                                      |
| unpickle_pure_python    | 171 us                                                      | 151 us: 1.13x faster                                                       |
| pprint_safe_repr        | 589 ms                                                      | 524 ms: 1.12x faster                                                       |
| float                   | 60.2 ms                                                     | 53.5 ms: 1.12x faster                                                      |
| sqlglot_optimize        | 39.0 ms                                                     | 34.7 ms: 1.12x faster                                                      |
| dask                    | 305 ms                                                      | 272 ms: 1.12x faster                                                       |
| pprint_pformat          | 1.21 sec                                                    | 1.08 sec: 1.11x faster                                                     |
| bench_thread_pool       | 946 us                                                      | 853 us: 1.11x faster                                                       |
| deepcopy_memo           | 28.5 us                                                     | 25.8 us: 1.11x faster                                                      |
| json_dumps              | 8.50 ms                                                     | 7.69 ms: 1.11x faster                                                      |
| sqlite_synth            | 1.84 us                                                     | 1.67 us: 1.10x faster                                                      |
| genshi_text             | 19.0 ms                                                     | 17.3 ms: 1.10x faster                                                      |
| xml_etree_parse         | 102 ms                                                      | 92.7 ms: 1.10x faster                                                      |
| python_startup          | 20.0 ms                                                     | 18.3 ms: 1.09x faster                                                      |
| pylint                  | 347 ms                                                      | 319 ms: 1.09x faster                                                       |
| asyncio_tcp             | 712 ms                                                      | 655 ms: 1.09x faster                                                       |
| regex_dna               | 132 ms                                                      | 121 ms: 1.09x faster                                                       |
| coroutines              | 15.9 ms                                                     | 14.7 ms: 1.09x faster                                                      |
| sympy_integrate         | 14.8 ms                                                     | 13.7 ms: 1.08x faster                                                      |
| pathlib                 | 77.4 ms                                                     | 71.4 ms: 1.08x faster                                                      |
| unpickle_list           | 2.81 us                                                     | 2.60 us: 1.08x faster                                                      |
| sympy_sum               | 104 ms                                                      | 97.1 ms: 1.07x faster                                                      |
| sympy_expand            | 315 ms                                                      | 295 ms: 1.07x faster                                                       |
| scimark_fft             | 193 ms                                                      | 181 ms: 1.07x faster                                                       |
| genshi_xml              | 40.5 ms                                                     | 38.1 ms: 1.06x faster                                                      |
| dulwich_log             | 47.6 ms                                                     | 44.7 ms: 1.06x faster                                                      |
| spectral_norm           | 78.0 ms                                                     | 74.1 ms: 1.05x faster                                                      |
| sqlglot_normalize       | 202 ms                                                      | 192 ms: 1.05x faster                                                       |
| regex_v8                | 15.0 ms                                                     | 14.4 ms: 1.05x faster                                                      |
| sqlalchemy_imperative   | 11.0 ms                                                     | 10.5 ms: 1.04x faster                                                      |
| deepcopy                | 255 us                                                      | 245 us: 1.04x faster                                                       |
| deepcopy_reduce         | 2.16 us                                                     | 2.07 us: 1.04x faster                                                      |
| mdp                     | 1.71 sec                                                    | 1.65 sec: 1.04x faster                                                     |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.57 ms: 1.04x faster                                                      |
| json_loads              | 14.2 us                                                     | 13.7 us: 1.03x faster                                                      |
| comprehensions          | 16.0 us                                                     | 15.5 us: 1.03x faster                                                      |
| xml_etree_generate      | 54.5 ms                                                     | 53.1 ms: 1.03x faster                                                      |
| pickle                  | 6.80 us                                                     | 6.65 us: 1.02x faster                                                      |
| nqueens                 | 67.0 ms                                                     | 65.7 ms: 1.02x faster                                                      |
| xml_etree_iterparse     | 63.5 ms                                                     | 62.3 ms: 1.02x faster                                                      |
| sympy_str               | 188 ms                                                      | 185 ms: 1.02x faster                                                       |
| nbody                   | 69.3 ms                                                     | 68.4 ms: 1.01x faster                                                      |
| python_startup_no_site  | 15.5 ms                                                     | 15.3 ms: 1.01x faster                                                      |
| regex_effbot            | 1.66 ms                                                     | 1.64 ms: 1.01x faster                                                      |
| bench_mp_pool           | 60.7 ms                                                     | 61.2 ms: 1.01x slower                                                      |
| pidigits                | 145 ms                                                      | 147 ms: 1.01x slower                                                       |
| fannkuch                | 258 ms                                                      | 265 ms: 1.03x slower                                                       |
| meteor_contest          | 72.5 ms                                                     | 75.4 ms: 1.04x slower                                                      |
| telco                   | 3.78 ms                                                     | 3.95 ms: 1.05x slower                                                      |
| gc_traversal            | 1.34 ms                                                     | 1.43 ms: 1.06x slower                                                      |
| logging_format          | 6.66 us                                                     | 7.07 us: 1.06x slower                                                      |
| pickle_list             | 2.59 us                                                     | 2.75 us: 1.06x slower                                                      |
| generators              | 31.6 ms                                                     | 33.8 ms: 1.07x slower                                                      |
| logging_simple          | 6.20 us                                                     | 6.73 us: 1.09x slower                                                      |
| pickle_dict             | 16.9 us                                                     | 18.5 us: 1.09x slower                                                      |
| unpack_sequence         | 37.8 ns                                                     | 43.5 ns: 1.15x slower                                                      |
| coverage                | 40.0 ms                                                     | 52.1 ms: 1.30x slower                                                      |
| Geometric mean          | (ref)                                                       | 1.11x faster                                                               |

Benchmark hidden because not significant (2): flaskblogging, json
Ignored benchmarks (4) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: asyncio_tcp_ssl, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.07x
