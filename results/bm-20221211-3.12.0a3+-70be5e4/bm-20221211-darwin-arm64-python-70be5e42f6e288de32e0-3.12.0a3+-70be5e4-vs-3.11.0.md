
# Results vs. 3.11.0

- fork: python
- ref: 70be5e42f6e288de32e0
- machine: darwin-arm64
- commit hash: 70be5e4
- commit date: 2022-12-11
- overall geometric mean: 1.00x faster \*
- HPT reliability: 83.17%
- HPT 99th percentile: 1.00x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 161 ms                                                 | 161 ms: 1.00x faster                                                   |
| chameleon      | 4.60 ms                                                | 4.45 ms: 1.03x faster                                                  |
| docutils       | 1.47 sec                                               | 1.46 sec: 1.01x faster                                                 |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 65.6 ms                                                | 64.0 ms: 1.02x faster                                                  |
| float          | 53.0 ms                                                | 52.5 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 76.7 ms                                                | 73.9 ms: 1.04x faster                                                  |
| regex_v8       | 16.1 ms                                                | 15.7 ms: 1.02x faster                                                  |
| regex_dna      | 152 ms                                                 | 150 ms: 1.01x faster                                                   |
| regex_effbot   | 2.63 ms                                                | 2.62 ms: 1.00x faster                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 7.63 ms                                                | 6.21 ms: 1.23x faster                                                  |
| xml_etree_parse      | 108 ms                                                 | 96.3 ms: 1.12x faster                                                  |
| unpickle_list        | 2.65 us                                                | 2.58 us: 1.03x faster                                                  |
| unpickle_pure_python | 159 us                                                 | 156 us: 1.02x faster                                                   |
| xml_etree_iterparse  | 70.1 ms                                                | 69.2 ms: 1.01x faster                                                  |
| xml_etree_generate   | 48.3 ms                                                | 47.6 ms: 1.01x faster                                                  |
| pickle_pure_python   | 201 us                                                 | 198 us: 1.01x faster                                                   |
| xml_etree_process    | 35.1 ms                                                | 34.7 ms: 1.01x faster                                                  |
| pickle_dict          | 18.0 us                                                | 17.9 us: 1.00x faster                                                  |
| json_loads           | 16.1 us                                                | 16.3 us: 1.01x slower                                                  |
| pickle               | 7.15 us                                                | 7.52 us: 1.05x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.03x faster                                                           |

Benchmark hidden because not significant (2): unpickle, pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.4 ms                                                | 12.2 ms: 1.01x faster                                                  |
| python_startup_no_site | 10.2 ms                                                | 10.3 ms: 1.01x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.00x faster                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 8.53 ms                                                | 7.99 ms: 1.07x faster                                                  |
| genshi_xml      | 29.8 ms                                                | 28.7 ms: 1.04x faster                                                  |
| genshi_text     | 15.3 ms                                                | 15.0 ms: 1.02x faster                                                  |
| django_template | 21.0 ms                                                | 20.8 ms: 1.01x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.03x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps              | 7.63 ms                                                | 6.21 ms: 1.23x faster                                                  |
| scimark_sparse_mat_mult | 3.19 ms                                                | 2.79 ms: 1.14x faster                                                  |
| xml_etree_parse         | 108 ms                                                 | 96.3 ms: 1.12x faster                                                  |
| deltablue               | 2.81 ms                                                | 2.55 ms: 1.10x faster                                                  |
| richards                | 32.2 ms                                                | 29.3 ms: 1.10x faster                                                  |
| logging_silent          | 68.1 ns                                                | 63.3 ns: 1.08x faster                                                  |
| mako                    | 8.53 ms                                                | 7.99 ms: 1.07x faster                                                  |
| scimark_sor             | 82.6 ms                                                | 78.2 ms: 1.06x faster                                                  |
| regex_compile           | 76.7 ms                                                | 73.9 ms: 1.04x faster                                                  |
| pycparser               | 698 ms                                                 | 672 ms: 1.04x faster                                                   |
| unpack_sequence         | 34.1 ns                                                | 32.8 ns: 1.04x faster                                                  |
| genshi_xml              | 29.8 ms                                                | 28.7 ms: 1.04x faster                                                  |
| bench_thread_pool       | 474 us                                                 | 458 us: 1.04x faster                                                   |
| chameleon               | 4.60 ms                                                | 4.45 ms: 1.03x faster                                                  |
| create_gc_cycles        | 716 us                                                 | 692 us: 1.03x faster                                                   |
| logging_simple          | 3.55 us                                                | 3.44 us: 1.03x faster                                                  |
| scimark_lu              | 73.3 ms                                                | 71.3 ms: 1.03x faster                                                  |
| scimark_fft             | 200 ms                                                 | 194 ms: 1.03x faster                                                   |
| unpickle_list           | 2.65 us                                                | 2.58 us: 1.03x faster                                                  |
| regex_v8                | 16.1 ms                                                | 15.7 ms: 1.02x faster                                                  |
| nbody                   | 65.6 ms                                                | 64.0 ms: 1.02x faster                                                  |
| unpickle_pure_python    | 159 us                                                 | 156 us: 1.02x faster                                                   |
| dulwich_log             | 30.3 ms                                                | 29.7 ms: 1.02x faster                                                  |
| thrift                  | 442 us                                                 | 433 us: 1.02x faster                                                   |
| mdp                     | 1.55 sec                                               | 1.51 sec: 1.02x faster                                                 |
| genshi_text             | 15.3 ms                                                | 15.0 ms: 1.02x faster                                                  |
| raytrace                | 207 ms                                                 | 204 ms: 1.02x faster                                                   |
| python_startup          | 12.4 ms                                                | 12.2 ms: 1.01x faster                                                  |
| regex_dna               | 152 ms                                                 | 150 ms: 1.01x faster                                                   |
| xml_etree_iterparse     | 70.1 ms                                                | 69.2 ms: 1.01x faster                                                  |
| xml_etree_generate      | 48.3 ms                                                | 47.6 ms: 1.01x faster                                                  |
| pickle_pure_python      | 201 us                                                 | 198 us: 1.01x faster                                                   |
| xml_etree_process       | 35.1 ms                                                | 34.7 ms: 1.01x faster                                                  |
| fannkuch                | 261 ms                                                 | 259 ms: 1.01x faster                                                   |
| django_template         | 21.0 ms                                                | 20.8 ms: 1.01x faster                                                  |
| logging_format          | 3.78 us                                                | 3.74 us: 1.01x faster                                                  |
| hexiom                  | 4.72 ms                                                | 4.68 ms: 1.01x faster                                                  |
| float                   | 53.0 ms                                                | 52.5 ms: 1.01x faster                                                  |
| sqlglot_parse           | 959 us                                                 | 951 us: 1.01x faster                                                   |
| docutils                | 1.47 sec                                               | 1.46 sec: 1.01x faster                                                 |
| sqlglot_transpile       | 1.12 ms                                                | 1.12 ms: 1.01x faster                                                  |
| 2to3                    | 161 ms                                                 | 161 ms: 1.00x faster                                                   |
| go                      | 109 ms                                                 | 108 ms: 1.00x faster                                                   |
| sympy_integrate         | 11.5 ms                                                | 11.5 ms: 1.00x faster                                                  |
| pickle_dict             | 18.0 us                                                | 17.9 us: 1.00x faster                                                  |
| regex_effbot            | 2.63 ms                                                | 2.62 ms: 1.00x faster                                                  |
| telco                   | 3.41 ms                                                | 3.42 ms: 1.00x slower                                                  |
| chaos                   | 49.4 ms                                                | 49.6 ms: 1.00x slower                                                  |
| sympy_str               | 151 ms                                                 | 152 ms: 1.01x slower                                                   |
| sqlglot_normalize       | 171 ms                                                 | 172 ms: 1.01x slower                                                   |
| sympy_expand            | 242 ms                                                 | 244 ms: 1.01x slower                                                   |
| json                    | 2.78 ms                                                | 2.80 ms: 1.01x slower                                                  |
| python_startup_no_site  | 10.2 ms                                                | 10.3 ms: 1.01x slower                                                  |
| json_loads              | 16.1 us                                                | 16.3 us: 1.01x slower                                                  |
| nqueens                 | 59.8 ms                                                | 60.4 ms: 1.01x slower                                                  |
| sympy_sum               | 85.5 ms                                                | 86.6 ms: 1.01x slower                                                  |
| scimark_monte_carlo     | 46.5 ms                                                | 47.2 ms: 1.02x slower                                                  |
| sqlglot_optimize        | 31.1 ms                                                | 31.6 ms: 1.02x slower                                                  |
| crypto_pyaes            | 51.7 ms                                                | 52.7 ms: 1.02x slower                                                  |
| async_tree_none         | 286 ms                                                 | 291 ms: 1.02x slower                                                   |
| asyncio_tcp             | 651 ms                                                 | 665 ms: 1.02x slower                                                   |
| async_tree_cpu_io_mixed | 533 ms                                                 | 547 ms: 1.03x slower                                                   |
| async_generators        | 196 ms                                                 | 201 ms: 1.03x slower                                                   |
| pprint_pformat          | 954 ms                                                 | 978 ms: 1.03x slower                                                   |
| pprint_safe_repr        | 466 ms                                                 | 480 ms: 1.03x slower                                                   |
| comprehensions          | 16.1 us                                                | 16.6 us: 1.03x slower                                                  |
| coroutines              | 17.8 ms                                                | 18.4 ms: 1.03x slower                                                  |
| meteor_contest          | 73.5 ms                                                | 76.4 ms: 1.04x slower                                                  |
| deepcopy_reduce         | 1.91 us                                                | 1.99 us: 1.04x slower                                                  |
| deepcopy                | 223 us                                                 | 232 us: 1.04x slower                                                   |
| pickle                  | 7.15 us                                                | 7.52 us: 1.05x slower                                                  |
| pyflate                 | 310 ms                                                 | 327 ms: 1.06x slower                                                   |
| async_tree_io           | 704 ms                                                 | 747 ms: 1.06x slower                                                   |
| deepcopy_memo           | 26.3 us                                                | 28.0 us: 1.06x slower                                                  |
| sqlite_synth            | 1.27 us                                                | 1.44 us: 1.13x slower                                                  |
| generators              | 28.8 ms                                                | 32.7 ms: 1.14x slower                                                  |
| mypy2                   | 195 ms                                                 | 262 ms: 1.34x slower                                                   |
| Geometric mean          | (ref)                                                  | 1.00x faster                                                           |

Benchmark hidden because not significant (10): async_tree_memoization, html5lib, pidigits, spectral_norm, unpickle, gc_traversal, pickle_list, coverage, bench_mp_pool, pathlib
Ignored benchmarks (11) of results/bm-20221024-3.11.0-deaf509/bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20221211-3.12.0a3+-70be5e4/bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4.json: dask


# HPT report

- Reliability score: 83.17% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x
