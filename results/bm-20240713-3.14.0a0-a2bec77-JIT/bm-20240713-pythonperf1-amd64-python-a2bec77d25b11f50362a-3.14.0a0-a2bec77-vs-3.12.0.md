# Results vs. 3.12.0

- fork: python
- ref: a2bec77d25b11f50362a
- machine: windows-amd64
- commit hash: a2bec77
- commit date: 2024-07-13
- overall geometric mean: 1.07x faster
- HPT reliability: 97.72%
- HPT 99th percentile: 1.00x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 233 ms: 1.07x slower                                                       |
| docutils       | 1.66 sec                                                    | 1.77 sec: 1.06x slower                                                     |
| tornado_http   | 89.5 ms                                                     | 84.3 ms: 1.06x faster                                                      |
| Geometric mean | (ref)                                                       | 1.02x slower                                                               |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_none_tg         | 285 ms                                                      | 186 ms: 1.54x faster                                                       |
| async_tree_memoization_tg  | 367 ms                                                      | 241 ms: 1.52x faster                                                       |
| async_tree_io_tg           | 771 ms                                                      | 519 ms: 1.49x faster                                                       |
| async_tree_none            | 291 ms                                                      | 205 ms: 1.42x faster                                                       |
| async_tree_io              | 731 ms                                                      | 525 ms: 1.39x faster                                                       |
| async_tree_memoization     | 339 ms                                                      | 250 ms: 1.35x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 385 ms: 1.31x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 384 ms: 1.27x faster                                                       |
| Geometric mean             | (ref)                                                       | 1.41x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| nbody          | 71.9 ms                                                     | 51.0 ms: 1.41x faster                                                      |
| float          | 56.8 ms                                                     | 45.1 ms: 1.26x faster                                                      |
| pidigits       | 152 ms                                                      | 149 ms: 1.02x faster                                                       |
| Geometric mean | (ref)                                                       | 1.22x faster                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 121 ms: 1.04x faster                                                       |
| regex_effbot   | 1.62 ms                                                     | 1.57 ms: 1.03x faster                                                      |
| regex_compile  | 87.5 ms                                                     | 90.5 ms: 1.03x slower                                                      |
| regex_v8       | 14.2 ms                                                     | 20.8 ms: 1.46x slower                                                      |
| Geometric mean | (ref)                                                       | 1.09x slower                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| pickle_pure_python   | 195 us                                                      | 173 us: 1.13x faster                                                       |
| tomli_loads          | 1.37 sec                                                    | 1.25 sec: 1.09x faster                                                     |
| xml_etree_iterparse  | 65.2 ms                                                     | 60.7 ms: 1.07x faster                                                      |
| xml_etree_generate   | 55.8 ms                                                     | 52.8 ms: 1.06x faster                                                      |
| unpickle_pure_python | 133 us                                                      | 130 us: 1.02x faster                                                       |
| xml_etree_process    | 37.7 ms                                                     | 37.4 ms: 1.01x faster                                                      |
| xml_etree_parse      | 93.0 ms                                                     | 92.3 ms: 1.01x faster                                                      |
| json_loads           | 13.9 us                                                     | 14.3 us: 1.03x slower                                                      |
| json_dumps           | 5.70 ms                                                     | 5.86 ms: 1.03x slower                                                      |
| Geometric mean       | (ref)                                                       | 1.04x faster                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 17.5 ms: 1.08x slower                                                      |
| python_startup         | 19.5 ms                                                     | 21.0 ms: 1.08x slower                                                      |
| Geometric mean         | (ref)                                                       | 1.08x slower                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| mako            | 7.09 ms                                                     | 5.16 ms: 1.37x faster                                                      |
| django_template | 22.9 ms                                                     | 25.6 ms: 1.11x slower                                                      |
| Geometric mean  | (ref)                                                       | 1.11x faster                                                               |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77 |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_none_tg         | 285 ms                                                      | 186 ms: 1.54x faster                                                       |
| async_tree_memoization_tg  | 367 ms                                                      | 241 ms: 1.52x faster                                                       |
| deepcopy_memo              | 23.7 us                                                     | 15.8 us: 1.50x faster                                                      |
| async_tree_io_tg           | 771 ms                                                      | 519 ms: 1.49x faster                                                       |
| spectral_norm              | 66.9 ms                                                     | 45.5 ms: 1.47x faster                                                      |
| asyncio_tcp_ssl            | 2.10 sec                                                    | 1.44 sec: 1.46x faster                                                     |
| async_tree_none            | 291 ms                                                      | 205 ms: 1.42x faster                                                       |
| nbody                      | 71.9 ms                                                     | 51.0 ms: 1.41x faster                                                      |
| async_tree_io              | 731 ms                                                      | 525 ms: 1.39x faster                                                       |
| mako                       | 7.09 ms                                                     | 5.16 ms: 1.37x faster                                                      |
| comprehensions             | 14.1 us                                                     | 10.4 us: 1.36x faster                                                      |
| async_tree_memoization     | 339 ms                                                      | 250 ms: 1.35x faster                                                       |
| deepcopy                   | 238 us                                                      | 181 us: 1.32x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 385 ms: 1.31x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 384 ms: 1.27x faster                                                       |
| float                      | 56.8 ms                                                     | 45.1 ms: 1.26x faster                                                      |
| scimark_fft                | 184 ms                                                      | 151 ms: 1.22x faster                                                       |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.12 ms: 1.21x faster                                                      |
| crypto_pyaes               | 48.5 ms                                                     | 40.7 ms: 1.19x faster                                                      |
| deepcopy_reduce            | 2.09 us                                                     | 1.79 us: 1.17x faster                                                      |
| scimark_monte_carlo        | 43.7 ms                                                     | 37.6 ms: 1.16x faster                                                      |
| pyflate                    | 295 ms                                                      | 258 ms: 1.14x faster                                                       |
| pickle_pure_python         | 195 us                                                      | 173 us: 1.13x faster                                                       |
| chaos                      | 43.3 ms                                                     | 38.8 ms: 1.12x faster                                                      |
| fannkuch                   | 247 ms                                                      | 221 ms: 1.11x faster                                                       |
| pprint_safe_repr           | 513 ms                                                      | 464 ms: 1.11x faster                                                       |
| pprint_pformat             | 1.05 sec                                                    | 953 ms: 1.10x faster                                                       |
| logging_simple             | 6.28 us                                                     | 5.74 us: 1.09x faster                                                      |
| tomli_loads                | 1.37 sec                                                    | 1.25 sec: 1.09x faster                                                     |
| logging_format             | 6.72 us                                                     | 6.19 us: 1.09x faster                                                      |
| pathlib                    | 80.5 ms                                                     | 74.6 ms: 1.08x faster                                                      |
| raytrace                   | 192 ms                                                      | 179 ms: 1.08x faster                                                       |
| xml_etree_iterparse        | 65.2 ms                                                     | 60.7 ms: 1.07x faster                                                      |
| bench_thread_pool          | 857 us                                                      | 804 us: 1.07x faster                                                       |
| logging_silent             | 60.5 ns                                                     | 56.8 ns: 1.06x faster                                                      |
| tornado_http               | 89.5 ms                                                     | 84.3 ms: 1.06x faster                                                      |
| xml_etree_generate         | 55.8 ms                                                     | 52.8 ms: 1.06x faster                                                      |
| asyncio_tcp                | 487 ms                                                      | 465 ms: 1.05x faster                                                       |
| nqueens                    | 62.8 ms                                                     | 59.9 ms: 1.05x faster                                                      |
| regex_dna                  | 126 ms                                                      | 121 ms: 1.04x faster                                                       |
| coroutines                 | 14.3 ms                                                     | 13.8 ms: 1.03x faster                                                      |
| regex_effbot               | 1.62 ms                                                     | 1.57 ms: 1.03x faster                                                      |
| unpickle_pure_python       | 133 us                                                      | 130 us: 1.02x faster                                                       |
| pidigits                   | 152 ms                                                      | 149 ms: 1.02x faster                                                       |
| meteor_contest             | 74.6 ms                                                     | 73.6 ms: 1.01x faster                                                      |
| xml_etree_process          | 37.7 ms                                                     | 37.4 ms: 1.01x faster                                                      |
| xml_etree_parse            | 93.0 ms                                                     | 92.3 ms: 1.01x faster                                                      |
| bench_mp_pool              | 69.2 ms                                                     | 69.6 ms: 1.01x slower                                                      |
| sqlglot_transpile          | 1.02 ms                                                     | 1.03 ms: 1.01x slower                                                      |
| sqlglot_normalize          | 187 ms                                                      | 190 ms: 1.01x slower                                                       |
| sympy_sum                  | 91.5 ms                                                     | 92.9 ms: 1.01x slower                                                      |
| gc_traversal               | 1.52 ms                                                     | 1.55 ms: 1.02x slower                                                      |
| go                         | 91.6 ms                                                     | 94.2 ms: 1.03x slower                                                      |
| json_loads                 | 13.9 us                                                     | 14.3 us: 1.03x slower                                                      |
| json_dumps                 | 5.70 ms                                                     | 5.86 ms: 1.03x slower                                                      |
| sqlglot_optimize           | 34.5 ms                                                     | 35.6 ms: 1.03x slower                                                      |
| sympy_str                  | 175 ms                                                      | 181 ms: 1.03x slower                                                       |
| regex_compile              | 87.5 ms                                                     | 90.5 ms: 1.03x slower                                                      |
| mdp                        | 1.37 sec                                                    | 1.42 sec: 1.04x slower                                                     |
| deltablue                  | 2.16 ms                                                     | 2.25 ms: 1.04x slower                                                      |
| generators                 | 22.5 ms                                                     | 23.6 ms: 1.05x slower                                                      |
| richards_super             | 32.1 ms                                                     | 34.0 ms: 1.06x slower                                                      |
| docutils                   | 1.66 sec                                                    | 1.77 sec: 1.06x slower                                                     |
| 2to3                       | 218 ms                                                      | 233 ms: 1.07x slower                                                       |
| async_generators           | 239 ms                                                      | 257 ms: 1.07x slower                                                       |
| python_startup_no_site     | 16.2 ms                                                     | 17.5 ms: 1.08x slower                                                      |
| richards                   | 28.4 ms                                                     | 30.6 ms: 1.08x slower                                                      |
| sympy_integrate            | 13.0 ms                                                     | 14.0 ms: 1.08x slower                                                      |
| python_startup             | 19.5 ms                                                     | 21.0 ms: 1.08x slower                                                      |
| telco                      | 4.13 ms                                                     | 4.48 ms: 1.09x slower                                                      |
| sympy_expand               | 284 ms                                                      | 313 ms: 1.10x slower                                                       |
| django_template            | 22.9 ms                                                     | 25.6 ms: 1.11x slower                                                      |
| hexiom                     | 4.10 ms                                                     | 4.62 ms: 1.13x slower                                                      |
| coverage                   | 40.8 ms                                                     | 46.1 ms: 1.13x slower                                                      |
| scimark_sor                | 78.8 ms                                                     | 88.9 ms: 1.13x slower                                                      |
| pycparser                  | 699 ms                                                      | 822 ms: 1.18x slower                                                       |
| typing_runtime_protocols   | 95.1 us                                                     | 113 us: 1.19x slower                                                       |
| scimark_lu                 | 58.9 ms                                                     | 70.3 ms: 1.19x slower                                                      |
| create_gc_cycles           | 752 us                                                      | 901 us: 1.20x slower                                                       |
| regex_v8                   | 14.2 ms                                                     | 20.8 ms: 1.46x slower                                                      |
| Geometric mean             | (ref)                                                       | 1.07x faster                                                               |

Benchmark hidden because not significant (2): sqlglot_parse, json
Ignored benchmarks (14) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, dulwich_log, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240713-3.14.0a0-a2bec77-JIT/bm-20240713-pythonperf1-amd64-python-a2bec77d25b11f50362a-3.14.0a0-a2bec77.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 97.72% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: unknown