# Results vs. 3.13.0

- fork: brandtbucher
- ref: justin_mcmodel
- machine: linux-aarch64
- commit hash: d19b26c
- commit date: 2024-07-14
- overall geometric mean: 1.08x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x slower
- Memory change: 1.09x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 361 ms: 1.18x slower                                                    |
| docutils       | 2.91 sec                                                 | 3.55 sec: 1.22x slower                                                  |
| html5lib       | 64.3 ms                                                  | 71.2 ms: 1.11x slower                                                   |
| tornado_http   | 131 ms                                                   | 144 ms: 1.10x slower                                                    |
| Geometric mean | (ref)                                                    | 1.15x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 541 ms: 1.20x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 408 ms: 1.15x faster                                                    |
| async_tree_none            | 493 ms                                                   | 447 ms: 1.10x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 587 ms: 1.07x faster                                                    |
| async_tree_io              | 1.11 sec                                                 | 1.07 sec: 1.04x faster                                                  |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 737 ms: 1.04x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 722 ms: 1.02x faster                                                    |
| asyncio_websockets         | 656 ms                                                   | 662 ms: 1.01x slower                                                    |
| async_generators           | 496 ms                                                   | 511 ms: 1.03x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.26 sec: 1.04x slower                                                  |
| coroutines                 | 28.2 ms                                                  | 29.6 ms: 1.05x slower                                                   |
| asyncio_tcp                | 568 ms                                                   | 625 ms: 1.10x slower                                                    |
| Geometric mean             | (ref)                                                    | 1.03x faster                                                            |

Benchmark hidden because not significant (1): async_tree_io_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 89.2 ms: 1.06x faster                                                   |
| nbody          | 114 ms                                                   | 122 ms: 1.07x slower                                                    |
| Geometric mean | (ref)                                                    | 1.00x slower                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.5 ms                                                  | 30.3 ms: 1.04x faster                                                   |
| regex_effbot   | 4.87 ms                                                  | 4.95 ms: 1.02x slower                                                   |
| regex_dna      | 246 ms                                                   | 259 ms: 1.05x slower                                                    |
| regex_compile  | 128 ms                                                   | 175 ms: 1.36x slower                                                    |
| Geometric mean | (ref)                                                    | 1.09x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| tomli_loads          | 2.53 sec                                                 | 2.67 sec: 1.06x slower                                                  |
| json_loads           | 31.4 us                                                  | 33.2 us: 1.06x slower                                                   |
| unpickle_pure_python | 254 us                                                   | 279 us: 1.10x slower                                                    |
| pickle_pure_python   | 359 us                                                   | 411 us: 1.14x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.04x slower                                                            |

Benchmark hidden because not significant (4): xml_etree_generate, xml_etree_process, xml_etree_parse, json_dumps

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                   |
| python_startup_no_site | 8.75 ms                                                  | 8.90 ms: 1.02x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.00x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 13.4 ms: 1.01x slower                                                   |
| django_template | 42.3 ms                                                  | 50.3 ms: 1.19x slower                                                   |
| genshi_text     | 27.7 ms                                                  | 34.6 ms: 1.25x slower                                                   |
| genshi_xml      | 60.2 ms                                                  | 84.9 ms: 1.41x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.21x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 51.0 us                                                  | 38.8 us: 1.32x faster                                                   |
| async_tree_memoization_tg  | 649 ms                                                   | 541 ms: 1.20x faster                                                    |
| deepcopy                   | 451 us                                                   | 382 us: 1.18x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 408 ms: 1.15x faster                                                    |
| async_tree_none            | 493 ms                                                   | 447 ms: 1.10x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 587 ms: 1.07x faster                                                    |
| float                      | 94.4 ms                                                  | 89.2 ms: 1.06x faster                                                   |
| regex_v8                   | 31.5 ms                                                  | 30.3 ms: 1.04x faster                                                   |
| async_tree_io              | 1.11 sec                                                 | 1.07 sec: 1.04x faster                                                  |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 737 ms: 1.04x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 722 ms: 1.02x faster                                                    |
| python_startup             | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                   |
| mako                       | 13.3 ms                                                  | 13.4 ms: 1.01x slower                                                   |
| xml_etree_iterparse        | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| asyncio_websockets         | 656 ms                                                   | 662 ms: 1.01x slower                                                    |
| coverage                   | 98.5 ms                                                  | 99.7 ms: 1.01x slower                                                   |
| generators                 | 37.8 ms                                                  | 38.3 ms: 1.01x slower                                                   |
| regex_effbot               | 4.87 ms                                                  | 4.95 ms: 1.02x slower                                                   |
| python_startup_no_site     | 8.75 ms                                                  | 8.90 ms: 1.02x slower                                                   |
| mdp                        | 3.36 sec                                                 | 3.43 sec: 1.02x slower                                                  |
| bpe_tokeniser              | 5.90 sec                                                 | 6.02 sec: 1.02x slower                                                  |
| richards                   | 53.5 ms                                                  | 54.8 ms: 1.02x slower                                                   |
| richards_super             | 60.3 ms                                                  | 62.1 ms: 1.03x slower                                                   |
| async_generators           | 496 ms                                                   | 511 ms: 1.03x slower                                                    |
| logging_simple             | 7.04 us                                                  | 7.28 us: 1.03x slower                                                   |
| thrift                     | 946 us                                                   | 978 us: 1.03x slower                                                    |
| deepcopy_reduce            | 4.07 us                                                  | 4.21 us: 1.03x slower                                                   |
| json                       | 5.61 ms                                                  | 5.81 ms: 1.03x slower                                                   |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.26 sec: 1.04x slower                                                  |
| meteor_contest             | 113 ms                                                   | 118 ms: 1.04x slower                                                    |
| fannkuch                   | 452 ms                                                   | 474 ms: 1.05x slower                                                    |
| coroutines                 | 28.2 ms                                                  | 29.6 ms: 1.05x slower                                                   |
| regex_dna                  | 246 ms                                                   | 259 ms: 1.05x slower                                                    |
| logging_silent             | 135 ns                                                   | 142 ns: 1.05x slower                                                    |
| scimark_fft                | 447 ms                                                   | 472 ms: 1.05x slower                                                    |
| bench_thread_pool          | 1.28 ms                                                  | 1.35 ms: 1.05x slower                                                   |
| tomli_loads                | 2.53 sec                                                 | 2.67 sec: 1.06x slower                                                  |
| json_loads                 | 31.4 us                                                  | 33.2 us: 1.06x slower                                                   |
| spectral_norm              | 141 ms                                                   | 149 ms: 1.06x slower                                                    |
| scimark_sparse_mat_mult    | 6.58 ms                                                  | 7.00 ms: 1.06x slower                                                   |
| nbody                      | 114 ms                                                   | 122 ms: 1.07x slower                                                    |
| telco                      | 9.73 ms                                                  | 10.4 ms: 1.07x slower                                                   |
| pycparser                  | 1.26 sec                                                 | 1.36 sec: 1.08x slower                                                  |
| typing_runtime_protocols   | 192 us                                                   | 208 us: 1.08x slower                                                    |
| crypto_pyaes               | 82.7 ms                                                  | 90.0 ms: 1.09x slower                                                   |
| raytrace                   | 298 ms                                                   | 325 ms: 1.09x slower                                                    |
| scimark_monte_carlo        | 83.8 ms                                                  | 91.5 ms: 1.09x slower                                                   |
| tornado_http               | 131 ms                                                   | 144 ms: 1.10x slower                                                    |
| unpickle_pure_python       | 254 us                                                   | 279 us: 1.10x slower                                                    |
| sqlglot_optimize           | 62.4 ms                                                  | 68.5 ms: 1.10x slower                                                   |
| asyncio_tcp                | 568 ms                                                   | 625 ms: 1.10x slower                                                    |
| go                         | 163 ms                                                   | 179 ms: 1.10x slower                                                    |
| html5lib                   | 64.3 ms                                                  | 71.2 ms: 1.11x slower                                                   |
| gc_traversal               | 4.53 ms                                                  | 5.04 ms: 1.11x slower                                                   |
| scimark_sor                | 159 ms                                                   | 177 ms: 1.11x slower                                                    |
| dask                       | 350 ms                                                   | 390 ms: 1.11x slower                                                    |
| sqlglot_normalize          | 128 ms                                                   | 143 ms: 1.11x slower                                                    |
| create_gc_cycles           | 2.12 ms                                                  | 2.37 ms: 1.12x slower                                                   |
| pyflate                    | 556 ms                                                   | 621 ms: 1.12x slower                                                    |
| pickle_pure_python         | 359 us                                                   | 411 us: 1.14x slower                                                    |
| deltablue                  | 3.85 ms                                                  | 4.47 ms: 1.16x slower                                                   |
| comprehensions             | 20.4 us                                                  | 23.7 us: 1.16x slower                                                   |
| sqlglot_parse              | 1.38 ms                                                  | 1.61 ms: 1.17x slower                                                   |
| sqlglot_transpile          | 1.73 ms                                                  | 2.04 ms: 1.18x slower                                                   |
| 2to3                       | 306 ms                                                   | 361 ms: 1.18x slower                                                    |
| pylint                     | 343 ms                                                   | 408 ms: 1.19x slower                                                    |
| nqueens                    | 98.7 ms                                                  | 117 ms: 1.19x slower                                                    |
| django_template            | 42.3 ms                                                  | 50.3 ms: 1.19x slower                                                   |
| sympy_expand               | 455 ms                                                   | 542 ms: 1.19x slower                                                    |
| docutils                   | 2.91 sec                                                 | 3.55 sec: 1.22x slower                                                  |
| sympy_str                  | 264 ms                                                   | 323 ms: 1.23x slower                                                    |
| hexiom                     | 7.13 ms                                                  | 8.81 ms: 1.23x slower                                                   |
| genshi_text                | 27.7 ms                                                  | 34.6 ms: 1.25x slower                                                   |
| sympy_integrate            | 21.0 ms                                                  | 26.3 ms: 1.25x slower                                                   |
| pprint_safe_repr           | 926 ms                                                   | 1.16 sec: 1.26x slower                                                  |
| sympy_sum                  | 143 ms                                                   | 183 ms: 1.27x slower                                                    |
| pprint_pformat             | 1.90 sec                                                 | 2.42 sec: 1.27x slower                                                  |
| scimark_lu                 | 139 ms                                                   | 184 ms: 1.32x slower                                                    |
| chaos                      | 68.8 ms                                                  | 93.3 ms: 1.36x slower                                                   |
| regex_compile              | 128 ms                                                   | 175 ms: 1.36x slower                                                    |
| genshi_xml                 | 60.2 ms                                                  | 84.9 ms: 1.41x slower                                                   |
| bench_mp_pool              | 7.30 ms                                                  | 13.2 ms: 1.81x slower                                                   |
| Geometric mean             | (ref)                                                    | 1.08x slower                                                            |

Benchmark hidden because not significant (8): xml_etree_generate, xml_etree_process, pidigits, xml_etree_parse, pathlib, logging_format, json_dumps, async_tree_io_tg
Ignored benchmarks (12) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (1) of results/bm-20240714-3.14.0a0-d19b26c-JIT/bm-20240714-arminc-aarch64-brandtbucher-justin_mcmodel-3.14.0a0-d19b26c.json: dulwich_log

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.04x
- 99% likely to have a slowdown of 1.03x

# Memory
- memory change: 1.09x