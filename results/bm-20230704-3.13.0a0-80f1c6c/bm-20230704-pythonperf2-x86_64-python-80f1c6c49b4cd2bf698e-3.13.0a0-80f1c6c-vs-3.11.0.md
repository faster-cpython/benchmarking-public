
# Results vs. 3.11.0

- fork: python
- ref: 80f1c6c49b4cd2bf698e
- machine: linux-x86_64
- commit hash: 80f1c6c
- commit date: 2023-07-04
- overall geometric mean: 1.04x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| docutils       | 2.86 sec                                                     | 2.92 sec: 1.02x slower                                                      |
| tornado_http   | 122 ms                                                       | 123 ms: 1.02x slower                                                        |
| Geometric mean | (ref)                                                        | 1.02x slower                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 90.7 ms                                                      | 85.6 ms: 1.06x faster                                                       |
| pidigits       | 251 ms                                                       | 260 ms: 1.04x slower                                                        |
| float          | 74.2 ms                                                      | 81.9 ms: 1.10x slower                                                       |
| Geometric mean | (ref)                                                        | 1.03x slower                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 158 ms                                                       | 151 ms: 1.04x faster                                                        |
| regex_effbot   | 3.50 ms                                                      | 3.38 ms: 1.03x faster                                                       |
| regex_v8       | 23.9 ms                                                      | 23.2 ms: 1.03x faster                                                       |
| regex_dna      | 227 ms                                                       | 237 ms: 1.04x slower                                                        |
| Geometric mean | (ref)                                                        | 1.02x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_dumps           | 13.4 ms                                                      | 10.4 ms: 1.29x faster                                                       |
| json_loads           | 28.7 us                                                      | 24.5 us: 1.17x faster                                                       |
| unpickle_pure_python | 241 us                                                       | 219 us: 1.10x faster                                                        |
| xml_etree_parse      | 158 ms                                                       | 148 ms: 1.07x faster                                                        |
| pickle_dict          | 30.8 us                                                      | 31.2 us: 1.01x slower                                                       |
| xml_etree_iterparse  | 104 ms                                                       | 107 ms: 1.02x slower                                                        |
| tomli_loads          | 2.26 sec                                                     | 2.32 sec: 1.03x slower                                                      |
| pickle_pure_python   | 319 us                                                       | 330 us: 1.03x slower                                                        |
| unpickle_list        | 4.53 us                                                      | 4.71 us: 1.04x slower                                                       |
| xml_etree_process    | 56.5 ms                                                      | 59.7 ms: 1.06x slower                                                       |
| pickle               | 9.64 us                                                      | 10.2 us: 1.06x slower                                                       |
| unpickle             | 13.4 us                                                      | 14.4 us: 1.08x slower                                                       |
| xml_etree_generate   | 80.5 ms                                                      | 86.9 ms: 1.08x slower                                                       |
| pickle_list          | 3.83 us                                                      | 4.33 us: 1.13x slower                                                       |
| Geometric mean       | (ref)                                                        | 1.00x faster                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 10.8 ms                                                      | 11.3 ms: 1.05x slower                                                       |
| python_startup_no_site | 7.76 ms                                                      | 8.37 ms: 1.08x slower                                                       |
| Geometric mean         | (ref)                                                        | 1.07x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|-----------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako      | 11.0 ms                                                      | 10.3 ms: 1.06x faster                                                       |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230704-pythonperf2-x86_64-python-80f1c6c49b4cd2bf698e-3.13.0a0-80f1c6c |
|--------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| typing_runtime_protocols | 523 us                                                       | 149 us: 3.51x faster                                                        |
| asyncio_tcp              | 753 ms                                                       | 382 ms: 1.97x faster                                                        |
| asyncio_tcp_ssl          | 3.08 sec                                                     | 1.57 sec: 1.96x faster                                                      |
| generators               | 56.0 ms                                                      | 36.4 ms: 1.54x faster                                                       |
| chaos                    | 80.9 ms                                                      | 61.6 ms: 1.31x faster                                                       |
| json_dumps               | 13.4 ms                                                      | 10.4 ms: 1.29x faster                                                       |
| mypy2                    | 451 ms                                                       | 370 ms: 1.22x faster                                                        |
| coroutines               | 27.6 ms                                                      | 22.9 ms: 1.20x faster                                                       |
| json_loads               | 28.7 us                                                      | 24.5 us: 1.17x faster                                                       |
| raytrace                 | 317 ms                                                       | 278 ms: 1.14x faster                                                        |
| nqueens                  | 103 ms                                                       | 91.2 ms: 1.13x faster                                                       |
| deltablue                | 4.00 ms                                                      | 3.55 ms: 1.13x faster                                                       |
| async_tree_memoization   | 630 ms                                                       | 560 ms: 1.13x faster                                                        |
| scimark_lu               | 115 ms                                                       | 102 ms: 1.12x faster                                                        |
| async_tree_none          | 519 ms                                                       | 465 ms: 1.12x faster                                                        |
| comprehensions           | 24.6 us                                                      | 22.2 us: 1.11x faster                                                       |
| json                     | 5.65 ms                                                      | 5.13 ms: 1.10x faster                                                       |
| unpickle_pure_python     | 241 us                                                       | 219 us: 1.10x faster                                                        |
| async_tree_io            | 1.17 sec                                                     | 1.08 sec: 1.09x faster                                                      |
| fannkuch                 | 429 ms                                                       | 394 ms: 1.09x faster                                                        |
| hexiom                   | 7.13 ms                                                      | 6.57 ms: 1.09x faster                                                       |
| logging_format           | 8.11 us                                                      | 7.51 us: 1.08x faster                                                       |
| mdp                      | 2.75 sec                                                     | 2.55 sec: 1.08x faster                                                      |
| gc_traversal             | 3.85 ms                                                      | 3.59 ms: 1.07x faster                                                       |
| xml_etree_parse          | 158 ms                                                       | 148 ms: 1.07x faster                                                        |
| mako                     | 11.0 ms                                                      | 10.3 ms: 1.06x faster                                                       |
| nbody                    | 90.7 ms                                                      | 85.6 ms: 1.06x faster                                                       |
| sqlglot_normalize        | 126 ms                                                       | 119 ms: 1.06x faster                                                        |
| bench_thread_pool        | 1.01 ms                                                      | 960 us: 1.05x faster                                                        |
| logging_silent           | 101 ns                                                       | 96.0 ns: 1.05x faster                                                       |
| deepcopy                 | 399 us                                                       | 382 us: 1.05x faster                                                        |
| async_tree_cpu_io_mixed  | 749 ms                                                       | 717 ms: 1.04x faster                                                        |
| richards_super           | 61.0 ms                                                      | 58.5 ms: 1.04x faster                                                       |
| regex_compile            | 158 ms                                                       | 151 ms: 1.04x faster                                                        |
| logging_simple           | 7.19 us                                                      | 6.92 us: 1.04x faster                                                       |
| regex_effbot             | 3.50 ms                                                      | 3.38 ms: 1.03x faster                                                       |
| sqlglot_parse            | 1.53 ms                                                      | 1.48 ms: 1.03x faster                                                       |
| regex_v8                 | 23.9 ms                                                      | 23.2 ms: 1.03x faster                                                       |
| meteor_contest           | 131 ms                                                       | 129 ms: 1.02x faster                                                        |
| deepcopy_reduce          | 3.51 us                                                      | 3.47 us: 1.01x faster                                                       |
| crypto_pyaes             | 83.4 ms                                                      | 82.6 ms: 1.01x faster                                                       |
| deepcopy_memo            | 38.8 us                                                      | 38.4 us: 1.01x faster                                                       |
| sqlglot_transpile        | 1.92 ms                                                      | 1.90 ms: 1.01x faster                                                       |
| dulwich_log              | 68.4 ms                                                      | 68.0 ms: 1.01x faster                                                       |
| sqlglot_optimize         | 59.8 ms                                                      | 59.5 ms: 1.00x faster                                                       |
| spectral_norm            | 93.3 ms                                                      | 94.4 ms: 1.01x slower                                                       |
| pickle_dict              | 30.8 us                                                      | 31.2 us: 1.01x slower                                                       |
| tornado_http             | 122 ms                                                       | 123 ms: 1.02x slower                                                        |
| xml_etree_iterparse      | 104 ms                                                       | 107 ms: 1.02x slower                                                        |
| pathlib                  | 19.1 ms                                                      | 19.5 ms: 1.02x slower                                                       |
| docutils                 | 2.86 sec                                                     | 2.92 sec: 1.02x slower                                                      |
| tomli_loads              | 2.26 sec                                                     | 2.32 sec: 1.03x slower                                                      |
| create_gc_cycles         | 1.61 ms                                                      | 1.66 ms: 1.03x slower                                                       |
| coverage                 | 84.8 ms                                                      | 87.5 ms: 1.03x slower                                                       |
| pickle_pure_python       | 319 us                                                       | 330 us: 1.03x slower                                                        |
| pidigits                 | 251 ms                                                       | 260 ms: 1.04x slower                                                        |
| unpickle_list            | 4.53 us                                                      | 4.71 us: 1.04x slower                                                       |
| pprint_pformat           | 1.63 sec                                                     | 1.70 sec: 1.04x slower                                                      |
| regex_dna                | 227 ms                                                       | 237 ms: 1.04x slower                                                        |
| python_startup           | 10.8 ms                                                      | 11.3 ms: 1.05x slower                                                       |
| xml_etree_process        | 56.5 ms                                                      | 59.7 ms: 1.06x slower                                                       |
| pickle                   | 9.64 us                                                      | 10.2 us: 1.06x slower                                                       |
| pprint_safe_repr         | 784 ms                                                       | 836 ms: 1.07x slower                                                        |
| go                       | 164 ms                                                       | 176 ms: 1.07x slower                                                        |
| unpickle                 | 13.4 us                                                      | 14.4 us: 1.08x slower                                                       |
| xml_etree_generate       | 80.5 ms                                                      | 86.9 ms: 1.08x slower                                                       |
| python_startup_no_site   | 7.76 ms                                                      | 8.37 ms: 1.08x slower                                                       |
| scimark_monte_carlo      | 68.2 ms                                                      | 74.0 ms: 1.09x slower                                                       |
| richards                 | 48.3 ms                                                      | 52.6 ms: 1.09x slower                                                       |
| sqlite_synth             | 2.50 us                                                      | 2.74 us: 1.10x slower                                                       |
| scimark_fft              | 285 ms                                                       | 313 ms: 1.10x slower                                                        |
| float                    | 74.2 ms                                                      | 81.9 ms: 1.10x slower                                                       |
| scimark_sparse_mat_mult  | 4.05 ms                                                      | 4.47 ms: 1.10x slower                                                       |
| pickle_list              | 3.83 us                                                      | 4.33 us: 1.13x slower                                                       |
| telco                    | 6.86 ms                                                      | 7.79 ms: 1.14x slower                                                       |
| bench_mp_pool            | 4.62 ms                                                      | 5.29 ms: 1.14x slower                                                       |
| pyflate                  | 449 ms                                                       | 520 ms: 1.16x slower                                                        |
| unpack_sequence          | 45.6 ns                                                      | 56.7 ns: 1.24x slower                                                       |
| async_generators         | 316 ms                                                       | 400 ms: 1.27x slower                                                        |
| scimark_sor              | 111 ms                                                       | 147 ms: 1.32x slower                                                        |
| dask                     | 410 ms                                                       | 593 ms: 1.45x slower                                                        |
| Geometric mean           | (ref)                                                        | 1.04x faster                                                                |

Benchmark hidden because not significant (1): pycparser
Ignored benchmarks (17) of results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
