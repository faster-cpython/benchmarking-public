# Results vs. 3.12.0

- fork: brandtbucher
- ref: underflow_no_yield
- machine: linux-x86_64
- commit hash: ae6f199
- commit date: 2024-07-08
- overall geometric mean: 1.08x faster
- HPT reliability: 99.83%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.05x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| 2to3           | 274 ms                                                 | 275 ms: 1.00x slower                                                      |
| docutils       | 2.77 sec                                               | 3.00 sec: 1.08x slower                                                    |
| tornado_http   | 103 ms                                                 | 96.8 ms: 1.06x faster                                                     |
| Geometric mean | (ref)                                                  | 1.01x slower                                                              |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 575 ms                                                 | 378 ms: 1.52x faster                                                      |
| async_tree_none_tg         | 450 ms                                                 | 298 ms: 1.51x faster                                                      |
| async_tree_none            | 472 ms                                                 | 324 ms: 1.46x faster                                                      |
| async_tree_memoization     | 578 ms                                                 | 411 ms: 1.40x faster                                                      |
| async_tree_io_tg           | 1.18 sec                                               | 842 ms: 1.40x faster                                                      |
| async_tree_io              | 1.16 sec                                               | 834 ms: 1.39x faster                                                      |
| async_tree_cpu_io_mixed_tg | 726 ms                                                 | 543 ms: 1.34x faster                                                      |
| async_tree_cpu_io_mixed    | 726 ms                                                 | 568 ms: 1.28x faster                                                      |
| Geometric mean             | (ref)                                                  | 1.41x faster                                                              |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| float          | 84.7 ms                                                | 69.5 ms: 1.22x faster                                                     |
| nbody          | 97.0 ms                                                | 81.0 ms: 1.20x faster                                                     |
| pidigits       | 187 ms                                                 | 187 ms: 1.00x slower                                                      |
| Geometric mean | (ref)                                                  | 1.13x faster                                                              |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| regex_compile  | 148 ms                                                 | 140 ms: 1.06x faster                                                      |
| regex_effbot   | 3.61 ms                                                | 3.70 ms: 1.02x slower                                                     |
| regex_v8       | 23.1 ms                                                | 24.7 ms: 1.07x slower                                                     |
| regex_dna      | 212 ms                                                 | 232 ms: 1.09x slower                                                      |
| Geometric mean | (ref)                                                  | 1.03x slower                                                              |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| tomli_loads          | 2.33 sec                                               | 2.03 sec: 1.15x faster                                                    |
| unpickle_pure_python | 230 us                                                 | 206 us: 1.11x faster                                                      |
| pickle_pure_python   | 324 us                                                 | 295 us: 1.10x faster                                                      |
| xml_etree_generate   | 89.2 ms                                                | 82.2 ms: 1.08x faster                                                     |
| xml_etree_iterparse  | 107 ms                                                 | 99.7 ms: 1.07x faster                                                     |
| xml_etree_parse      | 159 ms                                                 | 149 ms: 1.07x faster                                                      |
| xml_etree_process    | 61.7 ms                                                | 57.9 ms: 1.07x faster                                                     |
| json_loads           | 28.5 us                                                | 27.8 us: 1.02x faster                                                     |
| json_dumps           | 10.4 ms                                                | 10.3 ms: 1.01x faster                                                     |
| Geometric mean       | (ref)                                                  | 1.07x faster                                                              |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| python_startup_no_site | 6.94 ms                                                | 7.10 ms: 1.02x slower                                                     |
| python_startup         | 9.55 ms                                                | 10.5 ms: 1.10x slower                                                     |
| Geometric mean         | (ref)                                                  | 1.06x slower                                                              |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|-----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| mako            | 11.9 ms                                                | 9.81 ms: 1.21x faster                                                     |
| django_template | 34.6 ms                                                | 36.9 ms: 1.07x slower                                                     |
| Geometric mean  | (ref)                                                  | 1.07x faster                                                              |

All benchmarks:
===============

| Benchmark                  | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199 |
|----------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 575 ms                                                 | 378 ms: 1.52x faster                                                      |
| async_tree_none_tg         | 450 ms                                                 | 298 ms: 1.51x faster                                                      |
| async_tree_none            | 472 ms                                                 | 324 ms: 1.46x faster                                                      |
| deepcopy_memo              | 40.7 us                                                | 28.8 us: 1.41x faster                                                     |
| async_tree_memoization     | 578 ms                                                 | 411 ms: 1.40x faster                                                      |
| async_tree_io_tg           | 1.18 sec                                               | 842 ms: 1.40x faster                                                      |
| async_tree_io              | 1.16 sec                                               | 834 ms: 1.39x faster                                                      |
| deepcopy                   | 371 us                                                 | 270 us: 1.37x faster                                                      |
| async_tree_cpu_io_mixed_tg | 726 ms                                                 | 543 ms: 1.34x faster                                                      |
| scimark_monte_carlo        | 75.1 ms                                                | 57.4 ms: 1.31x faster                                                     |
| comprehensions             | 21.8 us                                                | 16.6 us: 1.31x faster                                                     |
| async_tree_cpu_io_mixed    | 726 ms                                                 | 568 ms: 1.28x faster                                                      |
| float                      | 84.7 ms                                                | 69.5 ms: 1.22x faster                                                     |
| deepcopy_reduce            | 3.35 us                                                | 2.75 us: 1.22x faster                                                     |
| mako                       | 11.9 ms                                                | 9.81 ms: 1.21x faster                                                     |
| scimark_fft                | 382 ms                                                 | 316 ms: 1.21x faster                                                      |
| pathlib                    | 19.3 ms                                                | 16.0 ms: 1.21x faster                                                     |
| crypto_pyaes               | 81.9 ms                                                | 67.8 ms: 1.21x faster                                                     |
| nbody                      | 97.0 ms                                                | 81.0 ms: 1.20x faster                                                     |
| logging_format             | 7.23 us                                                | 6.15 us: 1.18x faster                                                     |
| scimark_sparse_mat_mult    | 5.06 ms                                                | 4.33 ms: 1.17x faster                                                     |
| raytrace                   | 312 ms                                                 | 269 ms: 1.16x faster                                                      |
| fannkuch                   | 417 ms                                                 | 362 ms: 1.15x faster                                                      |
| logging_simple             | 6.46 us                                                | 5.61 us: 1.15x faster                                                     |
| tomli_loads                | 2.33 sec                                               | 2.03 sec: 1.15x faster                                                    |
| chaos                      | 67.0 ms                                                | 58.8 ms: 1.14x faster                                                     |
| spectral_norm              | 115 ms                                                 | 102 ms: 1.13x faster                                                      |
| unpickle_pure_python       | 230 us                                                 | 206 us: 1.11x faster                                                      |
| pickle_pure_python         | 324 us                                                 | 295 us: 1.10x faster                                                      |
| pprint_safe_repr           | 776 ms                                                 | 707 ms: 1.10x faster                                                      |
| xml_etree_generate         | 89.2 ms                                                | 82.2 ms: 1.08x faster                                                     |
| richards_super             | 51.5 ms                                                | 47.6 ms: 1.08x faster                                                     |
| pyflate                    | 482 ms                                                 | 448 ms: 1.08x faster                                                      |
| xml_etree_iterparse        | 107 ms                                                 | 99.7 ms: 1.07x faster                                                     |
| xml_etree_parse            | 159 ms                                                 | 149 ms: 1.07x faster                                                      |
| pprint_pformat             | 1.57 sec                                               | 1.47 sec: 1.07x faster                                                    |
| gc_traversal               | 3.79 ms                                                | 3.55 ms: 1.07x faster                                                     |
| xml_etree_process          | 61.7 ms                                                | 57.9 ms: 1.07x faster                                                     |
| logging_silent             | 104 ns                                                 | 98.1 ns: 1.07x faster                                                     |
| regex_compile              | 148 ms                                                 | 140 ms: 1.06x faster                                                      |
| richards                   | 45.8 ms                                                | 43.1 ms: 1.06x faster                                                     |
| tornado_http               | 103 ms                                                 | 96.8 ms: 1.06x faster                                                     |
| sqlglot_parse              | 1.36 ms                                                | 1.30 ms: 1.05x faster                                                     |
| meteor_contest             | 112 ms                                                 | 108 ms: 1.04x faster                                                      |
| generators                 | 31.2 ms                                                | 30.0 ms: 1.04x faster                                                     |
| sqlglot_transpile          | 1.68 ms                                                | 1.62 ms: 1.04x faster                                                     |
| pycparser                  | 1.18 sec                                               | 1.15 sec: 1.03x faster                                                    |
| async_generators           | 463 ms                                                 | 452 ms: 1.02x faster                                                      |
| json_loads                 | 28.5 us                                                | 27.8 us: 1.02x faster                                                     |
| json                       | 5.26 ms                                                | 5.19 ms: 1.01x faster                                                     |
| json_dumps                 | 10.4 ms                                                | 10.3 ms: 1.01x faster                                                     |
| dulwich_log                | 68.5 ms                                                | 68.2 ms: 1.00x faster                                                     |
| pidigits                   | 187 ms                                                 | 187 ms: 1.00x slower                                                      |
| 2to3                       | 274 ms                                                 | 275 ms: 1.00x slower                                                      |
| bench_thread_pool          | 842 us                                                 | 845 us: 1.00x slower                                                      |
| sympy_str                  | 300 ms                                                 | 302 ms: 1.01x slower                                                      |
| asyncio_tcp_ssl            | 1.79 sec                                               | 1.80 sec: 1.01x slower                                                    |
| asyncio_websockets         | 551 ms                                                 | 558 ms: 1.01x slower                                                      |
| sqlglot_optimize           | 54.8 ms                                                | 55.6 ms: 1.01x slower                                                     |
| coroutines                 | 23.2 ms                                                | 23.5 ms: 1.01x slower                                                     |
| scimark_sor                | 129 ms                                                 | 131 ms: 1.02x slower                                                      |
| python_startup_no_site     | 6.94 ms                                                | 7.10 ms: 1.02x slower                                                     |
| regex_effbot               | 3.61 ms                                                | 3.70 ms: 1.02x slower                                                     |
| asyncio_tcp                | 491 ms                                                 | 504 ms: 1.03x slower                                                      |
| sqlglot_normalize          | 110 ms                                                 | 114 ms: 1.03x slower                                                      |
| go                         | 139 ms                                                 | 145 ms: 1.04x slower                                                      |
| nqueens                    | 83.3 ms                                                | 86.5 ms: 1.04x slower                                                     |
| mdp                        | 2.63 sec                                               | 2.74 sec: 1.04x slower                                                    |
| hexiom                     | 6.41 ms                                                | 6.68 ms: 1.04x slower                                                     |
| sympy_integrate            | 21.4 ms                                                | 22.5 ms: 1.05x slower                                                     |
| scimark_lu                 | 118 ms                                                 | 124 ms: 1.05x slower                                                      |
| django_template            | 34.6 ms                                                | 36.9 ms: 1.07x slower                                                     |
| regex_v8                   | 23.1 ms                                                | 24.7 ms: 1.07x slower                                                     |
| sympy_expand               | 478 ms                                                 | 512 ms: 1.07x slower                                                      |
| docutils                   | 2.77 sec                                               | 3.00 sec: 1.08x slower                                                    |
| typing_runtime_protocols   | 158 us                                                 | 172 us: 1.09x slower                                                      |
| regex_dna                  | 212 ms                                                 | 232 ms: 1.09x slower                                                      |
| python_startup             | 9.55 ms                                                | 10.5 ms: 1.10x slower                                                     |
| telco                      | 7.10 ms                                                | 7.89 ms: 1.11x slower                                                     |
| deltablue                  | 3.72 ms                                                | 4.20 ms: 1.13x slower                                                     |
| create_gc_cycles           | 1.48 ms                                                | 1.74 ms: 1.18x slower                                                     |
| coverage                   | 72.7 ms                                                | 92.6 ms: 1.27x slower                                                     |
| Geometric mean             | (ref)                                                  | 1.08x faster                                                              |

Benchmark hidden because not significant (3): dask, sympy_sum, bench_mp_pool
Ignored benchmarks (13) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20240708-3.14.0a0-ae6f199-JIT/bm-20240708-linux-x86_64-brandtbucher-underflow_no_yield-3.14.0a0-ae6f199.json: bpe_tokeniser, genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 99.83% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.05x