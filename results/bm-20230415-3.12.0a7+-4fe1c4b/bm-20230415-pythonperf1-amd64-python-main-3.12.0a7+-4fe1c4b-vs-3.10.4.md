
# Results vs. 3.10.4

- fork: python
- ref: main
- machine: windows-amd64
- commit hash: 4fe1c4b
- commit date: 2023-04-15
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.18x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 198 ms: 1.20x faster                                        |
| chameleon      | 5.92 ms                                                     | 4.67 ms: 1.27x faster                                       |
| docutils       | 1.89 sec                                                    | 1.49 sec: 1.27x faster                                      |
| html5lib       | 46.5 ms                                                     | 35.4 ms: 1.31x faster                                       |
| tornado_http   | 109 ms                                                      | 85.4 ms: 1.28x faster                                       |
| Geometric mean | (ref)                                                       | 1.26x faster                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| float          | 60.2 ms                                                     | 48.3 ms: 1.25x faster                                       |
| nbody          | 69.3 ms                                                     | 57.8 ms: 1.20x faster                                       |
| pidigits       | 145 ms                                                      | 146 ms: 1.00x slower                                        |
| Geometric mean | (ref)                                                       | 1.14x faster                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 80.9 ms: 1.28x faster                                       |
| regex_dna      | 132 ms                                                      | 119 ms: 1.11x faster                                        |
| regex_v8       | 15.0 ms                                                     | 14.0 ms: 1.07x faster                                       |
| regex_effbot   | 1.66 ms                                                     | 1.56 ms: 1.07x faster                                       |
| Geometric mean | (ref)                                                       | 1.13x faster                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|----------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.35 ms: 1.59x faster                                       |
| pickle_pure_python   | 257 us                                                      | 183 us: 1.40x faster                                        |
| unpickle_pure_python | 171 us                                                      | 127 us: 1.35x faster                                        |
| xml_etree_process    | 43.4 ms                                                     | 36.1 ms: 1.20x faster                                       |
| xml_etree_parse      | 102 ms                                                      | 89.2 ms: 1.14x faster                                       |
| unpickle             | 9.17 us                                                     | 8.08 us: 1.14x faster                                       |
| json_loads           | 14.2 us                                                     | 12.7 us: 1.12x faster                                       |
| xml_etree_iterparse  | 63.5 ms                                                     | 60.1 ms: 1.06x faster                                       |
| unpickle_list        | 2.81 us                                                     | 2.68 us: 1.05x faster                                       |
| xml_etree_generate   | 54.5 ms                                                     | 52.7 ms: 1.03x faster                                       |
| pickle               | 6.80 us                                                     | 6.68 us: 1.02x faster                                       |
| pickle_list          | 2.59 us                                                     | 2.82 us: 1.09x slower                                       |
| pickle_dict          | 16.9 us                                                     | 18.6 us: 1.10x slower                                       |
| Geometric mean       | (ref)                                                       | 1.13x faster                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| python_startup | 20.0 ms                                                     | 18.1 ms: 1.10x faster                                       |
| Geometric mean | (ref)                                                       | 1.05x faster                                                |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|-----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 6.53 ms: 1.35x faster                                       |
| genshi_text     | 19.0 ms                                                     | 14.3 ms: 1.33x faster                                       |
| django_template | 28.2 ms                                                     | 21.7 ms: 1.30x faster                                       |
| genshi_xml      | 40.5 ms                                                     | 31.5 ms: 1.29x faster                                       |
| Geometric mean  | (ref)                                                       | 1.32x faster                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230415-pythonperf1-amd64-python-main-3.12.0a7+-4fe1c4b |
|-------------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.15 ms: 1.94x faster                                       |
| mypy2                   | 352 ms                                                      | 206 ms: 1.71x faster                                        |
| logging_silent          | 96.4 ns                                                     | 59.7 ns: 1.61x faster                                       |
| sqlglot_parse           | 1.22 ms                                                     | 755 us: 1.61x faster                                        |
| richards                | 41.2 ms                                                     | 25.9 ms: 1.59x faster                                       |
| json_dumps              | 8.50 ms                                                     | 5.35 ms: 1.59x faster                                       |
| generators              | 31.6 ms                                                     | 20.0 ms: 1.58x faster                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 930 us: 1.57x faster                                        |
| raytrace                | 271 ms                                                      | 173 ms: 1.57x faster                                        |
| scimark_lu              | 85.4 ms                                                     | 54.8 ms: 1.56x faster                                       |
| go                      | 136 ms                                                      | 88.8 ms: 1.53x faster                                       |
| asyncio_tcp             | 712 ms                                                      | 471 ms: 1.51x faster                                        |
| scimark_sor             | 105 ms                                                      | 72.5 ms: 1.45x faster                                       |
| async_tree_io           | 1.07 sec                                                    | 749 ms: 1.42x faster                                        |
| pickle_pure_python      | 257 us                                                      | 183 us: 1.40x faster                                        |
| scimark_monte_carlo     | 55.9 ms                                                     | 40.1 ms: 1.39x faster                                       |
| hexiom                  | 5.52 ms                                                     | 3.97 ms: 1.39x faster                                       |
| async_tree_none         | 420 ms                                                      | 303 ms: 1.39x faster                                        |
| async_tree_memoization  | 497 ms                                                      | 360 ms: 1.38x faster                                        |
| crypto_pyaes            | 62.3 ms                                                     | 45.3 ms: 1.38x faster                                       |
| chaos                   | 58.9 ms                                                     | 42.8 ms: 1.38x faster                                       |
| pycparser               | 868 ms                                                      | 639 ms: 1.36x faster                                        |
| unpickle_pure_python    | 171 us                                                      | 127 us: 1.35x faster                                        |
| mako                    | 8.80 ms                                                     | 6.53 ms: 1.35x faster                                       |
| pyflate                 | 387 ms                                                      | 289 ms: 1.34x faster                                        |
| genshi_text             | 19.0 ms                                                     | 14.3 ms: 1.33x faster                                       |
| html5lib                | 46.5 ms                                                     | 35.4 ms: 1.31x faster                                       |
| thrift                  | 615 us                                                      | 471 us: 1.31x faster                                        |
| django_template         | 28.2 ms                                                     | 21.7 ms: 1.30x faster                                       |
| deepcopy_memo           | 28.5 us                                                     | 22.0 us: 1.30x faster                                       |
| genshi_xml              | 40.5 ms                                                     | 31.5 ms: 1.29x faster                                       |
| spectral_norm           | 78.0 ms                                                     | 60.7 ms: 1.29x faster                                       |
| pylint                  | 347 ms                                                      | 270 ms: 1.28x faster                                        |
| regex_compile           | 103 ms                                                      | 80.9 ms: 1.28x faster                                       |
| tornado_http            | 109 ms                                                      | 85.4 ms: 1.28x faster                                       |
| async_tree_cpu_io_mixed | 609 ms                                                      | 478 ms: 1.27x faster                                        |
| docutils                | 1.89 sec                                                    | 1.49 sec: 1.27x faster                                      |
| chameleon               | 5.92 ms                                                     | 4.67 ms: 1.27x faster                                       |
| mdp                     | 1.71 sec                                                    | 1.37 sec: 1.25x faster                                      |
| float                   | 60.2 ms                                                     | 48.3 ms: 1.25x faster                                       |
| sqlglot_optimize        | 39.0 ms                                                     | 31.3 ms: 1.25x faster                                       |
| scimark_fft             | 193 ms                                                      | 157 ms: 1.23x faster                                        |
| pprint_safe_repr        | 589 ms                                                      | 480 ms: 1.23x faster                                        |
| pprint_pformat          | 1.21 sec                                                    | 985 ms: 1.23x faster                                        |
| xml_etree_process       | 43.4 ms                                                     | 36.1 ms: 1.20x faster                                       |
| bench_thread_pool       | 946 us                                                      | 789 us: 1.20x faster                                        |
| nbody                   | 69.3 ms                                                     | 57.8 ms: 1.20x faster                                       |
| 2to3                    | 237 ms                                                      | 198 ms: 1.20x faster                                        |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.24 ms: 1.19x faster                                       |
| dulwich_log             | 47.6 ms                                                     | 40.3 ms: 1.18x faster                                       |
| sqlglot_normalize       | 202 ms                                                      | 172 ms: 1.17x faster                                        |
| sympy_expand            | 315 ms                                                      | 269 ms: 1.17x faster                                        |
| deepcopy                | 255 us                                                      | 219 us: 1.17x faster                                        |
| sympy_integrate         | 14.8 ms                                                     | 12.7 ms: 1.16x faster                                       |
| unpack_sequence         | 37.8 ns                                                     | 32.8 ns: 1.15x faster                                       |
| create_gc_cycles        | 782 us                                                      | 677 us: 1.15x faster                                        |
| nqueens                 | 67.0 ms                                                     | 58.4 ms: 1.15x faster                                       |
| coroutines              | 15.9 ms                                                     | 13.9 ms: 1.14x faster                                       |
| xml_etree_parse         | 102 ms                                                      | 89.2 ms: 1.14x faster                                       |
| json                    | 3.05 ms                                                     | 2.68 ms: 1.14x faster                                       |
| unpickle                | 9.17 us                                                     | 8.08 us: 1.14x faster                                       |
| deepcopy_reduce         | 2.16 us                                                     | 1.91 us: 1.13x faster                                       |
| json_loads              | 14.2 us                                                     | 12.7 us: 1.12x faster                                       |
| sympy_sum               | 104 ms                                                      | 93.7 ms: 1.11x faster                                       |
| regex_dna               | 132 ms                                                      | 119 ms: 1.11x faster                                        |
| sympy_str               | 188 ms                                                      | 170 ms: 1.10x faster                                        |
| comprehensions          | 16.0 us                                                     | 14.5 us: 1.10x faster                                       |
| python_startup          | 20.0 ms                                                     | 18.1 ms: 1.10x faster                                       |
| regex_v8                | 15.0 ms                                                     | 14.0 ms: 1.07x faster                                       |
| regex_effbot            | 1.66 ms                                                     | 1.56 ms: 1.07x faster                                       |
| async_generators        | 224 ms                                                      | 211 ms: 1.06x faster                                        |
| xml_etree_iterparse     | 63.5 ms                                                     | 60.1 ms: 1.06x faster                                       |
| fannkuch                | 258 ms                                                      | 245 ms: 1.05x faster                                        |
| unpickle_list           | 2.81 us                                                     | 2.68 us: 1.05x faster                                       |
| xml_etree_generate      | 54.5 ms                                                     | 52.7 ms: 1.03x faster                                       |
| sqlite_synth            | 1.84 us                                                     | 1.80 us: 1.02x faster                                       |
| meteor_contest          | 72.5 ms                                                     | 71.2 ms: 1.02x faster                                       |
| pickle                  | 6.80 us                                                     | 6.68 us: 1.02x faster                                       |
| telco                   | 3.78 ms                                                     | 3.76 ms: 1.01x faster                                       |
| pidigits                | 145 ms                                                      | 146 ms: 1.00x slower                                        |
| logging_simple          | 6.20 us                                                     | 6.27 us: 1.01x slower                                       |
| logging_format          | 6.66 us                                                     | 6.78 us: 1.02x slower                                       |
| bench_mp_pool           | 60.7 ms                                                     | 64.4 ms: 1.06x slower                                       |
| pathlib                 | 77.4 ms                                                     | 82.3 ms: 1.06x slower                                       |
| gc_traversal            | 1.34 ms                                                     | 1.46 ms: 1.09x slower                                       |
| pickle_list             | 2.59 us                                                     | 2.82 us: 1.09x slower                                       |
| pickle_dict             | 16.9 us                                                     | 18.6 us: 1.10x slower                                       |
| dask                    | 305 ms                                                      | 352 ms: 1.15x slower                                        |
| coverage                | 40.0 ms                                                     | 52.1 ms: 1.30x slower                                       |
| Geometric mean          | (ref)                                                       | 1.22x faster                                                |

Benchmark hidden because not significant (1): python_startup_no_site
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.20x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.18x
