# Results vs. 3.13.0

- fork: python
- ref: 58ce131037ecb34d506a
- machine: linux-x86_64
- commit hash: 58ce131
- commit date: 2024-08-29
- overall geometric mean: 1.033x faster
- HPT reliability: 99.88%
- HPT 99th percentile: 1.00x faster
- Memory change: 0.92x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 267 ms                                                 | 257 ms: 1.04x faster                                                  |
| docutils       | 2.59 sec                                               | 2.66 sec: 1.03x slower                                                |
| html5lib       | 64.2 ms                                                | 63.1 ms: 1.02x faster                                                 |
| tornado_http   | 92.4 ms                                                | 90.1 ms: 1.03x faster                                                 |
| Geometric mean | (ref)                                                  | 1.01x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|---------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_memoization_tg | 464 ms                                                 | 404 ms: 1.15x faster                                                  |
| async_tree_memoization    | 442 ms                                                 | 394 ms: 1.12x faster                                                  |
| async_tree_none           | 351 ms                                                 | 322 ms: 1.09x faster                                                  |
| async_tree_cpu_io_mixed   | 577 ms                                                 | 556 ms: 1.04x faster                                                  |
| async_tree_none_tg        | 321 ms                                                 | 310 ms: 1.03x faster                                                  |
| asyncio_websockets        | 552 ms                                                 | 557 ms: 1.01x slower                                                  |
| coroutines                | 22.7 ms                                                | 23.1 ms: 1.02x slower                                                 |
| async_tree_io_tg          | 857 ms                                                 | 901 ms: 1.05x slower                                                  |
| async_tree_io             | 842 ms                                                 | 930 ms: 1.10x slower                                                  |
| Geometric mean            | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (2): async_tree_cpu_io_mixed_tg, async_generators

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 87.0 ms                                                | 84.1 ms: 1.03x faster                                                 |
| float          | 79.2 ms                                                | 77.5 ms: 1.02x faster                                                 |
| pidigits       | 186 ms                                                 | 187 ms: 1.00x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 132 ms                                                 | 128 ms: 1.03x faster                                                  |
| regex_v8       | 26.2 ms                                                | 26.2 ms: 1.00x faster                                                 |
| regex_dna      | 218 ms                                                 | 220 ms: 1.01x slower                                                  |
| regex_effbot   | 3.73 ms                                                | 3.81 ms: 1.02x slower                                                 |
| Geometric mean | (ref)                                                  | 1.00x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| xml_etree_process    | 60.6 ms                                                | 58.7 ms: 1.03x faster                                                 |
| pickle_pure_python   | 310 us                                                 | 301 us: 1.03x faster                                                  |
| xml_etree_generate   | 86.7 ms                                                | 84.6 ms: 1.03x faster                                                 |
| tomli_loads          | 2.14 sec                                               | 2.09 sec: 1.02x faster                                                |
| unpickle_pure_python | 216 us                                                 | 213 us: 1.01x faster                                                  |
| json_dumps           | 10.6 ms                                                | 10.5 ms: 1.00x faster                                                 |
| xml_etree_iterparse  | 101 ms                                                 | 104 ms: 1.02x slower                                                  |
| json_loads           | 27.2 us                                                | 28.6 us: 1.05x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.01x faster                                                          |

Benchmark hidden because not significant (1): xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 12.5 ms                                                | 10.6 ms: 1.18x faster                                                 |
| python_startup_no_site | 6.96 ms                                                | 7.11 ms: 1.02x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.07x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 35.2 ms                                                | 33.7 ms: 1.04x faster                                                 |
| genshi_xml      | 50.9 ms                                                | 49.5 ms: 1.03x faster                                                 |
| genshi_text     | 23.5 ms                                                | 23.0 ms: 1.02x faster                                                 |
| mako            | 11.1 ms                                                | 11.3 ms: 1.02x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.02x faster                                                          |

All benchmarks:
===============

| Benchmark                 | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|---------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| create_gc_cycles          | 2.41 ms                                                | 1.74 ms: 1.38x faster                                                 |
| deepcopy                  | 358 us                                                 | 261 us: 1.37x faster                                                  |
| deepcopy_memo             | 39.1 us                                                | 29.7 us: 1.32x faster                                                 |
| go                        | 144 ms                                                 | 118 ms: 1.22x faster                                                  |
| python_startup            | 12.5 ms                                                | 10.6 ms: 1.18x faster                                                 |
| deepcopy_reduce           | 3.20 us                                                | 2.72 us: 1.18x faster                                                 |
| gc_traversal              | 4.37 ms                                                | 3.72 ms: 1.17x faster                                                 |
| async_tree_memoization_tg | 464 ms                                                 | 404 ms: 1.15x faster                                                  |
| async_tree_memoization    | 442 ms                                                 | 394 ms: 1.12x faster                                                  |
| async_tree_none           | 351 ms                                                 | 322 ms: 1.09x faster                                                  |
| pathlib                   | 17.5 ms                                                | 16.2 ms: 1.09x faster                                                 |
| mdp                       | 2.72 sec                                               | 2.52 sec: 1.08x faster                                                |
| scimark_sparse_mat_mult   | 5.04 ms                                                | 4.71 ms: 1.07x faster                                                 |
| richards_super            | 54.9 ms                                                | 51.7 ms: 1.06x faster                                                 |
| richards                  | 48.7 ms                                                | 46.0 ms: 1.06x faster                                                 |
| bench_thread_pool         | 822 us                                                 | 780 us: 1.05x faster                                                  |
| logging_format            | 6.40 us                                                | 6.08 us: 1.05x faster                                                 |
| thrift                    | 809 us                                                 | 771 us: 1.05x faster                                                  |
| generators                | 29.0 ms                                                | 27.7 ms: 1.05x faster                                                 |
| django_template           | 35.2 ms                                                | 33.7 ms: 1.04x faster                                                 |
| typing_runtime_protocols  | 165 us                                                 | 158 us: 1.04x faster                                                  |
| 2to3                      | 267 ms                                                 | 257 ms: 1.04x faster                                                  |
| logging_silent            | 102 ns                                                 | 97.9 ns: 1.04x faster                                                 |
| logging_simple            | 5.72 us                                                | 5.51 us: 1.04x faster                                                 |
| async_tree_cpu_io_mixed   | 577 ms                                                 | 556 ms: 1.04x faster                                                  |
| crypto_pyaes              | 75.3 ms                                                | 72.6 ms: 1.04x faster                                                 |
| pprint_safe_repr          | 728 ms                                                 | 704 ms: 1.03x faster                                                  |
| nbody                     | 87.0 ms                                                | 84.1 ms: 1.03x faster                                                 |
| regex_compile             | 132 ms                                                 | 128 ms: 1.03x faster                                                  |
| async_tree_none_tg        | 321 ms                                                 | 310 ms: 1.03x faster                                                  |
| xml_etree_process         | 60.6 ms                                                | 58.7 ms: 1.03x faster                                                 |
| scimark_fft               | 364 ms                                                 | 353 ms: 1.03x faster                                                  |
| raytrace                  | 267 ms                                                 | 259 ms: 1.03x faster                                                  |
| pickle_pure_python        | 310 us                                                 | 301 us: 1.03x faster                                                  |
| pprint_pformat            | 1.49 sec                                               | 1.45 sec: 1.03x faster                                                |
| genshi_xml                | 50.9 ms                                                | 49.5 ms: 1.03x faster                                                 |
| sympy_str                 | 275 ms                                                 | 267 ms: 1.03x faster                                                  |
| tornado_http              | 92.4 ms                                                | 90.1 ms: 1.03x faster                                                 |
| xml_etree_generate        | 86.7 ms                                                | 84.6 ms: 1.03x faster                                                 |
| genshi_text               | 23.5 ms                                                | 23.0 ms: 1.02x faster                                                 |
| tomli_loads               | 2.14 sec                                               | 2.09 sec: 1.02x faster                                                |
| sympy_sum                 | 150 ms                                                 | 147 ms: 1.02x faster                                                  |
| float                     | 79.2 ms                                                | 77.5 ms: 1.02x faster                                                 |
| meteor_contest            | 109 ms                                                 | 107 ms: 1.02x faster                                                  |
| sympy_integrate           | 19.9 ms                                                | 19.5 ms: 1.02x faster                                                 |
| sympy_expand              | 463 ms                                                 | 455 ms: 1.02x faster                                                  |
| html5lib                  | 64.2 ms                                                | 63.1 ms: 1.02x faster                                                 |
| telco                     | 8.54 ms                                                | 8.42 ms: 1.02x faster                                                 |
| unpickle_pure_python      | 216 us                                                 | 213 us: 1.01x faster                                                  |
| scimark_monte_carlo       | 67.4 ms                                                | 66.5 ms: 1.01x faster                                                 |
| spectral_norm             | 115 ms                                                 | 114 ms: 1.01x faster                                                  |
| deltablue                 | 3.23 ms                                                | 3.20 ms: 1.01x faster                                                 |
| json_dumps                | 10.6 ms                                                | 10.5 ms: 1.00x faster                                                 |
| sqlglot_optimize          | 53.7 ms                                                | 53.6 ms: 1.00x faster                                                 |
| regex_v8                  | 26.2 ms                                                | 26.2 ms: 1.00x faster                                                 |
| pidigits                  | 186 ms                                                 | 187 ms: 1.00x slower                                                  |
| comprehensions            | 16.5 us                                                | 16.6 us: 1.00x slower                                                 |
| hexiom                    | 6.16 ms                                                | 6.20 ms: 1.00x slower                                                 |
| pyflate                   | 471 ms                                                 | 474 ms: 1.01x slower                                                  |
| chaos                     | 58.1 ms                                                | 58.4 ms: 1.01x slower                                                 |
| regex_dna                 | 218 ms                                                 | 220 ms: 1.01x slower                                                  |
| bpe_tokeniser             | 4.75 sec                                               | 4.78 sec: 1.01x slower                                                |
| asyncio_websockets        | 552 ms                                                 | 557 ms: 1.01x slower                                                  |
| scimark_sor               | 124 ms                                                 | 125 ms: 1.01x slower                                                  |
| coverage                  | 84.0 ms                                                | 85.3 ms: 1.02x slower                                                 |
| mako                      | 11.1 ms                                                | 11.3 ms: 1.02x slower                                                 |
| coroutines                | 22.7 ms                                                | 23.1 ms: 1.02x slower                                                 |
| sqlglot_parse             | 1.27 ms                                                | 1.30 ms: 1.02x slower                                                 |
| regex_effbot              | 3.73 ms                                                | 3.81 ms: 1.02x slower                                                 |
| python_startup_no_site    | 6.96 ms                                                | 7.11 ms: 1.02x slower                                                 |
| xml_etree_iterparse       | 101 ms                                                 | 104 ms: 1.02x slower                                                  |
| docutils                  | 2.59 sec                                               | 2.66 sec: 1.03x slower                                                |
| nqueens                   | 78.4 ms                                                | 81.3 ms: 1.04x slower                                                 |
| json_loads                | 27.2 us                                                | 28.6 us: 1.05x slower                                                 |
| async_tree_io_tg          | 857 ms                                                 | 901 ms: 1.05x slower                                                  |
| async_tree_io             | 842 ms                                                 | 930 ms: 1.10x slower                                                  |
| Geometric mean            | (ref)                                                  | 1.03x faster                                                          |

Benchmark hidden because not significant (11): async_tree_cpu_io_mixed_tg, json, pycparser, xml_etree_parse, scimark_lu, bench_mp_pool, sqlglot_transpile, sqlglot_normalize, async_generators, fannkuch, pylint
Ignored benchmarks (10) of results/bm-20241007-3.13.0-60403a5/bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, dulwich_log, gevent_hub, k_core, mypy2, shortest_path, sphinx, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (2) of results/bm-20240829-3.14.0a0-58ce131/bm-20240829-linux-x86_64-python-58ce131037ecb34d506a-3.14.0a0-58ce131.json: asyncio_tcp, asyncio_tcp_ssl

- Geometric mean (including insignificant results): 1.033x faster
# HPT report

- Reliability score: 99.88% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 0.92x