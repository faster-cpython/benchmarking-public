# Results vs. 3.13.0

- fork: diegorusso
- ref: gh_119726_generate_a
- machine: linux-aarch64
- commit hash: 8cbca05
- commit date: 2024-09-10
- overall geometric mean: 1.076x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x slower
- Memory change: 0.98x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 304 ms                                                   | 376 ms: 1.24x slower                                                        |
| docutils       | 2.89 sec                                                 | 3.94 sec: 1.36x slower                                                      |
| html5lib       | 66.4 ms                                                  | 71.4 ms: 1.08x slower                                                       |
| tornado_http   | 128 ms                                                   | 151 ms: 1.18x slower                                                        |
| Geometric mean | (ref)                                                    | 1.21x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 550 ms: 1.18x faster                                                        |
| async_tree_memoization     | 651 ms                                                   | 583 ms: 1.12x faster                                                        |
| async_tree_none_tg         | 470 ms                                                   | 423 ms: 1.11x faster                                                        |
| async_tree_none            | 497 ms                                                   | 449 ms: 1.11x faster                                                        |
| async_tree_cpu_io_mixed_tg | 769 ms                                                   | 725 ms: 1.06x faster                                                        |
| async_tree_io              | 1.11 sec                                                 | 1.15 sec: 1.04x slower                                                      |
| async_tree_io_tg           | 1.13 sec                                                 | 1.18 sec: 1.04x slower                                                      |
| async_generators           | 489 ms                                                   | 510 ms: 1.04x slower                                                        |
| Geometric mean             | (ref)                                                    | 1.04x faster                                                                |

Benchmark hidden because not significant (3): async_tree_cpu_io_mixed, coroutines, asyncio_websockets

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 93.3 ms                                                  | 87.8 ms: 1.06x faster                                                       |
| nbody          | 114 ms                                                   | 111 ms: 1.04x faster                                                        |
| Geometric mean | (ref)                                                    | 1.03x faster                                                                |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_v8       | 31.8 ms                                                  | 30.8 ms: 1.03x faster                                                       |
| regex_dna      | 253 ms                                                   | 255 ms: 1.01x slower                                                        |
| regex_effbot   | 4.89 ms                                                  | 4.99 ms: 1.02x slower                                                       |
| regex_compile  | 127 ms                                                   | 186 ms: 1.46x slower                                                        |
| Geometric mean | (ref)                                                    | 1.10x slower                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| xml_etree_parse      | 197 ms                                                   | 187 ms: 1.05x faster                                                        |
| xml_etree_generate   | 113 ms                                                   | 110 ms: 1.03x faster                                                        |
| json_loads           | 31.7 us                                                  | 31.3 us: 1.01x faster                                                       |
| tomli_loads          | 2.54 sec                                                 | 2.63 sec: 1.04x slower                                                      |
| unpickle_pure_python | 251 us                                                   | 264 us: 1.05x slower                                                        |
| pickle_pure_python   | 357 us                                                   | 384 us: 1.08x slower                                                        |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                                |

Benchmark hidden because not significant (3): xml_etree_process, xml_etree_iterparse, json_dumps

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup | 15.4 ms                                                  | 13.1 ms: 1.18x faster                                                       |
| Geometric mean | (ref)                                                    | 1.08x faster                                                                |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|-----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 13.4 ms                                                  | 13.0 ms: 1.03x faster                                                       |
| django_template | 41.6 ms                                                  | 50.5 ms: 1.21x slower                                                       |
| genshi_text     | 27.7 ms                                                  | 34.9 ms: 1.26x slower                                                       |
| genshi_xml      | 60.3 ms                                                  | 81.5 ms: 1.35x slower                                                       |
| Geometric mean  | (ref)                                                    | 1.19x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05 |
|----------------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------:|
| create_gc_cycles           | 3.35 ms                                                  | 2.34 ms: 1.43x faster                                                       |
| deepcopy_memo              | 50.4 us                                                  | 37.7 us: 1.33x faster                                                       |
| async_tree_memoization_tg  | 649 ms                                                   | 550 ms: 1.18x faster                                                        |
| python_startup             | 15.4 ms                                                  | 13.1 ms: 1.18x faster                                                       |
| gc_traversal               | 5.77 ms                                                  | 5.13 ms: 1.13x faster                                                       |
| deepcopy                   | 447 us                                                   | 400 us: 1.12x faster                                                        |
| async_tree_memoization     | 651 ms                                                   | 583 ms: 1.12x faster                                                        |
| async_tree_none_tg         | 470 ms                                                   | 423 ms: 1.11x faster                                                        |
| async_tree_none            | 497 ms                                                   | 449 ms: 1.11x faster                                                        |
| scimark_sor                | 160 ms                                                   | 149 ms: 1.07x faster                                                        |
| float                      | 93.3 ms                                                  | 87.8 ms: 1.06x faster                                                       |
| async_tree_cpu_io_mixed_tg | 769 ms                                                   | 725 ms: 1.06x faster                                                        |
| xml_etree_parse            | 197 ms                                                   | 187 ms: 1.05x faster                                                        |
| deepcopy_reduce            | 4.07 us                                                  | 3.86 us: 1.05x faster                                                       |
| pathlib                    | 22.7 ms                                                  | 21.7 ms: 1.05x faster                                                       |
| nbody                      | 114 ms                                                   | 111 ms: 1.04x faster                                                        |
| regex_v8                   | 31.8 ms                                                  | 30.8 ms: 1.03x faster                                                       |
| xml_etree_generate         | 113 ms                                                   | 110 ms: 1.03x faster                                                        |
| mako                       | 13.4 ms                                                  | 13.0 ms: 1.03x faster                                                       |
| json                       | 5.73 ms                                                  | 5.61 ms: 1.02x faster                                                       |
| json_loads                 | 31.7 us                                                  | 31.3 us: 1.01x faster                                                       |
| bench_mp_pool              | 7.68 ms                                                  | 7.61 ms: 1.01x faster                                                       |
| regex_dna                  | 253 ms                                                   | 255 ms: 1.01x slower                                                        |
| bpe_tokeniser              | 5.87 sec                                                 | 5.94 sec: 1.01x slower                                                      |
| scimark_fft                | 447 ms                                                   | 454 ms: 1.02x slower                                                        |
| regex_effbot               | 4.89 ms                                                  | 4.99 ms: 1.02x slower                                                       |
| spectral_norm              | 143 ms                                                   | 146 ms: 1.02x slower                                                        |
| logging_format             | 7.82 us                                                  | 8.00 us: 1.02x slower                                                       |
| logging_silent             | 133 ns                                                   | 137 ns: 1.03x slower                                                        |
| tomli_loads                | 2.54 sec                                                 | 2.63 sec: 1.04x slower                                                      |
| async_tree_io              | 1.11 sec                                                 | 1.15 sec: 1.04x slower                                                      |
| async_tree_io_tg           | 1.13 sec                                                 | 1.18 sec: 1.04x slower                                                      |
| async_generators           | 489 ms                                                   | 510 ms: 1.04x slower                                                        |
| scimark_sparse_mat_mult    | 6.51 ms                                                  | 6.80 ms: 1.04x slower                                                       |
| logging_simple             | 7.07 us                                                  | 7.40 us: 1.05x slower                                                       |
| mdp                        | 3.34 sec                                                 | 3.50 sec: 1.05x slower                                                      |
| unpickle_pure_python       | 251 us                                                   | 264 us: 1.05x slower                                                        |
| telco                      | 9.74 ms                                                  | 10.3 ms: 1.05x slower                                                       |
| bench_thread_pool          | 1.27 ms                                                  | 1.35 ms: 1.06x slower                                                       |
| typing_runtime_protocols   | 193 us                                                   | 206 us: 1.07x slower                                                        |
| scimark_lu                 | 139 ms                                                   | 149 ms: 1.07x slower                                                        |
| html5lib                   | 66.4 ms                                                  | 71.4 ms: 1.08x slower                                                       |
| pickle_pure_python         | 357 us                                                   | 384 us: 1.08x slower                                                        |
| fannkuch                   | 461 ms                                                   | 497 ms: 1.08x slower                                                        |
| crypto_pyaes               | 83.7 ms                                                  | 90.5 ms: 1.08x slower                                                       |
| meteor_contest             | 114 ms                                                   | 124 ms: 1.09x slower                                                        |
| scimark_monte_carlo        | 83.6 ms                                                  | 92.8 ms: 1.11x slower                                                       |
| deltablue                  | 3.82 ms                                                  | 4.33 ms: 1.13x slower                                                       |
| pyflate                    | 556 ms                                                   | 641 ms: 1.15x slower                                                        |
| go                         | 160 ms                                                   | 184 ms: 1.15x slower                                                        |
| raytrace                   | 300 ms                                                   | 348 ms: 1.16x slower                                                        |
| sqlglot_normalize          | 127 ms                                                   | 149 ms: 1.18x slower                                                        |
| tornado_http               | 128 ms                                                   | 151 ms: 1.18x slower                                                        |
| pycparser                  | 1.27 sec                                                 | 1.50 sec: 1.18x slower                                                      |
| comprehensions             | 20.4 us                                                  | 24.6 us: 1.21x slower                                                       |
| django_template            | 41.6 ms                                                  | 50.5 ms: 1.21x slower                                                       |
| nqueens                    | 100 ms                                                   | 123 ms: 1.23x slower                                                        |
| 2to3                       | 304 ms                                                   | 376 ms: 1.24x slower                                                        |
| richards                   | 53.6 ms                                                  | 67.3 ms: 1.26x slower                                                       |
| richards_super             | 60.1 ms                                                  | 75.5 ms: 1.26x slower                                                       |
| genshi_text                | 27.7 ms                                                  | 34.9 ms: 1.26x slower                                                       |
| sqlglot_optimize           | 62.2 ms                                                  | 78.7 ms: 1.27x slower                                                       |
| sqlglot_parse              | 1.38 ms                                                  | 1.74 ms: 1.27x slower                                                       |
| sqlglot_transpile          | 1.73 ms                                                  | 2.20 ms: 1.27x slower                                                       |
| sympy_expand               | 457 ms                                                   | 601 ms: 1.32x slower                                                        |
| pprint_safe_repr           | 926 ms                                                   | 1.23 sec: 1.33x slower                                                      |
| sympy_str                  | 264 ms                                                   | 355 ms: 1.34x slower                                                        |
| genshi_xml                 | 60.3 ms                                                  | 81.5 ms: 1.35x slower                                                       |
| pprint_pformat             | 1.90 sec                                                 | 2.57 sec: 1.35x slower                                                      |
| sympy_integrate            | 20.8 ms                                                  | 28.2 ms: 1.36x slower                                                       |
| chaos                      | 68.0 ms                                                  | 92.4 ms: 1.36x slower                                                       |
| docutils                   | 2.89 sec                                                 | 3.94 sec: 1.36x slower                                                      |
| pylint                     | 342 ms                                                   | 475 ms: 1.39x slower                                                        |
| hexiom                     | 7.11 ms                                                  | 9.91 ms: 1.39x slower                                                       |
| sympy_sum                  | 144 ms                                                   | 208 ms: 1.45x slower                                                        |
| regex_compile              | 127 ms                                                   | 186 ms: 1.46x slower                                                        |
| generators                 | 37.6 ms                                                  | 58.1 ms: 1.55x slower                                                       |
| Geometric mean             | (ref)                                                    | 1.08x slower                                                                |

Benchmark hidden because not significant (10): xml_etree_process, async_tree_cpu_io_mixed, xml_etree_iterparse, thrift, python_startup_no_site, pidigits, coroutines, asyncio_websockets, coverage, json_dumps
Ignored benchmarks (9) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, gevent_hub, k_core, mypy2, shortest_path, sphinx, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (10) of results/bm-20240910-3.14.0a0-8cbca05-JIT/bm-20240910-arminc-aarch64-diegorusso-gh_119726_generate_a-3.14.0a0-8cbca05.json: asyncio_tcp, asyncio_tcp_ssl, dulwich_log, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.076x slower
# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.03x
- 99% likely to have a slowdown of 1.02x

# Memory
- memory change: 0.98x