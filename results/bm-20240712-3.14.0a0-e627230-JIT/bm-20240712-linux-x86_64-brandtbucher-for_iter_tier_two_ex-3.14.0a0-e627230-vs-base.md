# Results vs. base

- fork: brandtbucher
- ref: for_iter_tier_two_ex
- machine: linux-x86_64
- commit hash: e627230
- commit date: 2024-07-12
- overall geometric mean: 1.00x slower
- HPT reliability: 99.62%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.02x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 271 ms                                                                | 272 ms: 1.00x slower                                                        |
| html5lib       | 64.7 ms                                                               | 64.2 ms: 1.01x faster                                                       |
| tornado_http   | 92.4 ms                                                               | 92.8 ms: 1.00x slower                                                       |
| Geometric mean | (ref)                                                                 | 1.00x slower                                                                |

Benchmark hidden because not significant (1): docutils

Benchmarks with tag 'asyncio':
==============================

Benchmark hidden because not significant (8): async_tree_memoization_tg, async_tree_io_tg, async_tree_none_tg, async_tree_io, async_tree_memoization, async_tree_cpu_io_mixed_tg, async_tree_none, async_tree_cpu_io_mixed

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pidigits       | 186 ms                                                                | 185 ms: 1.01x faster                                                        |
| float          | 70.0 ms                                                               | 69.7 ms: 1.00x faster                                                       |
| Geometric mean | (ref)                                                                 | 1.00x faster                                                                |

Benchmark hidden because not significant (1): nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_effbot   | 3.86 ms                                                               | 3.81 ms: 1.01x faster                                                       |
| regex_dna      | 222 ms                                                                | 221 ms: 1.01x faster                                                        |
| regex_v8       | 25.3 ms                                                               | 25.1 ms: 1.00x faster                                                       |
| regex_compile  | 134 ms                                                                | 134 ms: 1.00x slower                                                        |
| Geometric mean | (ref)                                                                 | 1.00x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|----------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_loads           | 28.1 us                                                               | 27.6 us: 1.02x faster                                                       |
| xml_etree_generate   | 81.6 ms                                                               | 80.6 ms: 1.01x faster                                                       |
| tomli_loads          | 1.92 sec                                                              | 1.90 sec: 1.01x faster                                                      |
| xml_etree_process    | 57.5 ms                                                               | 56.9 ms: 1.01x faster                                                       |
| json_dumps           | 10.4 ms                                                               | 10.3 ms: 1.01x faster                                                       |
| unpickle_pure_python | 215 us                                                                | 217 us: 1.01x slower                                                        |
| Geometric mean       | (ref)                                                                 | 1.01x faster                                                                |

Benchmark hidden because not significant (3): xml_etree_parse, xml_etree_iterparse, pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 10.5 ms                                                               | 10.5 ms: 1.00x slower                                                       |
| python_startup_no_site | 7.10 ms                                                               | 7.12 ms: 1.00x slower                                                       |
| Geometric mean         | (ref)                                                                 | 1.00x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|-----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| genshi_xml      | 58.0 ms                                                               | 56.1 ms: 1.03x faster                                                       |
| django_template | 35.1 ms                                                               | 35.6 ms: 1.01x slower                                                       |
| Geometric mean  | (ref)                                                                 | 1.00x faster                                                                |

Benchmark hidden because not significant (2): mako, genshi_text

All benchmarks:
===============

| Benchmark                | bm-20240708-linux-x86_64-python-006b53a42f72be83ecdf-3.14.0a0-006b53a | bm-20240712-linux-x86_64-brandtbucher-for_iter_tier_two_ex-3.14.0a0-e627230 |
|--------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| genshi_xml               | 58.0 ms                                                               | 56.1 ms: 1.03x faster                                                       |
| spectral_norm            | 104 ms                                                                | 101 ms: 1.03x faster                                                        |
| pathlib                  | 16.2 ms                                                               | 15.8 ms: 1.03x faster                                                       |
| telco                    | 8.08 ms                                                               | 7.88 ms: 1.02x faster                                                       |
| meteor_contest           | 107 ms                                                                | 104 ms: 1.02x faster                                                        |
| deepcopy_reduce          | 2.78 us                                                               | 2.72 us: 1.02x faster                                                       |
| json_loads               | 28.1 us                                                               | 27.6 us: 1.02x faster                                                       |
| crypto_pyaes             | 67.8 ms                                                               | 66.8 ms: 1.01x faster                                                       |
| richards                 | 41.8 ms                                                               | 41.2 ms: 1.01x faster                                                       |
| json                     | 5.19 ms                                                               | 5.12 ms: 1.01x faster                                                       |
| typing_runtime_protocols | 172 us                                                                | 170 us: 1.01x faster                                                        |
| xml_etree_generate       | 81.6 ms                                                               | 80.6 ms: 1.01x faster                                                       |
| logging_silent           | 107 ns                                                                | 106 ns: 1.01x faster                                                        |
| regex_effbot             | 3.86 ms                                                               | 3.81 ms: 1.01x faster                                                       |
| tomli_loads              | 1.92 sec                                                              | 1.90 sec: 1.01x faster                                                      |
| xml_etree_process        | 57.5 ms                                                               | 56.9 ms: 1.01x faster                                                       |
| scimark_monte_carlo      | 61.1 ms                                                               | 60.5 ms: 1.01x faster                                                       |
| json_dumps               | 10.4 ms                                                               | 10.3 ms: 1.01x faster                                                       |
| gc_traversal             | 3.74 ms                                                               | 3.71 ms: 1.01x faster                                                       |
| go                       | 145 ms                                                                | 143 ms: 1.01x faster                                                        |
| html5lib                 | 64.7 ms                                                               | 64.2 ms: 1.01x faster                                                       |
| pyflate                  | 444 ms                                                                | 441 ms: 1.01x faster                                                        |
| pidigits                 | 186 ms                                                                | 185 ms: 1.01x faster                                                        |
| regex_dna                | 222 ms                                                                | 221 ms: 1.01x faster                                                        |
| regex_v8                 | 25.3 ms                                                               | 25.1 ms: 1.00x faster                                                       |
| float                    | 70.0 ms                                                               | 69.7 ms: 1.00x faster                                                       |
| coverage                 | 92.4 ms                                                               | 92.0 ms: 1.00x faster                                                       |
| bench_thread_pool        | 832 us                                                                | 830 us: 1.00x faster                                                        |
| python_startup           | 10.5 ms                                                               | 10.5 ms: 1.00x slower                                                       |
| python_startup_no_site   | 7.10 ms                                                               | 7.12 ms: 1.00x slower                                                       |
| 2to3                     | 271 ms                                                                | 272 ms: 1.00x slower                                                        |
| asyncio_tcp_ssl          | 1.80 sec                                                              | 1.80 sec: 1.00x slower                                                      |
| create_gc_cycles         | 1.76 ms                                                               | 1.77 ms: 1.00x slower                                                       |
| dulwich_log              | 67.1 ms                                                               | 67.3 ms: 1.00x slower                                                       |
| regex_compile            | 134 ms                                                                | 134 ms: 1.00x slower                                                        |
| tornado_http             | 92.4 ms                                                               | 92.8 ms: 1.00x slower                                                       |
| hexiom                   | 6.48 ms                                                               | 6.50 ms: 1.00x slower                                                       |
| sqlglot_normalize        | 111 ms                                                                | 112 ms: 1.01x slower                                                        |
| logging_simple           | 5.42 us                                                               | 5.46 us: 1.01x slower                                                       |
| sqlglot_transpile        | 1.60 ms                                                               | 1.61 ms: 1.01x slower                                                       |
| scimark_sor              | 129 ms                                                                | 130 ms: 1.01x slower                                                        |
| sympy_expand             | 492 ms                                                                | 496 ms: 1.01x slower                                                        |
| sympy_str                | 292 ms                                                                | 294 ms: 1.01x slower                                                        |
| sqlglot_optimize         | 55.0 ms                                                               | 55.5 ms: 1.01x slower                                                       |
| unpickle_pure_python     | 215 us                                                                | 217 us: 1.01x slower                                                        |
| comprehensions           | 16.7 us                                                               | 16.8 us: 1.01x slower                                                       |
| sympy_sum                | 165 ms                                                                | 167 ms: 1.01x slower                                                        |
| deltablue                | 3.52 ms                                                               | 3.56 ms: 1.01x slower                                                       |
| scimark_sparse_mat_mult  | 4.28 ms                                                               | 4.33 ms: 1.01x slower                                                       |
| fannkuch                 | 361 ms                                                                | 365 ms: 1.01x slower                                                        |
| django_template          | 35.1 ms                                                               | 35.6 ms: 1.01x slower                                                       |
| sympy_integrate          | 21.8 ms                                                               | 22.1 ms: 1.01x slower                                                       |
| raytrace                 | 266 ms                                                                | 269 ms: 1.01x slower                                                        |
| nqueens                  | 85.5 ms                                                               | 86.7 ms: 1.01x slower                                                       |
| generators               | 29.5 ms                                                               | 30.0 ms: 1.01x slower                                                       |
| async_generators         | 452 ms                                                                | 459 ms: 1.02x slower                                                        |
| richards_super           | 47.5 ms                                                               | 48.3 ms: 1.02x slower                                                       |
| logging_format           | 5.94 us                                                               | 6.05 us: 1.02x slower                                                       |
| scimark_fft              | 309 ms                                                                | 316 ms: 1.02x slower                                                        |
| asyncio_tcp              | 488 ms                                                                | 501 ms: 1.03x slower                                                        |
| mdp                      | 2.52 sec                                                              | 2.68 sec: 1.06x slower                                                      |
| Geometric mean           | (ref)                                                                 | 1.00x slower                                                                |

Benchmark hidden because not significant (30): xml_etree_parse, mako, thrift, chaos, bpe_tokeniser, scimark_lu, xml_etree_iterparse, bench_mp_pool, dask, coroutines, deepcopy, pycparser, docutils, pprint_safe_repr, sqlglot_parse, deepcopy_memo, asyncio_websockets, async_tree_memoization_tg, async_tree_io_tg, nbody, async_tree_none_tg, pickle_pure_python, async_tree_io, pylint, async_tree_memoization, genshi_text, pprint_pformat, async_tree_cpu_io_mixed_tg, async_tree_none, async_tree_cpu_io_mixed

# HPT report

- Reliability score: 99.62% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.02x