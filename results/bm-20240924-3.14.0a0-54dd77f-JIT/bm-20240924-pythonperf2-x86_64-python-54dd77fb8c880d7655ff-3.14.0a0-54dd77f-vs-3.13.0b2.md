# Results vs. 3.13.0b2

- fork: python
- ref: 54dd77fb8c880d7655ff
- machine: linux-x86_64
- commit hash: 54dd77f
- commit date: 2024-09-24
- overall geometric mean: 1.01x faster
- HPT reliability: 79.66%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.08x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                           | 312 ms: 1.07x slower                                                        |
| docutils       | 2.98 sec                                                         | 3.18 sec: 1.07x slower                                                      |
| html5lib       | 74.7 ms                                                          | 71.4 ms: 1.05x faster                                                       |
| tornado_http   | 119 ms                                                           | 122 ms: 1.03x slower                                                        |
| Geometric mean | (ref)                                                            | 1.03x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_io_tg           | 900 ms                                                           | 795 ms: 1.13x faster                                                        |
| async_tree_memoization     | 460 ms                                                           | 408 ms: 1.13x faster                                                        |
| async_tree_none            | 365 ms                                                           | 325 ms: 1.12x faster                                                        |
| async_tree_none_tg         | 340 ms                                                           | 313 ms: 1.09x faster                                                        |
| async_tree_io              | 873 ms                                                           | 806 ms: 1.08x faster                                                        |
| async_tree_memoization_tg  | 421 ms                                                           | 394 ms: 1.07x faster                                                        |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 547 ms: 1.05x faster                                                        |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 578 ms: 1.05x faster                                                        |
| Geometric mean             | (ref)                                                            | 1.09x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 80.2 ms                                                          | 73.6 ms: 1.09x faster                                                       |
| nbody          | 87.8 ms                                                          | 83.8 ms: 1.05x faster                                                       |
| pidigits       | 254 ms                                                           | 250 ms: 1.01x faster                                                        |
| Geometric mean | (ref)                                                            | 1.05x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_dna      | 249 ms                                                           | 234 ms: 1.07x faster                                                        |
| regex_v8       | 26.0 ms                                                          | 25.8 ms: 1.01x faster                                                       |
| regex_compile  | 144 ms                                                           | 148 ms: 1.03x slower                                                        |
| regex_effbot   | 3.40 ms                                                          | 3.50 ms: 1.03x slower                                                       |
| Geometric mean | (ref)                                                            | 1.00x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| tomli_loads          | 2.40 sec                                                         | 2.11 sec: 1.14x faster                                                      |
| xml_etree_generate   | 85.7 ms                                                          | 80.2 ms: 1.07x faster                                                       |
| xml_etree_process    | 59.7 ms                                                          | 56.4 ms: 1.06x faster                                                       |
| unpickle             | 15.7 us                                                          | 14.9 us: 1.05x faster                                                       |
| xml_etree_iterparse  | 103 ms                                                           | 98.2 ms: 1.04x faster                                                       |
| unpickle_pure_python | 224 us                                                           | 216 us: 1.04x faster                                                        |
| json_loads           | 25.0 us                                                          | 24.2 us: 1.04x faster                                                       |
| pickle_dict          | 32.8 us                                                          | 32.2 us: 1.02x faster                                                       |
| json_dumps           | 10.8 ms                                                          | 10.6 ms: 1.02x faster                                                       |
| unpickle_list        | 4.70 us                                                          | 4.63 us: 1.01x faster                                                       |
| pickle_list          | 4.44 us                                                          | 4.54 us: 1.02x slower                                                       |
| pickle_pure_python   | 307 us                                                           | 325 us: 1.06x slower                                                        |
| Geometric mean       | (ref)                                                            | 1.03x faster                                                                |

Benchmark hidden because not significant (2): xml_etree_parse, pickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 13.2 ms                                                          | 13.4 ms: 1.02x slower                                                       |
| python_startup_no_site | 8.85 ms                                                          | 9.00 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                            | 1.02x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|-----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 10.4 ms                                                          | 9.16 ms: 1.13x faster                                                       |
| django_template | 39.0 ms                                                          | 42.4 ms: 1.09x slower                                                       |
| genshi_text     | 25.9 ms                                                          | 29.5 ms: 1.14x slower                                                       |
| genshi_xml      | 58.1 ms                                                          | 68.6 ms: 1.18x slower                                                       |
| Geometric mean  | (ref)                                                            | 1.07x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo              | 37.3 us                                                          | 26.9 us: 1.39x faster                                                       |
| deepcopy                   | 377 us                                                           | 298 us: 1.27x faster                                                        |
| richards                   | 53.4 ms                                                          | 44.6 ms: 1.20x faster                                                       |
| spectral_norm              | 97.3 ms                                                          | 81.4 ms: 1.20x faster                                                       |
| richards_super             | 61.2 ms                                                          | 51.6 ms: 1.18x faster                                                       |
| deepcopy_reduce            | 3.39 us                                                          | 2.92 us: 1.16x faster                                                       |
| scimark_sor                | 119 ms                                                           | 104 ms: 1.14x faster                                                        |
| tomli_loads                | 2.40 sec                                                         | 2.11 sec: 1.14x faster                                                      |
| async_tree_io_tg           | 900 ms                                                           | 795 ms: 1.13x faster                                                        |
| mako                       | 10.4 ms                                                          | 9.16 ms: 1.13x faster                                                       |
| async_tree_memoization     | 460 ms                                                           | 408 ms: 1.13x faster                                                        |
| async_tree_none            | 365 ms                                                           | 325 ms: 1.12x faster                                                        |
| go                         | 165 ms                                                           | 148 ms: 1.11x faster                                                        |
| float                      | 80.2 ms                                                          | 73.6 ms: 1.09x faster                                                       |
| scimark_fft                | 312 ms                                                           | 287 ms: 1.09x faster                                                        |
| async_tree_none_tg         | 340 ms                                                           | 313 ms: 1.09x faster                                                        |
| bpe_tokeniser              | 5.14 sec                                                         | 4.73 sec: 1.09x faster                                                      |
| async_tree_io              | 873 ms                                                           | 806 ms: 1.08x faster                                                        |
| pyflate                    | 482 ms                                                           | 445 ms: 1.08x faster                                                        |
| pathlib                    | 17.1 ms                                                          | 15.9 ms: 1.08x faster                                                       |
| xml_etree_generate         | 85.7 ms                                                          | 80.2 ms: 1.07x faster                                                       |
| scimark_sparse_mat_mult    | 4.28 ms                                                          | 4.01 ms: 1.07x faster                                                       |
| async_tree_memoization_tg  | 421 ms                                                           | 394 ms: 1.07x faster                                                        |
| regex_dna                  | 249 ms                                                           | 234 ms: 1.07x faster                                                        |
| deltablue                  | 3.37 ms                                                          | 3.17 ms: 1.06x faster                                                       |
| gc_traversal               | 4.69 ms                                                          | 4.42 ms: 1.06x faster                                                       |
| xml_etree_process          | 59.7 ms                                                          | 56.4 ms: 1.06x faster                                                       |
| unpickle                   | 15.7 us                                                          | 14.9 us: 1.05x faster                                                       |
| create_gc_cycles           | 2.00 ms                                                          | 1.91 ms: 1.05x faster                                                       |
| crypto_pyaes               | 73.6 ms                                                          | 69.9 ms: 1.05x faster                                                       |
| sqlite_synth               | 2.80 us                                                          | 2.66 us: 1.05x faster                                                       |
| nbody                      | 87.8 ms                                                          | 83.8 ms: 1.05x faster                                                       |
| dulwich_log                | 67.3 ms                                                          | 64.3 ms: 1.05x faster                                                       |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 547 ms: 1.05x faster                                                        |
| coverage                   | 83.5 ms                                                          | 79.8 ms: 1.05x faster                                                       |
| html5lib                   | 74.7 ms                                                          | 71.4 ms: 1.05x faster                                                       |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 578 ms: 1.05x faster                                                        |
| xml_etree_iterparse        | 103 ms                                                           | 98.2 ms: 1.04x faster                                                       |
| unpickle_pure_python       | 224 us                                                           | 216 us: 1.04x faster                                                        |
| json_loads                 | 25.0 us                                                          | 24.2 us: 1.04x faster                                                       |
| coroutines                 | 22.0 ms                                                          | 21.3 ms: 1.03x faster                                                       |
| pprint_safe_repr           | 818 ms                                                           | 792 ms: 1.03x faster                                                        |
| telco                      | 8.40 ms                                                          | 8.20 ms: 1.02x faster                                                       |
| pickle_dict                | 32.8 us                                                          | 32.2 us: 1.02x faster                                                       |
| json_dumps                 | 10.8 ms                                                          | 10.6 ms: 1.02x faster                                                       |
| pidigits                   | 254 ms                                                           | 250 ms: 1.01x faster                                                        |
| unpickle_list              | 4.70 us                                                          | 4.63 us: 1.01x faster                                                       |
| asyncio_websockets         | 395 ms                                                           | 392 ms: 1.01x faster                                                        |
| regex_v8                   | 26.0 ms                                                          | 25.8 ms: 1.01x faster                                                       |
| pprint_pformat             | 1.66 sec                                                         | 1.65 sec: 1.01x faster                                                      |
| asyncio_tcp_ssl            | 1.58 sec                                                         | 1.58 sec: 1.00x slower                                                      |
| asyncio_tcp                | 378 ms                                                           | 380 ms: 1.01x slower                                                        |
| meteor_contest             | 128 ms                                                           | 129 ms: 1.01x slower                                                        |
| scimark_lu                 | 97.5 ms                                                          | 98.4 ms: 1.01x slower                                                       |
| python_startup             | 13.2 ms                                                          | 13.4 ms: 1.02x slower                                                       |
| python_startup_no_site     | 8.85 ms                                                          | 9.00 ms: 1.02x slower                                                       |
| fannkuch                   | 353 ms                                                           | 360 ms: 1.02x slower                                                        |
| logging_format             | 7.11 us                                                          | 7.26 us: 1.02x slower                                                       |
| bench_thread_pool          | 908 us                                                           | 927 us: 1.02x slower                                                        |
| pickle_list                | 4.44 us                                                          | 4.54 us: 1.02x slower                                                       |
| tornado_http               | 119 ms                                                           | 122 ms: 1.03x slower                                                        |
| regex_compile              | 144 ms                                                           | 148 ms: 1.03x slower                                                        |
| regex_effbot               | 3.40 ms                                                          | 3.50 ms: 1.03x slower                                                       |
| thrift                     | 880 us                                                           | 915 us: 1.04x slower                                                        |
| logging_simple             | 6.40 us                                                          | 6.67 us: 1.04x slower                                                       |
| scimark_monte_carlo        | 65.5 ms                                                          | 68.5 ms: 1.05x slower                                                       |
| mdp                        | 2.46 sec                                                         | 2.60 sec: 1.05x slower                                                      |
| pycparser                  | 1.22 sec                                                         | 1.29 sec: 1.06x slower                                                      |
| pickle_pure_python         | 307 us                                                           | 325 us: 1.06x slower                                                        |
| sympy_str                  | 295 ms                                                           | 313 ms: 1.06x slower                                                        |
| sqlglot_normalize          | 120 ms                                                           | 128 ms: 1.07x slower                                                        |
| async_generators           | 363 ms                                                           | 387 ms: 1.07x slower                                                        |
| docutils                   | 2.98 sec                                                         | 3.18 sec: 1.07x slower                                                      |
| sqlglot_parse              | 1.39 ms                                                          | 1.49 ms: 1.07x slower                                                       |
| 2to3                       | 291 ms                                                           | 312 ms: 1.07x slower                                                        |
| comprehensions             | 17.0 us                                                          | 18.2 us: 1.07x slower                                                       |
| typing_runtime_protocols   | 171 us                                                           | 183 us: 1.07x slower                                                        |
| sympy_expand               | 501 ms                                                           | 537 ms: 1.07x slower                                                        |
| logging_silent             | 96.3 ns                                                          | 104 ns: 1.08x slower                                                        |
| django_template            | 39.0 ms                                                          | 42.4 ms: 1.09x slower                                                       |
| hexiom                     | 6.35 ms                                                          | 6.95 ms: 1.09x slower                                                       |
| sympy_sum                  | 155 ms                                                           | 170 ms: 1.09x slower                                                        |
| sqlglot_transpile          | 1.76 ms                                                          | 1.95 ms: 1.10x slower                                                       |
| sqlglot_optimize           | 59.5 ms                                                          | 65.6 ms: 1.10x slower                                                       |
| nqueens                    | 88.4 ms                                                          | 98.2 ms: 1.11x slower                                                       |
| generators                 | 33.5 ms                                                          | 37.3 ms: 1.11x slower                                                       |
| chaos                      | 59.6 ms                                                          | 66.9 ms: 1.12x slower                                                       |
| genshi_text                | 25.9 ms                                                          | 29.5 ms: 1.14x slower                                                       |
| sympy_integrate            | 23.2 ms                                                          | 26.5 ms: 1.14x slower                                                       |
| genshi_xml                 | 58.1 ms                                                          | 68.6 ms: 1.18x slower                                                       |
| pylint                     | 339 ms                                                           | 411 ms: 1.21x slower                                                        |
| raytrace                   | 260 ms                                                           | 332 ms: 1.27x slower                                                        |
| Geometric mean             | (ref)                                                            | 1.01x faster                                                                |

Benchmark hidden because not significant (4): json, xml_etree_parse, pickle, bench_mp_pool
Ignored benchmarks (6) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2
Ignored benchmarks (1) of results/bm-20240924-3.14.0a0-54dd77f-JIT/bm-20240924-pythonperf2-x86_64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f.json: unpack_sequence

# HPT report

- Reliability score: 79.66% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.08x