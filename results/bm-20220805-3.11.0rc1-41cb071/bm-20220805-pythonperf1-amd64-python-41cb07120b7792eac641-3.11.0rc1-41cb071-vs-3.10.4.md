
# Results vs. 3.10.4

- fork: python
- ref: 41cb07120b7792eac641
- machine: windows-amd64
- commit hash: 41cb071
- commit date: 2022-08-05
- overall geometric mean: 1.12x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 202 ms: 1.17x faster                                                        |
| chameleon      | 5.92 ms                                                     | 5.05 ms: 1.17x faster                                                       |
| docutils       | 1.89 sec                                                    | 1.59 sec: 1.19x faster                                                      |
| html5lib       | 46.5 ms                                                     | 38.4 ms: 1.21x faster                                                       |
| tornado_http   | 109 ms                                                      | 90.9 ms: 1.20x faster                                                       |
| Geometric mean | (ref)                                                       | 1.19x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 52.9 ms: 1.14x faster                                                       |
| nbody          | 69.3 ms                                                     | 68.3 ms: 1.01x faster                                                       |
| pidigits       | 145 ms                                                      | 146 ms: 1.01x slower                                                        |
| Geometric mean | (ref)                                                       | 1.05x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 89.4 ms: 1.16x faster                                                       |
| regex_dna      | 132 ms                                                      | 123 ms: 1.07x faster                                                        |
| regex_v8       | 15.0 ms                                                     | 14.1 ms: 1.07x faster                                                       |
| Geometric mean | (ref)                                                       | 1.07x faster                                                                |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pickle_pure_python   | 257 us                                                      | 202 us: 1.27x faster                                                        |
| xml_etree_process    | 43.4 ms                                                     | 36.7 ms: 1.18x faster                                                       |
| unpickle_pure_python | 171 us                                                      | 150 us: 1.14x faster                                                        |
| unpickle             | 9.17 us                                                     | 8.11 us: 1.13x faster                                                       |
| json_dumps           | 8.50 ms                                                     | 7.73 ms: 1.10x faster                                                       |
| xml_etree_parse      | 102 ms                                                      | 94.8 ms: 1.07x faster                                                       |
| unpickle_list        | 2.81 us                                                     | 2.64 us: 1.06x faster                                                       |
| pickle               | 6.80 us                                                     | 6.44 us: 1.06x faster                                                       |
| xml_etree_generate   | 54.5 ms                                                     | 52.6 ms: 1.04x faster                                                       |
| json_loads           | 14.2 us                                                     | 14.0 us: 1.01x faster                                                       |
| pickle_list          | 2.59 us                                                     | 2.57 us: 1.01x faster                                                       |
| pickle_dict          | 16.9 us                                                     | 18.4 us: 1.09x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.07x faster                                                                |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.4 ms: 1.09x faster                                                       |
| python_startup_no_site | 15.5 ms                                                     | 15.2 ms: 1.02x faster                                                       |
| Geometric mean         | (ref)                                                       | 1.05x faster                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|-----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 7.26 ms: 1.21x faster                                                       |
| django_template | 28.2 ms                                                     | 23.7 ms: 1.19x faster                                                       |
| genshi_text     | 19.0 ms                                                     | 16.7 ms: 1.14x faster                                                       |
| genshi_xml      | 40.5 ms                                                     | 37.7 ms: 1.07x faster                                                       |
| Geometric mean  | (ref)                                                       | 1.15x faster                                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20220805-pythonperf1-amd64-python-41cb07120b7792eac641-3.11.0rc1-41cb071 |
|-------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.63 ms: 1.59x faster                                                       |
| async_tree_io           | 1.07 sec                                                    | 739 ms: 1.44x faster                                                        |
| scimark_sor             | 105 ms                                                      | 77.3 ms: 1.36x faster                                                       |
| async_tree_none         | 420 ms                                                      | 310 ms: 1.36x faster                                                        |
| go                      | 136 ms                                                      | 101 ms: 1.35x faster                                                        |
| async_tree_memoization  | 497 ms                                                      | 368 ms: 1.35x faster                                                        |
| scimark_lu              | 85.4 ms                                                     | 63.7 ms: 1.34x faster                                                       |
| logging_silent          | 96.4 ns                                                     | 71.9 ns: 1.34x faster                                                       |
| raytrace                | 271 ms                                                      | 204 ms: 1.33x faster                                                        |
| sqlglot_parse           | 1.22 ms                                                     | 925 us: 1.32x faster                                                        |
| richards                | 41.2 ms                                                     | 31.3 ms: 1.32x faster                                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.13 ms: 1.29x faster                                                       |
| pyflate                 | 387 ms                                                      | 303 ms: 1.28x faster                                                        |
| crypto_pyaes            | 62.3 ms                                                     | 48.7 ms: 1.28x faster                                                       |
| mypy2                   | 352 ms                                                      | 275 ms: 1.28x faster                                                        |
| thrift                  | 615 us                                                      | 482 us: 1.28x faster                                                        |
| async_generators        | 224 ms                                                      | 176 ms: 1.27x faster                                                        |
| pickle_pure_python      | 257 us                                                      | 202 us: 1.27x faster                                                        |
| pycparser               | 868 ms                                                      | 689 ms: 1.26x faster                                                        |
| scimark_monte_carlo     | 55.9 ms                                                     | 44.4 ms: 1.26x faster                                                       |
| async_tree_cpu_io_mixed | 609 ms                                                      | 491 ms: 1.24x faster                                                        |
| hexiom                  | 5.52 ms                                                     | 4.46 ms: 1.24x faster                                                       |
| mako                    | 8.80 ms                                                     | 7.26 ms: 1.21x faster                                                       |
| html5lib                | 46.5 ms                                                     | 38.4 ms: 1.21x faster                                                       |
| tornado_http            | 109 ms                                                      | 90.9 ms: 1.20x faster                                                       |
| django_template         | 28.2 ms                                                     | 23.7 ms: 1.19x faster                                                       |
| docutils                | 1.89 sec                                                    | 1.59 sec: 1.19x faster                                                      |
| chaos                   | 58.9 ms                                                     | 49.6 ms: 1.19x faster                                                       |
| xml_etree_process       | 43.4 ms                                                     | 36.7 ms: 1.18x faster                                                       |
| create_gc_cycles        | 782 us                                                      | 665 us: 1.18x faster                                                        |
| chameleon               | 5.92 ms                                                     | 5.05 ms: 1.17x faster                                                       |
| aiohttp                 | 1.01 ms                                                     | 861 us: 1.17x faster                                                        |
| 2to3                    | 237 ms                                                      | 202 ms: 1.17x faster                                                        |
| regex_compile           | 103 ms                                                      | 89.4 ms: 1.16x faster                                                       |
| pprint_pformat          | 1.21 sec                                                    | 1.04 sec: 1.15x faster                                                      |
| pprint_safe_repr        | 589 ms                                                      | 513 ms: 1.15x faster                                                        |
| sqlalchemy_declarative  | 95.4 ms                                                     | 83.4 ms: 1.14x faster                                                       |
| dask                    | 305 ms                                                      | 267 ms: 1.14x faster                                                        |
| unpickle_pure_python    | 171 us                                                      | 150 us: 1.14x faster                                                        |
| genshi_text             | 19.0 ms                                                     | 16.7 ms: 1.14x faster                                                       |
| float                   | 60.2 ms                                                     | 52.9 ms: 1.14x faster                                                       |
| unpickle                | 9.17 us                                                     | 8.11 us: 1.13x faster                                                       |
| sqlglot_optimize        | 39.0 ms                                                     | 34.5 ms: 1.13x faster                                                       |
| deepcopy_memo           | 28.5 us                                                     | 25.3 us: 1.13x faster                                                       |
| bench_thread_pool       | 946 us                                                      | 856 us: 1.11x faster                                                        |
| dulwich_log             | 47.6 ms                                                     | 43.3 ms: 1.10x faster                                                       |
| json_dumps              | 8.50 ms                                                     | 7.73 ms: 1.10x faster                                                       |
| sympy_integrate         | 14.8 ms                                                     | 13.5 ms: 1.10x faster                                                       |
| sqlite_synth            | 1.84 us                                                     | 1.68 us: 1.10x faster                                                       |
| pylint                  | 347 ms                                                      | 318 ms: 1.09x faster                                                        |
| python_startup          | 20.0 ms                                                     | 18.4 ms: 1.09x faster                                                       |
| spectral_norm           | 78.0 ms                                                     | 71.8 ms: 1.09x faster                                                       |
| sympy_expand            | 315 ms                                                      | 290 ms: 1.09x faster                                                        |
| asyncio_tcp             | 712 ms                                                      | 659 ms: 1.08x faster                                                        |
| coroutines              | 15.9 ms                                                     | 14.8 ms: 1.08x faster                                                       |
| genshi_xml              | 40.5 ms                                                     | 37.7 ms: 1.07x faster                                                       |
| sympy_sum               | 104 ms                                                      | 97.0 ms: 1.07x faster                                                       |
| xml_etree_parse         | 102 ms                                                      | 94.8 ms: 1.07x faster                                                       |
| mdp                     | 1.71 sec                                                    | 1.60 sec: 1.07x faster                                                      |
| regex_dna               | 132 ms                                                      | 123 ms: 1.07x faster                                                        |
| deepcopy                | 255 us                                                      | 238 us: 1.07x faster                                                        |
| sqlglot_normalize       | 202 ms                                                      | 189 ms: 1.07x faster                                                        |
| regex_v8                | 15.0 ms                                                     | 14.1 ms: 1.07x faster                                                       |
| pathlib                 | 77.4 ms                                                     | 72.6 ms: 1.07x faster                                                       |
| unpickle_list           | 2.81 us                                                     | 2.64 us: 1.06x faster                                                       |
| scimark_fft             | 193 ms                                                      | 182 ms: 1.06x faster                                                        |
| pickle                  | 6.80 us                                                     | 6.44 us: 1.06x faster                                                       |
| sqlalchemy_imperative   | 11.0 ms                                                     | 10.4 ms: 1.05x faster                                                       |
| deepcopy_reduce         | 2.16 us                                                     | 2.05 us: 1.05x faster                                                       |
| comprehensions          | 16.0 us                                                     | 15.2 us: 1.05x faster                                                       |
| sympy_str               | 188 ms                                                      | 180 ms: 1.05x faster                                                        |
| nqueens                 | 67.0 ms                                                     | 64.2 ms: 1.04x faster                                                       |
| fannkuch                | 258 ms                                                      | 247 ms: 1.04x faster                                                        |
| json                    | 3.05 ms                                                     | 2.94 ms: 1.04x faster                                                       |
| xml_etree_generate      | 54.5 ms                                                     | 52.6 ms: 1.04x faster                                                       |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.61 ms: 1.02x faster                                                       |
| python_startup_no_site  | 15.5 ms                                                     | 15.2 ms: 1.02x faster                                                       |
| nbody                   | 69.3 ms                                                     | 68.3 ms: 1.01x faster                                                       |
| json_loads              | 14.2 us                                                     | 14.0 us: 1.01x faster                                                       |
| pickle_list             | 2.59 us                                                     | 2.57 us: 1.01x faster                                                       |
| pidigits                | 145 ms                                                      | 146 ms: 1.01x slower                                                        |
| meteor_contest          | 72.5 ms                                                     | 73.3 ms: 1.01x slower                                                       |
| bench_mp_pool           | 60.7 ms                                                     | 61.4 ms: 1.01x slower                                                       |
| telco                   | 3.78 ms                                                     | 3.87 ms: 1.02x slower                                                       |
| logging_simple          | 6.20 us                                                     | 6.37 us: 1.03x slower                                                       |
| logging_format          | 6.66 us                                                     | 6.90 us: 1.04x slower                                                       |
| gc_traversal            | 1.34 ms                                                     | 1.40 ms: 1.04x slower                                                       |
| generators              | 31.6 ms                                                     | 33.8 ms: 1.07x slower                                                       |
| pickle_dict             | 16.9 us                                                     | 18.4 us: 1.09x slower                                                       |
| unpack_sequence         | 37.8 ns                                                     | 41.9 ns: 1.11x slower                                                       |
| coverage                | 40.0 ms                                                     | 53.6 ms: 1.34x slower                                                       |
| Geometric mean          | (ref)                                                       | 1.12x faster                                                                |

Benchmark hidden because not significant (3): xml_etree_iterparse, regex_effbot, flaskblogging
Ignored benchmarks (4) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: asyncio_tcp_ssl, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.08x
