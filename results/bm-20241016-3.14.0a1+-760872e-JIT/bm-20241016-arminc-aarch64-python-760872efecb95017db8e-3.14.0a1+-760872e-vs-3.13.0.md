# Results vs. 3.13.0

- fork: python
- ref: 760872efecb95017db8e
- machine: linux-aarch64
- commit hash: 760872e
- commit date: 2024-10-16
- overall geometric mean: 1.17x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x slower
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 381 ms: 1.24x slower                                                     |
| docutils       | 2.91 sec                                                 | 3.65 sec: 1.26x slower                                                   |
| html5lib       | 64.3 ms                                                  | 71.5 ms: 1.11x slower                                                    |
| tornado_http   | 131 ms                                                   | 147 ms: 1.12x slower                                                     |
| Geometric mean | (ref)                                                    | 1.18x slower                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|---------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_memoization_tg | 649 ms                                                   | 540 ms: 1.20x faster                                                     |
| async_tree_none           | 493 ms                                                   | 455 ms: 1.08x faster                                                     |
| async_tree_none_tg        | 467 ms                                                   | 446 ms: 1.05x faster                                                     |
| async_tree_memoization    | 626 ms                                                   | 597 ms: 1.05x faster                                                     |
| async_tree_cpu_io_mixed   | 764 ms                                                   | 752 ms: 1.02x faster                                                     |
| coroutines                | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                    |
| async_generators          | 496 ms                                                   | 510 ms: 1.03x slower                                                     |
| asyncio_tcp_ssl           | 2.18 sec                                                 | 2.27 sec: 1.04x slower                                                   |
| async_tree_io             | 1.11 sec                                                 | 1.18 sec: 1.07x slower                                                   |
| asyncio_tcp               | 568 ms                                                   | 629 ms: 1.11x slower                                                     |
| async_tree_io_tg          | 1.09 sec                                                 | 1.25 sec: 1.14x slower                                                   |
| Geometric mean            | (ref)                                                    | 1.00x slower                                                             |

Benchmark hidden because not significant (2): async_tree_cpu_io_mixed_tg, asyncio_websockets

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 98.0 ms: 1.04x slower                                                    |
| Geometric mean | (ref)                                                    | 1.01x slower                                                             |

Benchmark hidden because not significant (2): pidigits, nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_dna      | 246 ms                                                   | 244 ms: 1.01x faster                                                     |
| regex_compile  | 128 ms                                                   | 169 ms: 1.31x slower                                                     |
| Geometric mean | (ref)                                                    | 1.07x slower                                                             |

Benchmark hidden because not significant (2): regex_effbot, regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| unpickle             | 20.2 us                                                  | 19.0 us: 1.06x faster                                                    |
| pickle_list          | 5.34 us                                                  | 5.26 us: 1.01x faster                                                    |
| pickle               | 13.5 us                                                  | 13.8 us: 1.02x slower                                                    |
| xml_etree_iterparse  | 147 ms                                                   | 150 ms: 1.02x slower                                                     |
| tomli_loads          | 2.53 sec                                                 | 2.64 sec: 1.05x slower                                                   |
| unpickle_pure_python | 254 us                                                   | 268 us: 1.05x slower                                                     |
| json_dumps           | 13.4 ms                                                  | 14.3 ms: 1.07x slower                                                    |
| pickle_pure_python   | 359 us                                                   | 389 us: 1.08x slower                                                     |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                             |

Benchmark hidden because not significant (6): xml_etree_generate, pickle_dict, json_loads, xml_etree_parse, unpickle_list, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup | 13.3 ms                                                  | 14.6 ms: 1.10x slower                                                    |
| Geometric mean | (ref)                                                    | 1.05x slower                                                             |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                    |
| genshi_text     | 27.7 ms                                                  | 33.5 ms: 1.21x slower                                                    |
| django_template | 42.3 ms                                                  | 51.1 ms: 1.21x slower                                                    |
| genshi_xml      | 60.2 ms                                                  | 81.5 ms: 1.35x slower                                                    |
| Geometric mean  | (ref)                                                    | 1.18x slower                                                             |

All benchmarks:
===============

| Benchmark                 | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e |
|---------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| deepcopy_memo             | 51.0 us                                                  | 38.9 us: 1.31x faster                                                    |
| async_tree_memoization_tg | 649 ms                                                   | 540 ms: 1.20x faster                                                     |
| deepcopy                  | 451 us                                                   | 377 us: 1.20x faster                                                     |
| async_tree_none           | 493 ms                                                   | 455 ms: 1.08x faster                                                     |
| unpickle                  | 20.2 us                                                  | 19.0 us: 1.06x faster                                                    |
| async_tree_none_tg        | 467 ms                                                   | 446 ms: 1.05x faster                                                     |
| async_tree_memoization    | 626 ms                                                   | 597 ms: 1.05x faster                                                     |
| pathlib                   | 22.4 ms                                                  | 21.6 ms: 1.04x faster                                                    |
| deepcopy_reduce           | 4.07 us                                                  | 3.96 us: 1.03x faster                                                    |
| logging_silent            | 135 ns                                                   | 132 ns: 1.02x faster                                                     |
| async_tree_cpu_io_mixed   | 764 ms                                                   | 752 ms: 1.02x faster                                                     |
| pickle_list               | 5.34 us                                                  | 5.26 us: 1.01x faster                                                    |
| mako                      | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                    |
| telco                     | 9.73 ms                                                  | 9.61 ms: 1.01x faster                                                    |
| regex_dna                 | 246 ms                                                   | 244 ms: 1.01x faster                                                     |
| bpe_tokeniser             | 5.90 sec                                                 | 5.92 sec: 1.00x slower                                                   |
| coroutines                | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                    |
| thrift                    | 946 us                                                   | 957 us: 1.01x slower                                                     |
| scimark_fft               | 447 ms                                                   | 456 ms: 1.02x slower                                                     |
| pickle                    | 13.5 us                                                  | 13.8 us: 1.02x slower                                                    |
| xml_etree_iterparse       | 147 ms                                                   | 150 ms: 1.02x slower                                                     |
| async_generators          | 496 ms                                                   | 510 ms: 1.03x slower                                                     |
| mdp                       | 3.36 sec                                                 | 3.49 sec: 1.04x slower                                                   |
| float                     | 94.4 ms                                                  | 98.0 ms: 1.04x slower                                                    |
| asyncio_tcp_ssl           | 2.18 sec                                                 | 2.27 sec: 1.04x slower                                                   |
| logging_format            | 7.83 us                                                  | 8.18 us: 1.04x slower                                                    |
| tomli_loads               | 2.53 sec                                                 | 2.64 sec: 1.05x slower                                                   |
| unpickle_pure_python      | 254 us                                                   | 268 us: 1.05x slower                                                     |
| logging_simple            | 7.04 us                                                  | 7.47 us: 1.06x slower                                                    |
| async_tree_io             | 1.11 sec                                                 | 1.18 sec: 1.07x slower                                                   |
| json_dumps                | 13.4 ms                                                  | 14.3 ms: 1.07x slower                                                    |
| bench_thread_pool         | 1.28 ms                                                  | 1.37 ms: 1.07x slower                                                    |
| scimark_monte_carlo       | 83.8 ms                                                  | 90.1 ms: 1.08x slower                                                    |
| pickle_pure_python        | 359 us                                                   | 389 us: 1.08x slower                                                     |
| scimark_lu                | 139 ms                                                   | 151 ms: 1.08x slower                                                     |
| crypto_pyaes              | 82.7 ms                                                  | 90.2 ms: 1.09x slower                                                    |
| scimark_sparse_mat_mult   | 6.58 ms                                                  | 7.19 ms: 1.09x slower                                                    |
| fannkuch                  | 452 ms                                                   | 495 ms: 1.10x slower                                                     |
| meteor_contest            | 113 ms                                                   | 125 ms: 1.10x slower                                                     |
| spectral_norm             | 141 ms                                                   | 155 ms: 1.10x slower                                                     |
| pyflate                   | 556 ms                                                   | 612 ms: 1.10x slower                                                     |
| python_startup            | 13.3 ms                                                  | 14.6 ms: 1.10x slower                                                    |
| asyncio_tcp               | 568 ms                                                   | 629 ms: 1.11x slower                                                     |
| html5lib                  | 64.3 ms                                                  | 71.5 ms: 1.11x slower                                                    |
| typing_runtime_protocols  | 192 us                                                   | 214 us: 1.12x slower                                                     |
| tornado_http              | 131 ms                                                   | 147 ms: 1.12x slower                                                     |
| go                        | 163 ms                                                   | 184 ms: 1.13x slower                                                     |
| async_tree_io_tg          | 1.09 sec                                                 | 1.25 sec: 1.14x slower                                                   |
| pycparser                 | 1.26 sec                                                 | 1.48 sec: 1.17x slower                                                   |
| deltablue                 | 3.85 ms                                                  | 4.52 ms: 1.17x slower                                                    |
| richards_super            | 60.3 ms                                                  | 71.5 ms: 1.18x slower                                                    |
| raytrace                  | 298 ms                                                   | 358 ms: 1.20x slower                                                     |
| richards                  | 53.5 ms                                                  | 64.3 ms: 1.20x slower                                                    |
| genshi_text               | 27.7 ms                                                  | 33.5 ms: 1.21x slower                                                    |
| django_template           | 42.3 ms                                                  | 51.1 ms: 1.21x slower                                                    |
| sqlglot_normalize         | 128 ms                                                   | 156 ms: 1.21x slower                                                     |
| comprehensions            | 20.4 us                                                  | 24.7 us: 1.21x slower                                                    |
| sqlglot_parse             | 1.38 ms                                                  | 1.70 ms: 1.23x slower                                                    |
| chaos                     | 68.8 ms                                                  | 85.0 ms: 1.24x slower                                                    |
| nqueens                   | 98.7 ms                                                  | 122 ms: 1.24x slower                                                     |
| 2to3                      | 306 ms                                                   | 381 ms: 1.24x slower                                                     |
| docutils                  | 2.91 sec                                                 | 3.65 sec: 1.26x slower                                                   |
| sqlglot_transpile         | 1.73 ms                                                  | 2.18 ms: 1.26x slower                                                    |
| pprint_safe_repr          | 926 ms                                                   | 1.20 sec: 1.29x slower                                                   |
| sympy_expand              | 455 ms                                                   | 592 ms: 1.30x slower                                                     |
| sqlglot_optimize          | 62.4 ms                                                  | 81.8 ms: 1.31x slower                                                    |
| regex_compile             | 128 ms                                                   | 169 ms: 1.31x slower                                                     |
| pprint_pformat            | 1.90 sec                                                 | 2.54 sec: 1.34x slower                                                   |
| genshi_xml                | 60.2 ms                                                  | 81.5 ms: 1.35x slower                                                    |
| sympy_str                 | 264 ms                                                   | 358 ms: 1.36x slower                                                     |
| sympy_integrate           | 21.0 ms                                                  | 29.1 ms: 1.39x slower                                                    |
| gc_traversal              | 4.53 ms                                                  | 6.43 ms: 1.42x slower                                                    |
| hexiom                    | 7.13 ms                                                  | 10.2 ms: 1.43x slower                                                    |
| pylint                    | 343 ms                                                   | 492 ms: 1.43x slower                                                     |
| sympy_sum                 | 143 ms                                                   | 215 ms: 1.50x slower                                                     |
| generators                | 37.8 ms                                                  | 59.0 ms: 1.56x slower                                                    |
| create_gc_cycles          | 2.12 ms                                                  | 3.71 ms: 1.75x slower                                                    |
| unpack_sequence           | 57.2 ns                                                  | 145 ns: 2.53x slower                                                     |
| bench_mp_pool             | 7.30 ms                                                  | 1.59 sec: 217.67x slower                                                 |
| Geometric mean            | (ref)                                                    | 1.17x slower                                                             |

Benchmark hidden because not significant (17): xml_etree_generate, scimark_sor, pickle_dict, regex_effbot, regex_v8, json, async_tree_cpu_io_mixed_tg, python_startup_no_site, json_loads, xml_etree_parse, pidigits, unpickle_list, coverage, nbody, xml_etree_process, asyncio_websockets, sqlite_synth
Ignored benchmarks (6) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2
Ignored benchmarks (2) of results/bm-20241016-3.14.0a1+-760872e-JIT/bm-20241016-arminc-aarch64-python-760872efecb95017db8e-3.14.0a1+-760872e.json: dulwich_log, sphinx

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.05x
- 95% likely to have a slowdown of 1.04x
- 99% likely to have a slowdown of 1.03x

# Memory
- memory change: 1.20x