# Results vs. 3.13.0

- fork: brandtbucher
- ref: tier_two_improvement
- machine: linux-x86_64
- commit hash: 6f4aaba
- commit date: 2024-07-15
- overall geometric mean: 1.01x faster
- HPT reliability: 63.79%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.07x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 265 ms                                                 | 272 ms: 1.03x slower                                                        |
| docutils       | 2.58 sec                                               | 2.88 sec: 1.12x slower                                                      |
| html5lib       | 64.5 ms                                                | 63.2 ms: 1.02x faster                                                       |
| tornado_http   | 91.5 ms                                                | 93.5 ms: 1.02x slower                                                       |
| Geometric mean | (ref)                                                  | 1.03x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|---------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_memoization_tg | 465 ms                                                 | 378 ms: 1.23x faster                                                        |
| async_tree_none           | 354 ms                                                 | 327 ms: 1.08x faster                                                        |
| async_tree_memoization    | 442 ms                                                 | 408 ms: 1.08x faster                                                        |
| async_tree_none_tg        | 320 ms                                                 | 303 ms: 1.06x faster                                                        |
| coroutines                | 22.5 ms                                                | 22.2 ms: 1.01x faster                                                       |
| asyncio_websockets        | 555 ms                                                 | 558 ms: 1.00x slower                                                        |
| asyncio_tcp_ssl           | 1.79 sec                                               | 1.81 sec: 1.01x slower                                                      |
| async_tree_io_tg          | 825 ms                                                 | 849 ms: 1.03x slower                                                        |
| asyncio_tcp               | 488 ms                                                 | 509 ms: 1.04x slower                                                        |
| async_generators          | 433 ms                                                 | 455 ms: 1.05x slower                                                        |
| Geometric mean            | (ref)                                                  | 1.02x faster                                                                |

Benchmark hidden because not significant (3): async_tree_cpu_io_mixed, async_tree_cpu_io_mixed_tg, async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 78.5 ms                                                | 70.4 ms: 1.12x faster                                                       |
| nbody          | 85.7 ms                                                | 81.2 ms: 1.06x faster                                                       |
| pidigits       | 186 ms                                                 | 186 ms: 1.00x slower                                                        |
| Geometric mean | (ref)                                                  | 1.06x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_effbot   | 3.88 ms                                                | 3.81 ms: 1.02x faster                                                       |
| regex_v8       | 25.3 ms                                                | 25.4 ms: 1.00x slower                                                       |
| regex_compile  | 131 ms                                                 | 134 ms: 1.02x slower                                                        |
| regex_dna      | 220 ms                                                 | 229 ms: 1.04x slower                                                        |
| Geometric mean | (ref)                                                  | 1.01x slower                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.91 sec: 1.11x faster                                                      |
| xml_etree_generate   | 87.0 ms                                                | 81.0 ms: 1.07x faster                                                       |
| xml_etree_process    | 60.4 ms                                                | 56.9 ms: 1.06x faster                                                       |
| xml_etree_parse      | 156 ms                                                 | 147 ms: 1.06x faster                                                        |
| xml_etree_iterparse  | 102 ms                                                 | 98.9 ms: 1.03x faster                                                       |
| json_dumps           | 10.4 ms                                                | 10.3 ms: 1.01x faster                                                       |
| pickle_pure_python   | 300 us                                                 | 298 us: 1.01x faster                                                        |
| json_loads           | 27.0 us                                                | 27.9 us: 1.03x slower                                                       |
| unpickle_pure_python | 213 us                                                 | 226 us: 1.06x slower                                                        |
| Geometric mean       | (ref)                                                  | 1.03x faster                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 10.6 ms                                                | 10.5 ms: 1.00x faster                                                       |
| python_startup_no_site | 6.99 ms                                                | 7.13 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                  | 1.01x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 11.1 ms                                                | 9.74 ms: 1.14x faster                                                       |
| django_template | 34.4 ms                                                | 35.8 ms: 1.04x slower                                                       |
| genshi_text     | 22.9 ms                                                | 25.8 ms: 1.13x slower                                                       |
| genshi_xml      | 50.3 ms                                                | 57.4 ms: 1.14x slower                                                       |
| Geometric mean  | (ref)                                                  | 1.04x slower                                                                |

All benchmarks:
===============

| Benchmark                 | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240715-linux-x86_64-brandtbucher-tier_two_improvement-3.14.0a0-6f4aaba |
|---------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo             | 38.0 us                                                | 28.8 us: 1.32x faster                                                       |
| deepcopy                  | 352 us                                                 | 274 us: 1.29x faster                                                        |
| async_tree_memoization_tg | 465 ms                                                 | 378 ms: 1.23x faster                                                        |
| scimark_fft               | 369 ms                                                 | 310 ms: 1.19x faster                                                        |
| richards                  | 48.1 ms                                                | 41.1 ms: 1.17x faster                                                       |
| deepcopy_reduce           | 3.17 us                                                | 2.75 us: 1.15x faster                                                       |
| richards_super            | 54.4 ms                                                | 47.3 ms: 1.15x faster                                                       |
| mako                      | 11.1 ms                                                | 9.74 ms: 1.14x faster                                                       |
| spectral_norm             | 115 ms                                                 | 101 ms: 1.13x faster                                                        |
| scimark_sparse_mat_mult   | 5.03 ms                                                | 4.44 ms: 1.13x faster                                                       |
| float                     | 78.5 ms                                                | 70.4 ms: 1.12x faster                                                       |
| tomli_loads               | 2.11 sec                                               | 1.91 sec: 1.11x faster                                                      |
| crypto_pyaes              | 73.0 ms                                                | 67.0 ms: 1.09x faster                                                       |
| async_tree_none           | 354 ms                                                 | 327 ms: 1.08x faster                                                        |
| mdp                       | 2.74 sec                                               | 2.53 sec: 1.08x faster                                                      |
| async_tree_memoization    | 442 ms                                                 | 408 ms: 1.08x faster                                                        |
| scimark_monte_carlo       | 66.3 ms                                                | 61.3 ms: 1.08x faster                                                       |
| xml_etree_generate        | 87.0 ms                                                | 81.0 ms: 1.07x faster                                                       |
| pprint_safe_repr          | 744 ms                                                 | 694 ms: 1.07x faster                                                        |
| pathlib                   | 17.1 ms                                                | 16.0 ms: 1.07x faster                                                       |
| gc_traversal              | 3.81 ms                                                | 3.58 ms: 1.06x faster                                                       |
| pyflate                   | 460 ms                                                 | 433 ms: 1.06x faster                                                        |
| xml_etree_process         | 60.4 ms                                                | 56.9 ms: 1.06x faster                                                       |
| telco                     | 8.45 ms                                                | 7.96 ms: 1.06x faster                                                       |
| xml_etree_parse           | 156 ms                                                 | 147 ms: 1.06x faster                                                        |
| async_tree_none_tg        | 320 ms                                                 | 303 ms: 1.06x faster                                                        |
| nbody                     | 85.7 ms                                                | 81.2 ms: 1.06x faster                                                       |
| fannkuch                  | 381 ms                                                 | 367 ms: 1.04x faster                                                        |
| pprint_pformat            | 1.51 sec                                               | 1.46 sec: 1.03x faster                                                      |
| xml_etree_iterparse       | 102 ms                                                 | 98.9 ms: 1.03x faster                                                       |
| logging_simple            | 5.66 us                                                | 5.53 us: 1.02x faster                                                       |
| html5lib                  | 64.5 ms                                                | 63.2 ms: 1.02x faster                                                       |
| regex_effbot              | 3.88 ms                                                | 3.81 ms: 1.02x faster                                                       |
| bpe_tokeniser             | 4.69 sec                                               | 4.60 sec: 1.02x faster                                                      |
| logging_format            | 6.25 us                                                | 6.15 us: 1.02x faster                                                       |
| pycparser                 | 1.19 sec                                               | 1.18 sec: 1.01x faster                                                      |
| coroutines                | 22.5 ms                                                | 22.2 ms: 1.01x faster                                                       |
| json_dumps                | 10.4 ms                                                | 10.3 ms: 1.01x faster                                                       |
| meteor_contest            | 108 ms                                                 | 106 ms: 1.01x faster                                                        |
| pickle_pure_python        | 300 us                                                 | 298 us: 1.01x faster                                                        |
| python_startup            | 10.6 ms                                                | 10.5 ms: 1.00x faster                                                       |
| pidigits                  | 186 ms                                                 | 186 ms: 1.00x slower                                                        |
| asyncio_websockets        | 555 ms                                                 | 558 ms: 1.00x slower                                                        |
| regex_v8                  | 25.3 ms                                                | 25.4 ms: 1.00x slower                                                       |
| comprehensions            | 16.4 us                                                | 16.5 us: 1.01x slower                                                       |
| asyncio_tcp_ssl           | 1.79 sec                                               | 1.81 sec: 1.01x slower                                                      |
| sqlglot_parse             | 1.27 ms                                                | 1.28 ms: 1.01x slower                                                       |
| thrift                    | 796 us                                                 | 808 us: 1.01x slower                                                        |
| sqlglot_transpile         | 1.57 ms                                                | 1.61 ms: 1.02x slower                                                       |
| tornado_http              | 91.5 ms                                                | 93.5 ms: 1.02x slower                                                       |
| python_startup_no_site    | 6.99 ms                                                | 7.13 ms: 1.02x slower                                                       |
| bench_thread_pool         | 815 us                                                 | 834 us: 1.02x slower                                                        |
| regex_compile             | 131 ms                                                 | 134 ms: 1.02x slower                                                        |
| 2to3                      | 265 ms                                                 | 272 ms: 1.03x slower                                                        |
| async_tree_io_tg          | 825 ms                                                 | 849 ms: 1.03x slower                                                        |
| raytrace                  | 262 ms                                                 | 269 ms: 1.03x slower                                                        |
| go                        | 142 ms                                                 | 146 ms: 1.03x slower                                                        |
| json_loads                | 27.0 us                                                | 27.9 us: 1.03x slower                                                       |
| sqlglot_optimize          | 53.9 ms                                                | 56.0 ms: 1.04x slower                                                       |
| generators                | 28.8 ms                                                | 29.9 ms: 1.04x slower                                                       |
| django_template           | 34.4 ms                                                | 35.8 ms: 1.04x slower                                                       |
| asyncio_tcp               | 488 ms                                                 | 509 ms: 1.04x slower                                                        |
| regex_dna                 | 220 ms                                                 | 229 ms: 1.04x slower                                                        |
| sqlglot_normalize         | 107 ms                                                 | 113 ms: 1.05x slower                                                        |
| async_generators          | 433 ms                                                 | 455 ms: 1.05x slower                                                        |
| logging_silent            | 102 ns                                                 | 108 ns: 1.06x slower                                                        |
| unpickle_pure_python      | 213 us                                                 | 226 us: 1.06x slower                                                        |
| nqueens                   | 80.6 ms                                                | 86.5 ms: 1.07x slower                                                       |
| dulwich_log               | 63.0 ms                                                | 67.5 ms: 1.07x slower                                                       |
| scimark_sor               | 122 ms                                                 | 131 ms: 1.07x slower                                                        |
| pylint                    | 313 ms                                                 | 336 ms: 1.08x slower                                                        |
| hexiom                    | 6.13 ms                                                | 6.59 ms: 1.08x slower                                                       |
| dask                      | 338 ms                                                 | 364 ms: 1.08x slower                                                        |
| typing_runtime_protocols  | 159 us                                                 | 172 us: 1.08x slower                                                        |
| create_gc_cycles          | 1.61 ms                                                | 1.74 ms: 1.08x slower                                                       |
| sympy_str                 | 274 ms                                                 | 298 ms: 1.09x slower                                                        |
| sympy_expand              | 462 ms                                                 | 503 ms: 1.09x slower                                                        |
| scimark_lu                | 115 ms                                                 | 127 ms: 1.11x slower                                                        |
| docutils                  | 2.58 sec                                               | 2.88 sec: 1.12x slower                                                      |
| sympy_integrate           | 19.9 ms                                                | 22.3 ms: 1.12x slower                                                       |
| genshi_text               | 22.9 ms                                                | 25.8 ms: 1.13x slower                                                       |
| coverage                  | 83.7 ms                                                | 94.5 ms: 1.13x slower                                                       |
| sympy_sum                 | 150 ms                                                 | 169 ms: 1.13x slower                                                        |
| genshi_xml                | 50.3 ms                                                | 57.4 ms: 1.14x slower                                                       |
| deltablue                 | 3.15 ms                                                | 3.65 ms: 1.16x slower                                                       |
| Geometric mean            | (ref)                                                  | 1.01x faster                                                                |

Benchmark hidden because not significant (6): async_tree_cpu_io_mixed, chaos, bench_mp_pool, json, async_tree_cpu_io_mixed_tg, async_tree_io
Ignored benchmarks (13) of results/bm-20241007-3.13.0-60403a5/bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, djangocms, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 63.79% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.07x