# Results vs. 3.10.4

- fork: python
- ref: c8db0e817e7e0db50153
- machine: linux-aarch64
- commit hash: c8db0e8
- commit date: 2024-10-03
- overall geometric mean: 1.22x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.22x faster
- Memory change: 1.14x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 295 ms: 1.29x faster                                                    |
| docutils       | 3.53 sec                                                              | 2.99 sec: 1.18x faster                                                  |
| html5lib       | 86.5 ms                                                               | 65.2 ms: 1.33x faster                                                   |
| tornado_http   | 178 ms                                                                | 124 ms: 1.43x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.30x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 418 ms: 2.27x faster                                                    |
| async_tree_memoization  | 1.13 sec                                                              | 558 ms: 2.03x faster                                                    |
| async_tree_io           | 2.28 sec                                                              | 1.15 sec: 1.99x faster                                                  |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 726 ms: 1.75x faster                                                    |
| Geometric mean          | (ref)                                                                 | 2.00x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| nbody          | 166 ms                                                                | 111 ms: 1.50x faster                                                    |
| float          | 135 ms                                                                | 94.6 ms: 1.42x faster                                                   |
| Geometric mean | (ref)                                                                 | 1.29x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_compile  | 177 ms                                                                | 126 ms: 1.40x faster                                                    |
| regex_v8       | 32.2 ms                                                               | 30.4 ms: 1.06x faster                                                   |
| regex_dna      | 257 ms                                                                | 262 ms: 1.02x slower                                                    |
| regex_effbot   | 4.25 ms                                                               | 5.03 ms: 1.18x slower                                                   |
| Geometric mean | (ref)                                                                 | 1.05x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 254 us: 1.44x faster                                                    |
| pickle_pure_python   | 529 us                                                                | 370 us: 1.43x faster                                                    |
| tomli_loads          | 3.36 sec                                                              | 2.64 sec: 1.27x faster                                                  |
| json_dumps           | 16.7 ms                                                               | 13.4 ms: 1.24x faster                                                   |
| xml_etree_process    | 99.5 ms                                                               | 80.2 ms: 1.24x faster                                                   |
| xml_etree_parse      | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| xml_etree_generate   | 123 ms                                                                | 112 ms: 1.09x faster                                                    |
| unpickle_list        | 6.99 us                                                               | 6.59 us: 1.06x faster                                                   |
| xml_etree_iterparse  | 156 ms                                                                | 150 ms: 1.04x faster                                                    |
| json_loads           | 30.9 us                                                               | 31.3 us: 1.01x slower                                                   |
| pickle_list          | 5.24 us                                                               | 5.32 us: 1.01x slower                                                   |
| pickle_dict          | 35.1 us                                                               | 37.8 us: 1.08x slower                                                   |
| pickle               | 12.5 us                                                               | 13.6 us: 1.09x slower                                                   |
| unpickle             | 17.5 us                                                               | 19.4 us: 1.11x slower                                                   |
| Geometric mean       | (ref)                                                                 | 1.11x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 13.1 ms: 1.17x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 8.64 ms: 1.25x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.21x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.5 ms: 1.40x faster                                                   |
| genshi_text     | 35.2 ms                                                               | 28.1 ms: 1.25x faster                                                   |
| django_template | 53.3 ms                                                               | 42.6 ms: 1.25x faster                                                   |
| genshi_xml      | 69.8 ms                                                               | 60.8 ms: 1.15x faster                                                   |
| Geometric mean  | (ref)                                                                 | 1.26x faster                                                            |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 194 us: 3.40x faster                                                    |
| deltablue                | 8.94 ms                                                               | 3.93 ms: 2.28x faster                                                   |
| async_tree_none          | 950 ms                                                                | 418 ms: 2.27x faster                                                    |
| async_tree_memoization   | 1.13 sec                                                              | 558 ms: 2.03x faster                                                    |
| async_tree_io            | 2.28 sec                                                              | 1.15 sec: 1.99x faster                                                  |
| generators               | 68.1 ms                                                               | 34.9 ms: 1.95x faster                                                   |
| go                       | 264 ms                                                                | 139 ms: 1.90x faster                                                    |
| raytrace                 | 573 ms                                                                | 304 ms: 1.89x faster                                                    |
| richards_super           | 107 ms                                                                | 59.7 ms: 1.80x faster                                                   |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 726 ms: 1.75x faster                                                    |
| richards                 | 91.7 ms                                                               | 52.7 ms: 1.74x faster                                                   |
| chaos                    | 121 ms                                                                | 69.7 ms: 1.74x faster                                                   |
| scimark_lu               | 227 ms                                                                | 133 ms: 1.71x faster                                                    |
| logging_silent           | 222 ns                                                                | 130 ns: 1.70x faster                                                    |
| sqlglot_parse            | 2.40 ms                                                               | 1.41 ms: 1.70x faster                                                   |
| asyncio_tcp              | 944 ms                                                                | 558 ms: 1.69x faster                                                    |
| deepcopy_memo            | 61.7 us                                                               | 37.6 us: 1.64x faster                                                   |
| sqlglot_transpile        | 2.84 ms                                                               | 1.74 ms: 1.63x faster                                                   |
| crypto_pyaes             | 134 ms                                                                | 82.5 ms: 1.62x faster                                                   |
| comprehensions           | 33.1 us                                                               | 20.7 us: 1.60x faster                                                   |
| scimark_monte_carlo      | 128 ms                                                                | 82.0 ms: 1.56x faster                                                   |
| scimark_sor              | 246 ms                                                                | 159 ms: 1.55x faster                                                    |
| deepcopy                 | 511 us                                                                | 333 us: 1.53x faster                                                    |
| hexiom                   | 10.9 ms                                                               | 7.14 ms: 1.53x faster                                                   |
| nbody                    | 166 ms                                                                | 111 ms: 1.50x faster                                                    |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.21 sec: 1.49x faster                                                  |
| unpickle_pure_python     | 366 us                                                                | 254 us: 1.44x faster                                                    |
| tornado_http             | 178 ms                                                                | 124 ms: 1.43x faster                                                    |
| pickle_pure_python       | 529 us                                                                | 370 us: 1.43x faster                                                    |
| pyflate                  | 795 ms                                                                | 558 ms: 1.43x faster                                                    |
| float                    | 135 ms                                                                | 94.6 ms: 1.42x faster                                                   |
| mako                     | 18.9 ms                                                               | 13.5 ms: 1.40x faster                                                   |
| regex_compile            | 177 ms                                                                | 126 ms: 1.40x faster                                                    |
| logging_simple           | 9.78 us                                                               | 7.05 us: 1.39x faster                                                   |
| logging_format           | 10.6 us                                                               | 7.70 us: 1.38x faster                                                   |
| pycparser                | 1.69 sec                                                              | 1.23 sec: 1.38x faster                                                  |
| thrift                   | 1.26 ms                                                               | 931 us: 1.35x faster                                                    |
| pylint                   | 485 ms                                                                | 359 ms: 1.35x faster                                                    |
| spectral_norm            | 186 ms                                                                | 140 ms: 1.33x faster                                                    |
| html5lib                 | 86.5 ms                                                               | 65.2 ms: 1.33x faster                                                   |
| sympy_sum                | 184 ms                                                                | 141 ms: 1.31x faster                                                    |
| deepcopy_reduce          | 4.60 us                                                               | 3.52 us: 1.31x faster                                                   |
| coroutines               | 37.2 ms                                                               | 28.6 ms: 1.30x faster                                                   |
| 2to3                     | 381 ms                                                                | 295 ms: 1.29x faster                                                    |
| tomli_loads              | 3.36 sec                                                              | 2.64 sec: 1.27x faster                                                  |
| sympy_integrate          | 26.5 ms                                                               | 21.0 ms: 1.26x faster                                                   |
| pprint_safe_repr         | 1.15 sec                                                              | 915 ms: 1.25x faster                                                    |
| genshi_text              | 35.2 ms                                                               | 28.1 ms: 1.25x faster                                                   |
| django_template          | 53.3 ms                                                               | 42.6 ms: 1.25x faster                                                   |
| dulwich_log              | 73.5 ms                                                               | 58.8 ms: 1.25x faster                                                   |
| pprint_pformat           | 2.36 sec                                                              | 1.89 sec: 1.24x faster                                                  |
| json_dumps               | 16.7 ms                                                               | 13.4 ms: 1.24x faster                                                   |
| sympy_str                | 329 ms                                                                | 264 ms: 1.24x faster                                                    |
| xml_etree_process        | 99.5 ms                                                               | 80.2 ms: 1.24x faster                                                   |
| pathlib                  | 26.3 ms                                                               | 21.3 ms: 1.23x faster                                                   |
| sqlglot_normalize        | 156 ms                                                                | 127 ms: 1.23x faster                                                    |
| bench_thread_pool        | 1.59 ms                                                               | 1.30 ms: 1.22x faster                                                   |
| sqlglot_optimize         | 75.4 ms                                                               | 63.3 ms: 1.19x faster                                                   |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.41 ms: 1.19x faster                                                   |
| sympy_expand             | 543 ms                                                                | 457 ms: 1.19x faster                                                    |
| fannkuch                 | 546 ms                                                                | 463 ms: 1.18x faster                                                    |
| docutils                 | 3.53 sec                                                              | 2.99 sec: 1.18x faster                                                  |
| nqueens                  | 117 ms                                                                | 99.8 ms: 1.18x faster                                                   |
| scimark_fft              | 500 ms                                                                | 434 ms: 1.15x faster                                                    |
| genshi_xml               | 69.8 ms                                                               | 60.8 ms: 1.15x faster                                                   |
| xml_etree_parse          | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| meteor_contest           | 126 ms                                                                | 113 ms: 1.12x faster                                                    |
| mdp                      | 3.70 sec                                                              | 3.31 sec: 1.12x faster                                                  |
| xml_etree_generate       | 123 ms                                                                | 112 ms: 1.09x faster                                                    |
| sqlite_synth             | 4.12 us                                                               | 3.83 us: 1.07x faster                                                   |
| unpickle_list            | 6.99 us                                                               | 6.59 us: 1.06x faster                                                   |
| regex_v8                 | 32.2 ms                                                               | 30.4 ms: 1.06x faster                                                   |
| json                     | 5.88 ms                                                               | 5.56 ms: 1.06x faster                                                   |
| xml_etree_iterparse      | 156 ms                                                                | 150 ms: 1.04x faster                                                    |
| asyncio_websockets       | 657 ms                                                                | 661 ms: 1.01x slower                                                    |
| json_loads               | 30.9 us                                                               | 31.3 us: 1.01x slower                                                   |
| pickle_list              | 5.24 us                                                               | 5.32 us: 1.01x slower                                                   |
| regex_dna                | 257 ms                                                                | 262 ms: 1.02x slower                                                    |
| async_generators         | 452 ms                                                                | 477 ms: 1.05x slower                                                    |
| pickle_dict              | 35.1 us                                                               | 37.8 us: 1.08x slower                                                   |
| pickle                   | 12.5 us                                                               | 13.6 us: 1.09x slower                                                   |
| unpickle                 | 17.5 us                                                               | 19.4 us: 1.11x slower                                                   |
| telco                    | 8.49 ms                                                               | 9.47 ms: 1.12x slower                                                   |
| python_startup           | 11.2 ms                                                               | 13.1 ms: 1.17x slower                                                   |
| regex_effbot             | 4.25 ms                                                               | 5.03 ms: 1.18x slower                                                   |
| coverage                 | 83.6 ms                                                               | 98.9 ms: 1.18x slower                                                   |
| gc_traversal             | 4.15 ms                                                               | 5.16 ms: 1.24x slower                                                   |
| python_startup_no_site   | 6.89 ms                                                               | 8.64 ms: 1.25x slower                                                   |
| bench_mp_pool            | 14.5 ms                                                               | 5.71 sec: 392.80x slower                                                |
| Geometric mean           | (ref)                                                                 | 1.22x faster                                                            |

Benchmark hidden because not significant (2): pidigits, create_gc_cycles
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (6) of results/bm-20241003-3.14.0a0-c8db0e8/bm-20241003-arminc-aarch64-python-c8db0e817e7e0db50153-3.14.0a0-c8db0e8.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, unpack_sequence

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.25x
- 95% likely to have a speedup of 1.24x
- 99% likely to have a speedup of 1.22x

# Memory
- memory change: 1.14x