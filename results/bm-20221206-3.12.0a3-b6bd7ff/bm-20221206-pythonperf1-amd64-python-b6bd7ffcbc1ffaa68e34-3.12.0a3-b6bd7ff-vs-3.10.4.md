
# Results vs. 3.10.4

- fork: python
- ref: b6bd7ffcbc1ffaa68e34
- machine: windows-amd64
- commit hash: b6bd7ff
- commit date: 2022-12-06
- overall geometric mean: 1.20x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.14x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 196 ms: 1.21x faster                                                       |
| chameleon      | 5.92 ms                                                     | 4.53 ms: 1.31x faster                                                      |
| docutils       | 1.89 sec                                                    | 1.56 sec: 1.21x faster                                                     |
| html5lib       | 46.5 ms                                                     | 35.0 ms: 1.33x faster                                                      |
| Geometric mean | (ref)                                                       | 1.26x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 50.1 ms: 1.20x faster                                                      |
| nbody          | 69.3 ms                                                     | 62.3 ms: 1.11x faster                                                      |
| pidigits       | 145 ms                                                      | 151 ms: 1.04x slower                                                       |
| Geometric mean | (ref)                                                       | 1.09x faster                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 82.5 ms: 1.25x faster                                                      |
| regex_dna      | 132 ms                                                      | 120 ms: 1.10x faster                                                       |
| regex_effbot   | 1.66 ms                                                     | 1.54 ms: 1.08x faster                                                      |
| regex_v8       | 15.0 ms                                                     | 14.0 ms: 1.08x faster                                                      |
| Geometric mean | (ref)                                                       | 1.12x faster                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.17 ms: 1.65x faster                                                      |
| pickle_pure_python   | 257 us                                                      | 179 us: 1.44x faster                                                       |
| unpickle_pure_python | 171 us                                                      | 121 us: 1.41x faster                                                       |
| xml_etree_process    | 43.4 ms                                                     | 35.0 ms: 1.24x faster                                                      |
| unpickle             | 9.17 us                                                     | 7.99 us: 1.15x faster                                                      |
| xml_etree_parse      | 102 ms                                                      | 90.5 ms: 1.12x faster                                                      |
| json_loads           | 14.2 us                                                     | 12.9 us: 1.10x faster                                                      |
| xml_etree_generate   | 54.5 ms                                                     | 49.7 ms: 1.10x faster                                                      |
| unpickle_list        | 2.81 us                                                     | 2.61 us: 1.08x faster                                                      |
| xml_etree_iterparse  | 63.5 ms                                                     | 62.3 ms: 1.02x faster                                                      |
| pickle               | 6.80 us                                                     | 6.90 us: 1.01x slower                                                      |
| pickle_list          | 2.59 us                                                     | 2.85 us: 1.10x slower                                                      |
| pickle_dict          | 16.9 us                                                     | 19.4 us: 1.15x slower                                                      |
| Geometric mean       | (ref)                                                       | 1.14x faster                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.5 ms: 1.08x faster                                                      |
| python_startup_no_site | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                                      |
| Geometric mean         | (ref)                                                       | 1.03x faster                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 6.19 ms: 1.42x faster                                                      |
| genshi_text     | 19.0 ms                                                     | 13.8 ms: 1.38x faster                                                      |
| django_template | 28.2 ms                                                     | 21.4 ms: 1.32x faster                                                      |
| genshi_xml      | 40.5 ms                                                     | 32.3 ms: 1.26x faster                                                      |
| Geometric mean  | (ref)                                                       | 1.34x faster                                                               |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221206-pythonperf1-amd64-python-b6bd7ffcbc1ffaa68e34-3.12.0a3-b6bd7ff |
|-------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.07 ms: 2.01x faster                                                      |
| logging_silent          | 96.4 ns                                                     | 57.2 ns: 1.69x faster                                                      |
| richards                | 41.2 ms                                                     | 24.5 ms: 1.68x faster                                                      |
| json_dumps              | 8.50 ms                                                     | 5.17 ms: 1.65x faster                                                      |
| mypy2                   | 352 ms                                                      | 215 ms: 1.64x faster                                                       |
| scimark_sor             | 105 ms                                                      | 64.6 ms: 1.62x faster                                                      |
| go                      | 136 ms                                                      | 84.8 ms: 1.61x faster                                                      |
| scimark_lu              | 85.4 ms                                                     | 54.8 ms: 1.56x faster                                                      |
| raytrace                | 271 ms                                                      | 179 ms: 1.51x faster                                                       |
| pyflate                 | 387 ms                                                      | 264 ms: 1.47x faster                                                       |
| scimark_monte_carlo     | 55.9 ms                                                     | 38.1 ms: 1.47x faster                                                      |
| pickle_pure_python      | 257 us                                                      | 179 us: 1.44x faster                                                       |
| unpack_sequence         | 37.8 ns                                                     | 26.4 ns: 1.43x faster                                                      |
| mako                    | 8.80 ms                                                     | 6.19 ms: 1.42x faster                                                      |
| hexiom                  | 5.52 ms                                                     | 3.89 ms: 1.42x faster                                                      |
| sqlglot_parse           | 1.22 ms                                                     | 860 us: 1.42x faster                                                       |
| unpickle_pure_python    | 171 us                                                      | 121 us: 1.41x faster                                                       |
| crypto_pyaes            | 62.3 ms                                                     | 45.1 ms: 1.38x faster                                                      |
| sqlglot_transpile       | 1.46 ms                                                     | 1.06 ms: 1.38x faster                                                      |
| genshi_text             | 19.0 ms                                                     | 13.8 ms: 1.38x faster                                                      |
| async_tree_io           | 1.07 sec                                                    | 780 ms: 1.37x faster                                                       |
| chaos                   | 58.9 ms                                                     | 43.2 ms: 1.36x faster                                                      |
| deepcopy_memo           | 28.5 us                                                     | 21.2 us: 1.35x faster                                                      |
| html5lib                | 46.5 ms                                                     | 35.0 ms: 1.33x faster                                                      |
| async_tree_none         | 420 ms                                                      | 317 ms: 1.33x faster                                                       |
| django_template         | 28.2 ms                                                     | 21.4 ms: 1.32x faster                                                      |
| thrift                  | 615 us                                                      | 469 us: 1.31x faster                                                       |
| chameleon               | 5.92 ms                                                     | 4.53 ms: 1.31x faster                                                      |
| pprint_safe_repr        | 589 ms                                                      | 458 ms: 1.29x faster                                                       |
| async_tree_memoization  | 497 ms                                                      | 388 ms: 1.28x faster                                                       |
| pprint_pformat          | 1.21 sec                                                    | 947 ms: 1.27x faster                                                       |
| spectral_norm           | 78.0 ms                                                     | 61.4 ms: 1.27x faster                                                      |
| genshi_xml              | 40.5 ms                                                     | 32.3 ms: 1.26x faster                                                      |
| pycparser               | 868 ms                                                      | 691 ms: 1.26x faster                                                       |
| regex_compile           | 103 ms                                                      | 82.5 ms: 1.25x faster                                                      |
| async_generators        | 224 ms                                                      | 179 ms: 1.25x faster                                                       |
| xml_etree_process       | 43.4 ms                                                     | 35.0 ms: 1.24x faster                                                      |
| sqlglot_optimize        | 39.0 ms                                                     | 32.0 ms: 1.22x faster                                                      |
| async_tree_cpu_io_mixed | 609 ms                                                      | 501 ms: 1.21x faster                                                       |
| 2to3                    | 237 ms                                                      | 196 ms: 1.21x faster                                                       |
| docutils                | 1.89 sec                                                    | 1.56 sec: 1.21x faster                                                     |
| float                   | 60.2 ms                                                     | 50.1 ms: 1.20x faster                                                      |
| deepcopy                | 255 us                                                      | 214 us: 1.19x faster                                                       |
| dask                    | 305 ms                                                      | 258 ms: 1.18x faster                                                       |
| sqlglot_normalize       | 202 ms                                                      | 172 ms: 1.18x faster                                                       |
| fannkuch                | 258 ms                                                      | 222 ms: 1.16x faster                                                       |
| nqueens                 | 67.0 ms                                                     | 57.9 ms: 1.16x faster                                                      |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.30 ms: 1.16x faster                                                      |
| dulwich_log             | 47.6 ms                                                     | 41.2 ms: 1.16x faster                                                      |
| unpickle                | 9.17 us                                                     | 7.99 us: 1.15x faster                                                      |
| deepcopy_reduce         | 2.16 us                                                     | 1.88 us: 1.15x faster                                                      |
| create_gc_cycles        | 782 us                                                      | 682 us: 1.15x faster                                                       |
| bench_thread_pool       | 946 us                                                      | 837 us: 1.13x faster                                                       |
| xml_etree_parse         | 102 ms                                                      | 90.5 ms: 1.12x faster                                                      |
| json                    | 3.05 ms                                                     | 2.74 ms: 1.11x faster                                                      |
| sympy_integrate         | 14.8 ms                                                     | 13.3 ms: 1.11x faster                                                      |
| nbody                   | 69.3 ms                                                     | 62.3 ms: 1.11x faster                                                      |
| sympy_expand            | 315 ms                                                      | 286 ms: 1.10x faster                                                       |
| regex_dna               | 132 ms                                                      | 120 ms: 1.10x faster                                                       |
| json_loads              | 14.2 us                                                     | 12.9 us: 1.10x faster                                                      |
| xml_etree_generate      | 54.5 ms                                                     | 49.7 ms: 1.10x faster                                                      |
| scimark_fft             | 193 ms                                                      | 177 ms: 1.09x faster                                                       |
| sympy_str               | 188 ms                                                      | 173 ms: 1.09x faster                                                       |
| python_startup          | 20.0 ms                                                     | 18.5 ms: 1.08x faster                                                      |
| regex_effbot            | 1.66 ms                                                     | 1.54 ms: 1.08x faster                                                      |
| unpickle_list           | 2.81 us                                                     | 2.61 us: 1.08x faster                                                      |
| regex_v8                | 15.0 ms                                                     | 14.0 ms: 1.08x faster                                                      |
| pathlib                 | 77.4 ms                                                     | 72.1 ms: 1.07x faster                                                      |
| mdp                     | 1.71 sec                                                    | 1.61 sec: 1.07x faster                                                     |
| sqlite_synth            | 1.84 us                                                     | 1.73 us: 1.06x faster                                                      |
| coroutines              | 15.9 ms                                                     | 15.1 ms: 1.06x faster                                                      |
| meteor_contest          | 72.5 ms                                                     | 68.8 ms: 1.05x faster                                                      |
| sympy_sum               | 104 ms                                                      | 99.0 ms: 1.05x faster                                                      |
| telco                   | 3.78 ms                                                     | 3.64 ms: 1.04x faster                                                      |
| comprehensions          | 16.0 us                                                     | 15.5 us: 1.03x faster                                                      |
| xml_etree_iterparse     | 63.5 ms                                                     | 62.3 ms: 1.02x faster                                                      |
| python_startup_no_site  | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                                      |
| pickle                  | 6.80 us                                                     | 6.90 us: 1.01x slower                                                      |
| bench_mp_pool           | 60.7 ms                                                     | 62.8 ms: 1.03x slower                                                      |
| pidigits                | 145 ms                                                      | 151 ms: 1.04x slower                                                       |
| gc_traversal            | 1.34 ms                                                     | 1.44 ms: 1.07x slower                                                      |
| generators              | 31.6 ms                                                     | 33.9 ms: 1.07x slower                                                      |
| pickle_list             | 2.59 us                                                     | 2.85 us: 1.10x slower                                                      |
| pickle_dict             | 16.9 us                                                     | 19.4 us: 1.15x slower                                                      |
| coverage                | 40.0 ms                                                     | 51.6 ms: 1.29x slower                                                      |
| Geometric mean          | (ref)                                                       | 1.20x faster                                                               |

Benchmark hidden because not significant (3): logging_format, logging_simple, asyncio_tcp
Ignored benchmarks (10) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.17x
- 95% likely to have a speedup of 1.15x
- 99% likely to have a speedup of 1.14x
