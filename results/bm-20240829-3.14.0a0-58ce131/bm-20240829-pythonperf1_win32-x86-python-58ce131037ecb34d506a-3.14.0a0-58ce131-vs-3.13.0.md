# Results vs. 3.13.0

- fork: python
- ref: 58ce131037ecb34d506a
- machine: windows-x86
- commit hash: 58ce131
- commit date: 2024-08-29
- overall geometric mean: 1.025x faster
- HPT reliability: 99.68%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 255 ms                                                          | 250 ms: 1.02x faster                                                           |
| docutils       | 1.80 sec                                                        | 1.86 sec: 1.04x slower                                                         |
| html5lib       | 47.1 ms                                                         | 46.8 ms: 1.01x faster                                                          |
| tornado_http   | 105 ms                                                          | 106 ms: 1.01x slower                                                           |
| Geometric mean | (ref)                                                           | 1.01x slower                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 287 ms                                                          | 247 ms: 1.16x faster                                                           |
| async_tree_none            | 245 ms                                                          | 213 ms: 1.15x faster                                                           |
| async_tree_memoization     | 302 ms                                                          | 273 ms: 1.10x faster                                                           |
| async_tree_none_tg         | 216 ms                                                          | 197 ms: 1.09x faster                                                           |
| async_tree_cpu_io_mixed    | 490 ms                                                          | 464 ms: 1.06x faster                                                           |
| async_tree_cpu_io_mixed_tg | 469 ms                                                          | 459 ms: 1.02x faster                                                           |
| async_tree_io_tg           | 499 ms                                                          | 513 ms: 1.03x slower                                                           |
| coroutines                 | 16.1 ms                                                         | 17.7 ms: 1.09x slower                                                          |
| async_generators           | 267 ms                                                          | 304 ms: 1.14x slower                                                           |
| Geometric mean             | (ref)                                                           | 1.03x faster                                                                   |

Benchmark hidden because not significant (1): async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| pidigits       | 204 ms                                                          | 198 ms: 1.03x faster                                                           |
| float          | 56.4 ms                                                         | 62.3 ms: 1.10x slower                                                          |
| nbody          | 81.4 ms                                                         | 96.4 ms: 1.18x slower                                                          |
| Geometric mean | (ref)                                                           | 1.08x slower                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_v8       | 15.5 ms                                                         | 16.1 ms: 1.04x slower                                                          |
| regex_compile  | 101 ms                                                          | 108 ms: 1.06x slower                                                           |
| regex_effbot   | 1.82 ms                                                         | 1.94 ms: 1.07x slower                                                          |
| regex_dna      | 112 ms                                                          | 124 ms: 1.10x slower                                                           |
| Geometric mean | (ref)                                                           | 1.07x slower                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| json_loads           | 21.7 us                                                         | 20.4 us: 1.06x faster                                                          |
| json_dumps           | 7.53 ms                                                         | 7.60 ms: 1.01x slower                                                          |
| xml_etree_parse      | 102 ms                                                          | 108 ms: 1.05x slower                                                           |
| unpickle_pure_python | 162 us                                                          | 174 us: 1.08x slower                                                           |
| pickle_pure_python   | 239 us                                                          | 262 us: 1.10x slower                                                           |
| xml_etree_generate   | 61.9 ms                                                         | 68.1 ms: 1.10x slower                                                          |
| xml_etree_iterparse  | 61.3 ms                                                         | 67.6 ms: 1.10x slower                                                          |
| tomli_loads          | 1.74 sec                                                        | 1.92 sec: 1.10x slower                                                         |
| xml_etree_process    | 44.6 ms                                                         | 50.6 ms: 1.13x slower                                                          |
| Geometric mean       | (ref)                                                           | 1.07x slower                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup         | 28.3 ms                                                         | 24.0 ms: 1.18x faster                                                          |
| python_startup_no_site | 20.2 ms                                                         | 19.9 ms: 1.01x faster                                                          |
| Geometric mean         | (ref)                                                           | 1.09x faster                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| genshi_xml      | 49.0 ms                                                         | 47.2 ms: 1.04x faster                                                          |
| django_template | 32.0 ms                                                         | 32.8 ms: 1.02x slower                                                          |
| genshi_text     | 21.7 ms                                                         | 22.2 ms: 1.02x slower                                                          |
| mako            | 7.02 ms                                                         | 8.35 ms: 1.19x slower                                                          |
| Geometric mean  | (ref)                                                           | 1.05x slower                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| thrift                     | 10.3 ms                                                         | 742 us: 13.83x faster                                                          |
| coverage                   | 326 ms                                                          | 51.2 ms: 6.37x faster                                                          |
| create_gc_cycles           | 1.08 ms                                                         | 747 us: 1.45x faster                                                           |
| bench_mp_pool              | 93.6 ms                                                         | 73.4 ms: 1.28x faster                                                          |
| deepcopy                   | 311 us                                                          | 247 us: 1.26x faster                                                           |
| gc_traversal               | 1.76 ms                                                         | 1.45 ms: 1.21x faster                                                          |
| deepcopy_memo              | 26.2 us                                                         | 21.8 us: 1.20x faster                                                          |
| python_startup             | 28.3 ms                                                         | 24.0 ms: 1.18x faster                                                          |
| deepcopy_reduce            | 2.94 us                                                         | 2.51 us: 1.17x faster                                                          |
| async_tree_memoization_tg  | 287 ms                                                          | 247 ms: 1.16x faster                                                           |
| async_tree_none            | 245 ms                                                          | 213 ms: 1.15x faster                                                           |
| async_tree_memoization     | 302 ms                                                          | 273 ms: 1.10x faster                                                           |
| async_tree_none_tg         | 216 ms                                                          | 197 ms: 1.09x faster                                                           |
| go                         | 111 ms                                                          | 102 ms: 1.09x faster                                                           |
| json_loads                 | 21.7 us                                                         | 20.4 us: 1.06x faster                                                          |
| async_tree_cpu_io_mixed    | 490 ms                                                          | 464 ms: 1.06x faster                                                           |
| genshi_xml                 | 49.0 ms                                                         | 47.2 ms: 1.04x faster                                                          |
| bench_thread_pool          | 1.04 ms                                                         | 1.01 ms: 1.03x faster                                                          |
| pidigits                   | 204 ms                                                          | 198 ms: 1.03x faster                                                           |
| json                       | 4.40 ms                                                         | 4.29 ms: 1.02x faster                                                          |
| pathlib                    | 89.1 ms                                                         | 87.1 ms: 1.02x faster                                                          |
| async_tree_cpu_io_mixed_tg | 469 ms                                                          | 459 ms: 1.02x faster                                                           |
| 2to3                       | 255 ms                                                          | 250 ms: 1.02x faster                                                           |
| python_startup_no_site     | 20.2 ms                                                         | 19.9 ms: 1.01x faster                                                          |
| pprint_safe_repr           | 658 ms                                                          | 650 ms: 1.01x faster                                                           |
| html5lib                   | 47.1 ms                                                         | 46.8 ms: 1.01x faster                                                          |
| dulwich_log                | 50.2 ms                                                         | 49.9 ms: 1.01x faster                                                          |
| json_dumps                 | 7.53 ms                                                         | 7.60 ms: 1.01x slower                                                          |
| logging_simple             | 7.89 us                                                         | 7.97 us: 1.01x slower                                                          |
| sympy_integrate            | 15.2 ms                                                         | 15.4 ms: 1.01x slower                                                          |
| tornado_http               | 105 ms                                                          | 106 ms: 1.01x slower                                                           |
| sqlglot_normalize          | 223 ms                                                          | 225 ms: 1.01x slower                                                           |
| sympy_expand               | 377 ms                                                          | 383 ms: 1.01x slower                                                           |
| mdp                        | 1.70 sec                                                        | 1.72 sec: 1.02x slower                                                         |
| logging_format             | 8.59 us                                                         | 8.73 us: 1.02x slower                                                          |
| pprint_pformat             | 1.32 sec                                                        | 1.34 sec: 1.02x slower                                                         |
| django_template            | 32.0 ms                                                         | 32.8 ms: 1.02x slower                                                          |
| genshi_text                | 21.7 ms                                                         | 22.2 ms: 1.02x slower                                                          |
| async_tree_io_tg           | 499 ms                                                          | 513 ms: 1.03x slower                                                           |
| crypto_pyaes               | 56.6 ms                                                         | 58.3 ms: 1.03x slower                                                          |
| nqueens                    | 75.1 ms                                                         | 77.3 ms: 1.03x slower                                                          |
| sqlglot_optimize           | 42.4 ms                                                         | 43.8 ms: 1.03x slower                                                          |
| docutils                   | 1.80 sec                                                        | 1.86 sec: 1.04x slower                                                         |
| pylint                     | 225 ms                                                          | 233 ms: 1.04x slower                                                           |
| meteor_contest             | 78.1 ms                                                         | 81.3 ms: 1.04x slower                                                          |
| regex_v8                   | 15.5 ms                                                         | 16.1 ms: 1.04x slower                                                          |
| xml_etree_parse            | 102 ms                                                          | 108 ms: 1.05x slower                                                           |
| typing_runtime_protocols   | 141 us                                                          | 148 us: 1.05x slower                                                           |
| sqlglot_transpile          | 1.26 ms                                                         | 1.34 ms: 1.06x slower                                                          |
| comprehensions             | 13.1 us                                                         | 14.0 us: 1.06x slower                                                          |
| regex_compile              | 101 ms                                                          | 108 ms: 1.06x slower                                                           |
| regex_effbot               | 1.82 ms                                                         | 1.94 ms: 1.07x slower                                                          |
| sqlglot_parse              | 1.02 ms                                                         | 1.09 ms: 1.07x slower                                                          |
| unpickle_pure_python       | 162 us                                                          | 174 us: 1.08x slower                                                           |
| coroutines                 | 16.1 ms                                                         | 17.7 ms: 1.09x slower                                                          |
| pickle_pure_python         | 239 us                                                          | 262 us: 1.10x slower                                                           |
| spectral_norm              | 70.0 ms                                                         | 76.7 ms: 1.10x slower                                                          |
| pyflate                    | 322 ms                                                          | 354 ms: 1.10x slower                                                           |
| xml_etree_generate         | 61.9 ms                                                         | 68.1 ms: 1.10x slower                                                          |
| xml_etree_iterparse        | 61.3 ms                                                         | 67.6 ms: 1.10x slower                                                          |
| tomli_loads                | 1.74 sec                                                        | 1.92 sec: 1.10x slower                                                         |
| regex_dna                  | 112 ms                                                          | 124 ms: 1.10x slower                                                           |
| float                      | 56.4 ms                                                         | 62.3 ms: 1.10x slower                                                          |
| chaos                      | 49.4 ms                                                         | 54.9 ms: 1.11x slower                                                          |
| telco                      | 6.27 ms                                                         | 6.97 ms: 1.11x slower                                                          |
| hexiom                     | 4.60 ms                                                         | 5.20 ms: 1.13x slower                                                          |
| xml_etree_process          | 44.6 ms                                                         | 50.6 ms: 1.13x slower                                                          |
| fannkuch                   | 288 ms                                                          | 327 ms: 1.13x slower                                                           |
| async_generators           | 267 ms                                                          | 304 ms: 1.14x slower                                                           |
| deltablue                  | 2.35 ms                                                         | 2.68 ms: 1.14x slower                                                          |
| scimark_fft                | 204 ms                                                          | 234 ms: 1.15x slower                                                           |
| scimark_monte_carlo        | 48.7 ms                                                         | 55.9 ms: 1.15x slower                                                          |
| scimark_sor                | 85.8 ms                                                         | 99.5 ms: 1.16x slower                                                          |
| raytrace                   | 203 ms                                                          | 236 ms: 1.16x slower                                                           |
| scimark_lu                 | 60.7 ms                                                         | 71.7 ms: 1.18x slower                                                          |
| nbody                      | 81.4 ms                                                         | 96.4 ms: 1.18x slower                                                          |
| richards                   | 33.4 ms                                                         | 39.6 ms: 1.19x slower                                                          |
| mako                       | 7.02 ms                                                         | 8.35 ms: 1.19x slower                                                          |
| richards_super             | 37.0 ms                                                         | 44.4 ms: 1.20x slower                                                          |
| logging_silent             | 62.4 ns                                                         | 76.0 ns: 1.22x slower                                                          |
| scimark_sparse_mat_mult    | 2.88 ms                                                         | 3.54 ms: 1.23x slower                                                          |
| generators                 | 21.5 ms                                                         | 27.1 ms: 1.26x slower                                                          |
| Geometric mean             | (ref)                                                           | 1.03x faster                                                                   |

Benchmark hidden because not significant (4): sympy_sum, pycparser, sympy_str, async_tree_io
Ignored benchmarks (10) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5.json: asyncio_websockets, bpe_tokeniser, chameleon, connected_components, gevent_hub, k_core, shortest_path, sphinx, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (2) of results/bm-20240829-3.14.0a0-58ce131/bm-20240829-pythonperf1_win32-x86-python-58ce131037ecb34d506a-3.14.0a0-58ce131.json: asyncio_tcp, asyncio_tcp_ssl

- Geometric mean (including insignificant results): 1.025x faster
# HPT report

- Reliability score: 99.68% likely to be slow
- 90% likely to have a slowdown of 1.01x
- 95% likely to have a slowdown of 1.01x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown