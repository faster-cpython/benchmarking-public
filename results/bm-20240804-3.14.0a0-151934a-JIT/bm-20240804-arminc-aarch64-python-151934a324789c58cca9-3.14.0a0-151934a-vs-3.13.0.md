# Results vs. 3.13.0

- fork: python
- ref: 151934a324789c58cca9
- machine: linux-aarch64
- commit hash: 151934a
- commit date: 2024-08-04
- overall geometric mean: 1.08x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x slower
- Memory change: 1.10x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 369 ms: 1.21x slower                                                    |
| docutils       | 2.91 sec                                                 | 3.74 sec: 1.29x slower                                                  |
| html5lib       | 64.3 ms                                                  | 70.6 ms: 1.10x slower                                                   |
| tornado_http   | 131 ms                                                   | 140 ms: 1.07x slower                                                    |
| Geometric mean | (ref)                                                    | 1.16x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 537 ms: 1.21x faster                                                    |
| async_tree_none            | 493 ms                                                   | 440 ms: 1.12x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 421 ms: 1.11x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 585 ms: 1.07x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 707 ms: 1.04x faster                                                    |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 750 ms: 1.02x faster                                                    |
| asyncio_websockets         | 656 ms                                                   | 661 ms: 1.01x slower                                                    |
| coroutines                 | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                   |
| async_tree_io_tg           | 1.09 sec                                                 | 1.11 sec: 1.02x slower                                                  |
| async_generators           | 496 ms                                                   | 510 ms: 1.03x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.26 sec: 1.03x slower                                                  |
| async_tree_io              | 1.11 sec                                                 | 1.18 sec: 1.07x slower                                                  |
| asyncio_tcp                | 568 ms                                                   | 625 ms: 1.10x slower                                                    |
| Geometric mean             | (ref)                                                    | 1.02x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 91.8 ms: 1.03x faster                                                   |
| Geometric mean | (ref)                                                    | 1.00x faster                                                            |

Benchmark hidden because not significant (2): pidigits, nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.5 ms                                                  | 30.6 ms: 1.03x faster                                                   |
| regex_effbot   | 4.87 ms                                                  | 4.95 ms: 1.02x slower                                                   |
| regex_dna      | 246 ms                                                   | 260 ms: 1.05x slower                                                    |
| regex_compile  | 128 ms                                                   | 174 ms: 1.36x slower                                                    |
| Geometric mean | (ref)                                                    | 1.09x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| json_loads           | 31.4 us                                                  | 32.5 us: 1.03x slower                                                   |
| tomli_loads          | 2.53 sec                                                 | 2.64 sec: 1.05x slower                                                  |
| unpickle_pure_python | 254 us                                                   | 275 us: 1.08x slower                                                    |
| pickle_pure_python   | 359 us                                                   | 395 us: 1.10x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.03x slower                                                            |

Benchmark hidden because not significant (4): json_dumps, xml_etree_generate, xml_etree_parse, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 8.75 ms                                                  | 8.96 ms: 1.02x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.01x slower                                                            |

Benchmark hidden because not significant (1): python_startup

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 12.9 ms: 1.03x faster                                                   |
| genshi_text     | 27.7 ms                                                  | 32.1 ms: 1.16x slower                                                   |
| django_template | 42.3 ms                                                  | 51.5 ms: 1.22x slower                                                   |
| genshi_xml      | 60.2 ms                                                  | 74.0 ms: 1.23x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.14x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 51.0 us                                                  | 39.4 us: 1.30x faster                                                   |
| async_tree_memoization_tg  | 649 ms                                                   | 537 ms: 1.21x faster                                                    |
| deepcopy                   | 451 us                                                   | 382 us: 1.18x faster                                                    |
| async_tree_none            | 493 ms                                                   | 440 ms: 1.12x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 421 ms: 1.11x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 585 ms: 1.07x faster                                                    |
| scimark_sor                | 159 ms                                                   | 151 ms: 1.05x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 707 ms: 1.04x faster                                                    |
| mako                       | 13.3 ms                                                  | 12.9 ms: 1.03x faster                                                   |
| regex_v8                   | 31.5 ms                                                  | 30.6 ms: 1.03x faster                                                   |
| float                      | 94.4 ms                                                  | 91.8 ms: 1.03x faster                                                   |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 750 ms: 1.02x faster                                                    |
| xml_etree_iterparse        | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| asyncio_websockets         | 656 ms                                                   | 661 ms: 1.01x slower                                                    |
| coroutines                 | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                   |
| json                       | 5.61 ms                                                  | 5.69 ms: 1.01x slower                                                   |
| async_tree_io_tg           | 1.09 sec                                                 | 1.11 sec: 1.02x slower                                                  |
| regex_effbot               | 4.87 ms                                                  | 4.95 ms: 1.02x slower                                                   |
| scimark_fft                | 447 ms                                                   | 455 ms: 1.02x slower                                                    |
| coverage                   | 98.5 ms                                                  | 100 ms: 1.02x slower                                                    |
| richards                   | 53.5 ms                                                  | 54.6 ms: 1.02x slower                                                   |
| richards_super             | 60.3 ms                                                  | 61.6 ms: 1.02x slower                                                   |
| spectral_norm              | 141 ms                                                   | 144 ms: 1.02x slower                                                    |
| python_startup_no_site     | 8.75 ms                                                  | 8.96 ms: 1.02x slower                                                   |
| logging_format             | 7.83 us                                                  | 8.03 us: 1.03x slower                                                   |
| async_generators           | 496 ms                                                   | 510 ms: 1.03x slower                                                    |
| bench_thread_pool          | 1.28 ms                                                  | 1.32 ms: 1.03x slower                                                   |
| deepcopy_reduce            | 4.07 us                                                  | 4.20 us: 1.03x slower                                                   |
| json_loads                 | 31.4 us                                                  | 32.5 us: 1.03x slower                                                   |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.26 sec: 1.03x slower                                                  |
| thrift                     | 946 us                                                   | 979 us: 1.04x slower                                                    |
| scimark_sparse_mat_mult    | 6.58 ms                                                  | 6.82 ms: 1.04x slower                                                   |
| mdp                        | 3.36 sec                                                 | 3.50 sec: 1.04x slower                                                  |
| telco                      | 9.73 ms                                                  | 10.2 ms: 1.04x slower                                                   |
| logging_simple             | 7.04 us                                                  | 7.35 us: 1.04x slower                                                   |
| tomli_loads                | 2.53 sec                                                 | 2.64 sec: 1.05x slower                                                  |
| regex_dna                  | 246 ms                                                   | 260 ms: 1.05x slower                                                    |
| scimark_monte_carlo        | 83.8 ms                                                  | 88.5 ms: 1.06x slower                                                   |
| meteor_contest             | 113 ms                                                   | 120 ms: 1.06x slower                                                    |
| tornado_http               | 131 ms                                                   | 140 ms: 1.07x slower                                                    |
| pyflate                    | 556 ms                                                   | 595 ms: 1.07x slower                                                    |
| async_tree_io              | 1.11 sec                                                 | 1.18 sec: 1.07x slower                                                  |
| scimark_lu                 | 139 ms                                                   | 149 ms: 1.07x slower                                                    |
| unpickle_pure_python       | 254 us                                                   | 275 us: 1.08x slower                                                    |
| gc_traversal               | 4.53 ms                                                  | 4.92 ms: 1.08x slower                                                   |
| crypto_pyaes               | 82.7 ms                                                  | 89.9 ms: 1.09x slower                                                   |
| fannkuch                   | 452 ms                                                   | 492 ms: 1.09x slower                                                    |
| html5lib                   | 64.3 ms                                                  | 70.6 ms: 1.10x slower                                                   |
| pickle_pure_python         | 359 us                                                   | 395 us: 1.10x slower                                                    |
| asyncio_tcp                | 568 ms                                                   | 625 ms: 1.10x slower                                                    |
| raytrace                   | 298 ms                                                   | 329 ms: 1.10x slower                                                    |
| bench_mp_pool              | 7.30 ms                                                  | 8.06 ms: 1.10x slower                                                   |
| typing_runtime_protocols   | 192 us                                                   | 213 us: 1.11x slower                                                    |
| create_gc_cycles           | 2.12 ms                                                  | 2.35 ms: 1.11x slower                                                   |
| go                         | 163 ms                                                   | 182 ms: 1.12x slower                                                    |
| dask                       | 350 ms                                                   | 400 ms: 1.14x slower                                                    |
| sqlglot_normalize          | 128 ms                                                   | 147 ms: 1.14x slower                                                    |
| deltablue                  | 3.85 ms                                                  | 4.45 ms: 1.16x slower                                                   |
| genshi_text                | 27.7 ms                                                  | 32.1 ms: 1.16x slower                                                   |
| sqlglot_optimize           | 62.4 ms                                                  | 72.3 ms: 1.16x slower                                                   |
| pycparser                  | 1.26 sec                                                 | 1.48 sec: 1.17x slower                                                  |
| comprehensions             | 20.4 us                                                  | 24.2 us: 1.19x slower                                                   |
| sqlglot_parse              | 1.38 ms                                                  | 1.65 ms: 1.19x slower                                                   |
| 2to3                       | 306 ms                                                   | 369 ms: 1.21x slower                                                    |
| django_template            | 42.3 ms                                                  | 51.5 ms: 1.22x slower                                                   |
| sqlglot_transpile          | 1.73 ms                                                  | 2.13 ms: 1.23x slower                                                   |
| genshi_xml                 | 60.2 ms                                                  | 74.0 ms: 1.23x slower                                                   |
| pprint_safe_repr           | 926 ms                                                   | 1.14 sec: 1.23x slower                                                  |
| pylint                     | 343 ms                                                   | 431 ms: 1.26x slower                                                    |
| pprint_pformat             | 1.90 sec                                                 | 2.39 sec: 1.26x slower                                                  |
| nqueens                    | 98.7 ms                                                  | 125 ms: 1.27x slower                                                    |
| chaos                      | 68.8 ms                                                  | 87.4 ms: 1.27x slower                                                   |
| docutils                   | 2.91 sec                                                 | 3.74 sec: 1.29x slower                                                  |
| hexiom                     | 7.13 ms                                                  | 9.23 ms: 1.29x slower                                                   |
| sympy_expand               | 455 ms                                                   | 591 ms: 1.30x slower                                                    |
| sympy_integrate            | 21.0 ms                                                  | 27.5 ms: 1.31x slower                                                   |
| sympy_str                  | 264 ms                                                   | 349 ms: 1.32x slower                                                    |
| regex_compile              | 128 ms                                                   | 174 ms: 1.36x slower                                                    |
| sympy_sum                  | 143 ms                                                   | 200 ms: 1.40x slower                                                    |
| generators                 | 37.8 ms                                                  | 57.2 ms: 1.51x slower                                                   |
| Geometric mean             | (ref)                                                    | 1.08x slower                                                            |

Benchmark hidden because not significant (10): json_dumps, python_startup, logging_silent, xml_etree_generate, bpe_tokeniser, pathlib, xml_etree_parse, pidigits, nbody, xml_etree_process
Ignored benchmarks (12) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.03x
- 99% likely to have a slowdown of 1.02x

# Memory
- memory change: 1.10x