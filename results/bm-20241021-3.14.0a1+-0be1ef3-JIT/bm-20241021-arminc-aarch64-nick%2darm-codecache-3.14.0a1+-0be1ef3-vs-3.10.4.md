# Results vs. 3.10.4

- fork: nick-arm
- ref: codecache
- machine: linux-aarch64
- commit hash: 0be1ef3
- commit date: 2024-10-21
- overall geometric mean: 1.171x faster
- HPT reliability: 99.99%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.39x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 375 ms: 1.02x faster                                              |
| docutils       | 3.53 sec                                                              | 3.55 sec: 1.01x slower                                            |
| html5lib       | 86.5 ms                                                               | 72.7 ms: 1.19x faster                                             |
| tornado_http   | 178 ms                                                                | 150 ms: 1.19x faster                                              |
| Geometric mean | (ref)                                                                 | 1.09x faster                                                      |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 473 ms: 2.01x faster                                              |
| async_tree_io           | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                            |
| async_tree_memoization  | 1.13 sec                                                              | 607 ms: 1.87x faster                                              |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 760 ms: 1.67x faster                                              |
| Geometric mean          | (ref)                                                                 | 1.87x faster                                                      |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| nbody          | 166 ms                                                                | 114 ms: 1.45x faster                                              |
| float          | 135 ms                                                                | 97.7 ms: 1.38x faster                                             |
| Geometric mean | (ref)                                                                 | 1.26x faster                                                      |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| regex_dna      | 257 ms                                                                | 249 ms: 1.03x faster                                              |
| regex_v8       | 32.2 ms                                                               | 31.2 ms: 1.03x faster                                             |
| regex_compile  | 177 ms                                                                | 172 ms: 1.03x faster                                              |
| regex_effbot   | 4.25 ms                                                               | 5.06 ms: 1.19x slower                                             |
| Geometric mean | (ref)                                                                 | 1.02x slower                                                      |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 266 us: 1.37x faster                                              |
| pickle_pure_python   | 529 us                                                                | 389 us: 1.36x faster                                              |
| tomli_loads          | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                            |
| xml_etree_process    | 99.5 ms                                                               | 81.5 ms: 1.22x faster                                             |
| json_dumps           | 16.7 ms                                                               | 14.2 ms: 1.17x faster                                             |
| xml_etree_parse      | 212 ms                                                                | 189 ms: 1.12x faster                                              |
| xml_etree_generate   | 123 ms                                                                | 113 ms: 1.09x faster                                              |
| xml_etree_iterparse  | 156 ms                                                                | 149 ms: 1.04x faster                                              |
| json_loads           | 30.9 us                                                               | 31.8 us: 1.03x slower                                             |
| Geometric mean       | (ref)                                                                 | 1.17x faster                                                      |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| python_startup_no_site | 6.89 ms                                                               | 8.78 ms: 1.27x slower                                             |
| python_startup         | 11.2 ms                                                               | 14.6 ms: 1.30x slower                                             |
| Geometric mean         | (ref)                                                                 | 1.29x slower                                                      |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.2 ms: 1.43x faster                                             |
| django_template | 53.3 ms                                                               | 52.3 ms: 1.02x faster                                             |
| genshi_xml      | 69.8 ms                                                               | 82.5 ms: 1.18x slower                                             |
| Geometric mean  | (ref)                                                                 | 1.06x faster                                                      |

Benchmark hidden because not significant (1): genshi_text

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 212 us: 3.11x faster                                              |
| async_tree_none          | 950 ms                                                                | 473 ms: 2.01x faster                                              |
| async_tree_io            | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                            |
| deltablue                | 8.94 ms                                                               | 4.66 ms: 1.92x faster                                             |
| async_tree_memoization   | 1.13 sec                                                              | 607 ms: 1.87x faster                                              |
| logging_silent           | 222 ns                                                                | 133 ns: 1.67x faster                                              |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 760 ms: 1.67x faster                                              |
| raytrace                 | 573 ms                                                                | 353 ms: 1.63x faster                                              |
| richards_super           | 107 ms                                                                | 67.5 ms: 1.59x faster                                             |
| deepcopy_memo            | 61.7 us                                                               | 39.2 us: 1.57x faster                                             |
| scimark_sor              | 246 ms                                                                | 159 ms: 1.55x faster                                              |
| scimark_lu               | 227 ms                                                                | 151 ms: 1.50x faster                                              |
| crypto_pyaes             | 134 ms                                                                | 89.3 ms: 1.50x faster                                             |
| richards                 | 91.7 ms                                                               | 61.1 ms: 1.50x faster                                             |
| nbody                    | 166 ms                                                                | 114 ms: 1.45x faster                                              |
| mako                     | 18.9 ms                                                               | 13.2 ms: 1.43x faster                                             |
| scimark_monte_carlo      | 128 ms                                                                | 90.6 ms: 1.41x faster                                             |
| go                       | 264 ms                                                                | 188 ms: 1.41x faster                                              |
| chaos                    | 121 ms                                                                | 86.8 ms: 1.40x faster                                             |
| sqlglot_parse            | 2.40 ms                                                               | 1.73 ms: 1.39x faster                                             |
| float                    | 135 ms                                                                | 97.7 ms: 1.38x faster                                             |
| unpickle_pure_python     | 366 us                                                                | 266 us: 1.37x faster                                              |
| deepcopy                 | 511 us                                                                | 374 us: 1.37x faster                                              |
| pickle_pure_python       | 529 us                                                                | 389 us: 1.36x faster                                              |
| comprehensions           | 33.1 us                                                               | 24.7 us: 1.34x faster                                             |
| sqlglot_transpile        | 2.84 ms                                                               | 2.16 ms: 1.32x faster                                             |
| coroutines               | 37.2 ms                                                               | 28.7 ms: 1.29x faster                                             |
| thrift                   | 1.26 ms                                                               | 974 us: 1.29x faster                                              |
| pyflate                  | 795 ms                                                                | 616 ms: 1.29x faster                                              |
| logging_format           | 10.6 us                                                               | 8.26 us: 1.28x faster                                             |
| tomli_loads              | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                            |
| logging_simple           | 9.78 us                                                               | 7.65 us: 1.28x faster                                             |
| xml_etree_process        | 99.5 ms                                                               | 81.5 ms: 1.22x faster                                             |
| spectral_norm            | 186 ms                                                                | 154 ms: 1.21x faster                                              |
| pathlib                  | 26.3 ms                                                               | 21.8 ms: 1.21x faster                                             |
| tornado_http             | 178 ms                                                                | 150 ms: 1.19x faster                                              |
| html5lib                 | 86.5 ms                                                               | 72.7 ms: 1.19x faster                                             |
| deepcopy_reduce          | 4.60 us                                                               | 3.87 us: 1.19x faster                                             |
| json_dumps               | 16.7 ms                                                               | 14.2 ms: 1.17x faster                                             |
| bench_thread_pool        | 1.59 ms                                                               | 1.38 ms: 1.15x faster                                             |
| generators               | 68.1 ms                                                               | 60.0 ms: 1.13x faster                                             |
| pycparser                | 1.69 sec                                                              | 1.49 sec: 1.13x faster                                            |
| xml_etree_parse          | 212 ms                                                                | 189 ms: 1.12x faster                                              |
| scimark_fft              | 500 ms                                                                | 454 ms: 1.10x faster                                              |
| xml_etree_generate       | 123 ms                                                                | 113 ms: 1.09x faster                                              |
| hexiom                   | 10.9 ms                                                               | 10.1 ms: 1.08x faster                                             |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 7.13 ms: 1.07x faster                                             |
| mdp                      | 3.70 sec                                                              | 3.48 sec: 1.06x faster                                            |
| fannkuch                 | 546 ms                                                                | 515 ms: 1.06x faster                                              |
| pylint                   | 485 ms                                                                | 458 ms: 1.06x faster                                              |
| xml_etree_iterparse      | 156 ms                                                                | 149 ms: 1.04x faster                                              |
| json                     | 5.88 ms                                                               | 5.66 ms: 1.04x faster                                             |
| regex_dna                | 257 ms                                                                | 249 ms: 1.03x faster                                              |
| regex_v8                 | 32.2 ms                                                               | 31.2 ms: 1.03x faster                                             |
| regex_compile            | 177 ms                                                                | 172 ms: 1.03x faster                                              |
| meteor_contest           | 126 ms                                                                | 123 ms: 1.02x faster                                              |
| sqlglot_normalize        | 156 ms                                                                | 153 ms: 1.02x faster                                              |
| django_template          | 53.3 ms                                                               | 52.3 ms: 1.02x faster                                             |
| 2to3                     | 381 ms                                                                | 375 ms: 1.02x faster                                              |
| docutils                 | 3.53 sec                                                              | 3.55 sec: 1.01x slower                                            |
| json_loads               | 30.9 us                                                               | 31.8 us: 1.03x slower                                             |
| pprint_safe_repr         | 1.15 sec                                                              | 1.20 sec: 1.05x slower                                            |
| sqlglot_optimize         | 75.4 ms                                                               | 79.9 ms: 1.06x slower                                             |
| dulwich_log              | 73.5 ms                                                               | 78.1 ms: 1.06x slower                                             |
| sympy_str                | 329 ms                                                                | 353 ms: 1.07x slower                                              |
| pprint_pformat           | 2.36 sec                                                              | 2.55 sec: 1.08x slower                                            |
| sympy_expand             | 543 ms                                                                | 588 ms: 1.08x slower                                              |
| nqueens                  | 117 ms                                                                | 128 ms: 1.09x slower                                              |
| sympy_integrate          | 26.5 ms                                                               | 29.7 ms: 1.12x slower                                             |
| telco                    | 8.49 ms                                                               | 9.56 ms: 1.13x slower                                             |
| async_generators         | 452 ms                                                                | 511 ms: 1.13x slower                                              |
| sympy_sum                | 184 ms                                                                | 214 ms: 1.16x slower                                              |
| genshi_xml               | 69.8 ms                                                               | 82.5 ms: 1.18x slower                                             |
| coverage                 | 83.6 ms                                                               | 99.0 ms: 1.18x slower                                             |
| regex_effbot             | 4.25 ms                                                               | 5.06 ms: 1.19x slower                                             |
| python_startup_no_site   | 6.89 ms                                                               | 8.78 ms: 1.27x slower                                             |
| python_startup           | 11.2 ms                                                               | 14.6 ms: 1.30x slower                                             |
| gc_traversal             | 4.15 ms                                                               | 6.37 ms: 1.53x slower                                             |
| create_gc_cycles         | 2.26 ms                                                               | 3.62 ms: 1.60x slower                                             |
| bench_mp_pool            | 14.5 ms                                                               | 2.50 sec: 171.75x slower                                          |
| Geometric mean           | (ref)                                                                 | 1.10x faster                                                      |

Benchmark hidden because not significant (3): genshi_text, pidigits, asyncio_websockets
Ignored benchmarks (16) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20241021-3.14.0a1+-0be1ef3-JIT/bm-20241021-arminc-aarch64-nick%2darm-codecache-3.14.0a1+-0be1ef3.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, sphinx

- Geometric mean (including insignificant results): 1.171x faster
# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.06x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.39x