# Results vs. 3.12.0

- fork: mdboom
- ref: unicode_check_exact
- machine: windows-x86
- commit hash: 76aa43c
- commit date: 2024-09-11
- overall geometric mean: 1.13x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.09x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| 2to3           | 280 ms                                                          | 250 ms: 1.12x faster                                                          |
| docutils       | 1.98 sec                                                        | 1.86 sec: 1.07x faster                                                        |
| tornado_http   | 105 ms                                                          | 95.8 ms: 1.10x faster                                                         |
| Geometric mean | (ref)                                                           | 1.10x faster                                                                  |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 350 ms                                                          | 251 ms: 1.40x faster                                                          |
| async_tree_none_tg         | 278 ms                                                          | 200 ms: 1.39x faster                                                          |
| async_tree_none            | 298 ms                                                          | 218 ms: 1.37x faster                                                          |
| async_tree_memoization     | 364 ms                                                          | 271 ms: 1.34x faster                                                          |
| async_tree_io_tg           | 677 ms                                                          | 516 ms: 1.31x faster                                                          |
| async_tree_io              | 693 ms                                                          | 534 ms: 1.30x faster                                                          |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 469 ms: 1.20x faster                                                          |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 461 ms: 1.19x faster                                                          |
| Geometric mean             | (ref)                                                           | 1.31x faster                                                                  |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| nbody          | 127 ms                                                          | 91.6 ms: 1.39x faster                                                         |
| float          | 76.7 ms                                                         | 61.4 ms: 1.25x faster                                                         |
| pidigits       | 199 ms                                                          | 201 ms: 1.01x slower                                                          |
| Geometric mean | (ref)                                                           | 1.20x faster                                                                  |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| regex_compile  | 129 ms                                                          | 107 ms: 1.21x faster                                                          |
| regex_dna      | 127 ms                                                          | 116 ms: 1.09x faster                                                          |
| regex_effbot   | 2.04 ms                                                         | 1.89 ms: 1.08x faster                                                         |
| regex_v8       | 15.0 ms                                                         | 16.3 ms: 1.08x slower                                                         |
| Geometric mean | (ref)                                                           | 1.07x faster                                                                  |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| unpickle_pure_python | 210 us                                                          | 178 us: 1.18x faster                                                          |
| tomli_loads          | 2.20 sec                                                        | 1.90 sec: 1.16x faster                                                        |
| xml_etree_iterparse  | 77.6 ms                                                         | 67.8 ms: 1.15x faster                                                         |
| pickle_pure_python   | 286 us                                                          | 256 us: 1.12x faster                                                          |
| xml_etree_process    | 53.2 ms                                                         | 48.6 ms: 1.10x faster                                                         |
| xml_etree_generate   | 72.1 ms                                                         | 66.8 ms: 1.08x faster                                                         |
| xml_etree_parse      | 113 ms                                                          | 108 ms: 1.05x faster                                                          |
| pickle               | 7.79 us                                                         | 7.96 us: 1.02x slower                                                         |
| json_loads           | 20.4 us                                                         | 20.8 us: 1.02x slower                                                         |
| pickle_dict          | 19.9 us                                                         | 20.5 us: 1.03x slower                                                         |
| pickle_list          | 3.37 us                                                         | 3.50 us: 1.04x slower                                                         |
| unpickle_list        | 2.95 us                                                         | 3.08 us: 1.05x slower                                                         |
| unpickle             | 9.71 us                                                         | 10.4 us: 1.07x slower                                                         |
| Geometric mean       | (ref)                                                           | 1.04x faster                                                                  |

Benchmark hidden because not significant (1): json_dumps

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| python_startup_no_site | 19.1 ms                                                         | 19.4 ms: 1.02x slower                                                         |
| python_startup         | 22.4 ms                                                         | 23.5 ms: 1.05x slower                                                         |
| Geometric mean         | (ref)                                                           | 1.03x slower                                                                  |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|-----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| mako            | 9.96 ms                                                         | 7.98 ms: 1.25x faster                                                         |
| django_template | 36.9 ms                                                         | 32.5 ms: 1.13x faster                                                         |
| Geometric mean  | (ref)                                                           | 1.19x faster                                                                  |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c |
|----------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| deepcopy_memo              | 38.4 us                                                         | 22.2 us: 1.73x faster                                                         |
| deepcopy                   | 360 us                                                          | 244 us: 1.48x faster                                                          |
| generators                 | 38.5 ms                                                         | 26.6 ms: 1.45x faster                                                         |
| comprehensions             | 19.2 us                                                         | 13.6 us: 1.41x faster                                                         |
| async_tree_memoization_tg  | 350 ms                                                          | 251 ms: 1.40x faster                                                          |
| async_tree_none_tg         | 278 ms                                                          | 200 ms: 1.39x faster                                                          |
| nbody                      | 127 ms                                                          | 91.6 ms: 1.39x faster                                                         |
| async_tree_none            | 298 ms                                                          | 218 ms: 1.37x faster                                                          |
| logging_silent             | 101 ns                                                          | 74.8 ns: 1.35x faster                                                         |
| async_tree_memoization     | 364 ms                                                          | 271 ms: 1.34x faster                                                          |
| unpack_sequence            | 62.5 ns                                                         | 46.7 ns: 1.34x faster                                                         |
| spectral_norm              | 104 ms                                                          | 77.7 ms: 1.34x faster                                                         |
| hexiom                     | 6.82 ms                                                         | 5.10 ms: 1.34x faster                                                         |
| go                         | 137 ms                                                          | 103 ms: 1.33x faster                                                          |
| deltablue                  | 3.58 ms                                                         | 2.70 ms: 1.33x faster                                                         |
| scimark_lu                 | 93.2 ms                                                         | 70.5 ms: 1.32x faster                                                         |
| raytrace                   | 308 ms                                                          | 234 ms: 1.32x faster                                                          |
| async_tree_io_tg           | 677 ms                                                          | 516 ms: 1.31x faster                                                          |
| async_tree_io              | 693 ms                                                          | 534 ms: 1.30x faster                                                          |
| scimark_sor                | 130 ms                                                          | 101 ms: 1.29x faster                                                          |
| chaos                      | 69.4 ms                                                         | 53.9 ms: 1.29x faster                                                         |
| deepcopy_reduce            | 3.23 us                                                         | 2.54 us: 1.27x faster                                                         |
| nqueens                    | 93.7 ms                                                         | 73.8 ms: 1.27x faster                                                         |
| float                      | 76.7 ms                                                         | 61.4 ms: 1.25x faster                                                         |
| mako                       | 9.96 ms                                                         | 7.98 ms: 1.25x faster                                                         |
| scimark_sparse_mat_mult    | 3.86 ms                                                         | 3.13 ms: 1.23x faster                                                         |
| logging_simple             | 9.75 us                                                         | 7.99 us: 1.22x faster                                                         |
| crypto_pyaes               | 69.2 ms                                                         | 57.0 ms: 1.21x faster                                                         |
| regex_compile              | 129 ms                                                          | 107 ms: 1.21x faster                                                          |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 469 ms: 1.20x faster                                                          |
| logging_format             | 10.4 us                                                         | 8.66 us: 1.20x faster                                                         |
| pyflate                    | 424 ms                                                          | 356 ms: 1.19x faster                                                          |
| sqlglot_parse              | 1.25 ms                                                         | 1.05 ms: 1.19x faster                                                         |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 461 ms: 1.19x faster                                                          |
| unpickle_pure_python       | 210 us                                                          | 178 us: 1.18x faster                                                          |
| sqlglot_transpile          | 1.53 ms                                                         | 1.30 ms: 1.18x faster                                                         |
| sympy_integrate            | 17.5 ms                                                         | 15.1 ms: 1.16x faster                                                         |
| tomli_loads                | 2.20 sec                                                        | 1.90 sec: 1.16x faster                                                        |
| dulwich_log                | 58.5 ms                                                         | 50.6 ms: 1.15x faster                                                         |
| scimark_monte_carlo        | 66.4 ms                                                         | 57.6 ms: 1.15x faster                                                         |
| sympy_sum                  | 123 ms                                                          | 107 ms: 1.15x faster                                                          |
| xml_etree_iterparse        | 77.6 ms                                                         | 67.8 ms: 1.15x faster                                                         |
| sqlglot_optimize           | 48.5 ms                                                         | 42.5 ms: 1.14x faster                                                         |
| django_template            | 36.9 ms                                                         | 32.5 ms: 1.13x faster                                                         |
| pycparser                  | 978 ms                                                          | 865 ms: 1.13x faster                                                          |
| coroutines                 | 20.9 ms                                                         | 18.5 ms: 1.13x faster                                                         |
| sympy_str                  | 240 ms                                                          | 213 ms: 1.12x faster                                                          |
| 2to3                       | 280 ms                                                          | 250 ms: 1.12x faster                                                          |
| pickle_pure_python         | 286 us                                                          | 256 us: 1.12x faster                                                          |
| pathlib                    | 91.4 ms                                                         | 82.5 ms: 1.11x faster                                                         |
| pprint_pformat             | 1.50 sec                                                        | 1.36 sec: 1.10x faster                                                        |
| scimark_fft                | 271 ms                                                          | 246 ms: 1.10x faster                                                          |
| fannkuch                   | 354 ms                                                          | 322 ms: 1.10x faster                                                          |
| xml_etree_process          | 53.2 ms                                                         | 48.6 ms: 1.10x faster                                                         |
| tornado_http               | 105 ms                                                          | 95.8 ms: 1.10x faster                                                         |
| mdp                        | 1.91 sec                                                        | 1.75 sec: 1.09x faster                                                        |
| regex_dna                  | 127 ms                                                          | 116 ms: 1.09x faster                                                          |
| meteor_contest             | 86.9 ms                                                         | 79.8 ms: 1.09x faster                                                         |
| pprint_safe_repr           | 721 ms                                                          | 662 ms: 1.09x faster                                                          |
| xml_etree_generate         | 72.1 ms                                                         | 66.8 ms: 1.08x faster                                                         |
| bench_thread_pool          | 1.10 ms                                                         | 1.02 ms: 1.08x faster                                                         |
| regex_effbot               | 2.04 ms                                                         | 1.89 ms: 1.08x faster                                                         |
| bench_mp_pool              | 75.4 ms                                                         | 70.1 ms: 1.07x faster                                                         |
| docutils                   | 1.98 sec                                                        | 1.86 sec: 1.07x faster                                                        |
| richards                   | 41.3 ms                                                         | 38.8 ms: 1.06x faster                                                         |
| sympy_expand               | 398 ms                                                          | 377 ms: 1.06x faster                                                          |
| async_generators           | 313 ms                                                          | 297 ms: 1.05x faster                                                          |
| sqlite_synth               | 2.07 us                                                         | 1.97 us: 1.05x faster                                                         |
| richards_super             | 46.5 ms                                                         | 44.3 ms: 1.05x faster                                                         |
| xml_etree_parse            | 113 ms                                                          | 108 ms: 1.05x faster                                                          |
| asyncio_tcp_ssl            | 17.7 sec                                                        | 17.1 sec: 1.04x faster                                                        |
| gc_traversal               | 1.44 ms                                                         | 1.41 ms: 1.02x faster                                                         |
| pidigits                   | 199 ms                                                          | 201 ms: 1.01x slower                                                          |
| python_startup_no_site     | 19.1 ms                                                         | 19.4 ms: 1.02x slower                                                         |
| pickle                     | 7.79 us                                                         | 7.96 us: 1.02x slower                                                         |
| json_loads                 | 20.4 us                                                         | 20.8 us: 1.02x slower                                                         |
| pickle_dict                | 19.9 us                                                         | 20.5 us: 1.03x slower                                                         |
| pickle_list                | 3.37 us                                                         | 3.50 us: 1.04x slower                                                         |
| unpickle_list              | 2.95 us                                                         | 3.08 us: 1.05x slower                                                         |
| python_startup             | 22.4 ms                                                         | 23.5 ms: 1.05x slower                                                         |
| coverage                   | 48.4 ms                                                         | 51.9 ms: 1.07x slower                                                         |
| unpickle                   | 9.71 us                                                         | 10.4 us: 1.07x slower                                                         |
| regex_v8                   | 15.0 ms                                                         | 16.3 ms: 1.08x slower                                                         |
| create_gc_cycles           | 652 us                                                          | 734 us: 1.13x slower                                                          |
| typing_runtime_protocols   | 126 us                                                          | 142 us: 1.13x slower                                                          |
| telco                      | 5.51 ms                                                         | 6.34 ms: 1.15x slower                                                         |
| sqlglot_normalize          | 100 ms                                                          | 221 ms: 2.20x slower                                                          |
| Geometric mean             | (ref)                                                           | 1.13x faster                                                                  |

Benchmark hidden because not significant (3): asyncio_tcp, json, json_dumps
Ignored benchmarks (6) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (5) of results/bm-20240911-3.14.0a0-76aa43c/bm-20240911-pythonperf1_win32-x86-mdboom-unicode_check_exact-3.14.0a0-76aa43c.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.11x
- 95% likely to have a speedup of 1.10x
- 99% likely to have a speedup of 1.09x

# Memory
- memory change: unknown