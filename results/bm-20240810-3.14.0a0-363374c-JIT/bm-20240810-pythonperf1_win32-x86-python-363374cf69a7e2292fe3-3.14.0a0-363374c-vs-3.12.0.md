# Results vs. 3.12.0

- fork: python
- ref: 363374cf69a7e2292fe3
- machine: windows-x86
- commit hash: 363374c
- commit date: 2024-08-10
- overall geometric mean: 1.24x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.11x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 280 ms                                                          | 259 ms: 1.08x faster                                                           |
| docutils       | 1.98 sec                                                        | 1.96 sec: 1.01x faster                                                         |
| tornado_http   | 105 ms                                                          | 107 ms: 1.02x slower                                                           |
| Geometric mean | (ref)                                                           | 1.02x faster                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_none_tg         | 278 ms                                                          | 194 ms: 1.43x faster                                                           |
| async_tree_memoization_tg  | 350 ms                                                          | 250 ms: 1.40x faster                                                           |
| async_tree_io_tg           | 677 ms                                                          | 500 ms: 1.35x faster                                                           |
| async_tree_memoization     | 364 ms                                                          | 274 ms: 1.33x faster                                                           |
| async_tree_none            | 298 ms                                                          | 225 ms: 1.32x faster                                                           |
| async_tree_io              | 693 ms                                                          | 543 ms: 1.28x faster                                                           |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 503 ms: 1.12x faster                                                           |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 489 ms: 1.12x faster                                                           |
| Geometric mean             | (ref)                                                           | 1.29x faster                                                                   |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| nbody          | 127 ms                                                          | 49.9 ms: 2.54x faster                                                          |
| float          | 76.7 ms                                                         | 43.6 ms: 1.76x faster                                                          |
| pidigits       | 199 ms                                                          | 198 ms: 1.01x faster                                                           |
| Geometric mean | (ref)                                                           | 1.65x faster                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_compile  | 129 ms                                                          | 94.5 ms: 1.37x faster                                                          |
| regex_dna      | 127 ms                                                          | 128 ms: 1.01x slower                                                           |
| regex_effbot   | 2.04 ms                                                         | 2.09 ms: 1.02x slower                                                          |
| regex_v8       | 15.0 ms                                                         | 16.1 ms: 1.07x slower                                                          |
| Geometric mean | (ref)                                                           | 1.06x faster                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| unpickle_pure_python | 210 us                                                          | 146 us: 1.44x faster                                                           |
| tomli_loads          | 2.20 sec                                                        | 1.55 sec: 1.42x faster                                                         |
| pickle_pure_python   | 286 us                                                          | 210 us: 1.36x faster                                                           |
| xml_etree_generate   | 72.1 ms                                                         | 57.9 ms: 1.25x faster                                                          |
| xml_etree_process    | 53.2 ms                                                         | 43.4 ms: 1.23x faster                                                          |
| xml_etree_iterparse  | 77.6 ms                                                         | 63.9 ms: 1.21x faster                                                          |
| xml_etree_parse      | 113 ms                                                          | 106 ms: 1.06x faster                                                           |
| json_dumps           | 7.40 ms                                                         | 7.02 ms: 1.05x faster                                                          |
| json_loads           | 20.4 us                                                         | 20.8 us: 1.02x slower                                                          |
| Geometric mean       | (ref)                                                           | 1.21x faster                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup_no_site | 19.1 ms                                                         | 20.7 ms: 1.08x slower                                                          |
| python_startup         | 22.4 ms                                                         | 24.4 ms: 1.09x slower                                                          |
| Geometric mean         | (ref)                                                           | 1.09x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| mako            | 9.96 ms                                                         | 5.44 ms: 1.83x faster                                                          |
| django_template | 36.9 ms                                                         | 32.9 ms: 1.12x faster                                                          |
| Geometric mean  | (ref)                                                           | 1.43x faster                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| nbody                      | 127 ms                                                          | 49.9 ms: 2.54x faster                                                          |
| deepcopy_memo              | 38.4 us                                                         | 15.7 us: 2.44x faster                                                          |
| spectral_norm              | 104 ms                                                          | 50.4 ms: 2.06x faster                                                          |
| scimark_sor                | 130 ms                                                          | 66.4 ms: 1.95x faster                                                          |
| mako                       | 9.96 ms                                                         | 5.44 ms: 1.83x faster                                                          |
| float                      | 76.7 ms                                                         | 43.6 ms: 1.76x faster                                                          |
| logging_silent             | 101 ns                                                          | 58.2 ns: 1.74x faster                                                          |
| comprehensions             | 19.2 us                                                         | 11.2 us: 1.71x faster                                                          |
| scimark_sparse_mat_mult    | 3.86 ms                                                         | 2.35 ms: 1.64x faster                                                          |
| scimark_fft                | 271 ms                                                          | 166 ms: 1.63x faster                                                           |
| scimark_monte_carlo        | 66.4 ms                                                         | 41.1 ms: 1.62x faster                                                          |
| generators                 | 38.5 ms                                                         | 24.0 ms: 1.61x faster                                                          |
| pyflate                    | 424 ms                                                          | 268 ms: 1.58x faster                                                           |
| scimark_lu                 | 93.2 ms                                                         | 59.5 ms: 1.57x faster                                                          |
| fannkuch                   | 354 ms                                                          | 229 ms: 1.55x faster                                                           |
| crypto_pyaes               | 69.2 ms                                                         | 46.8 ms: 1.48x faster                                                          |
| deepcopy                   | 360 us                                                          | 244 us: 1.48x faster                                                           |
| unpickle_pure_python       | 210 us                                                          | 146 us: 1.44x faster                                                           |
| async_tree_none_tg         | 278 ms                                                          | 194 ms: 1.43x faster                                                           |
| hexiom                     | 6.82 ms                                                         | 4.81 ms: 1.42x faster                                                          |
| tomli_loads                | 2.20 sec                                                        | 1.55 sec: 1.42x faster                                                         |
| async_tree_memoization_tg  | 350 ms                                                          | 250 ms: 1.40x faster                                                           |
| raytrace                   | 308 ms                                                          | 224 ms: 1.37x faster                                                           |
| deepcopy_reduce            | 3.23 us                                                         | 2.35 us: 1.37x faster                                                          |
| regex_compile              | 129 ms                                                          | 94.5 ms: 1.37x faster                                                          |
| pickle_pure_python         | 286 us                                                          | 210 us: 1.36x faster                                                           |
| async_tree_io_tg           | 677 ms                                                          | 500 ms: 1.35x faster                                                           |
| chaos                      | 69.4 ms                                                         | 51.8 ms: 1.34x faster                                                          |
| async_tree_memoization     | 364 ms                                                          | 274 ms: 1.33x faster                                                           |
| async_tree_none            | 298 ms                                                          | 225 ms: 1.32x faster                                                           |
| deltablue                  | 3.58 ms                                                         | 2.74 ms: 1.31x faster                                                          |
| pprint_safe_repr           | 721 ms                                                          | 563 ms: 1.28x faster                                                           |
| async_tree_io              | 693 ms                                                          | 543 ms: 1.28x faster                                                           |
| sqlglot_parse              | 1.25 ms                                                         | 982 us: 1.27x faster                                                           |
| logging_simple             | 9.75 us                                                         | 7.70 us: 1.27x faster                                                          |
| xml_etree_generate         | 72.1 ms                                                         | 57.9 ms: 1.25x faster                                                          |
| pprint_pformat             | 1.50 sec                                                        | 1.21 sec: 1.24x faster                                                         |
| meteor_contest             | 86.9 ms                                                         | 70.1 ms: 1.24x faster                                                          |
| logging_format             | 10.4 us                                                         | 8.47 us: 1.23x faster                                                          |
| xml_etree_process          | 53.2 ms                                                         | 43.4 ms: 1.23x faster                                                          |
| go                         | 137 ms                                                          | 113 ms: 1.22x faster                                                           |
| xml_etree_iterparse        | 77.6 ms                                                         | 63.9 ms: 1.21x faster                                                          |
| sqlglot_transpile          | 1.53 ms                                                         | 1.26 ms: 1.21x faster                                                          |
| nqueens                    | 93.7 ms                                                         | 77.6 ms: 1.21x faster                                                          |
| richards                   | 41.3 ms                                                         | 34.4 ms: 1.20x faster                                                          |
| pycparser                  | 978 ms                                                          | 817 ms: 1.20x faster                                                           |
| richards_super             | 46.5 ms                                                         | 38.9 ms: 1.19x faster                                                          |
| dulwich_log                | 58.5 ms                                                         | 49.8 ms: 1.17x faster                                                          |
| coroutines                 | 20.9 ms                                                         | 18.0 ms: 1.16x faster                                                          |
| django_template            | 36.9 ms                                                         | 32.9 ms: 1.12x faster                                                          |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 503 ms: 1.12x faster                                                           |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 489 ms: 1.12x faster                                                           |
| sympy_sum                  | 123 ms                                                          | 112 ms: 1.10x faster                                                           |
| sqlglot_optimize           | 48.5 ms                                                         | 44.1 ms: 1.10x faster                                                          |
| bench_thread_pool          | 1.10 ms                                                         | 1.01 ms: 1.09x faster                                                          |
| mdp                        | 1.91 sec                                                        | 1.76 sec: 1.09x faster                                                         |
| sympy_str                  | 240 ms                                                          | 222 ms: 1.08x faster                                                           |
| 2to3                       | 280 ms                                                          | 259 ms: 1.08x faster                                                           |
| sympy_integrate            | 17.5 ms                                                         | 16.4 ms: 1.07x faster                                                          |
| xml_etree_parse            | 113 ms                                                          | 106 ms: 1.06x faster                                                           |
| json_dumps                 | 7.40 ms                                                         | 7.02 ms: 1.05x faster                                                          |
| asyncio_tcp_ssl            | 17.7 sec                                                        | 17.0 sec: 1.04x faster                                                         |
| sqlglot_normalize          | 100 ms                                                          | 97.7 ms: 1.03x faster                                                          |
| pathlib                    | 91.4 ms                                                         | 89.9 ms: 1.02x faster                                                          |
| sympy_expand               | 398 ms                                                          | 391 ms: 1.02x faster                                                           |
| docutils                   | 1.98 sec                                                        | 1.96 sec: 1.01x faster                                                         |
| pidigits                   | 199 ms                                                          | 198 ms: 1.01x faster                                                           |
| async_generators           | 313 ms                                                          | 312 ms: 1.00x faster                                                           |
| regex_dna                  | 127 ms                                                          | 128 ms: 1.01x slower                                                           |
| telco                      | 5.51 ms                                                         | 5.62 ms: 1.02x slower                                                          |
| json_loads                 | 20.4 us                                                         | 20.8 us: 1.02x slower                                                          |
| tornado_http               | 105 ms                                                          | 107 ms: 1.02x slower                                                           |
| regex_effbot               | 2.04 ms                                                         | 2.09 ms: 1.02x slower                                                          |
| gc_traversal               | 1.44 ms                                                         | 1.48 ms: 1.03x slower                                                          |
| regex_v8                   | 15.0 ms                                                         | 16.1 ms: 1.07x slower                                                          |
| json                       | 4.15 ms                                                         | 4.44 ms: 1.07x slower                                                          |
| coverage                   | 48.4 ms                                                         | 51.9 ms: 1.07x slower                                                          |
| bench_mp_pool              | 75.4 ms                                                         | 81.2 ms: 1.08x slower                                                          |
| python_startup_no_site     | 19.1 ms                                                         | 20.7 ms: 1.08x slower                                                          |
| python_startup             | 22.4 ms                                                         | 24.4 ms: 1.09x slower                                                          |
| typing_runtime_protocols   | 126 us                                                          | 147 us: 1.17x slower                                                           |
| create_gc_cycles           | 652 us                                                          | 763 us: 1.17x slower                                                           |
| Geometric mean             | (ref)                                                           | 1.24x faster                                                                   |

Benchmark hidden because not significant (1): asyncio_tcp
Ignored benchmarks (13) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240810-3.14.0a0-363374c-JIT/bm-20240810-pythonperf1_win32-x86-python-363374cf69a7e2292fe3-3.14.0a0-363374c.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.16x
- 95% likely to have a speedup of 1.14x
- 99% likely to have a speedup of 1.11x

# Memory
- memory change: unknown