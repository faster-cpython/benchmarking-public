# Results vs. 3.13.0

- fork: mdboom
- ref: simplify_richcompare
- machine: windows-x86
- commit hash: c9a5962
- commit date: 2024-08-29
- overall geometric mean: 1.03x faster
- HPT reliability: 99.33%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 253 ms                                                          | 248 ms: 1.02x faster                                                           |
| docutils       | 1.82 sec                                                        | 1.86 sec: 1.02x slower                                                         |
| html5lib       | 48.3 ms                                                         | 45.8 ms: 1.05x faster                                                          |
| Geometric mean | (ref)                                                           | 1.01x faster                                                                   |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| asyncio_tcp                | 842 ms                                                          | 722 ms: 1.17x faster                                                           |
| async_tree_memoization_tg  | 287 ms                                                          | 249 ms: 1.15x faster                                                           |
| async_tree_none            | 246 ms                                                          | 213 ms: 1.15x faster                                                           |
| async_tree_memoization     | 302 ms                                                          | 274 ms: 1.10x faster                                                           |
| async_tree_none_tg         | 219 ms                                                          | 199 ms: 1.10x faster                                                           |
| async_tree_cpu_io_mixed    | 494 ms                                                          | 464 ms: 1.07x faster                                                           |
| asyncio_tcp_ssl            | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                         |
| async_tree_cpu_io_mixed_tg | 464 ms                                                          | 460 ms: 1.01x faster                                                           |
| async_generators           | 274 ms                                                          | 292 ms: 1.07x slower                                                           |
| coroutines                 | 15.7 ms                                                         | 17.4 ms: 1.11x slower                                                          |
| Geometric mean             | (ref)                                                           | 1.05x faster                                                                   |

Benchmark hidden because not significant (2): async_tree_io, async_tree_io_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| pidigits       | 202 ms                                                          | 201 ms: 1.01x faster                                                           |
| float          | 57.8 ms                                                         | 62.2 ms: 1.08x slower                                                          |
| nbody          | 81.9 ms                                                         | 94.3 ms: 1.15x slower                                                          |
| Geometric mean | (ref)                                                           | 1.07x slower                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_dna      | 117 ms                                                          | 118 ms: 1.02x slower                                                           |
| regex_v8       | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                                          |
| regex_compile  | 103 ms                                                          | 107 ms: 1.04x slower                                                           |
| regex_effbot   | 1.81 ms                                                         | 1.92 ms: 1.06x slower                                                          |
| Geometric mean | (ref)                                                           | 1.04x slower                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| json_loads           | 21.0 us                                                         | 20.3 us: 1.03x faster                                                          |
| xml_etree_iterparse  | 65.1 ms                                                         | 68.8 ms: 1.06x slower                                                          |
| tomli_loads          | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                         |
| xml_etree_generate   | 62.6 ms                                                         | 67.5 ms: 1.08x slower                                                          |
| pickle_pure_python   | 238 us                                                          | 263 us: 1.10x slower                                                           |
| xml_etree_process    | 45.0 ms                                                         | 49.9 ms: 1.11x slower                                                          |
| unpickle_pure_python | 162 us                                                          | 183 us: 1.13x slower                                                           |
| Geometric mean       | (ref)                                                           | 1.06x slower                                                                   |

Benchmark hidden because not significant (2): json_dumps, xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup | 24.3 ms                                                         | 24.0 ms: 1.01x faster                                                          |
| Geometric mean | (ref)                                                           | 1.00x faster                                                                   |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| genshi_xml      | 49.5 ms                                                         | 46.1 ms: 1.08x faster                                                          |
| django_template | 32.7 ms                                                         | 31.9 ms: 1.02x faster                                                          |
| genshi_text     | 22.4 ms                                                         | 22.0 ms: 1.02x faster                                                          |
| mako            | 7.31 ms                                                         | 8.24 ms: 1.13x slower                                                          |
| Geometric mean  | (ref)                                                           | 1.00x slower                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240829-pythonperf1_win32-x86-mdboom-simplify_richcompare-3.14.0a0-c9a5962 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| thrift                     | 10.1 ms                                                         | 719 us: 14.12x faster                                                          |
| coverage                   | 333 ms                                                          | 53.6 ms: 6.21x faster                                                          |
| deepcopy                   | 307 us                                                          | 242 us: 1.27x faster                                                           |
| deepcopy_reduce            | 2.85 us                                                         | 2.42 us: 1.18x faster                                                          |
| deepcopy_memo              | 26.2 us                                                         | 22.2 us: 1.18x faster                                                          |
| asyncio_tcp                | 842 ms                                                          | 722 ms: 1.17x faster                                                           |
| async_tree_memoization_tg  | 287 ms                                                          | 249 ms: 1.15x faster                                                           |
| async_tree_none            | 246 ms                                                          | 213 ms: 1.15x faster                                                           |
| async_tree_memoization     | 302 ms                                                          | 274 ms: 1.10x faster                                                           |
| async_tree_none_tg         | 219 ms                                                          | 199 ms: 1.10x faster                                                           |
| genshi_xml                 | 49.5 ms                                                         | 46.1 ms: 1.08x faster                                                          |
| go                         | 111 ms                                                          | 104 ms: 1.07x faster                                                           |
| async_tree_cpu_io_mixed    | 494 ms                                                          | 464 ms: 1.07x faster                                                           |
| html5lib                   | 48.3 ms                                                         | 45.8 ms: 1.05x faster                                                          |
| json_loads                 | 21.0 us                                                         | 20.3 us: 1.03x faster                                                          |
| crypto_pyaes               | 58.2 ms                                                         | 56.7 ms: 1.03x faster                                                          |
| pathlib                    | 89.4 ms                                                         | 87.4 ms: 1.02x faster                                                          |
| django_template            | 32.7 ms                                                         | 31.9 ms: 1.02x faster                                                          |
| 2to3                       | 253 ms                                                          | 248 ms: 1.02x faster                                                           |
| genshi_text                | 22.4 ms                                                         | 22.0 ms: 1.02x faster                                                          |
| asyncio_tcp_ssl            | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                         |
| bench_mp_pool              | 74.3 ms                                                         | 73.2 ms: 1.02x faster                                                          |
| python_startup             | 24.3 ms                                                         | 24.0 ms: 1.01x faster                                                          |
| async_tree_cpu_io_mixed_tg | 464 ms                                                          | 460 ms: 1.01x faster                                                           |
| pycparser                  | 869 ms                                                          | 862 ms: 1.01x faster                                                           |
| pidigits                   | 202 ms                                                          | 201 ms: 1.01x faster                                                           |
| dulwich_log                | 49.7 ms                                                         | 49.9 ms: 1.01x slower                                                          |
| logging_format             | 8.57 us                                                         | 8.61 us: 1.01x slower                                                          |
| logging_simple             | 7.87 us                                                         | 7.94 us: 1.01x slower                                                          |
| regex_dna                  | 117 ms                                                          | 118 ms: 1.02x slower                                                           |
| sympy_expand               | 375 ms                                                          | 383 ms: 1.02x slower                                                           |
| sympy_integrate            | 15.2 ms                                                         | 15.5 ms: 1.02x slower                                                          |
| sqlglot_parse              | 1.05 ms                                                         | 1.07 ms: 1.02x slower                                                          |
| sqlglot_transpile          | 1.29 ms                                                         | 1.32 ms: 1.02x slower                                                          |
| pprint_pformat             | 1.30 sec                                                        | 1.33 sec: 1.02x slower                                                         |
| docutils                   | 1.82 sec                                                        | 1.86 sec: 1.02x slower                                                         |
| sqlglot_normalize          | 220 ms                                                          | 225 ms: 1.02x slower                                                           |
| mdp                        | 1.67 sec                                                        | 1.72 sec: 1.03x slower                                                         |
| chaos                      | 51.2 ms                                                         | 52.8 ms: 1.03x slower                                                          |
| sqlglot_optimize           | 42.5 ms                                                         | 43.8 ms: 1.03x slower                                                          |
| regex_v8                   | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                                          |
| regex_compile              | 103 ms                                                          | 107 ms: 1.04x slower                                                           |
| pylint                     | 225 ms                                                          | 234 ms: 1.04x slower                                                           |
| create_gc_cycles           | 713 us                                                          | 744 us: 1.04x slower                                                           |
| typing_runtime_protocols   | 136 us                                                          | 143 us: 1.05x slower                                                           |
| xml_etree_iterparse        | 65.1 ms                                                         | 68.8 ms: 1.06x slower                                                          |
| regex_effbot               | 1.81 ms                                                         | 1.92 ms: 1.06x slower                                                          |
| tomli_loads                | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                         |
| async_generators           | 274 ms                                                          | 292 ms: 1.07x slower                                                           |
| comprehensions             | 12.7 us                                                         | 13.7 us: 1.07x slower                                                          |
| meteor_contest             | 77.0 ms                                                         | 82.8 ms: 1.07x slower                                                          |
| float                      | 57.8 ms                                                         | 62.2 ms: 1.08x slower                                                          |
| xml_etree_generate         | 62.6 ms                                                         | 67.5 ms: 1.08x slower                                                          |
| scimark_sparse_mat_mult    | 2.90 ms                                                         | 3.14 ms: 1.08x slower                                                          |
| scimark_lu                 | 63.5 ms                                                         | 68.7 ms: 1.08x slower                                                          |
| pyflate                    | 326 ms                                                          | 354 ms: 1.09x slower                                                           |
| spectral_norm              | 71.3 ms                                                         | 77.6 ms: 1.09x slower                                                          |
| pickle_pure_python         | 238 us                                                          | 263 us: 1.10x slower                                                           |
| xml_etree_process          | 45.0 ms                                                         | 49.9 ms: 1.11x slower                                                          |
| coroutines                 | 15.7 ms                                                         | 17.4 ms: 1.11x slower                                                          |
| raytrace                   | 205 ms                                                          | 229 ms: 1.12x slower                                                           |
| scimark_sor                | 91.8 ms                                                         | 103 ms: 1.12x slower                                                           |
| hexiom                     | 4.64 ms                                                         | 5.20 ms: 1.12x slower                                                          |
| deltablue                  | 2.41 ms                                                         | 2.71 ms: 1.13x slower                                                          |
| mako                       | 7.31 ms                                                         | 8.24 ms: 1.13x slower                                                          |
| unpickle_pure_python       | 162 us                                                          | 183 us: 1.13x slower                                                           |
| scimark_monte_carlo        | 50.3 ms                                                         | 57.4 ms: 1.14x slower                                                          |
| fannkuch                   | 293 ms                                                          | 336 ms: 1.15x slower                                                           |
| scimark_fft                | 206 ms                                                          | 237 ms: 1.15x slower                                                           |
| nbody                      | 81.9 ms                                                         | 94.3 ms: 1.15x slower                                                          |
| richards_super             | 38.0 ms                                                         | 43.9 ms: 1.16x slower                                                          |
| richards                   | 33.8 ms                                                         | 39.4 ms: 1.17x slower                                                          |
| logging_silent             | 61.6 ns                                                         | 73.1 ns: 1.19x slower                                                          |
| generators                 | 22.1 ms                                                         | 26.8 ms: 1.21x slower                                                          |
| Geometric mean             | (ref)                                                           | 1.03x faster                                                                   |

Benchmark hidden because not significant (14): sympy_sum, async_tree_io, gc_traversal, pprint_safe_repr, sympy_str, json_dumps, tornado_http, xml_etree_parse, nqueens, telco, bench_thread_pool, python_startup_no_site, json, async_tree_io_tg
Ignored benchmarks (9) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5.json: chameleon, flaskblogging, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 99.33% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown