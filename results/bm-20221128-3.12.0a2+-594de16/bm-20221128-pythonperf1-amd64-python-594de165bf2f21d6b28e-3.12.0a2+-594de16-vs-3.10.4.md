
# Results vs. 3.10.4

- fork: python
- ref: 594de165bf2f21d6b28e
- machine: windows-amd64
- commit hash: 594de16
- commit date: 2022-11-28
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.17x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 193 ms: 1.23x faster                                                        |
| chameleon      | 5.92 ms                                                     | 4.45 ms: 1.33x faster                                                       |
| docutils       | 1.89 sec                                                    | 1.56 sec: 1.21x faster                                                      |
| html5lib       | 46.5 ms                                                     | 35.7 ms: 1.30x faster                                                       |
| tornado_http   | 109 ms                                                      | 92.1 ms: 1.18x faster                                                       |
| Geometric mean | (ref)                                                       | 1.25x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 48.3 ms: 1.25x faster                                                       |
| nbody          | 69.3 ms                                                     | 57.1 ms: 1.21x faster                                                       |
| pidigits       | 145 ms                                                      | 150 ms: 1.04x slower                                                        |
| Geometric mean | (ref)                                                       | 1.13x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 79.4 ms: 1.30x faster                                                       |
| regex_dna      | 132 ms                                                      | 121 ms: 1.09x faster                                                        |
| regex_v8       | 15.0 ms                                                     | 14.1 ms: 1.07x faster                                                       |
| regex_effbot   | 1.66 ms                                                     | 1.57 ms: 1.06x faster                                                       |
| Geometric mean | (ref)                                                       | 1.13x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.36 ms: 1.59x faster                                                       |
| pickle_pure_python   | 257 us                                                      | 176 us: 1.46x faster                                                        |
| unpickle_pure_python | 171 us                                                      | 121 us: 1.41x faster                                                        |
| xml_etree_process    | 43.4 ms                                                     | 33.7 ms: 1.29x faster                                                       |
| unpickle             | 9.17 us                                                     | 7.78 us: 1.18x faster                                                       |
| xml_etree_parse      | 102 ms                                                      | 88.8 ms: 1.15x faster                                                       |
| xml_etree_generate   | 54.5 ms                                                     | 49.2 ms: 1.11x faster                                                       |
| unpickle_list        | 2.81 us                                                     | 2.61 us: 1.08x faster                                                       |
| json_loads           | 14.2 us                                                     | 13.2 us: 1.07x faster                                                       |
| xml_etree_iterparse  | 63.5 ms                                                     | 60.8 ms: 1.04x faster                                                       |
| pickle_list          | 2.59 us                                                     | 2.85 us: 1.10x slower                                                       |
| pickle_dict          | 16.9 us                                                     | 19.0 us: 1.13x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.15x faster                                                                |

Benchmark hidden because not significant (1): pickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.6 ms: 1.07x faster                                                       |
| python_startup_no_site | 15.5 ms                                                     | 15.8 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                       | 1.03x faster                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|-----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 6.14 ms: 1.43x faster                                                       |
| django_template | 28.2 ms                                                     | 20.5 ms: 1.37x faster                                                       |
| genshi_text     | 19.0 ms                                                     | 13.9 ms: 1.37x faster                                                       |
| genshi_xml      | 40.5 ms                                                     | 32.0 ms: 1.27x faster                                                       |
| Geometric mean  | (ref)                                                       | 1.36x faster                                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221128-pythonperf1-amd64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|-------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 1.97 ms: 2.11x faster                                                       |
| richards                | 41.2 ms                                                     | 23.8 ms: 1.73x faster                                                       |
| logging_silent          | 96.4 ns                                                     | 56.7 ns: 1.70x faster                                                       |
| mypy2                   | 352 ms                                                      | 215 ms: 1.64x faster                                                        |
| scimark_sor             | 105 ms                                                      | 64.4 ms: 1.63x faster                                                       |
| go                      | 136 ms                                                      | 84.3 ms: 1.61x faster                                                       |
| json_dumps              | 8.50 ms                                                     | 5.36 ms: 1.59x faster                                                       |
| scimark_lu              | 85.4 ms                                                     | 55.0 ms: 1.55x faster                                                       |
| raytrace                | 271 ms                                                      | 177 ms: 1.53x faster                                                        |
| scimark_monte_carlo     | 55.9 ms                                                     | 36.9 ms: 1.51x faster                                                       |
| pickle_pure_python      | 257 us                                                      | 176 us: 1.46x faster                                                        |
| pyflate                 | 387 ms                                                      | 266 ms: 1.46x faster                                                        |
| sqlglot_parse           | 1.22 ms                                                     | 838 us: 1.45x faster                                                        |
| crypto_pyaes            | 62.3 ms                                                     | 43.4 ms: 1.44x faster                                                       |
| hexiom                  | 5.52 ms                                                     | 3.85 ms: 1.43x faster                                                       |
| mako                    | 8.80 ms                                                     | 6.14 ms: 1.43x faster                                                       |
| unpack_sequence         | 37.8 ns                                                     | 26.4 ns: 1.43x faster                                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.03 ms: 1.42x faster                                                       |
| unpickle_pure_python    | 171 us                                                      | 121 us: 1.41x faster                                                        |
| deepcopy_memo           | 28.5 us                                                     | 20.3 us: 1.41x faster                                                       |
| async_tree_io           | 1.07 sec                                                    | 767 ms: 1.39x faster                                                        |
| chaos                   | 58.9 ms                                                     | 42.5 ms: 1.39x faster                                                       |
| django_template         | 28.2 ms                                                     | 20.5 ms: 1.37x faster                                                       |
| genshi_text             | 19.0 ms                                                     | 13.9 ms: 1.37x faster                                                       |
| pycparser               | 868 ms                                                      | 638 ms: 1.36x faster                                                        |
| pprint_pformat          | 1.21 sec                                                    | 900 ms: 1.34x faster                                                        |
| async_tree_none         | 420 ms                                                      | 315 ms: 1.33x faster                                                        |
| async_tree_memoization  | 497 ms                                                      | 373 ms: 1.33x faster                                                        |
| async_generators        | 224 ms                                                      | 168 ms: 1.33x faster                                                        |
| chameleon               | 5.92 ms                                                     | 4.45 ms: 1.33x faster                                                       |
| pprint_safe_repr        | 589 ms                                                      | 443 ms: 1.33x faster                                                        |
| thrift                  | 615 us                                                      | 466 us: 1.32x faster                                                        |
| regex_compile           | 103 ms                                                      | 79.4 ms: 1.30x faster                                                       |
| html5lib                | 46.5 ms                                                     | 35.7 ms: 1.30x faster                                                       |
| xml_etree_process       | 43.4 ms                                                     | 33.7 ms: 1.29x faster                                                       |
| spectral_norm           | 78.0 ms                                                     | 61.0 ms: 1.28x faster                                                       |
| genshi_xml              | 40.5 ms                                                     | 32.0 ms: 1.27x faster                                                       |
| float                   | 60.2 ms                                                     | 48.3 ms: 1.25x faster                                                       |
| async_tree_cpu_io_mixed | 609 ms                                                      | 493 ms: 1.24x faster                                                        |
| 2to3                    | 237 ms                                                      | 193 ms: 1.23x faster                                                        |
| sqlglot_optimize        | 39.0 ms                                                     | 31.8 ms: 1.23x faster                                                       |
| nbody                   | 69.3 ms                                                     | 57.1 ms: 1.21x faster                                                       |
| docutils                | 1.89 sec                                                    | 1.56 sec: 1.21x faster                                                      |
| fannkuch                | 258 ms                                                      | 215 ms: 1.20x faster                                                        |
| sqlglot_normalize       | 202 ms                                                      | 168 ms: 1.20x faster                                                        |
| deepcopy                | 255 us                                                      | 214 us: 1.19x faster                                                        |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.24 ms: 1.19x faster                                                       |
| tornado_http            | 109 ms                                                      | 92.1 ms: 1.18x faster                                                       |
| scimark_fft             | 193 ms                                                      | 163 ms: 1.18x faster                                                        |
| unpickle                | 9.17 us                                                     | 7.78 us: 1.18x faster                                                       |
| dulwich_log             | 47.6 ms                                                     | 40.5 ms: 1.18x faster                                                       |
| nqueens                 | 67.0 ms                                                     | 57.1 ms: 1.17x faster                                                       |
| deepcopy_reduce         | 2.16 us                                                     | 1.84 us: 1.17x faster                                                       |
| dask                    | 305 ms                                                      | 261 ms: 1.17x faster                                                        |
| json                    | 3.05 ms                                                     | 2.65 ms: 1.15x faster                                                       |
| xml_etree_parse         | 102 ms                                                      | 88.8 ms: 1.15x faster                                                       |
| bench_thread_pool       | 946 us                                                      | 830 us: 1.14x faster                                                        |
| sympy_integrate         | 14.8 ms                                                     | 13.0 ms: 1.14x faster                                                       |
| sympy_expand            | 315 ms                                                      | 278 ms: 1.13x faster                                                        |
| mdp                     | 1.71 sec                                                    | 1.52 sec: 1.13x faster                                                      |
| create_gc_cycles        | 782 us                                                      | 700 us: 1.12x faster                                                        |
| xml_etree_generate      | 54.5 ms                                                     | 49.2 ms: 1.11x faster                                                       |
| coroutines              | 15.9 ms                                                     | 14.6 ms: 1.09x faster                                                       |
| regex_dna               | 132 ms                                                      | 121 ms: 1.09x faster                                                        |
| meteor_contest          | 72.5 ms                                                     | 66.7 ms: 1.09x faster                                                       |
| sympy_str               | 188 ms                                                      | 174 ms: 1.08x faster                                                        |
| unpickle_list           | 2.81 us                                                     | 2.61 us: 1.08x faster                                                       |
| json_loads              | 14.2 us                                                     | 13.2 us: 1.07x faster                                                       |
| python_startup          | 20.0 ms                                                     | 18.6 ms: 1.07x faster                                                       |
| sympy_sum               | 104 ms                                                      | 97.4 ms: 1.07x faster                                                       |
| regex_v8                | 15.0 ms                                                     | 14.1 ms: 1.07x faster                                                       |
| regex_effbot            | 1.66 ms                                                     | 1.57 ms: 1.06x faster                                                       |
| sqlite_synth            | 1.84 us                                                     | 1.74 us: 1.06x faster                                                       |
| comprehensions          | 16.0 us                                                     | 15.1 us: 1.06x faster                                                       |
| pathlib                 | 77.4 ms                                                     | 73.7 ms: 1.05x faster                                                       |
| xml_etree_iterparse     | 63.5 ms                                                     | 60.8 ms: 1.04x faster                                                       |
| telco                   | 3.78 ms                                                     | 3.73 ms: 1.01x faster                                                       |
| logging_format          | 6.66 us                                                     | 6.73 us: 1.01x slower                                                       |
| logging_simple          | 6.20 us                                                     | 6.27 us: 1.01x slower                                                       |
| python_startup_no_site  | 15.5 ms                                                     | 15.8 ms: 1.02x slower                                                       |
| bench_mp_pool           | 60.7 ms                                                     | 62.9 ms: 1.04x slower                                                       |
| pidigits                | 145 ms                                                      | 150 ms: 1.04x slower                                                        |
| gc_traversal            | 1.34 ms                                                     | 1.47 ms: 1.09x slower                                                       |
| generators              | 31.6 ms                                                     | 34.8 ms: 1.10x slower                                                       |
| pickle_list             | 2.59 us                                                     | 2.85 us: 1.10x slower                                                       |
| pickle_dict             | 16.9 us                                                     | 19.0 us: 1.13x slower                                                       |
| coverage                | 40.0 ms                                                     | 52.5 ms: 1.31x slower                                                       |
| Geometric mean          | (ref)                                                       | 1.22x faster                                                                |

Benchmark hidden because not significant (2): pickle, asyncio_tcp
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.19x
- 95% likely to have a speedup of 1.18x
- 99% likely to have a speedup of 1.17x
