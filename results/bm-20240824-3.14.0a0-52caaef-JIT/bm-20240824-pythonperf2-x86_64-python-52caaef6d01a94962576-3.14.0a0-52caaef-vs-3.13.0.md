# Results vs. 3.13.0

- fork: python
- ref: 52caaef6d01a94962576
- machine: linux-x86_64
- commit hash: 52caaef
- commit date: 2024-08-24
- overall geometric mean: 1.01x faster
- HPT reliability: 75.06%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.10x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                       | 308 ms: 1.06x slower                                                        |
| docutils       | 2.81 sec                                                     | 3.24 sec: 1.15x slower                                                      |
| html5lib       | 71.9 ms                                                      | 71.0 ms: 1.01x faster                                                       |
| tornado_http   | 120 ms                                                       | 121 ms: 1.01x slower                                                        |
| Geometric mean | (ref)                                                        | 1.05x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 461 ms                                                       | 394 ms: 1.17x faster                                                        |
| async_tree_none            | 372 ms                                                       | 329 ms: 1.13x faster                                                        |
| async_tree_memoization     | 452 ms                                                       | 402 ms: 1.12x faster                                                        |
| async_tree_none_tg         | 340 ms                                                       | 312 ms: 1.09x faster                                                        |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 565 ms: 1.06x faster                                                        |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 550 ms: 1.05x faster                                                        |
| asyncio_tcp                | 380 ms                                                       | 373 ms: 1.02x faster                                                        |
| asyncio_websockets         | 382 ms                                                       | 395 ms: 1.03x slower                                                        |
| async_generators           | 359 ms                                                       | 390 ms: 1.09x slower                                                        |
| Geometric mean             | (ref)                                                        | 1.04x faster                                                                |

Benchmark hidden because not significant (4): async_tree_io_tg, async_tree_io, asyncio_tcp_ssl, coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 88.0 ms                                                      | 80.4 ms: 1.09x faster                                                       |
| float          | 81.9 ms                                                      | 75.0 ms: 1.09x faster                                                       |
| pidigits       | 253 ms                                                       | 251 ms: 1.01x faster                                                        |
| Geometric mean | (ref)                                                        | 1.06x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_dna      | 244 ms                                                       | 233 ms: 1.05x faster                                                        |
| regex_v8       | 26.2 ms                                                      | 25.5 ms: 1.03x faster                                                       |
| regex_compile  | 144 ms                                                       | 146 ms: 1.01x slower                                                        |
| regex_effbot   | 3.37 ms                                                      | 3.49 ms: 1.04x slower                                                       |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| tomli_loads          | 2.41 sec                                                     | 2.12 sec: 1.13x faster                                                      |
| xml_etree_process    | 60.9 ms                                                      | 55.5 ms: 1.10x faster                                                       |
| xml_etree_generate   | 85.3 ms                                                      | 79.0 ms: 1.08x faster                                                       |
| json_dumps           | 11.0 ms                                                      | 10.3 ms: 1.07x faster                                                       |
| xml_etree_parse      | 145 ms                                                       | 142 ms: 1.02x faster                                                        |
| pickle_pure_python   | 318 us                                                       | 320 us: 1.01x slower                                                        |
| unpickle_pure_python | 214 us                                                       | 216 us: 1.01x slower                                                        |
| json_loads           | 24.0 us                                                      | 24.8 us: 1.03x slower                                                       |
| Geometric mean       | (ref)                                                        | 1.04x faster                                                                |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                       |
| python_startup_no_site | 8.94 ms                                                      | 9.04 ms: 1.01x slower                                                       |
| Geometric mean         | (ref)                                                        | 1.01x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 10.4 ms                                                      | 9.21 ms: 1.13x faster                                                       |
| genshi_text     | 26.6 ms                                                      | 28.1 ms: 1.05x slower                                                       |
| django_template | 38.9 ms                                                      | 42.6 ms: 1.10x slower                                                       |
| genshi_xml      | 57.3 ms                                                      | 63.2 ms: 1.10x slower                                                       |
| Geometric mean  | (ref)                                                        | 1.03x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240824-pythonperf2-x86_64-python-52caaef6d01a94962576-3.14.0a0-52caaef |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo              | 39.5 us                                                      | 26.8 us: 1.47x faster                                                       |
| deepcopy                   | 397 us                                                       | 295 us: 1.35x faster                                                        |
| scimark_sor                | 126 ms                                                       | 98.9 ms: 1.27x faster                                                       |
| deepcopy_reduce            | 3.54 us                                                      | 2.92 us: 1.21x faster                                                       |
| spectral_norm              | 97.4 ms                                                      | 81.2 ms: 1.20x faster                                                       |
| async_tree_memoization_tg  | 461 ms                                                       | 394 ms: 1.17x faster                                                        |
| richards                   | 52.7 ms                                                      | 45.3 ms: 1.16x faster                                                       |
| richards_super             | 59.8 ms                                                      | 52.2 ms: 1.15x faster                                                       |
| tomli_loads                | 2.41 sec                                                     | 2.12 sec: 1.13x faster                                                      |
| async_tree_none            | 372 ms                                                       | 329 ms: 1.13x faster                                                        |
| mako                       | 10.4 ms                                                      | 9.21 ms: 1.13x faster                                                       |
| async_tree_memoization     | 452 ms                                                       | 402 ms: 1.12x faster                                                        |
| scimark_fft                | 314 ms                                                       | 282 ms: 1.12x faster                                                        |
| pathlib                    | 17.4 ms                                                      | 15.9 ms: 1.10x faster                                                       |
| xml_etree_process          | 60.9 ms                                                      | 55.5 ms: 1.10x faster                                                       |
| nbody                      | 88.0 ms                                                      | 80.4 ms: 1.09x faster                                                       |
| float                      | 81.9 ms                                                      | 75.0 ms: 1.09x faster                                                       |
| async_tree_none_tg         | 340 ms                                                       | 312 ms: 1.09x faster                                                        |
| pyflate                    | 492 ms                                                       | 453 ms: 1.09x faster                                                        |
| xml_etree_generate         | 85.3 ms                                                      | 79.0 ms: 1.08x faster                                                       |
| json_dumps                 | 11.0 ms                                                      | 10.3 ms: 1.07x faster                                                       |
| telco                      | 8.58 ms                                                      | 8.04 ms: 1.07x faster                                                       |
| scimark_sparse_mat_mult    | 4.29 ms                                                      | 4.02 ms: 1.07x faster                                                       |
| deltablue                  | 3.41 ms                                                      | 3.21 ms: 1.06x faster                                                       |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 565 ms: 1.06x faster                                                        |
| bpe_tokeniser              | 5.10 sec                                                     | 4.80 sec: 1.06x faster                                                      |
| regex_dna                  | 244 ms                                                       | 233 ms: 1.05x faster                                                        |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 550 ms: 1.05x faster                                                        |
| crypto_pyaes               | 72.8 ms                                                      | 70.4 ms: 1.03x faster                                                       |
| pprint_safe_repr           | 812 ms                                                       | 786 ms: 1.03x faster                                                        |
| fannkuch                   | 365 ms                                                       | 355 ms: 1.03x faster                                                        |
| regex_v8                   | 26.2 ms                                                      | 25.5 ms: 1.03x faster                                                       |
| xml_etree_parse            | 145 ms                                                       | 142 ms: 1.02x faster                                                        |
| asyncio_tcp                | 380 ms                                                       | 373 ms: 1.02x faster                                                        |
| logging_format             | 7.07 us                                                      | 6.96 us: 1.02x faster                                                       |
| coverage                   | 81.1 ms                                                      | 80.0 ms: 1.01x faster                                                       |
| html5lib                   | 71.9 ms                                                      | 71.0 ms: 1.01x faster                                                       |
| pidigits                   | 253 ms                                                       | 251 ms: 1.01x faster                                                        |
| pprint_pformat             | 1.65 sec                                                     | 1.64 sec: 1.01x faster                                                      |
| python_startup             | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                       |
| pickle_pure_python         | 318 us                                                       | 320 us: 1.01x slower                                                        |
| unpickle_pure_python       | 214 us                                                       | 216 us: 1.01x slower                                                        |
| regex_compile              | 144 ms                                                       | 146 ms: 1.01x slower                                                        |
| python_startup_no_site     | 8.94 ms                                                      | 9.04 ms: 1.01x slower                                                       |
| tornado_http               | 120 ms                                                       | 121 ms: 1.01x slower                                                        |
| meteor_contest             | 128 ms                                                       | 131 ms: 1.02x slower                                                        |
| thrift                     | 877 us                                                       | 893 us: 1.02x slower                                                        |
| pycparser                  | 1.26 sec                                                     | 1.28 sec: 1.02x slower                                                      |
| bench_thread_pool          | 901 us                                                       | 923 us: 1.02x slower                                                        |
| mdp                        | 2.52 sec                                                     | 2.59 sec: 1.03x slower                                                      |
| json                       | 5.22 ms                                                      | 5.39 ms: 1.03x slower                                                       |
| asyncio_websockets         | 382 ms                                                       | 395 ms: 1.03x slower                                                        |
| json_loads                 | 24.0 us                                                      | 24.8 us: 1.03x slower                                                       |
| regex_effbot               | 3.37 ms                                                      | 3.49 ms: 1.04x slower                                                       |
| scimark_monte_carlo        | 64.9 ms                                                      | 67.3 ms: 1.04x slower                                                       |
| sympy_expand               | 505 ms                                                       | 524 ms: 1.04x slower                                                        |
| logging_silent             | 97.7 ns                                                      | 102 ns: 1.04x slower                                                        |
| typing_runtime_protocols   | 174 us                                                       | 182 us: 1.04x slower                                                        |
| sympy_str                  | 296 ms                                                       | 310 ms: 1.05x slower                                                        |
| genshi_text                | 26.6 ms                                                      | 28.1 ms: 1.05x slower                                                       |
| 2to3                       | 291 ms                                                       | 308 ms: 1.06x slower                                                        |
| gc_traversal               | 4.11 ms                                                      | 4.38 ms: 1.07x slower                                                       |
| comprehensions             | 17.3 us                                                      | 18.5 us: 1.07x slower                                                       |
| raytrace                   | 289 ms                                                       | 310 ms: 1.07x slower                                                        |
| bench_mp_pool              | 4.65 ms                                                      | 5.01 ms: 1.08x slower                                                       |
| sqlglot_parse              | 1.38 ms                                                      | 1.49 ms: 1.08x slower                                                       |
| chaos                      | 61.7 ms                                                      | 66.6 ms: 1.08x slower                                                       |
| async_generators           | 359 ms                                                       | 390 ms: 1.09x slower                                                        |
| sqlglot_transpile          | 1.78 ms                                                      | 1.94 ms: 1.09x slower                                                       |
| hexiom                     | 6.33 ms                                                      | 6.92 ms: 1.09x slower                                                       |
| create_gc_cycles           | 1.76 ms                                                      | 1.92 ms: 1.09x slower                                                       |
| django_template            | 38.9 ms                                                      | 42.6 ms: 1.10x slower                                                       |
| sqlglot_optimize           | 59.7 ms                                                      | 65.7 ms: 1.10x slower                                                       |
| genshi_xml                 | 57.3 ms                                                      | 63.2 ms: 1.10x slower                                                       |
| sympy_sum                  | 154 ms                                                       | 170 ms: 1.10x slower                                                        |
| sqlglot_normalize          | 118 ms                                                       | 131 ms: 1.10x slower                                                        |
| generators                 | 33.5 ms                                                      | 37.5 ms: 1.12x slower                                                       |
| nqueens                    | 88.2 ms                                                      | 99.3 ms: 1.13x slower                                                       |
| sympy_integrate            | 23.3 ms                                                      | 26.5 ms: 1.14x slower                                                       |
| docutils                   | 2.81 sec                                                     | 3.24 sec: 1.15x slower                                                      |
| pylint                     | 346 ms                                                       | 406 ms: 1.17x slower                                                        |
| go                         | 160 ms                                                       | 192 ms: 1.19x slower                                                        |
| Geometric mean             | (ref)                                                        | 1.01x faster                                                                |

Benchmark hidden because not significant (7): async_tree_io_tg, async_tree_io, scimark_lu, xml_etree_iterparse, logging_simple, asyncio_tcp_ssl, coroutines
Ignored benchmarks (14) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dask, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 75.06% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.10x