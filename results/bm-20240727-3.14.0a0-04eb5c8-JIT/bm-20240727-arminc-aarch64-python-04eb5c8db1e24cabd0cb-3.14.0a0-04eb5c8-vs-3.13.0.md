# Results vs. 3.13.0

- fork: python
- ref: 04eb5c8db1e24cabd0cb
- machine: linux-aarch64
- commit hash: 04eb5c8
- commit date: 2024-07-27
- overall geometric mean: 1.07x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x slower
- Memory change: 1.09x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 359 ms: 1.17x slower                                                    |
| docutils       | 2.91 sec                                                 | 3.61 sec: 1.24x slower                                                  |
| html5lib       | 64.3 ms                                                  | 71.0 ms: 1.10x slower                                                   |
| tornado_http   | 131 ms                                                   | 140 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                    | 1.14x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 546 ms: 1.19x faster                                                    |
| async_tree_none            | 493 ms                                                   | 434 ms: 1.14x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 413 ms: 1.13x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 570 ms: 1.10x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 700 ms: 1.05x faster                                                    |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 738 ms: 1.03x faster                                                    |
| asyncio_websockets         | 656 ms                                                   | 660 ms: 1.01x slower                                                    |
| async_generators           | 496 ms                                                   | 505 ms: 1.02x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.23 sec: 1.02x slower                                                  |
| async_tree_io              | 1.11 sec                                                 | 1.14 sec: 1.03x slower                                                  |
| asyncio_tcp                | 568 ms                                                   | 617 ms: 1.09x slower                                                    |
| Geometric mean             | (ref)                                                    | 1.03x faster                                                            |

Benchmark hidden because not significant (2): coroutines, async_tree_io_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 90.9 ms: 1.04x faster                                                   |
| nbody          | 114 ms                                                   | 112 ms: 1.02x faster                                                    |
| Geometric mean | (ref)                                                    | 1.02x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.5 ms                                                  | 30.2 ms: 1.04x faster                                                   |
| regex_dna      | 246 ms                                                   | 249 ms: 1.01x slower                                                    |
| regex_compile  | 128 ms                                                   | 180 ms: 1.40x slower                                                    |
| Geometric mean | (ref)                                                    | 1.08x slower                                                            |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_loads           | 31.4 us                                                  | 32.6 us: 1.04x slower                                                   |
| tomli_loads          | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                  |
| unpickle_pure_python | 254 us                                                   | 272 us: 1.07x slower                                                    |
| pickle_pure_python   | 359 us                                                   | 396 us: 1.10x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.03x slower                                                            |

Benchmark hidden because not significant (5): json_dumps, xml_etree_generate, xml_etree_iterparse, xml_etree_parse, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                   |
| python_startup_no_site | 8.75 ms                                                  | 8.92 ms: 1.02x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.00x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 12.8 ms: 1.04x faster                                                   |
| django_template | 42.3 ms                                                  | 50.7 ms: 1.20x slower                                                   |
| genshi_text     | 27.7 ms                                                  | 35.0 ms: 1.26x slower                                                   |
| genshi_xml      | 60.2 ms                                                  | 82.9 ms: 1.38x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.19x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 51.0 us                                                  | 38.7 us: 1.32x faster                                                   |
| async_tree_memoization_tg  | 649 ms                                                   | 546 ms: 1.19x faster                                                    |
| deepcopy                   | 451 us                                                   | 380 us: 1.19x faster                                                    |
| async_tree_none            | 493 ms                                                   | 434 ms: 1.14x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 413 ms: 1.13x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 570 ms: 1.10x faster                                                    |
| generators                 | 37.8 ms                                                  | 35.6 ms: 1.06x faster                                                   |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 700 ms: 1.05x faster                                                    |
| regex_v8                   | 31.5 ms                                                  | 30.2 ms: 1.04x faster                                                   |
| float                      | 94.4 ms                                                  | 90.9 ms: 1.04x faster                                                   |
| mako                       | 13.3 ms                                                  | 12.8 ms: 1.04x faster                                                   |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 738 ms: 1.03x faster                                                    |
| nbody                      | 114 ms                                                   | 112 ms: 1.02x faster                                                    |
| python_startup             | 13.3 ms                                                  | 13.1 ms: 1.01x faster                                                   |
| asyncio_websockets         | 656 ms                                                   | 660 ms: 1.01x slower                                                    |
| bpe_tokeniser              | 5.90 sec                                                 | 5.95 sec: 1.01x slower                                                  |
| regex_dna                  | 246 ms                                                   | 249 ms: 1.01x slower                                                    |
| async_generators           | 496 ms                                                   | 505 ms: 1.02x slower                                                    |
| scimark_fft                | 447 ms                                                   | 456 ms: 1.02x slower                                                    |
| bench_thread_pool          | 1.28 ms                                                  | 1.30 ms: 1.02x slower                                                   |
| richards_super             | 60.3 ms                                                  | 61.4 ms: 1.02x slower                                                   |
| python_startup_no_site     | 8.75 ms                                                  | 8.92 ms: 1.02x slower                                                   |
| mdp                        | 3.36 sec                                                 | 3.44 sec: 1.02x slower                                                  |
| spectral_norm              | 141 ms                                                   | 144 ms: 1.02x slower                                                    |
| meteor_contest             | 113 ms                                                   | 116 ms: 1.02x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.23 sec: 1.02x slower                                                  |
| pathlib                    | 22.4 ms                                                  | 23.0 ms: 1.03x slower                                                   |
| async_tree_io              | 1.11 sec                                                 | 1.14 sec: 1.03x slower                                                  |
| logging_simple             | 7.04 us                                                  | 7.24 us: 1.03x slower                                                   |
| scimark_sparse_mat_mult    | 6.58 ms                                                  | 6.77 ms: 1.03x slower                                                   |
| deepcopy_reduce            | 4.07 us                                                  | 4.20 us: 1.03x slower                                                   |
| json_loads                 | 31.4 us                                                  | 32.6 us: 1.04x slower                                                   |
| thrift                     | 946 us                                                   | 981 us: 1.04x slower                                                    |
| json                       | 5.61 ms                                                  | 5.86 ms: 1.04x slower                                                   |
| scimark_monte_carlo        | 83.8 ms                                                  | 87.7 ms: 1.05x slower                                                   |
| tomli_loads                | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                  |
| telco                      | 9.73 ms                                                  | 10.3 ms: 1.06x slower                                                   |
| fannkuch                   | 452 ms                                                   | 479 ms: 1.06x slower                                                    |
| tornado_http               | 131 ms                                                   | 140 ms: 1.06x slower                                                    |
| pyflate                    | 556 ms                                                   | 593 ms: 1.07x slower                                                    |
| crypto_pyaes               | 82.7 ms                                                  | 88.4 ms: 1.07x slower                                                   |
| unpickle_pure_python       | 254 us                                                   | 272 us: 1.07x slower                                                    |
| asyncio_tcp                | 568 ms                                                   | 617 ms: 1.09x slower                                                    |
| typing_runtime_protocols   | 192 us                                                   | 209 us: 1.09x slower                                                    |
| create_gc_cycles           | 2.12 ms                                                  | 2.33 ms: 1.10x slower                                                   |
| pickle_pure_python         | 359 us                                                   | 396 us: 1.10x slower                                                    |
| raytrace                   | 298 ms                                                   | 329 ms: 1.10x slower                                                    |
| html5lib                   | 64.3 ms                                                  | 71.0 ms: 1.10x slower                                                   |
| scimark_sor                | 159 ms                                                   | 176 ms: 1.11x slower                                                    |
| go                         | 163 ms                                                   | 180 ms: 1.11x slower                                                    |
| gc_traversal               | 4.53 ms                                                  | 5.04 ms: 1.11x slower                                                   |
| dask                       | 350 ms                                                   | 392 ms: 1.12x slower                                                    |
| pycparser                  | 1.26 sec                                                 | 1.41 sec: 1.12x slower                                                  |
| deltablue                  | 3.85 ms                                                  | 4.32 ms: 1.12x slower                                                   |
| sqlglot_normalize          | 128 ms                                                   | 145 ms: 1.13x slower                                                    |
| sqlglot_optimize           | 62.4 ms                                                  | 71.2 ms: 1.14x slower                                                   |
| bench_mp_pool              | 7.30 ms                                                  | 8.38 ms: 1.15x slower                                                   |
| comprehensions             | 20.4 us                                                  | 23.5 us: 1.16x slower                                                   |
| sqlglot_parse              | 1.38 ms                                                  | 1.60 ms: 1.16x slower                                                   |
| sqlglot_transpile          | 1.73 ms                                                  | 2.03 ms: 1.17x slower                                                   |
| 2to3                       | 306 ms                                                   | 359 ms: 1.17x slower                                                    |
| django_template            | 42.3 ms                                                  | 50.7 ms: 1.20x slower                                                   |
| nqueens                    | 98.7 ms                                                  | 119 ms: 1.20x slower                                                    |
| pprint_safe_repr           | 926 ms                                                   | 1.15 sec: 1.24x slower                                                  |
| docutils                   | 2.91 sec                                                 | 3.61 sec: 1.24x slower                                                  |
| pylint                     | 343 ms                                                   | 428 ms: 1.25x slower                                                    |
| sympy_expand               | 455 ms                                                   | 571 ms: 1.26x slower                                                    |
| pprint_pformat             | 1.90 sec                                                 | 2.38 sec: 1.26x slower                                                  |
| genshi_text                | 27.7 ms                                                  | 35.0 ms: 1.26x slower                                                   |
| chaos                      | 68.8 ms                                                  | 87.5 ms: 1.27x slower                                                   |
| hexiom                     | 7.13 ms                                                  | 9.16 ms: 1.28x slower                                                   |
| sympy_str                  | 264 ms                                                   | 339 ms: 1.29x slower                                                    |
| sympy_integrate            | 21.0 ms                                                  | 27.1 ms: 1.29x slower                                                   |
| scimark_lu                 | 139 ms                                                   | 181 ms: 1.30x slower                                                    |
| genshi_xml                 | 60.2 ms                                                  | 82.9 ms: 1.38x slower                                                   |
| sympy_sum                  | 143 ms                                                   | 199 ms: 1.38x slower                                                    |
| regex_compile              | 128 ms                                                   | 180 ms: 1.40x slower                                                    |
| Geometric mean             | (ref)                                                    | 1.07x slower                                                            |

Benchmark hidden because not significant (13): json_dumps, xml_etree_generate, coverage, xml_etree_iterparse, xml_etree_parse, coroutines, logging_silent, pidigits, async_tree_io_tg, richards, regex_effbot, logging_format, xml_etree_process
Ignored benchmarks (12) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.03x
- 95% likely to have a slowdown of 1.02x
- 99% likely to have a slowdown of 1.02x

# Memory
- memory change: 1.09x