
# Results vs. 3.11.0

- fork: python
- ref: 67a798888dcde13bbb1e
- machine: linux-x86_64
- commit hash: 67a7988
- commit date: 2023-07-06
- overall geometric mean: 1.04x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 2.63 sec                                               | 2.65 sec: 1.01x slower                                                |
| Geometric mean | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 198 ms                                                 | 197 ms: 1.01x faster                                                  |
| float          | 77.2 ms                                                | 78.5 ms: 1.02x slower                                                 |
| Geometric mean | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (1): nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.99 ms                                                | 3.53 ms: 1.13x faster                                                 |
| regex_compile  | 138 ms                                                 | 136 ms: 1.02x faster                                                  |
| regex_dna      | 204 ms                                                 | 208 ms: 1.02x slower                                                  |
| Geometric mean | (ref)                                                  | 1.03x faster                                                          |

Benchmark hidden because not significant (1): regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 12.6 ms                                                | 9.74 ms: 1.29x faster                                                 |
| unpickle_pure_python | 228 us                                                 | 211 us: 1.08x faster                                                  |
| xml_etree_parse      | 158 ms                                                 | 153 ms: 1.04x faster                                                  |
| json_loads           | 26.5 us                                                | 25.8 us: 1.03x faster                                                 |
| pickle_dict          | 31.1 us                                                | 30.9 us: 1.01x faster                                                 |
| unpickle_list        | 4.91 us                                                | 4.89 us: 1.00x faster                                                 |
| tomli_loads          | 2.22 sec                                               | 2.35 sec: 1.06x slower                                                |
| xml_etree_process    | 53.9 ms                                                | 57.4 ms: 1.06x slower                                                 |
| pickle               | 10.1 us                                                | 10.8 us: 1.07x slower                                                 |
| unpickle             | 13.7 us                                                | 14.8 us: 1.08x slower                                                 |
| pickle_list          | 4.11 us                                                | 4.51 us: 1.10x slower                                                 |
| xml_etree_generate   | 76.2 ms                                                | 83.7 ms: 1.10x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (2): xml_etree_iterparse, pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 8.52 ms                                                | 9.16 ms: 1.07x slower                                                 |
| python_startup_no_site | 6.01 ms                                                | 6.67 ms: 1.11x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.09x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|-----------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako      | 10.1 ms                                                | 10.6 ms: 1.05x slower                                                 |

All benchmarks:
===============

| Benchmark                | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230706-linux-x86_64-python-67a798888dcde13bbb1e-3.13.0a0-67a7988 |
|--------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| typing_runtime_protocols | 486 us                                                 | 141 us: 3.45x faster                                                  |
| generators               | 73.5 ms                                                | 29.5 ms: 2.49x faster                                                 |
| asyncio_tcp              | 922 ms                                                 | 512 ms: 1.80x faster                                                  |
| asyncio_tcp_ssl          | 3.14 sec                                               | 1.79 sec: 1.75x faster                                                |
| json_dumps               | 12.6 ms                                                | 9.74 ms: 1.29x faster                                                 |
| mypy2                    | 420 ms                                                 | 336 ms: 1.25x faster                                                  |
| chaos                    | 69.2 ms                                                | 59.5 ms: 1.16x faster                                                 |
| coroutines               | 25.5 ms                                                | 22.4 ms: 1.14x faster                                                 |
| regex_effbot             | 3.99 ms                                                | 3.53 ms: 1.13x faster                                                 |
| deltablue                | 3.67 ms                                                | 3.26 ms: 1.13x faster                                                 |
| raytrace                 | 297 ms                                                 | 266 ms: 1.12x faster                                                  |
| comprehensions           | 22.4 us                                                | 20.4 us: 1.10x faster                                                 |
| async_tree_none          | 526 ms                                                 | 482 ms: 1.09x faster                                                  |
| async_tree_io            | 1.30 sec                                               | 1.20 sec: 1.08x faster                                                |
| unpickle_pure_python     | 228 us                                                 | 211 us: 1.08x faster                                                  |
| richards_super           | 56.8 ms                                                | 53.0 ms: 1.07x faster                                                 |
| coverage                 | 100 ms                                                 | 93.4 ms: 1.07x faster                                                 |
| async_tree_memoization   | 627 ms                                                 | 588 ms: 1.07x faster                                                  |
| hexiom                   | 6.37 ms                                                | 6.03 ms: 1.06x faster                                                 |
| sqlglot_parse            | 1.40 ms                                                | 1.33 ms: 1.05x faster                                                 |
| nqueens                  | 83.4 ms                                                | 79.4 ms: 1.05x faster                                                 |
| xml_etree_parse          | 158 ms                                                 | 153 ms: 1.04x faster                                                  |
| mdp                      | 2.62 sec                                               | 2.52 sec: 1.04x faster                                                |
| sqlglot_transpile        | 1.70 ms                                                | 1.65 ms: 1.03x faster                                                 |
| go                       | 140 ms                                                 | 136 ms: 1.03x faster                                                  |
| json_loads               | 26.5 us                                                | 25.8 us: 1.03x faster                                                 |
| logging_format           | 6.68 us                                                | 6.52 us: 1.03x faster                                                 |
| logging_simple           | 6.03 us                                                | 5.89 us: 1.02x faster                                                 |
| sqlglot_normalize        | 108 ms                                                 | 106 ms: 1.02x faster                                                  |
| async_tree_cpu_io_mixed  | 739 ms                                                 | 725 ms: 1.02x faster                                                  |
| pycparser                | 1.18 sec                                               | 1.16 sec: 1.02x faster                                                |
| meteor_contest           | 107 ms                                                 | 105 ms: 1.02x faster                                                  |
| regex_compile            | 138 ms                                                 | 136 ms: 1.02x faster                                                  |
| pidigits                 | 198 ms                                                 | 197 ms: 1.01x faster                                                  |
| pickle_dict              | 31.1 us                                                | 30.9 us: 1.01x faster                                                 |
| fannkuch                 | 388 ms                                                 | 386 ms: 1.01x faster                                                  |
| sqlglot_optimize         | 53.1 ms                                                | 52.8 ms: 1.00x faster                                                 |
| unpickle_list            | 4.91 us                                                | 4.89 us: 1.00x faster                                                 |
| bench_thread_pool        | 819 us                                                 | 816 us: 1.00x faster                                                  |
| deepcopy_memo            | 37.0 us                                                | 37.3 us: 1.01x slower                                                 |
| pprint_safe_repr         | 701 ms                                                 | 708 ms: 1.01x slower                                                  |
| docutils                 | 2.63 sec                                               | 2.65 sec: 1.01x slower                                                |
| gc_traversal             | 4.02 ms                                                | 4.07 ms: 1.01x slower                                                 |
| float                    | 77.2 ms                                                | 78.5 ms: 1.02x slower                                                 |
| logging_silent           | 101 ns                                                 | 103 ns: 1.02x slower                                                  |
| scimark_sor              | 118 ms                                                 | 121 ms: 1.02x slower                                                  |
| regex_dna                | 204 ms                                                 | 208 ms: 1.02x slower                                                  |
| scimark_lu               | 110 ms                                                 | 112 ms: 1.02x slower                                                  |
| pathlib                  | 18.2 ms                                                | 18.7 ms: 1.03x slower                                                 |
| richards                 | 45.7 ms                                                | 47.0 ms: 1.03x slower                                                 |
| dulwich_log              | 63.7 ms                                                | 65.7 ms: 1.03x slower                                                 |
| crypto_pyaes             | 74.7 ms                                                | 78.0 ms: 1.05x slower                                                 |
| mako                     | 10.1 ms                                                | 10.6 ms: 1.05x slower                                                 |
| deepcopy_reduce          | 2.94 us                                                | 3.09 us: 1.05x slower                                                 |
| scimark_sparse_mat_mult  | 4.50 ms                                                | 4.74 ms: 1.06x slower                                                 |
| tomli_loads              | 2.22 sec                                               | 2.35 sec: 1.06x slower                                                |
| xml_etree_process        | 53.9 ms                                                | 57.4 ms: 1.06x slower                                                 |
| scimark_monte_carlo      | 68.1 ms                                                | 72.5 ms: 1.07x slower                                                 |
| pyflate                  | 418 ms                                                 | 446 ms: 1.07x slower                                                  |
| spectral_norm            | 100 ms                                                 | 107 ms: 1.07x slower                                                  |
| pickle                   | 10.1 us                                                | 10.8 us: 1.07x slower                                                 |
| python_startup           | 8.52 ms                                                | 9.16 ms: 1.07x slower                                                 |
| unpickle                 | 13.7 us                                                | 14.8 us: 1.08x slower                                                 |
| scimark_fft              | 328 ms                                                 | 357 ms: 1.09x slower                                                  |
| sqlite_synth             | 2.52 us                                                | 2.75 us: 1.09x slower                                                 |
| telco                    | 6.58 ms                                                | 7.20 ms: 1.09x slower                                                 |
| pickle_list              | 4.11 us                                                | 4.51 us: 1.10x slower                                                 |
| xml_etree_generate       | 76.2 ms                                                | 83.7 ms: 1.10x slower                                                 |
| python_startup_no_site   | 6.01 ms                                                | 6.67 ms: 1.11x slower                                                 |
| unpack_sequence          | 43.1 ns                                                | 49.2 ns: 1.14x slower                                                 |
| async_generators         | 368 ms                                                 | 450 ms: 1.22x slower                                                  |
| dask                     | 360 ms                                                 | 517 ms: 1.44x slower                                                  |
| Geometric mean           | (ref)                                                  | 1.04x faster                                                          |

Benchmark hidden because not significant (10): nbody, json, xml_etree_iterparse, tornado_http, create_gc_cycles, pprint_pformat, bench_mp_pool, regex_v8, pickle_pure_python, deepcopy
Ignored benchmarks (18) of results/bm-20221024-3.11.0-deaf509/bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, djangocms, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
