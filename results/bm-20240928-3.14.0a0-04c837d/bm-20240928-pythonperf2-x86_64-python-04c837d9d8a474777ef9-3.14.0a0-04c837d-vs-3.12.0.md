# Results vs. 3.12.0

- fork: python
- ref: 04c837d9d8a474777ef9
- machine: linux-x86_64
- commit hash: 04c837d
- commit date: 2024-09-28
- overall geometric mean: 1.04x slower
- HPT reliability: 86.07%
- HPT 99th percentile: 1.00x faster
- Memory change: 0.93x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 285 ms                                                       | 283 ms: 1.01x faster                                                        |
| docutils       | 2.87 sec                                                     | 2.89 sec: 1.01x slower                                                      |
| tornado_http   | 121 ms                                                       | 116 ms: 1.05x faster                                                        |
| Geometric mean | (ref)                                                        | 1.02x faster                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_none            | 452 ms                                                       | 321 ms: 1.41x faster                                                        |
| async_tree_none_tg         | 431 ms                                                       | 309 ms: 1.39x faster                                                        |
| async_tree_memoization_tg  | 540 ms                                                       | 391 ms: 1.38x faster                                                        |
| async_tree_memoization     | 544 ms                                                       | 400 ms: 1.36x faster                                                        |
| async_tree_io_tg           | 1.05 sec                                                     | 791 ms: 1.33x faster                                                        |
| async_tree_io              | 1.04 sec                                                     | 812 ms: 1.28x faster                                                        |
| async_tree_cpu_io_mixed_tg | 697 ms                                                       | 565 ms: 1.23x faster                                                        |
| async_tree_cpu_io_mixed    | 696 ms                                                       | 572 ms: 1.22x faster                                                        |
| Geometric mean             | (ref)                                                        | 1.32x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pidigits       | 265 ms                                                       | 253 ms: 1.05x faster                                                        |
| nbody          | 88.0 ms                                                      | 85.8 ms: 1.03x faster                                                       |
| float          | 76.6 ms                                                      | 80.5 ms: 1.05x slower                                                       |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_effbot   | 3.57 ms                                                      | 3.44 ms: 1.04x faster                                                       |
| regex_compile  | 144 ms                                                       | 140 ms: 1.03x faster                                                        |
| regex_dna      | 239 ms                                                       | 245 ms: 1.03x slower                                                        |
| regex_v8       | 23.6 ms                                                      | 25.1 ms: 1.06x slower                                                       |
| Geometric mean | (ref)                                                        | 1.01x slower                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pickle_dict          | 32.5 us                                                      | 30.1 us: 1.08x faster                                                       |
| pickle               | 10.5 us                                                      | 10.2 us: 1.04x faster                                                       |
| pickle_list          | 4.43 us                                                      | 4.34 us: 1.02x faster                                                       |
| pickle_pure_python   | 318 us                                                       | 316 us: 1.01x faster                                                        |
| xml_etree_generate   | 86.1 ms                                                      | 85.8 ms: 1.00x faster                                                       |
| unpickle_pure_python | 210 us                                                       | 214 us: 1.02x slower                                                        |
| unpickle             | 14.8 us                                                      | 15.2 us: 1.02x slower                                                       |
| json_loads           | 24.4 us                                                      | 25.2 us: 1.03x slower                                                       |
| xml_etree_process    | 58.4 ms                                                      | 60.5 ms: 1.04x slower                                                       |
| xml_etree_parse      | 144 ms                                                       | 149 ms: 1.04x slower                                                        |
| json_dumps           | 10.2 ms                                                      | 10.9 ms: 1.06x slower                                                       |
| tomli_loads          | 2.16 sec                                                     | 2.60 sec: 1.21x slower                                                      |
| Geometric mean       | (ref)                                                        | 1.02x slower                                                                |

Benchmark hidden because not significant (2): unpickle_list, xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup_no_site | 8.64 ms                                                      | 8.97 ms: 1.04x slower                                                       |
| python_startup         | 11.6 ms                                                      | 13.4 ms: 1.15x slower                                                       |
| Geometric mean         | (ref)                                                        | 1.09x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 10.0 ms                                                      | 10.3 ms: 1.03x slower                                                       |
| django_template | 38.2 ms                                                      | 39.4 ms: 1.03x slower                                                       |
| Geometric mean  | (ref)                                                        | 1.03x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_none            | 452 ms                                                       | 321 ms: 1.41x faster                                                        |
| async_tree_none_tg         | 431 ms                                                       | 309 ms: 1.39x faster                                                        |
| async_tree_memoization_tg  | 540 ms                                                       | 391 ms: 1.38x faster                                                        |
| async_tree_memoization     | 544 ms                                                       | 400 ms: 1.36x faster                                                        |
| async_tree_io_tg           | 1.05 sec                                                     | 791 ms: 1.33x faster                                                        |
| deepcopy                   | 368 us                                                       | 284 us: 1.30x faster                                                        |
| comprehensions             | 21.9 us                                                      | 16.9 us: 1.29x faster                                                       |
| generators                 | 37.4 ms                                                      | 29.0 ms: 1.29x faster                                                       |
| async_tree_io              | 1.04 sec                                                     | 812 ms: 1.28x faster                                                        |
| deepcopy_memo              | 36.8 us                                                      | 28.9 us: 1.27x faster                                                       |
| async_tree_cpu_io_mixed_tg | 697 ms                                                       | 565 ms: 1.23x faster                                                        |
| async_tree_cpu_io_mixed    | 696 ms                                                       | 572 ms: 1.22x faster                                                        |
| pathlib                    | 18.9 ms                                                      | 15.6 ms: 1.21x faster                                                       |
| deepcopy_reduce            | 3.37 us                                                      | 2.90 us: 1.16x faster                                                       |
| raytrace                   | 298 ms                                                       | 270 ms: 1.11x faster                                                        |
| crypto_pyaes               | 80.3 ms                                                      | 72.8 ms: 1.10x faster                                                       |
| go                         | 150 ms                                                       | 136 ms: 1.10x faster                                                        |
| async_generators           | 390 ms                                                       | 360 ms: 1.08x faster                                                        |
| sympy_sum                  | 162 ms                                                       | 150 ms: 1.08x faster                                                        |
| pickle_dict                | 32.5 us                                                      | 30.1 us: 1.08x faster                                                       |
| logging_format             | 7.48 us                                                      | 6.97 us: 1.07x faster                                                       |
| scimark_monte_carlo        | 69.0 ms                                                      | 64.5 ms: 1.07x faster                                                       |
| logging_simple             | 6.71 us                                                      | 6.34 us: 1.06x faster                                                       |
| sympy_integrate            | 23.9 ms                                                      | 22.7 ms: 1.05x faster                                                       |
| tornado_http               | 121 ms                                                       | 116 ms: 1.05x faster                                                        |
| pidigits                   | 265 ms                                                       | 253 ms: 1.05x faster                                                        |
| sympy_str                  | 302 ms                                                       | 289 ms: 1.04x faster                                                        |
| scimark_fft                | 301 ms                                                       | 289 ms: 1.04x faster                                                        |
| bench_thread_pool          | 950 us                                                       | 914 us: 1.04x faster                                                        |
| regex_effbot               | 3.57 ms                                                      | 3.44 ms: 1.04x faster                                                       |
| scimark_lu                 | 98.8 ms                                                      | 95.4 ms: 1.04x faster                                                       |
| pickle                     | 10.5 us                                                      | 10.2 us: 1.04x faster                                                       |
| coroutines                 | 23.0 ms                                                      | 22.2 ms: 1.03x faster                                                       |
| mdp                        | 2.57 sec                                                     | 2.48 sec: 1.03x faster                                                      |
| regex_compile              | 144 ms                                                       | 140 ms: 1.03x faster                                                        |
| scimark_sparse_mat_mult    | 4.21 ms                                                      | 4.08 ms: 1.03x faster                                                       |
| chaos                      | 64.0 ms                                                      | 62.2 ms: 1.03x faster                                                       |
| unpack_sequence            | 53.2 ns                                                      | 51.8 ns: 1.03x faster                                                       |
| nbody                      | 88.0 ms                                                      | 85.8 ms: 1.03x faster                                                       |
| meteor_contest             | 128 ms                                                       | 125 ms: 1.02x faster                                                        |
| pickle_list                | 4.43 us                                                      | 4.34 us: 1.02x faster                                                       |
| sqlite_synth               | 2.77 us                                                      | 2.73 us: 1.02x faster                                                       |
| asyncio_tcp                | 378 ms                                                       | 373 ms: 1.01x faster                                                        |
| 2to3                       | 285 ms                                                       | 283 ms: 1.01x faster                                                        |
| pickle_pure_python         | 318 us                                                       | 316 us: 1.01x faster                                                        |
| nqueens                    | 89.9 ms                                                      | 89.5 ms: 1.00x faster                                                       |
| xml_etree_generate         | 86.1 ms                                                      | 85.8 ms: 1.00x faster                                                       |
| sqlglot_normalize          | 116 ms                                                       | 116 ms: 1.01x slower                                                        |
| docutils                   | 2.87 sec                                                     | 2.89 sec: 1.01x slower                                                      |
| asyncio_websockets         | 387 ms                                                       | 392 ms: 1.01x slower                                                        |
| dulwich_log                | 65.4 ms                                                      | 66.3 ms: 1.01x slower                                                       |
| sqlglot_parse              | 1.38 ms                                                      | 1.40 ms: 1.02x slower                                                       |
| unpickle_pure_python       | 210 us                                                       | 214 us: 1.02x slower                                                        |
| sqlglot_optimize           | 57.5 ms                                                      | 58.7 ms: 1.02x slower                                                       |
| sympy_expand               | 484 ms                                                       | 495 ms: 1.02x slower                                                        |
| unpickle                   | 14.8 us                                                      | 15.2 us: 1.02x slower                                                       |
| mako                       | 10.0 ms                                                      | 10.3 ms: 1.03x slower                                                       |
| regex_dna                  | 239 ms                                                       | 245 ms: 1.03x slower                                                        |
| django_template            | 38.2 ms                                                      | 39.4 ms: 1.03x slower                                                       |
| json_loads                 | 24.4 us                                                      | 25.2 us: 1.03x slower                                                       |
| json                       | 5.12 ms                                                      | 5.30 ms: 1.04x slower                                                       |
| xml_etree_process          | 58.4 ms                                                      | 60.5 ms: 1.04x slower                                                       |
| python_startup_no_site     | 8.64 ms                                                      | 8.97 ms: 1.04x slower                                                       |
| xml_etree_parse            | 144 ms                                                       | 149 ms: 1.04x slower                                                        |
| fannkuch                   | 350 ms                                                       | 364 ms: 1.04x slower                                                        |
| logging_silent             | 94.4 ns                                                      | 98.8 ns: 1.05x slower                                                       |
| deltablue                  | 3.24 ms                                                      | 3.39 ms: 1.05x slower                                                       |
| hexiom                     | 5.96 ms                                                      | 6.25 ms: 1.05x slower                                                       |
| float                      | 76.6 ms                                                      | 80.5 ms: 1.05x slower                                                       |
| spectral_norm              | 91.6 ms                                                      | 97.2 ms: 1.06x slower                                                       |
| regex_v8                   | 23.6 ms                                                      | 25.1 ms: 1.06x slower                                                       |
| json_dumps                 | 10.2 ms                                                      | 10.9 ms: 1.06x slower                                                       |
| scimark_sor                | 109 ms                                                       | 117 ms: 1.08x slower                                                        |
| richards_super             | 51.3 ms                                                      | 56.8 ms: 1.11x slower                                                       |
| richards                   | 45.7 ms                                                      | 50.8 ms: 1.11x slower                                                       |
| typing_runtime_protocols   | 152 us                                                       | 173 us: 1.14x slower                                                        |
| pyflate                    | 439 ms                                                       | 503 ms: 1.15x slower                                                        |
| python_startup             | 11.6 ms                                                      | 13.4 ms: 1.15x slower                                                       |
| telco                      | 6.96 ms                                                      | 8.21 ms: 1.18x slower                                                       |
| tomli_loads                | 2.16 sec                                                     | 2.60 sec: 1.21x slower                                                      |
| coverage                   | 66.7 ms                                                      | 81.5 ms: 1.22x slower                                                       |
| create_gc_cycles           | 1.59 ms                                                      | 1.97 ms: 1.24x slower                                                       |
| gc_traversal               | 3.48 ms                                                      | 4.44 ms: 1.28x slower                                                       |
| bench_mp_pool              | 4.76 ms                                                      | 2.16 sec: 452.43x slower                                                    |
| Geometric mean             | (ref)                                                        | 1.04x slower                                                                |

Benchmark hidden because not significant (7): pprint_pformat, asyncio_tcp_ssl, unpickle_list, sqlglot_transpile, pprint_safe_repr, xml_etree_iterparse, pycparser
Ignored benchmarks (7) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf2-x86_64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (6) of results/bm-20240928-3.14.0a0-04c837d/bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d.json: bpe_tokeniser, genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 86.07% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 0.93x