# Results vs. 3.13.0

- fork: python
- ref: 3597642ed57d184511ca
- machine: linux-aarch64
- commit hash: 3597642
- commit date: 2024-09-10
- overall geometric mean: 1.081x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x slower
- Memory change: 0.99x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 304 ms                                                   | 381 ms: 1.25x slower                                                    |
| docutils       | 2.89 sec                                                 | 3.94 sec: 1.36x slower                                                  |
| html5lib       | 66.4 ms                                                  | 72.3 ms: 1.09x slower                                                   |
| tornado_http   | 128 ms                                                   | 146 ms: 1.15x slower                                                    |
| Geometric mean | (ref)                                                    | 1.21x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 556 ms: 1.17x faster                                                    |
| async_tree_none_tg         | 470 ms                                                   | 423 ms: 1.11x faster                                                    |
| async_tree_none            | 497 ms                                                   | 447 ms: 1.11x faster                                                    |
| async_tree_memoization     | 651 ms                                                   | 588 ms: 1.11x faster                                                    |
| async_tree_cpu_io_mixed_tg | 769 ms                                                   | 739 ms: 1.04x faster                                                    |
| async_tree_cpu_io_mixed    | 766 ms                                                   | 755 ms: 1.02x faster                                                    |
| async_generators           | 489 ms                                                   | 503 ms: 1.03x slower                                                    |
| async_tree_io_tg           | 1.13 sec                                                 | 1.17 sec: 1.03x slower                                                  |
| async_tree_io              | 1.11 sec                                                 | 1.15 sec: 1.04x slower                                                  |
| Geometric mean             | (ref)                                                    | 1.04x faster                                                            |

Benchmark hidden because not significant (2): coroutines, asyncio_websockets

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 93.3 ms                                                  | 87.6 ms: 1.06x faster                                                   |
| Geometric mean | (ref)                                                    | 1.02x faster                                                            |

Benchmark hidden because not significant (2): nbody, pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.8 ms                                                  | 30.7 ms: 1.04x faster                                                   |
| regex_dna      | 253 ms                                                   | 245 ms: 1.03x faster                                                    |
| regex_compile  | 127 ms                                                   | 190 ms: 1.49x slower                                                    |
| Geometric mean | (ref)                                                    | 1.09x slower                                                            |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_parse      | 197 ms                                                   | 189 ms: 1.04x faster                                                    |
| xml_etree_generate   | 113 ms                                                   | 111 ms: 1.02x faster                                                    |
| json_loads           | 31.7 us                                                  | 31.4 us: 1.01x faster                                                   |
| tomli_loads          | 2.54 sec                                                 | 2.63 sec: 1.04x slower                                                  |
| unpickle_pure_python | 251 us                                                   | 265 us: 1.06x slower                                                    |
| pickle_pure_python   | 357 us                                                   | 388 us: 1.09x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                            |

Benchmark hidden because not significant (3): json_dumps, xml_etree_process, xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup | 15.4 ms                                                  | 13.0 ms: 1.19x faster                                                   |
| Geometric mean | (ref)                                                    | 1.09x faster                                                            |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.4 ms                                                  | 13.1 ms: 1.02x faster                                                   |
| django_template | 41.6 ms                                                  | 51.2 ms: 1.23x slower                                                   |
| genshi_text     | 27.7 ms                                                  | 34.7 ms: 1.25x slower                                                   |
| genshi_xml      | 60.3 ms                                                  | 81.2 ms: 1.35x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.19x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| create_gc_cycles           | 3.35 ms                                                  | 2.34 ms: 1.43x faster                                                   |
| deepcopy_memo              | 50.4 us                                                  | 37.5 us: 1.34x faster                                                   |
| python_startup             | 15.4 ms                                                  | 13.0 ms: 1.19x faster                                                   |
| async_tree_memoization_tg  | 649 ms                                                   | 556 ms: 1.17x faster                                                    |
| gc_traversal               | 5.77 ms                                                  | 5.18 ms: 1.11x faster                                                   |
| async_tree_none_tg         | 470 ms                                                   | 423 ms: 1.11x faster                                                    |
| async_tree_none            | 497 ms                                                   | 447 ms: 1.11x faster                                                    |
| async_tree_memoization     | 651 ms                                                   | 588 ms: 1.11x faster                                                    |
| deepcopy                   | 447 us                                                   | 411 us: 1.09x faster                                                    |
| float                      | 93.3 ms                                                  | 87.6 ms: 1.06x faster                                                   |
| scimark_sor                | 160 ms                                                   | 151 ms: 1.06x faster                                                    |
| async_tree_cpu_io_mixed_tg | 769 ms                                                   | 739 ms: 1.04x faster                                                    |
| xml_etree_parse            | 197 ms                                                   | 189 ms: 1.04x faster                                                    |
| regex_v8                   | 31.8 ms                                                  | 30.7 ms: 1.04x faster                                                   |
| deepcopy_reduce            | 4.07 us                                                  | 3.93 us: 1.04x faster                                                   |
| regex_dna                  | 253 ms                                                   | 245 ms: 1.03x faster                                                    |
| pathlib                    | 22.7 ms                                                  | 22.0 ms: 1.03x faster                                                   |
| xml_etree_generate         | 113 ms                                                   | 111 ms: 1.02x faster                                                    |
| mako                       | 13.4 ms                                                  | 13.1 ms: 1.02x faster                                                   |
| async_tree_cpu_io_mixed    | 766 ms                                                   | 755 ms: 1.02x faster                                                    |
| json_loads                 | 31.7 us                                                  | 31.4 us: 1.01x faster                                                   |
| bpe_tokeniser              | 5.87 sec                                                 | 5.97 sec: 1.02x slower                                                  |
| async_generators           | 489 ms                                                   | 503 ms: 1.03x slower                                                    |
| scimark_fft                | 447 ms                                                   | 460 ms: 1.03x slower                                                    |
| logging_silent             | 133 ns                                                   | 137 ns: 1.03x slower                                                    |
| async_tree_io_tg           | 1.13 sec                                                 | 1.17 sec: 1.03x slower                                                  |
| spectral_norm              | 143 ms                                                   | 147 ms: 1.03x slower                                                    |
| tomli_loads                | 2.54 sec                                                 | 2.63 sec: 1.04x slower                                                  |
| async_tree_io              | 1.11 sec                                                 | 1.15 sec: 1.04x slower                                                  |
| mdp                        | 3.34 sec                                                 | 3.50 sec: 1.05x slower                                                  |
| scimark_sparse_mat_mult    | 6.51 ms                                                  | 6.86 ms: 1.05x slower                                                   |
| logging_simple             | 7.07 us                                                  | 7.45 us: 1.05x slower                                                   |
| logging_format             | 7.82 us                                                  | 8.27 us: 1.06x slower                                                   |
| unpickle_pure_python       | 251 us                                                   | 265 us: 1.06x slower                                                    |
| scimark_lu                 | 139 ms                                                   | 148 ms: 1.06x slower                                                    |
| bench_thread_pool          | 1.27 ms                                                  | 1.36 ms: 1.07x slower                                                   |
| typing_runtime_protocols   | 193 us                                                   | 209 us: 1.08x slower                                                    |
| crypto_pyaes               | 83.7 ms                                                  | 90.7 ms: 1.08x slower                                                   |
| pickle_pure_python         | 357 us                                                   | 388 us: 1.09x slower                                                    |
| html5lib                   | 66.4 ms                                                  | 72.3 ms: 1.09x slower                                                   |
| meteor_contest             | 114 ms                                                   | 124 ms: 1.09x slower                                                    |
| fannkuch                   | 461 ms                                                   | 503 ms: 1.09x slower                                                    |
| telco                      | 9.74 ms                                                  | 10.7 ms: 1.10x slower                                                   |
| scimark_monte_carlo        | 83.6 ms                                                  | 92.2 ms: 1.10x slower                                                   |
| deltablue                  | 3.82 ms                                                  | 4.35 ms: 1.14x slower                                                   |
| tornado_http               | 128 ms                                                   | 146 ms: 1.15x slower                                                    |
| go                         | 160 ms                                                   | 186 ms: 1.16x slower                                                    |
| raytrace                   | 300 ms                                                   | 349 ms: 1.17x slower                                                    |
| pyflate                    | 556 ms                                                   | 655 ms: 1.18x slower                                                    |
| sqlglot_normalize          | 127 ms                                                   | 151 ms: 1.19x slower                                                    |
| pycparser                  | 1.27 sec                                                 | 1.53 sec: 1.20x slower                                                  |
| comprehensions             | 20.4 us                                                  | 24.6 us: 1.21x slower                                                   |
| django_template            | 41.6 ms                                                  | 51.2 ms: 1.23x slower                                                   |
| genshi_text                | 27.7 ms                                                  | 34.7 ms: 1.25x slower                                                   |
| 2to3                       | 304 ms                                                   | 381 ms: 1.25x slower                                                    |
| richards_super             | 60.1 ms                                                  | 75.4 ms: 1.25x slower                                                   |
| sqlglot_parse              | 1.38 ms                                                  | 1.74 ms: 1.26x slower                                                   |
| sqlglot_optimize           | 62.2 ms                                                  | 78.8 ms: 1.27x slower                                                   |
| nqueens                    | 100 ms                                                   | 127 ms: 1.27x slower                                                    |
| richards                   | 53.6 ms                                                  | 68.2 ms: 1.27x slower                                                   |
| sqlglot_transpile          | 1.73 ms                                                  | 2.21 ms: 1.27x slower                                                   |
| sympy_expand               | 457 ms                                                   | 612 ms: 1.34x slower                                                    |
| genshi_xml                 | 60.3 ms                                                  | 81.2 ms: 1.35x slower                                                   |
| chaos                      | 68.0 ms                                                  | 91.5 ms: 1.35x slower                                                   |
| pprint_safe_repr           | 926 ms                                                   | 1.25 sec: 1.35x slower                                                  |
| docutils                   | 2.89 sec                                                 | 3.94 sec: 1.36x slower                                                  |
| sympy_integrate            | 20.8 ms                                                  | 28.4 ms: 1.36x slower                                                   |
| pprint_pformat             | 1.90 sec                                                 | 2.61 sec: 1.37x slower                                                  |
| sympy_str                  | 264 ms                                                   | 364 ms: 1.38x slower                                                    |
| pylint                     | 342 ms                                                   | 475 ms: 1.39x slower                                                    |
| hexiom                     | 7.11 ms                                                  | 10.2 ms: 1.43x slower                                                   |
| regex_compile              | 127 ms                                                   | 190 ms: 1.49x slower                                                    |
| sympy_sum                  | 144 ms                                                   | 215 ms: 1.50x slower                                                    |
| generators                 | 37.6 ms                                                  | 56.9 ms: 1.51x slower                                                   |
| Geometric mean             | (ref)                                                    | 1.09x slower                                                            |

Benchmark hidden because not significant (13): bench_mp_pool, thrift, json_dumps, json, nbody, python_startup_no_site, pidigits, xml_etree_process, coroutines, asyncio_websockets, regex_effbot, coverage, xml_etree_iterparse
Ignored benchmarks (9) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, gevent_hub, k_core, mypy2, shortest_path, sphinx, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (10) of results/bm-20240910-3.14.0a0-3597642-JIT/bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642.json: asyncio_tcp, asyncio_tcp_ssl, dulwich_log, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.081x slower
# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.03x
- 99% likely to have a slowdown of 1.02x

# Memory
- memory change: 0.99x