# Results

- fork: python
- version: 3.12.0a3+
- commit hash: [f20c553](https://github.com/python/cpython/commit/f20c553)
- commit date: 2023-01-05T12:20:09+00:00
- ref: f20c553a458659f247fa

## linux x86_64 (linux)

- cpu model: Intel(R) Xeon(R) W-2255 CPU @ 3.70GHz
- platform: Linux-5.4.0-122-generic-x86_64-with-glibc2.31
- [raw results](bm-20230105-linux-x86_64-python-f20c553a458659f247fa-3.12.0a3%2B-f20c553.json)

### vs. 3.10.4

- 1.30x faster \*
- missing benchmarks: aiohttp, asyncio_tcp, asyncio_tcp_ssl, comprehensions, create_gc_cycles, dask, flaskblogging, gc_traversal, gunicorn, mypy2, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols
- new benchmarks: mypy
- [table](bm-20230105-linux-x86_64-python-f20c553a458659f247fa-3.12.0a3%2B-f20c553-vs-3.10.4.md)
- [plot](bm-20230105-linux-x86_64-python-f20c553a458659f247fa-3.12.0a3%2B-f20c553-vs-3.10.4.png)

### vs. 3.11.0

- 1.03x faster \*
- missing benchmarks: aiohttp, asyncio_tcp, asyncio_tcp_ssl, comprehensions, create_gc_cycles, dask, flaskblogging, gc_traversal, gunicorn, mypy2, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols
- new benchmarks: mypy
- [table](bm-20230105-linux-x86_64-python-f20c553a458659f247fa-3.12.0a3%2B-f20c553-vs-3.11.0.md)
- [plot](bm-20230105-linux-x86_64-python-f20c553a458659f247fa-3.12.0a3%2B-f20c553-vs-3.11.0.png)

