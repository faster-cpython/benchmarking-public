# Results vs. 3.12.0

- fork: python
- ref: 498376d7a7d6f704f22a
- machine: windows-amd64
- commit hash: 498376d
- commit date: 2024-08-02
- overall geometric mean: 1.04x slower
- HPT reliability: 99.99%
- HPT 99th percentile: 1.03x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 232 ms: 1.06x slower                                                       |
| docutils       | 1.66 sec                                                    | 1.79 sec: 1.08x slower                                                     |
| tornado_http   | 89.5 ms                                                     | 95.3 ms: 1.06x slower                                                      |
| Geometric mean | (ref)                                                       | 1.07x slower                                                               |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 255 ms: 1.44x faster                                                       |
| async_tree_none_tg         | 285 ms                                                      | 202 ms: 1.41x faster                                                       |
| async_tree_io_tg           | 771 ms                                                      | 561 ms: 1.37x faster                                                       |
| async_tree_none            | 291 ms                                                      | 225 ms: 1.29x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 398 ms: 1.26x faster                                                       |
| async_tree_memoization     | 339 ms                                                      | 272 ms: 1.25x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 397 ms: 1.23x faster                                                       |
| async_tree_io              | 731 ms                                                      | 599 ms: 1.22x faster                                                       |
| Geometric mean             | (ref)                                                       | 1.31x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| float          | 56.8 ms                                                     | 55.3 ms: 1.03x faster                                                      |
| pidigits       | 152 ms                                                      | 151 ms: 1.00x faster                                                       |
| nbody          | 71.9 ms                                                     | 91.2 ms: 1.27x slower                                                      |
| Geometric mean | (ref)                                                       | 1.07x slower                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 125 ms: 1.01x faster                                                       |
| regex_effbot   | 1.62 ms                                                     | 1.63 ms: 1.01x slower                                                      |
| regex_compile  | 87.5 ms                                                     | 94.2 ms: 1.08x slower                                                      |
| regex_v8       | 14.2 ms                                                     | 16.7 ms: 1.17x slower                                                      |
| Geometric mean | (ref)                                                       | 1.06x slower                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| xml_etree_iterparse  | 65.2 ms                                                     | 66.2 ms: 1.02x slower                                                      |
| json_loads           | 13.9 us                                                     | 14.1 us: 1.02x slower                                                      |
| xml_etree_parse      | 93.0 ms                                                     | 95.6 ms: 1.03x slower                                                      |
| xml_etree_generate   | 55.8 ms                                                     | 59.1 ms: 1.06x slower                                                      |
| json_dumps           | 5.70 ms                                                     | 6.26 ms: 1.10x slower                                                      |
| xml_etree_process    | 37.7 ms                                                     | 42.5 ms: 1.13x slower                                                      |
| unpickle_pure_python | 133 us                                                      | 152 us: 1.14x slower                                                       |
| pickle_pure_python   | 195 us                                                      | 226 us: 1.15x slower                                                       |
| tomli_loads          | 1.37 sec                                                    | 1.66 sec: 1.21x slower                                                     |
| Geometric mean       | (ref)                                                       | 1.09x slower                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 18.0 ms: 1.11x slower                                                      |
| python_startup         | 19.5 ms                                                     | 22.0 ms: 1.13x slower                                                      |
| Geometric mean         | (ref)                                                       | 1.12x slower                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| django_template | 22.9 ms                                                     | 26.2 ms: 1.14x slower                                                      |
| Geometric mean  | (ref)                                                       | 1.07x slower                                                               |

Benchmark hidden because not significant (1): mako

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 255 ms: 1.44x faster                                                       |
| async_tree_none_tg         | 285 ms                                                      | 202 ms: 1.41x faster                                                       |
| async_tree_io_tg           | 771 ms                                                      | 561 ms: 1.37x faster                                                       |
| async_tree_none            | 291 ms                                                      | 225 ms: 1.29x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 398 ms: 1.26x faster                                                       |
| async_tree_memoization     | 339 ms                                                      | 272 ms: 1.25x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 397 ms: 1.23x faster                                                       |
| asyncio_tcp_ssl            | 2.10 sec                                                    | 1.70 sec: 1.23x faster                                                     |
| deepcopy                   | 238 us                                                      | 193 us: 1.23x faster                                                       |
| async_tree_io              | 731 ms                                                      | 599 ms: 1.22x faster                                                       |
| comprehensions             | 14.1 us                                                     | 12.5 us: 1.13x faster                                                      |
| deepcopy_memo              | 23.7 us                                                     | 21.3 us: 1.11x faster                                                      |
| generators                 | 22.5 ms                                                     | 20.9 ms: 1.08x faster                                                      |
| deepcopy_reduce            | 2.09 us                                                     | 2.02 us: 1.03x faster                                                      |
| bench_thread_pool          | 857 us                                                      | 831 us: 1.03x faster                                                       |
| float                      | 56.8 ms                                                     | 55.3 ms: 1.03x faster                                                      |
| coroutines                 | 14.3 ms                                                     | 13.9 ms: 1.02x faster                                                      |
| regex_dna                  | 126 ms                                                      | 125 ms: 1.01x faster                                                       |
| pidigits                   | 152 ms                                                      | 151 ms: 1.00x faster                                                       |
| pathlib                    | 80.5 ms                                                     | 81.0 ms: 1.01x slower                                                      |
| regex_effbot               | 1.62 ms                                                     | 1.63 ms: 1.01x slower                                                      |
| bench_mp_pool              | 69.2 ms                                                     | 69.9 ms: 1.01x slower                                                      |
| xml_etree_iterparse        | 65.2 ms                                                     | 66.2 ms: 1.02x slower                                                      |
| json_loads                 | 13.9 us                                                     | 14.1 us: 1.02x slower                                                      |
| sympy_sum                  | 91.5 ms                                                     | 94.0 ms: 1.03x slower                                                      |
| xml_etree_parse            | 93.0 ms                                                     | 95.6 ms: 1.03x slower                                                      |
| gc_traversal               | 1.52 ms                                                     | 1.57 ms: 1.03x slower                                                      |
| logging_simple             | 6.28 us                                                     | 6.48 us: 1.03x slower                                                      |
| logging_format             | 6.72 us                                                     | 6.94 us: 1.03x slower                                                      |
| async_generators           | 239 ms                                                      | 249 ms: 1.04x slower                                                       |
| meteor_contest             | 74.6 ms                                                     | 78.0 ms: 1.05x slower                                                      |
| sympy_integrate            | 13.0 ms                                                     | 13.6 ms: 1.05x slower                                                      |
| sympy_str                  | 175 ms                                                      | 184 ms: 1.05x slower                                                       |
| chaos                      | 43.3 ms                                                     | 45.8 ms: 1.06x slower                                                      |
| raytrace                   | 192 ms                                                      | 204 ms: 1.06x slower                                                       |
| xml_etree_generate         | 55.8 ms                                                     | 59.1 ms: 1.06x slower                                                      |
| 2to3                       | 218 ms                                                      | 232 ms: 1.06x slower                                                       |
| nqueens                    | 62.8 ms                                                     | 66.8 ms: 1.06x slower                                                      |
| tornado_http               | 89.5 ms                                                     | 95.3 ms: 1.06x slower                                                      |
| crypto_pyaes               | 48.5 ms                                                     | 51.9 ms: 1.07x slower                                                      |
| docutils                   | 1.66 sec                                                    | 1.79 sec: 1.08x slower                                                     |
| regex_compile              | 87.5 ms                                                     | 94.2 ms: 1.08x slower                                                      |
| sqlglot_normalize          | 187 ms                                                      | 202 ms: 1.08x slower                                                       |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.78 ms: 1.09x slower                                                      |
| deltablue                  | 2.16 ms                                                     | 2.35 ms: 1.09x slower                                                      |
| sqlglot_optimize           | 34.5 ms                                                     | 37.6 ms: 1.09x slower                                                      |
| logging_silent             | 60.5 ns                                                     | 66.1 ns: 1.09x slower                                                      |
| go                         | 91.6 ms                                                     | 101 ms: 1.10x slower                                                       |
| json_dumps                 | 5.70 ms                                                     | 6.26 ms: 1.10x slower                                                      |
| sympy_expand               | 284 ms                                                      | 312 ms: 1.10x slower                                                       |
| scimark_lu                 | 58.9 ms                                                     | 65.1 ms: 1.11x slower                                                      |
| python_startup_no_site     | 16.2 ms                                                     | 18.0 ms: 1.11x slower                                                      |
| scimark_fft                | 184 ms                                                      | 205 ms: 1.11x slower                                                       |
| sqlglot_transpile          | 1.02 ms                                                     | 1.14 ms: 1.11x slower                                                      |
| pprint_safe_repr           | 513 ms                                                      | 573 ms: 1.12x slower                                                       |
| pprint_pformat             | 1.05 sec                                                    | 1.18 sec: 1.12x slower                                                     |
| xml_etree_process          | 37.7 ms                                                     | 42.5 ms: 1.13x slower                                                      |
| python_startup             | 19.5 ms                                                     | 22.0 ms: 1.13x slower                                                      |
| pyflate                    | 295 ms                                                      | 333 ms: 1.13x slower                                                       |
| sqlglot_parse              | 804 us                                                      | 915 us: 1.14x slower                                                       |
| unpickle_pure_python       | 133 us                                                      | 152 us: 1.14x slower                                                       |
| django_template            | 22.9 ms                                                     | 26.2 ms: 1.14x slower                                                      |
| mdp                        | 1.37 sec                                                    | 1.58 sec: 1.15x slower                                                     |
| hexiom                     | 4.10 ms                                                     | 4.72 ms: 1.15x slower                                                      |
| pickle_pure_python         | 195 us                                                      | 226 us: 1.15x slower                                                       |
| spectral_norm              | 66.9 ms                                                     | 77.3 ms: 1.16x slower                                                      |
| coverage                   | 40.8 ms                                                     | 47.6 ms: 1.17x slower                                                      |
| regex_v8                   | 14.2 ms                                                     | 16.7 ms: 1.17x slower                                                      |
| richards_super             | 32.1 ms                                                     | 38.5 ms: 1.20x slower                                                      |
| scimark_monte_carlo        | 43.7 ms                                                     | 52.5 ms: 1.20x slower                                                      |
| telco                      | 4.13 ms                                                     | 4.97 ms: 1.20x slower                                                      |
| richards                   | 28.4 ms                                                     | 34.2 ms: 1.21x slower                                                      |
| fannkuch                   | 247 ms                                                      | 298 ms: 1.21x slower                                                       |
| tomli_loads                | 1.37 sec                                                    | 1.66 sec: 1.21x slower                                                     |
| create_gc_cycles           | 752 us                                                      | 913 us: 1.21x slower                                                       |
| scimark_sor                | 78.8 ms                                                     | 96.2 ms: 1.22x slower                                                      |
| typing_runtime_protocols   | 95.1 us                                                     | 117 us: 1.23x slower                                                       |
| nbody                      | 71.9 ms                                                     | 91.2 ms: 1.27x slower                                                      |
| asyncio_tcp                | 487 ms                                                      | 645 ms: 1.32x slower                                                       |
| pycparser                  | 699 ms                                                      | 928 ms: 1.33x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.04x slower                                                               |

Benchmark hidden because not significant (3): dulwich_log, mako, json
Ignored benchmarks (13) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240802-3.14.0a0-498376d/bm-20240802-pythonperf1-amd64-python-498376d7a7d6f704f22a-3.14.0a0-498376d.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 99.99% likely to be slow
- 90% likely to have a slowdown of 1.05x
- 95% likely to have a slowdown of 1.04x
- 99% likely to have a slowdown of 1.03x

# Memory
- memory change: unknown