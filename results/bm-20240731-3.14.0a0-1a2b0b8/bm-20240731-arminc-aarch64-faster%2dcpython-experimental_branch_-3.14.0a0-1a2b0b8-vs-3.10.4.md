# Results vs. 3.10.4

- fork: faster-cpython
- ref: experimental_branch_
- machine: linux-aarch64
- commit hash: 1a2b0b8
- commit date: 2024-07-31
- overall geometric mean: 1.33x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.21x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 303 ms: 1.26x faster                                                              |
| docutils       | 3.53 sec                                                              | 3.20 sec: 1.10x faster                                                            |
| html5lib       | 86.5 ms                                                               | 68.6 ms: 1.26x faster                                                             |
| tornado_http   | 178 ms                                                                | 128 ms: 1.39x faster                                                              |
| Geometric mean | (ref)                                                                 | 1.25x faster                                                                      |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|-------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 436 ms: 2.18x faster                                                              |
| async_tree_io           | 2.28 sec                                                              | 1.12 sec: 2.05x faster                                                            |
| async_tree_memoization  | 1.13 sec                                                              | 557 ms: 2.03x faster                                                              |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 726 ms: 1.75x faster                                                              |
| Geometric mean          | (ref)                                                                 | 2.00x faster                                                                      |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| nbody          | 166 ms                                                                | 111 ms: 1.49x faster                                                              |
| float          | 135 ms                                                                | 93.7 ms: 1.44x faster                                                             |
| Geometric mean | (ref)                                                                 | 1.29x faster                                                                      |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| regex_compile  | 177 ms                                                                | 128 ms: 1.38x faster                                                              |
| regex_v8       | 32.2 ms                                                               | 30.5 ms: 1.05x faster                                                             |
| regex_dna      | 257 ms                                                                | 253 ms: 1.02x faster                                                              |
| regex_effbot   | 4.25 ms                                                               | 4.92 ms: 1.16x slower                                                             |
| Geometric mean | (ref)                                                                 | 1.06x faster                                                                      |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| pickle_pure_python   | 529 us                                                                | 362 us: 1.46x faster                                                              |
| unpickle_pure_python | 366 us                                                                | 250 us: 1.46x faster                                                              |
| tomli_loads          | 3.36 sec                                                              | 2.56 sec: 1.31x faster                                                            |
| xml_etree_process    | 99.5 ms                                                               | 79.9 ms: 1.25x faster                                                             |
| json_dumps           | 16.7 ms                                                               | 13.4 ms: 1.25x faster                                                             |
| xml_etree_parse      | 212 ms                                                                | 187 ms: 1.13x faster                                                              |
| xml_etree_generate   | 123 ms                                                                | 114 ms: 1.08x faster                                                              |
| xml_etree_iterparse  | 156 ms                                                                | 150 ms: 1.04x faster                                                              |
| json_loads           | 30.9 us                                                               | 32.6 us: 1.05x slower                                                             |
| Geometric mean       | (ref)                                                                 | 1.20x faster                                                                      |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 13.2 ms: 1.18x slower                                                             |
| python_startup_no_site | 6.89 ms                                                               | 8.91 ms: 1.29x slower                                                             |
| Geometric mean         | (ref)                                                                 | 1.24x slower                                                                      |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|-----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.5 ms: 1.40x faster                                                             |
| genshi_text     | 35.2 ms                                                               | 27.9 ms: 1.26x faster                                                             |
| django_template | 53.3 ms                                                               | 42.3 ms: 1.26x faster                                                             |
| genshi_xml      | 69.8 ms                                                               | 61.6 ms: 1.13x faster                                                             |
| Geometric mean  | (ref)                                                                 | 1.26x faster                                                                      |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|--------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 199 us: 3.32x faster                                                              |
| deltablue                | 8.94 ms                                                               | 3.89 ms: 2.30x faster                                                             |
| async_tree_none          | 950 ms                                                                | 436 ms: 2.18x faster                                                              |
| bench_mp_pool            | 14.5 ms                                                               | 7.04 ms: 2.06x faster                                                             |
| async_tree_io            | 2.28 sec                                                              | 1.12 sec: 2.05x faster                                                            |
| async_tree_memoization   | 1.13 sec                                                              | 557 ms: 2.03x faster                                                              |
| raytrace                 | 573 ms                                                                | 296 ms: 1.93x faster                                                              |
| generators               | 68.1 ms                                                               | 35.4 ms: 1.92x faster                                                             |
| richards_super           | 107 ms                                                                | 59.2 ms: 1.81x faster                                                             |
| chaos                    | 121 ms                                                                | 67.6 ms: 1.79x faster                                                             |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 726 ms: 1.75x faster                                                              |
| richards                 | 91.7 ms                                                               | 52.9 ms: 1.73x faster                                                             |
| logging_silent           | 222 ns                                                                | 130 ns: 1.71x faster                                                              |
| sqlglot_parse            | 2.40 ms                                                               | 1.42 ms: 1.69x faster                                                             |
| asyncio_tcp              | 944 ms                                                                | 569 ms: 1.66x faster                                                              |
| go                       | 264 ms                                                                | 160 ms: 1.65x faster                                                              |
| sqlglot_transpile        | 2.84 ms                                                               | 1.72 ms: 1.65x faster                                                             |
| scimark_lu               | 227 ms                                                                | 139 ms: 1.63x faster                                                              |
| crypto_pyaes             | 134 ms                                                                | 82.3 ms: 1.63x faster                                                             |
| deepcopy_memo            | 61.7 us                                                               | 38.3 us: 1.61x faster                                                             |
| comprehensions           | 33.1 us                                                               | 20.6 us: 1.61x faster                                                             |
| deepcopy                 | 511 us                                                                | 329 us: 1.55x faster                                                              |
| scimark_monte_carlo      | 128 ms                                                                | 83.4 ms: 1.53x faster                                                             |
| hexiom                   | 10.9 ms                                                               | 7.18 ms: 1.52x faster                                                             |
| scimark_sor              | 246 ms                                                                | 162 ms: 1.52x faster                                                              |
| nbody                    | 166 ms                                                                | 111 ms: 1.49x faster                                                              |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.20 sec: 1.49x faster                                                            |
| pickle_pure_python       | 529 us                                                                | 362 us: 1.46x faster                                                              |
| unpickle_pure_python     | 366 us                                                                | 250 us: 1.46x faster                                                              |
| float                    | 135 ms                                                                | 93.7 ms: 1.44x faster                                                             |
| mako                     | 18.9 ms                                                               | 13.5 ms: 1.40x faster                                                             |
| tornado_http             | 178 ms                                                                | 128 ms: 1.39x faster                                                              |
| logging_simple           | 9.78 us                                                               | 7.01 us: 1.39x faster                                                             |
| pycparser                | 1.69 sec                                                              | 1.22 sec: 1.39x faster                                                            |
| logging_format           | 10.6 us                                                               | 7.70 us: 1.38x faster                                                             |
| regex_compile            | 177 ms                                                                | 128 ms: 1.38x faster                                                              |
| pyflate                  | 795 ms                                                                | 580 ms: 1.37x faster                                                              |
| pylint                   | 485 ms                                                                | 361 ms: 1.35x faster                                                              |
| deepcopy_reduce          | 4.60 us                                                               | 3.45 us: 1.33x faster                                                             |
| spectral_norm            | 186 ms                                                                | 141 ms: 1.33x faster                                                              |
| tomli_loads              | 3.36 sec                                                              | 2.56 sec: 1.31x faster                                                            |
| coroutines               | 37.2 ms                                                               | 28.5 ms: 1.31x faster                                                             |
| thrift                   | 1.26 ms                                                               | 965 us: 1.31x faster                                                              |
| bench_thread_pool        | 1.59 ms                                                               | 1.24 ms: 1.28x faster                                                             |
| sympy_sum                | 184 ms                                                                | 145 ms: 1.27x faster                                                              |
| genshi_text              | 35.2 ms                                                               | 27.9 ms: 1.26x faster                                                             |
| html5lib                 | 86.5 ms                                                               | 68.6 ms: 1.26x faster                                                             |
| django_template          | 53.3 ms                                                               | 42.3 ms: 1.26x faster                                                             |
| 2to3                     | 381 ms                                                                | 303 ms: 1.26x faster                                                              |
| sympy_integrate          | 26.5 ms                                                               | 21.2 ms: 1.25x faster                                                             |
| xml_etree_process        | 99.5 ms                                                               | 79.9 ms: 1.25x faster                                                             |
| json_dumps               | 16.7 ms                                                               | 13.4 ms: 1.25x faster                                                             |
| dask                     | 450 ms                                                                | 363 ms: 1.24x faster                                                              |
| sympy_str                | 329 ms                                                                | 268 ms: 1.23x faster                                                              |
| pprint_pformat           | 2.36 sec                                                              | 1.93 sec: 1.22x faster                                                            |
| sqlglot_normalize        | 156 ms                                                                | 128 ms: 1.22x faster                                                              |
| pprint_safe_repr         | 1.15 sec                                                              | 942 ms: 1.22x faster                                                              |
| pathlib                  | 26.3 ms                                                               | 21.7 ms: 1.21x faster                                                             |
| sqlglot_optimize         | 75.4 ms                                                               | 62.6 ms: 1.20x faster                                                             |
| nqueens                  | 117 ms                                                                | 99.8 ms: 1.18x faster                                                             |
| fannkuch                 | 546 ms                                                                | 465 ms: 1.17x faster                                                              |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.50 ms: 1.17x faster                                                             |
| sympy_expand             | 543 ms                                                                | 463 ms: 1.17x faster                                                              |
| xml_etree_parse          | 212 ms                                                                | 187 ms: 1.13x faster                                                              |
| genshi_xml               | 69.8 ms                                                               | 61.6 ms: 1.13x faster                                                             |
| scimark_fft              | 500 ms                                                                | 448 ms: 1.12x faster                                                              |
| meteor_contest           | 126 ms                                                                | 114 ms: 1.11x faster                                                              |
| mdp                      | 3.70 sec                                                              | 3.34 sec: 1.11x faster                                                            |
| docutils                 | 3.53 sec                                                              | 3.20 sec: 1.10x faster                                                            |
| xml_etree_generate       | 123 ms                                                                | 114 ms: 1.08x faster                                                              |
| regex_v8                 | 32.2 ms                                                               | 30.5 ms: 1.05x faster                                                             |
| xml_etree_iterparse      | 156 ms                                                                | 150 ms: 1.04x faster                                                              |
| json                     | 5.88 ms                                                               | 5.70 ms: 1.03x faster                                                             |
| regex_dna                | 257 ms                                                                | 253 ms: 1.02x faster                                                              |
| create_gc_cycles         | 2.26 ms                                                               | 2.36 ms: 1.04x slower                                                             |
| json_loads               | 30.9 us                                                               | 32.6 us: 1.05x slower                                                             |
| async_generators         | 452 ms                                                                | 486 ms: 1.07x slower                                                              |
| telco                    | 8.49 ms                                                               | 9.77 ms: 1.15x slower                                                             |
| regex_effbot             | 4.25 ms                                                               | 4.92 ms: 1.16x slower                                                             |
| python_startup           | 11.2 ms                                                               | 13.2 ms: 1.18x slower                                                             |
| coverage                 | 83.6 ms                                                               | 99.3 ms: 1.19x slower                                                             |
| gc_traversal             | 4.15 ms                                                               | 4.95 ms: 1.19x slower                                                             |
| python_startup_no_site   | 6.89 ms                                                               | 8.91 ms: 1.29x slower                                                             |
| Geometric mean           | (ref)                                                                 | 1.33x faster                                                                      |

Benchmark hidden because not significant (2): pidigits, asyncio_websockets
Ignored benchmarks (14) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240731-3.14.0a0-1a2b0b8/bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.23x
- 95% likely to have a speedup of 1.22x
- 99% likely to have a speedup of 1.21x

# Memory
- memory change: 1.15x