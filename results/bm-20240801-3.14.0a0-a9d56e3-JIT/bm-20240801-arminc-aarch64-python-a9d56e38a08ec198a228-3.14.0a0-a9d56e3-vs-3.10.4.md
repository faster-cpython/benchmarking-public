# Results vs. 3.10.4

- fork: python
- ref: a9d56e38a08ec198a228
- machine: linux-aarch64
- commit hash: a9d56e3
- commit date: 2024-08-01
- overall geometric mean: 1.21x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x faster
- Memory change: 1.25x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 372 ms: 1.03x faster                                                    |
| docutils       | 3.53 sec                                                              | 3.74 sec: 1.06x slower                                                  |
| html5lib       | 86.5 ms                                                               | 71.1 ms: 1.22x faster                                                   |
| tornado_http   | 178 ms                                                                | 140 ms: 1.27x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.11x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 440 ms: 2.16x faster                                                    |
| async_tree_io           | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                  |
| async_tree_memoization  | 1.13 sec                                                              | 582 ms: 1.95x faster                                                    |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 756 ms: 1.68x faster                                                    |
| Geometric mean          | (ref)                                                                 | 1.93x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 135 ms                                                                | 91.4 ms: 1.47x faster                                                   |
| nbody          | 166 ms                                                                | 116 ms: 1.43x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.28x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 32.2 ms                                                               | 30.6 ms: 1.05x faster                                                   |
| regex_dna      | 257 ms                                                                | 260 ms: 1.01x slower                                                    |
| regex_compile  | 177 ms                                                                | 187 ms: 1.06x slower                                                    |
| regex_effbot   | 4.25 ms                                                               | 4.95 ms: 1.17x slower                                                   |
| Geometric mean | (ref)                                                                 | 1.04x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 277 us: 1.32x faster                                                    |
| pickle_pure_python   | 529 us                                                                | 401 us: 1.32x faster                                                    |
| json_dumps           | 16.7 ms                                                               | 13.2 ms: 1.27x faster                                                   |
| tomli_loads          | 3.36 sec                                                              | 2.67 sec: 1.26x faster                                                  |
| xml_etree_process    | 99.5 ms                                                               | 80.9 ms: 1.23x faster                                                   |
| xml_etree_parse      | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| xml_etree_generate   | 123 ms                                                                | 113 ms: 1.09x faster                                                    |
| xml_etree_iterparse  | 156 ms                                                                | 148 ms: 1.05x faster                                                    |
| json_loads           | 30.9 us                                                               | 32.7 us: 1.06x slower                                                   |
| Geometric mean       | (ref)                                                                 | 1.17x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 13.1 ms: 1.17x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 8.91 ms: 1.29x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.23x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 12.9 ms: 1.46x faster                                                   |
| genshi_text     | 35.2 ms                                                               | 32.9 ms: 1.07x faster                                                   |
| django_template | 53.3 ms                                                               | 51.4 ms: 1.04x faster                                                   |
| genshi_xml      | 69.8 ms                                                               | 73.3 ms: 1.05x slower                                                   |
| Geometric mean  | (ref)                                                                 | 1.11x faster                                                            |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 212 us: 3.13x faster                                                    |
| async_tree_none          | 950 ms                                                                | 440 ms: 2.16x faster                                                    |
| deltablue                | 8.94 ms                                                               | 4.42 ms: 2.02x faster                                                   |
| async_tree_io            | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                  |
| async_tree_memoization   | 1.13 sec                                                              | 582 ms: 1.95x faster                                                    |
| bench_mp_pool            | 14.5 ms                                                               | 8.28 ms: 1.76x faster                                                   |
| raytrace                 | 573 ms                                                                | 329 ms: 1.74x faster                                                    |
| richards_super           | 107 ms                                                                | 62.0 ms: 1.73x faster                                                   |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 756 ms: 1.68x faster                                                    |
| richards                 | 91.7 ms                                                               | 55.4 ms: 1.66x faster                                                   |
| logging_silent           | 222 ns                                                                | 136 ns: 1.63x faster                                                    |
| deepcopy_memo            | 61.7 us                                                               | 39.0 us: 1.58x faster                                                   |
| asyncio_tcp              | 944 ms                                                                | 624 ms: 1.51x faster                                                    |
| crypto_pyaes             | 134 ms                                                                | 90.1 ms: 1.49x faster                                                   |
| float                    | 135 ms                                                                | 91.4 ms: 1.47x faster                                                   |
| mako                     | 18.9 ms                                                               | 12.9 ms: 1.46x faster                                                   |
| scimark_monte_carlo      | 128 ms                                                                | 87.6 ms: 1.46x faster                                                   |
| go                       | 264 ms                                                                | 183 ms: 1.45x faster                                                    |
| sqlglot_parse            | 2.40 ms                                                               | 1.66 ms: 1.44x faster                                                   |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.27 sec: 1.44x faster                                                  |
| nbody                    | 166 ms                                                                | 116 ms: 1.43x faster                                                    |
| scimark_sor              | 246 ms                                                                | 175 ms: 1.40x faster                                                    |
| chaos                    | 121 ms                                                                | 87.2 ms: 1.39x faster                                                   |
| comprehensions           | 33.1 us                                                               | 24.3 us: 1.36x faster                                                   |
| sqlglot_transpile        | 2.84 ms                                                               | 2.11 ms: 1.35x faster                                                   |
| pyflate                  | 795 ms                                                                | 597 ms: 1.33x faster                                                    |
| unpickle_pure_python     | 366 us                                                                | 277 us: 1.32x faster                                                    |
| coroutines               | 37.2 ms                                                               | 28.2 ms: 1.32x faster                                                   |
| pickle_pure_python       | 529 us                                                                | 401 us: 1.32x faster                                                    |
| deepcopy                 | 511 us                                                                | 388 us: 1.31x faster                                                    |
| logging_simple           | 9.78 us                                                               | 7.47 us: 1.31x faster                                                   |
| logging_format           | 10.6 us                                                               | 8.16 us: 1.30x faster                                                   |
| thrift                   | 1.26 ms                                                               | 974 us: 1.29x faster                                                    |
| spectral_norm            | 186 ms                                                                | 145 ms: 1.28x faster                                                    |
| tornado_http             | 178 ms                                                                | 140 ms: 1.27x faster                                                    |
| json_dumps               | 16.7 ms                                                               | 13.2 ms: 1.27x faster                                                   |
| scimark_lu               | 227 ms                                                                | 180 ms: 1.26x faster                                                    |
| tomli_loads              | 3.36 sec                                                              | 2.67 sec: 1.26x faster                                                  |
| xml_etree_process        | 99.5 ms                                                               | 80.9 ms: 1.23x faster                                                   |
| html5lib                 | 86.5 ms                                                               | 71.1 ms: 1.22x faster                                                   |
| bench_thread_pool        | 1.59 ms                                                               | 1.31 ms: 1.21x faster                                                   |
| generators               | 68.1 ms                                                               | 57.3 ms: 1.19x faster                                                   |
| pathlib                  | 26.3 ms                                                               | 22.4 ms: 1.18x faster                                                   |
| pycparser                | 1.69 sec                                                              | 1.44 sec: 1.17x faster                                                  |
| xml_etree_parse          | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| dask                     | 450 ms                                                                | 401 ms: 1.12x faster                                                    |
| hexiom                   | 10.9 ms                                                               | 9.75 ms: 1.12x faster                                                   |
| fannkuch                 | 546 ms                                                                | 488 ms: 1.12x faster                                                    |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.85 ms: 1.11x faster                                                   |
| pylint                   | 485 ms                                                                | 438 ms: 1.11x faster                                                    |
| scimark_fft              | 500 ms                                                                | 460 ms: 1.09x faster                                                    |
| xml_etree_generate       | 123 ms                                                                | 113 ms: 1.09x faster                                                    |
| deepcopy_reduce          | 4.60 us                                                               | 4.25 us: 1.08x faster                                                   |
| genshi_text              | 35.2 ms                                                               | 32.9 ms: 1.07x faster                                                   |
| mdp                      | 3.70 sec                                                              | 3.49 sec: 1.06x faster                                                  |
| regex_v8                 | 32.2 ms                                                               | 30.6 ms: 1.05x faster                                                   |
| sqlglot_normalize        | 156 ms                                                                | 148 ms: 1.05x faster                                                    |
| xml_etree_iterparse      | 156 ms                                                                | 148 ms: 1.05x faster                                                    |
| meteor_contest           | 126 ms                                                                | 121 ms: 1.04x faster                                                    |
| django_template          | 53.3 ms                                                               | 51.4 ms: 1.04x faster                                                   |
| sqlglot_optimize         | 75.4 ms                                                               | 72.8 ms: 1.04x faster                                                   |
| 2to3                     | 381 ms                                                                | 372 ms: 1.03x faster                                                    |
| pprint_safe_repr         | 1.15 sec                                                              | 1.14 sec: 1.01x faster                                                  |
| pprint_pformat           | 2.36 sec                                                              | 2.34 sec: 1.01x faster                                                  |
| asyncio_websockets       | 657 ms                                                                | 662 ms: 1.01x slower                                                    |
| regex_dna                | 257 ms                                                                | 260 ms: 1.01x slower                                                    |
| create_gc_cycles         | 2.26 ms                                                               | 2.33 ms: 1.03x slower                                                   |
| sympy_integrate          | 26.5 ms                                                               | 27.7 ms: 1.04x slower                                                   |
| nqueens                  | 117 ms                                                                | 123 ms: 1.04x slower                                                    |
| genshi_xml               | 69.8 ms                                                               | 73.3 ms: 1.05x slower                                                   |
| json_loads               | 30.9 us                                                               | 32.7 us: 1.06x slower                                                   |
| regex_compile            | 177 ms                                                                | 187 ms: 1.06x slower                                                    |
| docutils                 | 3.53 sec                                                              | 3.74 sec: 1.06x slower                                                  |
| sympy_str                | 329 ms                                                                | 348 ms: 1.06x slower                                                    |
| sympy_sum                | 184 ms                                                                | 200 ms: 1.09x slower                                                    |
| sympy_expand             | 543 ms                                                                | 593 ms: 1.09x slower                                                    |
| async_generators         | 452 ms                                                                | 504 ms: 1.11x slower                                                    |
| regex_effbot             | 4.25 ms                                                               | 4.95 ms: 1.17x slower                                                   |
| coverage                 | 83.6 ms                                                               | 97.8 ms: 1.17x slower                                                   |
| python_startup           | 11.2 ms                                                               | 13.1 ms: 1.17x slower                                                   |
| gc_traversal             | 4.15 ms                                                               | 5.01 ms: 1.20x slower                                                   |
| telco                    | 8.49 ms                                                               | 10.3 ms: 1.21x slower                                                   |
| python_startup_no_site   | 6.89 ms                                                               | 8.91 ms: 1.29x slower                                                   |
| Geometric mean           | (ref)                                                                 | 1.21x faster                                                            |

Benchmark hidden because not significant (2): pidigits, json
Ignored benchmarks (14) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240801-3.14.0a0-a9d56e3-JIT/bm-20240801-arminc-aarch64-python-a9d56e38a08ec198a228-3.14.0a0-a9d56e3.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.05x

# Memory
- memory change: 1.25x