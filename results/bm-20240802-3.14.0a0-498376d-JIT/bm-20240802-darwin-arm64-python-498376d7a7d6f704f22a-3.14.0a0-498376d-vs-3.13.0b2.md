# Results vs. 3.13.0b2

- fork: python
- ref: 498376d7a7d6f704f22a
- machine: darwin-arm64
- commit hash: 498376d
- commit date: 2024-08-02
- overall geometric mean: 1.01x slower
- HPT reliability: 98.08%
- HPT 99th percentile: 1.00x slower
- Memory change: 0.59x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 161 ms                                                     | 174 ms: 1.08x slower                                                  |
| docutils       | 1.44 sec                                                   | 1.54 sec: 1.07x slower                                                |
| html5lib       | 31.2 ms                                                    | 30.9 ms: 1.01x faster                                                 |
| Geometric mean | (ref)                                                      | 1.04x slower                                                          |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_eager_io              | 766 ms                                                     | 639 ms: 1.20x faster                                                  |
| async_tree_eager_io_tg           | 768 ms                                                     | 676 ms: 1.14x faster                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 506 ms: 1.12x faster                                                  |
| async_tree_none_tg               | 198 ms                                                     | 182 ms: 1.09x faster                                                  |
| async_tree_memoization           | 260 ms                                                     | 239 ms: 1.08x faster                                                  |
| async_tree_none                  | 209 ms                                                     | 194 ms: 1.08x faster                                                  |
| async_tree_memoization_tg        | 240 ms                                                     | 228 ms: 1.05x faster                                                  |
| async_tree_cpu_io_mixed          | 467 ms                                                     | 452 ms: 1.03x faster                                                  |
| async_tree_io                    | 563 ms                                                     | 546 ms: 1.03x faster                                                  |
| async_tree_eager_tg              | 41.4 ms                                                    | 42.0 ms: 1.01x slower                                                 |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 336 ms: 1.02x slower                                                  |
| async_tree_eager                 | 60.3 ms                                                    | 62.2 ms: 1.03x slower                                                 |
| Geometric mean                   | (ref)                                                      | 1.05x faster                                                          |

Benchmark hidden because not significant (4): async_tree_cpu_io_mixed_tg, async_tree_eager_memoization_tg, async_tree_eager_memoization, async_tree_eager_cpu_io_mixed

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 48.6 ms                                                    | 46.8 ms: 1.04x faster                                                 |
| nbody          | 59.6 ms                                                    | 62.8 ms: 1.05x slower                                                 |
| Geometric mean | (ref)                                                      | 1.00x slower                                                          |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 2.58 ms                                                    | 2.57 ms: 1.01x faster                                                 |
| regex_v8       | 17.3 ms                                                    | 17.2 ms: 1.01x faster                                                 |
| regex_dna      | 149 ms                                                     | 149 ms: 1.00x slower                                                  |
| regex_compile  | 68.5 ms                                                    | 71.5 ms: 1.04x slower                                                 |
| Geometric mean | (ref)                                                      | 1.01x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 1.47 sec                                                   | 1.24 sec: 1.18x faster                                                |
| unpickle_pure_python | 141 us                                                     | 132 us: 1.07x faster                                                  |
| xml_etree_process    | 37.1 ms                                                    | 35.8 ms: 1.04x faster                                                 |
| pickle_pure_python   | 179 us                                                     | 173 us: 1.03x faster                                                  |
| xml_etree_generate   | 52.7 ms                                                    | 51.9 ms: 1.01x faster                                                 |
| xml_etree_iterparse  | 71.5 ms                                                    | 72.5 ms: 1.01x slower                                                 |
| json_loads           | 16.8 us                                                    | 17.2 us: 1.02x slower                                                 |
| json_dumps           | 6.23 ms                                                    | 6.37 ms: 1.02x slower                                                 |
| xml_etree_parse      | 106 ms                                                     | 110 ms: 1.04x slower                                                  |
| Geometric mean       | (ref)                                                      | 1.02x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 15.2 ms                                                    | 16.7 ms: 1.10x slower                                                 |
| python_startup_no_site | 12.3 ms                                                    | 13.7 ms: 1.11x slower                                                 |
| Geometric mean         | (ref)                                                      | 1.11x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|-----------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 6.99 ms                                                    | 6.45 ms: 1.08x faster                                                 |
| genshi_text     | 13.9 ms                                                    | 14.7 ms: 1.06x slower                                                 |
| django_template | 19.4 ms                                                    | 21.6 ms: 1.11x slower                                                 |
| genshi_xml      | 29.9 ms                                                    | 35.1 ms: 1.17x slower                                                 |
| Geometric mean  | (ref)                                                      | 1.06x slower                                                          |

All benchmarks:
===============

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-darwin-arm64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------------|:----------------------------------------------------------:|:---------------------------------------------------------------------:|
| deepcopy_memo                    | 22.6 us                                                    | 16.8 us: 1.35x faster                                                 |
| deepcopy                         | 204 us                                                     | 153 us: 1.33x faster                                                  |
| async_tree_eager_io              | 766 ms                                                     | 639 ms: 1.20x faster                                                  |
| tomli_loads                      | 1.47 sec                                                   | 1.24 sec: 1.18x faster                                                |
| deepcopy_reduce                  | 1.82 us                                                    | 1.57 us: 1.16x faster                                                 |
| async_tree_eager_io_tg           | 768 ms                                                     | 676 ms: 1.14x faster                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 506 ms: 1.12x faster                                                  |
| async_tree_none_tg               | 198 ms                                                     | 182 ms: 1.09x faster                                                  |
| async_tree_memoization           | 260 ms                                                     | 239 ms: 1.08x faster                                                  |
| mako                             | 6.99 ms                                                    | 6.45 ms: 1.08x faster                                                 |
| async_tree_none                  | 209 ms                                                     | 194 ms: 1.08x faster                                                  |
| scimark_sor                      | 94.9 ms                                                    | 88.2 ms: 1.08x faster                                                 |
| unpickle_pure_python             | 141 us                                                     | 132 us: 1.07x faster                                                  |
| async_tree_memoization_tg        | 240 ms                                                     | 228 ms: 1.05x faster                                                  |
| mdp                              | 1.53 sec                                                   | 1.46 sec: 1.05x faster                                                |
| float                            | 48.6 ms                                                    | 46.8 ms: 1.04x faster                                                 |
| xml_etree_process                | 37.1 ms                                                    | 35.8 ms: 1.04x faster                                                 |
| async_tree_cpu_io_mixed          | 467 ms                                                     | 452 ms: 1.03x faster                                                  |
| async_tree_io                    | 563 ms                                                     | 546 ms: 1.03x faster                                                  |
| pickle_pure_python               | 179 us                                                     | 173 us: 1.03x faster                                                  |
| scimark_lu                       | 66.9 ms                                                    | 65.0 ms: 1.03x faster                                                 |
| richards                         | 31.8 ms                                                    | 31.0 ms: 1.03x faster                                                 |
| pyflate                          | 321 ms                                                     | 313 ms: 1.03x faster                                                  |
| logging_simple                   | 3.04 us                                                    | 2.99 us: 1.02x faster                                                 |
| xml_etree_generate               | 52.7 ms                                                    | 51.9 ms: 1.01x faster                                                 |
| bpe_tokeniser                    | 3.05 sec                                                   | 3.02 sec: 1.01x faster                                                |
| telco                            | 4.60 ms                                                    | 4.56 ms: 1.01x faster                                                 |
| html5lib                         | 31.2 ms                                                    | 30.9 ms: 1.01x faster                                                 |
| regex_effbot                     | 2.58 ms                                                    | 2.57 ms: 1.01x faster                                                 |
| regex_v8                         | 17.3 ms                                                    | 17.2 ms: 1.01x faster                                                 |
| richards_super                   | 35.2 ms                                                    | 35.1 ms: 1.00x faster                                                 |
| asyncio_websockets               | 409 ms                                                     | 408 ms: 1.00x faster                                                  |
| regex_dna                        | 149 ms                                                     | 149 ms: 1.00x slower                                                  |
| logging_format                   | 3.31 us                                                    | 3.32 us: 1.00x slower                                                 |
| fannkuch                         | 248 ms                                                     | 250 ms: 1.01x slower                                                  |
| gc_traversal                     | 2.45 ms                                                    | 2.47 ms: 1.01x slower                                                 |
| create_gc_cycles                 | 897 us                                                     | 907 us: 1.01x slower                                                  |
| pathlib                          | 23.3 ms                                                    | 23.6 ms: 1.01x slower                                                 |
| xml_etree_iterparse              | 71.5 ms                                                    | 72.5 ms: 1.01x slower                                                 |
| async_tree_eager_tg              | 41.4 ms                                                    | 42.0 ms: 1.01x slower                                                 |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 336 ms: 1.02x slower                                                  |
| dulwich_log                      | 27.6 ms                                                    | 28.0 ms: 1.02x slower                                                 |
| meteor_contest                   | 70.3 ms                                                    | 71.7 ms: 1.02x slower                                                 |
| coroutines                       | 15.8 ms                                                    | 16.2 ms: 1.02x slower                                                 |
| logging_silent                   | 60.1 ns                                                    | 61.3 ns: 1.02x slower                                                 |
| json_loads                       | 16.8 us                                                    | 17.2 us: 1.02x slower                                                 |
| json_dumps                       | 6.23 ms                                                    | 6.37 ms: 1.02x slower                                                 |
| async_tree_eager                 | 60.3 ms                                                    | 62.2 ms: 1.03x slower                                                 |
| spectral_norm                    | 66.4 ms                                                    | 68.5 ms: 1.03x slower                                                 |
| scimark_monte_carlo              | 42.5 ms                                                    | 43.8 ms: 1.03x slower                                                 |
| xml_etree_parse                  | 106 ms                                                     | 110 ms: 1.04x slower                                                  |
| crypto_pyaes                     | 49.5 ms                                                    | 51.4 ms: 1.04x slower                                                 |
| regex_compile                    | 68.5 ms                                                    | 71.5 ms: 1.04x slower                                                 |
| scimark_fft                      | 181 ms                                                     | 189 ms: 1.05x slower                                                  |
| async_generators                 | 281 ms                                                     | 295 ms: 1.05x slower                                                  |
| dask                             | 221 ms                                                     | 233 ms: 1.05x slower                                                  |
| pprint_safe_repr                 | 465 ms                                                     | 489 ms: 1.05x slower                                                  |
| nbody                            | 59.6 ms                                                    | 62.8 ms: 1.05x slower                                                 |
| genshi_text                      | 13.9 ms                                                    | 14.7 ms: 1.06x slower                                                 |
| bench_thread_pool                | 447 us                                                     | 472 us: 1.06x slower                                                  |
| pprint_pformat                   | 947 ms                                                     | 1000 ms: 1.06x slower                                                 |
| pycparser                        | 632 ms                                                     | 671 ms: 1.06x slower                                                  |
| sqlglot_normalize                | 166 ms                                                     | 176 ms: 1.06x slower                                                  |
| sqlglot_optimize                 | 30.9 ms                                                    | 33.0 ms: 1.07x slower                                                 |
| docutils                         | 1.44 sec                                                   | 1.54 sec: 1.07x slower                                                |
| generators                       | 22.9 ms                                                    | 24.6 ms: 1.07x slower                                                 |
| sympy_sum                        | 69.2 ms                                                    | 74.3 ms: 1.07x slower                                                 |
| 2to3                             | 161 ms                                                     | 174 ms: 1.08x slower                                                  |
| deltablue                        | 2.14 ms                                                    | 2.31 ms: 1.08x slower                                                 |
| sympy_str                        | 131 ms                                                     | 142 ms: 1.08x slower                                                  |
| scimark_sparse_mat_mult          | 2.77 ms                                                    | 3.01 ms: 1.08x slower                                                 |
| hexiom                           | 4.06 ms                                                    | 4.42 ms: 1.09x slower                                                 |
| sympy_expand                     | 226 ms                                                     | 246 ms: 1.09x slower                                                  |
| bench_mp_pool                    | 47.2 ms                                                    | 51.5 ms: 1.09x slower                                                 |
| raytrace                         | 147 ms                                                     | 161 ms: 1.10x slower                                                  |
| sympy_integrate                  | 10.3 ms                                                    | 11.3 ms: 1.10x slower                                                 |
| python_startup                   | 15.2 ms                                                    | 16.7 ms: 1.10x slower                                                 |
| typing_runtime_protocols         | 87.6 us                                                    | 96.7 us: 1.10x slower                                                 |
| nqueens                          | 52.2 ms                                                    | 57.7 ms: 1.11x slower                                                 |
| python_startup_no_site           | 12.3 ms                                                    | 13.7 ms: 1.11x slower                                                 |
| django_template                  | 19.4 ms                                                    | 21.6 ms: 1.11x slower                                                 |
| chaos                            | 34.0 ms                                                    | 38.8 ms: 1.14x slower                                                 |
| sqlglot_transpile                | 891 us                                                     | 1.02 ms: 1.14x slower                                                 |
| pylint                           | 170 ms                                                     | 194 ms: 1.14x slower                                                  |
| sqlglot_parse                    | 732 us                                                     | 850 us: 1.16x slower                                                  |
| genshi_xml                       | 29.9 ms                                                    | 35.1 ms: 1.17x slower                                                 |
| comprehensions                   | 10.2 us                                                    | 12.2 us: 1.20x slower                                                 |
| Geometric mean                   | (ref)                                                      | 1.01x slower                                                          |

Benchmark hidden because not significant (12): async_tree_cpu_io_mixed_tg, asyncio_tcp, coverage, pidigits, go, thrift, json, async_tree_eager_memoization_tg, async_tree_eager_memoization, async_tree_eager_cpu_io_mixed, asyncio_tcp_ssl, tornado_http
Ignored benchmarks (11) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 98.08% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 0.59x