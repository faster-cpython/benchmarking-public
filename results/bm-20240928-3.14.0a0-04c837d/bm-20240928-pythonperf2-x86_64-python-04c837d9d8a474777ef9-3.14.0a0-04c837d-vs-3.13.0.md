# Results vs. 3.13.0

- fork: python
- ref: 04c837d9d8a474777ef9
- machine: linux-x86_64
- commit hash: 04c837d
- commit date: 2024-09-28
- overall geometric mean: 1.04x slower
- HPT reliability: 99.99%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.01x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                       | 283 ms: 1.03x faster                                                        |
| docutils       | 2.81 sec                                                     | 2.89 sec: 1.03x slower                                                      |
| html5lib       | 71.9 ms                                                      | 70.2 ms: 1.02x faster                                                       |
| tornado_http   | 120 ms                                                       | 116 ms: 1.03x faster                                                        |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 461 ms                                                       | 391 ms: 1.18x faster                                                        |
| async_tree_none            | 372 ms                                                       | 321 ms: 1.16x faster                                                        |
| async_tree_memoization     | 452 ms                                                       | 400 ms: 1.13x faster                                                        |
| async_tree_none_tg         | 340 ms                                                       | 309 ms: 1.10x faster                                                        |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 572 ms: 1.05x faster                                                        |
| async_tree_io              | 847 ms                                                       | 812 ms: 1.04x faster                                                        |
| async_tree_io_tg           | 819 ms                                                       | 791 ms: 1.03x faster                                                        |
| asyncio_tcp                | 380 ms                                                       | 373 ms: 1.02x faster                                                        |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 565 ms: 1.02x faster                                                        |
| asyncio_tcp_ssl            | 1.58 sec                                                     | 1.59 sec: 1.00x slower                                                      |
| asyncio_websockets         | 382 ms                                                       | 392 ms: 1.03x slower                                                        |
| coroutines                 | 21.6 ms                                                      | 22.2 ms: 1.03x slower                                                       |
| Geometric mean             | (ref)                                                        | 1.05x faster                                                                |

Benchmark hidden because not significant (1): async_generators

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 88.0 ms                                                      | 85.8 ms: 1.03x faster                                                       |
| float          | 81.9 ms                                                      | 80.5 ms: 1.02x faster                                                       |
| pidigits       | 253 ms                                                       | 253 ms: 1.00x slower                                                        |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_v8       | 26.2 ms                                                      | 25.1 ms: 1.04x faster                                                       |
| regex_compile  | 144 ms                                                       | 140 ms: 1.03x faster                                                        |
| regex_dna      | 244 ms                                                       | 245 ms: 1.01x slower                                                        |
| regex_effbot   | 3.37 ms                                                      | 3.44 ms: 1.02x slower                                                       |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pickle_dict          | 32.1 us                                                      | 30.1 us: 1.06x faster                                                       |
| pickle_list          | 4.59 us                                                      | 4.34 us: 1.06x faster                                                       |
| pickle               | 10.5 us                                                      | 10.2 us: 1.04x faster                                                       |
| json_dumps           | 11.0 ms                                                      | 10.9 ms: 1.01x faster                                                       |
| pickle_pure_python   | 318 us                                                       | 316 us: 1.01x faster                                                        |
| xml_etree_process    | 60.9 ms                                                      | 60.5 ms: 1.01x faster                                                       |
| unpickle_pure_python | 214 us                                                       | 214 us: 1.00x faster                                                        |
| xml_etree_generate   | 85.3 ms                                                      | 85.8 ms: 1.01x slower                                                       |
| unpickle_list        | 4.62 us                                                      | 4.66 us: 1.01x slower                                                       |
| xml_etree_parse      | 145 ms                                                       | 149 ms: 1.03x slower                                                        |
| xml_etree_iterparse  | 100 ms                                                       | 103 ms: 1.03x slower                                                        |
| json_loads           | 24.0 us                                                      | 25.2 us: 1.05x slower                                                       |
| tomli_loads          | 2.41 sec                                                     | 2.60 sec: 1.08x slower                                                      |
| Geometric mean       | (ref)                                                        | 1.00x slower                                                                |

Benchmark hidden because not significant (1): unpickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup_no_site | 8.94 ms                                                      | 8.97 ms: 1.00x slower                                                       |
| python_startup         | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                       |
| Geometric mean         | (ref)                                                        | 1.00x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| genshi_xml      | 57.3 ms                                                      | 53.5 ms: 1.07x faster                                                       |
| genshi_text     | 26.6 ms                                                      | 25.3 ms: 1.05x faster                                                       |
| mako            | 10.4 ms                                                      | 10.3 ms: 1.01x faster                                                       |
| django_template | 38.9 ms                                                      | 39.4 ms: 1.02x slower                                                       |
| Geometric mean  | (ref)                                                        | 1.03x faster                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240928-pythonperf2-x86_64-python-04c837d9d8a474777ef9-3.14.0a0-04c837d |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy                   | 397 us                                                       | 284 us: 1.40x faster                                                        |
| deepcopy_memo              | 39.5 us                                                      | 28.9 us: 1.36x faster                                                       |
| deepcopy_reduce            | 3.54 us                                                      | 2.90 us: 1.22x faster                                                       |
| go                         | 160 ms                                                       | 136 ms: 1.18x faster                                                        |
| async_tree_memoization_tg  | 461 ms                                                       | 391 ms: 1.18x faster                                                        |
| async_tree_none            | 372 ms                                                       | 321 ms: 1.16x faster                                                        |
| generators                 | 33.5 ms                                                      | 29.0 ms: 1.16x faster                                                       |
| async_tree_memoization     | 452 ms                                                       | 400 ms: 1.13x faster                                                        |
| pathlib                    | 17.4 ms                                                      | 15.6 ms: 1.11x faster                                                       |
| async_tree_none_tg         | 340 ms                                                       | 309 ms: 1.10x faster                                                        |
| unpack_sequence            | 56.8 ns                                                      | 51.8 ns: 1.10x faster                                                       |
| scimark_fft                | 314 ms                                                       | 289 ms: 1.09x faster                                                        |
| raytrace                   | 289 ms                                                       | 270 ms: 1.07x faster                                                        |
| genshi_xml                 | 57.3 ms                                                      | 53.5 ms: 1.07x faster                                                       |
| scimark_sor                | 126 ms                                                       | 117 ms: 1.07x faster                                                        |
| pickle_dict                | 32.1 us                                                      | 30.1 us: 1.06x faster                                                       |
| pickle_list                | 4.59 us                                                      | 4.34 us: 1.06x faster                                                       |
| genshi_text                | 26.6 ms                                                      | 25.3 ms: 1.05x faster                                                       |
| richards_super             | 59.8 ms                                                      | 56.8 ms: 1.05x faster                                                       |
| scimark_sparse_mat_mult    | 4.29 ms                                                      | 4.08 ms: 1.05x faster                                                       |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 572 ms: 1.05x faster                                                        |
| telco                      | 8.58 ms                                                      | 8.21 ms: 1.05x faster                                                       |
| regex_v8                   | 26.2 ms                                                      | 25.1 ms: 1.04x faster                                                       |
| bpe_tokeniser              | 5.10 sec                                                     | 4.89 sec: 1.04x faster                                                      |
| async_tree_io              | 847 ms                                                       | 812 ms: 1.04x faster                                                        |
| richards                   | 52.7 ms                                                      | 50.8 ms: 1.04x faster                                                       |
| pickle                     | 10.5 us                                                      | 10.2 us: 1.04x faster                                                       |
| async_tree_io_tg           | 819 ms                                                       | 791 ms: 1.03x faster                                                        |
| tornado_http               | 120 ms                                                       | 116 ms: 1.03x faster                                                        |
| regex_compile              | 144 ms                                                       | 140 ms: 1.03x faster                                                        |
| 2to3                       | 291 ms                                                       | 283 ms: 1.03x faster                                                        |
| sympy_integrate            | 23.3 ms                                                      | 22.7 ms: 1.03x faster                                                       |
| scimark_lu                 | 97.8 ms                                                      | 95.4 ms: 1.03x faster                                                       |
| nbody                      | 88.0 ms                                                      | 85.8 ms: 1.03x faster                                                       |
| sympy_sum                  | 154 ms                                                       | 150 ms: 1.03x faster                                                        |
| html5lib                   | 71.9 ms                                                      | 70.2 ms: 1.02x faster                                                       |
| meteor_contest             | 128 ms                                                       | 125 ms: 1.02x faster                                                        |
| sympy_str                  | 296 ms                                                       | 289 ms: 1.02x faster                                                        |
| sqlite_synth               | 2.79 us                                                      | 2.73 us: 1.02x faster                                                       |
| sympy_expand               | 505 ms                                                       | 495 ms: 1.02x faster                                                        |
| comprehensions             | 17.3 us                                                      | 16.9 us: 1.02x faster                                                       |
| asyncio_tcp                | 380 ms                                                       | 373 ms: 1.02x faster                                                        |
| float                      | 81.9 ms                                                      | 80.5 ms: 1.02x faster                                                       |
| sqlglot_normalize          | 118 ms                                                       | 116 ms: 1.02x faster                                                        |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 565 ms: 1.02x faster                                                        |
| sqlglot_optimize           | 59.7 ms                                                      | 58.7 ms: 1.02x faster                                                       |
| mdp                        | 2.52 sec                                                     | 2.48 sec: 1.02x faster                                                      |
| logging_format             | 7.07 us                                                      | 6.97 us: 1.01x faster                                                       |
| mako                       | 10.4 ms                                                      | 10.3 ms: 1.01x faster                                                       |
| hexiom                     | 6.33 ms                                                      | 6.25 ms: 1.01x faster                                                       |
| pycparser                  | 1.26 sec                                                     | 1.24 sec: 1.01x faster                                                      |
| logging_simple             | 6.40 us                                                      | 6.34 us: 1.01x faster                                                       |
| json_dumps                 | 11.0 ms                                                      | 10.9 ms: 1.01x faster                                                       |
| pickle_pure_python         | 318 us                                                       | 316 us: 1.01x faster                                                        |
| scimark_monte_carlo        | 64.9 ms                                                      | 64.5 ms: 1.01x faster                                                       |
| xml_etree_process          | 60.9 ms                                                      | 60.5 ms: 1.01x faster                                                       |
| deltablue                  | 3.41 ms                                                      | 3.39 ms: 1.01x faster                                                       |
| unpickle_pure_python       | 214 us                                                       | 214 us: 1.00x faster                                                        |
| pidigits                   | 253 ms                                                       | 253 ms: 1.00x slower                                                        |
| python_startup_no_site     | 8.94 ms                                                      | 8.97 ms: 1.00x slower                                                       |
| python_startup             | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                       |
| asyncio_tcp_ssl            | 1.58 sec                                                     | 1.59 sec: 1.00x slower                                                      |
| coverage                   | 81.1 ms                                                      | 81.5 ms: 1.00x slower                                                       |
| xml_etree_generate         | 85.3 ms                                                      | 85.8 ms: 1.01x slower                                                       |
| regex_dna                  | 244 ms                                                       | 245 ms: 1.01x slower                                                        |
| chaos                      | 61.7 ms                                                      | 62.2 ms: 1.01x slower                                                       |
| unpickle_list              | 4.62 us                                                      | 4.66 us: 1.01x slower                                                       |
| logging_silent             | 97.7 ns                                                      | 98.8 ns: 1.01x slower                                                       |
| dulwich_log                | 65.5 ms                                                      | 66.3 ms: 1.01x slower                                                       |
| nqueens                    | 88.2 ms                                                      | 89.5 ms: 1.01x slower                                                       |
| json                       | 5.22 ms                                                      | 5.30 ms: 1.02x slower                                                       |
| django_template            | 38.9 ms                                                      | 39.4 ms: 1.02x slower                                                       |
| sqlglot_parse              | 1.38 ms                                                      | 1.40 ms: 1.02x slower                                                       |
| pyflate                    | 492 ms                                                       | 503 ms: 1.02x slower                                                        |
| regex_effbot               | 3.37 ms                                                      | 3.44 ms: 1.02x slower                                                       |
| asyncio_websockets         | 382 ms                                                       | 392 ms: 1.03x slower                                                        |
| docutils                   | 2.81 sec                                                     | 2.89 sec: 1.03x slower                                                      |
| coroutines                 | 21.6 ms                                                      | 22.2 ms: 1.03x slower                                                       |
| xml_etree_parse            | 145 ms                                                       | 149 ms: 1.03x slower                                                        |
| xml_etree_iterparse        | 100 ms                                                       | 103 ms: 1.03x slower                                                        |
| json_loads                 | 24.0 us                                                      | 25.2 us: 1.05x slower                                                       |
| gc_traversal               | 4.11 ms                                                      | 4.44 ms: 1.08x slower                                                       |
| tomli_loads                | 2.41 sec                                                     | 2.60 sec: 1.08x slower                                                      |
| create_gc_cycles           | 1.76 ms                                                      | 1.97 ms: 1.12x slower                                                       |
| bench_mp_pool              | 4.65 ms                                                      | 2.16 sec: 463.06x slower                                                    |
| Geometric mean             | (ref)                                                        | 1.04x slower                                                                |

Benchmark hidden because not significant (12): pylint, typing_runtime_protocols, pprint_safe_repr, thrift, fannkuch, spectral_norm, sqlglot_transpile, unpickle, crypto_pyaes, pprint_pformat, async_generators, bench_thread_pool
Ignored benchmarks (6) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2

# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.01x