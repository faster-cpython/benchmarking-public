# Results vs. 3.13.0b2

- fork: python
- ref: 151934a324789c58cca9
- machine: linux-aarch64
- commit hash: 151934a
- commit date: 2024-08-04
- overall geometric mean: 1.07x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.01x slower
- Memory change: 1.09x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 305 ms                                                       | 369 ms: 1.21x slower                                                    |
| docutils       | 3.10 sec                                                     | 3.74 sec: 1.21x slower                                                  |
| html5lib       | 66.1 ms                                                      | 70.6 ms: 1.07x slower                                                   |
| tornado_http   | 128 ms                                                       | 140 ms: 1.10x slower                                                    |
| Geometric mean | (ref)                                                        | 1.14x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.27 sec                                                     | 1.11 sec: 1.15x faster                                                  |
| async_tree_none_tg         | 476 ms                                                       | 421 ms: 1.13x faster                                                    |
| async_tree_memoization_tg  | 604 ms                                                       | 537 ms: 1.13x faster                                                    |
| async_tree_none            | 492 ms                                                       | 440 ms: 1.12x faster                                                    |
| async_tree_memoization     | 645 ms                                                       | 585 ms: 1.10x faster                                                    |
| async_tree_cpu_io_mixed_tg | 763 ms                                                       | 707 ms: 1.08x faster                                                    |
| async_tree_cpu_io_mixed    | 791 ms                                                       | 750 ms: 1.05x faster                                                    |
| async_tree_io              | 1.22 sec                                                     | 1.18 sec: 1.03x faster                                                  |
| Geometric mean             | (ref)                                                        | 1.10x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 91.4 ms                                                      | 91.8 ms: 1.00x slower                                                   |
| Geometric mean | (ref)                                                        | 1.01x slower                                                            |

Benchmark hidden because not significant (2): pidigits, nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.1 ms                                                      | 30.6 ms: 1.01x faster                                                   |
| regex_compile  | 128 ms                                                       | 174 ms: 1.36x slower                                                    |
| Geometric mean | (ref)                                                        | 1.08x slower                                                            |

Benchmark hidden because not significant (2): regex_effbot, regex_dna

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_generate   | 114 ms                                                       | 113 ms: 1.01x faster                                                    |
| xml_etree_iterparse  | 147 ms                                                       | 148 ms: 1.01x slower                                                    |
| json_dumps           | 13.1 ms                                                      | 13.2 ms: 1.01x slower                                                   |
| json_loads           | 32.1 us                                                      | 32.5 us: 1.01x slower                                                   |
| tomli_loads          | 2.57 sec                                                     | 2.64 sec: 1.03x slower                                                  |
| unpickle_pure_python | 251 us                                                       | 275 us: 1.10x slower                                                    |
| pickle_pure_python   | 359 us                                                       | 395 us: 1.10x slower                                                    |
| Geometric mean       | (ref)                                                        | 1.03x slower                                                            |

Benchmark hidden because not significant (2): xml_etree_parse, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|------------------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 13.0 ms                                                      | 13.2 ms: 1.01x slower                                                   |
| python_startup_no_site | 8.60 ms                                                      | 8.96 ms: 1.04x slower                                                   |
| Geometric mean         | (ref)                                                        | 1.03x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|-----------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.2 ms                                                      | 12.9 ms: 1.02x faster                                                   |
| genshi_text     | 27.4 ms                                                      | 32.1 ms: 1.17x slower                                                   |
| django_template | 42.3 ms                                                      | 51.5 ms: 1.22x slower                                                   |
| genshi_xml      | 60.4 ms                                                      | 74.0 ms: 1.23x slower                                                   |
| Geometric mean  | (ref)                                                        | 1.14x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-arminc-aarch64-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 50.8 us                                                      | 39.4 us: 1.29x faster                                                   |
| deepcopy                   | 448 us                                                       | 382 us: 1.17x faster                                                    |
| async_tree_io_tg           | 1.27 sec                                                     | 1.11 sec: 1.15x faster                                                  |
| async_tree_none_tg         | 476 ms                                                       | 421 ms: 1.13x faster                                                    |
| async_tree_memoization_tg  | 604 ms                                                       | 537 ms: 1.13x faster                                                    |
| async_tree_none            | 492 ms                                                       | 440 ms: 1.12x faster                                                    |
| async_tree_memoization     | 645 ms                                                       | 585 ms: 1.10x faster                                                    |
| async_tree_cpu_io_mixed_tg | 763 ms                                                       | 707 ms: 1.08x faster                                                    |
| scimark_sor                | 159 ms                                                       | 151 ms: 1.05x faster                                                    |
| async_tree_cpu_io_mixed    | 791 ms                                                       | 750 ms: 1.05x faster                                                    |
| gc_traversal               | 5.17 ms                                                      | 4.92 ms: 1.05x faster                                                   |
| async_tree_io              | 1.22 sec                                                     | 1.18 sec: 1.03x faster                                                  |
| richards                   | 55.9 ms                                                      | 54.6 ms: 1.02x faster                                                   |
| mako                       | 13.2 ms                                                      | 12.9 ms: 1.02x faster                                                   |
| pathlib                    | 22.8 ms                                                      | 22.4 ms: 1.02x faster                                                   |
| coroutines                 | 29.0 ms                                                      | 28.5 ms: 1.02x faster                                                   |
| regex_v8                   | 31.1 ms                                                      | 30.6 ms: 1.01x faster                                                   |
| xml_etree_generate         | 114 ms                                                       | 113 ms: 1.01x faster                                                    |
| float                      | 91.4 ms                                                      | 91.8 ms: 1.00x slower                                                   |
| asyncio_websockets         | 657 ms                                                       | 661 ms: 1.01x slower                                                    |
| xml_etree_iterparse        | 147 ms                                                       | 148 ms: 1.01x slower                                                    |
| bpe_tokeniser              | 5.83 sec                                                     | 5.88 sec: 1.01x slower                                                  |
| json_dumps                 | 13.1 ms                                                      | 13.2 ms: 1.01x slower                                                   |
| json_loads                 | 32.1 us                                                      | 32.5 us: 1.01x slower                                                   |
| telco                      | 10.0 ms                                                      | 10.2 ms: 1.01x slower                                                   |
| python_startup             | 13.0 ms                                                      | 13.2 ms: 1.01x slower                                                   |
| thrift                     | 962 us                                                       | 979 us: 1.02x slower                                                    |
| logging_simple             | 7.21 us                                                      | 7.35 us: 1.02x slower                                                   |
| coverage                   | 98.4 ms                                                      | 100 ms: 1.02x slower                                                    |
| asyncio_tcp_ssl            | 2.21 sec                                                     | 2.26 sec: 1.02x slower                                                  |
| scimark_fft                | 445 ms                                                       | 455 ms: 1.02x slower                                                    |
| spectral_norm              | 141 ms                                                       | 144 ms: 1.02x slower                                                    |
| logging_format             | 7.82 us                                                      | 8.03 us: 1.03x slower                                                   |
| tomli_loads                | 2.57 sec                                                     | 2.64 sec: 1.03x slower                                                  |
| deepcopy_reduce            | 4.04 us                                                      | 4.20 us: 1.04x slower                                                   |
| scimark_sparse_mat_mult    | 6.55 ms                                                      | 6.82 ms: 1.04x slower                                                   |
| python_startup_no_site     | 8.60 ms                                                      | 8.96 ms: 1.04x slower                                                   |
| async_generators           | 488 ms                                                       | 510 ms: 1.05x slower                                                    |
| bench_thread_pool          | 1.26 ms                                                      | 1.32 ms: 1.05x slower                                                   |
| mdp                        | 3.33 sec                                                     | 3.50 sec: 1.05x slower                                                  |
| scimark_lu                 | 141 ms                                                       | 149 ms: 1.06x slower                                                    |
| meteor_contest             | 113 ms                                                       | 120 ms: 1.06x slower                                                    |
| html5lib                   | 66.1 ms                                                      | 70.6 ms: 1.07x slower                                                   |
| pyflate                    | 557 ms                                                       | 595 ms: 1.07x slower                                                    |
| asyncio_tcp                | 584 ms                                                       | 625 ms: 1.07x slower                                                    |
| scimark_monte_carlo        | 82.3 ms                                                      | 88.5 ms: 1.08x slower                                                   |
| dask                       | 370 ms                                                       | 400 ms: 1.08x slower                                                    |
| fannkuch                   | 451 ms                                                       | 492 ms: 1.09x slower                                                    |
| unpickle_pure_python       | 251 us                                                       | 275 us: 1.10x slower                                                    |
| tornado_http               | 128 ms                                                       | 140 ms: 1.10x slower                                                    |
| crypto_pyaes               | 82.0 ms                                                      | 89.9 ms: 1.10x slower                                                   |
| typing_runtime_protocols   | 193 us                                                       | 213 us: 1.10x slower                                                    |
| pickle_pure_python         | 359 us                                                       | 395 us: 1.10x slower                                                    |
| raytrace                   | 297 ms                                                       | 329 ms: 1.11x slower                                                    |
| go                         | 161 ms                                                       | 182 ms: 1.13x slower                                                    |
| sqlglot_normalize          | 129 ms                                                       | 147 ms: 1.13x slower                                                    |
| bench_mp_pool              | 7.03 ms                                                      | 8.06 ms: 1.15x slower                                                   |
| deltablue                  | 3.88 ms                                                      | 4.45 ms: 1.15x slower                                                   |
| sqlglot_optimize           | 62.6 ms                                                      | 72.3 ms: 1.15x slower                                                   |
| sqlglot_parse              | 1.42 ms                                                      | 1.65 ms: 1.16x slower                                                   |
| genshi_text                | 27.4 ms                                                      | 32.1 ms: 1.17x slower                                                   |
| comprehensions             | 20.5 us                                                      | 24.2 us: 1.18x slower                                                   |
| docutils                   | 3.10 sec                                                     | 3.74 sec: 1.21x slower                                                  |
| 2to3                       | 305 ms                                                       | 369 ms: 1.21x slower                                                    |
| pycparser                  | 1.22 sec                                                     | 1.48 sec: 1.21x slower                                                  |
| django_template            | 42.3 ms                                                      | 51.5 ms: 1.22x slower                                                   |
| pprint_safe_repr           | 933 ms                                                       | 1.14 sec: 1.23x slower                                                  |
| genshi_xml                 | 60.4 ms                                                      | 74.0 ms: 1.23x slower                                                   |
| pprint_pformat             | 1.93 sec                                                     | 2.39 sec: 1.24x slower                                                  |
| sqlglot_transpile          | 1.71 ms                                                      | 2.13 ms: 1.24x slower                                                   |
| nqueens                    | 98.9 ms                                                      | 125 ms: 1.26x slower                                                    |
| pylint                     | 337 ms                                                       | 431 ms: 1.28x slower                                                    |
| chaos                      | 68.3 ms                                                      | 87.4 ms: 1.28x slower                                                   |
| sympy_expand               | 457 ms                                                       | 591 ms: 1.29x slower                                                    |
| hexiom                     | 7.08 ms                                                      | 9.23 ms: 1.30x slower                                                   |
| sympy_str                  | 265 ms                                                       | 349 ms: 1.31x slower                                                    |
| sympy_integrate            | 20.9 ms                                                      | 27.5 ms: 1.32x slower                                                   |
| regex_compile              | 128 ms                                                       | 174 ms: 1.36x slower                                                    |
| sympy_sum                  | 144 ms                                                       | 200 ms: 1.40x slower                                                    |
| generators                 | 37.1 ms                                                      | 57.2 ms: 1.54x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.07x slower                                                            |

Benchmark hidden because not significant (10): xml_etree_parse, richards_super, regex_effbot, pidigits, regex_dna, create_gc_cycles, logging_silent, json, xml_etree_process, nbody
Ignored benchmarks (12) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-arminc-aarch64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.02x
- 95% likely to have a slowdown of 1.02x
- 99% likely to have a slowdown of 1.01x

# Memory
- memory change: 1.09x