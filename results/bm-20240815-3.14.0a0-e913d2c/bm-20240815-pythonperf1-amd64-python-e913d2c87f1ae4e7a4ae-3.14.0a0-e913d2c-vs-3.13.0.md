# Results vs. 3.13.0

- fork: python
- ref: e913d2c87f1ae4e7a4ae
- machine: windows-amd64
- commit hash: e913d2c
- commit date: 2024-08-15
- overall geometric mean: 1.022x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 221 ms                                                      | 231 ms: 1.05x slower                                                       |
| docutils       | 1.57 sec                                                    | 1.75 sec: 1.11x slower                                                     |
| html5lib       | 38.9 ms                                                     | 42.0 ms: 1.08x slower                                                      |
| Geometric mean | (ref)                                                       | 1.06x slower                                                               |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 288 ms                                                      | 250 ms: 1.15x faster                                                       |
| async_tree_none_tg         | 209 ms                                                      | 198 ms: 1.05x faster                                                       |
| async_tree_memoization     | 276 ms                                                      | 266 ms: 1.04x faster                                                       |
| async_tree_none            | 226 ms                                                      | 219 ms: 1.03x faster                                                       |
| async_tree_cpu_io_mixed    | 383 ms                                                      | 391 ms: 1.02x slower                                                       |
| async_tree_cpu_io_mixed_tg | 377 ms                                                      | 395 ms: 1.05x slower                                                       |
| async_tree_io_tg           | 518 ms                                                      | 547 ms: 1.06x slower                                                       |
| coroutines                 | 12.8 ms                                                     | 14.1 ms: 1.10x slower                                                      |
| async_generators           | 223 ms                                                      | 247 ms: 1.11x slower                                                       |
| async_tree_io              | 521 ms                                                      | 577 ms: 1.11x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.02x slower                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| pidigits       | 148 ms                                                      | 151 ms: 1.02x slower                                                       |
| float          | 49.9 ms                                                     | 56.4 ms: 1.13x slower                                                      |
| nbody          | 68.4 ms                                                     | 82.3 ms: 1.20x slower                                                      |
| Geometric mean | (ref)                                                       | 1.12x slower                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_v8       | 21.4 ms                                                     | 16.5 ms: 1.30x faster                                                      |
| regex_effbot   | 1.58 ms                                                     | 1.56 ms: 1.01x faster                                                      |
| regex_dna      | 115 ms                                                      | 118 ms: 1.02x slower                                                       |
| regex_compile  | 80.5 ms                                                     | 91.7 ms: 1.14x slower                                                      |
| Geometric mean | (ref)                                                       | 1.03x faster                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| json_loads           | 15.1 us                                                     | 14.2 us: 1.06x faster                                                      |
| xml_etree_parse      | 93.6 ms                                                     | 94.6 ms: 1.01x slower                                                      |
| json_dumps           | 5.92 ms                                                     | 6.22 ms: 1.05x slower                                                      |
| xml_etree_iterparse  | 61.8 ms                                                     | 65.8 ms: 1.06x slower                                                      |
| xml_etree_generate   | 54.0 ms                                                     | 57.8 ms: 1.07x slower                                                      |
| unpickle_pure_python | 133 us                                                      | 148 us: 1.11x slower                                                       |
| xml_etree_process    | 37.0 ms                                                     | 41.2 ms: 1.11x slower                                                      |
| pickle_pure_python   | 190 us                                                      | 211 us: 1.12x slower                                                       |
| tomli_loads          | 1.39 sec                                                    | 1.58 sec: 1.13x slower                                                     |
| Geometric mean       | (ref)                                                       | 1.07x slower                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup         | 25.4 ms                                                     | 22.0 ms: 1.16x faster                                                      |
| python_startup_no_site | 18.1 ms                                                     | 17.8 ms: 1.02x faster                                                      |
| Geometric mean         | (ref)                                                       | 1.08x faster                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| django_template | 22.4 ms                                                     | 24.6 ms: 1.10x slower                                                      |
| mako            | 6.35 ms                                                     | 7.03 ms: 1.11x slower                                                      |
| genshi_text     | 15.6 ms                                                     | 17.5 ms: 1.12x slower                                                      |
| Geometric mean  | (ref)                                                       | 1.09x slower                                                               |

Benchmark hidden because not significant (1): genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5 | bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| thrift                     | 8.80 ms                                                     | 530 us: 16.61x faster                                                      |
| create_gc_cycles           | 1.26 ms                                                     | 909 us: 1.38x faster                                                       |
| regex_v8                   | 21.4 ms                                                     | 16.5 ms: 1.30x faster                                                      |
| gc_traversal               | 1.97 ms                                                     | 1.57 ms: 1.26x faster                                                      |
| bench_mp_pool              | 86.4 ms                                                     | 68.8 ms: 1.26x faster                                                      |
| deepcopy                   | 226 us                                                      | 188 us: 1.21x faster                                                       |
| python_startup             | 25.4 ms                                                     | 22.0 ms: 1.16x faster                                                      |
| async_tree_memoization_tg  | 288 ms                                                      | 250 ms: 1.15x faster                                                       |
| deepcopy_memo              | 23.3 us                                                     | 20.4 us: 1.14x faster                                                      |
| deepcopy_reduce            | 2.06 us                                                     | 1.89 us: 1.09x faster                                                      |
| json                       | 3.19 ms                                                     | 2.98 ms: 1.07x faster                                                      |
| json_loads                 | 15.1 us                                                     | 14.2 us: 1.06x faster                                                      |
| async_tree_none_tg         | 209 ms                                                      | 198 ms: 1.05x faster                                                       |
| async_tree_memoization     | 276 ms                                                      | 266 ms: 1.04x faster                                                       |
| pathlib                    | 80.9 ms                                                     | 78.3 ms: 1.03x faster                                                      |
| async_tree_none            | 226 ms                                                      | 219 ms: 1.03x faster                                                       |
| python_startup_no_site     | 18.1 ms                                                     | 17.8 ms: 1.02x faster                                                      |
| regex_effbot               | 1.58 ms                                                     | 1.56 ms: 1.01x faster                                                      |
| xml_etree_parse            | 93.6 ms                                                     | 94.6 ms: 1.01x slower                                                      |
| telco                      | 4.79 ms                                                     | 4.87 ms: 1.02x slower                                                      |
| async_tree_cpu_io_mixed    | 383 ms                                                      | 391 ms: 1.02x slower                                                       |
| pidigits                   | 148 ms                                                      | 151 ms: 1.02x slower                                                       |
| regex_dna                  | 115 ms                                                      | 118 ms: 1.02x slower                                                       |
| coverage                   | 45.6 ms                                                     | 47.4 ms: 1.04x slower                                                      |
| 2to3                       | 221 ms                                                      | 231 ms: 1.05x slower                                                       |
| async_tree_cpu_io_mixed_tg | 377 ms                                                      | 395 ms: 1.05x slower                                                       |
| meteor_contest             | 73.5 ms                                                     | 77.2 ms: 1.05x slower                                                      |
| crypto_pyaes               | 45.4 ms                                                     | 47.8 ms: 1.05x slower                                                      |
| json_dumps                 | 5.92 ms                                                     | 6.22 ms: 1.05x slower                                                      |
| logging_simple             | 5.96 us                                                     | 6.29 us: 1.06x slower                                                      |
| async_tree_io_tg           | 518 ms                                                      | 547 ms: 1.06x slower                                                       |
| sympy_sum                  | 86.9 ms                                                     | 92.3 ms: 1.06x slower                                                      |
| dulwich_log                | 40.8 ms                                                     | 43.4 ms: 1.06x slower                                                      |
| xml_etree_iterparse        | 61.8 ms                                                     | 65.8 ms: 1.06x slower                                                      |
| sympy_integrate            | 12.5 ms                                                     | 13.4 ms: 1.07x slower                                                      |
| sympy_str                  | 169 ms                                                      | 180 ms: 1.07x slower                                                       |
| sympy_expand               | 291 ms                                                      | 312 ms: 1.07x slower                                                       |
| xml_etree_generate         | 54.0 ms                                                     | 57.8 ms: 1.07x slower                                                      |
| mdp                        | 1.39 sec                                                    | 1.49 sec: 1.07x slower                                                     |
| logging_format             | 6.26 us                                                     | 6.72 us: 1.07x slower                                                      |
| spectral_norm              | 62.8 ms                                                     | 67.6 ms: 1.08x slower                                                      |
| html5lib                   | 38.9 ms                                                     | 42.0 ms: 1.08x slower                                                      |
| pylint                     | 211 ms                                                      | 228 ms: 1.08x slower                                                       |
| generators                 | 19.5 ms                                                     | 21.2 ms: 1.09x slower                                                      |
| typing_runtime_protocols   | 105 us                                                      | 115 us: 1.09x slower                                                       |
| scimark_sparse_mat_mult    | 2.46 ms                                                     | 2.68 ms: 1.09x slower                                                      |
| coroutines                 | 12.8 ms                                                     | 14.1 ms: 1.10x slower                                                      |
| django_template            | 22.4 ms                                                     | 24.6 ms: 1.10x slower                                                      |
| nqueens                    | 56.7 ms                                                     | 62.4 ms: 1.10x slower                                                      |
| sqlglot_optimize           | 32.9 ms                                                     | 36.3 ms: 1.10x slower                                                      |
| mako                       | 6.35 ms                                                     | 7.03 ms: 1.11x slower                                                      |
| sqlglot_normalize          | 175 ms                                                      | 193 ms: 1.11x slower                                                       |
| async_generators           | 223 ms                                                      | 247 ms: 1.11x slower                                                       |
| async_tree_io              | 521 ms                                                      | 577 ms: 1.11x slower                                                       |
| unpickle_pure_python       | 133 us                                                      | 148 us: 1.11x slower                                                       |
| pprint_safe_repr           | 494 ms                                                      | 549 ms: 1.11x slower                                                       |
| xml_etree_process          | 37.0 ms                                                     | 41.2 ms: 1.11x slower                                                      |
| docutils                   | 1.57 sec                                                    | 1.75 sec: 1.11x slower                                                     |
| pickle_pure_python         | 190 us                                                      | 211 us: 1.12x slower                                                       |
| genshi_text                | 15.6 ms                                                     | 17.5 ms: 1.12x slower                                                      |
| float                      | 49.9 ms                                                     | 56.4 ms: 1.13x slower                                                      |
| go                         | 87.0 ms                                                     | 98.5 ms: 1.13x slower                                                      |
| chaos                      | 38.5 ms                                                     | 43.7 ms: 1.13x slower                                                      |
| tomli_loads                | 1.39 sec                                                    | 1.58 sec: 1.13x slower                                                     |
| regex_compile              | 80.5 ms                                                     | 91.7 ms: 1.14x slower                                                      |
| pyflate                    | 283 ms                                                      | 324 ms: 1.14x slower                                                       |
| pprint_pformat             | 999 ms                                                      | 1.14 sec: 1.15x slower                                                     |
| hexiom                     | 3.89 ms                                                     | 4.46 ms: 1.15x slower                                                      |
| sqlglot_parse              | 771 us                                                      | 888 us: 1.15x slower                                                       |
| sqlglot_transpile          | 956 us                                                      | 1.11 ms: 1.16x slower                                                      |
| fannkuch                   | 253 ms                                                      | 294 ms: 1.16x slower                                                       |
| logging_silent             | 54.6 ns                                                     | 63.7 ns: 1.17x slower                                                      |
| comprehensions             | 10.3 us                                                     | 12.0 us: 1.17x slower                                                      |
| richards_super             | 30.9 ms                                                     | 36.2 ms: 1.17x slower                                                      |
| richards                   | 27.3 ms                                                     | 32.3 ms: 1.18x slower                                                      |
| deltablue                  | 1.92 ms                                                     | 2.27 ms: 1.19x slower                                                      |
| scimark_fft                | 172 ms                                                      | 205 ms: 1.19x slower                                                       |
| nbody                      | 68.4 ms                                                     | 82.3 ms: 1.20x slower                                                      |
| scimark_sor                | 76.2 ms                                                     | 92.2 ms: 1.21x slower                                                      |
| raytrace                   | 160 ms                                                      | 195 ms: 1.22x slower                                                       |
| scimark_monte_carlo        | 40.8 ms                                                     | 50.3 ms: 1.23x slower                                                      |
| scimark_lu                 | 53.0 ms                                                     | 65.5 ms: 1.24x slower                                                      |
| pycparser                  | 683 ms                                                      | 888 ms: 1.30x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.02x slower                                                               |

Benchmark hidden because not significant (3): bench_thread_pool, tornado_http, genshi_xml
Ignored benchmarks (11) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1-amd64-python-v3.13.0-3.13.0-60403a5.json: asyncio_websockets, bpe_tokeniser, chameleon, connected_components, gevent_hub, k_core, mypy2, shortest_path, sphinx, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (2) of results/bm-20240815-3.14.0a0-e913d2c/bm-20240815-pythonperf1-amd64-python-e913d2c87f1ae4e7a4ae-3.14.0a0-e913d2c.json: asyncio_tcp, asyncio_tcp_ssl

- Geometric mean (including insignificant results): 1.022x slower
# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.06x
- 95% likely to have a slowdown of 1.06x
- 99% likely to have a slowdown of 1.05x

# Memory
- memory change: unknown