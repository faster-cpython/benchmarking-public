
# Results vs. 3.11.0

- fork: python
- ref: 3d5d3f7af6498effbc60
- machine: linux-x86_64
- commit hash: 3d5d3f7
- commit date: 2023-01-10
- overall geometric mean: 1.04x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|----------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 257 ms                                                              | 253 ms: 1.02x faster                                                  |
| chameleon      | 6.52 ms                                                             | 6.46 ms: 1.01x faster                                                 |
| docutils       | 2.59 sec                                                            | 2.48 sec: 1.04x faster                                                |
| html5lib       | 64.0 ms                                                             | 59.8 ms: 1.07x faster                                                 |
| Geometric mean | (ref)                                                               | 1.03x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|----------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 76.0 ms                                                             | 72.2 ms: 1.05x faster                                                 |
| nbody          | 96.7 ms                                                             | 93.1 ms: 1.04x faster                                                 |
| pidigits       | 197 ms                                                              | 192 ms: 1.02x faster                                                  |
| Geometric mean | (ref)                                                               | 1.04x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|----------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_v8       | 22.0 ms                                                             | 21.1 ms: 1.04x faster                                                 |
| regex_compile  | 137 ms                                                              | 132 ms: 1.04x faster                                                  |
| regex_effbot   | 3.32 ms                                                             | 3.49 ms: 1.05x slower                                                 |
| regex_dna      | 196 ms                                                              | 209 ms: 1.06x slower                                                  |
| Geometric mean | (ref)                                                               | 1.01x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|----------------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 12.5 ms                                                             | 9.54 ms: 1.31x faster                                                 |
| unpickle_pure_python | 228 us                                                              | 200 us: 1.14x faster                                                  |
| xml_etree_parse      | 162 ms                                                              | 149 ms: 1.09x faster                                                  |
| pickle_pure_python   | 307 us                                                              | 285 us: 1.08x faster                                                  |
| json_loads           | 26.2 us                                                             | 24.3 us: 1.08x faster                                                 |
| pickle_dict          | 30.9 us                                                             | 30.0 us: 1.03x faster                                                 |
| unpickle             | 13.2 us                                                             | 13.0 us: 1.02x faster                                                 |
| xml_etree_iterparse  | 108 ms                                                              | 106 ms: 1.01x faster                                                  |
| xml_etree_process    | 54.1 ms                                                             | 53.9 ms: 1.00x faster                                                 |
| xml_etree_generate   | 76.5 ms                                                             | 77.5 ms: 1.01x slower                                                 |
| pickle               | 9.79 us                                                             | 10.0 us: 1.02x slower                                                 |
| Geometric mean       | (ref)                                                               | 1.05x faster                                                          |

Benchmark hidden because not significant (2): pickle_list, unpickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|------------------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 8.49 ms                                                             | 8.54 ms: 1.01x slower                                                 |
| python_startup_no_site | 5.98 ms                                                             | 6.09 ms: 1.02x slower                                                 |
| Geometric mean         | (ref)                                                               | 1.01x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|-----------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| genshi_xml      | 51.8 ms                                                             | 46.5 ms: 1.11x faster                                                 |
| genshi_text     | 21.8 ms                                                             | 20.8 ms: 1.05x faster                                                 |
| django_template | 32.9 ms                                                             | 32.6 ms: 1.01x faster                                                 |
| mako            | 9.82 ms                                                             | 9.74 ms: 1.01x faster                                                 |
| Geometric mean  | (ref)                                                               | 1.04x faster                                                          |

All benchmarks:
===============

| Benchmark               | bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509 | bm-20230110-linux-x86_64-python-3d5d3f7af6498effbc60-3.12.0a4-3d5d3f7 |
|-------------------------|:-------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| asyncio_tcp             | 888 ms                                                              | 504 ms: 1.76x faster                                                  |
| json_dumps              | 12.5 ms                                                             | 9.54 ms: 1.31x faster                                                 |
| mypy2                   | 422 ms                                                              | 332 ms: 1.27x faster                                                  |
| unpack_sequence         | 49.5 ns                                                             | 41.4 ns: 1.19x faster                                                 |
| unpickle_pure_python    | 228 us                                                              | 200 us: 1.14x faster                                                  |
| deltablue               | 3.66 ms                                                             | 3.25 ms: 1.13x faster                                                 |
| genshi_xml              | 51.8 ms                                                             | 46.5 ms: 1.11x faster                                                 |
| xml_etree_parse         | 162 ms                                                              | 149 ms: 1.09x faster                                                  |
| hexiom                  | 6.48 ms                                                             | 5.98 ms: 1.08x faster                                                 |
| scimark_sparse_mat_mult | 4.47 ms                                                             | 4.13 ms: 1.08x faster                                                 |
| richards                | 45.7 ms                                                             | 42.3 ms: 1.08x faster                                                 |
| nqueens                 | 84.0 ms                                                             | 78.0 ms: 1.08x faster                                                 |
| pickle_pure_python      | 307 us                                                              | 285 us: 1.08x faster                                                  |
| json_loads              | 26.2 us                                                             | 24.3 us: 1.08x faster                                                 |
| html5lib                | 64.0 ms                                                             | 59.8 ms: 1.07x faster                                                 |
| scimark_sor             | 115 ms                                                              | 108 ms: 1.06x faster                                                  |
| fannkuch                | 384 ms                                                              | 362 ms: 1.06x faster                                                  |
| pycparser               | 1.14 sec                                                            | 1.08 sec: 1.06x faster                                                |
| logging_simple          | 6.09 us                                                             | 5.77 us: 1.06x faster                                                 |
| logging_silent          | 98.7 ns                                                             | 93.5 ns: 1.06x faster                                                 |
| telco                   | 6.59 ms                                                             | 6.26 ms: 1.05x faster                                                 |
| sqlglot_optimize        | 53.4 ms                                                             | 50.7 ms: 1.05x faster                                                 |
| float                   | 76.0 ms                                                             | 72.2 ms: 1.05x faster                                                 |
| genshi_text             | 21.8 ms                                                             | 20.8 ms: 1.05x faster                                                 |
| pprint_pformat          | 1.45 sec                                                            | 1.38 sec: 1.05x faster                                                |
| deepcopy_memo           | 36.4 us                                                             | 34.7 us: 1.05x faster                                                 |
| sympy_expand            | 477 ms                                                              | 455 ms: 1.05x faster                                                  |
| bench_thread_pool       | 820 us                                                              | 782 us: 1.05x faster                                                  |
| spectral_norm           | 99.5 ms                                                             | 95.0 ms: 1.05x faster                                                 |
| logging_format          | 6.64 us                                                             | 6.35 us: 1.04x faster                                                 |
| scimark_fft             | 328 ms                                                              | 314 ms: 1.04x faster                                                  |
| docutils                | 2.59 sec                                                            | 2.48 sec: 1.04x faster                                                |
| djangocms               | 32.3 us                                                             | 30.9 us: 1.04x faster                                                 |
| sqlglot_normalize       | 108 ms                                                              | 104 ms: 1.04x faster                                                  |
| pyflate                 | 414 ms                                                              | 397 ms: 1.04x faster                                                  |
| regex_v8                | 22.0 ms                                                             | 21.1 ms: 1.04x faster                                                 |
| nbody                   | 96.7 ms                                                             | 93.1 ms: 1.04x faster                                                 |
| dask                    | 368 ms                                                              | 355 ms: 1.04x faster                                                  |
| sympy_integrate         | 21.0 ms                                                             | 20.3 ms: 1.04x faster                                                 |
| regex_compile           | 137 ms                                                              | 132 ms: 1.04x faster                                                  |
| sympy_str               | 291 ms                                                              | 282 ms: 1.03x faster                                                  |
| scimark_monte_carlo     | 67.8 ms                                                             | 65.7 ms: 1.03x faster                                                 |
| coroutines              | 26.3 ms                                                             | 25.4 ms: 1.03x faster                                                 |
| pickle_dict             | 30.9 us                                                             | 30.0 us: 1.03x faster                                                 |
| pprint_safe_repr        | 701 ms                                                              | 680 ms: 1.03x faster                                                  |
| raytrace                | 292 ms                                                              | 284 ms: 1.03x faster                                                  |
| sympy_sum               | 167 ms                                                              | 163 ms: 1.03x faster                                                  |
| dulwich_log             | 63.6 ms                                                             | 62.1 ms: 1.03x faster                                                 |
| json                    | 4.86 ms                                                             | 4.74 ms: 1.02x faster                                                 |
| go                      | 138 ms                                                              | 135 ms: 1.02x faster                                                  |
| pidigits                | 197 ms                                                              | 192 ms: 1.02x faster                                                  |
| coverage                | 101 ms                                                              | 99.0 ms: 1.02x faster                                                 |
| unpickle                | 13.2 us                                                             | 13.0 us: 1.02x faster                                                 |
| async_generators        | 361 ms                                                              | 354 ms: 1.02x faster                                                  |
| meteor_contest          | 106 ms                                                              | 104 ms: 1.02x faster                                                  |
| create_gc_cycles        | 1.48 ms                                                             | 1.45 ms: 1.02x faster                                                 |
| thrift                  | 766 us                                                              | 752 us: 1.02x faster                                                  |
| 2to3                    | 257 ms                                                              | 253 ms: 1.02x faster                                                  |
| gc_traversal            | 3.63 ms                                                             | 3.57 ms: 1.02x faster                                                 |
| xml_etree_iterparse     | 108 ms                                                              | 106 ms: 1.01x faster                                                  |
| django_template         | 32.9 ms                                                             | 32.6 ms: 1.01x faster                                                 |
| pathlib                 | 18.2 ms                                                             | 18.0 ms: 1.01x faster                                                 |
| scimark_lu              | 108 ms                                                              | 107 ms: 1.01x faster                                                  |
| chameleon               | 6.52 ms                                                             | 6.46 ms: 1.01x faster                                                 |
| mako                    | 9.82 ms                                                             | 9.74 ms: 1.01x faster                                                 |
| xml_etree_process       | 54.1 ms                                                             | 53.9 ms: 1.00x faster                                                 |
| python_startup          | 8.49 ms                                                             | 8.54 ms: 1.01x slower                                                 |
| mdp                     | 2.64 sec                                                            | 2.66 sec: 1.01x slower                                                |
| deepcopy_reduce         | 2.96 us                                                             | 2.99 us: 1.01x slower                                                 |
| async_tree_io           | 1.28 sec                                                            | 1.30 sec: 1.01x slower                                                |
| xml_etree_generate      | 76.5 ms                                                             | 77.5 ms: 1.01x slower                                                 |
| async_tree_cpu_io_mixed | 736 ms                                                              | 747 ms: 1.02x slower                                                  |
| python_startup_no_site  | 5.98 ms                                                             | 6.09 ms: 1.02x slower                                                 |
| pickle                  | 9.79 us                                                             | 10.0 us: 1.02x slower                                                 |
| sqlite_synth            | 2.51 us                                                             | 2.57 us: 1.02x slower                                                 |
| sqlglot_transpile       | 1.65 ms                                                             | 1.69 ms: 1.02x slower                                                 |
| crypto_pyaes            | 73.8 ms                                                             | 75.7 ms: 1.03x slower                                                 |
| sqlglot_parse           | 1.36 ms                                                             | 1.41 ms: 1.03x slower                                                 |
| regex_effbot            | 3.32 ms                                                             | 3.49 ms: 1.05x slower                                                 |
| regex_dna               | 196 ms                                                              | 209 ms: 1.06x slower                                                  |
| comprehensions          | 22.2 us                                                             | 23.7 us: 1.07x slower                                                 |
| generators              | 73.4 ms                                                             | 79.1 ms: 1.08x slower                                                 |
| Geometric mean          | (ref)                                                               | 1.04x faster                                                          |

Benchmark hidden because not significant (7): async_tree_none, async_tree_memoization, chaos, pickle_list, bench_mp_pool, unpickle_list, deepcopy
Ignored benchmarks (7) of /home/runner/work/benchmarking/benchmarking/results/bm-20221024-3.11.0-deaf509/bm-20221024-linux-x86_64-python-deaf509e8fc6e0363bd6-3.11.0-deaf509.json: aiohttp, flaskblogging, gunicorn, pylint, sqlalchemy_declarative, sqlalchemy_imperative, tornado_http
