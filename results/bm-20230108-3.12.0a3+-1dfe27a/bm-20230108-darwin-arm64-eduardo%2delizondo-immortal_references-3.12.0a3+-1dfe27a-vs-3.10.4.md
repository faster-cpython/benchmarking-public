
# Results vs. 3.10.4

- fork: eduardo-elizondo
- ref: immortal_references
- machine: darwin-arm64
- commit hash: 1dfe27a
- commit date: 2023-01-08
- overall geometric mean: 1.25x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.20x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 178 ms: 1.13x faster                                                              |
| chameleon      | 5.84 ms                                                | 4.51 ms: 1.30x faster                                                             |
| docutils       | 1.78 sec                                               | 1.45 sec: 1.23x faster                                                            |
| html5lib       | 44.1 ms                                                | 33.6 ms: 1.31x faster                                                             |
| Geometric mean | (ref)                                                  | 1.24x faster                                                                      |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 66.3 ms: 1.42x faster                                                             |
| float          | 72.3 ms                                                | 52.0 ms: 1.39x faster                                                             |
| pidigits       | 282 ms                                                 | 282 ms: 1.00x slower                                                              |
| Geometric mean | (ref)                                                  | 1.25x faster                                                                      |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 69.8 ms: 1.38x faster                                                             |
| regex_v8       | 17.5 ms                                                | 16.1 ms: 1.09x faster                                                             |
| regex_dna      | 160 ms                                                 | 149 ms: 1.07x faster                                                              |
| regex_effbot   | 2.45 ms                                                | 2.63 ms: 1.07x slower                                                             |
| Geometric mean | (ref)                                                  | 1.11x faster                                                                      |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 187 us: 1.52x faster                                                              |
| unpickle_pure_python | 203 us                                                 | 143 us: 1.43x faster                                                              |
| json_dumps           | 8.38 ms                                                | 5.95 ms: 1.41x faster                                                             |
| xml_etree_process    | 45.1 ms                                                | 35.0 ms: 1.29x faster                                                             |
| xml_etree_parse      | 107 ms                                                 | 94.7 ms: 1.13x faster                                                             |
| xml_etree_generate   | 54.3 ms                                                | 49.3 ms: 1.10x faster                                                             |
| xml_etree_iterparse  | 72.6 ms                                                | 66.3 ms: 1.10x faster                                                             |
| json_loads           | 16.9 us                                                | 16.3 us: 1.03x faster                                                             |
| pickle_list          | 2.83 us                                                | 2.86 us: 1.01x slower                                                             |
| pickle_dict          | 17.8 us                                                | 18.1 us: 1.02x slower                                                             |
| unpickle             | 9.77 us                                                | 10.2 us: 1.04x slower                                                             |
| pickle               | 7.36 us                                                | 7.69 us: 1.04x slower                                                             |
| Geometric mean       | (ref)                                                  | 1.13x faster                                                                      |

Benchmark hidden because not significant (1): unpickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 9.33 ms: 1.35x faster                                                             |
| python_startup_no_site | 9.73 ms                                                | 7.31 ms: 1.33x faster                                                             |
| Geometric mean         | (ref)                                                  | 1.34x faster                                                                      |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| genshi_xml      | 37.6 ms                                                | 28.5 ms: 1.32x faster                                                             |
| django_template | 27.3 ms                                                | 20.9 ms: 1.31x faster                                                             |
| genshi_text     | 18.4 ms                                                | 14.4 ms: 1.28x faster                                                             |
| mako            | 10.5 ms                                                | 8.51 ms: 1.23x faster                                                             |
| Geometric mean  | (ref)                                                  | 1.28x faster                                                                      |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a |
|-------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.56 ms: 2.01x faster                                                             |
| logging_silent          | 119 ns                                                 | 63.7 ns: 1.87x faster                                                             |
| richards                | 51.4 ms                                                | 28.9 ms: 1.78x faster                                                             |
| asyncio_tcp             | 673 ms                                                 | 412 ms: 1.63x faster                                                              |
| go                      | 165 ms                                                 | 103 ms: 1.60x faster                                                              |
| scimark_monte_carlo     | 72.2 ms                                                | 46.2 ms: 1.56x faster                                                             |
| async_tree_memoization  | 493 ms                                                 | 317 ms: 1.55x faster                                                              |
| pickle_pure_python      | 284 us                                                 | 187 us: 1.52x faster                                                              |
| scimark_sor             | 127 ms                                                 | 84.0 ms: 1.51x faster                                                             |
| crypto_pyaes            | 78.0 ms                                                | 52.0 ms: 1.50x faster                                                             |
| raytrace                | 328 ms                                                 | 222 ms: 1.48x faster                                                              |
| async_tree_none         | 402 ms                                                 | 276 ms: 1.46x faster                                                              |
| scimark_lu              | 110 ms                                                 | 75.7 ms: 1.45x faster                                                             |
| chaos                   | 66.8 ms                                                | 46.1 ms: 1.45x faster                                                             |
| unpickle_pure_python    | 203 us                                                 | 143 us: 1.43x faster                                                              |
| hexiom                  | 6.32 ms                                                | 4.44 ms: 1.42x faster                                                             |
| nbody                   | 94.1 ms                                                | 66.3 ms: 1.42x faster                                                             |
| pyflate                 | 453 ms                                                 | 319 ms: 1.42x faster                                                              |
| async_tree_io           | 1.02 sec                                               | 720 ms: 1.42x faster                                                              |
| json_dumps              | 8.38 ms                                                | 5.95 ms: 1.41x faster                                                             |
| logging_simple          | 4.63 us                                                | 3.30 us: 1.40x faster                                                             |
| logging_format          | 5.01 us                                                | 3.59 us: 1.40x faster                                                             |
| float                   | 72.3 ms                                                | 52.0 ms: 1.39x faster                                                             |
| pathlib                 | 28.8 ms                                                | 20.8 ms: 1.39x faster                                                             |
| regex_compile           | 96.6 ms                                                | 69.8 ms: 1.38x faster                                                             |
| pycparser               | 915 ms                                                 | 667 ms: 1.37x faster                                                              |
| fannkuch                | 317 ms                                                 | 232 ms: 1.37x faster                                                              |
| pprint_pformat          | 1.24 sec                                               | 910 ms: 1.36x faster                                                              |
| thrift                  | 586 us                                                 | 431 us: 1.36x faster                                                              |
| pprint_safe_repr        | 609 ms                                                 | 448 ms: 1.36x faster                                                              |
| python_startup          | 12.6 ms                                                | 9.33 ms: 1.35x faster                                                             |
| unpack_sequence         | 38.7 ns                                                | 28.8 ns: 1.34x faster                                                             |
| deepcopy_memo           | 34.5 us                                                | 25.6 us: 1.34x faster                                                             |
| sqlglot_parse           | 1.33 ms                                                | 991 us: 1.34x faster                                                              |
| sqlglot_transpile       | 1.54 ms                                                | 1.15 ms: 1.34x faster                                                             |
| python_startup_no_site  | 9.73 ms                                                | 7.31 ms: 1.33x faster                                                             |
| dulwich_log             | 37.1 ms                                                | 27.9 ms: 1.33x faster                                                             |
| spectral_norm           | 96.4 ms                                                | 73.0 ms: 1.32x faster                                                             |
| genshi_xml              | 37.6 ms                                                | 28.5 ms: 1.32x faster                                                             |
| html5lib                | 44.1 ms                                                | 33.6 ms: 1.31x faster                                                             |
| django_template         | 27.3 ms                                                | 20.9 ms: 1.31x faster                                                             |
| chameleon               | 5.84 ms                                                | 4.51 ms: 1.30x faster                                                             |
| create_gc_cycles        | 876 us                                                 | 677 us: 1.29x faster                                                              |
| deepcopy                | 278 us                                                 | 215 us: 1.29x faster                                                              |
| xml_etree_process       | 45.1 ms                                                | 35.0 ms: 1.29x faster                                                             |
| deepcopy_reduce         | 2.38 us                                                | 1.85 us: 1.29x faster                                                             |
| genshi_text             | 18.4 ms                                                | 14.4 ms: 1.28x faster                                                             |
| async_tree_cpu_io_mixed | 670 ms                                                 | 530 ms: 1.26x faster                                                              |
| bench_thread_pool       | 548 us                                                 | 443 us: 1.24x faster                                                              |
| mako                    | 10.5 ms                                                | 8.51 ms: 1.23x faster                                                             |
| docutils                | 1.78 sec                                               | 1.45 sec: 1.23x faster                                                            |
| sqlglot_optimize        | 38.0 ms                                                | 31.1 ms: 1.22x faster                                                             |
| nqueens                 | 68.1 ms                                                | 55.9 ms: 1.22x faster                                                             |
| coroutines              | 20.2 ms                                                | 16.8 ms: 1.20x faster                                                             |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.90 ms: 1.20x faster                                                             |
| sympy_expand            | 276 ms                                                 | 232 ms: 1.19x faster                                                              |
| sympy_integrate         | 13.4 ms                                                | 11.2 ms: 1.19x faster                                                             |
| async_generators        | 233 ms                                                 | 197 ms: 1.18x faster                                                              |
| sympy_str               | 169 ms                                                 | 145 ms: 1.17x faster                                                              |
| scimark_fft             | 232 ms                                                 | 199 ms: 1.17x faster                                                              |
| sqlglot_normalize       | 197 ms                                                 | 170 ms: 1.16x faster                                                              |
| 2to3                    | 202 ms                                                 | 178 ms: 1.13x faster                                                              |
| sympy_sum               | 94.3 ms                                                | 83.2 ms: 1.13x faster                                                             |
| xml_etree_parse         | 107 ms                                                 | 94.7 ms: 1.13x faster                                                             |
| mdp                     | 1.67 sec                                               | 1.50 sec: 1.11x faster                                                            |
| xml_etree_generate      | 54.3 ms                                                | 49.3 ms: 1.10x faster                                                             |
| xml_etree_iterparse     | 72.6 ms                                                | 66.3 ms: 1.10x faster                                                             |
| json                    | 3.10 ms                                                | 2.83 ms: 1.09x faster                                                             |
| regex_v8                | 17.5 ms                                                | 16.1 ms: 1.09x faster                                                             |
| regex_dna               | 160 ms                                                 | 149 ms: 1.07x faster                                                              |
| telco                   | 3.68 ms                                                | 3.45 ms: 1.07x faster                                                             |
| meteor_contest          | 78.6 ms                                                | 74.3 ms: 1.06x faster                                                             |
| sqlite_synth            | 1.47 us                                                | 1.40 us: 1.05x faster                                                             |
| json_loads              | 16.9 us                                                | 16.3 us: 1.03x faster                                                             |
| pidigits                | 282 ms                                                 | 282 ms: 1.00x slower                                                              |
| gc_traversal            | 2.40 ms                                                | 2.40 ms: 1.00x slower                                                             |
| pickle_list             | 2.83 us                                                | 2.86 us: 1.01x slower                                                             |
| pickle_dict             | 17.8 us                                                | 18.1 us: 1.02x slower                                                             |
| generators              | 32.9 ms                                                | 33.7 ms: 1.02x slower                                                             |
| unpickle                | 9.77 us                                                | 10.2 us: 1.04x slower                                                             |
| pickle                  | 7.36 us                                                | 7.69 us: 1.04x slower                                                             |
| bench_mp_pool           | 41.0 ms                                                | 42.9 ms: 1.05x slower                                                             |
| regex_effbot            | 2.45 ms                                                | 2.63 ms: 1.07x slower                                                             |
| dask                    | 258 ms                                                 | 315 ms: 1.22x slower                                                              |
| coverage                | 40.8 ms                                                | 56.2 ms: 1.38x slower                                                             |
| Geometric mean          | (ref)                                                  | 1.25x faster                                                                      |

Benchmark hidden because not significant (1): unpickle_list
Ignored benchmarks (13) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, comprehensions, flaskblogging, gunicorn, mypy2, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20230108-3.12.0a3+-1dfe27a/bm-20230108-darwin-arm64-eduardo%2delizondo-immortal_references-3.12.0a3+-1dfe27a.json: mypy


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.23x
- 95% likely to have a speedup of 1.22x
- 99% likely to have a speedup of 1.20x
