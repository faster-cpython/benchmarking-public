# Results vs. 3.13.0

- fork: python
- ref: 498376d7a7d6f704f22a
- machine: linux-aarch64
- commit hash: 498376d
- commit date: 2024-08-02
- overall geometric mean: 1.08x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x slower
- Memory change: 1.10x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 372 ms: 1.22x slower                                                    |
| docutils       | 2.91 sec                                                 | 3.73 sec: 1.28x slower                                                  |
| html5lib       | 64.3 ms                                                  | 70.5 ms: 1.10x slower                                                   |
| tornado_http   | 131 ms                                                   | 137 ms: 1.04x slower                                                    |
| Geometric mean | (ref)                                                    | 1.16x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 537 ms: 1.21x faster                                                    |
| async_tree_none            | 493 ms                                                   | 435 ms: 1.13x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 419 ms: 1.12x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 579 ms: 1.08x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 708 ms: 1.04x faster                                                    |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 742 ms: 1.03x faster                                                    |
| asyncio_websockets         | 656 ms                                                   | 661 ms: 1.01x slower                                                    |
| async_tree_io_tg           | 1.09 sec                                                 | 1.11 sec: 1.02x slower                                                  |
| async_generators           | 496 ms                                                   | 509 ms: 1.03x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.28 sec: 1.05x slower                                                  |
| async_tree_io              | 1.11 sec                                                 | 1.17 sec: 1.06x slower                                                  |
| asyncio_tcp                | 568 ms                                                   | 622 ms: 1.10x slower                                                    |
| Geometric mean             | (ref)                                                    | 1.03x faster                                                            |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 91.4 ms: 1.03x faster                                                   |
| nbody          | 114 ms                                                   | 117 ms: 1.03x slower                                                    |
| Geometric mean | (ref)                                                    | 1.00x slower                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 31.5 ms                                                  | 30.7 ms: 1.03x faster                                                   |
| regex_dna      | 246 ms                                                   | 258 ms: 1.05x slower                                                    |
| regex_compile  | 128 ms                                                   | 175 ms: 1.36x slower                                                    |
| Geometric mean | (ref)                                                    | 1.09x slower                                                            |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| json_loads           | 31.4 us                                                  | 32.7 us: 1.04x slower                                                   |
| tomli_loads          | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                  |
| unpickle_pure_python | 254 us                                                   | 279 us: 1.10x slower                                                    |
| pickle_pure_python   | 359 us                                                   | 396 us: 1.10x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.03x slower                                                            |

Benchmark hidden because not significant (4): json_dumps, xml_etree_parse, xml_etree_generate, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 8.75 ms                                                  | 8.86 ms: 1.01x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.00x slower                                                            |

Benchmark hidden because not significant (1): python_startup

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 12.9 ms: 1.03x faster                                                   |
| genshi_text     | 27.7 ms                                                  | 33.2 ms: 1.20x slower                                                   |
| django_template | 42.3 ms                                                  | 52.0 ms: 1.23x slower                                                   |
| genshi_xml      | 60.2 ms                                                  | 74.4 ms: 1.24x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.15x slower                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240802-arminc-aarch64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 51.0 us                                                  | 38.8 us: 1.32x faster                                                   |
| async_tree_memoization_tg  | 649 ms                                                   | 537 ms: 1.21x faster                                                    |
| deepcopy                   | 451 us                                                   | 386 us: 1.17x faster                                                    |
| async_tree_none            | 493 ms                                                   | 435 ms: 1.13x faster                                                    |
| async_tree_none_tg         | 467 ms                                                   | 419 ms: 1.12x faster                                                    |
| async_tree_memoization     | 626 ms                                                   | 579 ms: 1.08x faster                                                    |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 708 ms: 1.04x faster                                                    |
| pathlib                    | 22.4 ms                                                  | 21.7 ms: 1.03x faster                                                   |
| mako                       | 13.3 ms                                                  | 12.9 ms: 1.03x faster                                                   |
| float                      | 94.4 ms                                                  | 91.4 ms: 1.03x faster                                                   |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 742 ms: 1.03x faster                                                    |
| scimark_sor                | 159 ms                                                   | 155 ms: 1.03x faster                                                    |
| regex_v8                   | 31.5 ms                                                  | 30.7 ms: 1.03x faster                                                   |
| xml_etree_iterparse        | 147 ms                                                   | 148 ms: 1.01x slower                                                    |
| asyncio_websockets         | 656 ms                                                   | 661 ms: 1.01x slower                                                    |
| python_startup_no_site     | 8.75 ms                                                  | 8.86 ms: 1.01x slower                                                   |
| logging_silent             | 135 ns                                                   | 137 ns: 1.01x slower                                                    |
| async_tree_io_tg           | 1.09 sec                                                 | 1.11 sec: 1.02x slower                                                  |
| deepcopy_reduce            | 4.07 us                                                  | 4.15 us: 1.02x slower                                                   |
| async_generators           | 496 ms                                                   | 509 ms: 1.03x slower                                                    |
| nbody                      | 114 ms                                                   | 117 ms: 1.03x slower                                                    |
| scimark_fft                | 447 ms                                                   | 461 ms: 1.03x slower                                                    |
| spectral_norm              | 141 ms                                                   | 145 ms: 1.03x slower                                                    |
| scimark_sparse_mat_mult    | 6.58 ms                                                  | 6.79 ms: 1.03x slower                                                   |
| thrift                     | 946 us                                                   | 977 us: 1.03x slower                                                    |
| richards                   | 53.5 ms                                                  | 55.3 ms: 1.03x slower                                                   |
| logging_format             | 7.83 us                                                  | 8.09 us: 1.03x slower                                                   |
| bench_thread_pool          | 1.28 ms                                                  | 1.32 ms: 1.04x slower                                                   |
| telco                      | 9.73 ms                                                  | 10.1 ms: 1.04x slower                                                   |
| richards_super             | 60.3 ms                                                  | 62.7 ms: 1.04x slower                                                   |
| mdp                        | 3.36 sec                                                 | 3.50 sec: 1.04x slower                                                  |
| json_loads                 | 31.4 us                                                  | 32.7 us: 1.04x slower                                                   |
| tornado_http               | 131 ms                                                   | 137 ms: 1.04x slower                                                    |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.28 sec: 1.05x slower                                                  |
| json                       | 5.61 ms                                                  | 5.88 ms: 1.05x slower                                                   |
| regex_dna                  | 246 ms                                                   | 258 ms: 1.05x slower                                                    |
| meteor_contest             | 113 ms                                                   | 119 ms: 1.05x slower                                                    |
| tomli_loads                | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                  |
| logging_simple             | 7.04 us                                                  | 7.42 us: 1.05x slower                                                   |
| async_tree_io              | 1.11 sec                                                 | 1.17 sec: 1.06x slower                                                  |
| scimark_monte_carlo        | 83.8 ms                                                  | 89.5 ms: 1.07x slower                                                   |
| scimark_lu                 | 139 ms                                                   | 149 ms: 1.07x slower                                                    |
| gc_traversal               | 4.53 ms                                                  | 4.89 ms: 1.08x slower                                                   |
| fannkuch                   | 452 ms                                                   | 488 ms: 1.08x slower                                                    |
| crypto_pyaes               | 82.7 ms                                                  | 90.0 ms: 1.09x slower                                                   |
| typing_runtime_protocols   | 192 us                                                   | 210 us: 1.09x slower                                                    |
| asyncio_tcp                | 568 ms                                                   | 622 ms: 1.10x slower                                                    |
| unpickle_pure_python       | 254 us                                                   | 279 us: 1.10x slower                                                    |
| html5lib                   | 64.3 ms                                                  | 70.5 ms: 1.10x slower                                                   |
| pyflate                    | 556 ms                                                   | 611 ms: 1.10x slower                                                    |
| pickle_pure_python         | 359 us                                                   | 396 us: 1.10x slower                                                    |
| create_gc_cycles           | 2.12 ms                                                  | 2.34 ms: 1.10x slower                                                   |
| raytrace                   | 298 ms                                                   | 332 ms: 1.11x slower                                                    |
| go                         | 163 ms                                                   | 182 ms: 1.12x slower                                                    |
| dask                       | 350 ms                                                   | 394 ms: 1.12x slower                                                    |
| sqlglot_optimize           | 62.4 ms                                                  | 72.2 ms: 1.16x slower                                                   |
| sqlglot_normalize          | 128 ms                                                   | 150 ms: 1.17x slower                                                    |
| pycparser                  | 1.26 sec                                                 | 1.47 sec: 1.17x slower                                                  |
| deltablue                  | 3.85 ms                                                  | 4.51 ms: 1.17x slower                                                   |
| bench_mp_pool              | 7.30 ms                                                  | 8.57 ms: 1.17x slower                                                   |
| comprehensions             | 20.4 us                                                  | 24.2 us: 1.18x slower                                                   |
| genshi_text                | 27.7 ms                                                  | 33.2 ms: 1.20x slower                                                   |
| sqlglot_parse              | 1.38 ms                                                  | 1.67 ms: 1.21x slower                                                   |
| 2to3                       | 306 ms                                                   | 372 ms: 1.22x slower                                                    |
| nqueens                    | 98.7 ms                                                  | 121 ms: 1.23x slower                                                    |
| django_template            | 42.3 ms                                                  | 52.0 ms: 1.23x slower                                                   |
| sqlglot_transpile          | 1.73 ms                                                  | 2.13 ms: 1.23x slower                                                   |
| genshi_xml                 | 60.2 ms                                                  | 74.4 ms: 1.24x slower                                                   |
| pprint_safe_repr           | 926 ms                                                   | 1.17 sec: 1.26x slower                                                  |
| pprint_pformat             | 1.90 sec                                                 | 2.40 sec: 1.26x slower                                                  |
| pylint                     | 343 ms                                                   | 434 ms: 1.26x slower                                                    |
| chaos                      | 68.8 ms                                                  | 87.7 ms: 1.27x slower                                                   |
| docutils                   | 2.91 sec                                                 | 3.73 sec: 1.28x slower                                                  |
| sympy_expand               | 455 ms                                                   | 593 ms: 1.30x slower                                                    |
| sympy_integrate            | 21.0 ms                                                  | 27.5 ms: 1.31x slower                                                   |
| hexiom                     | 7.13 ms                                                  | 9.36 ms: 1.31x slower                                                   |
| sympy_str                  | 264 ms                                                   | 348 ms: 1.32x slower                                                    |
| regex_compile              | 128 ms                                                   | 175 ms: 1.36x slower                                                    |
| sympy_sum                  | 143 ms                                                   | 202 ms: 1.41x slower                                                    |
| generators                 | 37.8 ms                                                  | 57.0 ms: 1.51x slower                                                   |
| Geometric mean             | (ref)                                                    | 1.08x slower                                                            |

Benchmark hidden because not significant (10): json_dumps, xml_etree_parse, python_startup, xml_etree_generate, xml_etree_process, bpe_tokeniser, coroutines, pidigits, regex_effbot, coverage
Ignored benchmarks (12) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.04x
- 99% likely to have a slowdown of 1.03x

# Memory
- memory change: 1.10x