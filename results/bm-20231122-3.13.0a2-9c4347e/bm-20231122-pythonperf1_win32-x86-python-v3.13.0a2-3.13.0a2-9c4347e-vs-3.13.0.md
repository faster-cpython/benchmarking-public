# Results vs. 3.13.0

- fork: python
- ref: v3.13.0a2
- machine: windows-x86
- commit hash: 9c4347e
- commit date: 2023-11-22
- overall geometric mean: 1.00x slower
- HPT reliability: 95.99%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| 2to3           | 253 ms                                                          | 246 ms: 1.03x faster                                                |
| chameleon      | 6.14 ms                                                         | 5.96 ms: 1.03x faster                                               |
| docutils       | 1.82 sec                                                        | 1.78 sec: 1.02x faster                                              |
| html5lib       | 48.3 ms                                                         | 45.7 ms: 1.06x faster                                               |
| tornado_http   | 104 ms                                                          | 98.2 ms: 1.06x faster                                               |
| Geometric mean | (ref)                                                           | 1.04x faster                                                        |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| asyncio_tcp                | 842 ms                                                          | 654 ms: 1.29x faster                                                |
| asyncio_tcp_ssl            | 17.4 sec                                                        | 17.2 sec: 1.01x faster                                              |
| async_generators           | 274 ms                                                          | 278 ms: 1.01x slower                                                |
| async_tree_none            | 246 ms                                                          | 254 ms: 1.03x slower                                                |
| async_tree_cpu_io_mixed    | 494 ms                                                          | 511 ms: 1.03x slower                                                |
| async_tree_memoization     | 302 ms                                                          | 317 ms: 1.05x slower                                                |
| coroutines                 | 15.7 ms                                                         | 16.7 ms: 1.07x slower                                               |
| async_tree_memoization_tg  | 287 ms                                                          | 312 ms: 1.09x slower                                                |
| async_tree_cpu_io_mixed_tg | 464 ms                                                          | 506 ms: 1.09x slower                                                |
| async_tree_none_tg         | 219 ms                                                          | 248 ms: 1.14x slower                                                |
| async_tree_io              | 537 ms                                                          | 623 ms: 1.16x slower                                                |
| async_tree_io_tg           | 509 ms                                                          | 619 ms: 1.21x slower                                                |
| Geometric mean             | (ref)                                                           | 1.05x slower                                                        |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| pidigits       | 202 ms                                                          | 200 ms: 1.01x faster                                                |
| float          | 57.8 ms                                                         | 59.8 ms: 1.03x slower                                               |
| nbody          | 81.9 ms                                                         | 88.9 ms: 1.09x slower                                               |
| Geometric mean | (ref)                                                           | 1.04x slower                                                        |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                          | 106 ms: 1.02x slower                                                |
| regex_dna      | 117 ms                                                          | 120 ms: 1.03x slower                                                |
| regex_v8       | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                               |
| regex_effbot   | 1.81 ms                                                         | 1.92 ms: 1.06x slower                                               |
| Geometric mean | (ref)                                                           | 1.04x slower                                                        |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| pickle               | 8.42 us                                                         | 7.57 us: 1.11x faster                                               |
| unpickle             | 10.5 us                                                         | 9.58 us: 1.10x faster                                               |
| pickle_dict          | 21.7 us                                                         | 19.8 us: 1.09x faster                                               |
| pickle_list          | 3.40 us                                                         | 3.17 us: 1.07x faster                                               |
| json_dumps           | 7.40 ms                                                         | 6.96 ms: 1.06x faster                                               |
| json_loads           | 21.0 us                                                         | 19.8 us: 1.06x faster                                               |
| unpickle_list        | 3.09 us                                                         | 2.94 us: 1.05x faster                                               |
| unpickle_pure_python | 162 us                                                          | 156 us: 1.04x faster                                                |
| pickle_pure_python   | 238 us                                                          | 231 us: 1.03x faster                                                |
| xml_etree_parse      | 109 ms                                                          | 108 ms: 1.01x faster                                                |
| xml_etree_process    | 45.0 ms                                                         | 45.8 ms: 1.02x slower                                               |
| tomli_loads          | 1.73 sec                                                        | 1.81 sec: 1.04x slower                                              |
| xml_etree_iterparse  | 65.1 ms                                                         | 69.5 ms: 1.07x slower                                               |
| Geometric mean       | (ref)                                                           | 1.03x faster                                                        |

Benchmark hidden because not significant (1): xml_etree_generate

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| python_startup | 24.3 ms                                                         | 22.2 ms: 1.09x faster                                               |
| Geometric mean | (ref)                                                           | 1.04x faster                                                        |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|-----------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| django_template | 32.7 ms                                                         | 31.2 ms: 1.05x faster                                               |
| genshi_text     | 22.4 ms                                                         | 21.7 ms: 1.03x faster                                               |
| genshi_xml      | 49.5 ms                                                         | 48.4 ms: 1.02x faster                                               |
| mako            | 7.31 ms                                                         | 7.72 ms: 1.06x slower                                               |
| Geometric mean  | (ref)                                                           | 1.01x faster                                                        |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20231122-pythonperf1_win32-x86-python-v3.13.0a2-3.13.0a2-9c4347e |
|----------------------------|:---------------------------------------------------------------:|:-------------------------------------------------------------------:|
| typing_runtime_protocols   | 136 us                                                          | 98.7 us: 1.38x faster                                               |
| asyncio_tcp                | 842 ms                                                          | 654 ms: 1.29x faster                                                |
| pickle                     | 8.42 us                                                         | 7.57 us: 1.11x faster                                               |
| create_gc_cycles           | 713 us                                                          | 645 us: 1.11x faster                                                |
| sqlglot_parse              | 1.05 ms                                                         | 953 us: 1.10x faster                                                |
| unpickle                   | 10.5 us                                                         | 9.58 us: 1.10x faster                                               |
| telco                      | 6.67 ms                                                         | 6.08 ms: 1.10x faster                                               |
| pickle_dict                | 21.7 us                                                         | 19.8 us: 1.09x faster                                               |
| python_startup             | 24.3 ms                                                         | 22.2 ms: 1.09x faster                                               |
| deepcopy_reduce            | 2.85 us                                                         | 2.63 us: 1.08x faster                                               |
| bench_mp_pool              | 74.3 ms                                                         | 69.3 ms: 1.07x faster                                               |
| sqlglot_transpile          | 1.29 ms                                                         | 1.21 ms: 1.07x faster                                               |
| pickle_list                | 3.40 us                                                         | 3.17 us: 1.07x faster                                               |
| sqlite_synth               | 1.97 us                                                         | 1.84 us: 1.07x faster                                               |
| json_dumps                 | 7.40 ms                                                         | 6.96 ms: 1.06x faster                                               |
| tornado_http               | 104 ms                                                          | 98.2 ms: 1.06x faster                                               |
| json_loads                 | 21.0 us                                                         | 19.8 us: 1.06x faster                                               |
| html5lib                   | 48.3 ms                                                         | 45.7 ms: 1.06x faster                                               |
| unpickle_list              | 3.09 us                                                         | 2.94 us: 1.05x faster                                               |
| crypto_pyaes               | 58.2 ms                                                         | 55.4 ms: 1.05x faster                                               |
| django_template            | 32.7 ms                                                         | 31.2 ms: 1.05x faster                                               |
| gc_traversal               | 1.45 ms                                                         | 1.39 ms: 1.04x faster                                               |
| sympy_sum                  | 108 ms                                                          | 104 ms: 1.04x faster                                                |
| sqlglot_normalize          | 220 ms                                                          | 212 ms: 1.04x faster                                                |
| deepcopy                   | 307 us                                                          | 295 us: 1.04x faster                                                |
| sympy_str                  | 215 ms                                                          | 207 ms: 1.04x faster                                                |
| sympy_expand               | 375 ms                                                          | 362 ms: 1.04x faster                                                |
| pprint_safe_repr           | 644 ms                                                          | 621 ms: 1.04x faster                                                |
| unpickle_pure_python       | 162 us                                                          | 156 us: 1.04x faster                                                |
| go                         | 111 ms                                                          | 108 ms: 1.03x faster                                                |
| genshi_text                | 22.4 ms                                                         | 21.7 ms: 1.03x faster                                               |
| pickle_pure_python         | 238 us                                                          | 231 us: 1.03x faster                                                |
| chameleon                  | 6.14 ms                                                         | 5.96 ms: 1.03x faster                                               |
| json                       | 4.27 ms                                                         | 4.14 ms: 1.03x faster                                               |
| 2to3                       | 253 ms                                                          | 246 ms: 1.03x faster                                                |
| genshi_xml                 | 49.5 ms                                                         | 48.4 ms: 1.02x faster                                               |
| sqlglot_optimize           | 42.5 ms                                                         | 41.5 ms: 1.02x faster                                               |
| docutils                   | 1.82 sec                                                        | 1.78 sec: 1.02x faster                                              |
| sympy_integrate            | 15.2 ms                                                         | 14.9 ms: 1.02x faster                                               |
| pprint_pformat             | 1.30 sec                                                        | 1.28 sec: 1.02x faster                                              |
| comprehensions             | 12.7 us                                                         | 12.6 us: 1.01x faster                                               |
| pidigits                   | 202 ms                                                          | 200 ms: 1.01x faster                                                |
| xml_etree_parse            | 109 ms                                                          | 108 ms: 1.01x faster                                                |
| pycparser                  | 869 ms                                                          | 862 ms: 1.01x faster                                                |
| asyncio_tcp_ssl            | 17.4 sec                                                        | 17.2 sec: 1.01x faster                                              |
| flaskblogging              | 2.04 sec                                                        | 2.03 sec: 1.00x faster                                              |
| chaos                      | 51.2 ms                                                         | 51.8 ms: 1.01x slower                                               |
| async_generators           | 274 ms                                                          | 278 ms: 1.01x slower                                                |
| deepcopy_memo              | 26.2 us                                                         | 26.6 us: 1.02x slower                                               |
| mdp                        | 1.67 sec                                                        | 1.70 sec: 1.02x slower                                              |
| xml_etree_process          | 45.0 ms                                                         | 45.8 ms: 1.02x slower                                               |
| regex_compile              | 103 ms                                                          | 106 ms: 1.02x slower                                                |
| bench_thread_pool          | 1.02 ms                                                         | 1.05 ms: 1.02x slower                                               |
| pyflate                    | 326 ms                                                          | 335 ms: 1.03x slower                                                |
| fannkuch                   | 293 ms                                                          | 302 ms: 1.03x slower                                                |
| regex_dna                  | 117 ms                                                          | 120 ms: 1.03x slower                                                |
| async_tree_none            | 246 ms                                                          | 254 ms: 1.03x slower                                                |
| async_tree_cpu_io_mixed    | 494 ms                                                          | 511 ms: 1.03x slower                                                |
| float                      | 57.8 ms                                                         | 59.8 ms: 1.03x slower                                               |
| logging_format             | 8.57 us                                                         | 8.90 us: 1.04x slower                                               |
| regex_v8                   | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                               |
| meteor_contest             | 77.0 ms                                                         | 80.4 ms: 1.04x slower                                               |
| tomli_loads                | 1.73 sec                                                        | 1.81 sec: 1.04x slower                                              |
| async_tree_memoization     | 302 ms                                                          | 317 ms: 1.05x slower                                                |
| spectral_norm              | 71.3 ms                                                         | 75.0 ms: 1.05x slower                                               |
| thrift                     | 10.1 ms                                                         | 10.7 ms: 1.05x slower                                               |
| scimark_sparse_mat_mult    | 2.90 ms                                                         | 3.07 ms: 1.06x slower                                               |
| mako                       | 7.31 ms                                                         | 7.72 ms: 1.06x slower                                               |
| regex_effbot               | 1.81 ms                                                         | 1.92 ms: 1.06x slower                                               |
| deltablue                  | 2.41 ms                                                         | 2.55 ms: 1.06x slower                                               |
| logging_simple             | 7.87 us                                                         | 8.35 us: 1.06x slower                                               |
| scimark_sor                | 91.8 ms                                                         | 97.7 ms: 1.06x slower                                               |
| scimark_fft                | 206 ms                                                          | 220 ms: 1.06x slower                                                |
| nqueens                    | 75.1 ms                                                         | 80.1 ms: 1.07x slower                                               |
| coroutines                 | 15.7 ms                                                         | 16.7 ms: 1.07x slower                                               |
| xml_etree_iterparse        | 65.1 ms                                                         | 69.5 ms: 1.07x slower                                               |
| hexiom                     | 4.64 ms                                                         | 4.97 ms: 1.07x slower                                               |
| raytrace                   | 205 ms                                                          | 223 ms: 1.08x slower                                                |
| scimark_lu                 | 63.5 ms                                                         | 69.0 ms: 1.09x slower                                               |
| nbody                      | 81.9 ms                                                         | 88.9 ms: 1.09x slower                                               |
| async_tree_memoization_tg  | 287 ms                                                          | 312 ms: 1.09x slower                                                |
| async_tree_cpu_io_mixed_tg | 464 ms                                                          | 506 ms: 1.09x slower                                                |
| async_tree_none_tg         | 219 ms                                                          | 248 ms: 1.14x slower                                                |
| logging_silent             | 61.6 ns                                                         | 70.3 ns: 1.14x slower                                               |
| async_tree_io              | 537 ms                                                          | 623 ms: 1.16x slower                                                |
| async_tree_io_tg           | 509 ms                                                          | 619 ms: 1.21x slower                                                |
| generators                 | 22.1 ms                                                         | 27.0 ms: 1.22x slower                                               |
| coverage                   | 333 ms                                                          | 512 ms: 1.54x slower                                                |
| Geometric mean             | (ref)                                                           | 1.00x slower                                                        |

Benchmark hidden because not significant (7): richards, richards_super, python_startup_no_site, pathlib, xml_etree_generate, scimark_monte_carlo, pylint
Ignored benchmarks (2) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5.json: dulwich_log, unpack_sequence

# HPT report

- Reliability score: 95.99% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown