
# Results vs. 3.10.4

- fork: python
- ref: 70be5e42f6e288de32e0
- machine: darwin-arm64
- commit hash: 70be5e4
- commit date: 2022-12-11
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.16x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 161 ms: 1.26x faster                                                   |
| chameleon      | 5.84 ms                                                | 4.45 ms: 1.31x faster                                                  |
| docutils       | 1.78 sec                                               | 1.46 sec: 1.22x faster                                                 |
| html5lib       | 44.1 ms                                                | 34.6 ms: 1.28x faster                                                  |
| Geometric mean | (ref)                                                  | 1.27x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 64.0 ms: 1.47x faster                                                  |
| float          | 72.3 ms                                                | 52.5 ms: 1.38x faster                                                  |
| pidigits       | 282 ms                                                 | 281 ms: 1.00x faster                                                   |
| Geometric mean | (ref)                                                  | 1.27x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 73.9 ms: 1.31x faster                                                  |
| regex_v8       | 17.5 ms                                                | 15.7 ms: 1.12x faster                                                  |
| regex_dna      | 160 ms                                                 | 150 ms: 1.07x faster                                                   |
| regex_effbot   | 2.45 ms                                                | 2.62 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.10x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 198 us: 1.43x faster                                                   |
| json_dumps           | 8.38 ms                                                | 6.21 ms: 1.35x faster                                                  |
| unpickle_pure_python | 203 us                                                 | 156 us: 1.31x faster                                                   |
| xml_etree_process    | 45.1 ms                                                | 34.7 ms: 1.30x faster                                                  |
| xml_etree_generate   | 54.3 ms                                                | 47.6 ms: 1.14x faster                                                  |
| xml_etree_parse      | 107 ms                                                 | 96.3 ms: 1.11x faster                                                  |
| xml_etree_iterparse  | 72.6 ms                                                | 69.2 ms: 1.05x faster                                                  |
| json_loads           | 16.9 us                                                | 16.3 us: 1.04x faster                                                  |
| unpickle_list        | 2.66 us                                                | 2.58 us: 1.03x faster                                                  |
| unpickle             | 9.77 us                                                | 9.67 us: 1.01x faster                                                  |
| pickle_list          | 2.83 us                                                | 2.82 us: 1.00x faster                                                  |
| pickle_dict          | 17.8 us                                                | 17.9 us: 1.01x slower                                                  |
| pickle               | 7.36 us                                                | 7.52 us: 1.02x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.12x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 12.2 ms: 1.03x faster                                                  |
| python_startup_no_site | 9.73 ms                                                | 10.3 ms: 1.06x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.01x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| django_template | 27.3 ms                                                | 20.8 ms: 1.31x faster                                                  |
| mako            | 10.5 ms                                                | 7.99 ms: 1.31x faster                                                  |
| genshi_xml      | 37.6 ms                                                | 28.7 ms: 1.31x faster                                                  |
| genshi_text     | 18.4 ms                                                | 15.0 ms: 1.22x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.29x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221211-darwin-arm64-python-70be5e42f6e288de32e0-3.12.0a3+-70be5e4 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.55 ms: 2.02x faster                                                  |
| logging_silent          | 119 ns                                                 | 63.3 ns: 1.88x faster                                                  |
| richards                | 51.4 ms                                                | 29.3 ms: 1.76x faster                                                  |
| scimark_sor             | 127 ms                                                 | 78.2 ms: 1.62x faster                                                  |
| raytrace                | 328 ms                                                 | 204 ms: 1.61x faster                                                   |
| scimark_lu              | 110 ms                                                 | 71.3 ms: 1.54x faster                                                  |
| scimark_monte_carlo     | 72.2 ms                                                | 47.2 ms: 1.53x faster                                                  |
| go                      | 165 ms                                                 | 108 ms: 1.52x faster                                                   |
| async_tree_memoization  | 493 ms                                                 | 331 ms: 1.49x faster                                                   |
| crypto_pyaes            | 78.0 ms                                                | 52.7 ms: 1.48x faster                                                  |
| nbody                   | 94.1 ms                                                | 64.0 ms: 1.47x faster                                                  |
| pickle_pure_python      | 284 us                                                 | 198 us: 1.43x faster                                                   |
| sqlglot_parse           | 1.33 ms                                                | 951 us: 1.40x faster                                                   |
| pyflate                 | 453 ms                                                 | 327 ms: 1.39x faster                                                   |
| async_tree_none         | 402 ms                                                 | 291 ms: 1.38x faster                                                   |
| float                   | 72.3 ms                                                | 52.5 ms: 1.38x faster                                                  |
| sqlglot_transpile       | 1.54 ms                                                | 1.12 ms: 1.38x faster                                                  |
| async_tree_io           | 1.02 sec                                               | 747 ms: 1.37x faster                                                   |
| pycparser               | 915 ms                                                 | 672 ms: 1.36x faster                                                   |
| thrift                  | 586 us                                                 | 433 us: 1.36x faster                                                   |
| hexiom                  | 6.32 ms                                                | 4.68 ms: 1.35x faster                                                  |
| json_dumps              | 8.38 ms                                                | 6.21 ms: 1.35x faster                                                  |
| chaos                   | 66.8 ms                                                | 49.6 ms: 1.35x faster                                                  |
| logging_simple          | 4.63 us                                                | 3.44 us: 1.34x faster                                                  |
| logging_format          | 5.01 us                                                | 3.74 us: 1.34x faster                                                  |
| spectral_norm           | 96.4 ms                                                | 72.6 ms: 1.33x faster                                                  |
| chameleon               | 5.84 ms                                                | 4.45 ms: 1.31x faster                                                  |
| django_template         | 27.3 ms                                                | 20.8 ms: 1.31x faster                                                  |
| mako                    | 10.5 ms                                                | 7.99 ms: 1.31x faster                                                  |
| genshi_xml              | 37.6 ms                                                | 28.7 ms: 1.31x faster                                                  |
| regex_compile           | 96.6 ms                                                | 73.9 ms: 1.31x faster                                                  |
| unpickle_pure_python    | 203 us                                                 | 156 us: 1.31x faster                                                   |
| xml_etree_process       | 45.1 ms                                                | 34.7 ms: 1.30x faster                                                  |
| html5lib                | 44.1 ms                                                | 34.6 ms: 1.28x faster                                                  |
| pprint_pformat          | 1.24 sec                                               | 978 ms: 1.27x faster                                                   |
| create_gc_cycles        | 876 us                                                 | 692 us: 1.27x faster                                                   |
| pprint_safe_repr        | 609 ms                                                 | 480 ms: 1.27x faster                                                   |
| 2to3                    | 202 ms                                                 | 161 ms: 1.26x faster                                                   |
| dulwich_log             | 37.1 ms                                                | 29.7 ms: 1.25x faster                                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.79 ms: 1.24x faster                                                  |
| deepcopy_memo           | 34.5 us                                                | 28.0 us: 1.23x faster                                                  |
| fannkuch                | 317 ms                                                 | 259 ms: 1.23x faster                                                   |
| async_tree_cpu_io_mixed | 670 ms                                                 | 547 ms: 1.23x faster                                                   |
| genshi_text             | 18.4 ms                                                | 15.0 ms: 1.22x faster                                                  |
| docutils                | 1.78 sec                                               | 1.46 sec: 1.22x faster                                                 |
| sqlglot_optimize        | 38.0 ms                                                | 31.6 ms: 1.20x faster                                                  |
| deepcopy_reduce         | 2.38 us                                                | 1.99 us: 1.20x faster                                                  |
| bench_thread_pool       | 548 us                                                 | 458 us: 1.20x faster                                                   |
| deepcopy                | 278 us                                                 | 232 us: 1.20x faster                                                   |
| scimark_fft             | 232 ms                                                 | 194 ms: 1.19x faster                                                   |
| unpack_sequence         | 38.7 ns                                                | 32.8 ns: 1.18x faster                                                  |
| mypy2                   | 308 ms                                                 | 262 ms: 1.18x faster                                                   |
| sympy_integrate         | 13.4 ms                                                | 11.5 ms: 1.16x faster                                                  |
| async_generators        | 233 ms                                                 | 201 ms: 1.16x faster                                                   |
| dask                    | 258 ms                                                 | 225 ms: 1.15x faster                                                   |
| sqlglot_normalize       | 197 ms                                                 | 172 ms: 1.14x faster                                                   |
| xml_etree_generate      | 54.3 ms                                                | 47.6 ms: 1.14x faster                                                  |
| sympy_expand            | 276 ms                                                 | 244 ms: 1.13x faster                                                   |
| nqueens                 | 68.1 ms                                                | 60.4 ms: 1.13x faster                                                  |
| regex_v8                | 17.5 ms                                                | 15.7 ms: 1.12x faster                                                  |
| xml_etree_parse         | 107 ms                                                 | 96.3 ms: 1.11x faster                                                  |
| sympy_str               | 169 ms                                                 | 152 ms: 1.11x faster                                                   |
| json                    | 3.10 ms                                                | 2.80 ms: 1.11x faster                                                  |
| mdp                     | 1.67 sec                                               | 1.51 sec: 1.10x faster                                                 |
| coroutines              | 20.2 ms                                                | 18.4 ms: 1.10x faster                                                  |
| sympy_sum               | 94.3 ms                                                | 86.6 ms: 1.09x faster                                                  |
| telco                   | 3.68 ms                                                | 3.42 ms: 1.08x faster                                                  |
| regex_dna               | 160 ms                                                 | 150 ms: 1.07x faster                                                   |
| comprehensions          | 17.7 us                                                | 16.6 us: 1.06x faster                                                  |
| xml_etree_iterparse     | 72.6 ms                                                | 69.2 ms: 1.05x faster                                                  |
| pathlib                 | 28.8 ms                                                | 27.5 ms: 1.05x faster                                                  |
| json_loads              | 16.9 us                                                | 16.3 us: 1.04x faster                                                  |
| unpickle_list           | 2.66 us                                                | 2.58 us: 1.03x faster                                                  |
| python_startup          | 12.6 ms                                                | 12.2 ms: 1.03x faster                                                  |
| meteor_contest          | 78.6 ms                                                | 76.4 ms: 1.03x faster                                                  |
| sqlite_synth            | 1.47 us                                                | 1.44 us: 1.03x faster                                                  |
| asyncio_tcp             | 673 ms                                                 | 665 ms: 1.01x faster                                                   |
| unpickle                | 9.77 us                                                | 9.67 us: 1.01x faster                                                  |
| generators              | 32.9 ms                                                | 32.7 ms: 1.01x faster                                                  |
| pidigits                | 282 ms                                                 | 281 ms: 1.00x faster                                                   |
| pickle_list             | 2.83 us                                                | 2.82 us: 1.00x faster                                                  |
| gc_traversal            | 2.40 ms                                                | 2.41 ms: 1.01x slower                                                  |
| pickle_dict             | 17.8 us                                                | 17.9 us: 1.01x slower                                                  |
| pickle                  | 7.36 us                                                | 7.52 us: 1.02x slower                                                  |
| python_startup_no_site  | 9.73 ms                                                | 10.3 ms: 1.06x slower                                                  |
| regex_effbot            | 2.45 ms                                                | 2.62 ms: 1.07x slower                                                  |
| bench_mp_pool           | 41.0 ms                                                | 44.2 ms: 1.08x slower                                                  |
| coverage                | 40.8 ms                                                | 58.6 ms: 1.43x slower                                                  |
| Geometric mean          | (ref)                                                  | 1.22x faster                                                           |
Ignored benchmarks (11) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, tornado_http, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.20x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.16x
