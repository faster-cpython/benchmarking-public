# Results vs. 3.13.0

- fork: faster-cpython
- ref: bound_method_instrum
- machine: windows-x86
- commit hash: 5787d3e
- commit date: 2024-07-23
- overall geometric mean: 1.04x faster
- HPT reliability: 99.97%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|----------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| docutils       | 1.82 sec                                                        | 1.90 sec: 1.05x slower                                                                   |
| Geometric mean | (ref)                                                           | 1.01x slower                                                                             |

Benchmark hidden because not significant (3): 2to3, html5lib, tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|---------------------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| asyncio_tcp               | 842 ms                                                          | 719 ms: 1.17x faster                                                                     |
| async_tree_memoization_tg | 287 ms                                                          | 246 ms: 1.17x faster                                                                     |
| async_tree_none_tg        | 219 ms                                                          | 190 ms: 1.15x faster                                                                     |
| async_tree_memoization    | 302 ms                                                          | 274 ms: 1.10x faster                                                                     |
| async_tree_none           | 246 ms                                                          | 223 ms: 1.10x faster                                                                     |
| async_tree_cpu_io_mixed   | 494 ms                                                          | 472 ms: 1.05x faster                                                                     |
| asyncio_tcp_ssl           | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                                   |
| async_tree_io_tg          | 509 ms                                                          | 501 ms: 1.02x faster                                                                     |
| async_tree_io             | 537 ms                                                          | 547 ms: 1.02x slower                                                                     |
| async_generators          | 274 ms                                                          | 284 ms: 1.04x slower                                                                     |
| coroutines                | 15.7 ms                                                         | 17.1 ms: 1.09x slower                                                                    |
| Geometric mean            | (ref)                                                           | 1.05x faster                                                                             |

Benchmark hidden because not significant (1): async_tree_cpu_io_mixed_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|----------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| pidigits       | 202 ms                                                          | 201 ms: 1.00x faster                                                                     |
| float          | 57.8 ms                                                         | 58.9 ms: 1.02x slower                                                                    |
| nbody          | 81.9 ms                                                         | 90.3 ms: 1.10x slower                                                                    |
| Geometric mean | (ref)                                                           | 1.04x slower                                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|----------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                          | 105 ms: 1.02x slower                                                                     |
| regex_v8       | 15.6 ms                                                         | 16.1 ms: 1.03x slower                                                                    |
| regex_effbot   | 1.81 ms                                                         | 1.98 ms: 1.09x slower                                                                    |
| regex_dna      | 117 ms                                                          | 128 ms: 1.09x slower                                                                     |
| Geometric mean | (ref)                                                           | 1.06x slower                                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|----------------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| json_dumps           | 7.40 ms                                                         | 7.12 ms: 1.04x faster                                                                    |
| json_loads           | 21.0 us                                                         | 20.3 us: 1.03x faster                                                                    |
| xml_etree_parse      | 109 ms                                                          | 105 ms: 1.03x faster                                                                     |
| pickle_pure_python   | 238 us                                                          | 242 us: 1.02x slower                                                                     |
| unpickle_pure_python | 162 us                                                          | 166 us: 1.03x slower                                                                     |
| xml_etree_generate   | 62.6 ms                                                         | 64.4 ms: 1.03x slower                                                                    |
| xml_etree_iterparse  | 65.1 ms                                                         | 67.6 ms: 1.04x slower                                                                    |
| xml_etree_process    | 45.0 ms                                                         | 47.2 ms: 1.05x slower                                                                    |
| tomli_loads          | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                                   |
| Geometric mean       | (ref)                                                           | 1.01x slower                                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|------------------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| python_startup         | 24.3 ms                                                         | 23.6 ms: 1.03x faster                                                                    |
| python_startup_no_site | 19.9 ms                                                         | 19.7 ms: 1.01x faster                                                                    |
| Geometric mean         | (ref)                                                           | 1.02x faster                                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|-----------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| genshi_xml      | 49.5 ms                                                         | 48.2 ms: 1.03x faster                                                                    |
| genshi_text     | 22.4 ms                                                         | 22.2 ms: 1.01x faster                                                                    |
| django_template | 32.7 ms                                                         | 35.0 ms: 1.07x slower                                                                    |
| mako            | 7.31 ms                                                         | 7.96 ms: 1.09x slower                                                                    |
| Geometric mean  | (ref)                                                           | 1.03x slower                                                                             |

All benchmarks:
===============

| Benchmark                 | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240723-pythonperf1_win32-x86-faster%2dcpython-bound_method_instrum-3.14.0a0-5787d3e |
|---------------------------|:---------------------------------------------------------------:|:----------------------------------------------------------------------------------------:|
| thrift                    | 10.1 ms                                                         | 758 us: 13.39x faster                                                                    |
| coverage                  | 333 ms                                                          | 51.5 ms: 6.46x faster                                                                    |
| deepcopy                  | 307 us                                                          | 249 us: 1.23x faster                                                                     |
| deepcopy_memo             | 26.2 us                                                         | 21.3 us: 1.23x faster                                                                    |
| asyncio_tcp               | 842 ms                                                          | 719 ms: 1.17x faster                                                                     |
| async_tree_memoization_tg | 287 ms                                                          | 246 ms: 1.17x faster                                                                     |
| async_tree_none_tg        | 219 ms                                                          | 190 ms: 1.15x faster                                                                     |
| deepcopy_reduce           | 2.85 us                                                         | 2.53 us: 1.13x faster                                                                    |
| async_tree_memoization    | 302 ms                                                          | 274 ms: 1.10x faster                                                                     |
| async_tree_none           | 246 ms                                                          | 223 ms: 1.10x faster                                                                     |
| async_tree_cpu_io_mixed   | 494 ms                                                          | 472 ms: 1.05x faster                                                                     |
| telco                     | 6.67 ms                                                         | 6.37 ms: 1.05x faster                                                                    |
| json_dumps                | 7.40 ms                                                         | 7.12 ms: 1.04x faster                                                                    |
| json_loads                | 21.0 us                                                         | 20.3 us: 1.03x faster                                                                    |
| bench_mp_pool             | 74.3 ms                                                         | 72.0 ms: 1.03x faster                                                                    |
| xml_etree_parse           | 109 ms                                                          | 105 ms: 1.03x faster                                                                     |
| genshi_xml                | 49.5 ms                                                         | 48.2 ms: 1.03x faster                                                                    |
| python_startup            | 24.3 ms                                                         | 23.6 ms: 1.03x faster                                                                    |
| asyncio_tcp_ssl           | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                                   |
| async_tree_io_tg          | 509 ms                                                          | 501 ms: 1.02x faster                                                                     |
| python_startup_no_site    | 19.9 ms                                                         | 19.7 ms: 1.01x faster                                                                    |
| gc_traversal              | 1.45 ms                                                         | 1.43 ms: 1.01x faster                                                                    |
| pathlib                   | 89.4 ms                                                         | 88.6 ms: 1.01x faster                                                                    |
| genshi_text               | 22.4 ms                                                         | 22.2 ms: 1.01x faster                                                                    |
| pidigits                  | 202 ms                                                          | 201 ms: 1.00x faster                                                                     |
| pprint_safe_repr          | 644 ms                                                          | 648 ms: 1.01x slower                                                                     |
| meteor_contest            | 77.0 ms                                                         | 77.5 ms: 1.01x slower                                                                    |
| mdp                       | 1.67 sec                                                        | 1.69 sec: 1.01x slower                                                                   |
| sympy_sum                 | 108 ms                                                          | 109 ms: 1.01x slower                                                                     |
| crypto_pyaes              | 58.2 ms                                                         | 58.9 ms: 1.01x slower                                                                    |
| logging_simple            | 7.87 us                                                         | 7.99 us: 1.01x slower                                                                    |
| logging_format            | 8.57 us                                                         | 8.72 us: 1.02x slower                                                                    |
| pickle_pure_python        | 238 us                                                          | 242 us: 1.02x slower                                                                     |
| regex_compile             | 103 ms                                                          | 105 ms: 1.02x slower                                                                     |
| float                     | 57.8 ms                                                         | 58.9 ms: 1.02x slower                                                                    |
| async_tree_io             | 537 ms                                                          | 547 ms: 1.02x slower                                                                     |
| scimark_sor               | 91.8 ms                                                         | 93.9 ms: 1.02x slower                                                                    |
| pprint_pformat            | 1.30 sec                                                        | 1.33 sec: 1.02x slower                                                                   |
| sympy_integrate           | 15.2 ms                                                         | 15.6 ms: 1.03x slower                                                                    |
| go                        | 111 ms                                                          | 114 ms: 1.03x slower                                                                     |
| unpickle_pure_python      | 162 us                                                          | 166 us: 1.03x slower                                                                     |
| sympy_str                 | 215 ms                                                          | 221 ms: 1.03x slower                                                                     |
| xml_etree_generate        | 62.6 ms                                                         | 64.4 ms: 1.03x slower                                                                    |
| sqlglot_parse             | 1.05 ms                                                         | 1.08 ms: 1.03x slower                                                                    |
| pyflate                   | 326 ms                                                          | 337 ms: 1.03x slower                                                                     |
| fannkuch                  | 293 ms                                                          | 303 ms: 1.03x slower                                                                     |
| regex_v8                  | 15.6 ms                                                         | 16.1 ms: 1.03x slower                                                                    |
| sqlglot_transpile         | 1.29 ms                                                         | 1.34 ms: 1.04x slower                                                                    |
| async_generators          | 274 ms                                                          | 284 ms: 1.04x slower                                                                     |
| xml_etree_iterparse       | 65.1 ms                                                         | 67.6 ms: 1.04x slower                                                                    |
| create_gc_cycles          | 713 us                                                          | 741 us: 1.04x slower                                                                     |
| sympy_expand              | 375 ms                                                          | 392 ms: 1.04x slower                                                                     |
| docutils                  | 1.82 sec                                                        | 1.90 sec: 1.05x slower                                                                   |
| pylint                    | 225 ms                                                          | 235 ms: 1.05x slower                                                                     |
| xml_etree_process         | 45.0 ms                                                         | 47.2 ms: 1.05x slower                                                                    |
| scimark_monte_carlo       | 50.3 ms                                                         | 52.7 ms: 1.05x slower                                                                    |
| chaos                     | 51.2 ms                                                         | 53.7 ms: 1.05x slower                                                                    |
| spectral_norm             | 71.3 ms                                                         | 75.4 ms: 1.06x slower                                                                    |
| tomli_loads               | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                                   |
| sqlglot_normalize         | 220 ms                                                          | 235 ms: 1.07x slower                                                                     |
| django_template           | 32.7 ms                                                         | 35.0 ms: 1.07x slower                                                                    |
| deltablue                 | 2.41 ms                                                         | 2.58 ms: 1.07x slower                                                                    |
| sqlglot_optimize          | 42.5 ms                                                         | 45.7 ms: 1.08x slower                                                                    |
| comprehensions            | 12.7 us                                                         | 13.7 us: 1.08x slower                                                                    |
| raytrace                  | 205 ms                                                          | 221 ms: 1.08x slower                                                                     |
| nqueens                   | 75.1 ms                                                         | 80.9 ms: 1.08x slower                                                                    |
| scimark_fft               | 206 ms                                                          | 223 ms: 1.08x slower                                                                     |
| coroutines                | 15.7 ms                                                         | 17.1 ms: 1.09x slower                                                                    |
| mako                      | 7.31 ms                                                         | 7.96 ms: 1.09x slower                                                                    |
| hexiom                    | 4.64 ms                                                         | 5.06 ms: 1.09x slower                                                                    |
| typing_runtime_protocols  | 136 us                                                          | 149 us: 1.09x slower                                                                     |
| scimark_lu                | 63.5 ms                                                         | 69.4 ms: 1.09x slower                                                                    |
| regex_effbot              | 1.81 ms                                                         | 1.98 ms: 1.09x slower                                                                    |
| regex_dna                 | 117 ms                                                          | 128 ms: 1.09x slower                                                                     |
| richards_super            | 38.0 ms                                                         | 41.8 ms: 1.10x slower                                                                    |
| nbody                     | 81.9 ms                                                         | 90.3 ms: 1.10x slower                                                                    |
| richards                  | 33.8 ms                                                         | 37.3 ms: 1.10x slower                                                                    |
| scimark_sparse_mat_mult   | 2.90 ms                                                         | 3.29 ms: 1.13x slower                                                                    |
| logging_silent            | 61.6 ns                                                         | 72.7 ns: 1.18x slower                                                                    |
| generators                | 22.1 ms                                                         | 26.3 ms: 1.19x slower                                                                    |
| Geometric mean            | (ref)                                                           | 1.04x faster                                                                             |

Benchmark hidden because not significant (7): bench_thread_pool, pycparser, async_tree_cpu_io_mixed_tg, html5lib, json, tornado_http, 2to3
Ignored benchmarks (10) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5.json: chameleon, dulwich_log, flaskblogging, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 99.97% likely to be slow
- 90% likely to have a slowdown of 1.01x
- 95% likely to have a slowdown of 1.01x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown