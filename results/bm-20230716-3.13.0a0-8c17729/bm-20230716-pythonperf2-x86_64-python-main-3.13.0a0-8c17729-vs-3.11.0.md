
# Results vs. 3.11.0

- fork: python
- ref: main
- machine: linux-x86_64
- commit hash: 8c17729
- commit date: 2023-07-16
- overall geometric mean: 1.04x faster
- HPT reliability: 71.13%
- HPT 99th percentile: 1.00x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| docutils       | 2.86 sec                                                     | 2.93 sec: 1.03x slower                                      |
| Geometric mean | (ref)                                                        | 1.02x slower                                                |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| nbody          | 90.7 ms                                                      | 89.4 ms: 1.01x faster                                       |
| pidigits       | 251 ms                                                       | 260 ms: 1.03x slower                                        |
| float          | 74.2 ms                                                      | 82.1 ms: 1.11x slower                                       |
| Geometric mean | (ref)                                                        | 1.04x slower                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|----------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| regex_compile  | 158 ms                                                       | 152 ms: 1.04x faster                                        |
| regex_v8       | 23.9 ms                                                      | 23.8 ms: 1.01x faster                                       |
| regex_effbot   | 3.50 ms                                                      | 3.68 ms: 1.05x slower                                       |
| regex_dna      | 227 ms                                                       | 240 ms: 1.06x slower                                        |
| Geometric mean | (ref)                                                        | 1.02x slower                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|----------------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| json_dumps           | 13.4 ms                                                      | 10.3 ms: 1.30x faster                                       |
| json_loads           | 28.7 us                                                      | 24.9 us: 1.15x faster                                       |
| unpickle_pure_python | 241 us                                                       | 220 us: 1.09x faster                                        |
| xml_etree_parse      | 158 ms                                                       | 150 ms: 1.05x faster                                        |
| pickle_pure_python   | 319 us                                                       | 326 us: 1.02x slower                                        |
| xml_etree_iterparse  | 104 ms                                                       | 107 ms: 1.03x slower                                        |
| tomli_loads          | 2.26 sec                                                     | 2.35 sec: 1.04x slower                                      |
| unpickle_list        | 4.53 us                                                      | 4.75 us: 1.05x slower                                       |
| xml_etree_process    | 56.5 ms                                                      | 59.7 ms: 1.06x slower                                       |
| pickle               | 9.64 us                                                      | 10.2 us: 1.06x slower                                       |
| pickle_dict          | 30.8 us                                                      | 32.8 us: 1.07x slower                                       |
| xml_etree_generate   | 80.5 ms                                                      | 86.1 ms: 1.07x slower                                       |
| unpickle             | 13.4 us                                                      | 14.4 us: 1.07x slower                                       |
| pickle_list          | 3.83 us                                                      | 4.30 us: 1.12x slower                                       |
| Geometric mean       | (ref)                                                        | 1.00x slower                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|------------------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| python_startup         | 10.8 ms                                                      | 11.4 ms: 1.06x slower                                       |
| python_startup_no_site | 7.76 ms                                                      | 8.42 ms: 1.09x slower                                       |
| Geometric mean         | (ref)                                                        | 1.07x slower                                                |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|-----------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| mako      | 11.0 ms                                                      | 10.2 ms: 1.07x faster                                       |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230716-pythonperf2-x86_64-python-main-3.13.0a0-8c17729 |
|--------------------------|:------------------------------------------------------------:|:-----------------------------------------------------------:|
| typing_runtime_protocols | 523 us                                                       | 152 us: 3.45x faster                                        |
| asyncio_tcp              | 753 ms                                                       | 383 ms: 1.97x faster                                        |
| asyncio_tcp_ssl          | 3.08 sec                                                     | 1.58 sec: 1.95x faster                                      |
| generators               | 56.0 ms                                                      | 37.0 ms: 1.51x faster                                       |
| chaos                    | 80.9 ms                                                      | 61.8 ms: 1.31x faster                                       |
| json_dumps               | 13.4 ms                                                      | 10.3 ms: 1.30x faster                                       |
| mypy2                    | 451 ms                                                       | 372 ms: 1.21x faster                                        |
| coroutines               | 27.6 ms                                                      | 23.3 ms: 1.18x faster                                       |
| json_loads               | 28.7 us                                                      | 24.9 us: 1.15x faster                                       |
| scimark_lu               | 115 ms                                                       | 101 ms: 1.13x faster                                        |
| nqueens                  | 103 ms                                                       | 91.8 ms: 1.12x faster                                       |
| comprehensions           | 24.6 us                                                      | 22.1 us: 1.12x faster                                       |
| async_tree_memoization   | 630 ms                                                       | 567 ms: 1.11x faster                                        |
| crypto_pyaes             | 83.4 ms                                                      | 75.2 ms: 1.11x faster                                       |
| async_tree_none          | 519 ms                                                       | 470 ms: 1.10x faster                                        |
| json                     | 5.65 ms                                                      | 5.13 ms: 1.10x faster                                       |
| unpickle_pure_python     | 241 us                                                       | 220 us: 1.09x faster                                        |
| fannkuch                 | 429 ms                                                       | 393 ms: 1.09x faster                                        |
| async_tree_io            | 1.17 sec                                                     | 1.08 sec: 1.08x faster                                      |
| hexiom                   | 7.13 ms                                                      | 6.58 ms: 1.08x faster                                       |
| mako                     | 11.0 ms                                                      | 10.2 ms: 1.07x faster                                       |
| deltablue                | 4.00 ms                                                      | 3.73 ms: 1.07x faster                                       |
| sqlglot_normalize        | 126 ms                                                       | 118 ms: 1.07x faster                                        |
| raytrace                 | 317 ms                                                       | 299 ms: 1.06x faster                                        |
| sqlglot_parse            | 1.53 ms                                                      | 1.45 ms: 1.06x faster                                       |
| bench_thread_pool        | 1.01 ms                                                      | 959 us: 1.05x faster                                        |
| mdp                      | 2.75 sec                                                     | 2.61 sec: 1.05x faster                                      |
| xml_etree_parse          | 158 ms                                                       | 150 ms: 1.05x faster                                        |
| gc_traversal             | 3.85 ms                                                      | 3.67 ms: 1.05x faster                                       |
| async_tree_cpu_io_mixed  | 749 ms                                                       | 717 ms: 1.04x faster                                        |
| logging_silent           | 101 ns                                                       | 96.8 ns: 1.04x faster                                       |
| sqlglot_transpile        | 1.92 ms                                                      | 1.85 ms: 1.04x faster                                       |
| regex_compile            | 158 ms                                                       | 152 ms: 1.04x faster                                        |
| deepcopy                 | 399 us                                                       | 391 us: 1.02x faster                                        |
| logging_format           | 8.11 us                                                      | 7.97 us: 1.02x faster                                       |
| meteor_contest           | 131 ms                                                       | 129 ms: 1.02x faster                                        |
| nbody                    | 90.7 ms                                                      | 89.4 ms: 1.01x faster                                       |
| regex_v8                 | 23.9 ms                                                      | 23.8 ms: 1.01x faster                                       |
| sqlglot_optimize         | 59.8 ms                                                      | 59.4 ms: 1.01x faster                                       |
| scimark_monte_carlo      | 68.2 ms                                                      | 68.7 ms: 1.01x slower                                       |
| deepcopy_memo            | 38.8 us                                                      | 39.1 us: 1.01x slower                                       |
| logging_simple           | 7.19 us                                                      | 7.28 us: 1.01x slower                                       |
| spectral_norm            | 93.3 ms                                                      | 94.6 ms: 1.01x slower                                       |
| deepcopy_reduce          | 3.51 us                                                      | 3.57 us: 1.02x slower                                       |
| pickle_pure_python       | 319 us                                                       | 326 us: 1.02x slower                                        |
| docutils                 | 2.86 sec                                                     | 2.93 sec: 1.03x slower                                      |
| xml_etree_iterparse      | 104 ms                                                       | 107 ms: 1.03x slower                                        |
| pidigits                 | 251 ms                                                       | 260 ms: 1.03x slower                                        |
| pathlib                  | 19.1 ms                                                      | 19.7 ms: 1.03x slower                                       |
| scimark_sparse_mat_mult  | 4.05 ms                                                      | 4.19 ms: 1.04x slower                                       |
| tomli_loads              | 2.26 sec                                                     | 2.35 sec: 1.04x slower                                      |
| unpickle_list            | 4.53 us                                                      | 4.75 us: 1.05x slower                                       |
| coverage                 | 84.8 ms                                                      | 89.1 ms: 1.05x slower                                       |
| regex_effbot             | 3.50 ms                                                      | 3.68 ms: 1.05x slower                                       |
| bench_mp_pool            | 4.62 ms                                                      | 4.87 ms: 1.05x slower                                       |
| pprint_pformat           | 1.63 sec                                                     | 1.72 sec: 1.05x slower                                      |
| regex_dna                | 227 ms                                                       | 240 ms: 1.06x slower                                        |
| xml_etree_process        | 56.5 ms                                                      | 59.7 ms: 1.06x slower                                       |
| go                       | 164 ms                                                       | 174 ms: 1.06x slower                                        |
| pickle                   | 9.64 us                                                      | 10.2 us: 1.06x slower                                       |
| python_startup           | 10.8 ms                                                      | 11.4 ms: 1.06x slower                                       |
| pickle_dict              | 30.8 us                                                      | 32.8 us: 1.07x slower                                       |
| xml_etree_generate       | 80.5 ms                                                      | 86.1 ms: 1.07x slower                                       |
| unpickle                 | 13.4 us                                                      | 14.4 us: 1.07x slower                                       |
| pprint_safe_repr         | 784 ms                                                       | 843 ms: 1.08x slower                                        |
| python_startup_no_site   | 7.76 ms                                                      | 8.42 ms: 1.09x slower                                       |
| scimark_fft              | 285 ms                                                       | 310 ms: 1.09x slower                                        |
| sqlite_synth             | 2.50 us                                                      | 2.73 us: 1.09x slower                                       |
| float                    | 74.2 ms                                                      | 82.1 ms: 1.11x slower                                       |
| pickle_list              | 3.83 us                                                      | 4.30 us: 1.12x slower                                       |
| telco                    | 6.86 ms                                                      | 7.77 ms: 1.13x slower                                       |
| richards                 | 48.3 ms                                                      | 55.0 ms: 1.14x slower                                       |
| pyflate                  | 449 ms                                                       | 514 ms: 1.15x slower                                        |
| unpack_sequence          | 45.6 ns                                                      | 54.3 ns: 1.19x slower                                       |
| async_generators         | 316 ms                                                       | 397 ms: 1.26x slower                                        |
| scimark_sor              | 111 ms                                                       | 146 ms: 1.32x slower                                        |
| dask                     | 410 ms                                                       | 592 ms: 1.44x slower                                        |
| Geometric mean           | (ref)                                                        | 1.04x faster                                                |

Benchmark hidden because not significant (5): dulwich_log, pycparser, richards_super, create_gc_cycles, tornado_http
Ignored benchmarks (17) of results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift


# HPT report

- Reliability score: 71.13% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x
