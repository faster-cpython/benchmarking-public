# Results vs. 3.13.0

- fork: nick-arm
- ref: codecache
- machine: linux-x86_64
- commit hash: c2fad93
- commit date: 2024-10-17
- overall geometric mean: 1.003x faster
- HPT reliability: 54.62%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.08x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| 2to3           | 293 ms                                                       | 316 ms: 1.08x slower                                                 |
| docutils       | 2.81 sec                                                     | 3.22 sec: 1.15x slower                                               |
| html5lib       | 72.9 ms                                                      | 71.5 ms: 1.02x faster                                                |
| sphinx         | 1.11 sec                                                     | 1.27 sec: 1.14x slower                                               |
| tornado_http   | 119 ms                                                       | 122 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                        | 1.07x slower                                                         |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| async_tree_memoization_tg  | 458 ms                                                       | 375 ms: 1.22x faster                                                 |
| async_tree_none            | 370 ms                                                       | 322 ms: 1.15x faster                                                 |
| async_tree_none_tg         | 342 ms                                                       | 306 ms: 1.12x faster                                                 |
| async_tree_memoization     | 447 ms                                                       | 411 ms: 1.09x faster                                                 |
| async_tree_cpu_io_mixed    | 596 ms                                                       | 564 ms: 1.06x faster                                                 |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 557 ms: 1.03x faster                                                 |
| asyncio_websockets         | 395 ms                                                       | 383 ms: 1.03x faster                                                 |
| async_generators           | 364 ms                                                       | 386 ms: 1.06x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.05x faster                                                         |

Benchmark hidden because not significant (3): coroutines, async_tree_io_tg, async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| nbody          | 92.1 ms                                                      | 80.3 ms: 1.15x faster                                                |
| float          | 81.6 ms                                                      | 78.1 ms: 1.04x faster                                                |
| pidigits       | 252 ms                                                       | 252 ms: 1.00x faster                                                 |
| Geometric mean | (ref)                                                        | 1.06x faster                                                         |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| regex_effbot   | 3.51 ms                                                      | 3.47 ms: 1.01x faster                                                |
| regex_v8       | 24.9 ms                                                      | 25.6 ms: 1.03x slower                                                |
| regex_compile  | 143 ms                                                       | 147 ms: 1.03x slower                                                 |
| regex_dna      | 238 ms                                                       | 247 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                        | 1.02x slower                                                         |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| tomli_loads          | 2.43 sec                                                     | 2.18 sec: 1.12x faster                                               |
| xml_etree_generate   | 85.2 ms                                                      | 79.4 ms: 1.07x faster                                                |
| xml_etree_process    | 60.7 ms                                                      | 56.6 ms: 1.07x faster                                                |
| json_loads           | 24.7 us                                                      | 23.9 us: 1.03x faster                                                |
| json_dumps           | 10.8 ms                                                      | 10.5 ms: 1.03x faster                                                |
| xml_etree_parse      | 144 ms                                                       | 145 ms: 1.01x slower                                                 |
| unpickle_pure_python | 216 us                                                       | 220 us: 1.02x slower                                                 |
| xml_etree_iterparse  | 99.8 ms                                                      | 102 ms: 1.02x slower                                                 |
| pickle_pure_python   | 322 us                                                       | 335 us: 1.04x slower                                                 |
| Geometric mean       | (ref)                                                        | 1.03x faster                                                         |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| python_startup         | 15.6 ms                                                      | 14.9 ms: 1.05x faster                                                |
| python_startup_no_site | 8.93 ms                                                      | 8.99 ms: 1.01x slower                                                |
| Geometric mean         | (ref)                                                        | 1.02x faster                                                         |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|-----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| mako            | 10.3 ms                                                      | 9.15 ms: 1.13x faster                                                |
| genshi_text     | 27.2 ms                                                      | 28.9 ms: 1.06x slower                                                |
| django_template | 38.9 ms                                                      | 42.5 ms: 1.09x slower                                                |
| genshi_xml      | 58.0 ms                                                      | 64.8 ms: 1.12x slower                                                |
| Geometric mean  | (ref)                                                        | 1.04x slower                                                         |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93 |
|----------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| deepcopy_memo              | 38.9 us                                                      | 28.1 us: 1.38x faster                                                |
| deepcopy                   | 388 us                                                       | 302 us: 1.29x faster                                                 |
| async_tree_memoization_tg  | 458 ms                                                       | 375 ms: 1.22x faster                                                 |
| scimark_sor                | 125 ms                                                       | 106 ms: 1.18x faster                                                 |
| deepcopy_reduce            | 3.49 us                                                      | 2.98 us: 1.17x faster                                                |
| richards                   | 52.5 ms                                                      | 45.0 ms: 1.17x faster                                                |
| async_tree_none            | 370 ms                                                       | 322 ms: 1.15x faster                                                 |
| nbody                      | 92.1 ms                                                      | 80.3 ms: 1.15x faster                                                |
| telco                      | 8.77 ms                                                      | 7.76 ms: 1.13x faster                                                |
| richards_super             | 60.2 ms                                                      | 53.3 ms: 1.13x faster                                                |
| mako                       | 10.3 ms                                                      | 9.15 ms: 1.13x faster                                                |
| async_tree_none_tg         | 342 ms                                                       | 306 ms: 1.12x faster                                                 |
| tomli_loads                | 2.43 sec                                                     | 2.18 sec: 1.12x faster                                               |
| pathlib                    | 17.4 ms                                                      | 15.7 ms: 1.11x faster                                                |
| go                         | 167 ms                                                       | 152 ms: 1.10x faster                                                 |
| pyflate                    | 493 ms                                                       | 450 ms: 1.09x faster                                                 |
| async_tree_memoization     | 447 ms                                                       | 411 ms: 1.09x faster                                                 |
| json                       | 5.62 ms                                                      | 5.17 ms: 1.09x faster                                                |
| logging_silent             | 97.5 ns                                                      | 90.2 ns: 1.08x faster                                                |
| spectral_norm              | 97.4 ms                                                      | 90.2 ms: 1.08x faster                                                |
| coverage                   | 84.5 ms                                                      | 78.4 ms: 1.08x faster                                                |
| xml_etree_generate         | 85.2 ms                                                      | 79.4 ms: 1.07x faster                                                |
| xml_etree_process          | 60.7 ms                                                      | 56.6 ms: 1.07x faster                                                |
| scimark_fft                | 298 ms                                                       | 278 ms: 1.07x faster                                                 |
| bpe_tokeniser              | 5.09 sec                                                     | 4.79 sec: 1.06x faster                                               |
| async_tree_cpu_io_mixed    | 596 ms                                                       | 564 ms: 1.06x faster                                                 |
| pprint_safe_repr           | 835 ms                                                       | 794 ms: 1.05x faster                                                 |
| pprint_pformat             | 1.70 sec                                                     | 1.62 sec: 1.05x faster                                               |
| fannkuch                   | 384 ms                                                       | 367 ms: 1.05x faster                                                 |
| python_startup             | 15.6 ms                                                      | 14.9 ms: 1.05x faster                                                |
| float                      | 81.6 ms                                                      | 78.1 ms: 1.04x faster                                                |
| deltablue                  | 3.38 ms                                                      | 3.24 ms: 1.04x faster                                                |
| thrift                     | 918 us                                                       | 883 us: 1.04x faster                                                 |
| json_loads                 | 24.7 us                                                      | 23.9 us: 1.03x faster                                                |
| crypto_pyaes               | 73.5 ms                                                      | 71.1 ms: 1.03x faster                                                |
| json_dumps                 | 10.8 ms                                                      | 10.5 ms: 1.03x faster                                                |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 557 ms: 1.03x faster                                                 |
| asyncio_websockets         | 395 ms                                                       | 383 ms: 1.03x faster                                                 |
| pycparser                  | 1.28 sec                                                     | 1.25 sec: 1.02x faster                                               |
| scimark_sparse_mat_mult    | 4.21 ms                                                      | 4.13 ms: 1.02x faster                                                |
| html5lib                   | 72.9 ms                                                      | 71.5 ms: 1.02x faster                                                |
| regex_effbot               | 3.51 ms                                                      | 3.47 ms: 1.01x faster                                                |
| scimark_lu                 | 97.4 ms                                                      | 97.0 ms: 1.00x faster                                                |
| pidigits                   | 252 ms                                                       | 252 ms: 1.00x faster                                                 |
| python_startup_no_site     | 8.93 ms                                                      | 8.99 ms: 1.01x slower                                                |
| dulwich_log                | 65.5 ms                                                      | 65.9 ms: 1.01x slower                                                |
| xml_etree_parse            | 144 ms                                                       | 145 ms: 1.01x slower                                                 |
| unpickle_pure_python       | 216 us                                                       | 220 us: 1.02x slower                                                 |
| xml_etree_iterparse        | 99.8 ms                                                      | 102 ms: 1.02x slower                                                 |
| meteor_contest             | 130 ms                                                       | 134 ms: 1.03x slower                                                 |
| tornado_http               | 119 ms                                                       | 122 ms: 1.03x slower                                                 |
| regex_v8                   | 24.9 ms                                                      | 25.6 ms: 1.03x slower                                                |
| regex_compile              | 143 ms                                                       | 147 ms: 1.03x slower                                                 |
| typing_runtime_protocols   | 176 us                                                       | 181 us: 1.03x slower                                                 |
| regex_dna                  | 238 ms                                                       | 247 ms: 1.03x slower                                                 |
| mdp                        | 2.53 sec                                                     | 2.62 sec: 1.04x slower                                               |
| sympy_expand               | 506 ms                                                       | 526 ms: 1.04x slower                                                 |
| pickle_pure_python         | 322 us                                                       | 335 us: 1.04x slower                                                 |
| bench_thread_pool          | 929 us                                                       | 968 us: 1.04x slower                                                 |
| logging_format             | 6.95 us                                                      | 7.28 us: 1.05x slower                                                |
| scimark_monte_carlo        | 65.2 ms                                                      | 68.3 ms: 1.05x slower                                                |
| create_gc_cycles           | 2.65 ms                                                      | 2.80 ms: 1.06x slower                                                |
| async_generators           | 364 ms                                                       | 386 ms: 1.06x slower                                                 |
| genshi_text                | 27.2 ms                                                      | 28.9 ms: 1.06x slower                                                |
| logging_simple             | 6.28 us                                                      | 6.71 us: 1.07x slower                                                |
| sympy_str                  | 297 ms                                                       | 320 ms: 1.08x slower                                                 |
| 2to3                       | 293 ms                                                       | 316 ms: 1.08x slower                                                 |
| django_template            | 38.9 ms                                                      | 42.5 ms: 1.09x slower                                                |
| sqlglot_parse              | 1.37 ms                                                      | 1.50 ms: 1.10x slower                                                |
| hexiom                     | 6.47 ms                                                      | 7.13 ms: 1.10x slower                                                |
| nqueens                    | 92.3 ms                                                      | 102 ms: 1.11x slower                                                 |
| genshi_xml                 | 58.0 ms                                                      | 64.8 ms: 1.12x slower                                                |
| sqlglot_transpile          | 1.76 ms                                                      | 1.97 ms: 1.12x slower                                                |
| comprehensions             | 17.3 us                                                      | 19.4 us: 1.12x slower                                                |
| chaos                      | 60.6 ms                                                      | 68.4 ms: 1.13x slower                                                |
| sqlglot_normalize          | 119 ms                                                       | 135 ms: 1.13x slower                                                 |
| sympy_sum                  | 154 ms                                                       | 174 ms: 1.13x slower                                                 |
| sphinx                     | 1.11 sec                                                     | 1.27 sec: 1.14x slower                                               |
| docutils                   | 2.81 sec                                                     | 3.22 sec: 1.15x slower                                               |
| gc_traversal               | 4.48 ms                                                      | 5.17 ms: 1.15x slower                                                |
| sympy_integrate            | 23.4 ms                                                      | 27.3 ms: 1.17x slower                                                |
| generators                 | 33.9 ms                                                      | 39.8 ms: 1.17x slower                                                |
| sqlglot_optimize           | 58.7 ms                                                      | 69.3 ms: 1.18x slower                                                |
| pylint                     | 345 ms                                                       | 418 ms: 1.21x slower                                                 |
| raytrace                   | 267 ms                                                       | 324 ms: 1.21x slower                                                 |
| bench_mp_pool              | 4.82 ms                                                      | 2.97 sec: 616.89x slower                                             |
| Geometric mean             | (ref)                                                        | 1.07x slower                                                         |

Benchmark hidden because not significant (3): coroutines, async_tree_io_tg, async_tree_io
Ignored benchmarks (8) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, gevent_hub, k_core, mypy2, shortest_path, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (9) of results/bm-20241017-3.14.0a0-c2fad93-JIT/bm-20241017-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-c2fad93.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.003x faster
# HPT report

- Reliability score: 54.62% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.08x