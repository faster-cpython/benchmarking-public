
# Results vs. 3.10.4

- fork: python
- ref: 2e91dba437fe5c56c6f8
- machine: darwin-arm64
- commit hash: 2e91dba
- commit date: 2021-12-08
- overall geometric mean: 1.15x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.11x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 168 ms: 1.21x faster                                                  |
| chameleon      | 5.84 ms                                                | 5.16 ms: 1.13x faster                                                 |
| docutils       | 1.78 sec                                               | 1.57 sec: 1.14x faster                                                |
| html5lib       | 44.1 ms                                                | 37.0 ms: 1.19x faster                                                 |
| tornado_http   | 91.9 ms                                                | 79.1 ms: 1.16x faster                                                 |
| Geometric mean | (ref)                                                  | 1.17x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 65.3 ms: 1.44x faster                                                 |
| float          | 72.3 ms                                                | 56.7 ms: 1.28x faster                                                 |
| pidigits       | 282 ms                                                 | 281 ms: 1.00x faster                                                  |
| Geometric mean | (ref)                                                  | 1.23x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 78.8 ms: 1.23x faster                                                 |
| regex_v8       | 17.5 ms                                                | 17.1 ms: 1.02x faster                                                 |
| regex_dna      | 160 ms                                                 | 159 ms: 1.00x faster                                                  |
| regex_effbot   | 2.45 ms                                                | 2.46 ms: 1.01x slower                                                 |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 224 us: 1.27x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 37.7 ms: 1.20x faster                                                 |
| unpickle_pure_python | 203 us                                                 | 176 us: 1.15x faster                                                  |
| xml_etree_parse      | 107 ms                                                 | 99.0 ms: 1.08x faster                                                 |
| xml_etree_generate   | 54.3 ms                                                | 50.3 ms: 1.08x faster                                                 |
| unpickle_list        | 2.66 us                                                | 2.51 us: 1.06x faster                                                 |
| json_dumps           | 8.38 ms                                                | 7.99 ms: 1.05x faster                                                 |
| xml_etree_iterparse  | 72.6 ms                                                | 70.1 ms: 1.04x faster                                                 |
| json_loads           | 16.9 us                                                | 16.6 us: 1.02x faster                                                 |
| pickle_dict          | 17.8 us                                                | 17.9 us: 1.01x slower                                                 |
| pickle               | 7.36 us                                                | 7.42 us: 1.01x slower                                                 |
| unpickle             | 9.77 us                                                | 10.1 us: 1.03x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.07x faster                                                          |

Benchmark hidden because not significant (1): pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 9.73 ms                                                | 9.23 ms: 1.05x faster                                                 |
| python_startup         | 12.6 ms                                                | 15.4 ms: 1.22x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.08x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 10.5 ms                                                | 8.53 ms: 1.23x faster                                                 |
| django_template | 27.3 ms                                                | 22.9 ms: 1.19x faster                                                 |
| genshi_xml      | 37.6 ms                                                | 32.3 ms: 1.16x faster                                                 |
| genshi_text     | 18.4 ms                                                | 16.3 ms: 1.13x faster                                                 |
| Geometric mean  | (ref)                                                  | 1.18x faster                                                          |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20211208-darwin-arm64-python-2e91dba437fe5c56c6f8-3.11.0a3-2e91dba |
|-------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 3.10 ms: 1.66x faster                                                 |
| scimark_sor             | 127 ms                                                 | 86.2 ms: 1.47x faster                                                 |
| logging_silent          | 119 ns                                                 | 81.6 ns: 1.46x faster                                                 |
| nbody                   | 94.1 ms                                                | 65.3 ms: 1.44x faster                                                 |
| raytrace                | 328 ms                                                 | 227 ms: 1.44x faster                                                  |
| richards                | 51.4 ms                                                | 36.2 ms: 1.42x faster                                                 |
| scimark_monte_carlo     | 72.2 ms                                                | 51.4 ms: 1.41x faster                                                 |
| scimark_lu              | 110 ms                                                 | 78.6 ms: 1.40x faster                                                 |
| go                      | 165 ms                                                 | 119 ms: 1.39x faster                                                  |
| hexiom                  | 6.32 ms                                                | 4.70 ms: 1.34x faster                                                 |
| chaos                   | 66.8 ms                                                | 50.3 ms: 1.33x faster                                                 |
| async_tree_none         | 402 ms                                                 | 305 ms: 1.32x faster                                                  |
| logging_simple          | 4.63 us                                                | 3.52 us: 1.31x faster                                                 |
| pyflate                 | 453 ms                                                 | 346 ms: 1.31x faster                                                  |
| logging_format          | 5.01 us                                                | 3.83 us: 1.31x faster                                                 |
| crypto_pyaes            | 78.0 ms                                                | 59.7 ms: 1.31x faster                                                 |
| float                   | 72.3 ms                                                | 56.7 ms: 1.28x faster                                                 |
| pickle_pure_python      | 284 us                                                 | 224 us: 1.27x faster                                                  |
| spectral_norm           | 96.4 ms                                                | 76.8 ms: 1.25x faster                                                 |
| thrift                  | 586 us                                                 | 470 us: 1.25x faster                                                  |
| async_tree_memoization  | 493 ms                                                 | 400 ms: 1.23x faster                                                  |
| mako                    | 10.5 ms                                                | 8.53 ms: 1.23x faster                                                 |
| regex_compile           | 96.6 ms                                                | 78.8 ms: 1.23x faster                                                 |
| deepcopy_memo           | 34.5 us                                                | 28.1 us: 1.23x faster                                                 |
| generators              | 32.9 ms                                                | 27.1 ms: 1.22x faster                                                 |
| unpack_sequence         | 38.7 ns                                                | 32.0 ns: 1.21x faster                                                 |
| 2to3                    | 202 ms                                                 | 168 ms: 1.21x faster                                                  |
| pprint_safe_repr        | 609 ms                                                 | 505 ms: 1.21x faster                                                  |
| pycparser               | 915 ms                                                 | 759 ms: 1.21x faster                                                  |
| pprint_pformat          | 1.24 sec                                               | 1.03 sec: 1.20x faster                                                |
| xml_etree_process       | 45.1 ms                                                | 37.7 ms: 1.20x faster                                                 |
| create_gc_cycles        | 876 us                                                 | 733 us: 1.20x faster                                                  |
| django_template         | 27.3 ms                                                | 22.9 ms: 1.19x faster                                                 |
| async_tree_io           | 1.02 sec                                               | 855 ms: 1.19x faster                                                  |
| html5lib                | 44.1 ms                                                | 37.0 ms: 1.19x faster                                                 |
| async_generators        | 233 ms                                                 | 198 ms: 1.18x faster                                                  |
| fannkuch                | 317 ms                                                 | 270 ms: 1.17x faster                                                  |
| deepcopy_reduce         | 2.38 us                                                | 2.04 us: 1.17x faster                                                 |
| dulwich_log             | 37.1 ms                                                | 31.8 ms: 1.16x faster                                                 |
| genshi_xml              | 37.6 ms                                                | 32.3 ms: 1.16x faster                                                 |
| tornado_http            | 91.9 ms                                                | 79.1 ms: 1.16x faster                                                 |
| deepcopy                | 278 us                                                 | 240 us: 1.16x faster                                                  |
| sqlalchemy_imperative   | 9.03 ms                                                | 7.80 ms: 1.16x faster                                                 |
| unpickle_pure_python    | 203 us                                                 | 176 us: 1.15x faster                                                  |
| scimark_fft             | 232 ms                                                 | 202 ms: 1.15x faster                                                  |
| async_tree_cpu_io_mixed | 670 ms                                                 | 587 ms: 1.14x faster                                                  |
| docutils                | 1.78 sec                                               | 1.57 sec: 1.14x faster                                                |
| chameleon               | 5.84 ms                                                | 5.16 ms: 1.13x faster                                                 |
| genshi_text             | 18.4 ms                                                | 16.3 ms: 1.13x faster                                                 |
| nqueens                 | 68.1 ms                                                | 61.1 ms: 1.11x faster                                                 |
| sqlalchemy_declarative  | 74.8 ms                                                | 67.3 ms: 1.11x faster                                                 |
| sqlglot_optimize        | 38.0 ms                                                | 34.4 ms: 1.11x faster                                                 |
| sympy_integrate         | 13.4 ms                                                | 12.1 ms: 1.10x faster                                                 |
| sqlglot_normalize       | 197 ms                                                 | 179 ms: 1.10x faster                                                  |
| coroutines              | 20.2 ms                                                | 18.6 ms: 1.09x faster                                                 |
| xml_etree_parse         | 107 ms                                                 | 99.0 ms: 1.08x faster                                                 |
| flaskblogging           | 2.75 ms                                                | 2.54 ms: 1.08x faster                                                 |
| xml_etree_generate      | 54.3 ms                                                | 50.3 ms: 1.08x faster                                                 |
| sqlite_synth            | 1.47 us                                                | 1.37 us: 1.08x faster                                                 |
| pylint                  | 307 ms                                                 | 287 ms: 1.07x faster                                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 3.26 ms: 1.07x faster                                                 |
| meteor_contest          | 78.6 ms                                                | 73.7 ms: 1.07x faster                                                 |
| unpickle_list           | 2.66 us                                                | 2.51 us: 1.06x faster                                                 |
| bench_thread_pool       | 548 us                                                 | 517 us: 1.06x faster                                                  |
| sympy_expand            | 276 ms                                                 | 261 ms: 1.06x faster                                                  |
| mdp                     | 1.67 sec                                               | 1.58 sec: 1.06x faster                                                |
| sympy_sum               | 94.3 ms                                                | 89.6 ms: 1.05x faster                                                 |
| python_startup_no_site  | 9.73 ms                                                | 9.23 ms: 1.05x faster                                                 |
| json_dumps              | 8.38 ms                                                | 7.99 ms: 1.05x faster                                                 |
| pathlib                 | 28.8 ms                                                | 27.5 ms: 1.05x faster                                                 |
| sympy_str               | 169 ms                                                 | 162 ms: 1.05x faster                                                  |
| sqlglot_transpile       | 1.54 ms                                                | 1.47 ms: 1.05x faster                                                 |
| xml_etree_iterparse     | 72.6 ms                                                | 70.1 ms: 1.04x faster                                                 |
| json                    | 3.10 ms                                                | 2.99 ms: 1.03x faster                                                 |
| sqlglot_parse           | 1.33 ms                                                | 1.29 ms: 1.03x faster                                                 |
| telco                   | 3.68 ms                                                | 3.57 ms: 1.03x faster                                                 |
| regex_v8                | 17.5 ms                                                | 17.1 ms: 1.02x faster                                                 |
| json_loads              | 16.9 us                                                | 16.6 us: 1.02x faster                                                 |
| pidigits                | 282 ms                                                 | 281 ms: 1.00x faster                                                  |
| regex_dna               | 160 ms                                                 | 159 ms: 1.00x faster                                                  |
| pickle_dict             | 17.8 us                                                | 17.9 us: 1.01x slower                                                 |
| regex_effbot            | 2.45 ms                                                | 2.46 ms: 1.01x slower                                                 |
| pickle                  | 7.36 us                                                | 7.42 us: 1.01x slower                                                 |
| gc_traversal            | 2.40 ms                                                | 2.43 ms: 1.01x slower                                                 |
| comprehensions          | 17.7 us                                                | 18.0 us: 1.02x slower                                                 |
| unpickle                | 9.77 us                                                | 10.1 us: 1.03x slower                                                 |
| bench_mp_pool           | 41.0 ms                                                | 43.8 ms: 1.07x slower                                                 |
| coverage                | 40.8 ms                                                | 45.6 ms: 1.12x slower                                                 |
| python_startup          | 12.6 ms                                                | 15.4 ms: 1.22x slower                                                 |
| Geometric mean          | (ref)                                                  | 1.15x faster                                                          |

Benchmark hidden because not significant (2): pickle_list, dask
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, gunicorn, mypy2, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.13x
- 95% likely to have a speedup of 1.12x
- 99% likely to have a speedup of 1.11x
