
# Results vs. 3.11.0

- fork: python
- ref: dc3f97549a8fe4f7fea8
- machine: linux-x86_64
- commit hash: dc3f975
- commit date: 2023-04-26
- overall geometric mean: 1.01x slower \*
- HPT reliability: 99.91%
- HPT 99th percentile: 1.00x slower

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 259 ms                                                 | 268 ms: 1.03x slower                                                   |
| chameleon      | 6.47 ms                                                | 6.94 ms: 1.07x slower                                                  |
| docutils       | 2.63 sec                                               | 2.70 sec: 1.03x slower                                                 |
| tornado_http   | 96.3 ms                                                | 105 ms: 1.09x slower                                                   |
| Geometric mean | (ref)                                                  | 1.05x slower                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 93.1 ms                                                | 88.2 ms: 1.06x faster                                                  |
| pidigits       | 198 ms                                                 | 198 ms: 1.00x faster                                                   |
| float          | 77.2 ms                                                | 80.0 ms: 1.04x slower                                                  |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.99 ms                                                | 3.75 ms: 1.06x faster                                                  |
| regex_v8       | 22.0 ms                                                | 22.2 ms: 1.01x slower                                                  |
| regex_compile  | 138 ms                                                 | 145 ms: 1.05x slower                                                   |
| Geometric mean | (ref)                                                  | 1.00x faster                                                           |

Benchmark hidden because not significant (1): regex_dna

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 12.6 ms                                                | 10.0 ms: 1.26x faster                                                  |
| unpickle_pure_python | 228 us                                                 | 218 us: 1.05x faster                                                   |
| xml_etree_parse      | 158 ms                                                 | 152 ms: 1.05x faster                                                   |
| json_loads           | 26.5 us                                                | 25.8 us: 1.03x faster                                                  |
| xml_etree_iterparse  | 104 ms                                                 | 103 ms: 1.01x faster                                                   |
| pickle_dict          | 31.1 us                                                | 31.6 us: 1.01x slower                                                  |
| pickle_pure_python   | 306 us                                                 | 313 us: 1.02x slower                                                   |
| pickle               | 10.1 us                                                | 10.7 us: 1.06x slower                                                  |
| xml_etree_process    | 53.9 ms                                                | 58.6 ms: 1.09x slower                                                  |
| unpickle             | 13.7 us                                                | 15.0 us: 1.10x slower                                                  |
| xml_etree_generate   | 76.2 ms                                                | 83.9 ms: 1.10x slower                                                  |
| pickle_list          | 4.11 us                                                | 4.62 us: 1.12x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.01x slower                                                           |

Benchmark hidden because not significant (1): unpickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 8.52 ms                                                | 9.03 ms: 1.06x slower                                                  |
| python_startup_no_site | 6.01 ms                                                | 6.62 ms: 1.10x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.08x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 21.6 ms                                                | 22.7 ms: 1.05x slower                                                  |
| django_template | 32.6 ms                                                | 34.7 ms: 1.06x slower                                                  |
| mako            | 10.1 ms                                                | 10.9 ms: 1.08x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.05x slower                                                           |

Benchmark hidden because not significant (1): genshi_xml

All benchmarks:
===============

| Benchmark               | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230426-linux-x86_64-python-dc3f97549a8fe4f7fea8-3.12.0a7+-dc3f975 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| generators              | 73.5 ms                                                | 32.4 ms: 2.27x faster                                                  |
| asyncio_tcp             | 922 ms                                                 | 507 ms: 1.82x faster                                                   |
| json_dumps              | 12.6 ms                                                | 10.0 ms: 1.26x faster                                                  |
| mypy2                   | 420 ms                                                 | 351 ms: 1.20x faster                                                   |
| coroutines              | 25.5 ms                                                | 22.5 ms: 1.13x faster                                                  |
| gc_traversal            | 4.02 ms                                                | 3.76 ms: 1.07x faster                                                  |
| regex_effbot            | 3.99 ms                                                | 3.75 ms: 1.06x faster                                                  |
| nbody                   | 93.1 ms                                                | 88.2 ms: 1.06x faster                                                  |
| richards                | 45.7 ms                                                | 43.4 ms: 1.05x faster                                                  |
| sqlglot_parse           | 1.40 ms                                                | 1.33 ms: 1.05x faster                                                  |
| unpickle_pure_python    | 228 us                                                 | 218 us: 1.05x faster                                                   |
| xml_etree_parse         | 158 ms                                                 | 152 ms: 1.05x faster                                                   |
| pycparser               | 1.18 sec                                               | 1.14 sec: 1.04x faster                                                 |
| deltablue               | 3.67 ms                                                | 3.53 ms: 1.04x faster                                                  |
| go                      | 140 ms                                                 | 135 ms: 1.03x faster                                                   |
| sqlglot_transpile       | 1.70 ms                                                | 1.65 ms: 1.03x faster                                                  |
| json_loads              | 26.5 us                                                | 25.8 us: 1.03x faster                                                  |
| mdp                     | 2.62 sec                                               | 2.57 sec: 1.02x faster                                                 |
| hexiom                  | 6.37 ms                                                | 6.27 ms: 1.02x faster                                                  |
| create_gc_cycles        | 1.49 ms                                                | 1.46 ms: 1.02x faster                                                  |
| logging_silent          | 101 ns                                                 | 99.9 ns: 1.01x faster                                                  |
| xml_etree_iterparse     | 104 ms                                                 | 103 ms: 1.01x faster                                                   |
| coverage                | 100 ms                                                 | 99.0 ms: 1.01x faster                                                  |
| async_tree_io           | 1.30 sec                                               | 1.28 sec: 1.01x faster                                                 |
| fannkuch                | 388 ms                                                 | 385 ms: 1.01x faster                                                   |
| pidigits                | 198 ms                                                 | 198 ms: 1.00x faster                                                   |
| regex_v8                | 22.0 ms                                                | 22.2 ms: 1.01x slower                                                  |
| async_tree_cpu_io_mixed | 739 ms                                                 | 747 ms: 1.01x slower                                                   |
| pathlib                 | 18.2 ms                                                | 18.5 ms: 1.01x slower                                                  |
| nqueens                 | 83.4 ms                                                | 84.5 ms: 1.01x slower                                                  |
| pickle_dict             | 31.1 us                                                | 31.6 us: 1.01x slower                                                  |
| raytrace                | 297 ms                                                 | 302 ms: 1.02x slower                                                   |
| pprint_pformat          | 1.46 sec                                               | 1.48 sec: 1.02x slower                                                 |
| bench_thread_pool       | 819 us                                                 | 836 us: 1.02x slower                                                   |
| sqlglot_optimize        | 53.1 ms                                                | 54.3 ms: 1.02x slower                                                  |
| pickle_pure_python      | 306 us                                                 | 313 us: 1.02x slower                                                   |
| scimark_lu              | 110 ms                                                 | 112 ms: 1.03x slower                                                   |
| docutils                | 2.63 sec                                               | 2.70 sec: 1.03x slower                                                 |
| telco                   | 6.58 ms                                                | 6.78 ms: 1.03x slower                                                  |
| logging_format          | 6.68 us                                                | 6.90 us: 1.03x slower                                                  |
| sqlglot_normalize       | 108 ms                                                 | 111 ms: 1.03x slower                                                   |
| 2to3                    | 259 ms                                                 | 268 ms: 1.03x slower                                                   |
| logging_simple          | 6.03 us                                                | 6.24 us: 1.03x slower                                                  |
| djangocms               | 32.4 us                                                | 33.6 us: 1.04x slower                                                  |
| pprint_safe_repr        | 701 ms                                                 | 726 ms: 1.04x slower                                                   |
| float                   | 77.2 ms                                                | 80.0 ms: 1.04x slower                                                  |
| async_tree_memoization  | 627 ms                                                 | 654 ms: 1.04x slower                                                   |
| sympy_integrate         | 21.0 ms                                                | 21.9 ms: 1.04x slower                                                  |
| comprehensions          | 22.4 us                                                | 23.5 us: 1.05x slower                                                  |
| regex_compile           | 138 ms                                                 | 145 ms: 1.05x slower                                                   |
| genshi_text             | 21.6 ms                                                | 22.7 ms: 1.05x slower                                                  |
| deepcopy_memo           | 37.0 us                                                | 39.0 us: 1.05x slower                                                  |
| sympy_expand            | 475 ms                                                 | 501 ms: 1.05x slower                                                   |
| sqlalchemy_declarative  | 138 ms                                                 | 146 ms: 1.06x slower                                                   |
| python_startup          | 8.52 ms                                                | 9.03 ms: 1.06x slower                                                  |
| thrift                  | 756 us                                                 | 805 us: 1.06x slower                                                   |
| django_template         | 32.6 ms                                                | 34.7 ms: 1.06x slower                                                  |
| scimark_sor             | 118 ms                                                 | 126 ms: 1.06x slower                                                   |
| pickle                  | 10.1 us                                                | 10.7 us: 1.06x slower                                                  |
| crypto_pyaes            | 74.7 ms                                                | 79.5 ms: 1.06x slower                                                  |
| dulwich_log             | 63.7 ms                                                | 67.9 ms: 1.07x slower                                                  |
| sqlalchemy_imperative   | 17.9 ms                                                | 19.1 ms: 1.07x slower                                                  |
| sqlite_synth            | 2.52 us                                                | 2.70 us: 1.07x slower                                                  |
| chameleon               | 6.47 ms                                                | 6.94 ms: 1.07x slower                                                  |
| scimark_monte_carlo     | 68.1 ms                                                | 73.1 ms: 1.07x slower                                                  |
| deepcopy                | 342 us                                                 | 368 us: 1.08x slower                                                   |
| mako                    | 10.1 ms                                                | 10.9 ms: 1.08x slower                                                  |
| sympy_str               | 290 ms                                                 | 312 ms: 1.08x slower                                                   |
| pyflate                 | 418 ms                                                 | 451 ms: 1.08x slower                                                   |
| unpack_sequence         | 43.1 ns                                                | 46.6 ns: 1.08x slower                                                  |
| spectral_norm           | 100 ms                                                 | 108 ms: 1.08x slower                                                   |
| meteor_contest          | 107 ms                                                 | 116 ms: 1.08x slower                                                   |
| xml_etree_process       | 53.9 ms                                                | 58.6 ms: 1.09x slower                                                  |
| tornado_http            | 96.3 ms                                                | 105 ms: 1.09x slower                                                   |
| sympy_sum               | 167 ms                                                 | 182 ms: 1.09x slower                                                   |
| deepcopy_reduce         | 2.94 us                                                | 3.22 us: 1.10x slower                                                  |
| unpickle                | 13.7 us                                                | 15.0 us: 1.10x slower                                                  |
| xml_etree_generate      | 76.2 ms                                                | 83.9 ms: 1.10x slower                                                  |
| python_startup_no_site  | 6.01 ms                                                | 6.62 ms: 1.10x slower                                                  |
| scimark_fft             | 328 ms                                                 | 364 ms: 1.11x slower                                                   |
| scimark_sparse_mat_mult | 4.50 ms                                                | 5.00 ms: 1.11x slower                                                  |
| pickle_list             | 4.11 us                                                | 4.62 us: 1.12x slower                                                  |
| async_generators        | 368 ms                                                 | 449 ms: 1.22x slower                                                   |
| dask                    | 360 ms                                                 | 541 ms: 1.50x slower                                                   |
| Geometric mean          | (ref)                                                  | 1.01x slower                                                           |

Benchmark hidden because not significant (9): genshi_xml, unpickle_list, regex_dna, bench_mp_pool, async_tree_none, chaos, json, html5lib, pylint
Ignored benchmarks (7) of results/bm-20221024-3.11.0-deaf509/bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 99.91% likely to be slow
- 90% likely to have a slowdown of 1.01x
- 95% likely to have a slowdown of 1.01x
- 99% likely to have a slowdown of 1.00x
