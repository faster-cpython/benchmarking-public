# Results vs. 3.13.0

- fork: python
- ref: 34653bba644aa5481613
- machine: darwin-arm64
- commit hash: 34653bb
- commit date: 2024-10-22
- overall geometric mean: 1.037x faster
- HPT reliability: 99.67%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.09x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 187 ms                                                 | 183 ms: 1.02x faster                                                   |
| docutils       | 1.44 sec                                               | 1.56 sec: 1.09x slower                                                 |
| html5lib       | 36.6 ms                                                | 32.8 ms: 1.11x faster                                                  |
| sphinx         | 603 ms                                                 | 663 ms: 1.10x slower                                                   |
| Geometric mean | (ref)                                                  | 1.01x slower                                                           |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_memoization_tg        | 288 ms                                                 | 234 ms: 1.23x faster                                                   |
| coroutines                       | 19.8 ms                                                | 16.4 ms: 1.20x faster                                                  |
| async_tree_eager_tg              | 47.8 ms                                                | 42.4 ms: 1.13x faster                                                  |
| async_tree_eager_memoization     | 168 ms                                                 | 153 ms: 1.10x faster                                                   |
| async_tree_eager                 | 70.1 ms                                                | 63.5 ms: 1.10x faster                                                  |
| async_tree_memoization           | 268 ms                                                 | 244 ms: 1.10x faster                                                   |
| async_tree_none                  | 215 ms                                                 | 198 ms: 1.09x faster                                                   |
| async_tree_eager_memoization_tg  | 138 ms                                                 | 130 ms: 1.06x faster                                                   |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 335 ms: 1.03x faster                                                   |
| async_tree_eager_cpu_io_mixed    | 373 ms                                                 | 362 ms: 1.03x faster                                                   |
| async_generators                 | 295 ms                                                 | 292 ms: 1.01x faster                                                   |
| asyncio_websockets               | 242 ms                                                 | 243 ms: 1.00x slower                                                   |
| async_tree_cpu_io_mixed_tg       | 448 ms                                                 | 470 ms: 1.05x slower                                                   |
| async_tree_none_tg               | 198 ms                                                 | 218 ms: 1.10x slower                                                   |
| async_tree_io                    | 507 ms                                                 | 581 ms: 1.15x slower                                                   |
| async_tree_io_tg                 | 497 ms                                                 | 611 ms: 1.23x slower                                                   |
| async_tree_eager_io              | 514 ms                                                 | 666 ms: 1.30x slower                                                   |
| async_tree_eager_io_tg           | 477 ms                                                 | 714 ms: 1.50x slower                                                   |
| Geometric mean                   | (ref)                                                  | 1.01x slower                                                           |

Benchmark hidden because not significant (1): async_tree_cpu_io_mixed

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 56.0 ms                                                | 48.3 ms: 1.16x faster                                                  |
| nbody          | 74.0 ms                                                | 65.9 ms: 1.12x faster                                                  |
| pidigits       | 284 ms                                                 | 283 ms: 1.00x faster                                                   |
| Geometric mean | (ref)                                                  | 1.09x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 2.63 ms                                                | 2.45 ms: 1.07x faster                                                  |
| regex_compile  | 78.6 ms                                                | 74.9 ms: 1.05x faster                                                  |
| regex_dna      | 149 ms                                                 | 142 ms: 1.05x faster                                                   |
| regex_v8       | 17.0 ms                                                | 16.7 ms: 1.02x faster                                                  |
| Geometric mean | (ref)                                                  | 1.05x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 1.57 sec                                               | 1.25 sec: 1.26x faster                                                 |
| unpickle_pure_python | 164 us                                                 | 133 us: 1.23x faster                                                   |
| pickle_pure_python   | 214 us                                                 | 178 us: 1.20x faster                                                   |
| xml_etree_process    | 41.0 ms                                                | 34.6 ms: 1.18x faster                                                  |
| xml_etree_generate   | 57.0 ms                                                | 49.4 ms: 1.15x faster                                                  |
| json_loads           | 17.0 us                                                | 16.6 us: 1.02x faster                                                  |
| xml_etree_iterparse  | 73.6 ms                                                | 73.0 ms: 1.01x faster                                                  |
| json_dumps           | 6.52 ms                                                | 7.15 ms: 1.10x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.10x faster                                                           |

Benchmark hidden because not significant (1): xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 18.9 ms                                                | 18.5 ms: 1.02x faster                                                  |
| python_startup_no_site | 14.5 ms                                                | 14.4 ms: 1.01x faster                                                  |
| Geometric mean         | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 7.68 ms                                                | 6.44 ms: 1.19x faster                                                  |
| genshi_text     | 16.9 ms                                                | 17.1 ms: 1.01x slower                                                  |
| django_template | 22.2 ms                                                | 23.0 ms: 1.03x slower                                                  |
| genshi_xml      | 34.4 ms                                                | 42.6 ms: 1.24x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.02x slower                                                           |

All benchmarks:
===============

| Benchmark                        | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241022-darwin-arm64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| deepcopy_memo                    | 27.4 us                                                | 16.5 us: 1.66x faster                                                  |
| deepcopy                         | 237 us                                                 | 156 us: 1.52x faster                                                   |
| deepcopy_reduce                  | 2.07 us                                                | 1.53 us: 1.35x faster                                                  |
| tomli_loads                      | 1.57 sec                                               | 1.25 sec: 1.26x faster                                                 |
| generators                       | 31.5 ms                                                | 25.3 ms: 1.25x faster                                                  |
| async_tree_memoization_tg        | 288 ms                                                 | 234 ms: 1.23x faster                                                   |
| unpickle_pure_python             | 164 us                                                 | 133 us: 1.23x faster                                                   |
| scimark_sor                      | 105 ms                                                 | 85.9 ms: 1.23x faster                                                  |
| pickle_pure_python               | 214 us                                                 | 178 us: 1.20x faster                                                   |
| coroutines                       | 19.8 ms                                                | 16.4 ms: 1.20x faster                                                  |
| mako                             | 7.68 ms                                                | 6.44 ms: 1.19x faster                                                  |
| scimark_lu                       | 76.7 ms                                                | 64.5 ms: 1.19x faster                                                  |
| xml_etree_process                | 41.0 ms                                                | 34.6 ms: 1.18x faster                                                  |
| float                            | 56.0 ms                                                | 48.3 ms: 1.16x faster                                                  |
| xml_etree_generate               | 57.0 ms                                                | 49.4 ms: 1.15x faster                                                  |
| go                               | 115 ms                                                 | 100 ms: 1.15x faster                                                   |
| logging_simple                   | 3.60 us                                                | 3.17 us: 1.14x faster                                                  |
| logging_format                   | 3.89 us                                                | 3.44 us: 1.13x faster                                                  |
| async_tree_eager_tg              | 47.8 ms                                                | 42.4 ms: 1.13x faster                                                  |
| nbody                            | 74.0 ms                                                | 65.9 ms: 1.12x faster                                                  |
| html5lib                         | 36.6 ms                                                | 32.8 ms: 1.11x faster                                                  |
| scimark_monte_carlo              | 50.4 ms                                                | 45.3 ms: 1.11x faster                                                  |
| deltablue                        | 2.68 ms                                                | 2.41 ms: 1.11x faster                                                  |
| async_tree_eager_memoization     | 168 ms                                                 | 153 ms: 1.10x faster                                                   |
| async_tree_eager                 | 70.1 ms                                                | 63.5 ms: 1.10x faster                                                  |
| async_tree_memoization           | 268 ms                                                 | 244 ms: 1.10x faster                                                   |
| spectral_norm                    | 76.3 ms                                                | 69.7 ms: 1.09x faster                                                  |
| thrift                           | 466 us                                                 | 426 us: 1.09x faster                                                   |
| pprint_pformat                   | 1.08 sec                                               | 991 ms: 1.09x faster                                                   |
| async_tree_none                  | 215 ms                                                 | 198 ms: 1.09x faster                                                   |
| pyflate                          | 351 ms                                                 | 326 ms: 1.08x faster                                                   |
| regex_effbot                     | 2.63 ms                                                | 2.45 ms: 1.07x faster                                                  |
| bpe_tokeniser                    | 3.25 sec                                               | 3.04 sec: 1.07x faster                                                 |
| fannkuch                         | 284 ms                                                 | 265 ms: 1.07x faster                                                   |
| bench_thread_pool                | 505 us                                                 | 473 us: 1.07x faster                                                   |
| nqueens                          | 62.5 ms                                                | 58.6 ms: 1.07x faster                                                  |
| pprint_safe_repr                 | 533 ms                                                 | 500 ms: 1.07x faster                                                   |
| telco                            | 4.76 ms                                                | 4.48 ms: 1.06x faster                                                  |
| raytrace                         | 181 ms                                                 | 170 ms: 1.06x faster                                                   |
| async_tree_eager_memoization_tg  | 138 ms                                                 | 130 ms: 1.06x faster                                                   |
| scimark_fft                      | 201 ms                                                 | 190 ms: 1.05x faster                                                   |
| coverage                         | 46.0 ms                                                | 43.7 ms: 1.05x faster                                                  |
| regex_compile                    | 78.6 ms                                                | 74.9 ms: 1.05x faster                                                  |
| regex_dna                        | 149 ms                                                 | 142 ms: 1.05x faster                                                   |
| pathlib                          | 23.4 ms                                                | 22.4 ms: 1.04x faster                                                  |
| json                             | 3.03 ms                                                | 2.91 ms: 1.04x faster                                                  |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 335 ms: 1.03x faster                                                   |
| pycparser                        | 705 ms                                                 | 683 ms: 1.03x faster                                                   |
| richards_super                   | 39.1 ms                                                | 37.9 ms: 1.03x faster                                                  |
| typing_runtime_protocols         | 101 us                                                 | 98.1 us: 1.03x faster                                                  |
| richards                         | 35.2 ms                                                | 34.1 ms: 1.03x faster                                                  |
| async_tree_eager_cpu_io_mixed    | 373 ms                                                 | 362 ms: 1.03x faster                                                   |
| python_startup                   | 18.9 ms                                                | 18.5 ms: 1.02x faster                                                  |
| json_loads                       | 17.0 us                                                | 16.6 us: 1.02x faster                                                  |
| sqlalchemy_imperative            | 6.69 ms                                                | 6.55 ms: 1.02x faster                                                  |
| 2to3                             | 187 ms                                                 | 183 ms: 1.02x faster                                                   |
| regex_v8                         | 17.0 ms                                                | 16.7 ms: 1.02x faster                                                  |
| sqlglot_normalize                | 189 ms                                                 | 187 ms: 1.01x faster                                                   |
| async_generators                 | 295 ms                                                 | 292 ms: 1.01x faster                                                   |
| bench_mp_pool                    | 62.5 ms                                                | 62.0 ms: 1.01x faster                                                  |
| python_startup_no_site           | 14.5 ms                                                | 14.4 ms: 1.01x faster                                                  |
| xml_etree_iterparse              | 73.6 ms                                                | 73.0 ms: 1.01x faster                                                  |
| pidigits                         | 284 ms                                                 | 283 ms: 1.00x faster                                                   |
| asyncio_websockets               | 242 ms                                                 | 243 ms: 1.00x slower                                                   |
| crypto_pyaes                     | 54.2 ms                                                | 54.5 ms: 1.00x slower                                                  |
| meteor_contest                   | 74.0 ms                                                | 74.4 ms: 1.01x slower                                                  |
| dulwich_log                      | 28.5 ms                                                | 28.8 ms: 1.01x slower                                                  |
| gc_traversal                     | 2.91 ms                                                | 2.94 ms: 1.01x slower                                                  |
| chaos                            | 41.2 ms                                                | 41.6 ms: 1.01x slower                                                  |
| genshi_text                      | 16.9 ms                                                | 17.1 ms: 1.01x slower                                                  |
| sqlglot_parse                    | 856 us                                                 | 879 us: 1.03x slower                                                   |
| hexiom                           | 4.86 ms                                                | 5.01 ms: 1.03x slower                                                  |
| django_template                  | 22.2 ms                                                | 23.0 ms: 1.03x slower                                                  |
| mdp                              | 1.49 sec                                               | 1.55 sec: 1.04x slower                                                 |
| sympy_str                        | 145 ms                                                 | 152 ms: 1.04x slower                                                   |
| async_tree_cpu_io_mixed_tg       | 448 ms                                                 | 470 ms: 1.05x slower                                                   |
| sqlalchemy_declarative           | 58.9 ms                                                | 61.8 ms: 1.05x slower                                                  |
| sqlglot_transpile                | 1.02 ms                                                | 1.08 ms: 1.05x slower                                                  |
| sympy_sum                        | 75.4 ms                                                | 79.8 ms: 1.06x slower                                                  |
| scimark_sparse_mat_mult          | 2.99 ms                                                | 3.16 ms: 1.06x slower                                                  |
| sqlglot_optimize                 | 34.9 ms                                                | 37.4 ms: 1.07x slower                                                  |
| docutils                         | 1.44 sec                                               | 1.56 sec: 1.09x slower                                                 |
| json_dumps                       | 6.52 ms                                                | 7.15 ms: 1.10x slower                                                  |
| sphinx                           | 603 ms                                                 | 663 ms: 1.10x slower                                                   |
| async_tree_none_tg               | 198 ms                                                 | 218 ms: 1.10x slower                                                   |
| sympy_integrate                  | 11.3 ms                                                | 12.5 ms: 1.10x slower                                                  |
| create_gc_cycles                 | 1.17 ms                                                | 1.33 ms: 1.13x slower                                                  |
| async_tree_io                    | 507 ms                                                 | 581 ms: 1.15x slower                                                   |
| pylint                           | 180 ms                                                 | 213 ms: 1.19x slower                                                   |
| async_tree_io_tg                 | 497 ms                                                 | 611 ms: 1.23x slower                                                   |
| genshi_xml                       | 34.4 ms                                                | 42.6 ms: 1.24x slower                                                  |
| async_tree_eager_io              | 514 ms                                                 | 666 ms: 1.30x slower                                                   |
| comprehensions                   | 12.3 us                                                | 17.0 us: 1.39x slower                                                  |
| async_tree_eager_io_tg           | 477 ms                                                 | 714 ms: 1.50x slower                                                   |
| Geometric mean                   | (ref)                                                  | 1.04x faster                                                           |

Benchmark hidden because not significant (5): tornado_http, async_tree_cpu_io_mixed, logging_silent, sympy_expand, xml_etree_parse
Ignored benchmarks (7) of results/bm-20241007-3.13.0-60403a5/bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, dask, gevent_hub, k_core, mypy2, shortest_path

- Geometric mean (including insignificant results): 1.037x faster
# HPT report

- Reliability score: 99.67% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.09x