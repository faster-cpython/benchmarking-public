# Results vs. 3.12.0

- fork: python
- ref: 79c542b5cc774ba758ac
- machine: linux-x86_64
- commit hash: 79c542b
- commit date: 2024-08-17
- overall geometric mean: 1.36x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.22x slower
- Memory change: 1.13x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 274 ms                                                 | 395 ms: 1.44x slower                                                  |
| docutils       | 2.77 sec                                               | 3.33 sec: 1.20x slower                                                |
| tornado_http   | 103 ms                                                 | 139 ms: 1.35x slower                                                  |
| Geometric mean | (ref)                                                  | 1.33x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 1.18 sec                                               | 839 ms: 1.41x faster                                                  |
| async_tree_none_tg         | 450 ms                                                 | 347 ms: 1.29x faster                                                  |
| async_tree_io              | 1.16 sec                                               | 899 ms: 1.29x faster                                                  |
| async_tree_memoization_tg  | 575 ms                                                 | 452 ms: 1.27x faster                                                  |
| async_tree_cpu_io_mixed_tg | 726 ms                                                 | 601 ms: 1.21x faster                                                  |
| async_tree_none            | 472 ms                                                 | 403 ms: 1.17x faster                                                  |
| async_tree_memoization     | 578 ms                                                 | 507 ms: 1.14x faster                                                  |
| async_tree_cpu_io_mixed    | 726 ms                                                 | 659 ms: 1.10x faster                                                  |
| Geometric mean             | (ref)                                                  | 1.23x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 187 ms                                                 | 184 ms: 1.02x faster                                                  |
| float          | 84.7 ms                                                | 140 ms: 1.65x slower                                                  |
| nbody          | 97.0 ms                                                | 220 ms: 2.27x slower                                                  |
| Geometric mean | (ref)                                                  | 1.54x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.61 ms                                                | 3.45 ms: 1.05x faster                                                 |
| regex_dna      | 212 ms                                                 | 225 ms: 1.06x slower                                                  |
| regex_v8       | 23.1 ms                                                | 26.0 ms: 1.12x slower                                                 |
| regex_compile  | 148 ms                                                 | 217 ms: 1.46x slower                                                  |
| Geometric mean | (ref)                                                  | 1.14x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| xml_etree_parse      | 159 ms                                                 | 151 ms: 1.05x faster                                                  |
| xml_etree_iterparse  | 107 ms                                                 | 112 ms: 1.05x slower                                                  |
| json_loads           | 28.5 us                                                | 32.5 us: 1.14x slower                                                 |
| xml_etree_generate   | 89.2 ms                                                | 111 ms: 1.24x slower                                                  |
| json_dumps           | 10.4 ms                                                | 13.8 ms: 1.33x slower                                                 |
| tomli_loads          | 2.33 sec                                               | 3.24 sec: 1.39x slower                                                |
| xml_etree_process    | 61.7 ms                                                | 89.1 ms: 1.44x slower                                                 |
| unpickle_pure_python | 230 us                                                 | 405 us: 1.76x slower                                                  |
| pickle_pure_python   | 324 us                                                 | 582 us: 1.79x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.32x slower                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 6.94 ms                                                | 9.38 ms: 1.35x slower                                                 |
| python_startup         | 9.55 ms                                                | 13.8 ms: 1.44x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.40x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.6 ms                                                | 59.9 ms: 1.73x slower                                                 |
| mako            | 11.9 ms                                                | 20.9 ms: 1.76x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.74x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 1.18 sec                                               | 839 ms: 1.41x faster                                                  |
| gc_traversal               | 3.79 ms                                                | 2.70 ms: 1.41x faster                                                 |
| async_tree_none_tg         | 450 ms                                                 | 347 ms: 1.29x faster                                                  |
| async_tree_io              | 1.16 sec                                               | 899 ms: 1.29x faster                                                  |
| async_tree_memoization_tg  | 575 ms                                                 | 452 ms: 1.27x faster                                                  |
| async_tree_cpu_io_mixed_tg | 726 ms                                                 | 601 ms: 1.21x faster                                                  |
| async_tree_none            | 472 ms                                                 | 403 ms: 1.17x faster                                                  |
| async_tree_memoization     | 578 ms                                                 | 507 ms: 1.14x faster                                                  |
| async_tree_cpu_io_mixed    | 726 ms                                                 | 659 ms: 1.10x faster                                                  |
| create_gc_cycles           | 1.48 ms                                                | 1.37 ms: 1.08x faster                                                 |
| xml_etree_parse            | 159 ms                                                 | 151 ms: 1.05x faster                                                  |
| regex_effbot               | 3.61 ms                                                | 3.45 ms: 1.05x faster                                                 |
| pathlib                    | 19.3 ms                                                | 19.0 ms: 1.02x faster                                                 |
| pidigits                   | 187 ms                                                 | 184 ms: 1.02x faster                                                  |
| bench_mp_pool              | 24.0 ms                                                | 23.9 ms: 1.00x faster                                                 |
| asyncio_websockets         | 551 ms                                                 | 554 ms: 1.00x slower                                                  |
| xml_etree_iterparse        | 107 ms                                                 | 112 ms: 1.05x slower                                                  |
| regex_dna                  | 212 ms                                                 | 225 ms: 1.06x slower                                                  |
| regex_v8                   | 23.1 ms                                                | 26.0 ms: 1.12x slower                                                 |
| deepcopy                   | 371 us                                                 | 421 us: 1.13x slower                                                  |
| json_loads                 | 28.5 us                                                | 32.5 us: 1.14x slower                                                 |
| json                       | 5.26 ms                                                | 6.00 ms: 1.14x slower                                                 |
| asyncio_tcp                | 491 ms                                                 | 562 ms: 1.15x slower                                                  |
| asyncio_tcp_ssl            | 1.79 sec                                               | 2.06 sec: 1.16x slower                                                |
| generators                 | 31.2 ms                                                | 36.4 ms: 1.16x slower                                                 |
| async_generators           | 463 ms                                                 | 556 ms: 1.20x slower                                                  |
| docutils                   | 2.77 sec                                               | 3.33 sec: 1.20x slower                                                |
| xml_etree_generate         | 89.2 ms                                                | 111 ms: 1.24x slower                                                  |
| scimark_fft                | 382 ms                                                 | 479 ms: 1.25x slower                                                  |
| mdp                        | 2.63 sec                                               | 3.35 sec: 1.27x slower                                                |
| deepcopy_memo              | 40.7 us                                                | 52.1 us: 1.28x slower                                                 |
| meteor_contest             | 112 ms                                                 | 144 ms: 1.28x slower                                                  |
| deepcopy_reduce            | 3.35 us                                                | 4.37 us: 1.31x slower                                                 |
| comprehensions             | 21.8 us                                                | 28.5 us: 1.31x slower                                                 |
| scimark_sparse_mat_mult    | 5.06 ms                                                | 6.63 ms: 1.31x slower                                                 |
| json_dumps                 | 10.4 ms                                                | 13.8 ms: 1.33x slower                                                 |
| pycparser                  | 1.18 sec                                               | 1.58 sec: 1.34x slower                                                |
| sympy_integrate            | 21.4 ms                                                | 28.8 ms: 1.34x slower                                                 |
| python_startup_no_site     | 6.94 ms                                                | 9.38 ms: 1.35x slower                                                 |
| tornado_http               | 103 ms                                                 | 139 ms: 1.35x slower                                                  |
| crypto_pyaes               | 81.9 ms                                                | 113 ms: 1.37x slower                                                  |
| tomli_loads                | 2.33 sec                                               | 3.24 sec: 1.39x slower                                                |
| sympy_str                  | 300 ms                                                 | 422 ms: 1.41x slower                                                  |
| nqueens                    | 83.3 ms                                                | 118 ms: 1.41x slower                                                  |
| coroutines                 | 23.2 ms                                                | 33.1 ms: 1.43x slower                                                 |
| 2to3                       | 274 ms                                                 | 395 ms: 1.44x slower                                                  |
| python_startup             | 9.55 ms                                                | 13.8 ms: 1.44x slower                                                 |
| xml_etree_process          | 61.7 ms                                                | 89.1 ms: 1.44x slower                                                 |
| telco                      | 7.10 ms                                                | 10.3 ms: 1.44x slower                                                 |
| regex_compile              | 148 ms                                                 | 217 ms: 1.46x slower                                                  |
| fannkuch                   | 417 ms                                                 | 614 ms: 1.47x slower                                                  |
| sympy_sum                  | 169 ms                                                 | 255 ms: 1.51x slower                                                  |
| coverage                   | 72.7 ms                                                | 111 ms: 1.53x slower                                                  |
| sympy_expand               | 478 ms                                                 | 740 ms: 1.55x slower                                                  |
| sqlglot_normalize          | 110 ms                                                 | 172 ms: 1.56x slower                                                  |
| pyflate                    | 482 ms                                                 | 762 ms: 1.58x slower                                                  |
| sqlglot_optimize           | 54.8 ms                                                | 86.9 ms: 1.58x slower                                                 |
| typing_runtime_protocols   | 158 us                                                 | 253 us: 1.60x slower                                                  |
| spectral_norm              | 115 ms                                                 | 185 ms: 1.61x slower                                                  |
| logging_simple             | 6.46 us                                                | 10.4 us: 1.62x slower                                                 |
| logging_format             | 7.23 us                                                | 11.7 us: 1.62x slower                                                 |
| pprint_safe_repr           | 776 ms                                                 | 1.27 sec: 1.64x slower                                                |
| float                      | 84.7 ms                                                | 140 ms: 1.65x slower                                                  |
| scimark_monte_carlo        | 75.1 ms                                                | 126 ms: 1.67x slower                                                  |
| pprint_pformat             | 1.57 sec                                               | 2.66 sec: 1.69x slower                                                |
| richards                   | 45.8 ms                                                | 78.8 ms: 1.72x slower                                                 |
| django_template            | 34.6 ms                                                | 59.9 ms: 1.73x slower                                                 |
| mako                       | 11.9 ms                                                | 20.9 ms: 1.76x slower                                                 |
| unpickle_pure_python       | 230 us                                                 | 405 us: 1.76x slower                                                  |
| pickle_pure_python         | 324 us                                                 | 582 us: 1.79x slower                                                  |
| richards_super             | 51.5 ms                                                | 93.5 ms: 1.81x slower                                                 |
| sqlglot_transpile          | 1.68 ms                                                | 3.11 ms: 1.85x slower                                                 |
| hexiom                     | 6.41 ms                                                | 11.9 ms: 1.86x slower                                                 |
| logging_silent             | 104 ns                                                 | 195 ns: 1.87x slower                                                  |
| chaos                      | 67.0 ms                                                | 125 ms: 1.87x slower                                                  |
| sqlglot_parse              | 1.36 ms                                                | 2.61 ms: 1.91x slower                                                 |
| raytrace                   | 312 ms                                                 | 614 ms: 1.97x slower                                                  |
| scimark_lu                 | 118 ms                                                 | 234 ms: 1.98x slower                                                  |
| scimark_sor                | 129 ms                                                 | 270 ms: 2.09x slower                                                  |
| go                         | 139 ms                                                 | 306 ms: 2.19x slower                                                  |
| deltablue                  | 3.72 ms                                                | 8.36 ms: 2.25x slower                                                 |
| nbody                      | 97.0 ms                                                | 220 ms: 2.27x slower                                                  |
| bench_thread_pool          | 842 us                                                 | 3.12 ms: 3.70x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.36x slower                                                          |
Ignored benchmarks (15) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-linux-x86_64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, dulwich_log, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20240817-3.14.0a0-79c542b-NOGIL/bm-20240817-linux-x86_64-python-79c542b5cc774ba758ac-3.14.0a0-79c542b.json: bpe_tokeniser, genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.28x
- 95% likely to have a slowdown of 1.27x
- 99% likely to have a slowdown of 1.22x

# Memory
- memory change: 1.13x