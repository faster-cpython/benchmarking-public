
# Results vs. 3.10.4

- fork: python
- ref: v3.11.0b3
- machine: linux-x86_64
- commit hash: eb0004c
- commit date: 2022-06-01
- overall geometric mean: 1.21x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.16x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| 2to3           | 350 ms                                                       | 284 ms: 1.23x faster                                             |
| chameleon      | 9.72 ms                                                      | 7.60 ms: 1.28x faster                                            |
| docutils       | 3.40 sec                                                     | 2.83 sec: 1.20x faster                                           |
| html5lib       | 94.6 ms                                                      | 71.0 ms: 1.33x faster                                            |
| tornado_http   | 152 ms                                                       | 119 ms: 1.28x faster                                             |
| Geometric mean | (ref)                                                        | 1.26x faster                                                     |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| float          | 110 ms                                                       | 73.9 ms: 1.49x faster                                            |
| nbody          | 137 ms                                                       | 93.0 ms: 1.48x faster                                            |
| pidigits       | 271 ms                                                       | 252 ms: 1.07x faster                                             |
| Geometric mean | (ref)                                                        | 1.33x faster                                                     |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| regex_compile  | 194 ms                                                       | 156 ms: 1.24x faster                                             |
| regex_dna      | 259 ms                                                       | 219 ms: 1.18x faster                                             |
| regex_v8       | 26.6 ms                                                      | 24.3 ms: 1.10x faster                                            |
| regex_effbot   | 2.99 ms                                                      | 3.05 ms: 1.02x slower                                            |
| Geometric mean | (ref)                                                        | 1.12x faster                                                     |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| pickle_pure_python   | 457 us                                                       | 319 us: 1.43x faster                                             |
| xml_etree_process    | 76.0 ms                                                      | 55.9 ms: 1.36x faster                                            |
| unpickle_pure_python | 321 us                                                       | 238 us: 1.35x faster                                             |
| json_loads           | 30.0 us                                                      | 25.2 us: 1.19x faster                                            |
| xml_etree_generate   | 94.6 ms                                                      | 80.2 ms: 1.18x faster                                            |
| unpickle             | 14.2 us                                                      | 13.2 us: 1.07x faster                                            |
| xml_etree_iterparse  | 110 ms                                                       | 103 ms: 1.07x faster                                             |
| json_dumps           | 14.2 ms                                                      | 13.5 ms: 1.05x faster                                            |
| xml_etree_parse      | 162 ms                                                       | 153 ms: 1.05x faster                                             |
| pickle_list          | 4.11 us                                                      | 3.92 us: 1.05x faster                                            |
| pickle               | 9.94 us                                                      | 9.80 us: 1.01x faster                                            |
| unpickle_list        | 4.49 us                                                      | 4.58 us: 1.02x slower                                            |
| pickle_dict          | 30.0 us                                                      | 31.3 us: 1.04x slower                                            |
| Geometric mean       | (ref)                                                        | 1.13x faster                                                     |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| python_startup         | 11.5 ms                                                      | 10.7 ms: 1.08x faster                                            |
| python_startup_no_site | 7.32 ms                                                      | 7.70 ms: 1.05x slower                                            |
| Geometric mean         | (ref)                                                        | 1.01x faster                                                     |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| mako            | 14.7 ms                                                      | 10.9 ms: 1.34x faster                                            |
| django_template | 51.5 ms                                                      | 41.4 ms: 1.25x faster                                            |
| genshi_text     | 31.5 ms                                                      | 26.3 ms: 1.19x faster                                            |
| genshi_xml      | 64.7 ms                                                      | 60.1 ms: 1.08x faster                                            |
| Geometric mean  | (ref)                                                        | 1.21x faster                                                     |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20220601-pythonperf2-x86_64-python-v3.11.0b3-3.11.0b3-eb0004c |
|-------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------:|
| deltablue               | 7.47 ms                                                      | 4.20 ms: 1.78x faster                                            |
| logging_silent          | 166 ns                                                       | 99.3 ns: 1.67x faster                                            |
| raytrace                | 488 ms                                                       | 304 ms: 1.61x faster                                             |
| bench_mp_pool           | 7.18 ms                                                      | 4.52 ms: 1.59x faster                                            |
| pyflate                 | 697 ms                                                       | 441 ms: 1.58x faster                                             |
| scimark_sor             | 177 ms                                                       | 112 ms: 1.58x faster                                             |
| scimark_monte_carlo     | 109 ms                                                       | 69.5 ms: 1.57x faster                                            |
| go                      | 259 ms                                                       | 166 ms: 1.56x faster                                             |
| richards                | 74.1 ms                                                      | 49.2 ms: 1.51x faster                                            |
| float                   | 110 ms                                                       | 73.9 ms: 1.49x faster                                            |
| scimark_lu              | 164 ms                                                       | 110 ms: 1.49x faster                                             |
| nbody                   | 137 ms                                                       | 93.0 ms: 1.48x faster                                            |
| chaos                   | 107 ms                                                       | 73.6 ms: 1.46x faster                                            |
| pickle_pure_python      | 457 us                                                       | 319 us: 1.43x faster                                             |
| spectral_norm           | 136 ms                                                       | 96.8 ms: 1.41x faster                                            |
| crypto_pyaes            | 118 ms                                                       | 84.5 ms: 1.40x faster                                            |
| async_tree_io           | 1.61 sec                                                     | 1.17 sec: 1.38x faster                                           |
| async_tree_none         | 700 ms                                                       | 513 ms: 1.36x faster                                             |
| hexiom                  | 9.52 ms                                                      | 7.00 ms: 1.36x faster                                            |
| xml_etree_process       | 76.0 ms                                                      | 55.9 ms: 1.36x faster                                            |
| pprint_safe_repr        | 1.05 sec                                                     | 774 ms: 1.36x faster                                             |
| unpickle_pure_python    | 321 us                                                       | 238 us: 1.35x faster                                             |
| mako                    | 14.7 ms                                                      | 10.9 ms: 1.34x faster                                            |
| pprint_pformat          | 2.15 sec                                                     | 1.61 sec: 1.34x faster                                           |
| async_tree_memoization  | 824 ms                                                       | 617 ms: 1.33x faster                                             |
| html5lib                | 94.6 ms                                                      | 71.0 ms: 1.33x faster                                            |
| async_generators        | 422 ms                                                       | 318 ms: 1.33x faster                                             |
| unpack_sequence         | 59.5 ns                                                      | 45.4 ns: 1.31x faster                                            |
| deepcopy_memo           | 48.9 us                                                      | 37.7 us: 1.30x faster                                            |
| scimark_sparse_mat_mult | 5.19 ms                                                      | 4.03 ms: 1.29x faster                                            |
| tornado_http            | 152 ms                                                       | 119 ms: 1.28x faster                                             |
| thrift                  | 1.16 ms                                                      | 909 us: 1.28x faster                                             |
| chameleon               | 9.72 ms                                                      | 7.60 ms: 1.28x faster                                            |
| pycparser               | 1.66 sec                                                     | 1.31 sec: 1.27x faster                                           |
| aiohttp                 | 1.21 ms                                                      | 958 us: 1.26x faster                                             |
| gunicorn                | 1.17 ms                                                      | 931 us: 1.26x faster                                             |
| scimark_fft             | 359 ms                                                       | 286 ms: 1.26x faster                                             |
| django_template         | 51.5 ms                                                      | 41.4 ms: 1.25x faster                                            |
| regex_compile           | 194 ms                                                       | 156 ms: 1.24x faster                                             |
| logging_simple          | 8.90 us                                                      | 7.16 us: 1.24x faster                                            |
| 2to3                    | 350 ms                                                       | 284 ms: 1.23x faster                                             |
| sqlalchemy_declarative  | 187 ms                                                       | 154 ms: 1.21x faster                                             |
| logging_format          | 9.57 us                                                      | 7.95 us: 1.20x faster                                            |
| docutils                | 3.40 sec                                                     | 2.83 sec: 1.20x faster                                           |
| genshi_text             | 31.5 ms                                                      | 26.3 ms: 1.19x faster                                            |
| json_loads              | 30.0 us                                                      | 25.2 us: 1.19x faster                                            |
| regex_dna               | 259 ms                                                       | 219 ms: 1.18x faster                                             |
| sqlglot_optimize        | 70.3 ms                                                      | 59.4 ms: 1.18x faster                                            |
| sqlite_synth            | 2.97 us                                                      | 2.51 us: 1.18x faster                                            |
| xml_etree_generate      | 94.6 ms                                                      | 80.2 ms: 1.18x faster                                            |
| async_tree_cpu_io_mixed | 952 ms                                                       | 810 ms: 1.18x faster                                             |
| sqlglot_normalize       | 144 ms                                                       | 123 ms: 1.17x faster                                             |
| dulwich_log             | 80.1 ms                                                      | 68.4 ms: 1.17x faster                                            |
| sqlglot_transpile       | 2.71 ms                                                      | 2.32 ms: 1.17x faster                                            |
| deepcopy                | 454 us                                                       | 390 us: 1.16x faster                                             |
| deepcopy_reduce         | 4.03 us                                                      | 3.48 us: 1.16x faster                                            |
| sqlalchemy_imperative   | 22.6 ms                                                      | 19.6 ms: 1.16x faster                                            |
| nqueens                 | 112 ms                                                       | 97.6 ms: 1.15x faster                                            |
| flaskblogging           | 4.39 ms                                                      | 3.82 ms: 1.15x faster                                            |
| json                    | 5.96 ms                                                      | 5.20 ms: 1.15x faster                                            |
| sqlglot_parse           | 2.26 ms                                                      | 1.97 ms: 1.15x faster                                            |
| pathlib                 | 21.7 ms                                                      | 19.2 ms: 1.13x faster                                            |
| sympy_expand            | 599 ms                                                       | 531 ms: 1.13x faster                                             |
| bench_thread_pool       | 1.14 ms                                                      | 1.01 ms: 1.12x faster                                            |
| dask                    | 463 ms                                                       | 414 ms: 1.12x faster                                             |
| sympy_integrate         | 28.1 ms                                                      | 25.2 ms: 1.11x faster                                            |
| regex_v8                | 26.6 ms                                                      | 24.3 ms: 1.10x faster                                            |
| sympy_str               | 358 ms                                                       | 327 ms: 1.09x faster                                             |
| coroutines              | 30.4 ms                                                      | 27.9 ms: 1.09x faster                                            |
| pylint                  | 562 ms                                                       | 515 ms: 1.09x faster                                             |
| sympy_sum               | 193 ms                                                       | 177 ms: 1.09x faster                                             |
| create_gc_cycles        | 1.82 ms                                                      | 1.68 ms: 1.08x faster                                            |
| mdp                     | 3.03 sec                                                     | 2.81 sec: 1.08x faster                                           |
| genshi_xml              | 64.7 ms                                                      | 60.1 ms: 1.08x faster                                            |
| python_startup          | 11.5 ms                                                      | 10.7 ms: 1.08x faster                                            |
| pidigits                | 271 ms                                                       | 252 ms: 1.07x faster                                             |
| unpickle                | 14.2 us                                                      | 13.2 us: 1.07x faster                                            |
| xml_etree_iterparse     | 110 ms                                                       | 103 ms: 1.07x faster                                             |
| mypy2                   | 466 ms                                                       | 438 ms: 1.06x faster                                             |
| telco                   | 7.14 ms                                                      | 6.75 ms: 1.06x faster                                            |
| json_dumps              | 14.2 ms                                                      | 13.5 ms: 1.05x faster                                            |
| xml_etree_parse         | 162 ms                                                       | 153 ms: 1.05x faster                                             |
| pickle_list             | 4.11 us                                                      | 3.92 us: 1.05x faster                                            |
| meteor_contest          | 137 ms                                                       | 131 ms: 1.04x faster                                             |
| asyncio_tcp             | 782 ms                                                       | 751 ms: 1.04x faster                                             |
| generators              | 58.0 ms                                                      | 56.0 ms: 1.04x faster                                            |
| pickle                  | 9.94 us                                                      | 9.80 us: 1.01x faster                                            |
| regex_effbot            | 2.99 ms                                                      | 3.05 ms: 1.02x slower                                            |
| unpickle_list           | 4.49 us                                                      | 4.58 us: 1.02x slower                                            |
| pickle_dict             | 30.0 us                                                      | 31.3 us: 1.04x slower                                            |
| comprehensions          | 26.9 us                                                      | 28.3 us: 1.05x slower                                            |
| python_startup_no_site  | 7.32 ms                                                      | 7.70 ms: 1.05x slower                                            |
| gc_traversal            | 3.45 ms                                                      | 4.03 ms: 1.17x slower                                            |
| Geometric mean          | (ref)                                                        | 1.21x faster                                                     |

Benchmark hidden because not significant (1): fannkuch
Ignored benchmarks (5) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120.json: asyncio_tcp_ssl, coverage, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.18x
- 95% likely to have a speedup of 1.17x
- 99% likely to have a speedup of 1.16x
