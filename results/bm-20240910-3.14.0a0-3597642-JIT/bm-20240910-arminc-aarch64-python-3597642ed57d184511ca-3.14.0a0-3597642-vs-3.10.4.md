# Results vs. 3.10.4

- fork: python
- ref: 3597642ed57d184511ca
- machine: linux-aarch64
- commit hash: 3597642
- commit date: 2024-09-10
- overall geometric mean: 1.177x faster
- HPT reliability: 99.99%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.25x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| docutils       | 3.53 sec                                                              | 3.94 sec: 1.12x slower                                                  |
| html5lib       | 86.5 ms                                                               | 72.3 ms: 1.20x faster                                                   |
| tornado_http   | 178 ms                                                                | 146 ms: 1.22x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.07x faster                                                            |

Benchmark hidden because not significant (1): 2to3

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 447 ms: 2.12x faster                                                    |
| async_tree_io           | 2.28 sec                                                              | 1.15 sec: 1.98x faster                                                  |
| async_tree_memoization  | 1.13 sec                                                              | 588 ms: 1.93x faster                                                    |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 755 ms: 1.69x faster                                                    |
| Geometric mean          | (ref)                                                                 | 1.92x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 135 ms                                                                | 87.6 ms: 1.54x faster                                                   |
| nbody          | 166 ms                                                                | 114 ms: 1.45x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.31x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_dna      | 257 ms                                                                | 245 ms: 1.05x faster                                                    |
| regex_v8       | 32.2 ms                                                               | 30.7 ms: 1.05x faster                                                   |
| regex_compile  | 177 ms                                                                | 190 ms: 1.07x slower                                                    |
| regex_effbot   | 4.25 ms                                                               | 4.91 ms: 1.16x slower                                                   |
| Geometric mean | (ref)                                                                 | 1.03x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 265 us: 1.38x faster                                                    |
| pickle_pure_python   | 529 us                                                                | 388 us: 1.37x faster                                                    |
| tomli_loads          | 3.36 sec                                                              | 2.63 sec: 1.28x faster                                                  |
| json_dumps           | 16.7 ms                                                               | 13.5 ms: 1.24x faster                                                   |
| xml_etree_process    | 99.5 ms                                                               | 80.8 ms: 1.23x faster                                                   |
| xml_etree_parse      | 212 ms                                                                | 189 ms: 1.12x faster                                                    |
| xml_etree_generate   | 123 ms                                                                | 111 ms: 1.11x faster                                                    |
| unpickle_list        | 6.99 us                                                               | 6.44 us: 1.09x faster                                                   |
| json_loads           | 30.9 us                                                               | 31.4 us: 1.02x slower                                                   |
| pickle_dict          | 35.1 us                                                               | 38.1 us: 1.08x slower                                                   |
| pickle               | 12.5 us                                                               | 13.8 us: 1.10x slower                                                   |
| unpickle             | 17.5 us                                                               | 19.8 us: 1.13x slower                                                   |
| Geometric mean       | (ref)                                                                 | 1.10x faster                                                            |

Benchmark hidden because not significant (2): xml_etree_iterparse, pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 13.0 ms: 1.16x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 8.72 ms: 1.27x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.21x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                   |
| django_template | 53.3 ms                                                               | 51.2 ms: 1.04x faster                                                   |
| genshi_text     | 35.2 ms                                                               | 34.7 ms: 1.01x faster                                                   |
| genshi_xml      | 69.8 ms                                                               | 81.2 ms: 1.16x slower                                                   |
| Geometric mean  | (ref)                                                                 | 1.07x faster                                                            |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 209 us: 3.16x faster                                                    |
| async_tree_none          | 950 ms                                                                | 447 ms: 2.12x faster                                                    |
| deltablue                | 8.94 ms                                                               | 4.35 ms: 2.05x faster                                                   |
| async_tree_io            | 2.28 sec                                                              | 1.15 sec: 1.98x faster                                                  |
| async_tree_memoization   | 1.13 sec                                                              | 588 ms: 1.93x faster                                                    |
| bench_mp_pool            | 14.5 ms                                                               | 7.58 ms: 1.92x faster                                                   |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 755 ms: 1.69x faster                                                    |
| deepcopy_memo            | 61.7 us                                                               | 37.5 us: 1.65x faster                                                   |
| raytrace                 | 573 ms                                                                | 349 ms: 1.64x faster                                                    |
| scimark_sor              | 246 ms                                                                | 151 ms: 1.63x faster                                                    |
| logging_silent           | 222 ns                                                                | 137 ns: 1.62x faster                                                    |
| float                    | 135 ms                                                                | 87.6 ms: 1.54x faster                                                   |
| scimark_lu               | 227 ms                                                                | 148 ms: 1.53x faster                                                    |
| asyncio_tcp              | 944 ms                                                                | 627 ms: 1.51x faster                                                    |
| crypto_pyaes             | 134 ms                                                                | 90.7 ms: 1.48x faster                                                   |
| nbody                    | 166 ms                                                                | 114 ms: 1.45x faster                                                    |
| mako                     | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                   |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.28 sec: 1.44x faster                                                  |
| go                       | 264 ms                                                                | 186 ms: 1.42x faster                                                    |
| richards_super           | 107 ms                                                                | 75.4 ms: 1.42x faster                                                   |
| scimark_monte_carlo      | 128 ms                                                                | 92.2 ms: 1.39x faster                                                   |
| sqlglot_parse            | 2.40 ms                                                               | 1.74 ms: 1.38x faster                                                   |
| unpickle_pure_python     | 366 us                                                                | 265 us: 1.38x faster                                                    |
| pickle_pure_python       | 529 us                                                                | 388 us: 1.37x faster                                                    |
| comprehensions           | 33.1 us                                                               | 24.6 us: 1.35x faster                                                   |
| richards                 | 91.7 ms                                                               | 68.2 ms: 1.34x faster                                                   |
| chaos                    | 121 ms                                                                | 91.5 ms: 1.32x faster                                                   |
| logging_simple           | 9.78 us                                                               | 7.45 us: 1.31x faster                                                   |
| thrift                   | 1.26 ms                                                               | 960 us: 1.31x faster                                                    |
| coroutines               | 37.2 ms                                                               | 28.6 ms: 1.30x faster                                                   |
| sqlglot_transpile        | 2.84 ms                                                               | 2.21 ms: 1.29x faster                                                   |
| logging_format           | 10.6 us                                                               | 8.27 us: 1.28x faster                                                   |
| tomli_loads              | 3.36 sec                                                              | 2.63 sec: 1.28x faster                                                  |
| spectral_norm            | 186 ms                                                                | 147 ms: 1.27x faster                                                    |
| deepcopy                 | 511 us                                                                | 411 us: 1.24x faster                                                    |
| json_dumps               | 16.7 ms                                                               | 13.5 ms: 1.24x faster                                                   |
| xml_etree_process        | 99.5 ms                                                               | 80.8 ms: 1.23x faster                                                   |
| tornado_http             | 178 ms                                                                | 146 ms: 1.22x faster                                                    |
| pyflate                  | 795 ms                                                                | 655 ms: 1.21x faster                                                    |
| html5lib                 | 86.5 ms                                                               | 72.3 ms: 1.20x faster                                                   |
| pathlib                  | 26.3 ms                                                               | 22.0 ms: 1.20x faster                                                   |
| generators               | 68.1 ms                                                               | 56.9 ms: 1.20x faster                                                   |
| deepcopy_reduce          | 4.60 us                                                               | 3.93 us: 1.17x faster                                                   |
| bench_thread_pool        | 1.59 ms                                                               | 1.36 ms: 1.17x faster                                                   |
| xml_etree_parse          | 212 ms                                                                | 189 ms: 1.12x faster                                                    |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.86 ms: 1.11x faster                                                   |
| pycparser                | 1.69 sec                                                              | 1.53 sec: 1.11x faster                                                  |
| xml_etree_generate       | 123 ms                                                                | 111 ms: 1.11x faster                                                    |
| scimark_fft              | 500 ms                                                                | 460 ms: 1.09x faster                                                    |
| fannkuch                 | 546 ms                                                                | 503 ms: 1.09x faster                                                    |
| unpickle_list            | 6.99 us                                                               | 6.44 us: 1.09x faster                                                   |
| sqlite_synth             | 4.12 us                                                               | 3.79 us: 1.08x faster                                                   |
| hexiom                   | 10.9 ms                                                               | 10.2 ms: 1.07x faster                                                   |
| mdp                      | 3.70 sec                                                              | 3.50 sec: 1.06x faster                                                  |
| regex_dna                | 257 ms                                                                | 245 ms: 1.05x faster                                                    |
| regex_v8                 | 32.2 ms                                                               | 30.7 ms: 1.05x faster                                                   |
| django_template          | 53.3 ms                                                               | 51.2 ms: 1.04x faster                                                   |
| sqlglot_normalize        | 156 ms                                                                | 151 ms: 1.03x faster                                                    |
| json                     | 5.88 ms                                                               | 5.70 ms: 1.03x faster                                                   |
| meteor_contest           | 126 ms                                                                | 124 ms: 1.02x faster                                                    |
| genshi_text              | 35.2 ms                                                               | 34.7 ms: 1.01x faster                                                   |
| asyncio_websockets       | 657 ms                                                                | 661 ms: 1.01x slower                                                    |
| json_loads               | 30.9 us                                                               | 31.4 us: 1.02x slower                                                   |
| create_gc_cycles         | 2.26 ms                                                               | 2.34 ms: 1.03x slower                                                   |
| sqlglot_optimize         | 75.4 ms                                                               | 78.8 ms: 1.04x slower                                                   |
| sympy_integrate          | 26.5 ms                                                               | 28.4 ms: 1.07x slower                                                   |
| regex_compile            | 177 ms                                                                | 190 ms: 1.07x slower                                                    |
| nqueens                  | 117 ms                                                                | 127 ms: 1.08x slower                                                    |
| pickle_dict              | 35.1 us                                                               | 38.1 us: 1.08x slower                                                   |
| pprint_safe_repr         | 1.15 sec                                                              | 1.25 sec: 1.09x slower                                                  |
| pickle                   | 12.5 us                                                               | 13.8 us: 1.10x slower                                                   |
| pprint_pformat           | 2.36 sec                                                              | 2.61 sec: 1.11x slower                                                  |
| sympy_str                | 329 ms                                                                | 364 ms: 1.11x slower                                                    |
| async_generators         | 452 ms                                                                | 503 ms: 1.11x slower                                                    |
| dulwich_log              | 73.5 ms                                                               | 82.0 ms: 1.12x slower                                                   |
| docutils                 | 3.53 sec                                                              | 3.94 sec: 1.12x slower                                                  |
| sympy_expand             | 543 ms                                                                | 612 ms: 1.13x slower                                                    |
| unpickle                 | 17.5 us                                                               | 19.8 us: 1.13x slower                                                   |
| regex_effbot             | 4.25 ms                                                               | 4.91 ms: 1.16x slower                                                   |
| python_startup           | 11.2 ms                                                               | 13.0 ms: 1.16x slower                                                   |
| genshi_xml               | 69.8 ms                                                               | 81.2 ms: 1.16x slower                                                   |
| sympy_sum                | 184 ms                                                                | 215 ms: 1.17x slower                                                    |
| coverage                 | 83.6 ms                                                               | 100 ms: 1.20x slower                                                    |
| gc_traversal             | 4.15 ms                                                               | 5.18 ms: 1.25x slower                                                   |
| telco                    | 8.49 ms                                                               | 10.7 ms: 1.26x slower                                                   |
| python_startup_no_site   | 6.89 ms                                                               | 8.72 ms: 1.27x slower                                                   |
| Geometric mean           | (ref)                                                                 | 1.18x faster                                                            |

Benchmark hidden because not significant (5): pylint, xml_etree_iterparse, pidigits, 2to3, pickle_list
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (6) of results/bm-20240910-3.14.0a0-3597642-JIT/bm-20240910-arminc-aarch64-python-3597642ed57d184511ca-3.14.0a0-3597642.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, unpack_sequence

- Geometric mean (including insignificant results): 1.177x faster
# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.25x