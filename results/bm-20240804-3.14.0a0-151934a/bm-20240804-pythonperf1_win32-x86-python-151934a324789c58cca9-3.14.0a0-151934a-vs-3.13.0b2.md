# Results vs. 3.13.0b2

- fork: python
- ref: 151934a324789c58cca9
- machine: windows-x86
- commit hash: 151934a
- commit date: 2024-08-04
- overall geometric mean: 1.05x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 233 ms                                                              | 261 ms: 1.12x slower                                                           |
| docutils       | 1.81 sec                                                            | 1.96 sec: 1.08x slower                                                         |
| html5lib       | 45.4 ms                                                             | 49.1 ms: 1.08x slower                                                          |
| tornado_http   | 94.3 ms                                                             | 105 ms: 1.11x slower                                                           |
| Geometric mean | (ref)                                                               | 1.10x slower                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_io_tg           | 529 ms                                                              | 521 ms: 1.02x faster                                                           |
| async_tree_cpu_io_mixed    | 471 ms                                                              | 483 ms: 1.02x slower                                                           |
| async_tree_none            | 228 ms                                                              | 235 ms: 1.03x slower                                                           |
| async_tree_memoization     | 275 ms                                                              | 284 ms: 1.03x slower                                                           |
| async_tree_cpu_io_mixed_tg | 447 ms                                                              | 468 ms: 1.05x slower                                                           |
| async_tree_io              | 530 ms                                                              | 568 ms: 1.07x slower                                                           |
| Geometric mean             | (ref)                                                               | 1.02x slower                                                                   |

Benchmark hidden because not significant (2): async_tree_memoization_tg, async_tree_none_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| pidigits       | 199 ms                                                              | 202 ms: 1.02x slower                                                           |
| float          | 52.4 ms                                                             | 61.5 ms: 1.17x slower                                                          |
| nbody          | 76.9 ms                                                             | 101 ms: 1.32x slower                                                           |
| Geometric mean | (ref)                                                               | 1.16x slower                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_dna      | 118 ms                                                              | 119 ms: 1.01x slower                                                           |
| regex_effbot   | 1.88 ms                                                             | 1.94 ms: 1.03x slower                                                          |
| regex_v8       | 15.7 ms                                                             | 16.3 ms: 1.04x slower                                                          |
| regex_compile  | 99.9 ms                                                             | 108 ms: 1.08x slower                                                           |
| Geometric mean | (ref)                                                               | 1.04x slower                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| xml_etree_parse      | 104 ms                                                              | 107 ms: 1.02x slower                                                           |
| json_loads           | 20.5 us                                                             | 21.2 us: 1.03x slower                                                          |
| xml_etree_iterparse  | 64.2 ms                                                             | 68.8 ms: 1.07x slower                                                          |
| xml_etree_generate   | 59.6 ms                                                             | 66.6 ms: 1.12x slower                                                          |
| json_dumps           | 6.84 ms                                                             | 7.80 ms: 1.14x slower                                                          |
| xml_etree_process    | 42.1 ms                                                             | 48.1 ms: 1.14x slower                                                          |
| tomli_loads          | 1.65 sec                                                            | 1.92 sec: 1.17x slower                                                         |
| unpickle_pure_python | 147 us                                                              | 174 us: 1.18x slower                                                           |
| pickle_pure_python   | 213 us                                                              | 258 us: 1.21x slower                                                           |
| Geometric mean       | (ref)                                                               | 1.12x slower                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup         | 22.2 ms                                                             | 23.7 ms: 1.07x slower                                                          |
| python_startup_no_site | 18.2 ms                                                             | 19.6 ms: 1.08x slower                                                          |
| Geometric mean         | (ref)                                                               | 1.07x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|-----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| genshi_xml      | 44.3 ms                                                             | 47.7 ms: 1.08x slower                                                          |
| django_template | 30.1 ms                                                             | 32.5 ms: 1.08x slower                                                          |
| genshi_text     | 20.1 ms                                                             | 23.6 ms: 1.17x slower                                                          |
| mako            | 6.94 ms                                                             | 8.28 ms: 1.19x slower                                                          |
| Geometric mean  | (ref)                                                               | 1.13x slower                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a |
|----------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| thrift                     | 9.73 ms                                                             | 763 us: 12.75x faster                                                          |
| coverage                   | 307 ms                                                              | 53.2 ms: 5.77x faster                                                          |
| deepcopy                   | 280 us                                                              | 258 us: 1.08x faster                                                           |
| async_tree_io_tg           | 529 ms                                                              | 521 ms: 1.02x faster                                                           |
| regex_dna                  | 118 ms                                                              | 119 ms: 1.01x slower                                                           |
| asyncio_tcp_ssl            | 16.9 sec                                                            | 17.1 sec: 1.01x slower                                                         |
| deepcopy_reduce            | 2.59 us                                                             | 2.63 us: 1.02x slower                                                          |
| pidigits                   | 199 ms                                                              | 202 ms: 1.02x slower                                                           |
| gc_traversal               | 1.43 ms                                                             | 1.46 ms: 1.02x slower                                                          |
| xml_etree_parse            | 104 ms                                                              | 107 ms: 1.02x slower                                                           |
| async_tree_cpu_io_mixed    | 471 ms                                                              | 483 ms: 1.02x slower                                                           |
| regex_effbot               | 1.88 ms                                                             | 1.94 ms: 1.03x slower                                                          |
| async_tree_none            | 228 ms                                                              | 235 ms: 1.03x slower                                                           |
| async_tree_memoization     | 275 ms                                                              | 284 ms: 1.03x slower                                                           |
| json_loads                 | 20.5 us                                                             | 21.2 us: 1.03x slower                                                          |
| bench_thread_pool          | 985 us                                                              | 1.02 ms: 1.03x slower                                                          |
| regex_v8                   | 15.7 ms                                                             | 16.3 ms: 1.04x slower                                                          |
| telco                      | 6.03 ms                                                             | 6.30 ms: 1.04x slower                                                          |
| async_tree_cpu_io_mixed_tg | 447 ms                                                              | 468 ms: 1.05x slower                                                           |
| sympy_expand               | 375 ms                                                              | 396 ms: 1.06x slower                                                           |
| sympy_str                  | 211 ms                                                              | 223 ms: 1.06x slower                                                           |
| sympy_sum                  | 105 ms                                                              | 111 ms: 1.06x slower                                                           |
| asyncio_tcp                | 662 ms                                                              | 701 ms: 1.06x slower                                                           |
| json                       | 4.10 ms                                                             | 4.34 ms: 1.06x slower                                                          |
| pathlib                    | 83.9 ms                                                             | 89.4 ms: 1.07x slower                                                          |
| python_startup             | 22.2 ms                                                             | 23.7 ms: 1.07x slower                                                          |
| bench_mp_pool              | 69.4 ms                                                             | 74.3 ms: 1.07x slower                                                          |
| async_tree_io              | 530 ms                                                              | 568 ms: 1.07x slower                                                           |
| xml_etree_iterparse        | 64.2 ms                                                             | 68.8 ms: 1.07x slower                                                          |
| crypto_pyaes               | 55.8 ms                                                             | 60.0 ms: 1.08x slower                                                          |
| python_startup_no_site     | 18.2 ms                                                             | 19.6 ms: 1.08x slower                                                          |
| mdp                        | 1.60 sec                                                            | 1.73 sec: 1.08x slower                                                         |
| genshi_xml                 | 44.3 ms                                                             | 47.7 ms: 1.08x slower                                                          |
| docutils                   | 1.81 sec                                                            | 1.96 sec: 1.08x slower                                                         |
| html5lib                   | 45.4 ms                                                             | 49.1 ms: 1.08x slower                                                          |
| regex_compile              | 99.9 ms                                                             | 108 ms: 1.08x slower                                                           |
| django_template            | 30.1 ms                                                             | 32.5 ms: 1.08x slower                                                          |
| scimark_sparse_mat_mult    | 2.87 ms                                                             | 3.13 ms: 1.09x slower                                                          |
| sympy_integrate            | 14.6 ms                                                             | 16.0 ms: 1.09x slower                                                          |
| pylint                     | 217 ms                                                              | 237 ms: 1.09x slower                                                           |
| chaos                      | 48.0 ms                                                             | 52.6 ms: 1.10x slower                                                          |
| pprint_safe_repr           | 607 ms                                                              | 670 ms: 1.10x slower                                                           |
| logging_simple             | 7.47 us                                                             | 8.25 us: 1.10x slower                                                          |
| meteor_contest             | 74.1 ms                                                             | 81.9 ms: 1.11x slower                                                          |
| logging_format             | 8.13 us                                                             | 9.03 us: 1.11x slower                                                          |
| tornado_http               | 94.3 ms                                                             | 105 ms: 1.11x slower                                                           |
| pprint_pformat             | 1.24 sec                                                            | 1.38 sec: 1.11x slower                                                         |
| xml_etree_generate         | 59.6 ms                                                             | 66.6 ms: 1.12x slower                                                          |
| sqlglot_normalize          | 206 ms                                                              | 230 ms: 1.12x slower                                                           |
| 2to3                       | 233 ms                                                              | 261 ms: 1.12x slower                                                           |
| typing_runtime_protocols   | 136 us                                                              | 152 us: 1.12x slower                                                           |
| async_generators           | 266 ms                                                              | 299 ms: 1.12x slower                                                           |
| sqlglot_optimize           | 39.7 ms                                                             | 44.7 ms: 1.13x slower                                                          |
| pycparser                  | 777 ms                                                              | 883 ms: 1.14x slower                                                           |
| coroutines                 | 15.5 ms                                                             | 17.6 ms: 1.14x slower                                                          |
| nqueens                    | 68.7 ms                                                             | 78.1 ms: 1.14x slower                                                          |
| json_dumps                 | 6.84 ms                                                             | 7.80 ms: 1.14x slower                                                          |
| xml_etree_process          | 42.1 ms                                                             | 48.1 ms: 1.14x slower                                                          |
| sqlglot_transpile          | 1.19 ms                                                             | 1.36 ms: 1.15x slower                                                          |
| pyflate                    | 308 ms                                                              | 354 ms: 1.15x slower                                                           |
| spectral_norm              | 68.0 ms                                                             | 78.3 ms: 1.15x slower                                                          |
| sqlglot_parse              | 964 us                                                              | 1.12 ms: 1.16x slower                                                          |
| comprehensions             | 11.9 us                                                             | 13.8 us: 1.16x slower                                                          |
| tomli_loads                | 1.65 sec                                                            | 1.92 sec: 1.17x slower                                                         |
| go                         | 101 ms                                                              | 118 ms: 1.17x slower                                                           |
| genshi_text                | 20.1 ms                                                             | 23.6 ms: 1.17x slower                                                          |
| float                      | 52.4 ms                                                             | 61.5 ms: 1.17x slower                                                          |
| scimark_fft                | 198 ms                                                              | 233 ms: 1.18x slower                                                           |
| unpickle_pure_python       | 147 us                                                              | 174 us: 1.18x slower                                                           |
| raytrace                   | 189 ms                                                              | 224 ms: 1.19x slower                                                           |
| fannkuch                   | 270 ms                                                              | 321 ms: 1.19x slower                                                           |
| mako                       | 6.94 ms                                                             | 8.28 ms: 1.19x slower                                                          |
| pickle_pure_python         | 213 us                                                              | 258 us: 1.21x slower                                                           |
| deltablue                  | 2.23 ms                                                             | 2.73 ms: 1.22x slower                                                          |
| scimark_sor                | 81.0 ms                                                             | 99.3 ms: 1.23x slower                                                          |
| scimark_lu                 | 59.4 ms                                                             | 73.4 ms: 1.24x slower                                                          |
| scimark_monte_carlo        | 45.2 ms                                                             | 56.0 ms: 1.24x slower                                                          |
| richards_super             | 35.5 ms                                                             | 44.7 ms: 1.26x slower                                                          |
| hexiom                     | 4.23 ms                                                             | 5.39 ms: 1.28x slower                                                          |
| logging_silent             | 57.9 ns                                                             | 75.2 ns: 1.30x slower                                                          |
| generators                 | 21.2 ms                                                             | 27.8 ms: 1.31x slower                                                          |
| richards                   | 31.2 ms                                                             | 41.1 ms: 1.32x slower                                                          |
| nbody                      | 76.9 ms                                                             | 101 ms: 1.32x slower                                                           |
| Geometric mean             | (ref)                                                               | 1.05x slower                                                                   |

Benchmark hidden because not significant (4): deepcopy_memo, create_gc_cycles, async_tree_memoization_tg, async_tree_none_tg
Ignored benchmarks (8) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17.json: chameleon, flaskblogging, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (1) of results/bm-20240804-3.14.0a0-151934a/bm-20240804-pythonperf1_win32-x86-python-151934a324789c58cca9-3.14.0a0-151934a.json: dulwich_log

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.07x
- 95% likely to have a slowdown of 1.07x
- 99% likely to have a slowdown of 1.07x

# Memory
- memory change: unknown