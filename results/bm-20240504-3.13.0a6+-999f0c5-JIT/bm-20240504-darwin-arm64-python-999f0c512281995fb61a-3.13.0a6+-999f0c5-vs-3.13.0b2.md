# Results vs. 3.13.0b2

- fork: python
- ref: 999f0c512281995fb61a
- machine: darwin-arm64
- commit hash: 999f0c5
- commit date: 2024-05-04
- overall geometric mean: 1.03x slower
- HPT reliability: 99.37%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 161 ms                                                     | 172 ms: 1.07x slower                                                   |
| chameleon      | 4.27 ms                                                    | 4.38 ms: 1.03x slower                                                  |
| html5lib       | 31.2 ms                                                    | 30.9 ms: 1.01x faster                                                  |
| tornado_http   | 66.7 ms                                                    | 74.5 ms: 1.12x slower                                                  |
| Geometric mean | (ref)                                                      | 1.05x slower                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------------------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_eager_io              | 766 ms                                                     | 742 ms: 1.03x faster                                                   |
| async_tree_none_tg               | 198 ms                                                     | 192 ms: 1.03x faster                                                   |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 328 ms: 1.01x faster                                                   |
| async_tree_eager_tg              | 41.4 ms                                                    | 42.2 ms: 1.02x slower                                                  |
| async_tree_eager                 | 60.3 ms                                                    | 63.2 ms: 1.05x slower                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 594 ms: 1.05x slower                                                   |
| async_tree_memoization_tg        | 240 ms                                                     | 255 ms: 1.07x slower                                                   |
| Geometric mean                   | (ref)                                                      | 1.01x slower                                                           |

Benchmark hidden because not significant (9): async_tree_memoization, async_tree_cpu_io_mixed_tg, async_tree_cpu_io_mixed, async_tree_eager_cpu_io_mixed, async_tree_eager_io_tg, async_tree_eager_memoization_tg, async_tree_io, async_tree_eager_memoization, async_tree_none

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 48.6 ms                                                    | 46.7 ms: 1.04x faster                                                  |
| nbody          | 59.6 ms                                                    | 63.6 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                      | 1.01x slower                                                           |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 2.58 ms                                                    | 2.54 ms: 1.02x faster                                                  |
| regex_dna      | 149 ms                                                     | 149 ms: 1.00x faster                                                   |
| regex_v8       | 17.3 ms                                                    | 17.5 ms: 1.01x slower                                                  |
| regex_compile  | 68.5 ms                                                    | 72.2 ms: 1.05x slower                                                  |
| Geometric mean | (ref)                                                      | 1.01x slower                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 1.47 sec                                                   | 1.24 sec: 1.18x faster                                                 |
| xml_etree_parse      | 106 ms                                                     | 97.3 ms: 1.09x faster                                                  |
| unpickle_pure_python | 141 us                                                     | 131 us: 1.08x faster                                                   |
| xml_etree_iterparse  | 71.5 ms                                                    | 66.6 ms: 1.07x faster                                                  |
| xml_etree_process    | 37.1 ms                                                    | 35.5 ms: 1.04x faster                                                  |
| pickle_pure_python   | 179 us                                                     | 171 us: 1.04x faster                                                   |
| xml_etree_generate   | 52.7 ms                                                    | 50.6 ms: 1.04x faster                                                  |
| json_dumps           | 6.23 ms                                                    | 6.17 ms: 1.01x faster                                                  |
| pickle               | 7.48 us                                                    | 7.44 us: 1.00x faster                                                  |
| pickle_dict          | 18.3 us                                                    | 18.2 us: 1.00x faster                                                  |
| pickle_list          | 2.96 us                                                    | 2.99 us: 1.01x slower                                                  |
| json_loads           | 16.8 us                                                    | 17.2 us: 1.02x slower                                                  |
| unpickle             | 9.12 us                                                    | 9.58 us: 1.05x slower                                                  |
| unpickle_list        | 2.88 us                                                    | 3.29 us: 1.14x slower                                                  |
| Geometric mean       | (ref)                                                      | 1.02x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|------------------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 15.2 ms                                                    | 15.2 ms: 1.00x slower                                                  |
| python_startup_no_site | 12.3 ms                                                    | 12.4 ms: 1.01x slower                                                  |
| Geometric mean         | (ref)                                                      | 1.01x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|-----------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 6.99 ms                                                    | 6.29 ms: 1.11x faster                                                  |
| django_template | 19.4 ms                                                    | 20.9 ms: 1.08x slower                                                  |
| genshi_text     | 13.9 ms                                                    | 16.2 ms: 1.16x slower                                                  |
| genshi_xml      | 29.9 ms                                                    | 38.9 ms: 1.30x slower                                                  |
| Geometric mean  | (ref)                                                      | 1.10x slower                                                           |

All benchmarks:
===============

| Benchmark                        | bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240504-darwin-arm64-python-999f0c512281995fb61a-3.13.0a6+-999f0c5 |
|----------------------------------|:----------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads                      | 1.47 sec                                                   | 1.24 sec: 1.18x faster                                                 |
| mako                             | 6.99 ms                                                    | 6.29 ms: 1.11x faster                                                  |
| xml_etree_parse                  | 106 ms                                                     | 97.3 ms: 1.09x faster                                                  |
| unpickle_pure_python             | 141 us                                                     | 131 us: 1.08x faster                                                   |
| xml_etree_iterparse              | 71.5 ms                                                    | 66.6 ms: 1.07x faster                                                  |
| deepcopy_memo                    | 22.6 us                                                    | 21.2 us: 1.07x faster                                                  |
| telco                            | 4.60 ms                                                    | 4.39 ms: 1.05x faster                                                  |
| fannkuch                         | 248 ms                                                     | 237 ms: 1.05x faster                                                   |
| xml_etree_process                | 37.1 ms                                                    | 35.5 ms: 1.04x faster                                                  |
| pickle_pure_python               | 179 us                                                     | 171 us: 1.04x faster                                                   |
| xml_etree_generate               | 52.7 ms                                                    | 50.6 ms: 1.04x faster                                                  |
| float                            | 48.6 ms                                                    | 46.7 ms: 1.04x faster                                                  |
| pathlib                          | 23.3 ms                                                    | 22.6 ms: 1.03x faster                                                  |
| async_tree_eager_io              | 766 ms                                                     | 742 ms: 1.03x faster                                                   |
| crypto_pyaes                     | 49.5 ms                                                    | 48.0 ms: 1.03x faster                                                  |
| async_tree_none_tg               | 198 ms                                                     | 192 ms: 1.03x faster                                                   |
| pyflate                          | 321 ms                                                     | 312 ms: 1.03x faster                                                   |
| richards                         | 31.8 ms                                                    | 31.2 ms: 1.02x faster                                                  |
| mdp                              | 1.53 sec                                                   | 1.51 sec: 1.02x faster                                                 |
| regex_effbot                     | 2.58 ms                                                    | 2.54 ms: 1.02x faster                                                  |
| spectral_norm                    | 66.4 ms                                                    | 65.4 ms: 1.02x faster                                                  |
| richards_super                   | 35.2 ms                                                    | 34.7 ms: 1.01x faster                                                  |
| logging_simple                   | 3.04 us                                                    | 3.01 us: 1.01x faster                                                  |
| json_dumps                       | 6.23 ms                                                    | 6.17 ms: 1.01x faster                                                  |
| deepcopy_reduce                  | 1.82 us                                                    | 1.80 us: 1.01x faster                                                  |
| html5lib                         | 31.2 ms                                                    | 30.9 ms: 1.01x faster                                                  |
| async_tree_eager_cpu_io_mixed_tg | 331 ms                                                     | 328 ms: 1.01x faster                                                   |
| logging_format                   | 3.31 us                                                    | 3.29 us: 1.01x faster                                                  |
| logging_silent                   | 60.1 ns                                                    | 59.9 ns: 1.00x faster                                                  |
| pickle                           | 7.48 us                                                    | 7.44 us: 1.00x faster                                                  |
| pickle_dict                      | 18.3 us                                                    | 18.2 us: 1.00x faster                                                  |
| asyncio_websockets               | 409 ms                                                     | 408 ms: 1.00x faster                                                   |
| regex_dna                        | 149 ms                                                     | 149 ms: 1.00x faster                                                   |
| python_startup                   | 15.2 ms                                                    | 15.2 ms: 1.00x slower                                                  |
| generators                       | 22.9 ms                                                    | 23.1 ms: 1.01x slower                                                  |
| coroutines                       | 15.8 ms                                                    | 16.0 ms: 1.01x slower                                                  |
| python_startup_no_site           | 12.3 ms                                                    | 12.4 ms: 1.01x slower                                                  |
| deepcopy                         | 204 us                                                     | 206 us: 1.01x slower                                                   |
| thrift                           | 435 us                                                     | 440 us: 1.01x slower                                                   |
| coverage                         | 45.0 ms                                                    | 45.5 ms: 1.01x slower                                                  |
| json                             | 2.93 ms                                                    | 2.97 ms: 1.01x slower                                                  |
| regex_v8                         | 17.3 ms                                                    | 17.5 ms: 1.01x slower                                                  |
| pickle_list                      | 2.96 us                                                    | 2.99 us: 1.01x slower                                                  |
| gc_traversal                     | 2.45 ms                                                    | 2.48 ms: 1.01x slower                                                  |
| go                               | 101 ms                                                     | 102 ms: 1.02x slower                                                   |
| scimark_fft                      | 181 ms                                                     | 184 ms: 1.02x slower                                                   |
| json_loads                       | 16.8 us                                                    | 17.2 us: 1.02x slower                                                  |
| async_tree_eager_tg              | 41.4 ms                                                    | 42.2 ms: 1.02x slower                                                  |
| sqlite_synth                     | 1.55 us                                                    | 1.58 us: 1.02x slower                                                  |
| chameleon                        | 4.27 ms                                                    | 4.38 ms: 1.03x slower                                                  |
| meteor_contest                   | 70.3 ms                                                    | 72.3 ms: 1.03x slower                                                  |
| asyncio_tcp_ssl                  | 1.29 sec                                                   | 1.33 sec: 1.03x slower                                                 |
| async_generators                 | 281 ms                                                     | 290 ms: 1.03x slower                                                   |
| scimark_monte_carlo              | 42.5 ms                                                    | 43.9 ms: 1.03x slower                                                  |
| bench_mp_pool                    | 47.2 ms                                                    | 48.8 ms: 1.03x slower                                                  |
| dulwich_log                      | 27.6 ms                                                    | 28.5 ms: 1.04x slower                                                  |
| create_gc_cycles                 | 897 us                                                     | 929 us: 1.04x slower                                                   |
| scimark_sparse_mat_mult          | 2.77 ms                                                    | 2.87 ms: 1.04x slower                                                  |
| sympy_str                        | 131 ms                                                     | 137 ms: 1.04x slower                                                   |
| sympy_integrate                  | 10.3 ms                                                    | 10.8 ms: 1.04x slower                                                  |
| sympy_sum                        | 69.2 ms                                                    | 72.2 ms: 1.04x slower                                                  |
| async_tree_eager                 | 60.3 ms                                                    | 63.2 ms: 1.05x slower                                                  |
| sympy_expand                     | 226 ms                                                     | 237 ms: 1.05x slower                                                   |
| unpickle                         | 9.12 us                                                    | 9.58 us: 1.05x slower                                                  |
| async_tree_io_tg                 | 565 ms                                                     | 594 ms: 1.05x slower                                                   |
| regex_compile                    | 68.5 ms                                                    | 72.2 ms: 1.05x slower                                                  |
| sqlglot_optimize                 | 30.9 ms                                                    | 32.6 ms: 1.05x slower                                                  |
| scimark_sor                      | 94.9 ms                                                    | 100 ms: 1.06x slower                                                   |
| pycparser                        | 632 ms                                                     | 671 ms: 1.06x slower                                                   |
| hexiom                           | 4.06 ms                                                    | 4.32 ms: 1.06x slower                                                  |
| async_tree_memoization_tg        | 240 ms                                                     | 255 ms: 1.07x slower                                                   |
| 2to3                             | 161 ms                                                     | 172 ms: 1.07x slower                                                   |
| nbody                            | 59.6 ms                                                    | 63.6 ms: 1.07x slower                                                  |
| bench_thread_pool                | 447 us                                                     | 479 us: 1.07x slower                                                   |
| flaskblogging                    | 3.61 ms                                                    | 3.88 ms: 1.08x slower                                                  |
| django_template                  | 19.4 ms                                                    | 20.9 ms: 1.08x slower                                                  |
| typing_runtime_protocols         | 87.6 us                                                    | 94.7 us: 1.08x slower                                                  |
| nqueens                          | 52.2 ms                                                    | 56.5 ms: 1.08x slower                                                  |
| raytrace                         | 147 ms                                                     | 159 ms: 1.08x slower                                                   |
| tornado_http                     | 66.7 ms                                                    | 74.5 ms: 1.12x slower                                                  |
| sqlglot_transpile                | 891 us                                                     | 1.00 ms: 1.12x slower                                                  |
| sqlglot_parse                    | 732 us                                                     | 830 us: 1.13x slower                                                   |
| deltablue                        | 2.14 ms                                                    | 2.43 ms: 1.14x slower                                                  |
| chaos                            | 34.0 ms                                                    | 38.8 ms: 1.14x slower                                                  |
| unpickle_list                    | 2.88 us                                                    | 3.29 us: 1.14x slower                                                  |
| aiohttp                          | 997 us                                                     | 1.14 ms: 1.14x slower                                                  |
| scimark_lu                       | 66.9 ms                                                    | 77.2 ms: 1.15x slower                                                  |
| genshi_text                      | 13.9 ms                                                    | 16.2 ms: 1.16x slower                                                  |
| gunicorn                         | 1.04 ms                                                    | 1.22 ms: 1.17x slower                                                  |
| comprehensions                   | 10.2 us                                                    | 12.2 us: 1.21x slower                                                  |
| mypy2                            | 379 ms                                                     | 483 ms: 1.27x slower                                                   |
| genshi_xml                       | 29.9 ms                                                    | 38.9 ms: 1.30x slower                                                  |
| Geometric mean                   | (ref)                                                      | 1.03x slower                                                           |

Benchmark hidden because not significant (15): async_tree_memoization, async_tree_cpu_io_mixed_tg, async_tree_cpu_io_mixed, pprint_safe_repr, async_tree_eager_cpu_io_mixed, async_tree_eager_io_tg, pprint_pformat, pidigits, async_tree_eager_memoization_tg, dask, async_tree_io, async_tree_eager_memoization, async_tree_none, asyncio_tcp, pylint
Ignored benchmarks (3) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-darwin-arm64-python-v3.13.0b2-3.13.0b2-3a83b17.json: bpe_tokeniser, docutils, sqlglot_normalize

# HPT report

- Reliability score: 99.37% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.20x