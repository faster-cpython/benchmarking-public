
# Results vs. 3.10.4

- fork: python
- ref: main
- machine: darwin-arm64
- commit hash: 3adb23a
- commit date: 2023-03-18
- overall geometric mean: 1.21x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.17x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 165 ms: 1.23x faster                                   |
| chameleon      | 5.84 ms                                                | 4.48 ms: 1.30x faster                                  |
| docutils       | 1.78 sec                                               | 1.51 sec: 1.18x faster                                 |
| html5lib       | 44.1 ms                                                | 35.7 ms: 1.23x faster                                  |
| tornado_http   | 91.9 ms                                                | 69.5 ms: 1.32x faster                                  |
| Geometric mean | (ref)                                                  | 1.25x faster                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 60.7 ms: 1.55x faster                                  |
| float          | 72.3 ms                                                | 52.7 ms: 1.37x faster                                  |
| Geometric mean | (ref)                                                  | 1.29x faster                                           |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 75.9 ms: 1.27x faster                                  |
| regex_v8       | 17.5 ms                                                | 16.1 ms: 1.09x faster                                  |
| regex_dna      | 160 ms                                                 | 152 ms: 1.05x faster                                   |
| regex_effbot   | 2.45 ms                                                | 2.68 ms: 1.10x slower                                  |
| Geometric mean | (ref)                                                  | 1.08x faster                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|----------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 192 us: 1.48x faster                                   |
| unpickle_pure_python | 203 us                                                 | 145 us: 1.40x faster                                   |
| json_dumps           | 8.38 ms                                                | 6.28 ms: 1.34x faster                                  |
| xml_etree_process    | 45.1 ms                                                | 36.7 ms: 1.23x faster                                  |
| xml_etree_parse      | 107 ms                                                 | 97.4 ms: 1.10x faster                                  |
| xml_etree_generate   | 54.3 ms                                                | 51.8 ms: 1.05x faster                                  |
| xml_etree_iterparse  | 72.6 ms                                                | 70.3 ms: 1.03x faster                                  |
| json_loads           | 16.9 us                                                | 16.3 us: 1.03x faster                                  |
| unpickle_list        | 2.66 us                                                | 2.64 us: 1.01x faster                                  |
| pickle               | 7.36 us                                                | 7.41 us: 1.01x slower                                  |
| pickle_dict          | 17.8 us                                                | 18.0 us: 1.01x slower                                  |
| Geometric mean       | (ref)                                                  | 1.12x faster                                           |

Benchmark hidden because not significant (2): pickle_list, unpickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|------------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 11.0 ms: 1.15x faster                                  |
| python_startup_no_site | 9.73 ms                                                | 8.99 ms: 1.08x faster                                  |
| Geometric mean         | (ref)                                                  | 1.12x faster                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|-----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| mako            | 10.5 ms                                                | 7.44 ms: 1.41x faster                                  |
| genshi_xml      | 37.6 ms                                                | 29.3 ms: 1.28x faster                                  |
| django_template | 27.3 ms                                                | 21.9 ms: 1.25x faster                                  |
| genshi_text     | 18.4 ms                                                | 14.8 ms: 1.24x faster                                  |
| Geometric mean  | (ref)                                                  | 1.29x faster                                           |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230318-darwin-arm64-python-main-3.12.0a6+-3adb23a |
|-------------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.56 ms: 2.01x faster                                  |
| logging_silent          | 119 ns                                                 | 66.9 ns: 1.78x faster                                  |
| richards                | 51.4 ms                                                | 30.7 ms: 1.67x faster                                  |
| nbody                   | 94.1 ms                                                | 60.7 ms: 1.55x faster                                  |
| scimark_lu              | 110 ms                                                 | 71.2 ms: 1.54x faster                                  |
| asyncio_tcp             | 673 ms                                                 | 449 ms: 1.50x faster                                   |
| scimark_sor             | 127 ms                                                 | 85.2 ms: 1.49x faster                                  |
| raytrace                | 328 ms                                                 | 222 ms: 1.48x faster                                   |
| async_tree_memoization  | 493 ms                                                 | 333 ms: 1.48x faster                                   |
| go                      | 165 ms                                                 | 111 ms: 1.48x faster                                   |
| pickle_pure_python      | 284 us                                                 | 192 us: 1.48x faster                                   |
| hexiom                  | 6.32 ms                                                | 4.40 ms: 1.43x faster                                  |
| crypto_pyaes            | 78.0 ms                                                | 54.6 ms: 1.43x faster                                  |
| scimark_monte_carlo     | 72.2 ms                                                | 50.7 ms: 1.43x faster                                  |
| chaos                   | 66.8 ms                                                | 47.4 ms: 1.41x faster                                  |
| mako                    | 10.5 ms                                                | 7.44 ms: 1.41x faster                                  |
| unpickle_pure_python    | 203 us                                                 | 145 us: 1.40x faster                                   |
| async_tree_none         | 402 ms                                                 | 288 ms: 1.39x faster                                   |
| float                   | 72.3 ms                                                | 52.7 ms: 1.37x faster                                  |
| async_tree_io           | 1.02 sec                                               | 743 ms: 1.37x faster                                   |
| pyflate                 | 453 ms                                                 | 332 ms: 1.37x faster                                   |
| pycparser               | 915 ms                                                 | 676 ms: 1.35x faster                                   |
| spectral_norm           | 96.4 ms                                                | 72.1 ms: 1.34x faster                                  |
| json_dumps              | 8.38 ms                                                | 6.28 ms: 1.34x faster                                  |
| deepcopy_memo           | 34.5 us                                                | 25.8 us: 1.33x faster                                  |
| tornado_http            | 91.9 ms                                                | 69.5 ms: 1.32x faster                                  |
| chameleon               | 5.84 ms                                                | 4.48 ms: 1.30x faster                                  |
| thrift                  | 586 us                                                 | 456 us: 1.29x faster                                   |
| pprint_pformat          | 1.24 sec                                               | 967 ms: 1.28x faster                                   |
| genshi_xml              | 37.6 ms                                                | 29.3 ms: 1.28x faster                                  |
| pprint_safe_repr        | 609 ms                                                 | 475 ms: 1.28x faster                                   |
| regex_compile           | 96.6 ms                                                | 75.9 ms: 1.27x faster                                  |
| logging_format          | 5.01 us                                                | 3.95 us: 1.27x faster                                  |
| logging_simple          | 4.63 us                                                | 3.66 us: 1.26x faster                                  |
| sqlglot_parse           | 1.33 ms                                                | 1.06 ms: 1.26x faster                                  |
| sqlalchemy_imperative   | 9.03 ms                                                | 7.19 ms: 1.26x faster                                  |
| dulwich_log             | 37.1 ms                                                | 29.6 ms: 1.25x faster                                  |
| django_template         | 27.3 ms                                                | 21.9 ms: 1.25x faster                                  |
| sqlglot_transpile       | 1.54 ms                                                | 1.23 ms: 1.25x faster                                  |
| deepcopy                | 278 us                                                 | 222 us: 1.25x faster                                   |
| create_gc_cycles        | 876 us                                                 | 705 us: 1.24x faster                                   |
| genshi_text             | 18.4 ms                                                | 14.8 ms: 1.24x faster                                  |
| scimark_fft             | 232 ms                                                 | 187 ms: 1.24x faster                                   |
| html5lib                | 44.1 ms                                                | 35.7 ms: 1.23x faster                                  |
| xml_etree_process       | 45.1 ms                                                | 36.7 ms: 1.23x faster                                  |
| 2to3                    | 202 ms                                                 | 165 ms: 1.23x faster                                   |
| async_tree_cpu_io_mixed | 670 ms                                                 | 548 ms: 1.22x faster                                   |
| deepcopy_reduce         | 2.38 us                                                | 1.96 us: 1.22x faster                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.86 ms: 1.21x faster                                  |
| fannkuch                | 317 ms                                                 | 262 ms: 1.21x faster                                   |
| sqlglot_optimize        | 38.0 ms                                                | 32.0 ms: 1.19x faster                                  |
| generators              | 32.9 ms                                                | 27.8 ms: 1.19x faster                                  |
| docutils                | 1.78 sec                                               | 1.51 sec: 1.18x faster                                 |
| mypy2                   | 308 ms                                                 | 264 ms: 1.17x faster                                   |
| bench_thread_pool       | 548 us                                                 | 472 us: 1.16x faster                                   |
| unpack_sequence         | 38.7 ns                                                | 33.3 ns: 1.16x faster                                  |
| sqlalchemy_declarative  | 74.8 ms                                                | 64.5 ms: 1.16x faster                                  |
| python_startup          | 12.6 ms                                                | 11.0 ms: 1.15x faster                                  |
| sympy_integrate         | 13.4 ms                                                | 11.7 ms: 1.14x faster                                  |
| sqlglot_normalize       | 197 ms                                                 | 174 ms: 1.13x faster                                   |
| mdp                     | 1.67 sec                                               | 1.49 sec: 1.12x faster                                 |
| coroutines              | 20.2 ms                                                | 18.1 ms: 1.11x faster                                  |
| sympy_expand            | 276 ms                                                 | 248 ms: 1.11x faster                                   |
| json                    | 3.10 ms                                                | 2.80 ms: 1.10x faster                                  |
| xml_etree_parse         | 107 ms                                                 | 97.4 ms: 1.10x faster                                  |
| nqueens                 | 68.1 ms                                                | 62.1 ms: 1.10x faster                                  |
| sympy_str               | 169 ms                                                 | 154 ms: 1.10x faster                                   |
| sqlite_synth            | 1.47 us                                                | 1.35 us: 1.09x faster                                  |
| regex_v8                | 17.5 ms                                                | 16.1 ms: 1.09x faster                                  |
| python_startup_no_site  | 9.73 ms                                                | 8.99 ms: 1.08x faster                                  |
| sympy_sum               | 94.3 ms                                                | 88.3 ms: 1.07x faster                                  |
| meteor_contest          | 78.6 ms                                                | 73.8 ms: 1.06x faster                                  |
| regex_dna               | 160 ms                                                 | 152 ms: 1.05x faster                                   |
| telco                   | 3.68 ms                                                | 3.49 ms: 1.05x faster                                  |
| xml_etree_generate      | 54.3 ms                                                | 51.8 ms: 1.05x faster                                  |
| pathlib                 | 28.8 ms                                                | 27.9 ms: 1.03x faster                                  |
| xml_etree_iterparse     | 72.6 ms                                                | 70.3 ms: 1.03x faster                                  |
| json_loads              | 16.9 us                                                | 16.3 us: 1.03x faster                                  |
| unpickle_list           | 2.66 us                                                | 2.64 us: 1.01x faster                                  |
| pickle                  | 7.36 us                                                | 7.41 us: 1.01x slower                                  |
| pickle_dict             | 17.8 us                                                | 18.0 us: 1.01x slower                                  |
| gc_traversal            | 2.40 ms                                                | 2.43 ms: 1.01x slower                                  |
| comprehensions          | 17.7 us                                                | 17.9 us: 1.01x slower                                  |
| bench_mp_pool           | 41.0 ms                                                | 44.8 ms: 1.09x slower                                  |
| regex_effbot            | 2.45 ms                                                | 2.68 ms: 1.10x slower                                  |
| async_generators        | 233 ms                                                 | 266 ms: 1.14x slower                                   |
| dask                    | 258 ms                                                 | 322 ms: 1.25x slower                                   |
| coverage                | 40.8 ms                                                | 56.0 ms: 1.37x slower                                  |
| Geometric mean          | (ref)                                                  | 1.21x faster                                           |

Benchmark hidden because not significant (3): pidigits, pickle_list, unpickle
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, pylint, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.20x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.17x
