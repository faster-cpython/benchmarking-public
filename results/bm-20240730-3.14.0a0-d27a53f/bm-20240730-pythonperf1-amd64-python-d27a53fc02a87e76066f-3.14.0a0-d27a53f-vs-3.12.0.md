# Results vs. 3.12.0

- fork: python
- ref: d27a53fc02a87e76066f
- machine: windows-amd64
- commit hash: d27a53f
- commit date: 2024-07-30
- overall geometric mean: 1.03x slower
- HPT reliability: 99.97%
- HPT 99th percentile: 1.01x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 232 ms: 1.07x slower                                                       |
| docutils       | 1.66 sec                                                    | 1.77 sec: 1.07x slower                                                     |
| tornado_http   | 89.5 ms                                                     | 93.6 ms: 1.05x slower                                                      |
| Geometric mean | (ref)                                                       | 1.06x slower                                                               |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 249 ms: 1.48x faster                                                       |
| async_tree_none_tg         | 285 ms                                                      | 200 ms: 1.42x faster                                                       |
| async_tree_io_tg           | 771 ms                                                      | 549 ms: 1.40x faster                                                       |
| async_tree_none            | 291 ms                                                      | 221 ms: 1.32x faster                                                       |
| async_tree_memoization     | 339 ms                                                      | 264 ms: 1.29x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 393 ms: 1.28x faster                                                       |
| async_tree_io              | 731 ms                                                      | 581 ms: 1.26x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 389 ms: 1.26x faster                                                       |
| Geometric mean             | (ref)                                                       | 1.34x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| pidigits       | 152 ms                                                      | 150 ms: 1.01x faster                                                       |
| nbody          | 71.9 ms                                                     | 85.8 ms: 1.19x slower                                                      |
| Geometric mean | (ref)                                                       | 1.05x slower                                                               |

Benchmark hidden because not significant (1): float

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 121 ms: 1.04x faster                                                       |
| regex_effbot   | 1.62 ms                                                     | 1.59 ms: 1.02x faster                                                      |
| regex_compile  | 87.5 ms                                                     | 93.3 ms: 1.07x slower                                                      |
| regex_v8       | 14.2 ms                                                     | 17.4 ms: 1.22x slower                                                      |
| Geometric mean | (ref)                                                       | 1.05x slower                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| json_loads           | 13.9 us                                                     | 14.4 us: 1.04x slower                                                      |
| xml_etree_generate   | 55.8 ms                                                     | 58.8 ms: 1.05x slower                                                      |
| pickle_pure_python   | 195 us                                                      | 217 us: 1.11x slower                                                       |
| json_dumps           | 5.70 ms                                                     | 6.31 ms: 1.11x slower                                                      |
| xml_etree_process    | 37.7 ms                                                     | 42.2 ms: 1.12x slower                                                      |
| unpickle_pure_python | 133 us                                                      | 152 us: 1.14x slower                                                       |
| tomli_loads          | 1.37 sec                                                    | 1.63 sec: 1.19x slower                                                     |
| Geometric mean       | (ref)                                                       | 1.08x slower                                                               |

Benchmark hidden because not significant (2): xml_etree_parse, xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 17.7 ms: 1.09x slower                                                      |
| python_startup         | 19.5 ms                                                     | 22.0 ms: 1.13x slower                                                      |
| Geometric mean         | (ref)                                                       | 1.11x slower                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|-----------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| mako            | 7.09 ms                                                     | 7.40 ms: 1.04x slower                                                      |
| django_template | 22.9 ms                                                     | 25.6 ms: 1.12x slower                                                      |
| Geometric mean  | (ref)                                                       | 1.08x slower                                                               |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f |
|----------------------------|:-----------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 249 ms: 1.48x faster                                                       |
| async_tree_none_tg         | 285 ms                                                      | 200 ms: 1.42x faster                                                       |
| async_tree_io_tg           | 771 ms                                                      | 549 ms: 1.40x faster                                                       |
| asyncio_tcp_ssl            | 2.10 sec                                                    | 1.53 sec: 1.37x faster                                                     |
| async_tree_none            | 291 ms                                                      | 221 ms: 1.32x faster                                                       |
| async_tree_memoization     | 339 ms                                                      | 264 ms: 1.29x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 393 ms: 1.28x faster                                                       |
| async_tree_io              | 731 ms                                                      | 581 ms: 1.26x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 389 ms: 1.26x faster                                                       |
| deepcopy                   | 238 us                                                      | 196 us: 1.22x faster                                                       |
| comprehensions             | 14.1 us                                                     | 12.4 us: 1.14x faster                                                      |
| deepcopy_memo              | 23.7 us                                                     | 21.3 us: 1.11x faster                                                      |
| bench_thread_pool          | 857 us                                                      | 804 us: 1.07x faster                                                       |
| generators                 | 22.5 ms                                                     | 21.2 ms: 1.06x faster                                                      |
| regex_dna                  | 126 ms                                                      | 121 ms: 1.04x faster                                                       |
| deepcopy_reduce            | 2.09 us                                                     | 2.03 us: 1.03x faster                                                      |
| dulwich_log                | 44.3 ms                                                     | 42.9 ms: 1.03x faster                                                      |
| coroutines                 | 14.3 ms                                                     | 14.0 ms: 1.02x faster                                                      |
| regex_effbot               | 1.62 ms                                                     | 1.59 ms: 1.02x faster                                                      |
| pidigits                   | 152 ms                                                      | 150 ms: 1.01x faster                                                       |
| sympy_sum                  | 91.5 ms                                                     | 92.0 ms: 1.01x slower                                                      |
| nqueens                    | 62.8 ms                                                     | 63.4 ms: 1.01x slower                                                      |
| logging_simple             | 6.28 us                                                     | 6.37 us: 1.02x slower                                                      |
| gc_traversal               | 1.52 ms                                                     | 1.56 ms: 1.03x slower                                                      |
| logging_format             | 6.72 us                                                     | 6.92 us: 1.03x slower                                                      |
| async_generators           | 239 ms                                                      | 248 ms: 1.03x slower                                                       |
| json_loads                 | 13.9 us                                                     | 14.4 us: 1.04x slower                                                      |
| sympy_str                  | 175 ms                                                      | 182 ms: 1.04x slower                                                       |
| sympy_integrate            | 13.0 ms                                                     | 13.4 ms: 1.04x slower                                                      |
| chaos                      | 43.3 ms                                                     | 45.0 ms: 1.04x slower                                                      |
| meteor_contest             | 74.6 ms                                                     | 77.9 ms: 1.04x slower                                                      |
| mako                       | 7.09 ms                                                     | 7.40 ms: 1.04x slower                                                      |
| tornado_http               | 89.5 ms                                                     | 93.6 ms: 1.05x slower                                                      |
| raytrace                   | 192 ms                                                      | 202 ms: 1.05x slower                                                       |
| sqlglot_normalize          | 187 ms                                                      | 197 ms: 1.05x slower                                                       |
| xml_etree_generate         | 55.8 ms                                                     | 58.8 ms: 1.05x slower                                                      |
| logging_silent             | 60.5 ns                                                     | 64.4 ns: 1.06x slower                                                      |
| 2to3                       | 218 ms                                                      | 232 ms: 1.07x slower                                                       |
| docutils                   | 1.66 sec                                                    | 1.77 sec: 1.07x slower                                                     |
| regex_compile              | 87.5 ms                                                     | 93.3 ms: 1.07x slower                                                      |
| sqlglot_optimize           | 34.5 ms                                                     | 36.8 ms: 1.07x slower                                                      |
| mdp                        | 1.37 sec                                                    | 1.47 sec: 1.07x slower                                                     |
| deltablue                  | 2.16 ms                                                     | 2.31 ms: 1.07x slower                                                      |
| crypto_pyaes               | 48.5 ms                                                     | 51.9 ms: 1.07x slower                                                      |
| spectral_norm              | 66.9 ms                                                     | 71.8 ms: 1.07x slower                                                      |
| go                         | 91.6 ms                                                     | 99.8 ms: 1.09x slower                                                      |
| sympy_expand               | 284 ms                                                      | 310 ms: 1.09x slower                                                       |
| python_startup_no_site     | 16.2 ms                                                     | 17.7 ms: 1.09x slower                                                      |
| sqlglot_transpile          | 1.02 ms                                                     | 1.12 ms: 1.10x slower                                                      |
| pyflate                    | 295 ms                                                      | 325 ms: 1.10x slower                                                       |
| pickle_pure_python         | 195 us                                                      | 217 us: 1.11x slower                                                       |
| json_dumps                 | 5.70 ms                                                     | 6.31 ms: 1.11x slower                                                      |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.85 ms: 1.11x slower                                                      |
| asyncio_tcp                | 487 ms                                                      | 544 ms: 1.12x slower                                                       |
| django_template            | 22.9 ms                                                     | 25.6 ms: 1.12x slower                                                      |
| xml_etree_process          | 37.7 ms                                                     | 42.2 ms: 1.12x slower                                                      |
| pprint_pformat             | 1.05 sec                                                    | 1.17 sec: 1.12x slower                                                     |
| pprint_safe_repr           | 513 ms                                                      | 574 ms: 1.12x slower                                                       |
| hexiom                     | 4.10 ms                                                     | 4.61 ms: 1.12x slower                                                      |
| scimark_lu                 | 58.9 ms                                                     | 66.4 ms: 1.13x slower                                                      |
| python_startup             | 19.5 ms                                                     | 22.0 ms: 1.13x slower                                                      |
| sqlglot_parse              | 804 us                                                      | 913 us: 1.13x slower                                                       |
| unpickle_pure_python       | 133 us                                                      | 152 us: 1.14x slower                                                       |
| scimark_fft                | 184 ms                                                      | 212 ms: 1.15x slower                                                       |
| richards                   | 28.4 ms                                                     | 33.1 ms: 1.17x slower                                                      |
| richards_super             | 32.1 ms                                                     | 37.5 ms: 1.17x slower                                                      |
| scimark_sor                | 78.8 ms                                                     | 92.2 ms: 1.17x slower                                                      |
| nbody                      | 71.9 ms                                                     | 85.8 ms: 1.19x slower                                                      |
| tomli_loads                | 1.37 sec                                                    | 1.63 sec: 1.19x slower                                                     |
| create_gc_cycles           | 752 us                                                      | 902 us: 1.20x slower                                                       |
| scimark_monte_carlo        | 43.7 ms                                                     | 52.6 ms: 1.20x slower                                                      |
| coverage                   | 40.8 ms                                                     | 49.4 ms: 1.21x slower                                                      |
| fannkuch                   | 247 ms                                                      | 298 ms: 1.21x slower                                                       |
| typing_runtime_protocols   | 95.1 us                                                     | 116 us: 1.22x slower                                                       |
| regex_v8                   | 14.2 ms                                                     | 17.4 ms: 1.22x slower                                                      |
| telco                      | 4.13 ms                                                     | 5.06 ms: 1.23x slower                                                      |
| pycparser                  | 699 ms                                                      | 912 ms: 1.30x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.03x slower                                                               |

Benchmark hidden because not significant (6): json, float, xml_etree_parse, pathlib, bench_mp_pool, xml_etree_iterparse
Ignored benchmarks (13) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240730-3.14.0a0-d27a53f/bm-20240730-pythonperf1-amd64-python-d27a53fc02a87e76066f-3.14.0a0-d27a53f.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 99.97% likely to be slow
- 90% likely to have a slowdown of 1.03x
- 95% likely to have a slowdown of 1.03x
- 99% likely to have a slowdown of 1.01x

# Memory
- memory change: unknown