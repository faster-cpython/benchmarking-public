
# Results vs. 3.11.0

- fork: python
- ref: eb49d32b9af0b3b01a55
- machine: linux-x86_64
- commit hash: eb49d32
- commit date: 2023-02-08
- overall geometric mean: 1.03x faster \*

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 257 ms                                                 | 251 ms: 1.03x faster                                                   |
| chameleon      | 6.41 ms                                                | 6.37 ms: 1.01x faster                                                  |
| docutils       | 2.60 sec                                               | 2.53 sec: 1.02x faster                                                 |
| html5lib       | 63.2 ms                                                | 61.1 ms: 1.03x faster                                                  |
| tornado_http   | 96.6 ms                                                | 94.4 ms: 1.02x faster                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 76.3 ms                                                | 72.9 ms: 1.05x faster                                                  |
| pidigits       | 199 ms                                                 | 193 ms: 1.03x faster                                                   |
| nbody          | 95.0 ms                                                | 94.3 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                  | 1.03x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 136 ms                                                 | 128 ms: 1.06x faster                                                   |
| regex_v8       | 22.3 ms                                                | 21.5 ms: 1.04x faster                                                  |
| regex_dna      | 203 ms                                                 | 219 ms: 1.08x slower                                                   |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 12.7 ms                                                | 9.51 ms: 1.33x faster                                                  |
| unpickle_pure_python | 225 us                                                 | 196 us: 1.15x faster                                                   |
| json_loads           | 26.2 us                                                | 23.5 us: 1.12x faster                                                  |
| pickle_pure_python   | 304 us                                                 | 286 us: 1.06x faster                                                   |
| xml_etree_parse      | 158 ms                                                 | 150 ms: 1.05x faster                                                   |
| pickle_list          | 4.17 us                                                | 4.04 us: 1.03x faster                                                  |
| unpickle             | 13.3 us                                                | 12.9 us: 1.03x faster                                                  |
| pickle_dict          | 31.4 us                                                | 30.8 us: 1.02x faster                                                  |
| unpickle_list        | 4.95 us                                                | 4.90 us: 1.01x faster                                                  |
| xml_etree_iterparse  | 103 ms                                                 | 103 ms: 1.01x slower                                                   |
| pickle               | 9.79 us                                                | 9.99 us: 1.02x slower                                                  |
| xml_etree_process    | 53.8 ms                                                | 55.8 ms: 1.04x slower                                                  |
| xml_etree_generate   | 76.2 ms                                                | 80.3 ms: 1.05x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.05x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 8.36 ms                                                | 8.90 ms: 1.07x slower                                                  |
| python_startup_no_site | 5.96 ms                                                | 6.46 ms: 1.08x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.07x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_xml      | 52.1 ms                                                | 46.9 ms: 1.11x faster                                                  |
| genshi_text     | 22.1 ms                                                | 20.8 ms: 1.06x faster                                                  |
| mako            | 9.85 ms                                                | 9.62 ms: 1.02x faster                                                  |
| django_template | 32.5 ms                                                | 32.7 ms: 1.01x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.05x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps              | 12.7 ms                                                | 9.51 ms: 1.33x faster                                                  |
| unpickle_pure_python    | 225 us                                                 | 196 us: 1.15x faster                                                   |
| deltablue               | 3.64 ms                                                | 3.21 ms: 1.13x faster                                                  |
| json_loads              | 26.2 us                                                | 23.5 us: 1.12x faster                                                  |
| genshi_xml              | 52.1 ms                                                | 46.9 ms: 1.11x faster                                                  |
| nqueens                 | 85.0 ms                                                | 77.7 ms: 1.09x faster                                                  |
| scimark_sparse_mat_mult | 4.40 ms                                                | 4.03 ms: 1.09x faster                                                  |
| richards                | 46.2 ms                                                | 42.7 ms: 1.08x faster                                                  |
| scimark_sor             | 115 ms                                                 | 107 ms: 1.08x faster                                                   |
| pycparser               | 1.17 sec                                               | 1.09 sec: 1.07x faster                                                 |
| scimark_fft             | 329 ms                                                 | 307 ms: 1.07x faster                                                   |
| go                      | 143 ms                                                 | 134 ms: 1.07x faster                                                   |
| hexiom                  | 6.35 ms                                                | 5.93 ms: 1.07x faster                                                  |
| regex_compile           | 136 ms                                                 | 128 ms: 1.06x faster                                                   |
| sympy_str               | 287 ms                                                 | 270 ms: 1.06x faster                                                   |
| genshi_text             | 22.1 ms                                                | 20.8 ms: 1.06x faster                                                  |
| pickle_pure_python      | 304 us                                                 | 286 us: 1.06x faster                                                   |
| deepcopy_memo           | 36.4 us                                                | 34.3 us: 1.06x faster                                                  |
| logging_simple          | 6.06 us                                                | 5.72 us: 1.06x faster                                                  |
| logging_silent          | 98.5 ns                                                | 93.2 ns: 1.06x faster                                                  |
| logging_format          | 6.62 us                                                | 6.27 us: 1.06x faster                                                  |
| json                    | 4.95 ms                                                | 4.69 ms: 1.05x faster                                                  |
| sympy_integrate         | 20.9 ms                                                | 19.8 ms: 1.05x faster                                                  |
| xml_etree_parse         | 158 ms                                                 | 150 ms: 1.05x faster                                                   |
| sympy_sum               | 166 ms                                                 | 158 ms: 1.05x faster                                                   |
| float                   | 76.3 ms                                                | 72.9 ms: 1.05x faster                                                  |
| chaos                   | 68.6 ms                                                | 65.8 ms: 1.04x faster                                                  |
| pyflate                 | 417 ms                                                 | 400 ms: 1.04x faster                                                   |
| spectral_norm           | 99.9 ms                                                | 96.0 ms: 1.04x faster                                                  |
| coverage                | 101 ms                                                 | 96.6 ms: 1.04x faster                                                  |
| gunicorn                | 1.12 ms                                                | 1.08 ms: 1.04x faster                                                  |
| regex_v8                | 22.3 ms                                                | 21.5 ms: 1.04x faster                                                  |
| fannkuch                | 388 ms                                                 | 374 ms: 1.04x faster                                                   |
| sympy_expand            | 472 ms                                                 | 455 ms: 1.04x faster                                                   |
| aiohttp                 | 1.05 ms                                                | 1.01 ms: 1.04x faster                                                  |
| mdp                     | 2.62 sec                                               | 2.53 sec: 1.04x faster                                                 |
| pidigits                | 199 ms                                                 | 193 ms: 1.03x faster                                                   |
| deepcopy                | 344 us                                                 | 332 us: 1.03x faster                                                   |
| html5lib                | 63.2 ms                                                | 61.1 ms: 1.03x faster                                                  |
| telco                   | 6.62 ms                                                | 6.41 ms: 1.03x faster                                                  |
| pickle_list             | 4.17 us                                                | 4.04 us: 1.03x faster                                                  |
| sqlglot_optimize        | 53.0 ms                                                | 51.4 ms: 1.03x faster                                                  |
| bench_thread_pool       | 810 us                                                 | 786 us: 1.03x faster                                                   |
| pprint_pformat          | 1.44 sec                                               | 1.40 sec: 1.03x faster                                                 |
| unpickle                | 13.3 us                                                | 12.9 us: 1.03x faster                                                  |
| scimark_monte_carlo     | 68.3 ms                                                | 66.4 ms: 1.03x faster                                                  |
| 2to3                    | 257 ms                                                 | 251 ms: 1.03x faster                                                   |
| docutils                | 2.60 sec                                               | 2.53 sec: 1.02x faster                                                 |
| mako                    | 9.85 ms                                                | 9.62 ms: 1.02x faster                                                  |
| tornado_http            | 96.6 ms                                                | 94.4 ms: 1.02x faster                                                  |
| coroutines              | 26.1 ms                                                | 25.5 ms: 1.02x faster                                                  |
| sqlalchemy_declarative  | 139 ms                                                 | 136 ms: 1.02x faster                                                   |
| pickle_dict             | 31.4 us                                                | 30.8 us: 1.02x faster                                                  |
| sqlglot_normalize       | 108 ms                                                 | 106 ms: 1.01x faster                                                   |
| dulwich_log             | 63.9 ms                                                | 63.1 ms: 1.01x faster                                                  |
| pathlib                 | 18.2 ms                                                | 17.9 ms: 1.01x faster                                                  |
| crypto_pyaes            | 73.9 ms                                                | 73.0 ms: 1.01x faster                                                  |
| unpickle_list           | 4.95 us                                                | 4.90 us: 1.01x faster                                                  |
| nbody                   | 95.0 ms                                                | 94.3 ms: 1.01x faster                                                  |
| raytrace                | 290 ms                                                 | 288 ms: 1.01x faster                                                   |
| chameleon               | 6.41 ms                                                | 6.37 ms: 1.01x faster                                                  |
| xml_etree_iterparse     | 103 ms                                                 | 103 ms: 1.01x slower                                                   |
| django_template         | 32.5 ms                                                | 32.7 ms: 1.01x slower                                                  |
| async_tree_io           | 1.31 sec                                               | 1.32 sec: 1.01x slower                                                 |
| deepcopy_reduce         | 2.97 us                                                | 3.01 us: 1.01x slower                                                  |
| pickle                  | 9.79 us                                                | 9.99 us: 1.02x slower                                                  |
| sqlite_synth            | 2.49 us                                                | 2.56 us: 1.03x slower                                                  |
| sqlglot_parse           | 1.37 ms                                                | 1.42 ms: 1.03x slower                                                  |
| sqlglot_transpile       | 1.66 ms                                                | 1.72 ms: 1.04x slower                                                  |
| xml_etree_process       | 53.8 ms                                                | 55.8 ms: 1.04x slower                                                  |
| generators              | 72.2 ms                                                | 75.2 ms: 1.04x slower                                                  |
| async_tree_memoization  | 625 ms                                                 | 651 ms: 1.04x slower                                                   |
| xml_etree_generate      | 76.2 ms                                                | 80.3 ms: 1.05x slower                                                  |
| python_startup          | 8.36 ms                                                | 8.90 ms: 1.07x slower                                                  |
| regex_dna               | 203 ms                                                 | 219 ms: 1.08x slower                                                   |
| python_startup_no_site  | 5.96 ms                                                | 6.46 ms: 1.08x slower                                                  |
| async_generators        | 359 ms                                                 | 424 ms: 1.18x slower                                                   |
| Geometric mean          | (ref)                                                  | 1.03x faster                                                           |

Benchmark hidden because not significant (10): unpack_sequence, scimark_lu, thrift, async_tree_none, pprint_safe_repr, bench_mp_pool, async_tree_cpu_io_mixed, regex_effbot, sqlalchemy_imperative, meteor_contest
Ignored benchmarks (3) of /home/mdboom/Work/builds/benchmarking/results/bm-20221024-3.11.0-deaf509/bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509.json: flaskblogging, mypy, pylint
Ignored benchmarks (5) of /home/mdboom/Work/builds/benchmarking/results/bm-20230208-3.12.0a5+-eb49d32/bm-20230208-linux-x86_64-python-eb49d32b9af0b3b01a55-3.12.0a5+-eb49d32.json: asyncio_tcp, create_gc_cycles, djangocms, gc_traversal, mypy2
