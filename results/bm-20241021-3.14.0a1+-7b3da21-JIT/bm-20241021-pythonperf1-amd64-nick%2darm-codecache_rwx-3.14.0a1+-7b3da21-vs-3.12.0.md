# Results vs. 3.12.0

- fork: nick-arm
- ref: codecache_rwx
- machine: windows-amd64
- commit hash: 7b3da21
- commit date: 2024-10-21
- overall geometric mean: 1.04x faster
- HPT reliability: 97.07%
- HPT 99th percentile: 1.00x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 234 ms: 1.07x slower                                                     |
| docutils       | 1.66 sec                                                    | 1.81 sec: 1.09x slower                                                   |
| tornado_http   | 89.5 ms                                                     | 98.5 ms: 1.10x slower                                                    |
| Geometric mean | (ref)                                                       | 1.09x slower                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 264 ms: 1.39x faster                                                     |
| async_tree_none_tg         | 285 ms                                                      | 209 ms: 1.36x faster                                                     |
| async_tree_none            | 291 ms                                                      | 219 ms: 1.33x faster                                                     |
| async_tree_io              | 731 ms                                                      | 565 ms: 1.29x faster                                                     |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 385 ms: 1.27x faster                                                     |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 400 ms: 1.25x faster                                                     |
| async_tree_io_tg           | 771 ms                                                      | 632 ms: 1.22x faster                                                     |
| async_tree_memoization     | 339 ms                                                      | 283 ms: 1.20x faster                                                     |
| Geometric mean             | (ref)                                                       | 1.29x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| nbody          | 71.9 ms                                                     | 53.1 ms: 1.35x faster                                                    |
| float          | 56.8 ms                                                     | 48.0 ms: 1.18x faster                                                    |
| pidigits       | 152 ms                                                      | 148 ms: 1.03x faster                                                     |
| Geometric mean | (ref)                                                       | 1.18x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 116 ms: 1.09x faster                                                     |
| regex_compile  | 87.5 ms                                                     | 82.6 ms: 1.06x faster                                                    |
| regex_effbot   | 1.62 ms                                                     | 1.60 ms: 1.01x faster                                                    |
| regex_v8       | 14.2 ms                                                     | 15.0 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                       | 1.02x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| xml_etree_generate   | 55.8 ms                                                     | 50.5 ms: 1.11x faster                                                    |
| tomli_loads          | 1.37 sec                                                    | 1.27 sec: 1.07x faster                                                   |
| xml_etree_process    | 37.7 ms                                                     | 35.3 ms: 1.07x faster                                                    |
| xml_etree_iterparse  | 65.2 ms                                                     | 63.4 ms: 1.03x faster                                                    |
| pickle_pure_python   | 195 us                                                      | 197 us: 1.01x slower                                                     |
| xml_etree_parse      | 93.0 ms                                                     | 94.0 ms: 1.01x slower                                                    |
| json_loads           | 13.9 us                                                     | 14.4 us: 1.04x slower                                                    |
| unpickle_pure_python | 133 us                                                      | 141 us: 1.06x slower                                                     |
| json_dumps           | 5.70 ms                                                     | 6.20 ms: 1.09x slower                                                    |
| Geometric mean       | (ref)                                                       | 1.01x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 18.2 ms: 1.12x slower                                                    |
| python_startup         | 19.5 ms                                                     | 24.2 ms: 1.25x slower                                                    |
| Geometric mean         | (ref)                                                       | 1.18x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|-----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 7.09 ms                                                     | 5.09 ms: 1.39x faster                                                    |
| django_template | 22.9 ms                                                     | 26.6 ms: 1.16x slower                                                    |
| Geometric mean  | (ref)                                                       | 1.10x faster                                                             |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21 |
|----------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| deepcopy_memo              | 23.7 us                                                     | 16.2 us: 1.46x faster                                                    |
| mako                       | 7.09 ms                                                     | 5.09 ms: 1.39x faster                                                    |
| async_tree_memoization_tg  | 367 ms                                                      | 264 ms: 1.39x faster                                                     |
| async_tree_none_tg         | 285 ms                                                      | 209 ms: 1.36x faster                                                     |
| nbody                      | 71.9 ms                                                     | 53.1 ms: 1.35x faster                                                    |
| async_tree_none            | 291 ms                                                      | 219 ms: 1.33x faster                                                     |
| async_tree_io              | 731 ms                                                      | 565 ms: 1.29x faster                                                     |
| comprehensions             | 14.1 us                                                     | 10.9 us: 1.29x faster                                                    |
| deepcopy                   | 238 us                                                      | 187 us: 1.28x faster                                                     |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 385 ms: 1.27x faster                                                     |
| spectral_norm              | 66.9 ms                                                     | 53.2 ms: 1.26x faster                                                    |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 400 ms: 1.25x faster                                                     |
| scimark_sor                | 78.8 ms                                                     | 63.1 ms: 1.25x faster                                                    |
| scimark_monte_carlo        | 43.7 ms                                                     | 35.8 ms: 1.22x faster                                                    |
| crypto_pyaes               | 48.5 ms                                                     | 39.7 ms: 1.22x faster                                                    |
| async_tree_io_tg           | 771 ms                                                      | 632 ms: 1.22x faster                                                     |
| async_tree_memoization     | 339 ms                                                      | 283 ms: 1.20x faster                                                     |
| scimark_fft                | 184 ms                                                      | 156 ms: 1.19x faster                                                     |
| float                      | 56.8 ms                                                     | 48.0 ms: 1.18x faster                                                    |
| pprint_safe_repr           | 513 ms                                                      | 442 ms: 1.16x faster                                                     |
| pprint_pformat             | 1.05 sec                                                    | 900 ms: 1.16x faster                                                     |
| fannkuch                   | 247 ms                                                      | 213 ms: 1.16x faster                                                     |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.24 ms: 1.14x faster                                                    |
| deepcopy_reduce            | 2.09 us                                                     | 1.85 us: 1.13x faster                                                    |
| go                         | 91.6 ms                                                     | 82.4 ms: 1.11x faster                                                    |
| logging_silent             | 60.5 ns                                                     | 54.5 ns: 1.11x faster                                                    |
| xml_etree_generate         | 55.8 ms                                                     | 50.5 ms: 1.11x faster                                                    |
| scimark_lu                 | 58.9 ms                                                     | 53.3 ms: 1.11x faster                                                    |
| dulwich_log                | 44.3 ms                                                     | 40.3 ms: 1.10x faster                                                    |
| chaos                      | 43.3 ms                                                     | 39.7 ms: 1.09x faster                                                    |
| regex_dna                  | 126 ms                                                      | 116 ms: 1.09x faster                                                     |
| tomli_loads                | 1.37 sec                                                    | 1.27 sec: 1.07x faster                                                   |
| pyflate                    | 295 ms                                                      | 276 ms: 1.07x faster                                                     |
| xml_etree_process          | 37.7 ms                                                     | 35.3 ms: 1.07x faster                                                    |
| meteor_contest             | 74.6 ms                                                     | 70.0 ms: 1.07x faster                                                    |
| generators                 | 22.5 ms                                                     | 21.2 ms: 1.06x faster                                                    |
| regex_compile              | 87.5 ms                                                     | 82.6 ms: 1.06x faster                                                    |
| json                       | 3.05 ms                                                     | 2.95 ms: 1.03x faster                                                    |
| logging_simple             | 6.28 us                                                     | 6.08 us: 1.03x faster                                                    |
| pidigits                   | 152 ms                                                      | 148 ms: 1.03x faster                                                     |
| coroutines                 | 14.3 ms                                                     | 13.9 ms: 1.03x faster                                                    |
| xml_etree_iterparse        | 65.2 ms                                                     | 63.4 ms: 1.03x faster                                                    |
| bench_thread_pool          | 857 us                                                      | 837 us: 1.02x faster                                                     |
| logging_format             | 6.72 us                                                     | 6.58 us: 1.02x faster                                                    |
| regex_effbot               | 1.62 ms                                                     | 1.60 ms: 1.01x faster                                                    |
| pickle_pure_python         | 195 us                                                      | 197 us: 1.01x slower                                                     |
| pathlib                    | 80.5 ms                                                     | 81.1 ms: 1.01x slower                                                    |
| xml_etree_parse            | 93.0 ms                                                     | 94.0 ms: 1.01x slower                                                    |
| pycparser                  | 699 ms                                                      | 707 ms: 1.01x slower                                                     |
| sympy_str                  | 175 ms                                                      | 180 ms: 1.03x slower                                                     |
| json_loads                 | 13.9 us                                                     | 14.4 us: 1.04x slower                                                    |
| hexiom                     | 4.10 ms                                                     | 4.25 ms: 1.04x slower                                                    |
| sympy_sum                  | 91.5 ms                                                     | 95.5 ms: 1.04x slower                                                    |
| deltablue                  | 2.16 ms                                                     | 2.27 ms: 1.05x slower                                                    |
| regex_v8                   | 14.2 ms                                                     | 15.0 ms: 1.06x slower                                                    |
| unpickle_pure_python       | 133 us                                                      | 141 us: 1.06x slower                                                     |
| sqlglot_parse              | 804 us                                                      | 858 us: 1.07x slower                                                     |
| raytrace                   | 192 ms                                                      | 206 ms: 1.07x slower                                                     |
| 2to3                       | 218 ms                                                      | 234 ms: 1.07x slower                                                     |
| mdp                        | 1.37 sec                                                    | 1.48 sec: 1.08x slower                                                   |
| sqlglot_normalize          | 187 ms                                                      | 202 ms: 1.08x slower                                                     |
| sympy_expand               | 284 ms                                                      | 308 ms: 1.09x slower                                                     |
| json_dumps                 | 5.70 ms                                                     | 6.20 ms: 1.09x slower                                                    |
| telco                      | 4.13 ms                                                     | 4.51 ms: 1.09x slower                                                    |
| docutils                   | 1.66 sec                                                    | 1.81 sec: 1.09x slower                                                   |
| sqlglot_transpile          | 1.02 ms                                                     | 1.12 ms: 1.10x slower                                                    |
| tornado_http               | 89.5 ms                                                     | 98.5 ms: 1.10x slower                                                    |
| async_generators           | 239 ms                                                      | 267 ms: 1.12x slower                                                     |
| python_startup_no_site     | 16.2 ms                                                     | 18.2 ms: 1.12x slower                                                    |
| sqlglot_optimize           | 34.5 ms                                                     | 38.8 ms: 1.12x slower                                                    |
| richards_super             | 32.1 ms                                                     | 36.2 ms: 1.13x slower                                                    |
| richards                   | 28.4 ms                                                     | 32.0 ms: 1.13x slower                                                    |
| sympy_integrate            | 13.0 ms                                                     | 14.8 ms: 1.14x slower                                                    |
| django_template            | 22.9 ms                                                     | 26.6 ms: 1.16x slower                                                    |
| typing_runtime_protocols   | 95.1 us                                                     | 115 us: 1.21x slower                                                     |
| coverage                   | 40.8 ms                                                     | 49.5 ms: 1.21x slower                                                    |
| python_startup             | 19.5 ms                                                     | 24.2 ms: 1.25x slower                                                    |
| bench_mp_pool              | 69.2 ms                                                     | 87.2 ms: 1.26x slower                                                    |
| gc_traversal               | 1.52 ms                                                     | 1.95 ms: 1.28x slower                                                    |
| create_gc_cycles           | 752 us                                                      | 1.41 ms: 1.87x slower                                                    |
| Geometric mean             | (ref)                                                       | 1.04x faster                                                             |

Benchmark hidden because not significant (1): nqueens
Ignored benchmarks (15) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20241021-3.14.0a1+-7b3da21-JIT/bm-20241021-pythonperf1-amd64-nick%2darm-codecache_rwx-3.14.0a1+-7b3da21.json: genshi_text, genshi_xml, html5lib, pylint, sphinx, thrift

# HPT report

- Reliability score: 97.07% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: unknown