# Results vs. 3.10.4

- fork: python
- ref: 7d7d56d8b1147a6b85e1
- machine: linux-x86_64
- commit hash: 7d7d56d
- commit date: 2024-11-02
- overall geometric mean: 1.277x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.16x faster
- Memory change: 1.34x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| 2to3           | 350 ms                                                       | 319 ms: 1.10x faster                                                         |
| docutils       | 3.41 sec                                                     | 3.22 sec: 1.06x faster                                                       |
| html5lib       | 94.6 ms                                                      | 71.6 ms: 1.32x faster                                                        |
| tornado_http   | 157 ms                                                       | 124 ms: 1.26x faster                                                         |
| Geometric mean | (ref)                                                        | 1.18x faster                                                                 |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|-------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| async_tree_none         | 692 ms                                                       | 337 ms: 2.05x faster                                                         |
| async_tree_memoization  | 819 ms                                                       | 414 ms: 1.98x faster                                                         |
| async_tree_io           | 1.60 sec                                                     | 851 ms: 1.88x faster                                                         |
| async_tree_cpu_io_mixed | 936 ms                                                       | 576 ms: 1.62x faster                                                         |
| Geometric mean          | (ref)                                                        | 1.88x faster                                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| nbody          | 134 ms                                                       | 83.5 ms: 1.61x faster                                                        |
| float          | 111 ms                                                       | 79.3 ms: 1.40x faster                                                        |
| pidigits       | 271 ms                                                       | 251 ms: 1.08x faster                                                         |
| Geometric mean | (ref)                                                        | 1.34x faster                                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| regex_compile  | 190 ms                                                       | 148 ms: 1.28x faster                                                         |
| regex_dna      | 261 ms                                                       | 246 ms: 1.06x faster                                                         |
| regex_v8       | 27.2 ms                                                      | 25.5 ms: 1.06x faster                                                        |
| regex_effbot   | 3.09 ms                                                      | 3.67 ms: 1.19x slower                                                        |
| Geometric mean | (ref)                                                        | 1.05x faster                                                                 |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| unpickle_pure_python | 312 us                                                       | 221 us: 1.41x faster                                                         |
| tomli_loads          | 2.92 sec                                                     | 2.09 sec: 1.39x faster                                                       |
| pickle_pure_python   | 455 us                                                       | 339 us: 1.34x faster                                                         |
| json_dumps           | 14.5 ms                                                      | 11.0 ms: 1.32x faster                                                        |
| xml_etree_process    | 75.9 ms                                                      | 58.5 ms: 1.30x faster                                                        |
| json_loads           | 30.3 us                                                      | 24.7 us: 1.23x faster                                                        |
| xml_etree_generate   | 92.3 ms                                                      | 82.8 ms: 1.11x faster                                                        |
| xml_etree_iterparse  | 110 ms                                                       | 102 ms: 1.08x faster                                                         |
| xml_etree_parse      | 160 ms                                                       | 150 ms: 1.07x faster                                                         |
| Geometric mean       | (ref)                                                        | 1.24x faster                                                                 |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| python_startup_no_site | 7.33 ms                                                      | 9.08 ms: 1.24x slower                                                        |
| python_startup         | 11.5 ms                                                      | 16.0 ms: 1.39x slower                                                        |
| Geometric mean         | (ref)                                                        | 1.31x slower                                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| mako            | 14.7 ms                                                      | 9.48 ms: 1.55x faster                                                        |
| django_template | 50.2 ms                                                      | 42.1 ms: 1.19x faster                                                        |
| genshi_text     | 31.4 ms                                                      | 27.2 ms: 1.15x faster                                                        |
| genshi_xml      | 63.3 ms                                                      | 61.7 ms: 1.03x faster                                                        |
| Geometric mean  | (ref)                                                        | 1.22x faster                                                                 |

All benchmarks:
===============

| Benchmark                | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d |
|--------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| typing_runtime_protocols | 537 us                                                       | 180 us: 2.99x faster                                                         |
| deltablue                | 7.50 ms                                                      | 3.33 ms: 2.26x faster                                                        |
| async_tree_none          | 692 ms                                                       | 337 ms: 2.05x faster                                                         |
| async_tree_memoization   | 819 ms                                                       | 414 ms: 1.98x faster                                                         |
| async_tree_io            | 1.60 sec                                                     | 851 ms: 1.88x faster                                                         |
| logging_silent           | 167 ns                                                       | 91.3 ns: 1.83x faster                                                        |
| scimark_sor              | 180 ms                                                       | 102 ms: 1.77x faster                                                         |
| deepcopy_memo            | 49.8 us                                                      | 28.6 us: 1.74x faster                                                        |
| scimark_lu               | 167 ms                                                       | 96.5 ms: 1.73x faster                                                        |
| go                       | 262 ms                                                       | 153 ms: 1.71x faster                                                         |
| richards_super           | 90.6 ms                                                      | 53.8 ms: 1.69x faster                                                        |
| richards                 | 75.1 ms                                                      | 46.1 ms: 1.63x faster                                                        |
| crypto_pyaes             | 119 ms                                                       | 73.4 ms: 1.62x faster                                                        |
| async_tree_cpu_io_mixed  | 936 ms                                                       | 576 ms: 1.62x faster                                                         |
| nbody                    | 134 ms                                                       | 83.5 ms: 1.61x faster                                                        |
| chaos                    | 109 ms                                                       | 67.8 ms: 1.60x faster                                                        |
| scimark_monte_carlo      | 107 ms                                                       | 68.0 ms: 1.58x faster                                                        |
| mako                     | 14.7 ms                                                      | 9.48 ms: 1.55x faster                                                        |
| pyflate                  | 733 ms                                                       | 478 ms: 1.53x faster                                                         |
| raytrace                 | 489 ms                                                       | 320 ms: 1.53x faster                                                         |
| deepcopy                 | 468 us                                                       | 309 us: 1.52x faster                                                         |
| spectral_norm            | 139 ms                                                       | 93.3 ms: 1.49x faster                                                        |
| sqlglot_parse            | 2.24 ms                                                      | 1.51 ms: 1.48x faster                                                        |
| generators               | 57.3 ms                                                      | 38.7 ms: 1.48x faster                                                        |
| pylint                   | 566 ms                                                       | 385 ms: 1.47x faster                                                         |
| coroutines               | 30.3 ms                                                      | 21.1 ms: 1.44x faster                                                        |
| unpickle_pure_python     | 312 us                                                       | 221 us: 1.41x faster                                                         |
| float                    | 111 ms                                                       | 79.3 ms: 1.40x faster                                                        |
| tomli_loads              | 2.92 sec                                                     | 2.09 sec: 1.39x faster                                                       |
| comprehensions           | 26.7 us                                                      | 19.2 us: 1.39x faster                                                        |
| pathlib                  | 21.4 ms                                                      | 15.7 ms: 1.36x faster                                                        |
| deepcopy_reduce          | 4.01 us                                                      | 2.97 us: 1.35x faster                                                        |
| sqlglot_transpile        | 2.68 ms                                                      | 1.99 ms: 1.35x faster                                                        |
| pickle_pure_python       | 455 us                                                       | 339 us: 1.34x faster                                                         |
| pprint_pformat           | 2.15 sec                                                     | 1.62 sec: 1.33x faster                                                       |
| hexiom                   | 9.42 ms                                                      | 7.12 ms: 1.32x faster                                                        |
| html5lib                 | 94.6 ms                                                      | 71.6 ms: 1.32x faster                                                        |
| json_dumps               | 14.5 ms                                                      | 11.0 ms: 1.32x faster                                                        |
| logging_simple           | 8.88 us                                                      | 6.80 us: 1.31x faster                                                        |
| logging_format           | 9.64 us                                                      | 7.39 us: 1.30x faster                                                        |
| xml_etree_process        | 75.9 ms                                                      | 58.5 ms: 1.30x faster                                                        |
| pprint_safe_repr         | 1.05 sec                                                     | 816 ms: 1.29x faster                                                         |
| thrift                   | 1.18 ms                                                      | 915 us: 1.28x faster                                                         |
| regex_compile            | 190 ms                                                       | 148 ms: 1.28x faster                                                         |
| pycparser                | 1.67 sec                                                     | 1.30 sec: 1.28x faster                                                       |
| fannkuch                 | 483 ms                                                       | 382 ms: 1.26x faster                                                         |
| tornado_http             | 157 ms                                                       | 124 ms: 1.26x faster                                                         |
| scimark_fft              | 361 ms                                                       | 290 ms: 1.25x faster                                                         |
| dulwich_log              | 81.1 ms                                                      | 66.0 ms: 1.23x faster                                                        |
| json_loads               | 30.3 us                                                      | 24.7 us: 1.23x faster                                                        |
| django_template          | 50.2 ms                                                      | 42.1 ms: 1.19x faster                                                        |
| sqlalchemy_imperative    | 22.7 ms                                                      | 19.3 ms: 1.18x faster                                                        |
| bench_thread_pool        | 1.14 ms                                                      | 970 us: 1.18x faster                                                         |
| mdp                      | 3.01 sec                                                     | 2.58 sec: 1.16x faster                                                       |
| scimark_sparse_mat_mult  | 5.08 ms                                                      | 4.39 ms: 1.16x faster                                                        |
| genshi_text              | 31.4 ms                                                      | 27.2 ms: 1.15x faster                                                        |
| nqueens                  | 115 ms                                                       | 99.8 ms: 1.15x faster                                                        |
| json                     | 5.86 ms                                                      | 5.17 ms: 1.13x faster                                                        |
| sqlalchemy_declarative   | 190 ms                                                       | 169 ms: 1.12x faster                                                         |
| sympy_expand             | 600 ms                                                       | 537 ms: 1.12x faster                                                         |
| xml_etree_generate       | 92.3 ms                                                      | 82.8 ms: 1.11x faster                                                        |
| sympy_str                | 360 ms                                                       | 323 ms: 1.11x faster                                                         |
| async_generators         | 421 ms                                                       | 378 ms: 1.11x faster                                                         |
| sqlite_synth             | 2.99 us                                                      | 2.71 us: 1.10x faster                                                        |
| sympy_sum                | 193 ms                                                       | 175 ms: 1.10x faster                                                         |
| 2to3                     | 350 ms                                                       | 319 ms: 1.10x faster                                                         |
| sqlglot_normalize        | 144 ms                                                       | 133 ms: 1.08x faster                                                         |
| xml_etree_iterparse      | 110 ms                                                       | 102 ms: 1.08x faster                                                         |
| pidigits                 | 271 ms                                                       | 251 ms: 1.08x faster                                                         |
| xml_etree_parse          | 160 ms                                                       | 150 ms: 1.07x faster                                                         |
| regex_dna                | 261 ms                                                       | 246 ms: 1.06x faster                                                         |
| regex_v8                 | 27.2 ms                                                      | 25.5 ms: 1.06x faster                                                        |
| docutils                 | 3.41 sec                                                     | 3.22 sec: 1.06x faster                                                       |
| meteor_contest           | 138 ms                                                       | 132 ms: 1.05x faster                                                         |
| asyncio_websockets       | 390 ms                                                       | 380 ms: 1.03x faster                                                         |
| genshi_xml               | 63.3 ms                                                      | 61.7 ms: 1.03x faster                                                        |
| sympy_integrate          | 28.2 ms                                                      | 27.5 ms: 1.02x faster                                                        |
| sqlglot_optimize         | 70.1 ms                                                      | 69.5 ms: 1.01x faster                                                        |
| telco                    | 7.23 ms                                                      | 7.67 ms: 1.06x slower                                                        |
| regex_effbot             | 3.09 ms                                                      | 3.67 ms: 1.19x slower                                                        |
| python_startup_no_site   | 7.33 ms                                                      | 9.08 ms: 1.24x slower                                                        |
| coverage                 | 63.3 ms                                                      | 80.3 ms: 1.27x slower                                                        |
| python_startup           | 11.5 ms                                                      | 16.0 ms: 1.39x slower                                                        |
| gc_traversal             | 3.42 ms                                                      | 5.24 ms: 1.53x slower                                                        |
| create_gc_cycles         | 1.76 ms                                                      | 2.88 ms: 1.63x slower                                                        |
| bench_mp_pool            | 6.37 ms                                                      | 1.97 sec: 309.62x slower                                                     |
| Geometric mean           | (ref)                                                        | 1.20x faster                                                                 |
Ignored benchmarks (14) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (9) of results/bm-20241102-3.14.0a1+-7d7d56d-JIT/bm-20241102-pythonperf2-x86_64-python-7d7d56d8b1147a6b85e1-3.14.0a1+-7d7d56d.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, connected_components, k_core, shortest_path, sphinx

- Geometric mean (including insignificant results): 1.277x faster
# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.21x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.16x

# Memory
- memory change: 1.34x