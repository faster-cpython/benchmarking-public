
# Results vs. 3.10.4

- fork: python
- ref: b1b375e2670a58fc37cb
- machine: windows-amd64
- commit hash: b1b375e
- commit date: 2023-02-19
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.16x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 199 ms: 1.19x faster                                                        |
| chameleon      | 5.92 ms                                                     | 4.63 ms: 1.28x faster                                                       |
| docutils       | 1.89 sec                                                    | 1.56 sec: 1.22x faster                                                      |
| html5lib       | 46.5 ms                                                     | 35.0 ms: 1.33x faster                                                       |
| tornado_http   | 109 ms                                                      | 91.4 ms: 1.19x faster                                                       |
| Geometric mean | (ref)                                                       | 1.24x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 48.6 ms: 1.24x faster                                                       |
| nbody          | 69.3 ms                                                     | 61.9 ms: 1.12x faster                                                       |
| pidigits       | 145 ms                                                      | 150 ms: 1.04x slower                                                        |
| Geometric mean | (ref)                                                       | 1.10x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 79.1 ms: 1.31x faster                                                       |
| regex_dna      | 132 ms                                                      | 120 ms: 1.10x faster                                                        |
| regex_v8       | 15.0 ms                                                     | 14.2 ms: 1.06x faster                                                       |
| regex_effbot   | 1.66 ms                                                     | 1.58 ms: 1.06x faster                                                       |
| Geometric mean | (ref)                                                       | 1.13x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.37 ms: 1.58x faster                                                       |
| pickle_pure_python   | 257 us                                                      | 173 us: 1.49x faster                                                        |
| unpickle_pure_python | 171 us                                                      | 122 us: 1.40x faster                                                        |
| xml_etree_process    | 43.4 ms                                                     | 35.8 ms: 1.22x faster                                                       |
| unpickle             | 9.17 us                                                     | 7.87 us: 1.17x faster                                                       |
| json_loads           | 14.2 us                                                     | 12.8 us: 1.11x faster                                                       |
| xml_etree_parse      | 102 ms                                                      | 92.5 ms: 1.10x faster                                                       |
| xml_etree_generate   | 54.5 ms                                                     | 51.1 ms: 1.07x faster                                                       |
| xml_etree_iterparse  | 63.5 ms                                                     | 61.7 ms: 1.03x faster                                                       |
| unpickle_list        | 2.81 us                                                     | 2.75 us: 1.02x faster                                                       |
| pickle_list          | 2.59 us                                                     | 2.71 us: 1.05x slower                                                       |
| pickle               | 6.80 us                                                     | 7.13 us: 1.05x slower                                                       |
| pickle_dict          | 16.9 us                                                     | 18.7 us: 1.10x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.14x faster                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.7 ms: 1.07x faster                                                       |
| python_startup_no_site | 15.5 ms                                                     | 15.8 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                       | 1.02x faster                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|-----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 6.19 ms: 1.42x faster                                                       |
| genshi_text     | 19.0 ms                                                     | 14.0 ms: 1.36x faster                                                       |
| django_template | 28.2 ms                                                     | 21.3 ms: 1.32x faster                                                       |
| genshi_xml      | 40.5 ms                                                     | 32.0 ms: 1.27x faster                                                       |
| Geometric mean  | (ref)                                                       | 1.34x faster                                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230219-pythonperf1-amd64-python-b1b375e2670a58fc37cb-3.12.0a5+-b1b375e |
|-------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 1.96 ms: 2.13x faster                                                       |
| logging_silent          | 96.4 ns                                                     | 54.2 ns: 1.78x faster                                                       |
| richards                | 41.2 ms                                                     | 23.9 ms: 1.72x faster                                                       |
| scimark_sor             | 105 ms                                                      | 63.5 ms: 1.65x faster                                                       |
| go                      | 136 ms                                                      | 83.6 ms: 1.63x faster                                                       |
| mypy2                   | 352 ms                                                      | 217 ms: 1.62x faster                                                        |
| json_dumps              | 8.50 ms                                                     | 5.37 ms: 1.58x faster                                                       |
| raytrace                | 271 ms                                                      | 176 ms: 1.54x faster                                                        |
| scimark_lu              | 85.4 ms                                                     | 56.1 ms: 1.52x faster                                                       |
| pyflate                 | 387 ms                                                      | 256 ms: 1.51x faster                                                        |
| pickle_pure_python      | 257 us                                                      | 173 us: 1.49x faster                                                        |
| asyncio_tcp             | 712 ms                                                      | 483 ms: 1.47x faster                                                        |
| unpack_sequence         | 37.8 ns                                                     | 25.9 ns: 1.46x faster                                                       |
| generators              | 31.6 ms                                                     | 21.6 ms: 1.46x faster                                                       |
| hexiom                  | 5.52 ms                                                     | 3.84 ms: 1.44x faster                                                       |
| mako                    | 8.80 ms                                                     | 6.19 ms: 1.42x faster                                                       |
| chaos                   | 58.9 ms                                                     | 41.6 ms: 1.42x faster                                                       |
| scimark_monte_carlo     | 55.9 ms                                                     | 39.7 ms: 1.41x faster                                                       |
| unpickle_pure_python    | 171 us                                                      | 122 us: 1.40x faster                                                        |
| crypto_pyaes            | 62.3 ms                                                     | 44.6 ms: 1.40x faster                                                       |
| async_tree_io           | 1.07 sec                                                    | 775 ms: 1.37x faster                                                        |
| sqlglot_parse           | 1.22 ms                                                     | 888 us: 1.37x faster                                                        |
| genshi_text             | 19.0 ms                                                     | 14.0 ms: 1.36x faster                                                       |
| async_tree_memoization  | 497 ms                                                      | 366 ms: 1.36x faster                                                        |
| comprehensions          | 16.0 us                                                     | 11.9 us: 1.34x faster                                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.09 ms: 1.34x faster                                                       |
| async_tree_none         | 420 ms                                                      | 316 ms: 1.33x faster                                                        |
| html5lib                | 46.5 ms                                                     | 35.0 ms: 1.33x faster                                                       |
| django_template         | 28.2 ms                                                     | 21.3 ms: 1.32x faster                                                       |
| thrift                  | 615 us                                                      | 470 us: 1.31x faster                                                        |
| deepcopy_memo           | 28.5 us                                                     | 21.8 us: 1.31x faster                                                       |
| regex_compile           | 103 ms                                                      | 79.1 ms: 1.31x faster                                                       |
| pycparser               | 868 ms                                                      | 672 ms: 1.29x faster                                                        |
| chameleon               | 5.92 ms                                                     | 4.63 ms: 1.28x faster                                                       |
| async_tree_cpu_io_mixed | 609 ms                                                      | 479 ms: 1.27x faster                                                        |
| spectral_norm           | 78.0 ms                                                     | 61.5 ms: 1.27x faster                                                       |
| genshi_xml              | 40.5 ms                                                     | 32.0 ms: 1.27x faster                                                       |
| pprint_pformat          | 1.21 sec                                                    | 961 ms: 1.26x faster                                                        |
| sqlglot_optimize        | 39.0 ms                                                     | 31.3 ms: 1.24x faster                                                       |
| float                   | 60.2 ms                                                     | 48.6 ms: 1.24x faster                                                       |
| pprint_safe_repr        | 589 ms                                                      | 476 ms: 1.24x faster                                                        |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.17 ms: 1.22x faster                                                       |
| fannkuch                | 258 ms                                                      | 212 ms: 1.22x faster                                                        |
| docutils                | 1.89 sec                                                    | 1.56 sec: 1.22x faster                                                      |
| xml_etree_process       | 43.4 ms                                                     | 35.8 ms: 1.22x faster                                                       |
| deepcopy                | 255 us                                                      | 213 us: 1.20x faster                                                        |
| tornado_http            | 109 ms                                                      | 91.4 ms: 1.19x faster                                                       |
| 2to3                    | 237 ms                                                      | 199 ms: 1.19x faster                                                        |
| dulwich_log             | 47.6 ms                                                     | 40.5 ms: 1.18x faster                                                       |
| nqueens                 | 67.0 ms                                                     | 57.1 ms: 1.17x faster                                                       |
| unpickle                | 9.17 us                                                     | 7.87 us: 1.17x faster                                                       |
| sqlglot_normalize       | 202 ms                                                      | 174 ms: 1.16x faster                                                        |
| sympy_expand            | 315 ms                                                      | 272 ms: 1.16x faster                                                        |
| scimark_fft             | 193 ms                                                      | 167 ms: 1.16x faster                                                        |
| deepcopy_reduce         | 2.16 us                                                     | 1.87 us: 1.15x faster                                                       |
| sympy_integrate         | 14.8 ms                                                     | 12.9 ms: 1.15x faster                                                       |
| sympy_str               | 188 ms                                                      | 164 ms: 1.15x faster                                                        |
| coroutines              | 15.9 ms                                                     | 14.1 ms: 1.13x faster                                                       |
| sympy_sum               | 104 ms                                                      | 92.8 ms: 1.12x faster                                                       |
| mdp                     | 1.71 sec                                                    | 1.53 sec: 1.12x faster                                                      |
| nbody                   | 69.3 ms                                                     | 61.9 ms: 1.12x faster                                                       |
| bench_thread_pool       | 946 us                                                      | 846 us: 1.12x faster                                                        |
| json_loads              | 14.2 us                                                     | 12.8 us: 1.11x faster                                                       |
| create_gc_cycles        | 782 us                                                      | 708 us: 1.10x faster                                                        |
| xml_etree_parse         | 102 ms                                                      | 92.5 ms: 1.10x faster                                                       |
| regex_dna               | 132 ms                                                      | 120 ms: 1.10x faster                                                        |
| python_startup          | 20.0 ms                                                     | 18.7 ms: 1.07x faster                                                       |
| xml_etree_generate      | 54.5 ms                                                     | 51.1 ms: 1.07x faster                                                       |
| regex_v8                | 15.0 ms                                                     | 14.2 ms: 1.06x faster                                                       |
| regex_effbot            | 1.66 ms                                                     | 1.58 ms: 1.06x faster                                                       |
| json                    | 3.05 ms                                                     | 2.89 ms: 1.06x faster                                                       |
| logging_format          | 6.66 us                                                     | 6.41 us: 1.04x faster                                                       |
| xml_etree_iterparse     | 63.5 ms                                                     | 61.7 ms: 1.03x faster                                                       |
| unpickle_list           | 2.81 us                                                     | 2.75 us: 1.02x faster                                                       |
| pathlib                 | 77.4 ms                                                     | 75.8 ms: 1.02x faster                                                       |
| telco                   | 3.78 ms                                                     | 3.72 ms: 1.02x faster                                                       |
| sqlite_synth            | 1.84 us                                                     | 1.81 us: 1.02x faster                                                       |
| async_generators        | 224 ms                                                      | 220 ms: 1.02x faster                                                        |
| meteor_contest          | 72.5 ms                                                     | 71.8 ms: 1.01x faster                                                       |
| python_startup_no_site  | 15.5 ms                                                     | 15.8 ms: 1.02x slower                                                       |
| pidigits                | 145 ms                                                      | 150 ms: 1.04x slower                                                        |
| pickle_list             | 2.59 us                                                     | 2.71 us: 1.05x slower                                                       |
| pickle                  | 6.80 us                                                     | 7.13 us: 1.05x slower                                                       |
| bench_mp_pool           | 60.7 ms                                                     | 64.9 ms: 1.07x slower                                                       |
| pickle_dict             | 16.9 us                                                     | 18.7 us: 1.10x slower                                                       |
| gc_traversal            | 1.34 ms                                                     | 1.48 ms: 1.10x slower                                                       |
| dask                    | 305 ms                                                      | 350 ms: 1.15x slower                                                        |
| coverage                | 40.0 ms                                                     | 50.6 ms: 1.26x slower                                                       |
| Geometric mean          | (ref)                                                       | 1.22x faster                                                                |

Benchmark hidden because not significant (1): logging_simple
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.19x
- 95% likely to have a speedup of 1.18x
- 99% likely to have a speedup of 1.16x
