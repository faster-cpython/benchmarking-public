# Results vs. 3.13.0

- fork: faster-cpython
- ref: experimental_branch_
- machine: linux-aarch64
- commit hash: 1a2b0b8
- commit date: 2024-07-31
- overall geometric mean: 1.01x faster
- HPT reliability: 65.92%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.01x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 303 ms: 1.01x faster                                                              |
| docutils       | 2.91 sec                                                 | 3.20 sec: 1.10x slower                                                            |
| html5lib       | 64.3 ms                                                  | 68.6 ms: 1.07x slower                                                             |
| Geometric mean | (ref)                                                    | 1.03x slower                                                                      |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 649 ms                                                   | 533 ms: 1.22x faster                                                              |
| async_tree_none_tg         | 467 ms                                                   | 404 ms: 1.16x faster                                                              |
| async_tree_none            | 493 ms                                                   | 436 ms: 1.13x faster                                                              |
| async_tree_memoization     | 626 ms                                                   | 557 ms: 1.12x faster                                                              |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 726 ms: 1.05x faster                                                              |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 713 ms: 1.03x faster                                                              |
| async_generators           | 496 ms                                                   | 486 ms: 1.02x faster                                                              |
| coroutines                 | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                             |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.20 sec: 1.01x slower                                                            |
| async_tree_io_tg           | 1.09 sec                                                 | 1.12 sec: 1.03x slower                                                            |
| Geometric mean             | (ref)                                                    | 1.05x faster                                                                      |

Benchmark hidden because not significant (3): asyncio_tcp, asyncio_websockets, async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| nbody          | 114 ms                                                   | 111 ms: 1.03x faster                                                              |
| float          | 94.4 ms                                                  | 93.7 ms: 1.01x faster                                                             |
| Geometric mean | (ref)                                                    | 1.01x faster                                                                      |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| regex_v8       | 31.5 ms                                                  | 30.5 ms: 1.03x faster                                                             |
| regex_dna      | 246 ms                                                   | 253 ms: 1.03x slower                                                              |
| Geometric mean | (ref)                                                    | 1.00x slower                                                                      |

Benchmark hidden because not significant (2): regex_compile, regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| unpickle_pure_python | 254 us                                                   | 250 us: 1.02x faster                                                              |
| tomli_loads          | 2.53 sec                                                 | 2.56 sec: 1.01x slower                                                            |
| json_loads           | 31.4 us                                                  | 32.6 us: 1.04x slower                                                             |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                                      |

Benchmark hidden because not significant (6): xml_etree_parse, xml_etree_process, json_dumps, xml_etree_generate, pickle_pure_python, xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

Benchmark hidden because not significant (2): python_startup, python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| mako           | 13.3 ms                                                  | 13.5 ms: 1.01x slower                                                             |
| genshi_xml     | 60.2 ms                                                  | 61.6 ms: 1.02x slower                                                             |
| Geometric mean | (ref)                                                    | 1.01x slower                                                                      |

Benchmark hidden because not significant (2): django_template, genshi_text

All benchmarks:
===============

| Benchmark                  | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20240731-arminc-aarch64-faster%2dcpython-experimental_branch_-3.14.0a0-1a2b0b8 |
|----------------------------|:--------------------------------------------------------:|:---------------------------------------------------------------------------------:|
| deepcopy                   | 451 us                                                   | 329 us: 1.37x faster                                                              |
| deepcopy_memo              | 51.0 us                                                  | 38.3 us: 1.33x faster                                                             |
| async_tree_memoization_tg  | 649 ms                                                   | 533 ms: 1.22x faster                                                              |
| deepcopy_reduce            | 4.07 us                                                  | 3.45 us: 1.18x faster                                                             |
| async_tree_none_tg         | 467 ms                                                   | 404 ms: 1.16x faster                                                              |
| async_tree_none            | 493 ms                                                   | 436 ms: 1.13x faster                                                              |
| async_tree_memoization     | 626 ms                                                   | 557 ms: 1.12x faster                                                              |
| generators                 | 37.8 ms                                                  | 35.4 ms: 1.07x faster                                                             |
| async_tree_cpu_io_mixed    | 764 ms                                                   | 726 ms: 1.05x faster                                                              |
| logging_silent             | 135 ns                                                   | 130 ns: 1.04x faster                                                              |
| bench_mp_pool              | 7.30 ms                                                  | 7.04 ms: 1.04x faster                                                             |
| pycparser                  | 1.26 sec                                                 | 1.22 sec: 1.04x faster                                                            |
| pathlib                    | 22.4 ms                                                  | 21.7 ms: 1.03x faster                                                             |
| async_tree_cpu_io_mixed_tg | 736 ms                                                   | 713 ms: 1.03x faster                                                              |
| regex_v8                   | 31.5 ms                                                  | 30.5 ms: 1.03x faster                                                             |
| bench_thread_pool          | 1.28 ms                                                  | 1.24 ms: 1.03x faster                                                             |
| nbody                      | 114 ms                                                   | 111 ms: 1.03x faster                                                              |
| async_generators           | 496 ms                                                   | 486 ms: 1.02x faster                                                              |
| richards_super             | 60.3 ms                                                  | 59.2 ms: 1.02x faster                                                             |
| unpickle_pure_python       | 254 us                                                   | 250 us: 1.02x faster                                                              |
| go                         | 163 ms                                                   | 160 ms: 1.02x faster                                                              |
| chaos                      | 68.8 ms                                                  | 67.6 ms: 1.02x faster                                                             |
| scimark_sparse_mat_mult    | 6.58 ms                                                  | 6.50 ms: 1.01x faster                                                             |
| richards                   | 53.5 ms                                                  | 52.9 ms: 1.01x faster                                                             |
| 2to3                       | 306 ms                                                   | 303 ms: 1.01x faster                                                              |
| mdp                        | 3.36 sec                                                 | 3.34 sec: 1.01x faster                                                            |
| float                      | 94.4 ms                                                  | 93.7 ms: 1.01x faster                                                             |
| bpe_tokeniser              | 5.90 sec                                                 | 5.87 sec: 1.00x faster                                                            |
| logging_simple             | 7.04 us                                                  | 7.01 us: 1.00x faster                                                             |
| coroutines                 | 28.2 ms                                                  | 28.5 ms: 1.01x slower                                                             |
| asyncio_tcp_ssl            | 2.18 sec                                                 | 2.20 sec: 1.01x slower                                                            |
| tomli_loads                | 2.53 sec                                                 | 2.56 sec: 1.01x slower                                                            |
| nqueens                    | 98.7 ms                                                  | 99.8 ms: 1.01x slower                                                             |
| sympy_integrate            | 21.0 ms                                                  | 21.2 ms: 1.01x slower                                                             |
| mako                       | 13.3 ms                                                  | 13.5 ms: 1.01x slower                                                             |
| pprint_pformat             | 1.90 sec                                                 | 1.93 sec: 1.02x slower                                                            |
| json                       | 5.61 ms                                                  | 5.70 ms: 1.02x slower                                                             |
| sympy_str                  | 264 ms                                                   | 268 ms: 1.02x slower                                                              |
| pprint_safe_repr           | 926 ms                                                   | 942 ms: 1.02x slower                                                              |
| sympy_expand               | 455 ms                                                   | 463 ms: 1.02x slower                                                              |
| scimark_sor                | 159 ms                                                   | 162 ms: 1.02x slower                                                              |
| thrift                     | 946 us                                                   | 965 us: 1.02x slower                                                              |
| genshi_xml                 | 60.2 ms                                                  | 61.6 ms: 1.02x slower                                                             |
| regex_dna                  | 246 ms                                                   | 253 ms: 1.03x slower                                                              |
| fannkuch                   | 452 ms                                                   | 465 ms: 1.03x slower                                                              |
| async_tree_io_tg           | 1.09 sec                                                 | 1.12 sec: 1.03x slower                                                            |
| sqlglot_parse              | 1.38 ms                                                  | 1.42 ms: 1.03x slower                                                             |
| dask                       | 350 ms                                                   | 363 ms: 1.04x slower                                                              |
| typing_runtime_protocols   | 192 us                                                   | 199 us: 1.04x slower                                                              |
| json_loads                 | 31.4 us                                                  | 32.6 us: 1.04x slower                                                             |
| pyflate                    | 556 ms                                                   | 580 ms: 1.04x slower                                                              |
| html5lib                   | 64.3 ms                                                  | 68.6 ms: 1.07x slower                                                             |
| gc_traversal               | 4.53 ms                                                  | 4.95 ms: 1.09x slower                                                             |
| docutils                   | 2.91 sec                                                 | 3.20 sec: 1.10x slower                                                            |
| create_gc_cycles           | 2.12 ms                                                  | 2.36 ms: 1.11x slower                                                             |
| Geometric mean             | (ref)                                                    | 1.01x faster                                                                      |

Benchmark hidden because not significant (35): tornado_http, logging_format, sqlglot_transpile, xml_etree_parse, python_startup, scimark_monte_carlo, raytrace, crypto_pyaes, sqlglot_normalize, scimark_lu, spectral_norm, regex_compile, xml_etree_process, pidigits, django_template, scimark_fft, asyncio_tcp, json_dumps, telco, meteor_contest, sqlglot_optimize, asyncio_websockets, xml_etree_generate, hexiom, pickle_pure_python, genshi_text, coverage, async_tree_io, comprehensions, deltablue, regex_effbot, sympy_sum, python_startup_no_site, xml_etree_iterparse, pylint
Ignored benchmarks (12) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 65.92% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.01x