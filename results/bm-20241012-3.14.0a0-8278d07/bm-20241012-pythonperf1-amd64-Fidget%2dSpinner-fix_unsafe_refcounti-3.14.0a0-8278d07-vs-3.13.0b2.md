# Results vs. 3.13.0b2

- fork: Fidget-Spinner
- ref: fix_unsafe_refcounti
- machine: windows-amd64
- commit hash: 8278d07
- commit date: 2024-10-12
- overall geometric mean: 1.07x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| 2to3           | 207 ms                                                          | 229 ms: 1.11x slower                                                                 |
| docutils       | 1.63 sec                                                        | 1.72 sec: 1.06x slower                                                               |
| html5lib       | 35.0 ms                                                         | 43.8 ms: 1.25x slower                                                                |
| tornado_http   | 81.8 ms                                                         | 94.1 ms: 1.15x slower                                                                |
| Geometric mean | (ref)                                                           | 1.14x slower                                                                         |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| async_tree_io_tg           | 605 ms                                                          | 581 ms: 1.04x faster                                                                 |
| async_tree_none            | 218 ms                                                          | 214 ms: 1.02x faster                                                                 |
| async_tree_cpu_io_mixed    | 389 ms                                                          | 396 ms: 1.02x slower                                                                 |
| async_tree_memoization     | 264 ms                                                          | 270 ms: 1.02x slower                                                                 |
| async_tree_cpu_io_mixed_tg | 382 ms                                                          | 395 ms: 1.03x slower                                                                 |
| Geometric mean             | (ref)                                                           | 1.01x slower                                                                         |

Benchmark hidden because not significant (3): async_tree_memoization_tg, async_tree_io, async_tree_none_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| pidigits       | 150 ms                                                          | 148 ms: 1.01x faster                                                                 |
| float          | 49.7 ms                                                         | 54.1 ms: 1.09x slower                                                                |
| nbody          | 67.6 ms                                                         | 82.9 ms: 1.23x slower                                                                |
| Geometric mean | (ref)                                                           | 1.10x slower                                                                         |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| regex_dna      | 119 ms                                                          | 114 ms: 1.04x faster                                                                 |
| regex_effbot   | 1.58 ms                                                         | 1.57 ms: 1.01x faster                                                                |
| regex_compile  | 78.0 ms                                                         | 90.8 ms: 1.16x slower                                                                |
| Geometric mean | (ref)                                                           | 1.01x slower                                                                         |

Benchmark hidden because not significant (1): regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| pickle_list          | 2.90 us                                                         | 2.97 us: 1.02x slower                                                                |
| json_loads           | 14.2 us                                                         | 14.8 us: 1.05x slower                                                                |
| pickle               | 7.11 us                                                         | 7.45 us: 1.05x slower                                                                |
| unpickle_list        | 2.62 us                                                         | 2.77 us: 1.06x slower                                                                |
| xml_etree_parse      | 90.9 ms                                                         | 96.2 ms: 1.06x slower                                                                |
| pickle_dict          | 18.1 us                                                         | 19.3 us: 1.06x slower                                                                |
| xml_etree_iterparse  | 62.3 ms                                                         | 67.0 ms: 1.07x slower                                                                |
| xml_etree_generate   | 53.2 ms                                                         | 58.0 ms: 1.09x slower                                                                |
| xml_etree_process    | 36.4 ms                                                         | 41.0 ms: 1.12x slower                                                                |
| unpickle             | 8.40 us                                                         | 9.47 us: 1.13x slower                                                                |
| json_dumps           | 5.61 ms                                                         | 6.73 ms: 1.20x slower                                                                |
| tomli_loads          | 1.35 sec                                                        | 1.65 sec: 1.22x slower                                                               |
| unpickle_pure_python | 122 us                                                          | 149 us: 1.22x slower                                                                 |
| pickle_pure_python   | 175 us                                                          | 217 us: 1.24x slower                                                                 |
| Geometric mean       | (ref)                                                           | 1.11x slower                                                                         |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| python_startup         | 20.3 ms                                                         | 22.2 ms: 1.10x slower                                                                |
| python_startup_no_site | 16.2 ms                                                         | 18.0 ms: 1.11x slower                                                                |
| Geometric mean         | (ref)                                                           | 1.10x slower                                                                         |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| mako            | 6.36 ms                                                         | 6.79 ms: 1.07x slower                                                                |
| genshi_xml      | 31.6 ms                                                         | 37.1 ms: 1.18x slower                                                                |
| genshi_text     | 14.4 ms                                                         | 17.4 ms: 1.21x slower                                                                |
| django_template | 21.7 ms                                                         | 26.3 ms: 1.21x slower                                                                |
| Geometric mean  | (ref)                                                           | 1.16x slower                                                                         |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| thrift                     | 8.11 ms                                                         | 524 us: 15.48x faster                                                                |
| deepcopy                   | 220 us                                                          | 193 us: 1.14x faster                                                                 |
| deepcopy_memo              | 22.1 us                                                         | 19.9 us: 1.11x faster                                                                |
| async_tree_io_tg           | 605 ms                                                          | 581 ms: 1.04x faster                                                                 |
| regex_dna                  | 119 ms                                                          | 114 ms: 1.04x faster                                                                 |
| async_tree_none            | 218 ms                                                          | 214 ms: 1.02x faster                                                                 |
| pidigits                   | 150 ms                                                          | 148 ms: 1.01x faster                                                                 |
| regex_effbot               | 1.58 ms                                                         | 1.57 ms: 1.01x faster                                                                |
| deepcopy_reduce            | 1.99 us                                                         | 2.02 us: 1.01x slower                                                                |
| async_tree_cpu_io_mixed    | 389 ms                                                          | 396 ms: 1.02x slower                                                                 |
| pickle_list                | 2.90 us                                                         | 2.97 us: 1.02x slower                                                                |
| async_tree_memoization     | 264 ms                                                          | 270 ms: 1.02x slower                                                                 |
| telco                      | 4.67 ms                                                         | 4.79 ms: 1.03x slower                                                                |
| async_tree_cpu_io_mixed_tg | 382 ms                                                          | 395 ms: 1.03x slower                                                                 |
| json_loads                 | 14.2 us                                                         | 14.8 us: 1.05x slower                                                                |
| pickle                     | 7.11 us                                                         | 7.45 us: 1.05x slower                                                                |
| sqlite_synth               | 1.60 us                                                         | 1.68 us: 1.05x slower                                                                |
| go                         | 82.1 ms                                                         | 86.4 ms: 1.05x slower                                                                |
| scimark_lu                 | 56.6 ms                                                         | 59.6 ms: 1.05x slower                                                                |
| docutils                   | 1.63 sec                                                        | 1.72 sec: 1.06x slower                                                               |
| create_gc_cycles           | 888 us                                                          | 939 us: 1.06x slower                                                                 |
| unpickle_list              | 2.62 us                                                         | 2.77 us: 1.06x slower                                                                |
| xml_etree_parse            | 90.9 ms                                                         | 96.2 ms: 1.06x slower                                                                |
| crypto_pyaes               | 45.5 ms                                                         | 48.1 ms: 1.06x slower                                                                |
| bench_mp_pool              | 64.8 ms                                                         | 68.7 ms: 1.06x slower                                                                |
| pathlib                    | 75.9 ms                                                         | 80.6 ms: 1.06x slower                                                                |
| sympy_sum                  | 84.4 ms                                                         | 89.8 ms: 1.06x slower                                                                |
| pickle_dict                | 18.1 us                                                         | 19.3 us: 1.06x slower                                                                |
| scimark_sparse_mat_mult    | 2.50 ms                                                         | 2.67 ms: 1.07x slower                                                                |
| mako                       | 6.36 ms                                                         | 6.79 ms: 1.07x slower                                                                |
| xml_etree_iterparse        | 62.3 ms                                                         | 67.0 ms: 1.07x slower                                                                |
| bench_thread_pool          | 768 us                                                          | 825 us: 1.08x slower                                                                 |
| spectral_norm              | 63.7 ms                                                         | 69.0 ms: 1.08x slower                                                                |
| logging_simple             | 5.78 us                                                         | 6.29 us: 1.09x slower                                                                |
| logging_format             | 6.22 us                                                         | 6.77 us: 1.09x slower                                                                |
| float                      | 49.7 ms                                                         | 54.1 ms: 1.09x slower                                                                |
| xml_etree_generate         | 53.2 ms                                                         | 58.0 ms: 1.09x slower                                                                |
| python_startup             | 20.3 ms                                                         | 22.2 ms: 1.10x slower                                                                |
| sympy_integrate            | 12.2 ms                                                         | 13.5 ms: 1.11x slower                                                                |
| 2to3                       | 207 ms                                                          | 229 ms: 1.11x slower                                                                 |
| meteor_contest             | 69.9 ms                                                         | 77.6 ms: 1.11x slower                                                                |
| python_startup_no_site     | 16.2 ms                                                         | 18.0 ms: 1.11x slower                                                                |
| pylint                     | 205 ms                                                          | 228 ms: 1.11x slower                                                                 |
| generators                 | 19.6 ms                                                         | 21.8 ms: 1.11x slower                                                                |
| sympy_str                  | 159 ms                                                          | 178 ms: 1.12x slower                                                                 |
| xml_etree_process          | 36.4 ms                                                         | 41.0 ms: 1.12x slower                                                                |
| coroutines                 | 12.8 ms                                                         | 14.3 ms: 1.12x slower                                                                |
| unpickle                   | 8.40 us                                                         | 9.47 us: 1.13x slower                                                                |
| async_generators           | 223 ms                                                          | 253 ms: 1.13x slower                                                                 |
| comprehensions             | 10.4 us                                                         | 11.8 us: 1.13x slower                                                                |
| sqlglot_optimize           | 32.7 ms                                                         | 37.1 ms: 1.13x slower                                                                |
| sympy_expand               | 271 ms                                                          | 307 ms: 1.14x slower                                                                 |
| nqueens                    | 56.7 ms                                                         | 64.6 ms: 1.14x slower                                                                |
| sqlglot_normalize          | 173 ms                                                          | 198 ms: 1.14x slower                                                                 |
| logging_silent             | 52.9 ns                                                         | 60.8 ns: 1.15x slower                                                                |
| tornado_http               | 81.8 ms                                                         | 94.1 ms: 1.15x slower                                                                |
| pyflate                    | 279 ms                                                          | 321 ms: 1.15x slower                                                                 |
| dulwich_log                | 38.0 ms                                                         | 43.9 ms: 1.15x slower                                                                |
| pprint_safe_repr           | 474 ms                                                          | 547 ms: 1.15x slower                                                                 |
| chaos                      | 38.4 ms                                                         | 44.2 ms: 1.15x slower                                                                |
| typing_runtime_protocols   | 101 us                                                          | 116 us: 1.15x slower                                                                 |
| coverage                   | 42.1 ms                                                         | 48.6 ms: 1.15x slower                                                                |
| mdp                        | 1.31 sec                                                        | 1.52 sec: 1.16x slower                                                               |
| pprint_pformat             | 966 ms                                                          | 1.12 sec: 1.16x slower                                                               |
| asyncio_tcp_ssl            | 1.48 sec                                                        | 1.72 sec: 1.16x slower                                                               |
| regex_compile              | 78.0 ms                                                         | 90.8 ms: 1.16x slower                                                                |
| sqlglot_transpile          | 955 us                                                          | 1.12 ms: 1.17x slower                                                                |
| genshi_xml                 | 31.6 ms                                                         | 37.1 ms: 1.18x slower                                                                |
| fannkuch                   | 243 ms                                                          | 286 ms: 1.18x slower                                                                 |
| hexiom                     | 3.72 ms                                                         | 4.39 ms: 1.18x slower                                                                |
| richards_super             | 30.2 ms                                                         | 35.7 ms: 1.18x slower                                                                |
| richards                   | 26.7 ms                                                         | 31.7 ms: 1.19x slower                                                                |
| scimark_monte_carlo        | 39.1 ms                                                         | 46.6 ms: 1.19x slower                                                                |
| scimark_fft                | 171 ms                                                          | 204 ms: 1.19x slower                                                                 |
| sqlglot_parse              | 748 us                                                          | 894 us: 1.19x slower                                                                 |
| scimark_sor                | 75.3 ms                                                         | 90.1 ms: 1.20x slower                                                                |
| raytrace                   | 162 ms                                                          | 194 ms: 1.20x slower                                                                 |
| json_dumps                 | 5.61 ms                                                         | 6.73 ms: 1.20x slower                                                                |
| genshi_text                | 14.4 ms                                                         | 17.4 ms: 1.21x slower                                                                |
| deltablue                  | 1.88 ms                                                         | 2.28 ms: 1.21x slower                                                                |
| django_template            | 21.7 ms                                                         | 26.3 ms: 1.21x slower                                                                |
| tomli_loads                | 1.35 sec                                                        | 1.65 sec: 1.22x slower                                                               |
| unpickle_pure_python       | 122 us                                                          | 149 us: 1.22x slower                                                                 |
| nbody                      | 67.6 ms                                                         | 82.9 ms: 1.23x slower                                                                |
| pickle_pure_python         | 175 us                                                          | 217 us: 1.24x slower                                                                 |
| html5lib                   | 35.0 ms                                                         | 43.8 ms: 1.25x slower                                                                |
| asyncio_tcp                | 471 ms                                                          | 641 ms: 1.36x slower                                                                 |
| json                       | 3.19 ms                                                         | 4.46 ms: 1.40x slower                                                                |
| Geometric mean             | (ref)                                                           | 1.07x slower                                                                         |

Benchmark hidden because not significant (6): regex_v8, pycparser, gc_traversal, async_tree_memoization_tg, async_tree_io, async_tree_none_tg
Ignored benchmarks (4) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, flaskblogging, mypy2
Ignored benchmarks (1) of results/bm-20241012-3.14.0a0-8278d07/bm-20241012-pythonperf1-amd64-Fidget%2dSpinner-fix_unsafe_refcounti-3.14.0a0-8278d07.json: unpack_sequence

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.09x
- 95% likely to have a slowdown of 1.09x
- 99% likely to have a slowdown of 1.08x

# Memory
- memory change: unknown