# Results vs. 3.12.0

- fork: python
- ref: 34653bba644aa5481613
- machine: windows-amd64
- commit hash: 34653bb
- commit date: 2024-10-22
- overall geometric mean: 1.016x faster
- HPT reliability: 83.20%
- HPT 99th percentile: 1.00x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 247 ms: 1.13x slower                                                        |
| docutils       | 1.66 sec                                                    | 1.91 sec: 1.15x slower                                                      |
| tornado_http   | 89.5 ms                                                     | 99.5 ms: 1.11x slower                                                       |
| Geometric mean | (ref)                                                       | 1.13x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 260 ms: 1.41x faster                                                        |
| async_tree_none_tg         | 285 ms                                                      | 210 ms: 1.36x faster                                                        |
| async_tree_none            | 291 ms                                                      | 216 ms: 1.35x faster                                                        |
| async_tree_io              | 731 ms                                                      | 542 ms: 1.35x faster                                                        |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 388 ms: 1.26x faster                                                        |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 399 ms: 1.26x faster                                                        |
| async_tree_memoization     | 339 ms                                                      | 277 ms: 1.22x faster                                                        |
| async_tree_io_tg           | 771 ms                                                      | 631 ms: 1.22x faster                                                        |
| Geometric mean             | (ref)                                                       | 1.30x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 71.9 ms                                                     | 56.2 ms: 1.28x faster                                                       |
| float          | 56.8 ms                                                     | 46.9 ms: 1.21x faster                                                       |
| pidigits       | 152 ms                                                      | 148 ms: 1.03x faster                                                        |
| Geometric mean | (ref)                                                       | 1.17x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 116 ms: 1.09x faster                                                        |
| regex_effbot   | 1.62 ms                                                     | 1.56 ms: 1.04x faster                                                       |
| regex_v8       | 14.2 ms                                                     | 14.7 ms: 1.03x slower                                                       |
| regex_compile  | 87.5 ms                                                     | 91.2 ms: 1.04x slower                                                       |
| Geometric mean | (ref)                                                       | 1.01x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| xml_etree_generate   | 55.8 ms                                                     | 50.5 ms: 1.11x faster                                                       |
| tomli_loads          | 1.37 sec                                                    | 1.28 sec: 1.07x faster                                                      |
| xml_etree_process    | 37.7 ms                                                     | 35.8 ms: 1.05x faster                                                       |
| xml_etree_iterparse  | 65.2 ms                                                     | 63.4 ms: 1.03x faster                                                       |
| pickle_pure_python   | 195 us                                                      | 199 us: 1.02x slower                                                        |
| xml_etree_parse      | 93.0 ms                                                     | 96.1 ms: 1.03x slower                                                       |
| json_loads           | 13.9 us                                                     | 14.4 us: 1.03x slower                                                       |
| unpickle_pure_python | 133 us                                                      | 144 us: 1.08x slower                                                        |
| json_dumps           | 5.70 ms                                                     | 6.22 ms: 1.09x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.00x slower                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 18.6 ms: 1.14x slower                                                       |
| python_startup         | 19.5 ms                                                     | 24.4 ms: 1.25x slower                                                       |
| Geometric mean         | (ref)                                                       | 1.20x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|-----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 7.09 ms                                                     | 4.96 ms: 1.43x faster                                                       |
| django_template | 22.9 ms                                                     | 28.0 ms: 1.22x slower                                                       |
| Geometric mean  | (ref)                                                       | 1.08x faster                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb |
|----------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo              | 23.7 us                                                     | 16.5 us: 1.43x faster                                                       |
| mako                       | 7.09 ms                                                     | 4.96 ms: 1.43x faster                                                       |
| async_tree_memoization_tg  | 367 ms                                                      | 260 ms: 1.41x faster                                                        |
| async_tree_none_tg         | 285 ms                                                      | 210 ms: 1.36x faster                                                        |
| async_tree_none            | 291 ms                                                      | 216 ms: 1.35x faster                                                        |
| async_tree_io              | 731 ms                                                      | 542 ms: 1.35x faster                                                        |
| nbody                      | 71.9 ms                                                     | 56.2 ms: 1.28x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 388 ms: 1.26x faster                                                        |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 399 ms: 1.26x faster                                                        |
| scimark_sor                | 78.8 ms                                                     | 62.9 ms: 1.25x faster                                                       |
| spectral_norm              | 66.9 ms                                                     | 54.0 ms: 1.24x faster                                                       |
| deepcopy                   | 238 us                                                      | 193 us: 1.23x faster                                                        |
| async_tree_memoization     | 339 ms                                                      | 277 ms: 1.22x faster                                                        |
| async_tree_io_tg           | 771 ms                                                      | 631 ms: 1.22x faster                                                        |
| float                      | 56.8 ms                                                     | 46.9 ms: 1.21x faster                                                       |
| comprehensions             | 14.1 us                                                     | 11.8 us: 1.20x faster                                                       |
| crypto_pyaes               | 48.5 ms                                                     | 40.7 ms: 1.19x faster                                                       |
| scimark_fft                | 184 ms                                                      | 156 ms: 1.18x faster                                                        |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.17 ms: 1.18x faster                                                       |
| scimark_monte_carlo        | 43.7 ms                                                     | 37.3 ms: 1.17x faster                                                       |
| logging_silent             | 60.5 ns                                                     | 52.3 ns: 1.16x faster                                                       |
| pprint_safe_repr           | 513 ms                                                      | 450 ms: 1.14x faster                                                        |
| pprint_pformat             | 1.05 sec                                                    | 928 ms: 1.13x faster                                                        |
| deepcopy_reduce            | 2.09 us                                                     | 1.88 us: 1.12x faster                                                       |
| scimark_lu                 | 58.9 ms                                                     | 52.8 ms: 1.11x faster                                                       |
| xml_etree_generate         | 55.8 ms                                                     | 50.5 ms: 1.11x faster                                                       |
| dulwich_log                | 44.3 ms                                                     | 40.7 ms: 1.09x faster                                                       |
| regex_dna                  | 126 ms                                                      | 116 ms: 1.09x faster                                                        |
| tomli_loads                | 1.37 sec                                                    | 1.28 sec: 1.07x faster                                                      |
| fannkuch                   | 247 ms                                                      | 234 ms: 1.05x faster                                                        |
| xml_etree_process          | 37.7 ms                                                     | 35.8 ms: 1.05x faster                                                       |
| chaos                      | 43.3 ms                                                     | 41.2 ms: 1.05x faster                                                       |
| coroutines                 | 14.3 ms                                                     | 13.6 ms: 1.05x faster                                                       |
| regex_effbot               | 1.62 ms                                                     | 1.56 ms: 1.04x faster                                                       |
| json                       | 3.05 ms                                                     | 2.94 ms: 1.04x faster                                                       |
| pyflate                    | 295 ms                                                      | 285 ms: 1.03x faster                                                        |
| bench_thread_pool          | 857 us                                                      | 833 us: 1.03x faster                                                        |
| xml_etree_iterparse        | 65.2 ms                                                     | 63.4 ms: 1.03x faster                                                       |
| pidigits                   | 152 ms                                                      | 148 ms: 1.03x faster                                                        |
| go                         | 91.6 ms                                                     | 90.0 ms: 1.02x faster                                                       |
| logging_simple             | 6.28 us                                                     | 6.19 us: 1.01x faster                                                       |
| nqueens                    | 62.8 ms                                                     | 63.7 ms: 1.01x slower                                                       |
| pickle_pure_python         | 195 us                                                      | 199 us: 1.02x slower                                                        |
| generators                 | 22.5 ms                                                     | 23.2 ms: 1.03x slower                                                       |
| regex_v8                   | 14.2 ms                                                     | 14.7 ms: 1.03x slower                                                       |
| xml_etree_parse            | 93.0 ms                                                     | 96.1 ms: 1.03x slower                                                       |
| json_loads                 | 13.9 us                                                     | 14.4 us: 1.03x slower                                                       |
| regex_compile              | 87.5 ms                                                     | 91.2 ms: 1.04x slower                                                       |
| pycparser                  | 699 ms                                                      | 730 ms: 1.05x slower                                                        |
| deltablue                  | 2.16 ms                                                     | 2.32 ms: 1.07x slower                                                       |
| unpickle_pure_python       | 133 us                                                      | 144 us: 1.08x slower                                                        |
| json_dumps                 | 5.70 ms                                                     | 6.22 ms: 1.09x slower                                                       |
| sympy_str                  | 175 ms                                                      | 192 ms: 1.10x slower                                                        |
| async_generators           | 239 ms                                                      | 263 ms: 1.10x slower                                                        |
| tornado_http               | 89.5 ms                                                     | 99.5 ms: 1.11x slower                                                       |
| sqlglot_normalize          | 187 ms                                                      | 208 ms: 1.11x slower                                                        |
| raytrace                   | 192 ms                                                      | 214 ms: 1.11x slower                                                        |
| sqlglot_parse              | 804 us                                                      | 903 us: 1.12x slower                                                        |
| sympy_sum                  | 91.5 ms                                                     | 103 ms: 1.12x slower                                                        |
| telco                      | 4.13 ms                                                     | 4.67 ms: 1.13x slower                                                       |
| 2to3                       | 218 ms                                                      | 247 ms: 1.13x slower                                                        |
| coverage                   | 40.8 ms                                                     | 46.6 ms: 1.14x slower                                                       |
| sympy_expand               | 284 ms                                                      | 324 ms: 1.14x slower                                                        |
| python_startup_no_site     | 16.2 ms                                                     | 18.6 ms: 1.14x slower                                                       |
| mdp                        | 1.37 sec                                                    | 1.58 sec: 1.15x slower                                                      |
| docutils                   | 1.66 sec                                                    | 1.91 sec: 1.15x slower                                                      |
| sqlglot_transpile          | 1.02 ms                                                     | 1.18 ms: 1.16x slower                                                       |
| richards                   | 28.4 ms                                                     | 33.7 ms: 1.19x slower                                                       |
| typing_runtime_protocols   | 95.1 us                                                     | 114 us: 1.19x slower                                                        |
| sympy_integrate            | 13.0 ms                                                     | 15.5 ms: 1.20x slower                                                       |
| richards_super             | 32.1 ms                                                     | 38.4 ms: 1.20x slower                                                       |
| django_template            | 22.9 ms                                                     | 28.0 ms: 1.22x slower                                                       |
| python_startup             | 19.5 ms                                                     | 24.4 ms: 1.25x slower                                                       |
| sqlglot_optimize           | 34.5 ms                                                     | 43.3 ms: 1.25x slower                                                       |
| hexiom                     | 4.10 ms                                                     | 5.15 ms: 1.26x slower                                                       |
| gc_traversal               | 1.52 ms                                                     | 1.96 ms: 1.29x slower                                                       |
| bench_mp_pool              | 69.2 ms                                                     | 89.1 ms: 1.29x slower                                                       |
| create_gc_cycles           | 752 us                                                      | 1.40 ms: 1.86x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.01x faster                                                                |

Benchmark hidden because not significant (3): meteor_contest, logging_format, pathlib
Ignored benchmarks (15) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20241022-3.14.0a1+-34653bb-JIT/bm-20241022-pythonperf1-amd64-python-34653bba644aa5481613-3.14.0a1+-34653bb.json: genshi_text, genshi_xml, html5lib, pylint, sphinx, thrift

- Geometric mean (including insignificant results): 1.016x faster
# HPT report

- Reliability score: 83.20% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: unknown