# Results vs. 3.13.0b2

- fork: python
- ref: 498376d7a7d6f704f22a
- machine: linux-x86_64
- commit hash: 498376d
- commit date: 2024-08-02
- overall geometric mean: 1.02x faster
- HPT reliability: 96.25%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.07x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                           | 307 ms: 1.05x slower                                                        |
| docutils       | 2.98 sec                                                         | 3.15 sec: 1.06x slower                                                      |
| html5lib       | 74.7 ms                                                          | 70.7 ms: 1.06x faster                                                       |
| Geometric mean | (ref)                                                            | 1.01x slower                                                                |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_io_tg           | 900 ms                                                           | 772 ms: 1.17x faster                                                        |
| async_tree_none            | 365 ms                                                           | 321 ms: 1.14x faster                                                        |
| async_tree_none_tg         | 340 ms                                                           | 303 ms: 1.13x faster                                                        |
| async_tree_memoization     | 460 ms                                                           | 411 ms: 1.12x faster                                                        |
| async_tree_memoization_tg  | 421 ms                                                           | 383 ms: 1.10x faster                                                        |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 563 ms: 1.07x faster                                                        |
| async_tree_io              | 873 ms                                                           | 817 ms: 1.07x faster                                                        |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 539 ms: 1.06x faster                                                        |
| Geometric mean             | (ref)                                                            | 1.11x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 87.8 ms                                                          | 82.0 ms: 1.07x faster                                                       |
| float          | 80.2 ms                                                          | 75.2 ms: 1.07x faster                                                       |
| pidigits       | 254 ms                                                           | 251 ms: 1.01x faster                                                        |
| Geometric mean | (ref)                                                            | 1.05x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 144 ms                                                           | 136 ms: 1.06x faster                                                        |
| regex_dna      | 249 ms                                                           | 240 ms: 1.04x faster                                                        |
| regex_v8       | 26.0 ms                                                          | 25.6 ms: 1.02x faster                                                       |
| regex_effbot   | 3.40 ms                                                          | 3.65 ms: 1.07x slower                                                       |
| Geometric mean | (ref)                                                            | 1.01x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| tomli_loads          | 2.40 sec                                                         | 2.08 sec: 1.15x faster                                                      |
| unpickle_pure_python | 224 us                                                           | 211 us: 1.06x faster                                                        |
| xml_etree_iterparse  | 103 ms                                                           | 97.6 ms: 1.05x faster                                                       |
| xml_etree_process    | 59.7 ms                                                          | 57.0 ms: 1.05x faster                                                       |
| xml_etree_generate   | 85.7 ms                                                          | 82.6 ms: 1.04x faster                                                       |
| xml_etree_parse      | 144 ms                                                           | 140 ms: 1.02x faster                                                        |
| json_loads           | 25.0 us                                                          | 25.5 us: 1.02x slower                                                       |
| pickle_pure_python   | 307 us                                                           | 318 us: 1.03x slower                                                        |
| Geometric mean       | (ref)                                                            | 1.04x faster                                                                |

Benchmark hidden because not significant (1): json_dumps

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 13.2 ms                                                          | 13.4 ms: 1.01x slower                                                       |
| python_startup_no_site | 8.85 ms                                                          | 9.06 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                            | 1.02x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|-----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 10.4 ms                                                          | 9.03 ms: 1.15x faster                                                       |
| genshi_text     | 25.9 ms                                                          | 27.0 ms: 1.04x slower                                                       |
| genshi_xml      | 58.1 ms                                                          | 60.8 ms: 1.05x slower                                                       |
| django_template | 39.0 ms                                                          | 42.3 ms: 1.09x slower                                                       |
| Geometric mean  | (ref)                                                            | 1.01x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240802-pythonperf2-x86_64-python-498376d7a7d6f704f22a-3.14.0a0-498376d |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo              | 37.3 us                                                          | 29.6 us: 1.26x faster                                                       |
| deepcopy                   | 377 us                                                           | 310 us: 1.22x faster                                                        |
| richards                   | 53.4 ms                                                          | 44.5 ms: 1.20x faster                                                       |
| spectral_norm              | 97.3 ms                                                          | 81.5 ms: 1.19x faster                                                       |
| richards_super             | 61.2 ms                                                          | 51.4 ms: 1.19x faster                                                       |
| scimark_sor                | 119 ms                                                           | 101 ms: 1.17x faster                                                        |
| async_tree_io_tg           | 900 ms                                                           | 772 ms: 1.17x faster                                                        |
| tomli_loads                | 2.40 sec                                                         | 2.08 sec: 1.15x faster                                                      |
| mako                       | 10.4 ms                                                          | 9.03 ms: 1.15x faster                                                       |
| async_tree_none            | 365 ms                                                           | 321 ms: 1.14x faster                                                        |
| async_tree_none_tg         | 340 ms                                                           | 303 ms: 1.13x faster                                                        |
| async_tree_memoization     | 460 ms                                                           | 411 ms: 1.12x faster                                                        |
| deepcopy_reduce            | 3.39 us                                                          | 3.05 us: 1.11x faster                                                       |
| pyflate                    | 482 ms                                                           | 438 ms: 1.10x faster                                                        |
| async_tree_memoization_tg  | 421 ms                                                           | 383 ms: 1.10x faster                                                        |
| scimark_fft                | 312 ms                                                           | 287 ms: 1.09x faster                                                        |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 563 ms: 1.07x faster                                                        |
| nbody                      | 87.8 ms                                                          | 82.0 ms: 1.07x faster                                                       |
| async_tree_io              | 873 ms                                                           | 817 ms: 1.07x faster                                                        |
| float                      | 80.2 ms                                                          | 75.2 ms: 1.07x faster                                                       |
| unpickle_pure_python       | 224 us                                                           | 211 us: 1.06x faster                                                        |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 539 ms: 1.06x faster                                                        |
| telco                      | 8.40 ms                                                          | 7.90 ms: 1.06x faster                                                       |
| scimark_sparse_mat_mult    | 4.28 ms                                                          | 4.03 ms: 1.06x faster                                                       |
| regex_compile              | 144 ms                                                           | 136 ms: 1.06x faster                                                        |
| gc_traversal               | 4.69 ms                                                          | 4.43 ms: 1.06x faster                                                       |
| html5lib                   | 74.7 ms                                                          | 70.7 ms: 1.06x faster                                                       |
| bpe_tokeniser              | 5.14 sec                                                         | 4.87 sec: 1.05x faster                                                      |
| crypto_pyaes               | 73.6 ms                                                          | 69.9 ms: 1.05x faster                                                       |
| xml_etree_iterparse        | 103 ms                                                           | 97.6 ms: 1.05x faster                                                       |
| xml_etree_process          | 59.7 ms                                                          | 57.0 ms: 1.05x faster                                                       |
| coroutines                 | 22.0 ms                                                          | 21.0 ms: 1.05x faster                                                       |
| create_gc_cycles           | 2.00 ms                                                          | 1.92 ms: 1.04x faster                                                       |
| logging_format             | 7.11 us                                                          | 6.81 us: 1.04x faster                                                       |
| regex_dna                  | 249 ms                                                           | 240 ms: 1.04x faster                                                        |
| pathlib                    | 17.1 ms                                                          | 16.5 ms: 1.04x faster                                                       |
| xml_etree_generate         | 85.7 ms                                                          | 82.6 ms: 1.04x faster                                                       |
| coverage                   | 83.5 ms                                                          | 81.1 ms: 1.03x faster                                                       |
| logging_simple             | 6.40 us                                                          | 6.23 us: 1.03x faster                                                       |
| xml_etree_parse            | 144 ms                                                           | 140 ms: 1.02x faster                                                        |
| regex_v8                   | 26.0 ms                                                          | 25.6 ms: 1.02x faster                                                       |
| pidigits                   | 254 ms                                                           | 251 ms: 1.01x faster                                                        |
| go                         | 165 ms                                                           | 163 ms: 1.01x faster                                                        |
| pprint_safe_repr           | 818 ms                                                           | 810 ms: 1.01x faster                                                        |
| meteor_contest             | 128 ms                                                           | 127 ms: 1.01x faster                                                        |
| fannkuch                   | 353 ms                                                           | 354 ms: 1.00x slower                                                        |
| scimark_monte_carlo        | 65.5 ms                                                          | 65.8 ms: 1.00x slower                                                       |
| python_startup             | 13.2 ms                                                          | 13.4 ms: 1.01x slower                                                       |
| json                       | 5.35 ms                                                          | 5.45 ms: 1.02x slower                                                       |
| json_loads                 | 25.0 us                                                          | 25.5 us: 1.02x slower                                                       |
| python_startup_no_site     | 8.85 ms                                                          | 9.06 ms: 1.02x slower                                                       |
| thrift                     | 880 us                                                           | 903 us: 1.03x slower                                                        |
| sqlglot_parse              | 1.39 ms                                                          | 1.43 ms: 1.03x slower                                                       |
| pickle_pure_python         | 307 us                                                           | 318 us: 1.03x slower                                                        |
| pycparser                  | 1.22 sec                                                         | 1.27 sec: 1.04x slower                                                      |
| genshi_text                | 25.9 ms                                                          | 27.0 ms: 1.04x slower                                                       |
| logging_silent             | 96.3 ns                                                          | 101 ns: 1.04x slower                                                        |
| async_generators           | 363 ms                                                           | 380 ms: 1.05x slower                                                        |
| genshi_xml                 | 58.1 ms                                                          | 60.8 ms: 1.05x slower                                                       |
| sqlglot_optimize           | 59.5 ms                                                          | 62.4 ms: 1.05x slower                                                       |
| sqlglot_transpile          | 1.76 ms                                                          | 1.85 ms: 1.05x slower                                                       |
| mdp                        | 2.46 sec                                                         | 2.59 sec: 1.05x slower                                                      |
| 2to3                       | 291 ms                                                           | 307 ms: 1.05x slower                                                        |
| comprehensions             | 17.0 us                                                          | 17.9 us: 1.05x slower                                                       |
| docutils                   | 2.98 sec                                                         | 3.15 sec: 1.06x slower                                                      |
| sympy_expand               | 501 ms                                                           | 530 ms: 1.06x slower                                                        |
| deltablue                  | 3.37 ms                                                          | 3.57 ms: 1.06x slower                                                       |
| hexiom                     | 6.35 ms                                                          | 6.74 ms: 1.06x slower                                                       |
| sympy_str                  | 295 ms                                                           | 314 ms: 1.07x slower                                                        |
| typing_runtime_protocols   | 171 us                                                           | 183 us: 1.07x slower                                                        |
| sqlglot_normalize          | 120 ms                                                           | 129 ms: 1.07x slower                                                        |
| regex_effbot               | 3.40 ms                                                          | 3.65 ms: 1.07x slower                                                       |
| django_template            | 39.0 ms                                                          | 42.3 ms: 1.09x slower                                                       |
| sympy_sum                  | 155 ms                                                           | 168 ms: 1.09x slower                                                        |
| generators                 | 33.5 ms                                                          | 36.6 ms: 1.09x slower                                                       |
| nqueens                    | 88.4 ms                                                          | 98.0 ms: 1.11x slower                                                       |
| raytrace                   | 260 ms                                                           | 292 ms: 1.12x slower                                                        |
| sympy_integrate            | 23.2 ms                                                          | 26.1 ms: 1.12x slower                                                       |
| chaos                      | 59.6 ms                                                          | 67.8 ms: 1.14x slower                                                       |
| pylint                     | 339 ms                                                           | 388 ms: 1.14x slower                                                        |
| Geometric mean             | (ref)                                                            | 1.02x faster                                                                |

Benchmark hidden because not significant (10): bench_thread_pool, bench_mp_pool, asyncio_websockets, asyncio_tcp, asyncio_tcp_ssl, json_dumps, tornado_http, dask, scimark_lu, pprint_pformat
Ignored benchmarks (12) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 96.25% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.07x