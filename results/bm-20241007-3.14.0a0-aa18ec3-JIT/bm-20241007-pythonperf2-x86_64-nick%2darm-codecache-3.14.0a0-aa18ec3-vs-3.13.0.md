# Results vs. 3.13.0

- fork: nick-arm
- ref: codecache
- machine: linux-x86_64
- commit hash: aa18ec3
- commit date: 2024-10-07
- overall geometric mean: 1.07x slower
- HPT reliability: 84.33%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.08x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| 2to3           | 291 ms                                                       | 317 ms: 1.09x slower                                                 |
| docutils       | 2.81 sec                                                     | 3.19 sec: 1.13x slower                                               |
| html5lib       | 71.9 ms                                                      | 73.1 ms: 1.02x slower                                                |
| tornado_http   | 120 ms                                                       | 122 ms: 1.02x slower                                                 |
| Geometric mean | (ref)                                                        | 1.06x slower                                                         |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| async_tree_memoization_tg  | 461 ms                                                       | 391 ms: 1.18x faster                                                 |
| async_tree_none            | 372 ms                                                       | 327 ms: 1.14x faster                                                 |
| async_tree_memoization     | 452 ms                                                       | 406 ms: 1.11x faster                                                 |
| async_tree_none_tg         | 340 ms                                                       | 310 ms: 1.10x faster                                                 |
| async_tree_io              | 847 ms                                                       | 804 ms: 1.05x faster                                                 |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 549 ms: 1.05x faster                                                 |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 580 ms: 1.03x faster                                                 |
| async_tree_io_tg           | 819 ms                                                       | 792 ms: 1.03x faster                                                 |
| asyncio_tcp                | 380 ms                                                       | 369 ms: 1.03x faster                                                 |
| coroutines                 | 21.6 ms                                                      | 21.2 ms: 1.02x faster                                                |
| asyncio_tcp_ssl            | 1.58 sec                                                     | 1.58 sec: 1.00x slower                                               |
| asyncio_websockets         | 382 ms                                                       | 389 ms: 1.02x slower                                                 |
| async_generators           | 359 ms                                                       | 383 ms: 1.07x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.05x faster                                                         |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| float          | 81.9 ms                                                      | 75.8 ms: 1.08x faster                                                |
| nbody          | 88.0 ms                                                      | 84.2 ms: 1.05x faster                                                |
| pidigits       | 253 ms                                                       | 250 ms: 1.01x faster                                                 |
| Geometric mean | (ref)                                                        | 1.05x faster                                                         |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| regex_v8       | 26.2 ms                                                      | 24.7 ms: 1.06x faster                                                |
| regex_dna      | 244 ms                                                       | 245 ms: 1.01x slower                                                 |
| regex_effbot   | 3.37 ms                                                      | 3.43 ms: 1.02x slower                                                |
| regex_compile  | 144 ms                                                       | 152 ms: 1.06x slower                                                 |
| Geometric mean | (ref)                                                        | 1.01x slower                                                         |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| tomli_loads          | 2.41 sec                                                     | 2.17 sec: 1.11x faster                                               |
| xml_etree_generate   | 85.3 ms                                                      | 78.0 ms: 1.09x faster                                                |
| pickle_list          | 4.59 us                                                      | 4.20 us: 1.09x faster                                                |
| xml_etree_process    | 60.9 ms                                                      | 56.2 ms: 1.08x faster                                                |
| json_dumps           | 11.0 ms                                                      | 10.5 ms: 1.05x faster                                                |
| pickle_dict          | 32.1 us                                                      | 30.8 us: 1.04x faster                                                |
| xml_etree_parse      | 145 ms                                                       | 140 ms: 1.04x faster                                                 |
| pickle               | 10.5 us                                                      | 10.3 us: 1.02x faster                                                |
| xml_etree_iterparse  | 100 ms                                                       | 98.4 ms: 1.02x faster                                                |
| json_loads           | 24.0 us                                                      | 23.8 us: 1.01x faster                                                |
| unpickle_pure_python | 214 us                                                       | 215 us: 1.00x slower                                                 |
| unpickle             | 15.1 us                                                      | 15.6 us: 1.03x slower                                                |
| unpickle_list        | 4.62 us                                                      | 4.76 us: 1.03x slower                                                |
| pickle_pure_python   | 318 us                                                       | 329 us: 1.03x slower                                                 |
| Geometric mean       | (ref)                                                        | 1.03x faster                                                         |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| python_startup_no_site | 8.94 ms                                                      | 8.96 ms: 1.00x slower                                                |
| python_startup         | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                |
| Geometric mean         | (ref)                                                        | 1.00x slower                                                         |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|-----------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| mako            | 10.4 ms                                                      | 9.20 ms: 1.13x faster                                                |
| genshi_text     | 26.6 ms                                                      | 29.0 ms: 1.09x slower                                                |
| django_template | 38.9 ms                                                      | 43.2 ms: 1.11x slower                                                |
| genshi_xml      | 57.3 ms                                                      | 64.4 ms: 1.12x slower                                                |
| Geometric mean  | (ref)                                                        | 1.05x slower                                                         |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241007-pythonperf2-x86_64-nick%2darm-codecache-3.14.0a0-aa18ec3 |
|----------------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------:|
| deepcopy_memo              | 39.5 us                                                      | 27.7 us: 1.42x faster                                                |
| deepcopy                   | 397 us                                                       | 299 us: 1.33x faster                                                 |
| deepcopy_reduce            | 3.54 us                                                      | 2.96 us: 1.20x faster                                                |
| richards                   | 52.7 ms                                                      | 44.2 ms: 1.19x faster                                                |
| richards_super             | 59.8 ms                                                      | 50.6 ms: 1.18x faster                                                |
| async_tree_memoization_tg  | 461 ms                                                       | 391 ms: 1.18x faster                                                 |
| scimark_sor                | 126 ms                                                       | 107 ms: 1.18x faster                                                 |
| spectral_norm              | 97.4 ms                                                      | 83.3 ms: 1.17x faster                                                |
| async_tree_none            | 372 ms                                                       | 327 ms: 1.14x faster                                                 |
| mako                       | 10.4 ms                                                      | 9.20 ms: 1.13x faster                                                |
| telco                      | 8.58 ms                                                      | 7.71 ms: 1.11x faster                                                |
| async_tree_memoization     | 452 ms                                                       | 406 ms: 1.11x faster                                                 |
| scimark_fft                | 314 ms                                                       | 283 ms: 1.11x faster                                                 |
| tomli_loads                | 2.41 sec                                                     | 2.17 sec: 1.11x faster                                               |
| pathlib                    | 17.4 ms                                                      | 15.8 ms: 1.11x faster                                                |
| async_tree_none_tg         | 340 ms                                                       | 310 ms: 1.10x faster                                                 |
| xml_etree_generate         | 85.3 ms                                                      | 78.0 ms: 1.09x faster                                                |
| pickle_list                | 4.59 us                                                      | 4.20 us: 1.09x faster                                                |
| pyflate                    | 492 ms                                                       | 451 ms: 1.09x faster                                                 |
| bpe_tokeniser              | 5.10 sec                                                     | 4.70 sec: 1.08x faster                                               |
| xml_etree_process          | 60.9 ms                                                      | 56.2 ms: 1.08x faster                                                |
| float                      | 81.9 ms                                                      | 75.8 ms: 1.08x faster                                                |
| pprint_safe_repr           | 812 ms                                                       | 755 ms: 1.07x faster                                                 |
| deltablue                  | 3.41 ms                                                      | 3.21 ms: 1.06x faster                                                |
| regex_v8                   | 26.2 ms                                                      | 24.7 ms: 1.06x faster                                                |
| async_tree_io              | 847 ms                                                       | 804 ms: 1.05x faster                                                 |
| pprint_pformat             | 1.65 sec                                                     | 1.58 sec: 1.05x faster                                               |
| json_dumps                 | 11.0 ms                                                      | 10.5 ms: 1.05x faster                                                |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 549 ms: 1.05x faster                                                 |
| nbody                      | 88.0 ms                                                      | 84.2 ms: 1.05x faster                                                |
| go                         | 160 ms                                                       | 154 ms: 1.04x faster                                                 |
| pickle_dict                | 32.1 us                                                      | 30.8 us: 1.04x faster                                                |
| xml_etree_parse            | 145 ms                                                       | 140 ms: 1.04x faster                                                 |
| scimark_sparse_mat_mult    | 4.29 ms                                                      | 4.14 ms: 1.04x faster                                                |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 580 ms: 1.03x faster                                                 |
| async_tree_io_tg           | 819 ms                                                       | 792 ms: 1.03x faster                                                 |
| asyncio_tcp                | 380 ms                                                       | 369 ms: 1.03x faster                                                 |
| crypto_pyaes               | 72.8 ms                                                      | 70.7 ms: 1.03x faster                                                |
| sqlite_synth               | 2.79 us                                                      | 2.71 us: 1.03x faster                                                |
| fannkuch                   | 365 ms                                                       | 356 ms: 1.02x faster                                                 |
| dulwich_log                | 65.5 ms                                                      | 64.0 ms: 1.02x faster                                                |
| pickle                     | 10.5 us                                                      | 10.3 us: 1.02x faster                                                |
| coroutines                 | 21.6 ms                                                      | 21.2 ms: 1.02x faster                                                |
| xml_etree_iterparse        | 100 ms                                                       | 98.4 ms: 1.02x faster                                                |
| pidigits                   | 253 ms                                                       | 250 ms: 1.01x faster                                                 |
| scimark_lu                 | 97.8 ms                                                      | 96.9 ms: 1.01x faster                                                |
| json_loads                 | 24.0 us                                                      | 23.8 us: 1.01x faster                                                |
| coverage                   | 81.1 ms                                                      | 80.7 ms: 1.01x faster                                                |
| asyncio_tcp_ssl            | 1.58 sec                                                     | 1.58 sec: 1.00x slower                                               |
| python_startup_no_site     | 8.94 ms                                                      | 8.96 ms: 1.00x slower                                                |
| unpickle_pure_python       | 214 us                                                       | 215 us: 1.00x slower                                                 |
| python_startup             | 13.3 ms                                                      | 13.4 ms: 1.00x slower                                                |
| regex_dna                  | 244 ms                                                       | 245 ms: 1.01x slower                                                 |
| logging_silent             | 97.7 ns                                                      | 98.9 ns: 1.01x slower                                                |
| logging_format             | 7.07 us                                                      | 7.18 us: 1.02x slower                                                |
| mdp                        | 2.52 sec                                                     | 2.56 sec: 1.02x slower                                               |
| html5lib                   | 71.9 ms                                                      | 73.1 ms: 1.02x slower                                                |
| asyncio_websockets         | 382 ms                                                       | 389 ms: 1.02x slower                                                 |
| tornado_http               | 120 ms                                                       | 122 ms: 1.02x slower                                                 |
| regex_effbot               | 3.37 ms                                                      | 3.43 ms: 1.02x slower                                                |
| pycparser                  | 1.26 sec                                                     | 1.29 sec: 1.03x slower                                               |
| unpickle                   | 15.1 us                                                      | 15.6 us: 1.03x slower                                                |
| unpickle_list              | 4.62 us                                                      | 4.76 us: 1.03x slower                                                |
| pickle_pure_python         | 318 us                                                       | 329 us: 1.03x slower                                                 |
| logging_simple             | 6.40 us                                                      | 6.61 us: 1.03x slower                                                |
| meteor_contest             | 128 ms                                                       | 133 ms: 1.04x slower                                                 |
| sympy_expand               | 505 ms                                                       | 525 ms: 1.04x slower                                                 |
| gc_traversal               | 4.11 ms                                                      | 4.30 ms: 1.04x slower                                                |
| thrift                     | 877 us                                                       | 916 us: 1.04x slower                                                 |
| bench_thread_pool          | 901 us                                                       | 947 us: 1.05x slower                                                 |
| regex_compile              | 144 ms                                                       | 152 ms: 1.06x slower                                                 |
| scimark_monte_carlo        | 64.9 ms                                                      | 68.7 ms: 1.06x slower                                                |
| create_gc_cycles           | 1.76 ms                                                      | 1.87 ms: 1.06x slower                                                |
| async_generators           | 359 ms                                                       | 383 ms: 1.07x slower                                                 |
| sympy_str                  | 296 ms                                                       | 318 ms: 1.07x slower                                                 |
| chaos                      | 61.7 ms                                                      | 66.8 ms: 1.08x slower                                                |
| typing_runtime_protocols   | 174 us                                                       | 189 us: 1.09x slower                                                 |
| comprehensions             | 17.3 us                                                      | 18.8 us: 1.09x slower                                                |
| 2to3                       | 291 ms                                                       | 317 ms: 1.09x slower                                                 |
| genshi_text                | 26.6 ms                                                      | 29.0 ms: 1.09x slower                                                |
| sqlglot_normalize          | 118 ms                                                       | 130 ms: 1.10x slower                                                 |
| sqlglot_parse              | 1.38 ms                                                      | 1.52 ms: 1.10x slower                                                |
| django_template            | 38.9 ms                                                      | 43.2 ms: 1.11x slower                                                |
| sqlglot_transpile          | 1.78 ms                                                      | 1.98 ms: 1.11x slower                                                |
| raytrace                   | 289 ms                                                       | 322 ms: 1.11x slower                                                 |
| hexiom                     | 6.33 ms                                                      | 7.06 ms: 1.11x slower                                                |
| sympy_sum                  | 154 ms                                                       | 173 ms: 1.12x slower                                                 |
| genshi_xml                 | 57.3 ms                                                      | 64.4 ms: 1.12x slower                                                |
| docutils                   | 2.81 sec                                                     | 3.19 sec: 1.13x slower                                               |
| nqueens                    | 88.2 ms                                                      | 101 ms: 1.14x slower                                                 |
| sqlglot_optimize           | 59.7 ms                                                      | 68.7 ms: 1.15x slower                                                |
| sympy_integrate            | 23.3 ms                                                      | 27.2 ms: 1.16x slower                                                |
| generators                 | 33.5 ms                                                      | 39.5 ms: 1.18x slower                                                |
| pylint                     | 346 ms                                                       | 414 ms: 1.20x slower                                                 |
| unpack_sequence            | 56.8 ns                                                      | 88.7 ns: 1.56x slower                                                |
| bench_mp_pool              | 4.65 ms                                                      | 4.58 sec: 985.03x slower                                             |
| Geometric mean             | (ref)                                                        | 1.07x slower                                                         |

Benchmark hidden because not significant (1): json
Ignored benchmarks (6) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2

# HPT report

- Reliability score: 84.33% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.08x