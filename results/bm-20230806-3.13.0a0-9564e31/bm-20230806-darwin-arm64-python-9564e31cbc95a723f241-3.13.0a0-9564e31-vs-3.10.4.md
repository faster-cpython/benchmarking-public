
# Results vs. 3.10.4

- fork: python
- ref: 9564e31cbc95a723f241
- machine: darwin-arm64
- commit hash: 9564e31
- commit date: 2023-08-06
- overall geometric mean: 1.18x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.10x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 1.78 sec                                               | 1.54 sec: 1.16x faster                                                |
| tornado_http   | 91.9 ms                                                | 72.2 ms: 1.27x faster                                                 |
| Geometric mean | (ref)                                                  | 1.21x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 72.1 ms: 1.31x faster                                                 |
| float          | 72.3 ms                                                | 59.7 ms: 1.21x faster                                                 |
| Geometric mean | (ref)                                                  | 1.17x faster                                                          |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 80.4 ms: 1.20x faster                                                 |
| regex_v8       | 17.5 ms                                                | 15.6 ms: 1.13x faster                                                 |
| regex_dna      | 160 ms                                                 | 149 ms: 1.07x faster                                                  |
| regex_effbot   | 2.45 ms                                                | 2.54 ms: 1.04x slower                                                 |
| Geometric mean | (ref)                                                  | 1.09x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 203 us: 1.40x faster                                                  |
| json_dumps           | 8.38 ms                                                | 6.70 ms: 1.25x faster                                                 |
| unpickle_pure_python | 203 us                                                 | 164 us: 1.24x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 40.8 ms: 1.11x faster                                                 |
| tomli_loads          | 1.76 sec                                               | 1.63 sec: 1.08x faster                                                |
| unpickle             | 9.77 us                                                | 9.27 us: 1.05x faster                                                 |
| pickle_list          | 2.83 us                                                | 2.85 us: 1.01x slower                                                 |
| pickle_dict          | 17.8 us                                                | 18.1 us: 1.02x slower                                                 |
| json_loads           | 16.9 us                                                | 17.4 us: 1.03x slower                                                 |
| xml_etree_parse      | 107 ms                                                 | 112 ms: 1.04x slower                                                  |
| xml_etree_iterparse  | 72.6 ms                                                | 76.6 ms: 1.05x slower                                                 |
| xml_etree_generate   | 54.3 ms                                                | 58.9 ms: 1.09x slower                                                 |
| unpickle_list        | 2.66 us                                                | 3.16 us: 1.19x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.04x faster                                                          |

Benchmark hidden because not significant (1): pickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 11.3 ms: 1.12x faster                                                 |
| python_startup_no_site | 9.73 ms                                                | 9.12 ms: 1.07x faster                                                 |
| Geometric mean         | (ref)                                                  | 1.09x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|-----------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako      | 10.5 ms                                                | 7.61 ms: 1.38x faster                                                 |

All benchmarks:
===============

| Benchmark                | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230806-darwin-arm64-python-9564e31cbc95a723f241-3.13.0a0-9564e31 |
|--------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| typing_runtime_protocols | 344 us                                                 | 93.4 us: 3.69x faster                                                 |
| deltablue                | 5.15 ms                                                | 2.83 ms: 1.82x faster                                                 |
| crypto_pyaes             | 78.0 ms                                                | 47.6 ms: 1.64x faster                                                 |
| logging_silent           | 119 ns                                                 | 76.8 ns: 1.55x faster                                                 |
| asyncio_tcp              | 673 ms                                                 | 439 ms: 1.53x faster                                                  |
| async_tree_memoization   | 493 ms                                                 | 323 ms: 1.53x faster                                                  |
| async_tree_io            | 1.02 sec                                               | 680 ms: 1.50x faster                                                  |
| richards_super           | 60.7 ms                                                | 40.9 ms: 1.48x faster                                                 |
| chaos                    | 66.8 ms                                                | 45.3 ms: 1.48x faster                                                 |
| async_tree_none          | 402 ms                                                 | 274 ms: 1.46x faster                                                  |
| sqlglot_parse            | 1.33 ms                                                | 924 us: 1.44x faster                                                  |
| scimark_lu               | 110 ms                                                 | 76.3 ms: 1.44x faster                                                 |
| raytrace                 | 328 ms                                                 | 230 ms: 1.43x faster                                                  |
| pickle_pure_python       | 284 us                                                 | 203 us: 1.40x faster                                                  |
| unpack_sequence          | 38.7 ns                                                | 27.8 ns: 1.39x faster                                                 |
| sqlglot_transpile        | 1.54 ms                                                | 1.11 ms: 1.39x faster                                                 |
| richards                 | 51.4 ms                                                | 37.2 ms: 1.38x faster                                                 |
| mako                     | 10.5 ms                                                | 7.61 ms: 1.38x faster                                                 |
| go                       | 165 ms                                                 | 120 ms: 1.37x faster                                                  |
| scimark_monte_carlo      | 72.2 ms                                                | 53.6 ms: 1.35x faster                                                 |
| deepcopy_memo            | 34.5 us                                                | 25.6 us: 1.35x faster                                                 |
| nbody                    | 94.1 ms                                                | 72.1 ms: 1.31x faster                                                 |
| spectral_norm            | 96.4 ms                                                | 74.1 ms: 1.30x faster                                                 |
| pycparser                | 915 ms                                                 | 709 ms: 1.29x faster                                                  |
| hexiom                   | 6.32 ms                                                | 4.96 ms: 1.27x faster                                                 |
| tornado_http             | 91.9 ms                                                | 72.2 ms: 1.27x faster                                                 |
| pyflate                  | 453 ms                                                 | 359 ms: 1.26x faster                                                  |
| async_tree_cpu_io_mixed  | 670 ms                                                 | 536 ms: 1.25x faster                                                  |
| json_dumps               | 8.38 ms                                                | 6.70 ms: 1.25x faster                                                 |
| unpickle_pure_python     | 203 us                                                 | 164 us: 1.24x faster                                                  |
| create_gc_cycles         | 876 us                                                 | 709 us: 1.24x faster                                                  |
| logging_format           | 5.01 us                                                | 4.11 us: 1.22x faster                                                 |
| float                    | 72.3 ms                                                | 59.7 ms: 1.21x faster                                                 |
| deepcopy                 | 278 us                                                 | 230 us: 1.21x faster                                                  |
| asyncio_tcp_ssl          | 1.64 sec                                               | 1.36 sec: 1.21x faster                                                |
| logging_simple           | 4.63 us                                                | 3.85 us: 1.20x faster                                                 |
| regex_compile            | 96.6 ms                                                | 80.4 ms: 1.20x faster                                                 |
| dulwich_log              | 37.1 ms                                                | 31.3 ms: 1.18x faster                                                 |
| pprint_safe_repr         | 609 ms                                                 | 515 ms: 1.18x faster                                                  |
| pprint_pformat           | 1.24 sec                                               | 1.05 sec: 1.18x faster                                                |
| mypy2                    | 308 ms                                                 | 262 ms: 1.18x faster                                                  |
| generators               | 32.9 ms                                                | 28.4 ms: 1.16x faster                                                 |
| docutils                 | 1.78 sec                                               | 1.54 sec: 1.16x faster                                                |
| scimark_fft              | 232 ms                                                 | 201 ms: 1.15x faster                                                  |
| nqueens                  | 68.1 ms                                                | 59.5 ms: 1.14x faster                                                 |
| deepcopy_reduce          | 2.38 us                                                | 2.09 us: 1.14x faster                                                 |
| regex_v8                 | 17.5 ms                                                | 15.6 ms: 1.13x faster                                                 |
| comprehensions           | 17.7 us                                                | 15.7 us: 1.12x faster                                                 |
| python_startup           | 12.6 ms                                                | 11.3 ms: 1.12x faster                                                 |
| bench_thread_pool        | 548 us                                                 | 495 us: 1.11x faster                                                  |
| xml_etree_process        | 45.1 ms                                                | 40.8 ms: 1.11x faster                                                 |
| scimark_sparse_mat_mult  | 3.47 ms                                                | 3.14 ms: 1.10x faster                                                 |
| fannkuch                 | 317 ms                                                 | 290 ms: 1.10x faster                                                  |
| tomli_loads              | 1.76 sec                                               | 1.63 sec: 1.08x faster                                                |
| scimark_sor              | 127 ms                                                 | 118 ms: 1.07x faster                                                  |
| regex_dna                | 160 ms                                                 | 149 ms: 1.07x faster                                                  |
| sqlglot_optimize         | 38.0 ms                                                | 35.4 ms: 1.07x faster                                                 |
| python_startup_no_site   | 9.73 ms                                                | 9.12 ms: 1.07x faster                                                 |
| unpickle                 | 9.77 us                                                | 9.27 us: 1.05x faster                                                 |
| meteor_contest           | 78.6 ms                                                | 75.0 ms: 1.05x faster                                                 |
| json                     | 3.10 ms                                                | 2.98 ms: 1.04x faster                                                 |
| sqlglot_normalize        | 197 ms                                                 | 192 ms: 1.02x faster                                                  |
| coroutines               | 20.2 ms                                                | 19.8 ms: 1.02x faster                                                 |
| mdp                      | 1.67 sec                                               | 1.64 sec: 1.02x faster                                                |
| pickle_list              | 2.83 us                                                | 2.85 us: 1.01x slower                                                 |
| pickle_dict              | 17.8 us                                                | 18.1 us: 1.02x slower                                                 |
| json_loads               | 16.9 us                                                | 17.4 us: 1.03x slower                                                 |
| regex_effbot             | 2.45 ms                                                | 2.54 ms: 1.04x slower                                                 |
| xml_etree_parse          | 107 ms                                                 | 112 ms: 1.04x slower                                                  |
| xml_etree_iterparse      | 72.6 ms                                                | 76.6 ms: 1.05x slower                                                 |
| gc_traversal             | 2.40 ms                                                | 2.54 ms: 1.06x slower                                                 |
| sqlite_synth             | 1.47 us                                                | 1.58 us: 1.07x slower                                                 |
| pathlib                  | 28.8 ms                                                | 31.2 ms: 1.08x slower                                                 |
| xml_etree_generate       | 54.3 ms                                                | 58.9 ms: 1.09x slower                                                 |
| bench_mp_pool            | 41.0 ms                                                | 45.4 ms: 1.11x slower                                                 |
| unpickle_list            | 2.66 us                                                | 3.16 us: 1.19x slower                                                 |
| coverage                 | 40.8 ms                                                | 50.8 ms: 1.25x slower                                                 |
| telco                    | 3.68 ms                                                | 4.62 ms: 1.26x slower                                                 |
| dask                     | 258 ms                                                 | 339 ms: 1.32x slower                                                  |
| async_generators         | 233 ms                                                 | 312 ms: 1.34x slower                                                  |
| Geometric mean           | (ref)                                                  | 1.18x faster                                                          |

Benchmark hidden because not significant (2): pidigits, pickle
Ignored benchmarks (17) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: 2to3, aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.14x
- 95% likely to have a speedup of 1.12x
- 99% likely to have a speedup of 1.10x
