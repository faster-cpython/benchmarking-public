
# Results vs. 3.11.0

- fork: faster-cpython
- ref: specialize_calls_to_
- machine: linux-x86_64
- commit hash: 1df353e
- commit date: 2023-06-18
- overall geometric mean: 1.06x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|----------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| docutils       | 2.86 sec                                                     | 2.89 sec: 1.01x slower                                                                |
| Geometric mean | (ref)                                                        | 1.00x slower                                                                          |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|----------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| nbody          | 90.7 ms                                                      | 86.9 ms: 1.04x faster                                                                 |
| pidigits       | 251 ms                                                       | 261 ms: 1.04x slower                                                                  |
| float          | 74.2 ms                                                      | 78.6 ms: 1.06x slower                                                                 |
| Geometric mean | (ref)                                                        | 1.02x slower                                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|----------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| regex_compile  | 158 ms                                                       | 143 ms: 1.10x faster                                                                  |
| regex_v8       | 23.9 ms                                                      | 23.6 ms: 1.01x faster                                                                 |
| regex_effbot   | 3.50 ms                                                      | 3.63 ms: 1.04x slower                                                                 |
| regex_dna      | 227 ms                                                       | 250 ms: 1.10x slower                                                                  |
| Geometric mean | (ref)                                                        | 1.01x slower                                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|----------------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| json_dumps           | 13.4 ms                                                      | 10.4 ms: 1.29x faster                                                                 |
| json_loads           | 28.7 us                                                      | 24.9 us: 1.15x faster                                                                 |
| unpickle_pure_python | 241 us                                                       | 214 us: 1.13x faster                                                                  |
| xml_etree_parse      | 158 ms                                                       | 151 ms: 1.05x faster                                                                  |
| tomli_loads          | 2.26 sec                                                     | 2.21 sec: 1.02x faster                                                                |
| pickle_pure_python   | 319 us                                                       | 322 us: 1.01x slower                                                                  |
| xml_etree_iterparse  | 104 ms                                                       | 105 ms: 1.01x slower                                                                  |
| unpickle_list        | 4.53 us                                                      | 4.68 us: 1.03x slower                                                                 |
| xml_etree_process    | 56.5 ms                                                      | 58.5 ms: 1.04x slower                                                                 |
| pickle_dict          | 30.8 us                                                      | 32.0 us: 1.04x slower                                                                 |
| pickle               | 9.64 us                                                      | 10.0 us: 1.04x slower                                                                 |
| xml_etree_generate   | 80.5 ms                                                      | 85.5 ms: 1.06x slower                                                                 |
| unpickle             | 13.4 us                                                      | 14.5 us: 1.08x slower                                                                 |
| pickle_list          | 3.83 us                                                      | 4.46 us: 1.17x slower                                                                 |
| Geometric mean       | (ref)                                                        | 1.01x faster                                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|------------------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| python_startup         | 10.8 ms                                                      | 11.3 ms: 1.05x slower                                                                 |
| python_startup_no_site | 7.76 ms                                                      | 8.36 ms: 1.08x slower                                                                 |
| Geometric mean         | (ref)                                                        | 1.06x slower                                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|-----------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| mako      | 11.0 ms                                                      | 10.1 ms: 1.09x faster                                                                 |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230618-pythonperf2-x86_64-faster%2dcpython-specialize_calls_to_-3.13.0a0-1df353e |
|--------------------------|:------------------------------------------------------------:|:-------------------------------------------------------------------------------------:|
| typing_runtime_protocols | 523 us                                                       | 150 us: 3.47x faster                                                                  |
| asyncio_tcp              | 753 ms                                                       | 378 ms: 1.99x faster                                                                  |
| asyncio_tcp_ssl          | 3.08 sec                                                     | 1.56 sec: 1.97x faster                                                                |
| generators               | 56.0 ms                                                      | 37.0 ms: 1.51x faster                                                                 |
| chaos                    | 80.9 ms                                                      | 60.3 ms: 1.34x faster                                                                 |
| json_dumps               | 13.4 ms                                                      | 10.4 ms: 1.29x faster                                                                 |
| coroutines               | 27.6 ms                                                      | 22.1 ms: 1.25x faster                                                                 |
| deltablue                | 4.00 ms                                                      | 3.22 ms: 1.24x faster                                                                 |
| hexiom                   | 7.13 ms                                                      | 5.81 ms: 1.23x faster                                                                 |
| mypy2                    | 451 ms                                                       | 370 ms: 1.22x faster                                                                  |
| scimark_lu               | 115 ms                                                       | 93.8 ms: 1.22x faster                                                                 |
| fannkuch                 | 429 ms                                                       | 361 ms: 1.19x faster                                                                  |
| json_loads               | 28.7 us                                                      | 24.9 us: 1.15x faster                                                                 |
| richards_super           | 61.0 ms                                                      | 53.5 ms: 1.14x faster                                                                 |
| raytrace                 | 317 ms                                                       | 278 ms: 1.14x faster                                                                  |
| nqueens                  | 103 ms                                                       | 90.4 ms: 1.14x faster                                                                 |
| async_tree_memoization   | 630 ms                                                       | 557 ms: 1.13x faster                                                                  |
| unpickle_pure_python     | 241 us                                                       | 214 us: 1.13x faster                                                                  |
| async_tree_none          | 519 ms                                                       | 463 ms: 1.12x faster                                                                  |
| comprehensions           | 24.6 us                                                      | 22.0 us: 1.12x faster                                                                 |
| go                       | 164 ms                                                       | 148 ms: 1.11x faster                                                                  |
| json                     | 5.65 ms                                                      | 5.11 ms: 1.11x faster                                                                 |
| regex_compile            | 158 ms                                                       | 143 ms: 1.10x faster                                                                  |
| async_tree_io            | 1.17 sec                                                     | 1.06 sec: 1.10x faster                                                                |
| logging_format           | 8.11 us                                                      | 7.43 us: 1.09x faster                                                                 |
| mako                     | 11.0 ms                                                      | 10.1 ms: 1.09x faster                                                                 |
| mdp                      | 2.75 sec                                                     | 2.52 sec: 1.09x faster                                                                |
| logging_silent           | 101 ns                                                       | 92.7 ns: 1.09x faster                                                                 |
| sqlglot_parse            | 1.53 ms                                                      | 1.42 ms: 1.08x faster                                                                 |
| gc_traversal             | 3.85 ms                                                      | 3.57 ms: 1.08x faster                                                                 |
| sqlglot_normalize        | 126 ms                                                       | 118 ms: 1.07x faster                                                                  |
| bench_thread_pool        | 1.01 ms                                                      | 947 us: 1.07x faster                                                                  |
| deepcopy                 | 399 us                                                       | 376 us: 1.06x faster                                                                  |
| spectral_norm            | 93.3 ms                                                      | 88.0 ms: 1.06x faster                                                                 |
| async_tree_cpu_io_mixed  | 749 ms                                                       | 710 ms: 1.06x faster                                                                  |
| logging_simple           | 7.19 us                                                      | 6.82 us: 1.06x faster                                                                 |
| sqlglot_transpile        | 1.92 ms                                                      | 1.82 ms: 1.05x faster                                                                 |
| xml_etree_parse          | 158 ms                                                       | 151 ms: 1.05x faster                                                                  |
| meteor_contest           | 131 ms                                                       | 125 ms: 1.04x faster                                                                  |
| nbody                    | 90.7 ms                                                      | 86.9 ms: 1.04x faster                                                                 |
| dulwich_log              | 68.4 ms                                                      | 65.6 ms: 1.04x faster                                                                 |
| richards                 | 48.3 ms                                                      | 46.4 ms: 1.04x faster                                                                 |
| sqlglot_optimize         | 59.8 ms                                                      | 58.0 ms: 1.03x faster                                                                 |
| scimark_sor              | 111 ms                                                       | 108 ms: 1.03x faster                                                                  |
| crypto_pyaes             | 83.4 ms                                                      | 81.3 ms: 1.03x faster                                                                 |
| pycparser                | 1.32 sec                                                     | 1.29 sec: 1.02x faster                                                                |
| tomli_loads              | 2.26 sec                                                     | 2.21 sec: 1.02x faster                                                                |
| deepcopy_memo            | 38.8 us                                                      | 38.3 us: 1.01x faster                                                                 |
| deepcopy_reduce          | 3.51 us                                                      | 3.47 us: 1.01x faster                                                                 |
| regex_v8                 | 23.9 ms                                                      | 23.6 ms: 1.01x faster                                                                 |
| coverage                 | 84.8 ms                                                      | 85.3 ms: 1.01x slower                                                                 |
| pickle_pure_python       | 319 us                                                       | 322 us: 1.01x slower                                                                  |
| xml_etree_iterparse      | 104 ms                                                       | 105 ms: 1.01x slower                                                                  |
| docutils                 | 2.86 sec                                                     | 2.89 sec: 1.01x slower                                                                |
| pprint_pformat           | 1.63 sec                                                     | 1.66 sec: 1.02x slower                                                                |
| unpickle_list            | 4.53 us                                                      | 4.68 us: 1.03x slower                                                                 |
| xml_etree_process        | 56.5 ms                                                      | 58.5 ms: 1.04x slower                                                                 |
| scimark_monte_carlo      | 68.2 ms                                                      | 70.8 ms: 1.04x slower                                                                 |
| pidigits                 | 251 ms                                                       | 261 ms: 1.04x slower                                                                  |
| regex_effbot             | 3.50 ms                                                      | 3.63 ms: 1.04x slower                                                                 |
| pprint_safe_repr         | 784 ms                                                       | 815 ms: 1.04x slower                                                                  |
| pickle_dict              | 30.8 us                                                      | 32.0 us: 1.04x slower                                                                 |
| pickle                   | 9.64 us                                                      | 10.0 us: 1.04x slower                                                                 |
| pathlib                  | 19.1 ms                                                      | 19.9 ms: 1.04x slower                                                                 |
| python_startup           | 10.8 ms                                                      | 11.3 ms: 1.05x slower                                                                 |
| telco                    | 6.86 ms                                                      | 7.23 ms: 1.05x slower                                                                 |
| float                    | 74.2 ms                                                      | 78.6 ms: 1.06x slower                                                                 |
| xml_etree_generate       | 80.5 ms                                                      | 85.5 ms: 1.06x slower                                                                 |
| scimark_fft              | 285 ms                                                       | 305 ms: 1.07x slower                                                                  |
| unpickle                 | 13.4 us                                                      | 14.5 us: 1.08x slower                                                                 |
| python_startup_no_site   | 7.76 ms                                                      | 8.36 ms: 1.08x slower                                                                 |
| sqlite_synth             | 2.50 us                                                      | 2.73 us: 1.09x slower                                                                 |
| regex_dna                | 227 ms                                                       | 250 ms: 1.10x slower                                                                  |
| scimark_sparse_mat_mult  | 4.05 ms                                                      | 4.47 ms: 1.10x slower                                                                 |
| unpack_sequence          | 45.6 ns                                                      | 51.2 ns: 1.12x slower                                                                 |
| pickle_list              | 3.83 us                                                      | 4.46 us: 1.17x slower                                                                 |
| async_generators         | 316 ms                                                       | 397 ms: 1.26x slower                                                                  |
| bench_mp_pool            | 4.62 ms                                                      | 16.1 ms: 3.47x slower                                                                 |
| Geometric mean           | (ref)                                                        | 1.06x faster                                                                          |

Benchmark hidden because not significant (3): tornado_http, pyflate, create_gc_cycles
Ignored benchmarks (18) of /home/runner/work/benchmarking/benchmarking/results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, dask, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
