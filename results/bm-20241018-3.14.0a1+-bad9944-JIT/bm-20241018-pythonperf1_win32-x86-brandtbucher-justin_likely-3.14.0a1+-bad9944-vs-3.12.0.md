# Results vs. 3.12.0

- fork: brandtbucher
- ref: justin_likely
- machine: windows-x86
- commit hash: bad9944
- commit date: 2024-10-18
- overall geometric mean: 1.185x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.12x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 280 ms                                                          | 266 ms: 1.05x faster                                                           |
| docutils       | 1.98 sec                                                        | 2.05 sec: 1.03x slower                                                         |
| tornado_http   | 105 ms                                                          | 109 ms: 1.04x slower                                                           |
| Geometric mean | (ref)                                                           | 1.01x slower                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 350 ms                                                          | 251 ms: 1.40x faster                                                           |
| async_tree_none            | 298 ms                                                          | 215 ms: 1.38x faster                                                           |
| async_tree_none_tg         | 278 ms                                                          | 201 ms: 1.38x faster                                                           |
| async_tree_io              | 693 ms                                                          | 510 ms: 1.36x faster                                                           |
| async_tree_memoization     | 364 ms                                                          | 273 ms: 1.33x faster                                                           |
| async_tree_io_tg           | 677 ms                                                          | 547 ms: 1.24x faster                                                           |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 465 ms: 1.21x faster                                                           |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 464 ms: 1.18x faster                                                           |
| Geometric mean             | (ref)                                                           | 1.31x faster                                                                   |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| nbody          | 127 ms                                                          | 59.6 ms: 2.13x faster                                                          |
| float          | 76.7 ms                                                         | 45.7 ms: 1.68x faster                                                          |
| Geometric mean | (ref)                                                           | 1.53x faster                                                                   |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_compile  | 129 ms                                                          | 106 ms: 1.21x faster                                                           |
| regex_effbot   | 2.04 ms                                                         | 1.76 ms: 1.16x faster                                                          |
| regex_dna      | 127 ms                                                          | 118 ms: 1.07x faster                                                           |
| regex_v8       | 15.0 ms                                                         | 15.6 ms: 1.04x slower                                                          |
| Geometric mean | (ref)                                                           | 1.10x faster                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| tomli_loads          | 2.20 sec                                                        | 1.51 sec: 1.46x faster                                                         |
| xml_etree_generate   | 72.1 ms                                                         | 56.1 ms: 1.29x faster                                                          |
| unpickle_pure_python | 210 us                                                          | 164 us: 1.28x faster                                                           |
| xml_etree_process    | 53.2 ms                                                         | 42.3 ms: 1.26x faster                                                          |
| xml_etree_iterparse  | 77.6 ms                                                         | 64.0 ms: 1.21x faster                                                          |
| pickle_pure_python   | 286 us                                                          | 251 us: 1.14x faster                                                           |
| unpickle_list        | 2.95 us                                                         | 2.81 us: 1.05x faster                                                          |
| xml_etree_parse      | 113 ms                                                          | 110 ms: 1.03x faster                                                           |
| json_loads           | 20.4 us                                                         | 20.5 us: 1.01x slower                                                          |
| pickle_list          | 3.37 us                                                         | 3.44 us: 1.02x slower                                                          |
| unpickle             | 9.71 us                                                         | 10.3 us: 1.06x slower                                                          |
| pickle_dict          | 19.9 us                                                         | 21.2 us: 1.07x slower                                                          |
| pickle               | 7.79 us                                                         | 8.39 us: 1.08x slower                                                          |
| json_dumps           | 7.40 ms                                                         | 8.74 ms: 1.18x slower                                                          |
| Geometric mean       | (ref)                                                           | 1.08x faster                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup_no_site | 19.1 ms                                                         | 20.3 ms: 1.06x slower                                                          |
| python_startup         | 22.4 ms                                                         | 26.9 ms: 1.20x slower                                                          |
| Geometric mean         | (ref)                                                           | 1.13x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| mako            | 9.96 ms                                                         | 5.78 ms: 1.72x faster                                                          |
| django_template | 36.9 ms                                                         | 33.1 ms: 1.11x faster                                                          |
| Geometric mean  | (ref)                                                           | 1.39x faster                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944 |
|----------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| deepcopy_memo              | 38.4 us                                                         | 16.7 us: 2.30x faster                                                          |
| nbody                      | 127 ms                                                          | 59.6 ms: 2.13x faster                                                          |
| scimark_sor                | 130 ms                                                          | 69.2 ms: 1.88x faster                                                          |
| logging_silent             | 101 ns                                                          | 55.7 ns: 1.81x faster                                                          |
| spectral_norm              | 104 ms                                                          | 57.8 ms: 1.80x faster                                                          |
| mako                       | 9.96 ms                                                         | 5.78 ms: 1.72x faster                                                          |
| scimark_monte_carlo        | 66.4 ms                                                         | 39.6 ms: 1.68x faster                                                          |
| float                      | 76.7 ms                                                         | 45.7 ms: 1.68x faster                                                          |
| generators                 | 38.5 ms                                                         | 24.3 ms: 1.58x faster                                                          |
| deepcopy                   | 360 us                                                          | 231 us: 1.56x faster                                                           |
| scimark_lu                 | 93.2 ms                                                         | 59.9 ms: 1.56x faster                                                          |
| unpack_sequence            | 62.5 ns                                                         | 41.2 ns: 1.52x faster                                                          |
| scimark_sparse_mat_mult    | 3.86 ms                                                         | 2.55 ms: 1.51x faster                                                          |
| comprehensions             | 19.2 us                                                         | 12.8 us: 1.50x faster                                                          |
| scimark_fft                | 271 ms                                                          | 184 ms: 1.47x faster                                                           |
| tomli_loads                | 2.20 sec                                                        | 1.51 sec: 1.46x faster                                                         |
| go                         | 137 ms                                                          | 96.0 ms: 1.43x faster                                                          |
| async_tree_memoization_tg  | 350 ms                                                          | 251 ms: 1.40x faster                                                           |
| fannkuch                   | 354 ms                                                          | 254 ms: 1.39x faster                                                           |
| deltablue                  | 3.58 ms                                                         | 2.58 ms: 1.39x faster                                                          |
| async_tree_none            | 298 ms                                                          | 215 ms: 1.38x faster                                                           |
| async_tree_none_tg         | 278 ms                                                          | 201 ms: 1.38x faster                                                           |
| crypto_pyaes               | 69.2 ms                                                         | 50.3 ms: 1.38x faster                                                          |
| async_tree_io              | 693 ms                                                          | 510 ms: 1.36x faster                                                           |
| deepcopy_reduce            | 3.23 us                                                         | 2.38 us: 1.35x faster                                                          |
| pyflate                    | 424 ms                                                          | 317 ms: 1.34x faster                                                           |
| async_tree_memoization     | 364 ms                                                          | 273 ms: 1.33x faster                                                           |
| xml_etree_generate         | 72.1 ms                                                         | 56.1 ms: 1.29x faster                                                          |
| logging_simple             | 9.75 us                                                         | 7.62 us: 1.28x faster                                                          |
| unpickle_pure_python       | 210 us                                                          | 164 us: 1.28x faster                                                           |
| logging_format             | 10.4 us                                                         | 8.24 us: 1.26x faster                                                          |
| xml_etree_process          | 53.2 ms                                                         | 42.3 ms: 1.26x faster                                                          |
| hexiom                     | 6.82 ms                                                         | 5.42 ms: 1.26x faster                                                          |
| chaos                      | 69.4 ms                                                         | 55.3 ms: 1.26x faster                                                          |
| pprint_safe_repr           | 721 ms                                                          | 577 ms: 1.25x faster                                                           |
| pprint_pformat             | 1.50 sec                                                        | 1.20 sec: 1.25x faster                                                         |
| async_tree_io_tg           | 677 ms                                                          | 547 ms: 1.24x faster                                                           |
| nqueens                    | 93.7 ms                                                         | 76.0 ms: 1.23x faster                                                          |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 465 ms: 1.21x faster                                                           |
| regex_compile              | 129 ms                                                          | 106 ms: 1.21x faster                                                           |
| xml_etree_iterparse        | 77.6 ms                                                         | 64.0 ms: 1.21x faster                                                          |
| meteor_contest             | 86.9 ms                                                         | 72.1 ms: 1.21x faster                                                          |
| sqlglot_parse              | 1.25 ms                                                         | 1.04 ms: 1.20x faster                                                          |
| coroutines                 | 20.9 ms                                                         | 17.5 ms: 1.19x faster                                                          |
| pycparser                  | 978 ms                                                          | 824 ms: 1.19x faster                                                           |
| dulwich_log                | 58.5 ms                                                         | 49.5 ms: 1.18x faster                                                          |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 464 ms: 1.18x faster                                                           |
| regex_effbot               | 2.04 ms                                                         | 1.76 ms: 1.16x faster                                                          |
| pickle_pure_python         | 286 us                                                          | 251 us: 1.14x faster                                                           |
| sqlglot_transpile          | 1.53 ms                                                         | 1.34 ms: 1.14x faster                                                          |
| raytrace                   | 308 ms                                                          | 272 ms: 1.13x faster                                                           |
| django_template            | 36.9 ms                                                         | 33.1 ms: 1.11x faster                                                          |
| bench_thread_pool          | 1.10 ms                                                         | 998 us: 1.10x faster                                                           |
| mdp                        | 1.91 sec                                                        | 1.75 sec: 1.09x faster                                                         |
| regex_dna                  | 127 ms                                                          | 118 ms: 1.07x faster                                                           |
| richards                   | 41.3 ms                                                         | 38.8 ms: 1.07x faster                                                          |
| sympy_str                  | 240 ms                                                          | 226 ms: 1.06x faster                                                           |
| sqlite_synth               | 2.07 us                                                         | 1.96 us: 1.06x faster                                                          |
| 2to3                       | 280 ms                                                          | 266 ms: 1.05x faster                                                           |
| sympy_sum                  | 123 ms                                                          | 117 ms: 1.05x faster                                                           |
| unpickle_list              | 2.95 us                                                         | 2.81 us: 1.05x faster                                                          |
| richards_super             | 46.5 ms                                                         | 45.2 ms: 1.03x faster                                                          |
| pathlib                    | 91.4 ms                                                         | 89.1 ms: 1.03x faster                                                          |
| xml_etree_parse            | 113 ms                                                          | 110 ms: 1.03x faster                                                           |
| sympy_expand               | 398 ms                                                          | 392 ms: 1.02x faster                                                           |
| asyncio_tcp_ssl            | 17.7 sec                                                        | 17.5 sec: 1.01x faster                                                         |
| sympy_integrate            | 17.5 ms                                                         | 17.4 ms: 1.01x faster                                                          |
| json_loads                 | 20.4 us                                                         | 20.5 us: 1.01x slower                                                          |
| pickle_list                | 3.37 us                                                         | 3.44 us: 1.02x slower                                                          |
| async_generators           | 313 ms                                                          | 322 ms: 1.03x slower                                                           |
| docutils                   | 1.98 sec                                                        | 2.05 sec: 1.03x slower                                                         |
| sqlglot_optimize           | 48.5 ms                                                         | 50.3 ms: 1.04x slower                                                          |
| regex_v8                   | 15.0 ms                                                         | 15.6 ms: 1.04x slower                                                          |
| tornado_http               | 105 ms                                                          | 109 ms: 1.04x slower                                                           |
| unpickle                   | 9.71 us                                                         | 10.3 us: 1.06x slower                                                          |
| python_startup_no_site     | 19.1 ms                                                         | 20.3 ms: 1.06x slower                                                          |
| pickle_dict                | 19.9 us                                                         | 21.2 us: 1.07x slower                                                          |
| pickle                     | 7.79 us                                                         | 8.39 us: 1.08x slower                                                          |
| coverage                   | 48.4 ms                                                         | 52.7 ms: 1.09x slower                                                          |
| telco                      | 5.51 ms                                                         | 6.10 ms: 1.11x slower                                                          |
| typing_runtime_protocols   | 126 us                                                          | 142 us: 1.12x slower                                                           |
| json                       | 4.15 ms                                                         | 4.74 ms: 1.14x slower                                                          |
| json_dumps                 | 7.40 ms                                                         | 8.74 ms: 1.18x slower                                                          |
| python_startup             | 22.4 ms                                                         | 26.9 ms: 1.20x slower                                                          |
| bench_mp_pool              | 75.4 ms                                                         | 94.1 ms: 1.25x slower                                                          |
| gc_traversal               | 1.44 ms                                                         | 1.83 ms: 1.27x slower                                                          |
| asyncio_tcp                | 662 ms                                                          | 866 ms: 1.31x slower                                                           |
| create_gc_cycles           | 652 us                                                          | 1.20 ms: 1.84x slower                                                          |
| sqlglot_normalize          | 100 ms                                                          | 244 ms: 2.43x slower                                                           |
| Geometric mean             | (ref)                                                           | 1.17x faster                                                                   |

Benchmark hidden because not significant (1): pidigits
Ignored benchmarks (6) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (6) of results/bm-20241018-3.14.0a1+-bad9944-JIT/bm-20241018-pythonperf1_win32-x86-brandtbucher-justin_likely-3.14.0a1+-bad9944.json: genshi_text, genshi_xml, html5lib, pylint, sphinx, thrift

- Geometric mean (including insignificant results): 1.185x faster
# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.16x
- 95% likely to have a speedup of 1.15x
- 99% likely to have a speedup of 1.12x

# Memory
- memory change: unknown