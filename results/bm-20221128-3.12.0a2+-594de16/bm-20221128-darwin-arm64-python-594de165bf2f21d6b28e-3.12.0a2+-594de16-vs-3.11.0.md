
# Results vs. 3.11.0

- fork: python
- ref: 594de165bf2f21d6b28e
- machine: darwin-arm64
- commit hash: 594de16
- commit date: 2022-11-28
- overall geometric mean: 1.03x slower \*
- HPT reliability: 99.94%
- HPT 99th percentile: 1.00x slower

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 161 ms                                                 | 166 ms: 1.03x slower                                                   |
| chameleon      | 4.60 ms                                                | 4.49 ms: 1.02x faster                                                  |
| html5lib       | 34.7 ms                                                | 36.2 ms: 1.04x slower                                                  |
| Geometric mean | (ref)                                                  | 1.01x slower                                                           |

Benchmark hidden because not significant (2): docutils, tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 65.6 ms                                                | 64.7 ms: 1.01x faster                                                  |
| pidigits       | 281 ms                                                 | 281 ms: 1.00x slower                                                   |
| float          | 53.0 ms                                                | 56.9 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_v8       | 16.1 ms                                                | 15.7 ms: 1.03x faster                                                  |
| regex_dna      | 152 ms                                                 | 149 ms: 1.02x faster                                                   |
| regex_compile  | 76.7 ms                                                | 75.9 ms: 1.01x faster                                                  |
| regex_effbot   | 2.63 ms                                                | 2.61 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 7.63 ms                                                | 6.08 ms: 1.25x faster                                                  |
| xml_etree_parse      | 108 ms                                                 | 96.2 ms: 1.12x faster                                                  |
| unpickle_list        | 2.65 us                                                | 2.57 us: 1.03x faster                                                  |
| xml_etree_iterparse  | 70.1 ms                                                | 69.3 ms: 1.01x faster                                                  |
| pickle_dict          | 18.0 us                                                | 17.8 us: 1.01x faster                                                  |
| xml_etree_process    | 35.1 ms                                                | 35.4 ms: 1.01x slower                                                  |
| xml_etree_generate   | 48.3 ms                                                | 48.7 ms: 1.01x slower                                                  |
| json_loads           | 16.1 us                                                | 16.3 us: 1.01x slower                                                  |
| unpickle             | 9.67 us                                                | 9.81 us: 1.01x slower                                                  |
| unpickle_pure_python | 159 us                                                 | 162 us: 1.02x slower                                                   |
| pickle_pure_python   | 201 us                                                 | 212 us: 1.06x slower                                                   |
| pickle               | 7.15 us                                                | 7.61 us: 1.07x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (1): pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.4 ms                                                | 12.2 ms: 1.02x faster                                                  |
| python_startup_no_site | 10.2 ms                                                | 10.2 ms: 1.00x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.01x faster                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 8.53 ms                                                | 7.31 ms: 1.17x faster                                                  |
| genshi_text     | 15.3 ms                                                | 14.6 ms: 1.05x faster                                                  |
| genshi_xml      | 29.8 ms                                                | 29.3 ms: 1.02x faster                                                  |
| django_template | 21.0 ms                                                | 21.4 ms: 1.02x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.05x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps              | 7.63 ms                                                | 6.08 ms: 1.25x faster                                                  |
| mako                    | 8.53 ms                                                | 7.31 ms: 1.17x faster                                                  |
| scimark_sparse_mat_mult | 3.19 ms                                                | 2.77 ms: 1.15x faster                                                  |
| xml_etree_parse         | 108 ms                                                 | 96.2 ms: 1.12x faster                                                  |
| logging_silent          | 68.1 ns                                                | 64.9 ns: 1.05x faster                                                  |
| genshi_text             | 15.3 ms                                                | 14.6 ms: 1.05x faster                                                  |
| scimark_lu              | 73.3 ms                                                | 71.1 ms: 1.03x faster                                                  |
| unpickle_list           | 2.65 us                                                | 2.57 us: 1.03x faster                                                  |
| regex_v8                | 16.1 ms                                                | 15.7 ms: 1.03x faster                                                  |
| create_gc_cycles        | 716 us                                                 | 698 us: 1.03x faster                                                   |
| chameleon               | 4.60 ms                                                | 4.49 ms: 1.02x faster                                                  |
| regex_dna               | 152 ms                                                 | 149 ms: 1.02x faster                                                   |
| python_startup          | 12.4 ms                                                | 12.2 ms: 1.02x faster                                                  |
| genshi_xml              | 29.8 ms                                                | 29.3 ms: 1.02x faster                                                  |
| coverage                | 58.4 ms                                                | 57.5 ms: 1.02x faster                                                  |
| nbody                   | 65.6 ms                                                | 64.7 ms: 1.01x faster                                                  |
| xml_etree_iterparse     | 70.1 ms                                                | 69.3 ms: 1.01x faster                                                  |
| regex_compile           | 76.7 ms                                                | 75.9 ms: 1.01x faster                                                  |
| scimark_fft             | 200 ms                                                 | 198 ms: 1.01x faster                                                   |
| deltablue               | 2.81 ms                                                | 2.79 ms: 1.01x faster                                                  |
| pickle_dict             | 18.0 us                                                | 17.8 us: 1.01x faster                                                  |
| regex_effbot            | 2.63 ms                                                | 2.61 ms: 1.01x faster                                                  |
| bench_thread_pool       | 474 us                                                 | 472 us: 1.01x faster                                                   |
| richards                | 32.2 ms                                                | 32.1 ms: 1.00x faster                                                  |
| thrift                  | 442 us                                                 | 440 us: 1.00x faster                                                   |
| mdp                     | 1.55 sec                                               | 1.54 sec: 1.00x faster                                                 |
| pidigits                | 281 ms                                                 | 281 ms: 1.00x slower                                                   |
| python_startup_no_site  | 10.2 ms                                                | 10.2 ms: 1.00x slower                                                  |
| xml_etree_process       | 35.1 ms                                                | 35.4 ms: 1.01x slower                                                  |
| xml_etree_generate      | 48.3 ms                                                | 48.7 ms: 1.01x slower                                                  |
| json                    | 2.78 ms                                                | 2.81 ms: 1.01x slower                                                  |
| logging_simple          | 3.55 us                                                | 3.59 us: 1.01x slower                                                  |
| json_loads              | 16.1 us                                                | 16.3 us: 1.01x slower                                                  |
| unpickle                | 9.67 us                                                | 9.81 us: 1.01x slower                                                  |
| async_tree_none         | 286 ms                                                 | 289 ms: 1.01x slower                                                   |
| spectral_norm           | 72.6 ms                                                | 73.7 ms: 1.02x slower                                                  |
| sympy_sum               | 85.5 ms                                                | 86.9 ms: 1.02x slower                                                  |
| unpickle_pure_python    | 159 us                                                 | 162 us: 1.02x slower                                                   |
| django_template         | 21.0 ms                                                | 21.4 ms: 1.02x slower                                                  |
| sqlglot_normalize       | 171 ms                                                 | 174 ms: 1.02x slower                                                   |
| sympy_integrate         | 11.5 ms                                                | 11.8 ms: 1.02x slower                                                  |
| sympy_str               | 151 ms                                                 | 155 ms: 1.02x slower                                                   |
| sqlglot_optimize        | 31.1 ms                                                | 31.8 ms: 1.02x slower                                                  |
| sympy_expand            | 242 ms                                                 | 248 ms: 1.02x slower                                                   |
| 2to3                    | 161 ms                                                 | 166 ms: 1.03x slower                                                   |
| logging_format          | 3.78 us                                                | 3.89 us: 1.03x slower                                                  |
| chaos                   | 49.4 ms                                                | 50.9 ms: 1.03x slower                                                  |
| crypto_pyaes            | 51.7 ms                                                | 53.5 ms: 1.03x slower                                                  |
| async_tree_cpu_io_mixed | 533 ms                                                 | 552 ms: 1.03x slower                                                   |
| sqlglot_parse           | 959 us                                                 | 994 us: 1.04x slower                                                   |
| sqlglot_transpile       | 1.12 ms                                                | 1.17 ms: 1.04x slower                                                  |
| nqueens                 | 59.8 ms                                                | 62.2 ms: 1.04x slower                                                  |
| html5lib                | 34.7 ms                                                | 36.2 ms: 1.04x slower                                                  |
| hexiom                  | 4.72 ms                                                | 4.93 ms: 1.04x slower                                                  |
| async_generators        | 196 ms                                                 | 205 ms: 1.05x slower                                                   |
| fannkuch                | 261 ms                                                 | 274 ms: 1.05x slower                                                   |
| meteor_contest          | 73.5 ms                                                | 77.2 ms: 1.05x slower                                                  |
| pickle_pure_python      | 201 us                                                 | 212 us: 1.06x slower                                                   |
| raytrace                | 207 ms                                                 | 219 ms: 1.06x slower                                                   |
| async_tree_io           | 704 ms                                                 | 747 ms: 1.06x slower                                                   |
| pprint_safe_repr        | 466 ms                                                 | 497 ms: 1.06x slower                                                   |
| pprint_pformat          | 954 ms                                                 | 1.02 sec: 1.06x slower                                                 |
| pickle                  | 7.15 us                                                | 7.61 us: 1.07x slower                                                  |
| float                   | 53.0 ms                                                | 56.9 ms: 1.07x slower                                                  |
| deepcopy_reduce         | 1.91 us                                                | 2.08 us: 1.09x slower                                                  |
| go                      | 109 ms                                                 | 118 ms: 1.09x slower                                                   |
| sqlite_synth            | 1.27 us                                                | 1.39 us: 1.09x slower                                                  |
| deepcopy                | 223 us                                                 | 243 us: 1.09x slower                                                   |
| comprehensions          | 16.1 us                                                | 17.8 us: 1.10x slower                                                  |
| deepcopy_memo           | 26.3 us                                                | 29.6 us: 1.12x slower                                                  |
| coroutines              | 17.8 ms                                                | 20.0 ms: 1.13x slower                                                  |
| pyflate                 | 310 ms                                                 | 354 ms: 1.14x slower                                                   |
| generators              | 28.8 ms                                                | 33.7 ms: 1.17x slower                                                  |
| scimark_sor             | 82.6 ms                                                | 97.5 ms: 1.18x slower                                                  |
| scimark_monte_carlo     | 46.5 ms                                                | 56.2 ms: 1.21x slower                                                  |
| mypy2                   | 195 ms                                                 | 263 ms: 1.35x slower                                                   |
| unpack_sequence         | 34.1 ns                                                | 62.0 ns: 1.82x slower                                                  |
| Geometric mean          | (ref)                                                  | 1.03x slower                                                           |

Benchmark hidden because not significant (11): async_tree_memoization, pathlib, tornado_http, dulwich_log, pycparser, gc_traversal, pickle_list, telco, asyncio_tcp, docutils, bench_mp_pool
Ignored benchmarks (10) of results/bm-20221024-3.11.0-deaf509/bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20221128-3.12.0a2+-594de16/bm-20221128-darwin-arm64-python-594de165bf2f21d6b28e-3.12.0a2+-594de16.json: dask


# HPT report

- Reliability score: 99.94% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x
