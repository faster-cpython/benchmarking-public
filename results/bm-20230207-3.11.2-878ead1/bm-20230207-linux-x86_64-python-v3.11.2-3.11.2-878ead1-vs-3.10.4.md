
# Results vs. 3.10.4

- fork: python
- ref: v3.11.2
- machine: linux-x86_64
- commit hash: 878ead1
- commit date: 2023-02-07
- overall geometric mean: 1.25x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| 2to3           | 335 ms                                                 | 257 ms: 1.30x faster                                   |
| chameleon      | 9.17 ms                                                | 6.49 ms: 1.41x faster                                  |
| docutils       | 3.18 sec                                               | 2.55 sec: 1.25x faster                                 |
| html5lib       | 85.9 ms                                                | 64.0 ms: 1.34x faster                                  |
| tornado_http   | 128 ms                                                 | 96.1 ms: 1.33x faster                                  |
| Geometric mean | (ref)                                                  | 1.33x faster                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| nbody          | 144 ms                                                 | 93.0 ms: 1.55x faster                                  |
| float          | 109 ms                                                 | 76.6 ms: 1.42x faster                                  |
| pidigits       | 190 ms                                                 | 190 ms: 1.00x faster                                   |
| Geometric mean | (ref)                                                  | 1.30x faster                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| regex_compile  | 177 ms                                                 | 138 ms: 1.29x faster                                   |
| regex_v8       | 25.0 ms                                                | 21.3 ms: 1.17x faster                                  |
| regex_dna      | 214 ms                                                 | 192 ms: 1.11x faster                                   |
| regex_effbot   | 3.19 ms                                                | 3.39 ms: 1.06x slower                                  |
| Geometric mean | (ref)                                                  | 1.12x faster                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|----------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| pickle_pure_python   | 452 us                                                 | 305 us: 1.48x faster                                   |
| xml_etree_process    | 74.5 ms                                                | 53.8 ms: 1.38x faster                                  |
| unpickle_pure_python | 302 us                                                 | 228 us: 1.32x faster                                   |
| xml_etree_generate   | 93.8 ms                                                | 76.5 ms: 1.23x faster                                  |
| pickle_list          | 4.72 us                                                | 4.16 us: 1.14x faster                                  |
| json_loads           | 28.7 us                                                | 26.2 us: 1.10x faster                                  |
| json_dumps           | 13.4 ms                                                | 12.5 ms: 1.08x faster                                  |
| unpickle             | 14.2 us                                                | 13.2 us: 1.07x faster                                  |
| xml_etree_iterparse  | 111 ms                                                 | 104 ms: 1.07x faster                                   |
| xml_etree_parse      | 163 ms                                                 | 159 ms: 1.02x faster                                   |
| pickle               | 10.2 us                                                | 10.00 us: 1.02x faster                                 |
| pickle_dict          | 27.6 us                                                | 31.4 us: 1.14x slower                                  |
| Geometric mean       | (ref)                                                  | 1.13x faster                                           |

Benchmark hidden because not significant (1): unpickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|------------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| python_startup         | 14.1 ms                                                | 8.47 ms: 1.66x faster                                  |
| python_startup_no_site | 5.78 ms                                                | 5.97 ms: 1.03x slower                                  |
| Geometric mean         | (ref)                                                  | 1.27x faster                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|-----------------|:------------------------------------------------------:|:------------------------------------------------------:|
| mako            | 14.7 ms                                                | 9.83 ms: 1.49x faster                                  |
| django_template | 46.3 ms                                                | 32.7 ms: 1.42x faster                                  |
| genshi_text     | 30.6 ms                                                | 21.7 ms: 1.41x faster                                  |
| genshi_xml      | 63.7 ms                                                | 51.5 ms: 1.24x faster                                  |
| Geometric mean  | (ref)                                                  | 1.39x faster                                           |

All benchmarks:
===============

| Benchmark               | bm-20220323-linux-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-linux-x86_64-python-v3.11.2-3.11.2-878ead1 |
|-------------------------|:------------------------------------------------------:|:------------------------------------------------------:|
| deltablue               | 7.28 ms                                                | 3.68 ms: 1.98x faster                                  |
| logging_silent          | 176 ns                                                 | 99.8 ns: 1.77x faster                                  |
| scimark_sor             | 197 ms                                                 | 115 ms: 1.72x faster                                   |
| python_startup          | 14.1 ms                                                | 8.47 ms: 1.66x faster                                  |
| richards                | 75.2 ms                                                | 45.6 ms: 1.65x faster                                  |
| pyflate                 | 676 ms                                                 | 417 ms: 1.62x faster                                   |
| raytrace                | 467 ms                                                 | 291 ms: 1.60x faster                                   |
| scimark_monte_carlo     | 109 ms                                                 | 67.7 ms: 1.60x faster                                  |
| go                      | 226 ms                                                 | 141 ms: 1.60x faster                                   |
| crypto_pyaes            | 117 ms                                                 | 73.8 ms: 1.58x faster                                  |
| chaos                   | 106 ms                                                 | 68.2 ms: 1.55x faster                                  |
| nbody                   | 144 ms                                                 | 93.0 ms: 1.55x faster                                  |
| spectral_norm           | 150 ms                                                 | 98.4 ms: 1.52x faster                                  |
| scimark_lu              | 161 ms                                                 | 107 ms: 1.50x faster                                   |
| mako                    | 14.7 ms                                                | 9.83 ms: 1.49x faster                                  |
| sqlglot_parse           | 2.04 ms                                                | 1.38 ms: 1.48x faster                                  |
| pickle_pure_python      | 452 us                                                 | 305 us: 1.48x faster                                   |
| hexiom                  | 9.43 ms                                                | 6.36 ms: 1.48x faster                                  |
| sqlglot_transpile       | 2.43 ms                                                | 1.67 ms: 1.46x faster                                  |
| float                   | 109 ms                                                 | 76.6 ms: 1.42x faster                                  |
| django_template         | 46.3 ms                                                | 32.7 ms: 1.42x faster                                  |
| chameleon               | 9.17 ms                                                | 6.49 ms: 1.41x faster                                  |
| genshi_text             | 30.6 ms                                                | 21.7 ms: 1.41x faster                                  |
| deepcopy_memo           | 51.7 us                                                | 36.8 us: 1.41x faster                                  |
| xml_etree_process       | 74.5 ms                                                | 53.8 ms: 1.38x faster                                  |
| pycparser               | 1.53 sec                                               | 1.11 sec: 1.38x faster                                 |
| async_tree_io           | 1.78 sec                                               | 1.30 sec: 1.37x faster                                 |
| pprint_pformat          | 1.98 sec                                               | 1.45 sec: 1.37x faster                                 |
| pprint_safe_repr        | 953 ms                                                 | 697 ms: 1.37x faster                                   |
| logging_format          | 8.85 us                                                | 6.49 us: 1.36x faster                                  |
| async_tree_memoization  | 855 ms                                                 | 628 ms: 1.36x faster                                   |
| async_tree_none         | 711 ms                                                 | 524 ms: 1.36x faster                                   |
| logging_simple          | 8.10 us                                                | 5.97 us: 1.36x faster                                  |
| thrift                  | 1.03 ms                                                | 770 us: 1.34x faster                                   |
| html5lib                | 85.9 ms                                                | 64.0 ms: 1.34x faster                                  |
| tornado_http            | 128 ms                                                 | 96.1 ms: 1.33x faster                                  |
| unpickle_pure_python    | 302 us                                                 | 228 us: 1.32x faster                                   |
| 2to3                    | 335 ms                                                 | 257 ms: 1.30x faster                                   |
| scimark_fft             | 421 ms                                                 | 327 ms: 1.29x faster                                   |
| regex_compile           | 177 ms                                                 | 138 ms: 1.29x faster                                   |
| async_tree_cpu_io_mixed | 949 ms                                                 | 740 ms: 1.28x faster                                   |
| aiohttp                 | 1.34 ms                                                | 1.05 ms: 1.28x faster                                  |
| gunicorn                | 1.43 ms                                                | 1.13 ms: 1.27x faster                                  |
| deepcopy                | 438 us                                                 | 348 us: 1.26x faster                                   |
| docutils                | 3.18 sec                                               | 2.55 sec: 1.25x faster                                 |
| deepcopy_reduce         | 3.79 us                                                | 3.04 us: 1.25x faster                                  |
| fannkuch                | 488 ms                                                 | 392 ms: 1.24x faster                                   |
| sqlglot_normalize       | 134 ms                                                 | 109 ms: 1.24x faster                                   |
| genshi_xml              | 63.7 ms                                                | 51.5 ms: 1.24x faster                                  |
| coroutines              | 31.6 ms                                                | 25.6 ms: 1.23x faster                                  |
| xml_etree_generate      | 93.8 ms                                                | 76.5 ms: 1.23x faster                                  |
| sqlglot_optimize        | 65.2 ms                                                | 53.3 ms: 1.22x faster                                  |
| unpack_sequence         | 59.4 ns                                                | 48.8 ns: 1.22x faster                                  |
| nqueens                 | 100 ms                                                 | 83.1 ms: 1.20x faster                                  |
| dask                    | 436 ms                                                 | 365 ms: 1.20x faster                                   |
| async_generators        | 426 ms                                                 | 357 ms: 1.20x faster                                   |
| dulwich_log             | 75.8 ms                                                | 63.7 ms: 1.19x faster                                  |
| sqlite_synth            | 2.92 us                                                | 2.46 us: 1.19x faster                                  |
| sqlalchemy_imperative   | 21.5 ms                                                | 18.2 ms: 1.18x faster                                  |
| scimark_sparse_mat_mult | 5.45 ms                                                | 4.63 ms: 1.18x faster                                  |
| flaskblogging           | 8.27 ms                                                | 7.02 ms: 1.18x faster                                  |
| regex_v8                | 25.0 ms                                                | 21.3 ms: 1.17x faster                                  |
| sqlalchemy_declarative  | 165 ms                                                 | 141 ms: 1.17x faster                                   |
| bench_thread_pool       | 946 us                                                 | 812 us: 1.16x faster                                   |
| sympy_integrate         | 24.0 ms                                                | 20.8 ms: 1.15x faster                                  |
| djangocms               | 36.9 us                                                | 32.3 us: 1.14x faster                                  |
| sympy_expand            | 534 ms                                                 | 468 ms: 1.14x faster                                   |
| pickle_list             | 4.72 us                                                | 4.16 us: 1.14x faster                                  |
| sympy_str               | 325 ms                                                 | 288 ms: 1.13x faster                                   |
| pathlib                 | 20.0 ms                                                | 17.8 ms: 1.12x faster                                  |
| pylint                  | 532 ms                                                 | 475 ms: 1.12x faster                                   |
| regex_dna               | 214 ms                                                 | 192 ms: 1.11x faster                                   |
| create_gc_cycles        | 1.65 ms                                                | 1.49 ms: 1.11x faster                                  |
| sympy_sum               | 183 ms                                                 | 166 ms: 1.11x faster                                   |
| json_loads              | 28.7 us                                                | 26.2 us: 1.10x faster                                  |
| meteor_contest          | 114 ms                                                 | 105 ms: 1.09x faster                                   |
| json                    | 5.35 ms                                                | 4.92 ms: 1.09x faster                                  |
| json_dumps              | 13.4 ms                                                | 12.5 ms: 1.08x faster                                  |
| unpickle                | 14.2 us                                                | 13.2 us: 1.07x faster                                  |
| xml_etree_iterparse     | 111 ms                                                 | 104 ms: 1.07x faster                                   |
| asyncio_tcp             | 914 ms                                                 | 884 ms: 1.03x faster                                   |
| generators              | 76.4 ms                                                | 74.1 ms: 1.03x faster                                  |
| xml_etree_parse         | 163 ms                                                 | 159 ms: 1.02x faster                                   |
| telco                   | 6.73 ms                                                | 6.59 ms: 1.02x faster                                  |
| pickle                  | 10.2 us                                                | 10.00 us: 1.02x faster                                 |
| pidigits                | 190 ms                                                 | 190 ms: 1.00x faster                                   |
| mdp                     | 2.74 sec                                               | 2.77 sec: 1.01x slower                                 |
| python_startup_no_site  | 5.78 ms                                                | 5.97 ms: 1.03x slower                                  |
| regex_effbot            | 3.19 ms                                                | 3.39 ms: 1.06x slower                                  |
| pickle_dict             | 27.6 us                                                | 31.4 us: 1.14x slower                                  |
| gc_traversal            | 3.53 ms                                                | 4.15 ms: 1.18x slower                                  |
| coverage                | 74.7 ms                                                | 103 ms: 1.38x slower                                   |
| Geometric mean          | (ref)                                                  | 1.25x faster                                           |

Benchmark hidden because not significant (3): mypy2, bench_mp_pool, unpickle_list
