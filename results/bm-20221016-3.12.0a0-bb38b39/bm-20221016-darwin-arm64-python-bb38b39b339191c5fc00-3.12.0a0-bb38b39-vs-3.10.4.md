
# Results vs. 3.10.4

- fork: python
- ref: bb38b39b339191c5fc00
- machine: darwin-arm64
- commit hash: bb38b39
- commit date: 2022-10-16
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.18x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 185 ms: 1.09x faster                                                  |
| chameleon      | 5.84 ms                                                | 4.62 ms: 1.26x faster                                                 |
| docutils       | 1.78 sec                                               | 1.47 sec: 1.21x faster                                                |
| html5lib       | 44.1 ms                                                | 36.5 ms: 1.21x faster                                                 |
| tornado_http   | 91.9 ms                                                | 70.7 ms: 1.30x faster                                                 |
| Geometric mean | (ref)                                                  | 1.21x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 64.2 ms: 1.47x faster                                                 |
| float          | 72.3 ms                                                | 56.1 ms: 1.29x faster                                                 |
| pidigits       | 282 ms                                                 | 282 ms: 1.00x slower                                                  |
| Geometric mean | (ref)                                                  | 1.24x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 76.2 ms: 1.27x faster                                                 |
| regex_v8       | 17.5 ms                                                | 16.3 ms: 1.07x faster                                                 |
| regex_dna      | 160 ms                                                 | 152 ms: 1.05x faster                                                  |
| regex_effbot   | 2.45 ms                                                | 2.68 ms: 1.10x slower                                                 |
| Geometric mean | (ref)                                                  | 1.07x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 8.38 ms                                                | 6.15 ms: 1.36x faster                                                 |
| pickle_pure_python   | 284 us                                                 | 209 us: 1.36x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 34.9 ms: 1.29x faster                                                 |
| unpickle_pure_python | 203 us                                                 | 162 us: 1.26x faster                                                  |
| xml_etree_parse      | 107 ms                                                 | 90.1 ms: 1.19x faster                                                 |
| xml_etree_generate   | 54.3 ms                                                | 47.5 ms: 1.14x faster                                                 |
| xml_etree_iterparse  | 72.6 ms                                                | 64.5 ms: 1.13x faster                                                 |
| unpickle_list        | 2.66 us                                                | 2.55 us: 1.05x faster                                                 |
| json_loads           | 16.9 us                                                | 16.2 us: 1.04x faster                                                 |
| pickle_dict          | 17.8 us                                                | 18.0 us: 1.01x slower                                                 |
| pickle_list          | 2.83 us                                                | 2.89 us: 1.02x slower                                                 |
| pickle               | 7.36 us                                                | 7.57 us: 1.03x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.13x faster                                                          |

Benchmark hidden because not significant (1): unpickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 9.16 ms: 1.38x faster                                                 |
| python_startup_no_site | 9.73 ms                                                | 7.20 ms: 1.35x faster                                                 |
| Geometric mean         | (ref)                                                  | 1.36x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 10.5 ms                                                | 8.12 ms: 1.29x faster                                                 |
| django_template | 27.3 ms                                                | 21.8 ms: 1.25x faster                                                 |
| genshi_xml      | 37.6 ms                                                | 30.3 ms: 1.24x faster                                                 |
| genshi_text     | 18.4 ms                                                | 15.4 ms: 1.20x faster                                                 |
| Geometric mean  | (ref)                                                  | 1.25x faster                                                          |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39 |
|-------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.74 ms: 1.88x faster                                                 |
| logging_silent          | 119 ns                                                 | 66.0 ns: 1.80x faster                                                 |
| richards                | 51.4 ms                                                | 33.5 ms: 1.53x faster                                                 |
| async_tree_memoization  | 493 ms                                                 | 325 ms: 1.52x faster                                                  |
| raytrace                | 328 ms                                                 | 217 ms: 1.51x faster                                                  |
| crypto_pyaes            | 78.0 ms                                                | 51.9 ms: 1.50x faster                                                 |
| scimark_lu              | 110 ms                                                 | 73.9 ms: 1.49x faster                                                 |
| nbody                   | 94.1 ms                                                | 64.2 ms: 1.47x faster                                                 |
| async_tree_none         | 402 ms                                                 | 287 ms: 1.40x faster                                                  |
| pathlib                 | 28.8 ms                                                | 20.7 ms: 1.39x faster                                                 |
| async_tree_io           | 1.02 sec                                               | 736 ms: 1.39x faster                                                  |
| python_startup          | 12.6 ms                                                | 9.16 ms: 1.38x faster                                                 |
| go                      | 165 ms                                                 | 120 ms: 1.38x faster                                                  |
| json_dumps              | 8.38 ms                                                | 6.15 ms: 1.36x faster                                                 |
| pickle_pure_python      | 284 us                                                 | 209 us: 1.36x faster                                                  |
| python_startup_no_site  | 9.73 ms                                                | 7.20 ms: 1.35x faster                                                 |
| thrift                  | 586 us                                                 | 438 us: 1.34x faster                                                  |
| chaos                   | 66.8 ms                                                | 50.2 ms: 1.33x faster                                                 |
| spectral_norm           | 96.4 ms                                                | 72.7 ms: 1.33x faster                                                 |
| scimark_monte_carlo     | 72.2 ms                                                | 54.6 ms: 1.32x faster                                                 |
| sqlglot_parse           | 1.33 ms                                                | 1.01 ms: 1.32x faster                                                 |
| sqlglot_transpile       | 1.54 ms                                                | 1.17 ms: 1.31x faster                                                 |
| tornado_http            | 91.9 ms                                                | 70.7 ms: 1.30x faster                                                 |
| pycparser               | 915 ms                                                 | 704 ms: 1.30x faster                                                  |
| pyflate                 | 453 ms                                                 | 351 ms: 1.29x faster                                                  |
| mako                    | 10.5 ms                                                | 8.12 ms: 1.29x faster                                                 |
| float                   | 72.3 ms                                                | 56.1 ms: 1.29x faster                                                 |
| xml_etree_process       | 45.1 ms                                                | 34.9 ms: 1.29x faster                                                 |
| hexiom                  | 6.32 ms                                                | 4.92 ms: 1.28x faster                                                 |
| sqlalchemy_imperative   | 9.03 ms                                                | 7.08 ms: 1.28x faster                                                 |
| regex_compile           | 96.6 ms                                                | 76.2 ms: 1.27x faster                                                 |
| dulwich_log             | 37.1 ms                                                | 29.3 ms: 1.27x faster                                                 |
| chameleon               | 5.84 ms                                                | 4.62 ms: 1.26x faster                                                 |
| logging_simple          | 4.63 us                                                | 3.67 us: 1.26x faster                                                 |
| unpickle_pure_python    | 203 us                                                 | 162 us: 1.26x faster                                                  |
| logging_format          | 5.01 us                                                | 3.99 us: 1.26x faster                                                 |
| django_template         | 27.3 ms                                                | 21.8 ms: 1.25x faster                                                 |
| scimark_sor             | 127 ms                                                 | 101 ms: 1.25x faster                                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.78 ms: 1.25x faster                                                 |
| pprint_safe_repr        | 609 ms                                                 | 488 ms: 1.25x faster                                                  |
| pprint_pformat          | 1.24 sec                                               | 995 ms: 1.25x faster                                                  |
| genshi_xml              | 37.6 ms                                                | 30.3 ms: 1.24x faster                                                 |
| async_tree_cpu_io_mixed | 670 ms                                                 | 545 ms: 1.23x faster                                                  |
| sqlalchemy_declarative  | 74.8 ms                                                | 61.0 ms: 1.23x faster                                                 |
| docutils                | 1.78 sec                                               | 1.47 sec: 1.21x faster                                                |
| bench_thread_pool       | 548 us                                                 | 452 us: 1.21x faster                                                  |
| html5lib                | 44.1 ms                                                | 36.5 ms: 1.21x faster                                                 |
| genshi_text             | 18.4 ms                                                | 15.4 ms: 1.20x faster                                                 |
| sqlglot_optimize        | 38.0 ms                                                | 31.8 ms: 1.20x faster                                                 |
| xml_etree_parse         | 107 ms                                                 | 90.1 ms: 1.19x faster                                                 |
| fannkuch                | 317 ms                                                 | 268 ms: 1.18x faster                                                  |
| deepcopy_memo           | 34.5 us                                                | 29.2 us: 1.18x faster                                                 |
| scimark_fft             | 232 ms                                                 | 199 ms: 1.17x faster                                                  |
| async_generators        | 233 ms                                                 | 200 ms: 1.17x faster                                                  |
| pylint                  | 307 ms                                                 | 265 ms: 1.16x faster                                                  |
| deepcopy                | 278 us                                                 | 240 us: 1.16x faster                                                  |
| unpack_sequence         | 38.7 ns                                                | 33.5 ns: 1.15x faster                                                 |
| deepcopy_reduce         | 2.38 us                                                | 2.07 us: 1.15x faster                                                 |
| sympy_integrate         | 13.4 ms                                                | 11.6 ms: 1.15x faster                                                 |
| xml_etree_generate      | 54.3 ms                                                | 47.5 ms: 1.14x faster                                                 |
| generators              | 32.9 ms                                                | 29.0 ms: 1.14x faster                                                 |
| sqlglot_normalize       | 197 ms                                                 | 173 ms: 1.14x faster                                                  |
| nqueens                 | 68.1 ms                                                | 60.1 ms: 1.13x faster                                                 |
| xml_etree_iterparse     | 72.6 ms                                                | 64.5 ms: 1.13x faster                                                 |
| sympy_expand            | 276 ms                                                 | 245 ms: 1.12x faster                                                  |
| sympy_str               | 169 ms                                                 | 153 ms: 1.11x faster                                                  |
| telco                   | 3.68 ms                                                | 3.33 ms: 1.10x faster                                                 |
| json                    | 3.10 ms                                                | 2.82 ms: 1.10x faster                                                 |
| mdp                     | 1.67 sec                                               | 1.52 sec: 1.10x faster                                                |
| sympy_sum               | 94.3 ms                                                | 86.0 ms: 1.10x faster                                                 |
| 2to3                    | 202 ms                                                 | 185 ms: 1.09x faster                                                  |
| regex_v8                | 17.5 ms                                                | 16.3 ms: 1.07x faster                                                 |
| regex_dna               | 160 ms                                                 | 152 ms: 1.05x faster                                                  |
| unpickle_list           | 2.66 us                                                | 2.55 us: 1.05x faster                                                 |
| coroutines              | 20.2 ms                                                | 19.3 ms: 1.05x faster                                                 |
| json_loads              | 16.9 us                                                | 16.2 us: 1.04x faster                                                 |
| meteor_contest          | 78.6 ms                                                | 76.3 ms: 1.03x faster                                                 |
| sqlite_synth            | 1.47 us                                                | 1.44 us: 1.02x faster                                                 |
| pidigits                | 282 ms                                                 | 282 ms: 1.00x slower                                                  |
| pickle_dict             | 17.8 us                                                | 18.0 us: 1.01x slower                                                 |
| pickle_list             | 2.83 us                                                | 2.89 us: 1.02x slower                                                 |
| pickle                  | 7.36 us                                                | 7.57 us: 1.03x slower                                                 |
| regex_effbot            | 2.45 ms                                                | 2.68 ms: 1.10x slower                                                 |
| coverage                | 40.8 ms                                                | 53.2 ms: 1.30x slower                                                 |
| Geometric mean          | (ref)                                                  | 1.22x faster                                                          |

Benchmark hidden because not significant (2): unpickle, bench_mp_pool
Ignored benchmarks (13) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, comprehensions, create_gc_cycles, dask, flaskblogging, gc_traversal, gunicorn, mypy2, richards_super, tomli_loads, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20221016-3.12.0a0-bb38b39/bm-20221016-darwin-arm64-python-bb38b39b339191c5fc00-3.12.0a0-bb38b39.json: mypy


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.20x
- 95% likely to have a speedup of 1.20x
- 99% likely to have a speedup of 1.18x
