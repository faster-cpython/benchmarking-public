
# Results vs. 3.10.4

- fork: python
- ref: a1f08f5f19753c7c9295
- machine: darwin-arm64
- commit hash: a1f08f5
- commit date: 2023-02-13
- overall geometric mean: 1.24x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.20x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 160 ms: 1.26x faster                                                   |
| chameleon      | 5.84 ms                                                | 4.26 ms: 1.37x faster                                                  |
| docutils       | 1.78 sec                                               | 1.48 sec: 1.21x faster                                                 |
| html5lib       | 44.1 ms                                                | 35.0 ms: 1.26x faster                                                  |
| tornado_http   | 91.9 ms                                                | 69.5 ms: 1.32x faster                                                  |
| Geometric mean | (ref)                                                  | 1.28x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 62.8 ms: 1.50x faster                                                  |
| float          | 72.3 ms                                                | 50.0 ms: 1.45x faster                                                  |
| pidigits       | 282 ms                                                 | 281 ms: 1.00x faster                                                   |
| Geometric mean | (ref)                                                  | 1.30x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 71.2 ms: 1.36x faster                                                  |
| regex_v8       | 17.5 ms                                                | 15.8 ms: 1.11x faster                                                  |
| regex_dna      | 160 ms                                                 | 151 ms: 1.06x faster                                                   |
| regex_effbot   | 2.45 ms                                                | 2.65 ms: 1.08x slower                                                  |
| Geometric mean | (ref)                                                  | 1.10x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 187 us: 1.52x faster                                                   |
| unpickle_pure_python | 203 us                                                 | 138 us: 1.48x faster                                                   |
| json_dumps           | 8.38 ms                                                | 6.09 ms: 1.38x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 35.2 ms: 1.28x faster                                                  |
| xml_etree_parse      | 107 ms                                                 | 96.0 ms: 1.12x faster                                                  |
| xml_etree_generate   | 54.3 ms                                                | 49.3 ms: 1.10x faster                                                  |
| json_loads           | 16.9 us                                                | 16.1 us: 1.05x faster                                                  |
| xml_etree_iterparse  | 72.6 ms                                                | 70.5 ms: 1.03x faster                                                  |
| unpickle_list        | 2.66 us                                                | 2.68 us: 1.01x slower                                                  |
| unpickle             | 9.77 us                                                | 9.91 us: 1.01x slower                                                  |
| pickle_dict          | 17.8 us                                                | 18.1 us: 1.02x slower                                                  |
| pickle               | 7.36 us                                                | 7.54 us: 1.03x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.13x faster                                                           |

Benchmark hidden because not significant (1): pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 12.4 ms: 1.02x faster                                                  |
| python_startup_no_site | 9.73 ms                                                | 10.4 ms: 1.07x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.03x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 10.5 ms                                                | 7.15 ms: 1.46x faster                                                  |
| genshi_xml      | 37.6 ms                                                | 27.3 ms: 1.38x faster                                                  |
| genshi_text     | 18.4 ms                                                | 13.7 ms: 1.34x faster                                                  |
| django_template | 27.3 ms                                                | 20.8 ms: 1.31x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.37x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20230213-darwin-arm64-python-a1f08f5f19753c7c9295-3.12.0a5+-a1f08f5 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.47 ms: 2.08x faster                                                  |
| logging_silent          | 119 ns                                                 | 64.3 ns: 1.85x faster                                                  |
| richards                | 51.4 ms                                                | 29.9 ms: 1.72x faster                                                  |
| scimark_lu              | 110 ms                                                 | 70.5 ms: 1.56x faster                                                  |
| go                      | 165 ms                                                 | 107 ms: 1.54x faster                                                   |
| pickle_pure_python      | 284 us                                                 | 187 us: 1.52x faster                                                   |
| async_tree_memoization  | 493 ms                                                 | 325 ms: 1.52x faster                                                   |
| scimark_sor             | 127 ms                                                 | 83.5 ms: 1.52x faster                                                  |
| raytrace                | 328 ms                                                 | 217 ms: 1.51x faster                                                   |
| hexiom                  | 6.32 ms                                                | 4.21 ms: 1.50x faster                                                  |
| nbody                   | 94.1 ms                                                | 62.8 ms: 1.50x faster                                                  |
| unpickle_pure_python    | 203 us                                                 | 138 us: 1.48x faster                                                   |
| crypto_pyaes            | 78.0 ms                                                | 53.2 ms: 1.47x faster                                                  |
| mako                    | 10.5 ms                                                | 7.15 ms: 1.46x faster                                                  |
| chaos                   | 66.8 ms                                                | 45.7 ms: 1.46x faster                                                  |
| float                   | 72.3 ms                                                | 50.0 ms: 1.45x faster                                                  |
| scimark_monte_carlo     | 72.2 ms                                                | 50.0 ms: 1.44x faster                                                  |
| asyncio_tcp             | 673 ms                                                 | 466 ms: 1.44x faster                                                   |
| async_tree_none         | 402 ms                                                 | 282 ms: 1.43x faster                                                   |
| pyflate                 | 453 ms                                                 | 321 ms: 1.41x faster                                                   |
| deepcopy_memo           | 34.5 us                                                | 24.8 us: 1.39x faster                                                  |
| async_tree_io           | 1.02 sec                                               | 736 ms: 1.39x faster                                                   |
| pycparser               | 915 ms                                                 | 661 ms: 1.39x faster                                                   |
| genshi_xml              | 37.6 ms                                                | 27.3 ms: 1.38x faster                                                  |
| json_dumps              | 8.38 ms                                                | 6.09 ms: 1.38x faster                                                  |
| chameleon               | 5.84 ms                                                | 4.26 ms: 1.37x faster                                                  |
| regex_compile           | 96.6 ms                                                | 71.2 ms: 1.36x faster                                                  |
| pprint_pformat          | 1.24 sec                                               | 922 ms: 1.35x faster                                                   |
| genshi_text             | 18.4 ms                                                | 13.7 ms: 1.34x faster                                                  |
| pprint_safe_repr        | 609 ms                                                 | 454 ms: 1.34x faster                                                   |
| spectral_norm           | 96.4 ms                                                | 72.8 ms: 1.32x faster                                                  |
| tornado_http            | 91.9 ms                                                | 69.5 ms: 1.32x faster                                                  |
| sqlglot_parse           | 1.33 ms                                                | 1.01 ms: 1.32x faster                                                  |
| logging_simple          | 4.63 us                                                | 3.52 us: 1.32x faster                                                  |
| thrift                  | 586 us                                                 | 446 us: 1.31x faster                                                   |
| logging_format          | 5.01 us                                                | 3.82 us: 1.31x faster                                                  |
| django_template         | 27.3 ms                                                | 20.8 ms: 1.31x faster                                                  |
| sqlglot_transpile       | 1.54 ms                                                | 1.18 ms: 1.30x faster                                                  |
| deepcopy                | 278 us                                                 | 215 us: 1.29x faster                                                   |
| xml_etree_process       | 45.1 ms                                                | 35.2 ms: 1.28x faster                                                  |
| deepcopy_reduce         | 2.38 us                                                | 1.86 us: 1.28x faster                                                  |
| dulwich_log             | 37.1 ms                                                | 29.1 ms: 1.27x faster                                                  |
| create_gc_cycles        | 876 us                                                 | 690 us: 1.27x faster                                                   |
| fannkuch                | 317 ms                                                 | 250 ms: 1.27x faster                                                   |
| 2to3                    | 202 ms                                                 | 160 ms: 1.26x faster                                                   |
| unpack_sequence         | 38.7 ns                                                | 30.7 ns: 1.26x faster                                                  |
| sqlalchemy_imperative   | 9.03 ms                                                | 7.17 ms: 1.26x faster                                                  |
| html5lib                | 44.1 ms                                                | 35.0 ms: 1.26x faster                                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.78 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed | 670 ms                                                 | 540 ms: 1.24x faster                                                   |
| scimark_fft             | 232 ms                                                 | 190 ms: 1.22x faster                                                   |
| sqlglot_optimize        | 38.0 ms                                                | 31.1 ms: 1.22x faster                                                  |
| docutils                | 1.78 sec                                               | 1.48 sec: 1.21x faster                                                 |
| sympy_integrate         | 13.4 ms                                                | 11.1 ms: 1.20x faster                                                  |
| bench_thread_pool       | 548 us                                                 | 458 us: 1.20x faster                                                   |
| mypy2                   | 308 ms                                                 | 258 ms: 1.19x faster                                                   |
| comprehensions          | 17.7 us                                                | 14.8 us: 1.19x faster                                                  |
| sqlalchemy_declarative  | 74.8 ms                                                | 63.8 ms: 1.17x faster                                                  |
| sympy_str               | 169 ms                                                 | 145 ms: 1.17x faster                                                   |
| sqlglot_normalize       | 197 ms                                                 | 170 ms: 1.16x faster                                                   |
| sympy_expand            | 276 ms                                                 | 238 ms: 1.16x faster                                                   |
| sympy_sum               | 94.3 ms                                                | 82.5 ms: 1.14x faster                                                  |
| nqueens                 | 68.1 ms                                                | 60.0 ms: 1.13x faster                                                  |
| json                    | 3.10 ms                                                | 2.76 ms: 1.12x faster                                                  |
| xml_etree_parse         | 107 ms                                                 | 96.0 ms: 1.12x faster                                                  |
| regex_v8                | 17.5 ms                                                | 15.8 ms: 1.11x faster                                                  |
| sqlite_synth            | 1.47 us                                                | 1.33 us: 1.11x faster                                                  |
| xml_etree_generate      | 54.3 ms                                                | 49.3 ms: 1.10x faster                                                  |
| mdp                     | 1.67 sec                                               | 1.52 sec: 1.10x faster                                                 |
| coroutines              | 20.2 ms                                                | 18.5 ms: 1.09x faster                                                  |
| meteor_contest          | 78.6 ms                                                | 72.3 ms: 1.09x faster                                                  |
| telco                   | 3.68 ms                                                | 3.43 ms: 1.07x faster                                                  |
| pathlib                 | 28.8 ms                                                | 26.9 ms: 1.07x faster                                                  |
| regex_dna               | 160 ms                                                 | 151 ms: 1.06x faster                                                   |
| json_loads              | 16.9 us                                                | 16.1 us: 1.05x faster                                                  |
| xml_etree_iterparse     | 72.6 ms                                                | 70.5 ms: 1.03x faster                                                  |
| python_startup          | 12.6 ms                                                | 12.4 ms: 1.02x faster                                                  |
| pidigits                | 282 ms                                                 | 281 ms: 1.00x faster                                                   |
| unpickle_list           | 2.66 us                                                | 2.68 us: 1.01x slower                                                  |
| gc_traversal            | 2.40 ms                                                | 2.42 ms: 1.01x slower                                                  |
| generators              | 32.9 ms                                                | 33.3 ms: 1.01x slower                                                  |
| unpickle                | 9.77 us                                                | 9.91 us: 1.01x slower                                                  |
| pickle_dict             | 17.8 us                                                | 18.1 us: 1.02x slower                                                  |
| pickle                  | 7.36 us                                                | 7.54 us: 1.03x slower                                                  |
| python_startup_no_site  | 9.73 ms                                                | 10.4 ms: 1.07x slower                                                  |
| regex_effbot            | 2.45 ms                                                | 2.65 ms: 1.08x slower                                                  |
| bench_mp_pool           | 41.0 ms                                                | 44.7 ms: 1.09x slower                                                  |
| async_generators        | 233 ms                                                 | 256 ms: 1.10x slower                                                   |
| coverage                | 40.8 ms                                                | 60.6 ms: 1.48x slower                                                  |
| Geometric mean          | (ref)                                                  | 1.24x faster                                                           |

Benchmark hidden because not significant (1): pickle_list
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, dask, flaskblogging, gunicorn, pylint, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.23x
- 95% likely to have a speedup of 1.22x
- 99% likely to have a speedup of 1.20x
