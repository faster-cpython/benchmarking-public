
# Results vs. 3.10.4

- fork: python
- ref: 8d32a5c8c4e9c90b0a21
- machine: darwin-arm64
- commit hash: 8d32a5c
- commit date: 2022-05-06
- overall geometric mean: 1.20x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.15x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 162 ms: 1.25x faster                                                  |
| chameleon      | 5.84 ms                                                | 4.64 ms: 1.26x faster                                                 |
| docutils       | 1.78 sec                                               | 1.44 sec: 1.24x faster                                                |
| html5lib       | 44.1 ms                                                | 33.8 ms: 1.30x faster                                                 |
| tornado_http   | 91.9 ms                                                | 71.3 ms: 1.29x faster                                                 |
| Geometric mean | (ref)                                                  | 1.27x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 64.1 ms: 1.47x faster                                                 |
| float          | 72.3 ms                                                | 54.1 ms: 1.34x faster                                                 |
| pidigits       | 282 ms                                                 | 281 ms: 1.00x faster                                                  |
| Geometric mean | (ref)                                                  | 1.25x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 77.9 ms: 1.24x faster                                                 |
| regex_effbot   | 2.45 ms                                                | 2.06 ms: 1.18x faster                                                 |
| regex_v8       | 17.5 ms                                                | 15.1 ms: 1.16x faster                                                 |
| regex_dna      | 160 ms                                                 | 142 ms: 1.13x faster                                                  |
| Geometric mean | (ref)                                                  | 1.18x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 219 us: 1.29x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 35.5 ms: 1.27x faster                                                 |
| unpickle_pure_python | 203 us                                                 | 179 us: 1.14x faster                                                  |
| xml_etree_generate   | 54.3 ms                                                | 48.6 ms: 1.12x faster                                                 |
| json_dumps           | 8.38 ms                                                | 7.69 ms: 1.09x faster                                                 |
| xml_etree_iterparse  | 72.6 ms                                                | 69.0 ms: 1.05x faster                                                 |
| json_loads           | 16.9 us                                                | 16.3 us: 1.04x faster                                                 |
| pickle_dict          | 17.8 us                                                | 17.2 us: 1.04x faster                                                 |
| pickle_list          | 2.83 us                                                | 2.74 us: 1.03x faster                                                 |
| pickle               | 7.36 us                                                | 7.16 us: 1.03x faster                                                 |
| xml_etree_parse      | 107 ms                                                 | 106 ms: 1.02x faster                                                  |
| unpickle_list        | 2.66 us                                                | 2.63 us: 1.01x faster                                                 |
| unpickle             | 9.77 us                                                | 9.90 us: 1.01x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.08x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 12.1 ms: 1.04x faster                                                 |
| python_startup_no_site | 9.73 ms                                                | 9.86 ms: 1.01x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.01x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 27.3 ms                                                | 21.7 ms: 1.26x faster                                                 |
| mako            | 10.5 ms                                                | 8.45 ms: 1.24x faster                                                 |
| genshi_xml      | 37.6 ms                                                | 30.6 ms: 1.23x faster                                                 |
| genshi_text     | 18.4 ms                                                | 15.4 ms: 1.20x faster                                                 |
| Geometric mean  | (ref)                                                  | 1.23x faster                                                          |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20220506-darwin-arm64-python-8d32a5c8c4e9c90b0a21-3.11.0b1-8d32a5c |
|-------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.69 ms: 1.91x faster                                                 |
| logging_silent          | 119 ns                                                 | 70.7 ns: 1.68x faster                                                 |
| scimark_sor             | 127 ms                                                 | 76.6 ms: 1.65x faster                                                 |
| raytrace                | 328 ms                                                 | 205 ms: 1.60x faster                                                  |
| go                      | 165 ms                                                 | 106 ms: 1.56x faster                                                  |
| scimark_lu              | 110 ms                                                 | 72.2 ms: 1.52x faster                                                 |
| richards                | 51.4 ms                                                | 34.3 ms: 1.50x faster                                                 |
| crypto_pyaes            | 78.0 ms                                                | 52.3 ms: 1.49x faster                                                 |
| nbody                   | 94.1 ms                                                | 64.1 ms: 1.47x faster                                                 |
| scimark_monte_carlo     | 72.2 ms                                                | 49.3 ms: 1.47x faster                                                 |
| async_tree_io           | 1.02 sec                                               | 710 ms: 1.44x faster                                                  |
| pyflate                 | 453 ms                                                 | 320 ms: 1.42x faster                                                  |
| async_tree_memoization  | 493 ms                                                 | 357 ms: 1.38x faster                                                  |
| async_tree_none         | 402 ms                                                 | 292 ms: 1.38x faster                                                  |
| spectral_norm           | 96.4 ms                                                | 71.7 ms: 1.34x faster                                                 |
| float                   | 72.3 ms                                                | 54.1 ms: 1.34x faster                                                 |
| chaos                   | 66.8 ms                                                | 50.1 ms: 1.33x faster                                                 |
| thrift                  | 586 us                                                 | 443 us: 1.32x faster                                                  |
| logging_simple          | 4.63 us                                                | 3.51 us: 1.32x faster                                                 |
| logging_format          | 5.01 us                                                | 3.80 us: 1.32x faster                                                 |
| hexiom                  | 6.32 ms                                                | 4.82 ms: 1.31x faster                                                 |
| html5lib                | 44.1 ms                                                | 33.8 ms: 1.30x faster                                                 |
| pickle_pure_python      | 284 us                                                 | 219 us: 1.29x faster                                                  |
| tornado_http            | 91.9 ms                                                | 71.3 ms: 1.29x faster                                                 |
| pycparser               | 915 ms                                                 | 710 ms: 1.29x faster                                                  |
| xml_etree_process       | 45.1 ms                                                | 35.5 ms: 1.27x faster                                                 |
| chameleon               | 5.84 ms                                                | 4.64 ms: 1.26x faster                                                 |
| django_template         | 27.3 ms                                                | 21.7 ms: 1.26x faster                                                 |
| 2to3                    | 202 ms                                                 | 162 ms: 1.25x faster                                                  |
| mako                    | 10.5 ms                                                | 8.45 ms: 1.24x faster                                                 |
| regex_compile           | 96.6 ms                                                | 77.9 ms: 1.24x faster                                                 |
| async_tree_cpu_io_mixed | 670 ms                                                 | 542 ms: 1.24x faster                                                  |
| docutils                | 1.78 sec                                               | 1.44 sec: 1.24x faster                                                |
| genshi_xml              | 37.6 ms                                                | 30.6 ms: 1.23x faster                                                 |
| mypy2                   | 308 ms                                                 | 251 ms: 1.23x faster                                                  |
| sqlalchemy_imperative   | 9.03 ms                                                | 7.38 ms: 1.22x faster                                                 |
| pprint_safe_repr        | 609 ms                                                 | 499 ms: 1.22x faster                                                  |
| aiohttp                 | 1.29 ms                                                | 1.06 ms: 1.22x faster                                                 |
| dulwich_log             | 37.1 ms                                                | 30.5 ms: 1.22x faster                                                 |
| create_gc_cycles        | 876 us                                                 | 723 us: 1.21x faster                                                  |
| pprint_pformat          | 1.24 sec                                               | 1.03 sec: 1.21x faster                                                |
| sqlalchemy_declarative  | 74.8 ms                                                | 62.5 ms: 1.20x faster                                                 |
| fannkuch                | 317 ms                                                 | 265 ms: 1.20x faster                                                  |
| genshi_text             | 18.4 ms                                                | 15.4 ms: 1.20x faster                                                 |
| unpack_sequence         | 38.7 ns                                                | 32.4 ns: 1.19x faster                                                 |
| regex_effbot            | 2.45 ms                                                | 2.06 ms: 1.18x faster                                                 |
| deepcopy_memo           | 34.5 us                                                | 29.1 us: 1.18x faster                                                 |
| generators              | 32.9 ms                                                | 28.1 ms: 1.17x faster                                                 |
| sqlglot_optimize        | 38.0 ms                                                | 32.4 ms: 1.17x faster                                                 |
| async_generators        | 233 ms                                                 | 200 ms: 1.16x faster                                                  |
| gunicorn                | 1.34 ms                                                | 1.15 ms: 1.16x faster                                                 |
| regex_v8                | 17.5 ms                                                | 15.1 ms: 1.16x faster                                                 |
| scimark_fft             | 232 ms                                                 | 201 ms: 1.15x faster                                                  |
| unpickle_pure_python    | 203 us                                                 | 179 us: 1.14x faster                                                  |
| deepcopy                | 278 us                                                 | 244 us: 1.14x faster                                                  |
| sympy_integrate         | 13.4 ms                                                | 11.8 ms: 1.14x faster                                                 |
| deepcopy_reduce         | 2.38 us                                                | 2.11 us: 1.13x faster                                                 |
| regex_dna               | 160 ms                                                 | 142 ms: 1.13x faster                                                  |
| flaskblogging           | 2.75 ms                                                | 2.44 ms: 1.12x faster                                                 |
| pylint                  | 307 ms                                                 | 275 ms: 1.12x faster                                                  |
| coroutines              | 20.2 ms                                                | 18.0 ms: 1.12x faster                                                 |
| sqlglot_normalize       | 197 ms                                                 | 176 ms: 1.12x faster                                                  |
| xml_etree_generate      | 54.3 ms                                                | 48.6 ms: 1.12x faster                                                 |
| sqlglot_transpile       | 1.54 ms                                                | 1.38 ms: 1.12x faster                                                 |
| sympy_sum               | 94.3 ms                                                | 84.6 ms: 1.11x faster                                                 |
| dask                    | 258 ms                                                 | 232 ms: 1.11x faster                                                  |
| nqueens                 | 68.1 ms                                                | 61.3 ms: 1.11x faster                                                 |
| sympy_expand            | 276 ms                                                 | 250 ms: 1.10x faster                                                  |
| sqlite_synth            | 1.47 us                                                | 1.34 us: 1.10x faster                                                 |
| bench_thread_pool       | 548 us                                                 | 499 us: 1.10x faster                                                  |
| sqlglot_parse           | 1.33 ms                                                | 1.21 ms: 1.10x faster                                                 |
| sympy_str               | 169 ms                                                 | 154 ms: 1.10x faster                                                  |
| json_dumps              | 8.38 ms                                                | 7.69 ms: 1.09x faster                                                 |
| json                    | 3.10 ms                                                | 2.85 ms: 1.09x faster                                                 |
| telco                   | 3.68 ms                                                | 3.39 ms: 1.09x faster                                                 |
| mdp                     | 1.67 sec                                               | 1.56 sec: 1.07x faster                                                |
| scimark_sparse_mat_mult | 3.47 ms                                                | 3.24 ms: 1.07x faster                                                 |
| pathlib                 | 28.8 ms                                                | 27.1 ms: 1.06x faster                                                 |
| xml_etree_iterparse     | 72.6 ms                                                | 69.0 ms: 1.05x faster                                                 |
| python_startup          | 12.6 ms                                                | 12.1 ms: 1.04x faster                                                 |
| json_loads              | 16.9 us                                                | 16.3 us: 1.04x faster                                                 |
| pickle_dict             | 17.8 us                                                | 17.2 us: 1.04x faster                                                 |
| asyncio_tcp             | 673 ms                                                 | 651 ms: 1.03x faster                                                  |
| pickle_list             | 2.83 us                                                | 2.74 us: 1.03x faster                                                 |
| pickle                  | 7.36 us                                                | 7.16 us: 1.03x faster                                                 |
| meteor_contest          | 78.6 ms                                                | 76.6 ms: 1.03x faster                                                 |
| xml_etree_parse         | 107 ms                                                 | 106 ms: 1.02x faster                                                  |
| unpickle_list           | 2.66 us                                                | 2.63 us: 1.01x faster                                                 |
| pidigits                | 282 ms                                                 | 281 ms: 1.00x faster                                                  |
| gc_traversal            | 2.40 ms                                                | 2.41 ms: 1.01x slower                                                 |
| unpickle                | 9.77 us                                                | 9.90 us: 1.01x slower                                                 |
| python_startup_no_site  | 9.73 ms                                                | 9.86 ms: 1.01x slower                                                 |
| bench_mp_pool           | 41.0 ms                                                | 43.5 ms: 1.06x slower                                                 |
| comprehensions          | 17.7 us                                                | 19.2 us: 1.09x slower                                                 |
| Geometric mean          | (ref)                                                  | 1.20x faster                                                          |
Ignored benchmarks (5) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: asyncio_tcp_ssl, coverage, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.17x
- 95% likely to have a speedup of 1.16x
- 99% likely to have a speedup of 1.15x
