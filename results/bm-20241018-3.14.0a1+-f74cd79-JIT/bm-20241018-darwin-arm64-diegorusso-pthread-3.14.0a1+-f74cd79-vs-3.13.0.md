# Results vs. 3.13.0

- fork: diegorusso
- ref: pthread
- machine: darwin-arm64
- commit hash: f74cd79
- commit date: 2024-10-18
- overall geometric mean: 1.042x faster
- HPT reliability: 99.56%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.09x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| 2to3           | 187 ms                                                 | 182 ms: 1.03x faster                                          |
| docutils       | 1.44 sec                                               | 1.57 sec: 1.09x slower                                        |
| html5lib       | 36.6 ms                                                | 34.0 ms: 1.08x faster                                         |
| sphinx         | 603 ms                                                 | 665 ms: 1.10x slower                                          |
| Geometric mean | (ref)                                                  | 1.02x slower                                                  |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| async_tree_memoization_tg        | 288 ms                                                 | 234 ms: 1.23x faster                                          |
| coroutines                       | 19.8 ms                                                | 16.5 ms: 1.20x faster                                         |
| async_tree_eager_tg              | 47.8 ms                                                | 42.5 ms: 1.12x faster                                         |
| async_tree_eager_memoization     | 168 ms                                                 | 153 ms: 1.10x faster                                          |
| async_tree_eager                 | 70.1 ms                                                | 64.0 ms: 1.09x faster                                         |
| async_tree_none                  | 215 ms                                                 | 198 ms: 1.09x faster                                          |
| async_tree_eager_memoization_tg  | 138 ms                                                 | 130 ms: 1.06x faster                                          |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 336 ms: 1.03x faster                                          |
| async_tree_eager_cpu_io_mixed    | 373 ms                                                 | 364 ms: 1.02x faster                                          |
| async_generators                 | 295 ms                                                 | 293 ms: 1.01x faster                                          |
| async_tree_cpu_io_mixed_tg       | 448 ms                                                 | 469 ms: 1.05x slower                                          |
| async_tree_none_tg               | 198 ms                                                 | 213 ms: 1.07x slower                                          |
| async_tree_io                    | 507 ms                                                 | 580 ms: 1.14x slower                                          |
| async_tree_io_tg                 | 497 ms                                                 | 610 ms: 1.23x slower                                          |
| async_tree_eager_io              | 514 ms                                                 | 671 ms: 1.30x slower                                          |
| async_tree_eager_io_tg           | 477 ms                                                 | 716 ms: 1.50x slower                                          |
| Geometric mean                   | (ref)                                                  | 1.01x slower                                                  |

Benchmark hidden because not significant (3): async_tree_memoization, asyncio_websockets, async_tree_cpu_io_mixed

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| float          | 56.0 ms                                                | 48.4 ms: 1.16x faster                                         |
| nbody          | 74.0 ms                                                | 65.3 ms: 1.13x faster                                         |
| pidigits       | 284 ms                                                 | 283 ms: 1.00x faster                                          |
| Geometric mean | (ref)                                                  | 1.10x faster                                                  |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| regex_compile  | 78.6 ms                                                | 74.4 ms: 1.06x faster                                         |
| regex_v8       | 17.0 ms                                                | 16.8 ms: 1.01x faster                                         |
| regex_dna      | 149 ms                                                 | 149 ms: 1.00x faster                                          |
| Geometric mean | (ref)                                                  | 1.02x faster                                                  |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| tomli_loads          | 1.57 sec                                               | 1.24 sec: 1.26x faster                                        |
| unpickle_pure_python | 164 us                                                 | 132 us: 1.24x faster                                          |
| pickle_pure_python   | 214 us                                                 | 180 us: 1.19x faster                                          |
| xml_etree_process    | 41.0 ms                                                | 34.8 ms: 1.18x faster                                         |
| xml_etree_generate   | 57.0 ms                                                | 50.5 ms: 1.13x faster                                         |
| json_loads           | 17.0 us                                                | 16.4 us: 1.03x faster                                         |
| xml_etree_iterparse  | 73.6 ms                                                | 72.7 ms: 1.01x faster                                         |
| json_dumps           | 6.52 ms                                                | 7.08 ms: 1.09x slower                                         |
| Geometric mean       | (ref)                                                  | 1.10x faster                                                  |

Benchmark hidden because not significant (1): xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|------------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| python_startup_no_site | 14.5 ms                                                | 11.9 ms: 1.21x faster                                         |
| python_startup         | 18.9 ms                                                | 15.7 ms: 1.20x faster                                         |
| Geometric mean         | (ref)                                                  | 1.21x faster                                                  |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|-----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| mako            | 7.68 ms                                                | 6.46 ms: 1.19x faster                                         |
| genshi_text     | 16.9 ms                                                | 16.3 ms: 1.04x faster                                         |
| django_template | 22.2 ms                                                | 22.6 ms: 1.02x slower                                         |
| genshi_xml      | 34.4 ms                                                | 41.0 ms: 1.19x slower                                         |
| Geometric mean  | (ref)                                                  | 1.00x faster                                                  |

All benchmarks:
===============

| Benchmark                        | bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-darwin-arm64-diegorusso-pthread-3.14.0a1+-f74cd79 |
|----------------------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| deepcopy_memo                    | 27.4 us                                                | 16.9 us: 1.62x faster                                         |
| deepcopy                         | 237 us                                                 | 152 us: 1.56x faster                                          |
| deepcopy_reduce                  | 2.07 us                                                | 1.54 us: 1.34x faster                                         |
| tomli_loads                      | 1.57 sec                                               | 1.24 sec: 1.26x faster                                        |
| generators                       | 31.5 ms                                                | 25.3 ms: 1.24x faster                                         |
| unpickle_pure_python             | 164 us                                                 | 132 us: 1.24x faster                                          |
| async_tree_memoization_tg        | 288 ms                                                 | 234 ms: 1.23x faster                                          |
| scimark_sor                      | 105 ms                                                 | 86.1 ms: 1.22x faster                                         |
| python_startup_no_site           | 14.5 ms                                                | 11.9 ms: 1.21x faster                                         |
| python_startup                   | 18.9 ms                                                | 15.7 ms: 1.20x faster                                         |
| coroutines                       | 19.8 ms                                                | 16.5 ms: 1.20x faster                                         |
| pickle_pure_python               | 214 us                                                 | 180 us: 1.19x faster                                          |
| mako                             | 7.68 ms                                                | 6.46 ms: 1.19x faster                                         |
| xml_etree_process                | 41.0 ms                                                | 34.8 ms: 1.18x faster                                         |
| scimark_lu                       | 76.7 ms                                                | 65.0 ms: 1.18x faster                                         |
| go                               | 115 ms                                                 | 98.4 ms: 1.17x faster                                         |
| logging_simple                   | 3.60 us                                                | 3.10 us: 1.16x faster                                         |
| float                            | 56.0 ms                                                | 48.4 ms: 1.16x faster                                         |
| logging_format                   | 3.89 us                                                | 3.38 us: 1.15x faster                                         |
| nbody                            | 74.0 ms                                                | 65.3 ms: 1.13x faster                                         |
| xml_etree_generate               | 57.0 ms                                                | 50.5 ms: 1.13x faster                                         |
| async_tree_eager_tg              | 47.8 ms                                                | 42.5 ms: 1.12x faster                                         |
| deltablue                        | 2.68 ms                                                | 2.39 ms: 1.12x faster                                         |
| thrift                           | 466 us                                                 | 420 us: 1.11x faster                                          |
| async_tree_eager_memoization     | 168 ms                                                 | 153 ms: 1.10x faster                                          |
| spectral_norm                    | 76.3 ms                                                | 69.3 ms: 1.10x faster                                         |
| scimark_monte_carlo              | 50.4 ms                                                | 45.8 ms: 1.10x faster                                         |
| async_tree_eager                 | 70.1 ms                                                | 64.0 ms: 1.09x faster                                         |
| async_tree_none                  | 215 ms                                                 | 198 ms: 1.09x faster                                          |
| pyflate                          | 351 ms                                                 | 325 ms: 1.08x faster                                          |
| html5lib                         | 36.6 ms                                                | 34.0 ms: 1.08x faster                                         |
| pprint_pformat                   | 1.08 sec                                               | 1.01 sec: 1.07x faster                                        |
| json                             | 3.03 ms                                                | 2.84 ms: 1.07x faster                                         |
| nqueens                          | 62.5 ms                                                | 58.7 ms: 1.06x faster                                         |
| bpe_tokeniser                    | 3.25 sec                                               | 3.05 sec: 1.06x faster                                        |
| async_tree_eager_memoization_tg  | 138 ms                                                 | 130 ms: 1.06x faster                                          |
| fannkuch                         | 284 ms                                                 | 268 ms: 1.06x faster                                          |
| bench_thread_pool                | 505 us                                                 | 476 us: 1.06x faster                                          |
| pprint_safe_repr                 | 533 ms                                                 | 504 ms: 1.06x faster                                          |
| coverage                         | 46.0 ms                                                | 43.5 ms: 1.06x faster                                         |
| regex_compile                    | 78.6 ms                                                | 74.4 ms: 1.06x faster                                         |
| pathlib                          | 23.4 ms                                                | 22.2 ms: 1.05x faster                                         |
| scimark_fft                      | 201 ms                                                 | 191 ms: 1.05x faster                                          |
| raytrace                         | 181 ms                                                 | 173 ms: 1.04x faster                                          |
| telco                            | 4.76 ms                                                | 4.58 ms: 1.04x faster                                         |
| typing_runtime_protocols         | 101 us                                                 | 97.4 us: 1.04x faster                                         |
| pycparser                        | 705 ms                                                 | 680 ms: 1.04x faster                                          |
| genshi_text                      | 16.9 ms                                                | 16.3 ms: 1.04x faster                                         |
| bench_mp_pool                    | 62.5 ms                                                | 60.5 ms: 1.03x faster                                         |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 336 ms: 1.03x faster                                          |
| json_loads                       | 17.0 us                                                | 16.4 us: 1.03x faster                                         |
| 2to3                             | 187 ms                                                 | 182 ms: 1.03x faster                                          |
| async_tree_eager_cpu_io_mixed    | 373 ms                                                 | 364 ms: 1.02x faster                                          |
| sqlglot_normalize                | 189 ms                                                 | 186 ms: 1.02x faster                                          |
| xml_etree_iterparse              | 73.6 ms                                                | 72.7 ms: 1.01x faster                                         |
| regex_v8                         | 17.0 ms                                                | 16.8 ms: 1.01x faster                                         |
| crypto_pyaes                     | 54.2 ms                                                | 53.9 ms: 1.01x faster                                         |
| async_generators                 | 295 ms                                                 | 293 ms: 1.01x faster                                          |
| pidigits                         | 284 ms                                                 | 283 ms: 1.00x faster                                          |
| regex_dna                        | 149 ms                                                 | 149 ms: 1.00x faster                                          |
| sympy_expand                     | 246 ms                                                 | 247 ms: 1.00x slower                                          |
| meteor_contest                   | 74.0 ms                                                | 74.5 ms: 1.01x slower                                         |
| sqlglot_parse                    | 856 us                                                 | 865 us: 1.01x slower                                          |
| richards_super                   | 39.1 ms                                                | 39.5 ms: 1.01x slower                                         |
| gc_traversal                     | 2.91 ms                                                | 2.94 ms: 1.01x slower                                         |
| richards                         | 35.2 ms                                                | 35.7 ms: 1.01x slower                                         |
| chaos                            | 41.2 ms                                                | 41.7 ms: 1.01x slower                                         |
| dulwich_log                      | 28.5 ms                                                | 29.0 ms: 1.02x slower                                         |
| django_template                  | 22.2 ms                                                | 22.6 ms: 1.02x slower                                         |
| hexiom                           | 4.86 ms                                                | 4.96 ms: 1.02x slower                                         |
| sqlglot_transpile                | 1.02 ms                                                | 1.05 ms: 1.03x slower                                         |
| sympy_str                        | 145 ms                                                 | 151 ms: 1.04x slower                                          |
| mdp                              | 1.49 sec                                               | 1.55 sec: 1.04x slower                                        |
| sympy_sum                        | 75.4 ms                                                | 78.8 ms: 1.04x slower                                         |
| async_tree_cpu_io_mixed_tg       | 448 ms                                                 | 469 ms: 1.05x slower                                          |
| sqlglot_optimize                 | 34.9 ms                                                | 36.7 ms: 1.05x slower                                         |
| scimark_sparse_mat_mult          | 2.99 ms                                                | 3.18 ms: 1.07x slower                                         |
| comprehensions                   | 12.3 us                                                | 13.1 us: 1.07x slower                                         |
| async_tree_none_tg               | 198 ms                                                 | 213 ms: 1.07x slower                                          |
| json_dumps                       | 6.52 ms                                                | 7.08 ms: 1.09x slower                                         |
| docutils                         | 1.44 sec                                               | 1.57 sec: 1.09x slower                                        |
| sphinx                           | 603 ms                                                 | 665 ms: 1.10x slower                                          |
| sympy_integrate                  | 11.3 ms                                                | 12.5 ms: 1.11x slower                                         |
| create_gc_cycles                 | 1.17 ms                                                | 1.33 ms: 1.14x slower                                         |
| async_tree_io                    | 507 ms                                                 | 580 ms: 1.14x slower                                          |
| genshi_xml                       | 34.4 ms                                                | 41.0 ms: 1.19x slower                                         |
| async_tree_io_tg                 | 497 ms                                                 | 610 ms: 1.23x slower                                          |
| async_tree_eager_io              | 514 ms                                                 | 671 ms: 1.30x slower                                          |
| async_tree_eager_io_tg           | 477 ms                                                 | 716 ms: 1.50x slower                                          |
| Geometric mean                   | (ref)                                                  | 1.04x faster                                                  |

Benchmark hidden because not significant (8): async_tree_memoization, pylint, asyncio_websockets, logging_silent, regex_effbot, xml_etree_parse, async_tree_cpu_io_mixed, tornado_http
Ignored benchmarks (9) of results/bm-20241007-3.13.0-60403a5/bm-20241007-darwin-arm64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, dask, gevent_hub, k_core, mypy2, shortest_path, sqlalchemy_declarative, sqlalchemy_imperative

- Geometric mean (including insignificant results): 1.042x faster
# HPT report

- Reliability score: 99.56% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.09x