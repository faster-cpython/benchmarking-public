
# Results vs. 3.10.4

- fork: python
- ref: 29f98b46b77ee528477b
- machine: darwin-arm64
- commit hash: 29f98b4
- commit date: 2022-10-31
- overall geometric mean: 1.21x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.18x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 202 ms                                                 | 186 ms: 1.09x faster                                                   |
| chameleon      | 5.84 ms                                                | 4.67 ms: 1.25x faster                                                  |
| docutils       | 1.78 sec                                               | 1.47 sec: 1.21x faster                                                 |
| html5lib       | 44.1 ms                                                | 36.6 ms: 1.20x faster                                                  |
| tornado_http   | 91.9 ms                                                | 70.1 ms: 1.31x faster                                                  |
| Geometric mean | (ref)                                                  | 1.21x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| nbody          | 94.1 ms                                                | 64.7 ms: 1.45x faster                                                  |
| float          | 72.3 ms                                                | 56.0 ms: 1.29x faster                                                  |
| Geometric mean | (ref)                                                  | 1.23x faster                                                           |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 96.6 ms                                                | 76.3 ms: 1.27x faster                                                  |
| regex_v8       | 17.5 ms                                                | 16.0 ms: 1.09x faster                                                  |
| regex_dna      | 160 ms                                                 | 149 ms: 1.07x faster                                                   |
| regex_effbot   | 2.45 ms                                                | 2.61 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.09x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| pickle_pure_python   | 284 us                                                 | 207 us: 1.37x faster                                                   |
| json_dumps           | 8.38 ms                                                | 6.15 ms: 1.36x faster                                                  |
| xml_etree_process    | 45.1 ms                                                | 34.9 ms: 1.29x faster                                                  |
| unpickle_pure_python | 203 us                                                 | 162 us: 1.26x faster                                                   |
| xml_etree_parse      | 107 ms                                                 | 91.1 ms: 1.18x faster                                                  |
| xml_etree_generate   | 54.3 ms                                                | 47.9 ms: 1.13x faster                                                  |
| xml_etree_iterparse  | 72.6 ms                                                | 64.9 ms: 1.12x faster                                                  |
| unpickle_list        | 2.66 us                                                | 2.55 us: 1.04x faster                                                  |
| json_loads           | 16.9 us                                                | 16.3 us: 1.03x faster                                                  |
| pickle_dict          | 17.8 us                                                | 18.0 us: 1.01x slower                                                  |
| pickle_list          | 2.83 us                                                | 2.86 us: 1.01x slower                                                  |
| pickle               | 7.36 us                                                | 7.58 us: 1.03x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.12x faster                                                           |

Benchmark hidden because not significant (1): unpickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.6 ms                                                | 9.49 ms: 1.33x faster                                                  |
| python_startup_no_site | 9.73 ms                                                | 7.51 ms: 1.29x faster                                                  |
| Geometric mean         | (ref)                                                  | 1.31x faster                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 10.5 ms                                                | 8.09 ms: 1.29x faster                                                  |
| django_template | 27.3 ms                                                | 21.7 ms: 1.26x faster                                                  |
| genshi_xml      | 37.6 ms                                                | 30.2 ms: 1.25x faster                                                  |
| genshi_text     | 18.4 ms                                                | 15.3 ms: 1.20x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.25x faster                                                           |

All benchmarks:
===============

| Benchmark               | bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120 | bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4 |
|-------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| deltablue               | 5.15 ms                                                | 2.74 ms: 1.88x faster                                                  |
| logging_silent          | 119 ns                                                 | 66.0 ns: 1.80x faster                                                  |
| crypto_pyaes            | 78.0 ms                                                | 51.4 ms: 1.52x faster                                                  |
| async_tree_memoization  | 493 ms                                                 | 325 ms: 1.52x faster                                                   |
| scimark_lu              | 110 ms                                                 | 72.9 ms: 1.51x faster                                                  |
| richards                | 51.4 ms                                                | 34.3 ms: 1.50x faster                                                  |
| nbody                   | 94.1 ms                                                | 64.7 ms: 1.45x faster                                                  |
| raytrace                | 328 ms                                                 | 226 ms: 1.45x faster                                                   |
| async_tree_none         | 402 ms                                                 | 287 ms: 1.40x faster                                                   |
| async_tree_io           | 1.02 sec                                               | 733 ms: 1.39x faster                                                   |
| pathlib                 | 28.8 ms                                                | 20.7 ms: 1.39x faster                                                  |
| go                      | 165 ms                                                 | 119 ms: 1.38x faster                                                   |
| pickle_pure_python      | 284 us                                                 | 207 us: 1.37x faster                                                   |
| json_dumps              | 8.38 ms                                                | 6.15 ms: 1.36x faster                                                  |
| chaos                   | 66.8 ms                                                | 50.2 ms: 1.33x faster                                                  |
| python_startup          | 12.6 ms                                                | 9.49 ms: 1.33x faster                                                  |
| spectral_norm           | 96.4 ms                                                | 72.5 ms: 1.33x faster                                                  |
| thrift                  | 586 us                                                 | 443 us: 1.32x faster                                                   |
| sqlglot_parse           | 1.33 ms                                                | 1.01 ms: 1.32x faster                                                  |
| sqlglot_transpile       | 1.54 ms                                                | 1.17 ms: 1.31x faster                                                  |
| tornado_http            | 91.9 ms                                                | 70.1 ms: 1.31x faster                                                  |
| scimark_monte_carlo     | 72.2 ms                                                | 55.5 ms: 1.30x faster                                                  |
| pycparser               | 915 ms                                                 | 705 ms: 1.30x faster                                                   |
| python_startup_no_site  | 9.73 ms                                                | 7.51 ms: 1.29x faster                                                  |
| mako                    | 10.5 ms                                                | 8.09 ms: 1.29x faster                                                  |
| xml_etree_process       | 45.1 ms                                                | 34.9 ms: 1.29x faster                                                  |
| float                   | 72.3 ms                                                | 56.0 ms: 1.29x faster                                                  |
| pyflate                 | 453 ms                                                 | 353 ms: 1.29x faster                                                   |
| hexiom                  | 6.32 ms                                                | 4.92 ms: 1.28x faster                                                  |
| regex_compile           | 96.6 ms                                                | 76.3 ms: 1.27x faster                                                  |
| dulwich_log             | 37.1 ms                                                | 29.4 ms: 1.26x faster                                                  |
| unpickle_pure_python    | 203 us                                                 | 162 us: 1.26x faster                                                   |
| django_template         | 27.3 ms                                                | 21.7 ms: 1.26x faster                                                  |
| scimark_sor             | 127 ms                                                 | 101 ms: 1.26x faster                                                   |
| logging_simple          | 4.63 us                                                | 3.70 us: 1.25x faster                                                  |
| chameleon               | 5.84 ms                                                | 4.67 ms: 1.25x faster                                                  |
| pprint_safe_repr        | 609 ms                                                 | 487 ms: 1.25x faster                                                   |
| pprint_pformat          | 1.24 sec                                               | 993 ms: 1.25x faster                                                   |
| genshi_xml              | 37.6 ms                                                | 30.2 ms: 1.25x faster                                                  |
| logging_format          | 5.01 us                                                | 4.02 us: 1.25x faster                                                  |
| scimark_sparse_mat_mult | 3.47 ms                                                | 2.79 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed | 670 ms                                                 | 541 ms: 1.24x faster                                                   |
| bench_thread_pool       | 548 us                                                 | 453 us: 1.21x faster                                                   |
| docutils                | 1.78 sec                                               | 1.47 sec: 1.21x faster                                                 |
| html5lib                | 44.1 ms                                                | 36.6 ms: 1.20x faster                                                  |
| genshi_text             | 18.4 ms                                                | 15.3 ms: 1.20x faster                                                  |
| sqlglot_optimize        | 38.0 ms                                                | 31.8 ms: 1.20x faster                                                  |
| deepcopy_memo           | 34.5 us                                                | 29.0 us: 1.19x faster                                                  |
| fannkuch                | 317 ms                                                 | 268 ms: 1.18x faster                                                   |
| scimark_fft             | 232 ms                                                 | 196 ms: 1.18x faster                                                   |
| xml_etree_parse         | 107 ms                                                 | 91.1 ms: 1.18x faster                                                  |
| async_generators        | 233 ms                                                 | 200 ms: 1.17x faster                                                   |
| deepcopy_reduce         | 2.38 us                                                | 2.06 us: 1.16x faster                                                  |
| deepcopy                | 278 us                                                 | 239 us: 1.16x faster                                                   |
| pylint                  | 307 ms                                                 | 265 ms: 1.16x faster                                                   |
| sympy_integrate         | 13.4 ms                                                | 11.6 ms: 1.15x faster                                                  |
| generators              | 32.9 ms                                                | 28.9 ms: 1.14x faster                                                  |
| unpack_sequence         | 38.7 ns                                                | 34.0 ns: 1.14x faster                                                  |
| sqlglot_normalize       | 197 ms                                                 | 173 ms: 1.13x faster                                                   |
| nqueens                 | 68.1 ms                                                | 60.1 ms: 1.13x faster                                                  |
| xml_etree_generate      | 54.3 ms                                                | 47.9 ms: 1.13x faster                                                  |
| xml_etree_iterparse     | 72.6 ms                                                | 64.9 ms: 1.12x faster                                                  |
| sympy_expand            | 276 ms                                                 | 247 ms: 1.12x faster                                                   |
| sympy_str               | 169 ms                                                 | 153 ms: 1.11x faster                                                   |
| telco                   | 3.68 ms                                                | 3.34 ms: 1.10x faster                                                  |
| sympy_sum               | 94.3 ms                                                | 85.8 ms: 1.10x faster                                                  |
| mdp                     | 1.67 sec                                               | 1.52 sec: 1.10x faster                                                 |
| regex_v8                | 17.5 ms                                                | 16.0 ms: 1.09x faster                                                  |
| json                    | 3.10 ms                                                | 2.84 ms: 1.09x faster                                                  |
| 2to3                    | 202 ms                                                 | 186 ms: 1.09x faster                                                   |
| regex_dna               | 160 ms                                                 | 149 ms: 1.07x faster                                                   |
| coroutines              | 20.2 ms                                                | 19.2 ms: 1.05x faster                                                  |
| unpickle_list           | 2.66 us                                                | 2.55 us: 1.04x faster                                                  |
| json_loads              | 16.9 us                                                | 16.3 us: 1.03x faster                                                  |
| meteor_contest          | 78.6 ms                                                | 76.6 ms: 1.03x faster                                                  |
| sqlite_synth            | 1.47 us                                                | 1.44 us: 1.02x faster                                                  |
| pickle_dict             | 17.8 us                                                | 18.0 us: 1.01x slower                                                  |
| pickle_list             | 2.83 us                                                | 2.86 us: 1.01x slower                                                  |
| bench_mp_pool           | 41.0 ms                                                | 41.8 ms: 1.02x slower                                                  |
| pickle                  | 7.36 us                                                | 7.58 us: 1.03x slower                                                  |
| regex_effbot            | 2.45 ms                                                | 2.61 ms: 1.07x slower                                                  |
| coverage                | 40.8 ms                                                | 53.1 ms: 1.30x slower                                                  |
| Geometric mean          | (ref)                                                  | 1.21x faster                                                           |

Benchmark hidden because not significant (2): pidigits, unpickle
Ignored benchmarks (15) of results/bm-20220323-3.10.4-9d38120/bm-20220323-darwin-arm64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, comprehensions, create_gc_cycles, dask, flaskblogging, gc_traversal, gunicorn, mypy2, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20221031-3.12.0a1+-29f98b4/bm-20221031-darwin-arm64-python-29f98b46b77ee528477b-3.12.0a1+-29f98b4.json: mypy


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.20x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.18x
