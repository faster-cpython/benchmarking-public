# Results vs. 3.12.0

- fork: python
- ref: cebae977a63f32c3c03d
- machine: darwin-arm64
- commit hash: cebae97
- commit date: 2025-06-01
- overall geometric mean: 1.072x slower
- HPT reliability: 99.96%
- HPT 99th percentile: 1.03x slower
- Memory change: 1.26x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 168 ms                                                 | 203 ms: 1.21x slower                                                  |
| docutils       | 1.45 sec                                               | 1.60 sec: 1.10x slower                                                |
| html5lib       | 33.4 ms                                                | 35.1 ms: 1.05x slower                                                 |
| sphinx         | 613 ms                                                 | 682 ms: 1.11x slower                                                  |
| Geometric mean | (ref)                                                  | 1.12x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg                 | 673 ms                                                 | 327 ms: 2.06x faster                                                  |
| async_tree_eager_io              | 666 ms                                                 | 333 ms: 2.00x faster                                                  |
| async_tree_io                    | 672 ms                                                 | 345 ms: 1.95x faster                                                  |
| async_tree_eager_io_tg           | 617 ms                                                 | 319 ms: 1.93x faster                                                  |
| async_tree_memoization_tg        | 318 ms                                                 | 181 ms: 1.76x faster                                                  |
| async_tree_none_tg               | 255 ms                                                 | 147 ms: 1.74x faster                                                  |
| async_tree_none                  | 263 ms                                                 | 165 ms: 1.59x faster                                                  |
| async_tree_memoization           | 310 ms                                                 | 200 ms: 1.55x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 528 ms                                                 | 414 ms: 1.27x faster                                                  |
| async_tree_cpu_io_mixed          | 527 ms                                                 | 434 ms: 1.21x faster                                                  |
| async_tree_eager_memoization     | 168 ms                                                 | 149 ms: 1.13x faster                                                  |
| coroutines                       | 19.7 ms                                                | 18.1 ms: 1.09x faster                                                 |
| asyncio_websockets               | 243 ms                                                 | 238 ms: 1.02x faster                                                  |
| async_tree_eager_cpu_io_mixed    | 374 ms                                                 | 392 ms: 1.05x slower                                                  |
| async_generators                 | 299 ms                                                 | 313 ms: 1.05x slower                                                  |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 399 ms: 1.15x slower                                                  |
| async_tree_eager_memoization_tg  | 142 ms                                                 | 165 ms: 1.16x slower                                                  |
| async_tree_eager                 | 65.8 ms                                                | 81.8 ms: 1.24x slower                                                 |
| async_tree_eager_tg              | 46.9 ms                                                | 128 ms: 2.73x slower                                                  |
| Geometric mean                   | (ref)                                                  | 1.22x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 54.1 ms                                                | 49.3 ms: 1.10x faster                                                 |
| pidigits       | 283 ms                                                 | 281 ms: 1.01x faster                                                  |
| nbody          | 67.6 ms                                                | 92.7 ms: 1.37x slower                                                 |
| Geometric mean | (ref)                                                  | 1.08x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 2.44 ms                                                | 2.20 ms: 1.11x faster                                                 |
| regex_dna      | 143 ms                                                 | 145 ms: 1.01x slower                                                  |
| regex_compile  | 75.9 ms                                                | 83.8 ms: 1.10x slower                                                 |
| regex_v8       | 15.1 ms                                                | 17.1 ms: 1.13x slower                                                 |
| Geometric mean | (ref)                                                  | 1.03x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| xml_etree_iterparse  | 75.5 ms                                                | 69.2 ms: 1.09x faster                                                 |
| xml_etree_parse      | 108 ms                                                 | 112 ms: 1.04x slower                                                  |
| tomli_loads          | 1.36 sec                                               | 1.57 sec: 1.15x slower                                                |
| unpickle_pure_python | 145 us                                                 | 169 us: 1.16x slower                                                  |
| pickle_pure_python   | 197 us                                                 | 238 us: 1.21x slower                                                  |
| xml_etree_process    | 38.9 ms                                                | 50.0 ms: 1.29x slower                                                 |
| xml_etree_generate   | 55.4 ms                                                | 72.5 ms: 1.31x slower                                                 |
| json_dumps           | 6.19 ms                                                | 8.38 ms: 1.35x slower                                                 |
| json_loads           | 17.0 us                                                | 24.4 us: 1.43x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.20x slower                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 17.8 ms                                                | 20.3 ms: 1.14x slower                                                 |
| python_startup_no_site | 13.2 ms                                                | 15.5 ms: 1.18x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| genshi_xml      | 30.5 ms                                                | 37.6 ms: 1.23x slower                                                 |
| genshi_text     | 14.7 ms                                                | 18.3 ms: 1.25x slower                                                 |
| django_template | 19.7 ms                                                | 28.2 ms: 1.43x slower                                                 |
| mako            | 7.44 ms                                                | 11.0 ms: 1.48x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.34x slower                                                          |

All benchmarks:
===============

| Benchmark                        | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97 |
|----------------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg                 | 673 ms                                                 | 327 ms: 2.06x faster                                                  |
| gc_traversal                     | 2.95 ms                                                | 1.43 ms: 2.05x faster                                                 |
| async_tree_eager_io              | 666 ms                                                 | 333 ms: 2.00x faster                                                  |
| async_tree_io                    | 672 ms                                                 | 345 ms: 1.95x faster                                                  |
| async_tree_eager_io_tg           | 617 ms                                                 | 319 ms: 1.93x faster                                                  |
| subparsers                       | 32.1 ms                                                | 17.0 ms: 1.89x faster                                                 |
| async_tree_memoization_tg        | 318 ms                                                 | 181 ms: 1.76x faster                                                  |
| async_tree_none_tg               | 255 ms                                                 | 147 ms: 1.74x faster                                                  |
| async_tree_none                  | 263 ms                                                 | 165 ms: 1.59x faster                                                  |
| async_tree_memoization           | 310 ms                                                 | 200 ms: 1.55x faster                                                  |
| mdp                              | 1.49 sec                                               | 1.00 sec: 1.49x faster                                                |
| create_gc_cycles                 | 1.15 ms                                                | 855 us: 1.34x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 528 ms                                                 | 414 ms: 1.27x faster                                                  |
| async_tree_cpu_io_mixed          | 527 ms                                                 | 434 ms: 1.21x faster                                                  |
| generators                       | 29.4 ms                                                | 24.2 ms: 1.21x faster                                                 |
| go                               | 98.5 ms                                                | 85.0 ms: 1.16x faster                                                 |
| deepcopy_memo                    | 26.0 us                                                | 22.5 us: 1.16x faster                                                 |
| async_tree_eager_memoization     | 168 ms                                                 | 149 ms: 1.13x faster                                                  |
| regex_effbot                     | 2.44 ms                                                | 2.20 ms: 1.11x faster                                                 |
| float                            | 54.1 ms                                                | 49.3 ms: 1.10x faster                                                 |
| xml_etree_iterparse              | 75.5 ms                                                | 69.2 ms: 1.09x faster                                                 |
| coroutines                       | 19.7 ms                                                | 18.1 ms: 1.09x faster                                                 |
| deepcopy                         | 226 us                                                 | 213 us: 1.06x faster                                                  |
| asyncio_websockets               | 243 ms                                                 | 238 ms: 1.02x faster                                                  |
| dulwich_log                      | 29.2 ms                                                | 28.9 ms: 1.01x faster                                                 |
| pidigits                         | 283 ms                                                 | 281 ms: 1.01x faster                                                  |
| k_core                           | 1.72 sec                                               | 1.74 sec: 1.01x slower                                                |
| regex_dna                        | 143 ms                                                 | 145 ms: 1.01x slower                                                  |
| xml_etree_parse                  | 108 ms                                                 | 112 ms: 1.04x slower                                                  |
| pyflate                          | 311 ms                                                 | 323 ms: 1.04x slower                                                  |
| pylint                           | 182 ms                                                 | 189 ms: 1.04x slower                                                  |
| pathlib                          | 24.7 ms                                                | 25.8 ms: 1.05x slower                                                 |
| async_tree_eager_cpu_io_mixed    | 374 ms                                                 | 392 ms: 1.05x slower                                                  |
| async_generators                 | 299 ms                                                 | 313 ms: 1.05x slower                                                  |
| html5lib                         | 33.4 ms                                                | 35.1 ms: 1.05x slower                                                 |
| sqlite_synth                     | 1.55 us                                                | 1.64 us: 1.06x slower                                                 |
| deltablue                        | 2.57 ms                                                | 2.74 ms: 1.07x slower                                                 |
| pycparser                        | 673 ms                                                 | 738 ms: 1.10x slower                                                  |
| comprehensions                   | 14.0 us                                                | 15.4 us: 1.10x slower                                                 |
| docutils                         | 1.45 sec                                               | 1.60 sec: 1.10x slower                                                |
| shortest_path                    | 331 ms                                                 | 365 ms: 1.10x slower                                                  |
| regex_compile                    | 75.9 ms                                                | 83.8 ms: 1.10x slower                                                 |
| sphinx                           | 613 ms                                                 | 682 ms: 1.11x slower                                                  |
| connected_components             | 300 ms                                                 | 334 ms: 1.11x slower                                                  |
| bpe_tokeniser                    | 3.28 sec                                               | 3.66 sec: 1.11x slower                                                |
| deepcopy_reduce                  | 2.01 us                                                | 2.27 us: 1.13x slower                                                 |
| scimark_sor                      | 85.8 ms                                                | 97.0 ms: 1.13x slower                                                 |
| regex_v8                         | 15.1 ms                                                | 17.1 ms: 1.13x slower                                                 |
| hexiom                           | 4.38 ms                                                | 4.96 ms: 1.13x slower                                                 |
| python_startup                   | 17.8 ms                                                | 20.3 ms: 1.14x slower                                                 |
| async_tree_eager_cpu_io_mixed_tg | 347 ms                                                 | 399 ms: 1.15x slower                                                  |
| tomli_loads                      | 1.36 sec                                               | 1.57 sec: 1.15x slower                                                |
| raytrace                         | 204 ms                                                 | 236 ms: 1.16x slower                                                  |
| async_tree_eager_memoization_tg  | 142 ms                                                 | 165 ms: 1.16x slower                                                  |
| unpickle_pure_python             | 145 us                                                 | 169 us: 1.16x slower                                                  |
| sympy_integrate                  | 11.1 ms                                                | 13.0 ms: 1.18x slower                                                 |
| python_startup_no_site           | 13.2 ms                                                | 15.5 ms: 1.18x slower                                                 |
| chaos                            | 41.6 ms                                                | 49.1 ms: 1.18x slower                                                 |
| spectral_norm                    | 76.5 ms                                                | 90.9 ms: 1.19x slower                                                 |
| bench_mp_pool                    | 65.5 ms                                                | 78.4 ms: 1.20x slower                                                 |
| meteor_contest                   | 69.1 ms                                                | 83.0 ms: 1.20x slower                                                 |
| 2to3                             | 168 ms                                                 | 203 ms: 1.21x slower                                                  |
| pickle_pure_python               | 197 us                                                 | 238 us: 1.21x slower                                                  |
| logging_simple                   | 3.60 us                                                | 4.39 us: 1.22x slower                                                 |
| nqueens                          | 59.5 ms                                                | 72.5 ms: 1.22x slower                                                 |
| genshi_xml                       | 30.5 ms                                                | 37.6 ms: 1.23x slower                                                 |
| sympy_sum                        | 76.2 ms                                                | 94.1 ms: 1.23x slower                                                 |
| logging_format                   | 3.90 us                                                | 4.82 us: 1.24x slower                                                 |
| async_tree_eager                 | 65.8 ms                                                | 81.8 ms: 1.24x slower                                                 |
| genshi_text                      | 14.7 ms                                                | 18.3 ms: 1.25x slower                                                 |
| scimark_monte_carlo              | 44.5 ms                                                | 56.2 ms: 1.26x slower                                                 |
| sympy_str                        | 144 ms                                                 | 184 ms: 1.28x slower                                                  |
| xml_etree_process                | 38.9 ms                                                | 50.0 ms: 1.29x slower                                                 |
| xml_etree_generate               | 55.4 ms                                                | 72.5 ms: 1.31x slower                                                 |
| richards                         | 30.6 ms                                                | 41.0 ms: 1.34x slower                                                 |
| json                             | 3.00 ms                                                | 4.05 ms: 1.35x slower                                                 |
| richards_super                   | 34.6 ms                                                | 46.9 ms: 1.35x slower                                                 |
| json_dumps                       | 6.19 ms                                                | 8.38 ms: 1.35x slower                                                 |
| thrift                           | 468 us                                                 | 636 us: 1.36x slower                                                  |
| many_optionals                   | 403 us                                                 | 549 us: 1.36x slower                                                  |
| crypto_pyaes                     | 51.4 ms                                                | 70.4 ms: 1.37x slower                                                 |
| nbody                            | 67.6 ms                                                | 92.7 ms: 1.37x slower                                                 |
| sympy_expand                     | 233 ms                                                 | 325 ms: 1.39x slower                                                  |
| scimark_lu                       | 73.5 ms                                                | 103 ms: 1.40x slower                                                  |
| pprint_pformat                   | 986 ms                                                 | 1.39 sec: 1.41x slower                                                |
| pprint_safe_repr                 | 483 ms                                                 | 684 ms: 1.42x slower                                                  |
| json_loads                       | 17.0 us                                                | 24.4 us: 1.43x slower                                                 |
| django_template                  | 19.7 ms                                                | 28.2 ms: 1.43x slower                                                 |
| fannkuch                         | 250 ms                                                 | 359 ms: 1.43x slower                                                  |
| scimark_sparse_mat_mult          | 3.14 ms                                                | 4.54 ms: 1.45x slower                                                 |
| mako                             | 7.44 ms                                                | 11.0 ms: 1.48x slower                                                 |
| scimark_fft                      | 194 ms                                                 | 288 ms: 1.48x slower                                                  |
| typing_runtime_protocols         | 91.5 us                                                | 140 us: 1.53x slower                                                  |
| telco                            | 3.92 ms                                                | 6.25 ms: 1.59x slower                                                 |
| bench_thread_pool                | 483 us                                                 | 774 us: 1.60x slower                                                  |
| coverage                         | 38.5 ms                                                | 76.0 ms: 1.97x slower                                                 |
| async_tree_eager_tg              | 46.9 ms                                                | 128 ms: 2.73x slower                                                  |
| logging_silent                   | 72.5 ns                                                | 450 ns: 6.20x slower                                                  |
| Geometric mean                   | (ref)                                                  | 1.10x slower                                                          |
Ignored benchmarks (12) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0.json: chameleon, dask, djangocms, gevent_hub, gunicorn, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20250601-3.15.0a0-cebae97-NOGIL/bm-20250601-darwin-arm64-python-cebae977a63f32c3c03d-3.15.0a0-cebae97.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.072x slower

# HPT report

- Reliability score: 99.96% likely to be slow
- 90% likely to have a slowdown of 1.07x
- 95% likely to have a slowdown of 1.05x
- 99% likely to have a slowdown of 1.03x

# Memory
- memory change: 1.26x