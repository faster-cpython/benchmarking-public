# Results vs. 3.10.4

- fork: python
- ref: db6f5e193315a3bbfa7b
- machine: linux-aarch64
- commit hash: db6f5e1
- commit date: 2024-08-13
- overall geometric mean: 1.22x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.18x slower
- Memory change: 1.33x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 517 ms: 1.36x slower                                                    |
| docutils       | 3.53 sec                                                              | 4.11 sec: 1.17x slower                                                  |
| html5lib       | 86.5 ms                                                               | 121 ms: 1.40x slower                                                    |
| tornado_http   | 178 ms                                                                | 208 ms: 1.16x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.27x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_io           | 2.28 sec                                                              | 1.41 sec: 1.63x faster                                                  |
| async_tree_none         | 950 ms                                                                | 608 ms: 1.56x faster                                                    |
| async_tree_memoization  | 1.13 sec                                                              | 746 ms: 1.52x faster                                                    |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 917 ms: 1.39x faster                                                    |
| Geometric mean          | (ref)                                                                 | 1.52x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 135 ms                                                                | 212 ms: 1.57x slower                                                    |
| nbody          | 166 ms                                                                | 286 ms: 1.72x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.40x slower                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_dna      | 257 ms                                                                | 249 ms: 1.03x faster                                                    |
| regex_v8       | 32.2 ms                                                               | 33.0 ms: 1.03x slower                                                   |
| regex_effbot   | 4.25 ms                                                               | 4.41 ms: 1.04x slower                                                   |
| regex_compile  | 177 ms                                                                | 258 ms: 1.46x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.11x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_parse      | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| xml_etree_iterparse  | 156 ms                                                                | 159 ms: 1.02x slower                                                    |
| json_dumps           | 16.7 ms                                                               | 17.8 ms: 1.07x slower                                                   |
| json_loads           | 30.9 us                                                               | 38.9 us: 1.26x slower                                                   |
| tomli_loads          | 3.36 sec                                                              | 4.23 sec: 1.26x slower                                                  |
| xml_etree_generate   | 123 ms                                                                | 162 ms: 1.32x slower                                                    |
| xml_etree_process    | 99.5 ms                                                               | 132 ms: 1.33x slower                                                    |
| pickle_pure_python   | 529 us                                                                | 782 us: 1.48x slower                                                    |
| unpickle_pure_python | 366 us                                                                | 543 us: 1.48x slower                                                    |
| Geometric mean       | (ref)                                                                 | 1.22x slower                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 18.1 ms: 1.62x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 12.1 ms: 1.76x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.69x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_xml      | 69.8 ms                                                               | 106 ms: 1.51x slower                                                    |
| genshi_text     | 35.2 ms                                                               | 53.6 ms: 1.52x slower                                                   |
| mako            | 18.9 ms                                                               | 29.0 ms: 1.53x slower                                                   |
| django_template | 53.3 ms                                                               | 84.2 ms: 1.58x slower                                                   |
| Geometric mean  | (ref)                                                                 | 1.54x slower                                                            |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| bench_mp_pool            | 14.5 ms                                                               | 7.13 ms: 2.04x faster                                                   |
| typing_runtime_protocols | 661 us                                                                | 343 us: 1.93x faster                                                    |
| async_tree_io            | 2.28 sec                                                              | 1.41 sec: 1.63x faster                                                  |
| async_tree_none          | 950 ms                                                                | 608 ms: 1.56x faster                                                    |
| async_tree_memoization   | 1.13 sec                                                              | 746 ms: 1.52x faster                                                    |
| create_gc_cycles         | 2.26 ms                                                               | 1.61 ms: 1.40x faster                                                   |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 917 ms: 1.39x faster                                                    |
| asyncio_tcp              | 944 ms                                                                | 684 ms: 1.38x faster                                                    |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.59 sec: 1.27x faster                                                  |
| gc_traversal             | 4.15 ms                                                               | 3.48 ms: 1.19x faster                                                   |
| generators               | 68.1 ms                                                               | 57.1 ms: 1.19x faster                                                   |
| xml_etree_parse          | 212 ms                                                                | 188 ms: 1.13x faster                                                    |
| regex_dna                | 257 ms                                                                | 249 ms: 1.03x faster                                                    |
| xml_etree_iterparse      | 156 ms                                                                | 159 ms: 1.02x slower                                                    |
| pathlib                  | 26.3 ms                                                               | 26.9 ms: 1.02x slower                                                   |
| crypto_pyaes             | 134 ms                                                                | 137 ms: 1.03x slower                                                    |
| regex_v8                 | 32.2 ms                                                               | 33.0 ms: 1.03x slower                                                   |
| asyncio_websockets       | 657 ms                                                                | 675 ms: 1.03x slower                                                    |
| regex_effbot             | 4.25 ms                                                               | 4.41 ms: 1.04x slower                                                   |
| pylint                   | 485 ms                                                                | 510 ms: 1.05x slower                                                    |
| json_dumps               | 16.7 ms                                                               | 17.8 ms: 1.07x slower                                                   |
| coroutines               | 37.2 ms                                                               | 40.0 ms: 1.08x slower                                                   |
| deepcopy                 | 511 us                                                                | 563 us: 1.10x slower                                                    |
| scimark_fft              | 500 ms                                                                | 580 ms: 1.16x slower                                                    |
| tornado_http             | 178 ms                                                                | 208 ms: 1.16x slower                                                    |
| docutils                 | 3.53 sec                                                              | 4.11 sec: 1.17x slower                                                  |
| deepcopy_memo            | 61.7 us                                                               | 72.2 us: 1.17x slower                                                   |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 8.95 ms: 1.17x slower                                                   |
| mdp                      | 3.70 sec                                                              | 4.36 sec: 1.18x slower                                                  |
| json                     | 5.88 ms                                                               | 7.00 ms: 1.19x slower                                                   |
| pycparser                | 1.69 sec                                                              | 2.05 sec: 1.21x slower                                                  |
| comprehensions           | 33.1 us                                                               | 40.8 us: 1.23x slower                                                   |
| spectral_norm            | 186 ms                                                                | 233 ms: 1.25x slower                                                    |
| json_loads               | 30.9 us                                                               | 38.9 us: 1.26x slower                                                   |
| tomli_loads              | 3.36 sec                                                              | 4.23 sec: 1.26x slower                                                  |
| pyflate                  | 795 ms                                                                | 1.02 sec: 1.28x slower                                                  |
| richards                 | 91.7 ms                                                               | 118 ms: 1.29x slower                                                    |
| logging_silent           | 222 ns                                                                | 286 ns: 1.29x slower                                                    |
| nqueens                  | 117 ms                                                                | 152 ms: 1.29x slower                                                    |
| richards_super           | 107 ms                                                                | 139 ms: 1.30x slower                                                    |
| chaos                    | 121 ms                                                                | 159 ms: 1.31x slower                                                    |
| xml_etree_generate       | 123 ms                                                                | 162 ms: 1.32x slower                                                    |
| sympy_integrate          | 26.5 ms                                                               | 35.0 ms: 1.32x slower                                                   |
| deepcopy_reduce          | 4.60 us                                                               | 6.08 us: 1.32x slower                                                   |
| xml_etree_process        | 99.5 ms                                                               | 132 ms: 1.33x slower                                                    |
| thrift                   | 1.26 ms                                                               | 1.69 ms: 1.34x slower                                                   |
| bench_thread_pool        | 1.59 ms                                                               | 2.13 ms: 1.34x slower                                                   |
| meteor_contest           | 126 ms                                                                | 169 ms: 1.34x slower                                                    |
| 2to3                     | 381 ms                                                                | 517 ms: 1.36x slower                                                    |
| fannkuch                 | 546 ms                                                                | 751 ms: 1.38x slower                                                    |
| scimark_sor              | 246 ms                                                                | 339 ms: 1.38x slower                                                    |
| scimark_monte_carlo      | 128 ms                                                                | 177 ms: 1.38x slower                                                    |
| html5lib                 | 86.5 ms                                                               | 121 ms: 1.40x slower                                                    |
| deltablue                | 8.94 ms                                                               | 12.7 ms: 1.42x slower                                                   |
| raytrace                 | 573 ms                                                                | 816 ms: 1.42x slower                                                    |
| hexiom                   | 10.9 ms                                                               | 15.8 ms: 1.44x slower                                                   |
| regex_compile            | 177 ms                                                                | 258 ms: 1.46x slower                                                    |
| pickle_pure_python       | 529 us                                                                | 782 us: 1.48x slower                                                    |
| unpickle_pure_python     | 366 us                                                                | 543 us: 1.48x slower                                                    |
| async_generators         | 452 ms                                                                | 673 ms: 1.49x slower                                                    |
| sqlglot_normalize        | 156 ms                                                                | 235 ms: 1.50x slower                                                    |
| sqlglot_transpile        | 2.84 ms                                                               | 4.27 ms: 1.50x slower                                                   |
| genshi_xml               | 69.8 ms                                                               | 106 ms: 1.51x slower                                                    |
| logging_format           | 10.6 us                                                               | 16.1 us: 1.52x slower                                                   |
| logging_simple           | 9.78 us                                                               | 14.8 us: 1.52x slower                                                   |
| genshi_text              | 35.2 ms                                                               | 53.6 ms: 1.52x slower                                                   |
| telco                    | 8.49 ms                                                               | 12.9 ms: 1.53x slower                                                   |
| scimark_lu               | 227 ms                                                                | 347 ms: 1.53x slower                                                    |
| mako                     | 18.9 ms                                                               | 29.0 ms: 1.53x slower                                                   |
| sqlglot_parse            | 2.40 ms                                                               | 3.71 ms: 1.54x slower                                                   |
| pprint_pformat           | 2.36 sec                                                              | 3.66 sec: 1.55x slower                                                  |
| pprint_safe_repr         | 1.15 sec                                                              | 1.78 sec: 1.55x slower                                                  |
| sqlglot_optimize         | 75.4 ms                                                               | 117 ms: 1.55x slower                                                    |
| float                    | 135 ms                                                                | 212 ms: 1.57x slower                                                    |
| django_template          | 53.3 ms                                                               | 84.2 ms: 1.58x slower                                                   |
| sympy_str                | 329 ms                                                                | 521 ms: 1.58x slower                                                    |
| python_startup           | 11.2 ms                                                               | 18.1 ms: 1.62x slower                                                   |
| coverage                 | 83.6 ms                                                               | 136 ms: 1.63x slower                                                    |
| go                       | 264 ms                                                                | 445 ms: 1.69x slower                                                    |
| sympy_sum                | 184 ms                                                                | 317 ms: 1.72x slower                                                    |
| nbody                    | 166 ms                                                                | 286 ms: 1.72x slower                                                    |
| python_startup_no_site   | 6.89 ms                                                               | 12.1 ms: 1.76x slower                                                   |
| sympy_expand             | 543 ms                                                                | 964 ms: 1.78x slower                                                    |
| Geometric mean           | (ref)                                                                 | 1.22x slower                                                            |

Benchmark hidden because not significant (1): pidigits
Ignored benchmarks (15) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dask, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240813-3.14.0a0-db6f5e1-NOGIL/bm-20240813-arminc-aarch64-python-db6f5e193315a3bbfa7b-3.14.0a0-db6f5e1.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.25x
- 95% likely to have a slowdown of 1.22x
- 99% likely to have a slowdown of 1.18x

# Memory
- memory change: 1.33x