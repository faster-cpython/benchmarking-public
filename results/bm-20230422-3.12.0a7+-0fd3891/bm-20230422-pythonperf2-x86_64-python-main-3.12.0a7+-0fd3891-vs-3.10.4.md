
# Results vs. 3.10.4

- fork: python
- ref: main
- machine: linux-x86_64
- commit hash: 0fd3891
- commit date: 2023-04-22
- overall geometric mean: 1.26x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.19x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| 2to3           | 350 ms                                                       | 284 ms: 1.23x faster                                         |
| chameleon      | 9.72 ms                                                      | 7.48 ms: 1.30x faster                                        |
| docutils       | 3.40 sec                                                     | 2.87 sec: 1.19x faster                                       |
| html5lib       | 94.6 ms                                                      | 67.8 ms: 1.40x faster                                        |
| tornado_http   | 152 ms                                                       | 119 ms: 1.28x faster                                         |
| Geometric mean | (ref)                                                        | 1.28x faster                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| nbody          | 137 ms                                                       | 86.6 ms: 1.59x faster                                        |
| float          | 110 ms                                                       | 79.1 ms: 1.39x faster                                        |
| pidigits       | 271 ms                                                       | 260 ms: 1.04x faster                                         |
| Geometric mean | (ref)                                                        | 1.32x faster                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| regex_compile  | 194 ms                                                       | 143 ms: 1.36x faster                                         |
| regex_dna      | 259 ms                                                       | 232 ms: 1.12x faster                                         |
| regex_v8       | 26.6 ms                                                      | 24.1 ms: 1.11x faster                                        |
| regex_effbot   | 2.99 ms                                                      | 3.53 ms: 1.18x slower                                        |
| Geometric mean | (ref)                                                        | 1.09x faster                                                 |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|----------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| unpickle_pure_python | 321 us                                                       | 205 us: 1.56x faster                                         |
| pickle_pure_python   | 457 us                                                       | 315 us: 1.45x faster                                         |
| json_dumps           | 14.2 ms                                                      | 10.1 ms: 1.40x faster                                        |
| xml_etree_process    | 76.0 ms                                                      | 58.2 ms: 1.31x faster                                        |
| json_loads           | 30.0 us                                                      | 24.5 us: 1.23x faster                                        |
| xml_etree_generate   | 94.6 ms                                                      | 84.5 ms: 1.12x faster                                        |
| xml_etree_parse      | 162 ms                                                       | 145 ms: 1.12x faster                                         |
| xml_etree_iterparse  | 110 ms                                                       | 104 ms: 1.06x faster                                         |
| pickle_list          | 4.11 us                                                      | 3.96 us: 1.04x faster                                        |
| pickle               | 9.94 us                                                      | 10.2 us: 1.02x slower                                        |
| pickle_dict          | 30.0 us                                                      | 30.9 us: 1.03x slower                                        |
| unpickle_list        | 4.49 us                                                      | 4.72 us: 1.05x slower                                        |
| Geometric mean       | (ref)                                                        | 1.15x faster                                                 |

Benchmark hidden because not significant (1): unpickle

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|------------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| python_startup         | 11.5 ms                                                      | 11.1 ms: 1.03x faster                                        |
| python_startup_no_site | 7.32 ms                                                      | 8.33 ms: 1.14x slower                                        |
| Geometric mean         | (ref)                                                        | 1.05x slower                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|-----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| mako            | 14.7 ms                                                      | 9.92 ms: 1.48x faster                                        |
| django_template | 51.5 ms                                                      | 38.6 ms: 1.34x faster                                        |
| genshi_text     | 31.5 ms                                                      | 24.9 ms: 1.26x faster                                        |
| genshi_xml      | 64.7 ms                                                      | 54.1 ms: 1.20x faster                                        |
| Geometric mean  | (ref)                                                        | 1.32x faster                                                 |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120 | bm-20230422-pythonperf2-x86_64-python-main-3.12.0a7+-0fd3891 |
|-------------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| deltablue               | 7.47 ms                                                      | 3.30 ms: 2.27x faster                                        |
| asyncio_tcp             | 782 ms                                                       | 376 ms: 2.08x faster                                         |
| go                      | 259 ms                                                       | 145 ms: 1.78x faster                                         |
| logging_silent          | 166 ns                                                       | 94.2 ns: 1.76x faster                                        |
| scimark_lu              | 164 ms                                                       | 94.7 ms: 1.73x faster                                        |
| richards                | 74.1 ms                                                      | 43.3 ms: 1.71x faster                                        |
| sqlglot_parse           | 2.26 ms                                                      | 1.38 ms: 1.64x faster                                        |
| hexiom                  | 9.52 ms                                                      | 5.84 ms: 1.63x faster                                        |
| scimark_sor             | 177 ms                                                       | 109 ms: 1.63x faster                                         |
| raytrace                | 488 ms                                                       | 300 ms: 1.63x faster                                         |
| chaos                   | 107 ms                                                       | 66.4 ms: 1.61x faster                                        |
| pyflate                 | 697 ms                                                       | 437 ms: 1.60x faster                                         |
| generators              | 58.0 ms                                                      | 36.4 ms: 1.59x faster                                        |
| nbody                   | 137 ms                                                       | 86.6 ms: 1.59x faster                                        |
| spectral_norm           | 136 ms                                                       | 86.5 ms: 1.57x faster                                        |
| scimark_monte_carlo     | 109 ms                                                       | 69.8 ms: 1.57x faster                                        |
| unpickle_pure_python    | 321 us                                                       | 205 us: 1.56x faster                                         |
| sqlglot_transpile       | 2.71 ms                                                      | 1.78 ms: 1.52x faster                                        |
| mako                    | 14.7 ms                                                      | 9.92 ms: 1.48x faster                                        |
| crypto_pyaes            | 118 ms                                                       | 80.4 ms: 1.47x faster                                        |
| fannkuch                | 496 ms                                                       | 341 ms: 1.46x faster                                         |
| pickle_pure_python      | 457 us                                                       | 315 us: 1.45x faster                                         |
| async_tree_none         | 700 ms                                                       | 498 ms: 1.41x faster                                         |
| async_tree_io           | 1.61 sec                                                     | 1.14 sec: 1.41x faster                                       |
| async_tree_memoization  | 824 ms                                                       | 587 ms: 1.40x faster                                         |
| json_dumps              | 14.2 ms                                                      | 10.1 ms: 1.40x faster                                        |
| html5lib                | 94.6 ms                                                      | 67.8 ms: 1.40x faster                                        |
| float                   | 110 ms                                                       | 79.1 ms: 1.39x faster                                        |
| logging_simple          | 8.90 us                                                      | 6.49 us: 1.37x faster                                        |
| regex_compile           | 194 ms                                                       | 143 ms: 1.36x faster                                         |
| coroutines              | 30.4 ms                                                      | 22.6 ms: 1.34x faster                                        |
| logging_format          | 9.57 us                                                      | 7.15 us: 1.34x faster                                        |
| django_template         | 51.5 ms                                                      | 38.6 ms: 1.34x faster                                        |
| deepcopy_memo           | 48.9 us                                                      | 36.7 us: 1.33x faster                                        |
| pycparser               | 1.66 sec                                                     | 1.26 sec: 1.32x faster                                       |
| xml_etree_process       | 76.0 ms                                                      | 58.2 ms: 1.31x faster                                        |
| chameleon               | 9.72 ms                                                      | 7.48 ms: 1.30x faster                                        |
| async_tree_cpu_io_mixed | 952 ms                                                       | 733 ms: 1.30x faster                                         |
| pprint_pformat          | 2.15 sec                                                     | 1.66 sec: 1.29x faster                                       |
| pprint_safe_repr        | 1.05 sec                                                     | 815 ms: 1.29x faster                                         |
| thrift                  | 1.16 ms                                                      | 906 us: 1.29x faster                                         |
| tornado_http            | 152 ms                                                       | 119 ms: 1.28x faster                                         |
| nqueens                 | 112 ms                                                       | 89.0 ms: 1.26x faster                                        |
| genshi_text             | 31.5 ms                                                      | 24.9 ms: 1.26x faster                                        |
| scimark_fft             | 359 ms                                                       | 288 ms: 1.25x faster                                         |
| dulwich_log             | 80.1 ms                                                      | 64.9 ms: 1.23x faster                                        |
| 2to3                    | 350 ms                                                       | 284 ms: 1.23x faster                                         |
| json_loads              | 30.0 us                                                      | 24.5 us: 1.23x faster                                        |
| mypy2                   | 466 ms                                                       | 387 ms: 1.20x faster                                         |
| genshi_xml              | 64.7 ms                                                      | 54.1 ms: 1.20x faster                                        |
| scimark_sparse_mat_mult | 5.19 ms                                                      | 4.35 ms: 1.20x faster                                        |
| docutils                | 3.40 sec                                                     | 2.87 sec: 1.19x faster                                       |
| sqlglot_normalize       | 144 ms                                                       | 122 ms: 1.18x faster                                         |
| json                    | 5.96 ms                                                      | 5.05 ms: 1.18x faster                                        |
| mdp                     | 3.03 sec                                                     | 2.57 sec: 1.18x faster                                       |
| sqlglot_optimize        | 70.3 ms                                                      | 59.6 ms: 1.18x faster                                        |
| bench_thread_pool       | 1.14 ms                                                      | 964 us: 1.18x faster                                         |
| pathlib                 | 21.7 ms                                                      | 18.4 ms: 1.18x faster                                        |
| deepcopy                | 454 us                                                       | 388 us: 1.17x faster                                         |
| deepcopy_reduce         | 4.03 us                                                      | 3.45 us: 1.17x faster                                        |
| sympy_expand            | 599 ms                                                       | 524 ms: 1.14x faster                                         |
| pylint                  | 562 ms                                                       | 491 ms: 1.14x faster                                         |
| unpack_sequence         | 59.5 ns                                                      | 52.6 ns: 1.13x faster                                        |
| create_gc_cycles        | 1.82 ms                                                      | 1.61 ms: 1.13x faster                                        |
| sqlite_synth            | 2.97 us                                                      | 2.65 us: 1.12x faster                                        |
| xml_etree_generate      | 94.6 ms                                                      | 84.5 ms: 1.12x faster                                        |
| regex_dna               | 259 ms                                                       | 232 ms: 1.12x faster                                         |
| xml_etree_parse         | 162 ms                                                       | 145 ms: 1.12x faster                                         |
| sympy_integrate         | 28.1 ms                                                      | 25.3 ms: 1.11x faster                                        |
| regex_v8                | 26.6 ms                                                      | 24.1 ms: 1.11x faster                                        |
| sympy_str               | 358 ms                                                       | 324 ms: 1.11x faster                                         |
| async_generators        | 422 ms                                                       | 386 ms: 1.09x faster                                         |
| comprehensions          | 26.9 us                                                      | 25.0 us: 1.08x faster                                        |
| xml_etree_iterparse     | 110 ms                                                       | 104 ms: 1.06x faster                                         |
| sympy_sum               | 193 ms                                                       | 182 ms: 1.06x faster                                         |
| pidigits                | 271 ms                                                       | 260 ms: 1.04x faster                                         |
| pickle_list             | 4.11 us                                                      | 3.96 us: 1.04x faster                                        |
| python_startup          | 11.5 ms                                                      | 11.1 ms: 1.03x faster                                        |
| telco                   | 7.14 ms                                                      | 7.05 ms: 1.01x faster                                        |
| meteor_contest          | 137 ms                                                       | 137 ms: 1.00x slower                                         |
| pickle                  | 9.94 us                                                      | 10.2 us: 1.02x slower                                        |
| pickle_dict             | 30.0 us                                                      | 30.9 us: 1.03x slower                                        |
| gc_traversal            | 3.45 ms                                                      | 3.56 ms: 1.03x slower                                        |
| unpickle_list           | 4.49 us                                                      | 4.72 us: 1.05x slower                                        |
| python_startup_no_site  | 7.32 ms                                                      | 8.33 ms: 1.14x slower                                        |
| regex_effbot            | 2.99 ms                                                      | 3.53 ms: 1.18x slower                                        |
| dask                    | 463 ms                                                       | 560 ms: 1.21x slower                                         |
| coverage                | 64.0 ms                                                      | 90.9 ms: 1.42x slower                                        |
| Geometric mean          | (ref)                                                        | 1.26x faster                                                 |

Benchmark hidden because not significant (2): unpickle, bench_mp_pool
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf2-x86_64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, gunicorn, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.23x
- 95% likely to have a speedup of 1.21x
- 99% likely to have a speedup of 1.19x
