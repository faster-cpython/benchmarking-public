# Results vs. 3.10.4

- fork: brandtbucher
- ref: justin_no_externs
- machine: linux-aarch64
- commit hash: 5791853
- commit date: 2024-10-25
- overall geometric mean: 1.160x faster
- HPT reliability: 99.98%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.36x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 385 ms: 1.01x slower                                                        |
| docutils       | 3.53 sec                                                              | 3.66 sec: 1.04x slower                                                      |
| html5lib       | 86.5 ms                                                               | 71.5 ms: 1.21x faster                                                       |
| tornado_http   | 178 ms                                                                | 148 ms: 1.20x faster                                                        |
| Geometric mean | (ref)                                                                 | 1.09x faster                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|-------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 465 ms: 2.04x faster                                                        |
| async_tree_io           | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                      |
| async_tree_memoization  | 1.13 sec                                                              | 595 ms: 1.90x faster                                                        |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 761 ms: 1.67x faster                                                        |
| Geometric mean          | (ref)                                                                 | 1.89x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 135 ms                                                                | 96.2 ms: 1.40x faster                                                       |
| nbody          | 166 ms                                                                | 119 ms: 1.39x faster                                                        |
| Geometric mean | (ref)                                                                 | 1.25x faster                                                                |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_v8       | 32.2 ms                                                               | 30.9 ms: 1.04x faster                                                       |
| regex_dna      | 257 ms                                                                | 251 ms: 1.03x faster                                                        |
| regex_effbot   | 4.25 ms                                                               | 4.94 ms: 1.16x slower                                                       |
| Geometric mean | (ref)                                                                 | 1.02x slower                                                                |

Benchmark hidden because not significant (1): regex_compile

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pickle_pure_python   | 529 us                                                                | 387 us: 1.37x faster                                                        |
| unpickle_pure_python | 366 us                                                                | 268 us: 1.37x faster                                                        |
| tomli_loads          | 3.36 sec                                                              | 2.67 sec: 1.26x faster                                                      |
| xml_etree_process    | 99.5 ms                                                               | 80.9 ms: 1.23x faster                                                       |
| json_dumps           | 16.7 ms                                                               | 14.0 ms: 1.19x faster                                                       |
| xml_etree_parse      | 212 ms                                                                | 186 ms: 1.14x faster                                                        |
| xml_etree_generate   | 123 ms                                                                | 111 ms: 1.10x faster                                                        |
| xml_etree_iterparse  | 156 ms                                                                | 148 ms: 1.05x faster                                                        |
| json_loads           | 30.9 us                                                               | 31.7 us: 1.03x slower                                                       |
| Geometric mean       | (ref)                                                                 | 1.18x faster                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup_no_site | 6.89 ms                                                               | 8.77 ms: 1.27x slower                                                       |
| python_startup         | 11.2 ms                                                               | 14.6 ms: 1.30x slower                                                       |
| Geometric mean         | (ref)                                                                 | 1.29x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|-----------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.7 ms: 1.39x faster                                                       |
| django_template | 53.3 ms                                                               | 52.3 ms: 1.02x faster                                                       |
| genshi_text     | 35.2 ms                                                               | 34.6 ms: 1.02x faster                                                       |
| genshi_xml      | 69.8 ms                                                               | 84.5 ms: 1.21x slower                                                       |
| Geometric mean  | (ref)                                                                 | 1.04x faster                                                                |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|--------------------------|:---------------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 219 us: 3.02x faster                                                        |
| async_tree_none          | 950 ms                                                                | 465 ms: 2.04x faster                                                        |
| deltablue                | 8.94 ms                                                               | 4.45 ms: 2.01x faster                                                       |
| async_tree_io            | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                      |
| async_tree_memoization   | 1.13 sec                                                              | 595 ms: 1.90x faster                                                        |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 761 ms: 1.67x faster                                                        |
| raytrace                 | 573 ms                                                                | 351 ms: 1.64x faster                                                        |
| deepcopy_memo            | 61.7 us                                                               | 39.0 us: 1.58x faster                                                       |
| logging_silent           | 222 ns                                                                | 147 ns: 1.51x faster                                                        |
| scimark_lu               | 227 ms                                                                | 151 ms: 1.51x faster                                                        |
| scimark_sor              | 246 ms                                                                | 164 ms: 1.51x faster                                                        |
| richards_super           | 107 ms                                                                | 71.8 ms: 1.49x faster                                                       |
| crypto_pyaes             | 134 ms                                                                | 92.0 ms: 1.46x faster                                                       |
| go                       | 264 ms                                                                | 185 ms: 1.43x faster                                                        |
| scimark_monte_carlo      | 128 ms                                                                | 90.7 ms: 1.41x faster                                                       |
| float                    | 135 ms                                                                | 96.2 ms: 1.40x faster                                                       |
| nbody                    | 166 ms                                                                | 119 ms: 1.39x faster                                                        |
| sqlglot_parse            | 2.40 ms                                                               | 1.73 ms: 1.39x faster                                                       |
| mako                     | 18.9 ms                                                               | 13.7 ms: 1.39x faster                                                       |
| chaos                    | 121 ms                                                                | 88.3 ms: 1.37x faster                                                       |
| pickle_pure_python       | 529 us                                                                | 387 us: 1.37x faster                                                        |
| unpickle_pure_python     | 366 us                                                                | 268 us: 1.37x faster                                                        |
| deepcopy                 | 511 us                                                                | 378 us: 1.35x faster                                                        |
| richards                 | 91.7 ms                                                               | 68.5 ms: 1.34x faster                                                       |
| thrift                   | 1.26 ms                                                               | 963 us: 1.31x faster                                                        |
| logging_simple           | 9.78 us                                                               | 7.54 us: 1.30x faster                                                       |
| comprehensions           | 33.1 us                                                               | 25.6 us: 1.30x faster                                                       |
| coroutines               | 37.2 ms                                                               | 28.7 ms: 1.29x faster                                                       |
| logging_format           | 10.6 us                                                               | 8.26 us: 1.28x faster                                                       |
| sqlglot_transpile        | 2.84 ms                                                               | 2.21 ms: 1.28x faster                                                       |
| tomli_loads              | 3.36 sec                                                              | 2.67 sec: 1.26x faster                                                      |
| pathlib                  | 26.3 ms                                                               | 21.3 ms: 1.23x faster                                                       |
| pyflate                  | 795 ms                                                                | 645 ms: 1.23x faster                                                        |
| xml_etree_process        | 99.5 ms                                                               | 80.9 ms: 1.23x faster                                                       |
| html5lib                 | 86.5 ms                                                               | 71.5 ms: 1.21x faster                                                       |
| tornado_http             | 178 ms                                                                | 148 ms: 1.20x faster                                                        |
| json_dumps               | 16.7 ms                                                               | 14.0 ms: 1.19x faster                                                       |
| spectral_norm            | 186 ms                                                                | 156 ms: 1.19x faster                                                        |
| generators               | 68.1 ms                                                               | 58.3 ms: 1.17x faster                                                       |
| bench_thread_pool        | 1.59 ms                                                               | 1.37 ms: 1.16x faster                                                       |
| deepcopy_reduce          | 4.60 us                                                               | 4.01 us: 1.15x faster                                                       |
| xml_etree_parse          | 212 ms                                                                | 186 ms: 1.14x faster                                                        |
| pycparser                | 1.69 sec                                                              | 1.51 sec: 1.12x faster                                                      |
| scimark_fft              | 500 ms                                                                | 450 ms: 1.11x faster                                                        |
| pylint                   | 485 ms                                                                | 437 ms: 1.11x faster                                                        |
| xml_etree_generate       | 123 ms                                                                | 111 ms: 1.10x faster                                                        |
| sqlalchemy_imperative    | 20.5 ms                                                               | 19.2 ms: 1.07x faster                                                       |
| fannkuch                 | 546 ms                                                                | 510 ms: 1.07x faster                                                        |
| mdp                      | 3.70 sec                                                              | 3.49 sec: 1.06x faster                                                      |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 7.20 ms: 1.06x faster                                                       |
| xml_etree_iterparse      | 156 ms                                                                | 148 ms: 1.05x faster                                                        |
| sqlalchemy_declarative   | 197 ms                                                                | 188 ms: 1.05x faster                                                        |
| json                     | 5.88 ms                                                               | 5.61 ms: 1.05x faster                                                       |
| regex_v8                 | 32.2 ms                                                               | 30.9 ms: 1.04x faster                                                       |
| hexiom                   | 10.9 ms                                                               | 10.5 ms: 1.04x faster                                                       |
| regex_dna                | 257 ms                                                                | 251 ms: 1.03x faster                                                        |
| django_template          | 53.3 ms                                                               | 52.3 ms: 1.02x faster                                                       |
| genshi_text              | 35.2 ms                                                               | 34.6 ms: 1.02x faster                                                       |
| meteor_contest           | 126 ms                                                                | 125 ms: 1.01x faster                                                        |
| 2to3                     | 381 ms                                                                | 385 ms: 1.01x slower                                                        |
| asyncio_websockets       | 657 ms                                                                | 664 ms: 1.01x slower                                                        |
| json_loads               | 30.9 us                                                               | 31.7 us: 1.03x slower                                                       |
| docutils                 | 3.53 sec                                                              | 3.66 sec: 1.04x slower                                                      |
| dulwich_log              | 73.5 ms                                                               | 78.8 ms: 1.07x slower                                                       |
| pprint_safe_repr         | 1.15 sec                                                              | 1.23 sec: 1.07x slower                                                      |
| sqlglot_optimize         | 75.4 ms                                                               | 81.4 ms: 1.08x slower                                                       |
| sympy_expand             | 543 ms                                                                | 590 ms: 1.09x slower                                                        |
| sympy_str                | 329 ms                                                                | 358 ms: 1.09x slower                                                        |
| sympy_integrate          | 26.5 ms                                                               | 29.3 ms: 1.10x slower                                                       |
| pprint_pformat           | 2.36 sec                                                              | 2.61 sec: 1.10x slower                                                      |
| nqueens                  | 117 ms                                                                | 132 ms: 1.13x slower                                                        |
| async_generators         | 452 ms                                                                | 510 ms: 1.13x slower                                                        |
| telco                    | 8.49 ms                                                               | 9.67 ms: 1.14x slower                                                       |
| regex_effbot             | 4.25 ms                                                               | 4.94 ms: 1.16x slower                                                       |
| sympy_sum                | 184 ms                                                                | 216 ms: 1.17x slower                                                        |
| coverage                 | 83.6 ms                                                               | 98.5 ms: 1.18x slower                                                       |
| genshi_xml               | 69.8 ms                                                               | 84.5 ms: 1.21x slower                                                       |
| python_startup_no_site   | 6.89 ms                                                               | 8.77 ms: 1.27x slower                                                       |
| python_startup           | 11.2 ms                                                               | 14.6 ms: 1.30x slower                                                       |
| gc_traversal             | 4.15 ms                                                               | 6.26 ms: 1.51x slower                                                       |
| create_gc_cycles         | 2.26 ms                                                               | 3.65 ms: 1.62x slower                                                       |
| bench_mp_pool            | 14.5 ms                                                               | 1.27 sec: 87.62x slower                                                     |
| Geometric mean           | (ref)                                                                 | 1.10x faster                                                                |

Benchmark hidden because not significant (3): regex_compile, pidigits, sqlglot_normalize
Ignored benchmarks (14) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20241025-3.14.0a1+-5791853-JIT/bm-20241025-arminc-aarch64-brandtbucher-justin_no_externs-3.14.0a1+-5791853.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, sphinx

- Geometric mean (including insignificant results): 1.160x faster
# HPT report

- Reliability score: 99.98% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.36x