
# Results vs. 3.11.0

- fork: python
- ref: main
- machine: windows-amd64
- commit hash: 5113ed7
- commit date: 2023-07-29
- overall geometric mean: 1.00x slower
- HPT reliability: 93.62%
- HPT 99th percentile: 1.00x slower

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|----------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| docutils       | 1.60 sec                                                    | 1.70 sec: 1.06x slower                                     |
| tornado_http   | 91.8 ms                                                     | 91.0 ms: 1.01x faster                                      |
| Geometric mean | (ref)                                                       | 1.03x slower                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|----------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| pidigits       | 148 ms                                                      | 147 ms: 1.01x faster                                       |
| float          | 54.6 ms                                                     | 55.9 ms: 1.02x slower                                      |
| nbody          | 70.0 ms                                                     | 76.7 ms: 1.10x slower                                      |
| Geometric mean | (ref)                                                       | 1.04x slower                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|----------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| regex_v8       | 13.8 ms                                                     | 13.6 ms: 1.02x faster                                      |
| regex_compile  | 90.6 ms                                                     | 94.2 ms: 1.04x slower                                      |
| regex_dna      | 115 ms                                                      | 122 ms: 1.05x slower                                       |
| regex_effbot   | 1.50 ms                                                     | 1.62 ms: 1.08x slower                                      |
| Geometric mean | (ref)                                                       | 1.04x slower                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|----------------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| json_dumps           | 7.56 ms                                                     | 5.77 ms: 1.31x faster                                      |
| unpickle_pure_python | 152 us                                                      | 146 us: 1.04x faster                                       |
| xml_etree_parse      | 95.9 ms                                                     | 93.5 ms: 1.03x faster                                      |
| pickle_pure_python   | 200 us                                                      | 198 us: 1.01x faster                                       |
| pickle_dict          | 18.5 us                                                     | 18.9 us: 1.02x slower                                      |
| unpickle             | 8.09 us                                                     | 8.38 us: 1.04x slower                                      |
| xml_etree_iterparse  | 62.6 ms                                                     | 65.6 ms: 1.05x slower                                      |
| xml_etree_process    | 37.1 ms                                                     | 39.2 ms: 1.06x slower                                      |
| pickle_list          | 2.68 us                                                     | 2.87 us: 1.07x slower                                      |
| json_loads           | 12.9 us                                                     | 13.9 us: 1.08x slower                                      |
| xml_etree_generate   | 52.2 ms                                                     | 57.4 ms: 1.10x slower                                      |
| pickle               | 6.61 us                                                     | 7.33 us: 1.11x slower                                      |
| unpickle_list        | 2.55 us                                                     | 2.87 us: 1.13x slower                                      |
| tomli_loads          | 1.41 sec                                                    | 1.63 sec: 1.15x slower                                     |
| Geometric mean       | (ref)                                                       | 1.03x slower                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|------------------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| python_startup         | 18.7 ms                                                     | 19.4 ms: 1.04x slower                                      |
| python_startup_no_site | 15.9 ms                                                     | 16.6 ms: 1.04x slower                                      |
| Geometric mean         | (ref)                                                       | 1.04x slower                                               |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|-----------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| mako      | 7.26 ms                                                     | 7.56 ms: 1.04x slower                                      |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509 | bm-20230729-pythonperf1-amd64-python-main-3.13.0a0-5113ed7 |
|--------------------------|:-----------------------------------------------------------:|:----------------------------------------------------------:|
| typing_runtime_protocols | 322 us                                                      | 95.6 us: 3.37x faster                                      |
| asyncio_tcp              | 699 ms                                                      | 502 ms: 1.39x faster                                       |
| generators               | 33.8 ms                                                     | 25.2 ms: 1.34x faster                                      |
| json_dumps               | 7.56 ms                                                     | 5.77 ms: 1.31x faster                                      |
| unpack_sequence          | 46.1 ns                                                     | 38.3 ns: 1.20x faster                                      |
| mdp                      | 1.67 sec                                                    | 1.47 sec: 1.14x faster                                     |
| raytrace                 | 211 ms                                                      | 187 ms: 1.13x faster                                       |
| deltablue                | 2.61 ms                                                     | 2.33 ms: 1.12x faster                                      |
| json                     | 3.25 ms                                                     | 2.95 ms: 1.10x faster                                      |
| richards_super           | 37.5 ms                                                     | 34.3 ms: 1.09x faster                                      |
| sqlglot_parse            | 952 us                                                      | 880 us: 1.08x faster                                       |
| coverage                 | 55.9 ms                                                     | 52.0 ms: 1.07x faster                                      |
| logging_silent           | 69.8 ns                                                     | 65.2 ns: 1.07x faster                                      |
| chaos                    | 47.1 ms                                                     | 44.2 ms: 1.07x faster                                      |
| asyncio_tcp_ssl          | 2.11 sec                                                    | 2.01 sec: 1.05x faster                                     |
| comprehensions           | 15.9 us                                                     | 15.2 us: 1.05x faster                                      |
| sqlglot_transpile        | 1.16 ms                                                     | 1.11 ms: 1.05x faster                                      |
| scimark_lu               | 63.5 ms                                                     | 60.8 ms: 1.04x faster                                      |
| unpickle_pure_python     | 152 us                                                      | 146 us: 1.04x faster                                       |
| async_tree_none          | 320 ms                                                      | 311 ms: 1.03x faster                                       |
| mypy2                    | 229 ms                                                      | 222 ms: 1.03x faster                                       |
| spectral_norm            | 67.9 ms                                                     | 66.2 ms: 1.03x faster                                      |
| xml_etree_parse          | 95.9 ms                                                     | 93.5 ms: 1.03x faster                                      |
| crypto_pyaes             | 47.6 ms                                                     | 46.5 ms: 1.02x faster                                      |
| async_tree_memoization   | 371 ms                                                      | 363 ms: 1.02x faster                                       |
| regex_v8                 | 13.8 ms                                                     | 13.6 ms: 1.02x faster                                      |
| async_tree_io            | 779 ms                                                      | 764 ms: 1.02x faster                                       |
| hexiom                   | 4.55 ms                                                     | 4.47 ms: 1.02x faster                                      |
| scimark_monte_carlo      | 44.6 ms                                                     | 43.8 ms: 1.02x faster                                      |
| bench_thread_pool        | 852 us                                                      | 839 us: 1.02x faster                                       |
| pickle_pure_python       | 200 us                                                      | 198 us: 1.01x faster                                       |
| scimark_sparse_mat_mult  | 2.57 ms                                                     | 2.54 ms: 1.01x faster                                      |
| tornado_http             | 91.8 ms                                                     | 91.0 ms: 1.01x faster                                      |
| pidigits                 | 148 ms                                                      | 147 ms: 1.01x faster                                       |
| deepcopy                 | 246 us                                                      | 244 us: 1.01x faster                                       |
| go                       | 97.3 ms                                                     | 98.4 ms: 1.01x slower                                      |
| pickle_dict              | 18.5 us                                                     | 18.9 us: 1.02x slower                                      |
| float                    | 54.6 ms                                                     | 55.9 ms: 1.02x slower                                      |
| meteor_contest           | 74.7 ms                                                     | 77.2 ms: 1.03x slower                                      |
| unpickle                 | 8.09 us                                                     | 8.38 us: 1.04x slower                                      |
| python_startup           | 18.7 ms                                                     | 19.4 ms: 1.04x slower                                      |
| sqlglot_normalize        | 190 ms                                                      | 198 ms: 1.04x slower                                       |
| regex_compile            | 90.6 ms                                                     | 94.2 ms: 1.04x slower                                      |
| fannkuch                 | 252 ms                                                      | 262 ms: 1.04x slower                                       |
| mako                     | 7.26 ms                                                     | 7.56 ms: 1.04x slower                                      |
| python_startup_no_site   | 15.9 ms                                                     | 16.6 ms: 1.04x slower                                      |
| coroutines               | 14.6 ms                                                     | 15.3 ms: 1.05x slower                                      |
| gc_traversal             | 1.46 ms                                                     | 1.53 ms: 1.05x slower                                      |
| xml_etree_iterparse      | 62.6 ms                                                     | 65.6 ms: 1.05x slower                                      |
| sqlglot_optimize         | 34.9 ms                                                     | 36.7 ms: 1.05x slower                                      |
| regex_dna                | 115 ms                                                      | 122 ms: 1.05x slower                                       |
| dulwich_log              | 44.5 ms                                                     | 47.0 ms: 1.06x slower                                      |
| xml_etree_process        | 37.1 ms                                                     | 39.2 ms: 1.06x slower                                      |
| docutils                 | 1.60 sec                                                    | 1.70 sec: 1.06x slower                                     |
| logging_simple           | 6.61 us                                                     | 7.05 us: 1.07x slower                                      |
| deepcopy_reduce          | 2.07 us                                                     | 2.21 us: 1.07x slower                                      |
| pyflate                  | 304 ms                                                      | 325 ms: 1.07x slower                                       |
| sqlite_synth             | 1.68 us                                                     | 1.80 us: 1.07x slower                                      |
| pickle_list              | 2.68 us                                                     | 2.87 us: 1.07x slower                                      |
| logging_format           | 7.01 us                                                     | 7.52 us: 1.07x slower                                      |
| scimark_fft              | 178 ms                                                      | 192 ms: 1.08x slower                                       |
| json_loads               | 12.9 us                                                     | 13.9 us: 1.08x slower                                      |
| pprint_pformat           | 1.04 sec                                                    | 1.12 sec: 1.08x slower                                     |
| regex_effbot             | 1.50 ms                                                     | 1.62 ms: 1.08x slower                                      |
| bench_mp_pool            | 62.5 ms                                                     | 67.9 ms: 1.09x slower                                      |
| pprint_safe_repr         | 512 ms                                                      | 556 ms: 1.09x slower                                       |
| create_gc_cycles         | 693 us                                                      | 754 us: 1.09x slower                                       |
| nbody                    | 70.0 ms                                                     | 76.7 ms: 1.10x slower                                      |
| xml_etree_generate       | 52.2 ms                                                     | 57.4 ms: 1.10x slower                                      |
| pickle                   | 6.61 us                                                     | 7.33 us: 1.11x slower                                      |
| pycparser                | 691 ms                                                      | 775 ms: 1.12x slower                                       |
| unpickle_list            | 2.55 us                                                     | 2.87 us: 1.13x slower                                      |
| scimark_sor              | 75.6 ms                                                     | 86.2 ms: 1.14x slower                                      |
| tomli_loads              | 1.41 sec                                                    | 1.63 sec: 1.15x slower                                     |
| pathlib                  | 71.4 ms                                                     | 84.0 ms: 1.18x slower                                      |
| telco                    | 3.90 ms                                                     | 5.02 ms: 1.28x slower                                      |
| async_generators         | 178 ms                                                      | 246 ms: 1.39x slower                                       |
| dask                     | 264 ms                                                      | 389 ms: 1.47x slower                                       |
| Geometric mean           | (ref)                                                       | 1.00x slower                                               |

Benchmark hidden because not significant (4): async_tree_cpu_io_mixed, deepcopy_memo, richards, nqueens
Ignored benchmarks (16) of results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf1-amd64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift


# HPT report

- Reliability score: 93.62% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x
