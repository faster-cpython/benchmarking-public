# Results vs. 3.13.0

- fork: brandtbucher
- ref: faster_jit_builds
- machine: linux-x86_64
- commit hash: 60b7e71
- commit date: 2024-07-27
- overall geometric mean: 1.01x faster
- HPT reliability: 79.15%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.08x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                       | 303 ms: 1.04x slower                                                           |
| docutils       | 2.81 sec                                                     | 3.12 sec: 1.11x slower                                                         |
| html5lib       | 71.9 ms                                                      | 69.9 ms: 1.03x faster                                                          |
| Geometric mean | (ref)                                                        | 1.03x slower                                                                   |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 461 ms                                                       | 379 ms: 1.22x faster                                                           |
| async_tree_memoization     | 452 ms                                                       | 400 ms: 1.13x faster                                                           |
| async_tree_none_tg         | 340 ms                                                       | 302 ms: 1.13x faster                                                           |
| async_tree_none            | 372 ms                                                       | 333 ms: 1.12x faster                                                           |
| async_tree_io_tg           | 819 ms                                                       | 764 ms: 1.07x faster                                                           |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 536 ms: 1.07x faster                                                           |
| async_tree_io              | 847 ms                                                       | 795 ms: 1.07x faster                                                           |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 571 ms: 1.05x faster                                                           |
| coroutines                 | 21.6 ms                                                      | 21.3 ms: 1.01x faster                                                          |
| asyncio_tcp                | 380 ms                                                       | 377 ms: 1.01x faster                                                           |
| asyncio_websockets         | 382 ms                                                       | 390 ms: 1.02x slower                                                           |
| async_generators           | 359 ms                                                       | 388 ms: 1.08x slower                                                           |
| Geometric mean             | (ref)                                                        | 1.06x faster                                                                   |

Benchmark hidden because not significant (1): asyncio_tcp_ssl

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| float          | 81.9 ms                                                      | 74.6 ms: 1.10x faster                                                          |
| nbody          | 88.0 ms                                                      | 81.5 ms: 1.08x faster                                                          |
| pidigits       | 253 ms                                                       | 251 ms: 1.01x faster                                                           |
| Geometric mean | (ref)                                                        | 1.06x faster                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_compile  | 144 ms                                                       | 137 ms: 1.05x faster                                                           |
| regex_dna      | 244 ms                                                       | 236 ms: 1.03x faster                                                           |
| regex_v8       | 26.2 ms                                                      | 26.5 ms: 1.01x slower                                                          |
| regex_effbot   | 3.37 ms                                                      | 3.52 ms: 1.04x slower                                                          |
| Geometric mean | (ref)                                                        | 1.01x faster                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| tomli_loads          | 2.41 sec                                                     | 2.09 sec: 1.15x faster                                                         |
| xml_etree_generate   | 85.3 ms                                                      | 81.4 ms: 1.05x faster                                                          |
| xml_etree_parse      | 145 ms                                                       | 140 ms: 1.04x faster                                                           |
| xml_etree_process    | 60.9 ms                                                      | 58.8 ms: 1.03x faster                                                          |
| xml_etree_iterparse  | 100 ms                                                       | 96.7 ms: 1.03x faster                                                          |
| json_dumps           | 11.0 ms                                                      | 10.8 ms: 1.02x faster                                                          |
| unpickle_pure_python | 214 us                                                       | 213 us: 1.01x faster                                                           |
| json_loads           | 24.0 us                                                      | 25.5 us: 1.06x slower                                                          |
| Geometric mean       | (ref)                                                        | 1.03x faster                                                                   |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|------------------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup         | 13.3 ms                                                      | 13.4 ms: 1.01x slower                                                          |
| python_startup_no_site | 8.94 ms                                                      | 9.08 ms: 1.02x slower                                                          |
| Geometric mean         | (ref)                                                        | 1.01x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|-----------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| mako            | 10.4 ms                                                      | 9.26 ms: 1.12x faster                                                          |
| django_template | 38.9 ms                                                      | 41.2 ms: 1.06x slower                                                          |
| genshi_text     | 26.6 ms                                                      | 30.3 ms: 1.14x slower                                                          |
| genshi_xml      | 57.3 ms                                                      | 68.9 ms: 1.20x slower                                                          |
| Geometric mean  | (ref)                                                        | 1.07x slower                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20240727-pythonperf2-x86_64-brandtbucher-faster_jit_builds-3.14.0a0-60b7e71 |
|----------------------------|:------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| deepcopy_memo              | 39.5 us                                                      | 28.4 us: 1.39x faster                                                          |
| deepcopy                   | 397 us                                                       | 302 us: 1.32x faster                                                           |
| async_tree_memoization_tg  | 461 ms                                                       | 379 ms: 1.22x faster                                                           |
| spectral_norm              | 97.4 ms                                                      | 81.0 ms: 1.20x faster                                                          |
| deepcopy_reduce            | 3.54 us                                                      | 3.00 us: 1.18x faster                                                          |
| richards                   | 52.7 ms                                                      | 45.1 ms: 1.17x faster                                                          |
| richards_super             | 59.8 ms                                                      | 51.2 ms: 1.17x faster                                                          |
| tomli_loads                | 2.41 sec                                                     | 2.09 sec: 1.15x faster                                                         |
| async_tree_memoization     | 452 ms                                                       | 400 ms: 1.13x faster                                                           |
| async_tree_none_tg         | 340 ms                                                       | 302 ms: 1.13x faster                                                           |
| pyflate                    | 492 ms                                                       | 437 ms: 1.13x faster                                                           |
| mako                       | 10.4 ms                                                      | 9.26 ms: 1.12x faster                                                          |
| async_tree_none            | 372 ms                                                       | 333 ms: 1.12x faster                                                           |
| float                      | 81.9 ms                                                      | 74.6 ms: 1.10x faster                                                          |
| pathlib                    | 17.4 ms                                                      | 16.1 ms: 1.08x faster                                                          |
| telco                      | 8.58 ms                                                      | 7.92 ms: 1.08x faster                                                          |
| nbody                      | 88.0 ms                                                      | 81.5 ms: 1.08x faster                                                          |
| scimark_fft                | 314 ms                                                       | 292 ms: 1.07x faster                                                           |
| async_tree_io_tg           | 819 ms                                                       | 764 ms: 1.07x faster                                                           |
| async_tree_cpu_io_mixed_tg | 574 ms                                                       | 536 ms: 1.07x faster                                                           |
| async_tree_io              | 847 ms                                                       | 795 ms: 1.07x faster                                                           |
| regex_compile              | 144 ms                                                       | 137 ms: 1.05x faster                                                           |
| bpe_tokeniser              | 5.10 sec                                                     | 4.84 sec: 1.05x faster                                                         |
| async_tree_cpu_io_mixed    | 600 ms                                                       | 571 ms: 1.05x faster                                                           |
| xml_etree_generate         | 85.3 ms                                                      | 81.4 ms: 1.05x faster                                                          |
| crypto_pyaes               | 72.8 ms                                                      | 69.7 ms: 1.04x faster                                                          |
| xml_etree_parse            | 145 ms                                                       | 140 ms: 1.04x faster                                                           |
| xml_etree_process          | 60.9 ms                                                      | 58.8 ms: 1.03x faster                                                          |
| xml_etree_iterparse        | 100 ms                                                       | 96.7 ms: 1.03x faster                                                          |
| regex_dna                  | 244 ms                                                       | 236 ms: 1.03x faster                                                           |
| pycparser                  | 1.26 sec                                                     | 1.22 sec: 1.03x faster                                                         |
| html5lib                   | 71.9 ms                                                      | 69.9 ms: 1.03x faster                                                          |
| json_dumps                 | 11.0 ms                                                      | 10.8 ms: 1.02x faster                                                          |
| scimark_sparse_mat_mult    | 4.29 ms                                                      | 4.22 ms: 1.02x faster                                                          |
| coroutines                 | 21.6 ms                                                      | 21.3 ms: 1.01x faster                                                          |
| logging_format             | 7.07 us                                                      | 7.01 us: 1.01x faster                                                          |
| asyncio_tcp                | 380 ms                                                       | 377 ms: 1.01x faster                                                           |
| pidigits                   | 253 ms                                                       | 251 ms: 1.01x faster                                                           |
| meteor_contest             | 128 ms                                                       | 127 ms: 1.01x faster                                                           |
| coverage                   | 81.1 ms                                                      | 80.7 ms: 1.01x faster                                                          |
| unpickle_pure_python       | 214 us                                                       | 213 us: 1.01x faster                                                           |
| python_startup             | 13.3 ms                                                      | 13.4 ms: 1.01x slower                                                          |
| thrift                     | 877 us                                                       | 882 us: 1.01x slower                                                           |
| go                         | 160 ms                                                       | 162 ms: 1.01x slower                                                           |
| regex_v8                   | 26.2 ms                                                      | 26.5 ms: 1.01x slower                                                          |
| pprint_pformat             | 1.65 sec                                                     | 1.67 sec: 1.01x slower                                                         |
| python_startup_no_site     | 8.94 ms                                                      | 9.08 ms: 1.02x slower                                                          |
| raytrace                   | 289 ms                                                       | 294 ms: 1.02x slower                                                           |
| asyncio_websockets         | 382 ms                                                       | 390 ms: 1.02x slower                                                           |
| sqlglot_parse              | 1.38 ms                                                      | 1.41 ms: 1.02x slower                                                          |
| generators                 | 33.5 ms                                                      | 34.2 ms: 1.02x slower                                                          |
| scimark_sor                | 126 ms                                                       | 128 ms: 1.02x slower                                                           |
| dask                       | 379 ms                                                       | 389 ms: 1.03x slower                                                           |
| sqlglot_transpile          | 1.78 ms                                                      | 1.83 ms: 1.03x slower                                                          |
| mdp                        | 2.52 sec                                                     | 2.60 sec: 1.03x slower                                                         |
| logging_silent             | 97.7 ns                                                      | 101 ns: 1.03x slower                                                           |
| 2to3                       | 291 ms                                                       | 303 ms: 1.04x slower                                                           |
| regex_effbot               | 3.37 ms                                                      | 3.52 ms: 1.04x slower                                                          |
| hexiom                     | 6.33 ms                                                      | 6.61 ms: 1.04x slower                                                          |
| sympy_expand               | 505 ms                                                       | 528 ms: 1.05x slower                                                           |
| json                       | 5.22 ms                                                      | 5.48 ms: 1.05x slower                                                          |
| sympy_str                  | 296 ms                                                       | 311 ms: 1.05x slower                                                           |
| deltablue                  | 3.41 ms                                                      | 3.59 ms: 1.05x slower                                                          |
| gc_traversal               | 4.11 ms                                                      | 4.34 ms: 1.06x slower                                                          |
| django_template            | 38.9 ms                                                      | 41.2 ms: 1.06x slower                                                          |
| json_loads                 | 24.0 us                                                      | 25.5 us: 1.06x slower                                                          |
| typing_runtime_protocols   | 174 us                                                       | 185 us: 1.07x slower                                                           |
| comprehensions             | 17.3 us                                                      | 18.4 us: 1.07x slower                                                          |
| sqlglot_optimize           | 59.7 ms                                                      | 63.6 ms: 1.07x slower                                                          |
| chaos                      | 61.7 ms                                                      | 66.5 ms: 1.08x slower                                                          |
| async_generators           | 359 ms                                                       | 388 ms: 1.08x slower                                                           |
| nqueens                    | 88.2 ms                                                      | 95.5 ms: 1.08x slower                                                          |
| sympy_sum                  | 154 ms                                                       | 167 ms: 1.09x slower                                                           |
| create_gc_cycles           | 1.76 ms                                                      | 1.92 ms: 1.09x slower                                                          |
| sympy_integrate            | 23.3 ms                                                      | 25.7 ms: 1.10x slower                                                          |
| docutils                   | 2.81 sec                                                     | 3.12 sec: 1.11x slower                                                         |
| pylint                     | 346 ms                                                       | 386 ms: 1.11x slower                                                           |
| sqlglot_normalize          | 118 ms                                                       | 133 ms: 1.12x slower                                                           |
| genshi_text                | 26.6 ms                                                      | 30.3 ms: 1.14x slower                                                          |
| scimark_lu                 | 97.8 ms                                                      | 116 ms: 1.18x slower                                                           |
| genshi_xml                 | 57.3 ms                                                      | 68.9 ms: 1.20x slower                                                          |
| Geometric mean             | (ref)                                                        | 1.01x faster                                                                   |

Benchmark hidden because not significant (9): pprint_safe_repr, pickle_pure_python, logging_simple, scimark_monte_carlo, tornado_http, asyncio_tcp_ssl, bench_thread_pool, fannkuch, bench_mp_pool
Ignored benchmarks (13) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

# HPT report

- Reliability score: 79.15% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.08x