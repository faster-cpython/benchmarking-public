# Results vs. 3.10.4

- fork: python
- ref: 54dd77fb8c880d7655ff
- machine: linux-aarch64
- commit hash: 54dd77f
- commit date: 2024-09-24
- overall geometric mean: 1.17x faster
- HPT reliability: 99.97%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.25x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| docutils       | 3.53 sec                                                              | 3.93 sec: 1.11x slower                                                  |
| html5lib       | 86.5 ms                                                               | 71.2 ms: 1.21x faster                                                   |
| tornado_http   | 178 ms                                                                | 146 ms: 1.22x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.07x faster                                                            |

Benchmark hidden because not significant (1): 2to3

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_none         | 950 ms                                                                | 440 ms: 2.16x faster                                                    |
| async_tree_io           | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                  |
| async_tree_memoization  | 1.13 sec                                                              | 588 ms: 1.93x faster                                                    |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 749 ms: 1.70x faster                                                    |
| Geometric mean          | (ref)                                                                 | 1.93x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 135 ms                                                                | 87.4 ms: 1.54x faster                                                   |
| nbody          | 166 ms                                                                | 113 ms: 1.47x faster                                                    |
| Geometric mean | (ref)                                                                 | 1.31x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 32.2 ms                                                               | 30.8 ms: 1.04x faster                                                   |
| regex_compile  | 177 ms                                                                | 194 ms: 1.10x slower                                                    |
| regex_effbot   | 4.25 ms                                                               | 4.96 ms: 1.17x slower                                                   |
| Geometric mean | (ref)                                                                 | 1.05x slower                                                            |

Benchmark hidden because not significant (1): regex_dna

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 265 us: 1.38x faster                                                    |
| pickle_pure_python   | 529 us                                                                | 404 us: 1.31x faster                                                    |
| tomli_loads          | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                                  |
| json_dumps           | 16.7 ms                                                               | 13.4 ms: 1.25x faster                                                   |
| xml_etree_process    | 99.5 ms                                                               | 81.3 ms: 1.22x faster                                                   |
| xml_etree_generate   | 123 ms                                                                | 109 ms: 1.12x faster                                                    |
| xml_etree_parse      | 212 ms                                                                | 190 ms: 1.11x faster                                                    |
| unpickle_list        | 6.99 us                                                               | 6.36 us: 1.10x faster                                                   |
| xml_etree_iterparse  | 156 ms                                                                | 151 ms: 1.03x faster                                                    |
| json_loads           | 30.9 us                                                               | 31.4 us: 1.02x slower                                                   |
| pickle_dict          | 35.1 us                                                               | 38.3 us: 1.09x slower                                                   |
| unpickle             | 17.5 us                                                               | 19.5 us: 1.11x slower                                                   |
| pickle               | 12.5 us                                                               | 13.9 us: 1.12x slower                                                   |
| Geometric mean       | (ref)                                                                 | 1.10x faster                                                            |

Benchmark hidden because not significant (1): pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 13.0 ms: 1.16x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 8.73 ms: 1.27x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.21x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                   |
| django_template | 53.3 ms                                                               | 51.7 ms: 1.03x faster                                                   |
| genshi_xml      | 69.8 ms                                                               | 80.8 ms: 1.16x slower                                                   |
| Geometric mean  | (ref)                                                                 | 1.07x faster                                                            |

Benchmark hidden because not significant (1): genshi_text

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 213 us: 3.11x faster                                                    |
| async_tree_none          | 950 ms                                                                | 440 ms: 2.16x faster                                                    |
| deltablue                | 8.94 ms                                                               | 4.40 ms: 2.03x faster                                                   |
| async_tree_io            | 2.28 sec                                                              | 1.17 sec: 1.96x faster                                                  |
| bench_mp_pool            | 14.5 ms                                                               | 7.53 ms: 1.93x faster                                                   |
| async_tree_memoization   | 1.13 sec                                                              | 588 ms: 1.93x faster                                                    |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 749 ms: 1.70x faster                                                    |
| raytrace                 | 573 ms                                                                | 346 ms: 1.65x faster                                                    |
| logging_silent           | 222 ns                                                                | 135 ns: 1.65x faster                                                    |
| scimark_sor              | 246 ms                                                                | 152 ms: 1.62x faster                                                    |
| deepcopy_memo            | 61.7 us                                                               | 39.8 us: 1.55x faster                                                   |
| float                    | 135 ms                                                                | 87.4 ms: 1.54x faster                                                   |
| scimark_lu               | 227 ms                                                                | 149 ms: 1.52x faster                                                    |
| asyncio_tcp              | 944 ms                                                                | 627 ms: 1.51x faster                                                    |
| crypto_pyaes             | 134 ms                                                                | 89.4 ms: 1.50x faster                                                   |
| nbody                    | 166 ms                                                                | 113 ms: 1.47x faster                                                    |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.25 sec: 1.46x faster                                                  |
| mako                     | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                   |
| richards_super           | 107 ms                                                                | 75.9 ms: 1.41x faster                                                   |
| go                       | 264 ms                                                                | 189 ms: 1.40x faster                                                    |
| sqlglot_parse            | 2.40 ms                                                               | 1.73 ms: 1.39x faster                                                   |
| unpickle_pure_python     | 366 us                                                                | 265 us: 1.38x faster                                                    |
| richards                 | 91.7 ms                                                               | 67.0 ms: 1.37x faster                                                   |
| scimark_monte_carlo      | 128 ms                                                                | 95.3 ms: 1.34x faster                                                   |
| comprehensions           | 33.1 us                                                               | 24.7 us: 1.34x faster                                                   |
| chaos                    | 121 ms                                                                | 92.3 ms: 1.31x faster                                                   |
| pickle_pure_python       | 529 us                                                                | 404 us: 1.31x faster                                                    |
| thrift                   | 1.26 ms                                                               | 966 us: 1.30x faster                                                    |
| logging_simple           | 9.78 us                                                               | 7.53 us: 1.30x faster                                                   |
| coroutines               | 37.2 ms                                                               | 28.7 ms: 1.29x faster                                                   |
| logging_format           | 10.6 us                                                               | 8.21 us: 1.29x faster                                                   |
| sqlglot_transpile        | 2.84 ms                                                               | 2.20 ms: 1.29x faster                                                   |
| tomli_loads              | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                                  |
| spectral_norm            | 186 ms                                                                | 147 ms: 1.27x faster                                                    |
| json_dumps               | 16.7 ms                                                               | 13.4 ms: 1.25x faster                                                   |
| pathlib                  | 26.3 ms                                                               | 21.4 ms: 1.23x faster                                                   |
| xml_etree_process        | 99.5 ms                                                               | 81.3 ms: 1.22x faster                                                   |
| deepcopy                 | 511 us                                                                | 417 us: 1.22x faster                                                    |
| tornado_http             | 178 ms                                                                | 146 ms: 1.22x faster                                                    |
| html5lib                 | 86.5 ms                                                               | 71.2 ms: 1.21x faster                                                   |
| pyflate                  | 795 ms                                                                | 664 ms: 1.20x faster                                                    |
| generators               | 68.1 ms                                                               | 57.0 ms: 1.19x faster                                                   |
| bench_thread_pool        | 1.59 ms                                                               | 1.34 ms: 1.19x faster                                                   |
| deepcopy_reduce          | 4.60 us                                                               | 3.93 us: 1.17x faster                                                   |
| xml_etree_generate       | 123 ms                                                                | 109 ms: 1.12x faster                                                    |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.81 ms: 1.12x faster                                                   |
| xml_etree_parse          | 212 ms                                                                | 190 ms: 1.11x faster                                                    |
| unpickle_list            | 6.99 us                                                               | 6.36 us: 1.10x faster                                                   |
| scimark_fft              | 500 ms                                                                | 458 ms: 1.09x faster                                                    |
| pycparser                | 1.69 sec                                                              | 1.56 sec: 1.08x faster                                                  |
| fannkuch                 | 546 ms                                                                | 505 ms: 1.08x faster                                                    |
| sqlite_synth             | 4.12 us                                                               | 3.82 us: 1.08x faster                                                   |
| hexiom                   | 10.9 ms                                                               | 10.2 ms: 1.07x faster                                                   |
| mdp                      | 3.70 sec                                                              | 3.48 sec: 1.06x faster                                                  |
| regex_v8                 | 32.2 ms                                                               | 30.8 ms: 1.04x faster                                                   |
| sqlglot_normalize        | 156 ms                                                                | 151 ms: 1.03x faster                                                    |
| xml_etree_iterparse      | 156 ms                                                                | 151 ms: 1.03x faster                                                    |
| django_template          | 53.3 ms                                                               | 51.7 ms: 1.03x faster                                                   |
| json                     | 5.88 ms                                                               | 5.71 ms: 1.03x faster                                                   |
| meteor_contest           | 126 ms                                                                | 125 ms: 1.01x faster                                                    |
| asyncio_websockets       | 657 ms                                                                | 663 ms: 1.01x slower                                                    |
| json_loads               | 30.9 us                                                               | 31.4 us: 1.02x slower                                                   |
| create_gc_cycles         | 2.26 ms                                                               | 2.30 ms: 1.02x slower                                                   |
| sqlglot_optimize         | 75.4 ms                                                               | 78.3 ms: 1.04x slower                                                   |
| sympy_integrate          | 26.5 ms                                                               | 28.5 ms: 1.07x slower                                                   |
| nqueens                  | 117 ms                                                                | 127 ms: 1.08x slower                                                    |
| pickle_dict              | 35.1 us                                                               | 38.3 us: 1.09x slower                                                   |
| pprint_safe_repr         | 1.15 sec                                                              | 1.26 sec: 1.09x slower                                                  |
| regex_compile            | 177 ms                                                                | 194 ms: 1.10x slower                                                    |
| docutils                 | 3.53 sec                                                              | 3.93 sec: 1.11x slower                                                  |
| unpickle                 | 17.5 us                                                               | 19.5 us: 1.11x slower                                                   |
| sympy_str                | 329 ms                                                                | 366 ms: 1.11x slower                                                    |
| pickle                   | 12.5 us                                                               | 13.9 us: 1.12x slower                                                   |
| pprint_pformat           | 2.36 sec                                                              | 2.64 sec: 1.12x slower                                                  |
| async_generators         | 452 ms                                                                | 510 ms: 1.13x slower                                                    |
| sympy_expand             | 543 ms                                                                | 613 ms: 1.13x slower                                                    |
| dulwich_log              | 73.5 ms                                                               | 83.2 ms: 1.13x slower                                                   |
| sympy_sum                | 184 ms                                                                | 211 ms: 1.15x slower                                                    |
| genshi_xml               | 69.8 ms                                                               | 80.8 ms: 1.16x slower                                                   |
| python_startup           | 11.2 ms                                                               | 13.0 ms: 1.16x slower                                                   |
| regex_effbot             | 4.25 ms                                                               | 4.96 ms: 1.17x slower                                                   |
| telco                    | 8.49 ms                                                               | 10.3 ms: 1.21x slower                                                   |
| coverage                 | 83.6 ms                                                               | 102 ms: 1.22x slower                                                    |
| python_startup_no_site   | 6.89 ms                                                               | 8.73 ms: 1.27x slower                                                   |
| gc_traversal             | 4.15 ms                                                               | 5.27 ms: 1.27x slower                                                   |
| Geometric mean           | (ref)                                                                 | 1.17x faster                                                            |

Benchmark hidden because not significant (6): pylint, regex_dna, genshi_text, pidigits, pickle_list, 2to3
Ignored benchmarks (8) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (6) of results/bm-20240924-3.14.0a0-54dd77f-JIT/bm-20240924-arminc-aarch64-python-54dd77fb8c880d7655ff-3.14.0a0-54dd77f.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser, unpack_sequence

# HPT report

- Reliability score: 99.97% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.25x