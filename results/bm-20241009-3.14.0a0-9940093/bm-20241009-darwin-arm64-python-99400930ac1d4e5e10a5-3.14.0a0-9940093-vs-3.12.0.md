# Results vs. 3.12.0

- fork: python
- ref: 99400930ac1d4e5e10a5
- machine: darwin-arm64
- commit hash: 9940093
- commit date: 2024-10-09
- overall geometric mean: 1.096x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 0.71x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 169 ms                                                 | 160 ms: 1.06x faster                                                  |
| docutils       | 1.50 sec                                               | 1.40 sec: 1.07x faster                                                |
| Geometric mean | (ref)                                                  | 1.03x faster                                                          |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 266 ms                                                 | 192 ms: 1.38x faster                                                  |
| async_tree_memoization_tg  | 323 ms                                                 | 245 ms: 1.32x faster                                                  |
| async_tree_none_tg         | 258 ms                                                 | 197 ms: 1.30x faster                                                  |
| async_tree_memoization     | 312 ms                                                 | 251 ms: 1.24x faster                                                  |
| async_tree_io_tg           | 669 ms                                                 | 564 ms: 1.19x faster                                                  |
| async_tree_cpu_io_mixed_tg | 532 ms                                                 | 459 ms: 1.16x faster                                                  |
| async_tree_cpu_io_mixed    | 526 ms                                                 | 458 ms: 1.15x faster                                                  |
| async_tree_io              | 668 ms                                                 | 592 ms: 1.13x faster                                                  |
| Geometric mean             | (ref)                                                  | 1.23x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 55.8 ms                                                | 48.3 ms: 1.15x faster                                                 |
| nbody          | 68.8 ms                                                | 59.7 ms: 1.15x faster                                                 |
| pidigits       | 282 ms                                                 | 282 ms: 1.00x slower                                                  |
| Geometric mean | (ref)                                                  | 1.10x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 77.9 ms                                                | 67.6 ms: 1.15x faster                                                 |
| regex_dna      | 154 ms                                                 | 146 ms: 1.06x faster                                                  |
| regex_effbot   | 2.64 ms                                                | 2.59 ms: 1.02x faster                                                 |
| regex_v8       | 16.0 ms                                                | 16.4 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                  | 1.05x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pickle_pure_python   | 200 us                                                 | 182 us: 1.10x faster                                                  |
| xml_etree_generate   | 55.8 ms                                                | 52.3 ms: 1.07x faster                                                 |
| unpickle_pure_python | 151 us                                                 | 141 us: 1.06x faster                                                  |
| xml_etree_process    | 39.7 ms                                                | 37.7 ms: 1.05x faster                                                 |
| json_loads           | 17.2 us                                                | 16.7 us: 1.03x faster                                                 |
| unpickle_list        | 3.02 us                                                | 2.97 us: 1.02x faster                                                 |
| xml_etree_iterparse  | 74.0 ms                                                | 73.2 ms: 1.01x faster                                                 |
| unpickle             | 9.12 us                                                | 9.08 us: 1.00x faster                                                 |
| json_dumps           | 6.22 ms                                                | 6.26 ms: 1.01x slower                                                 |
| pickle_dict          | 18.1 us                                                | 18.2 us: 1.01x slower                                                 |
| pickle               | 7.23 us                                                | 7.31 us: 1.01x slower                                                 |
| tomli_loads          | 1.39 sec                                               | 1.48 sec: 1.06x slower                                                |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (2): xml_etree_parse, pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 9.37 ms                                                | 13.8 ms: 1.48x slower                                                 |
| python_startup         | 11.4 ms                                                | 16.9 ms: 1.48x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.48x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 22.3 ms                                                | 19.9 ms: 1.12x faster                                                 |
| mako            | 7.71 ms                                                | 6.88 ms: 1.12x faster                                                 |
| Geometric mean  | (ref)                                                  | 1.12x faster                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| asyncio_websockets         | 409 ms                                                 | 242 ms: 1.69x faster                                                  |
| deepcopy_memo              | 27.7 us                                                | 17.2 us: 1.61x faster                                                 |
| deepcopy                   | 235 us                                                 | 146 us: 1.60x faster                                                  |
| generators                 | 31.1 ms                                                | 20.1 ms: 1.55x faster                                                 |
| raytrace                   | 212 ms                                                 | 150 ms: 1.41x faster                                                  |
| async_tree_none            | 266 ms                                                 | 192 ms: 1.38x faster                                                  |
| deepcopy_reduce            | 2.07 us                                                | 1.53 us: 1.36x faster                                                 |
| async_tree_memoization_tg  | 323 ms                                                 | 245 ms: 1.32x faster                                                  |
| comprehensions             | 14.5 us                                                | 11.1 us: 1.32x faster                                                 |
| async_tree_none_tg         | 258 ms                                                 | 197 ms: 1.30x faster                                                  |
| logging_silent             | 76.4 ns                                                | 60.5 ns: 1.26x faster                                                 |
| go                         | 102 ms                                                 | 81.4 ms: 1.25x faster                                                 |
| async_tree_memoization     | 312 ms                                                 | 251 ms: 1.24x faster                                                  |
| deltablue                  | 2.71 ms                                                | 2.21 ms: 1.22x faster                                                 |
| chaos                      | 42.5 ms                                                | 35.5 ms: 1.20x faster                                                 |
| logging_simple             | 3.69 us                                                | 3.09 us: 1.19x faster                                                 |
| async_tree_io_tg           | 669 ms                                                 | 564 ms: 1.19x faster                                                  |
| logging_format             | 3.98 us                                                | 3.36 us: 1.18x faster                                                 |
| coroutines                 | 19.2 ms                                                | 16.3 ms: 1.18x faster                                                 |
| unpack_sequence            | 31.5 ns                                                | 26.7 ns: 1.18x faster                                                 |
| nqueens                    | 62.4 ms                                                | 53.7 ms: 1.16x faster                                                 |
| async_tree_cpu_io_mixed_tg | 532 ms                                                 | 459 ms: 1.16x faster                                                  |
| float                      | 55.8 ms                                                | 48.3 ms: 1.15x faster                                                 |
| scimark_lu                 | 76.0 ms                                                | 65.8 ms: 1.15x faster                                                 |
| regex_compile              | 77.9 ms                                                | 67.6 ms: 1.15x faster                                                 |
| nbody                      | 68.8 ms                                                | 59.7 ms: 1.15x faster                                                 |
| sqlglot_parse              | 848 us                                                 | 736 us: 1.15x faster                                                  |
| sqlglot_transpile          | 1.02 ms                                                | 891 us: 1.15x faster                                                  |
| async_tree_cpu_io_mixed    | 526 ms                                                 | 458 ms: 1.15x faster                                                  |
| async_tree_io              | 668 ms                                                 | 592 ms: 1.13x faster                                                  |
| scimark_sparse_mat_mult    | 3.14 ms                                                | 2.78 ms: 1.13x faster                                                 |
| django_template            | 22.3 ms                                                | 19.9 ms: 1.12x faster                                                 |
| mako                       | 7.71 ms                                                | 6.88 ms: 1.12x faster                                                 |
| sympy_sum                  | 77.6 ms                                                | 69.3 ms: 1.12x faster                                                 |
| sqlglot_normalize          | 186 ms                                                 | 166 ms: 1.12x faster                                                  |
| pathlib                    | 24.2 ms                                                | 21.7 ms: 1.12x faster                                                 |
| sympy_str                  | 148 ms                                                 | 133 ms: 1.11x faster                                                  |
| bench_thread_pool          | 504 us                                                 | 456 us: 1.11x faster                                                  |
| hexiom                     | 4.54 ms                                                | 4.12 ms: 1.10x faster                                                 |
| spectral_norm              | 76.4 ms                                                | 69.4 ms: 1.10x faster                                                 |
| sqlglot_optimize           | 34.0 ms                                                | 31.0 ms: 1.10x faster                                                 |
| pickle_pure_python         | 200 us                                                 | 182 us: 1.10x faster                                                  |
| async_generators           | 304 ms                                                 | 277 ms: 1.10x faster                                                  |
| pprint_safe_repr           | 497 ms                                                 | 456 ms: 1.09x faster                                                  |
| dulwich_log                | 29.8 ms                                                | 27.4 ms: 1.09x faster                                                 |
| mdp                        | 1.58 sec                                               | 1.45 sec: 1.09x faster                                                |
| pprint_pformat             | 1.01 sec                                               | 932 ms: 1.08x faster                                                  |
| json                       | 3.09 ms                                                | 2.86 ms: 1.08x faster                                                 |
| sympy_expand               | 241 ms                                                 | 225 ms: 1.07x faster                                                  |
| docutils                   | 1.50 sec                                               | 1.40 sec: 1.07x faster                                                |
| scimark_fft                | 195 ms                                                 | 183 ms: 1.07x faster                                                  |
| xml_etree_generate         | 55.8 ms                                                | 52.3 ms: 1.07x faster                                                 |
| pycparser                  | 677 ms                                                 | 636 ms: 1.06x faster                                                  |
| unpickle_pure_python       | 151 us                                                 | 141 us: 1.06x faster                                                  |
| 2to3                       | 169 ms                                                 | 160 ms: 1.06x faster                                                  |
| regex_dna                  | 154 ms                                                 | 146 ms: 1.06x faster                                                  |
| scimark_monte_carlo        | 45.0 ms                                                | 42.7 ms: 1.05x faster                                                 |
| xml_etree_process          | 39.7 ms                                                | 37.7 ms: 1.05x faster                                                 |
| sqlite_synth               | 1.57 us                                                | 1.50 us: 1.05x faster                                                 |
| sympy_integrate            | 11.4 ms                                                | 10.9 ms: 1.04x faster                                                 |
| json_loads                 | 17.2 us                                                | 16.7 us: 1.03x faster                                                 |
| richards_super             | 36.0 ms                                                | 35.0 ms: 1.03x faster                                                 |
| crypto_pyaes               | 51.9 ms                                                | 50.7 ms: 1.02x faster                                                 |
| regex_effbot               | 2.64 ms                                                | 2.59 ms: 1.02x faster                                                 |
| unpickle_list              | 3.02 us                                                | 2.97 us: 1.02x faster                                                 |
| richards                   | 32.1 ms                                                | 31.7 ms: 1.01x faster                                                 |
| meteor_contest             | 71.7 ms                                                | 70.8 ms: 1.01x faster                                                 |
| xml_etree_iterparse        | 74.0 ms                                                | 73.2 ms: 1.01x faster                                                 |
| unpickle                   | 9.12 us                                                | 9.08 us: 1.00x faster                                                 |
| pidigits                   | 282 ms                                                 | 282 ms: 1.00x slower                                                  |
| json_dumps                 | 6.22 ms                                                | 6.26 ms: 1.01x slower                                                 |
| pickle_dict                | 18.1 us                                                | 18.2 us: 1.01x slower                                                 |
| pickle                     | 7.23 us                                                | 7.31 us: 1.01x slower                                                 |
| pyflate                    | 316 ms                                                 | 324 ms: 1.03x slower                                                  |
| regex_v8                   | 16.0 ms                                                | 16.4 ms: 1.03x slower                                                 |
| gc_traversal               | 2.40 ms                                                | 2.49 ms: 1.04x slower                                                 |
| asyncio_tcp_ssl            | 1.24 sec                                               | 1.31 sec: 1.05x slower                                                |
| fannkuch                   | 248 ms                                                 | 262 ms: 1.06x slower                                                  |
| tomli_loads                | 1.39 sec                                               | 1.48 sec: 1.06x slower                                                |
| scimark_sor                | 87.4 ms                                                | 94.4 ms: 1.08x slower                                                 |
| bench_mp_pool              | 44.9 ms                                                | 49.1 ms: 1.09x slower                                                 |
| coverage                   | 38.9 ms                                                | 43.5 ms: 1.12x slower                                                 |
| telco                      | 3.68 ms                                                | 4.47 ms: 1.22x slower                                                 |
| create_gc_cycles           | 701 us                                                 | 913 us: 1.30x slower                                                  |
| python_startup_no_site     | 9.37 ms                                                | 13.8 ms: 1.48x slower                                                 |
| python_startup             | 11.4 ms                                                | 16.9 ms: 1.48x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.09x faster                                                          |

Benchmark hidden because not significant (5): asyncio_tcp, xml_etree_parse, pickle_list, typing_runtime_protocols, tornado_http
Ignored benchmarks (7) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (14) of results/bm-20241009-3.14.0a0-9940093/bm-20241009-darwin-arm64-python-99400930ac1d4e5e10a5-3.14.0a0-9940093.json: async_tree_eager, async_tree_eager_cpu_io_mixed, async_tree_eager_cpu_io_mixed_tg, async_tree_eager_io, async_tree_eager_io_tg, async_tree_eager_memoization, async_tree_eager_memoization_tg, async_tree_eager_tg, bpe_tokeniser, genshi_text, genshi_xml, html5lib, pylint, thrift

- Geometric mean (including insignificant results): 1.096x faster
# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 0.71x