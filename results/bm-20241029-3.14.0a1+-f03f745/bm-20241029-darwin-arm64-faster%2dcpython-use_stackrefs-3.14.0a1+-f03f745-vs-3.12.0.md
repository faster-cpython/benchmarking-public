# Results vs. 3.12.0

- fork: faster-cpython
- ref: use_stackrefs
- machine: darwin-arm64
- commit hash: f03f745
- commit date: 2024-10-29
- overall geometric mean: 1.021x faster
- HPT reliability: 98.20%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.17x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| 2to3           | 169 ms                                                 | 171 ms: 1.01x slower                                                      |
| docutils       | 1.50 sec                                               | 1.47 sec: 1.02x faster                                                    |
| tornado_http   | 69.3 ms                                                | 85.2 ms: 1.23x slower                                                     |
| Geometric mean | (ref)                                                  | 1.07x slower                                                              |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 323 ms                                                 | 244 ms: 1.32x faster                                                      |
| async_tree_none            | 266 ms                                                 | 213 ms: 1.25x faster                                                      |
| async_tree_memoization     | 312 ms                                                 | 262 ms: 1.19x faster                                                      |
| async_tree_none_tg         | 258 ms                                                 | 224 ms: 1.15x faster                                                      |
| async_tree_io              | 668 ms                                                 | 596 ms: 1.12x faster                                                      |
| async_tree_cpu_io_mixed    | 526 ms                                                 | 469 ms: 1.12x faster                                                      |
| async_tree_cpu_io_mixed_tg | 532 ms                                                 | 478 ms: 1.11x faster                                                      |
| async_tree_io_tg           | 669 ms                                                 | 640 ms: 1.05x faster                                                      |
| Geometric mean             | (ref)                                                  | 1.16x faster                                                              |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| float          | 55.8 ms                                                | 51.4 ms: 1.09x faster                                                     |
| nbody          | 68.8 ms                                                | 64.9 ms: 1.06x faster                                                     |
| pidigits       | 282 ms                                                 | 283 ms: 1.01x slower                                                      |
| Geometric mean | (ref)                                                  | 1.05x faster                                                              |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| regex_dna      | 154 ms                                                 | 140 ms: 1.10x faster                                                      |
| regex_effbot   | 2.64 ms                                                | 2.45 ms: 1.08x faster                                                     |
| regex_compile  | 77.9 ms                                                | 74.2 ms: 1.05x faster                                                     |
| regex_v8       | 16.0 ms                                                | 16.5 ms: 1.04x slower                                                     |
| Geometric mean | (ref)                                                  | 1.05x faster                                                              |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| json_loads           | 17.2 us                                                | 16.9 us: 1.02x faster                                                     |
| xml_etree_iterparse  | 74.0 ms                                                | 74.8 ms: 1.01x slower                                                     |
| xml_etree_process    | 39.7 ms                                                | 40.8 ms: 1.03x slower                                                     |
| pickle_pure_python   | 200 us                                                 | 209 us: 1.04x slower                                                      |
| xml_etree_generate   | 55.8 ms                                                | 58.6 ms: 1.05x slower                                                     |
| unpickle_pure_python | 151 us                                                 | 167 us: 1.11x slower                                                      |
| tomli_loads          | 1.39 sec                                               | 1.64 sec: 1.17x slower                                                    |
| json_dumps           | 6.22 ms                                                | 7.37 ms: 1.18x slower                                                     |
| Geometric mean       | (ref)                                                  | 1.06x slower                                                              |

Benchmark hidden because not significant (1): xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| python_startup_no_site | 9.37 ms                                                | 12.4 ms: 1.33x slower                                                     |
| python_startup         | 11.4 ms                                                | 17.1 ms: 1.50x slower                                                     |
| Geometric mean         | (ref)                                                  | 1.41x slower                                                              |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|-----------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| mako            | 7.71 ms                                                | 7.35 ms: 1.05x faster                                                     |
| django_template | 22.3 ms                                                | 21.5 ms: 1.04x faster                                                     |
| Geometric mean  | (ref)                                                  | 1.04x faster                                                              |

All benchmarks:
===============

| Benchmark                  | bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745 |
|----------------------------|:------------------------------------------------------:|:-------------------------------------------------------------------------:|
| asyncio_websockets         | 409 ms                                                 | 242 ms: 1.69x faster                                                      |
| deepcopy_memo              | 27.7 us                                                | 17.8 us: 1.55x faster                                                     |
| deepcopy                   | 235 us                                                 | 155 us: 1.51x faster                                                      |
| generators                 | 31.1 ms                                                | 22.8 ms: 1.36x faster                                                     |
| async_tree_memoization_tg  | 323 ms                                                 | 244 ms: 1.32x faster                                                      |
| comprehensions             | 14.5 us                                                | 11.2 us: 1.30x faster                                                     |
| raytrace                   | 212 ms                                                 | 164 ms: 1.30x faster                                                      |
| deepcopy_reduce            | 2.07 us                                                | 1.65 us: 1.26x faster                                                     |
| async_tree_none            | 266 ms                                                 | 213 ms: 1.25x faster                                                      |
| async_tree_memoization     | 312 ms                                                 | 262 ms: 1.19x faster                                                      |
| async_tree_none_tg         | 258 ms                                                 | 224 ms: 1.15x faster                                                      |
| async_tree_io              | 668 ms                                                 | 596 ms: 1.12x faster                                                      |
| async_tree_cpu_io_mixed    | 526 ms                                                 | 469 ms: 1.12x faster                                                      |
| async_tree_cpu_io_mixed_tg | 532 ms                                                 | 478 ms: 1.11x faster                                                      |
| logging_silent             | 76.4 ns                                                | 69.0 ns: 1.11x faster                                                     |
| regex_dna                  | 154 ms                                                 | 140 ms: 1.10x faster                                                      |
| float                      | 55.8 ms                                                | 51.4 ms: 1.09x faster                                                     |
| nqueens                    | 62.4 ms                                                | 57.6 ms: 1.08x faster                                                     |
| regex_effbot               | 2.64 ms                                                | 2.45 ms: 1.08x faster                                                     |
| logging_simple             | 3.69 us                                                | 3.44 us: 1.07x faster                                                     |
| sqlglot_parse              | 848 us                                                 | 792 us: 1.07x faster                                                      |
| deltablue                  | 2.71 ms                                                | 2.53 ms: 1.07x faster                                                     |
| coroutines                 | 19.2 ms                                                | 18.1 ms: 1.06x faster                                                     |
| pathlib                    | 24.2 ms                                                | 22.8 ms: 1.06x faster                                                     |
| bench_thread_pool          | 504 us                                                 | 474 us: 1.06x faster                                                      |
| chaos                      | 42.5 ms                                                | 40.0 ms: 1.06x faster                                                     |
| sqlglot_transpile          | 1.02 ms                                                | 962 us: 1.06x faster                                                      |
| nbody                      | 68.8 ms                                                | 64.9 ms: 1.06x faster                                                     |
| sympy_sum                  | 77.6 ms                                                | 73.2 ms: 1.06x faster                                                     |
| go                         | 102 ms                                                 | 96.4 ms: 1.05x faster                                                     |
| logging_format             | 3.98 us                                                | 3.78 us: 1.05x faster                                                     |
| mdp                        | 1.58 sec                                               | 1.50 sec: 1.05x faster                                                    |
| async_generators           | 304 ms                                                 | 289 ms: 1.05x faster                                                      |
| regex_compile              | 77.9 ms                                                | 74.2 ms: 1.05x faster                                                     |
| mako                       | 7.71 ms                                                | 7.35 ms: 1.05x faster                                                     |
| json                       | 3.09 ms                                                | 2.94 ms: 1.05x faster                                                     |
| sqlglot_normalize          | 186 ms                                                 | 177 ms: 1.05x faster                                                      |
| sqlalchemy_declarative     | 62.3 ms                                                | 59.6 ms: 1.05x faster                                                     |
| async_tree_io_tg           | 669 ms                                                 | 640 ms: 1.05x faster                                                      |
| dulwich_log                | 29.8 ms                                                | 28.8 ms: 1.04x faster                                                     |
| django_template            | 22.3 ms                                                | 21.5 ms: 1.04x faster                                                     |
| sympy_str                  | 148 ms                                                 | 143 ms: 1.04x faster                                                      |
| pprint_safe_repr           | 497 ms                                                 | 480 ms: 1.03x faster                                                      |
| pprint_pformat             | 1.01 sec                                               | 979 ms: 1.03x faster                                                      |
| sqlite_synth               | 1.57 us                                                | 1.54 us: 1.02x faster                                                     |
| docutils                   | 1.50 sec                                               | 1.47 sec: 1.02x faster                                                    |
| sqlglot_optimize           | 34.0 ms                                                | 33.3 ms: 1.02x faster                                                     |
| json_loads                 | 17.2 us                                                | 16.9 us: 1.02x faster                                                     |
| scimark_lu                 | 76.0 ms                                                | 75.3 ms: 1.01x faster                                                     |
| sympy_expand               | 241 ms                                                 | 242 ms: 1.00x slower                                                      |
| pidigits                   | 282 ms                                                 | 283 ms: 1.01x slower                                                      |
| 2to3                       | 169 ms                                                 | 171 ms: 1.01x slower                                                      |
| sympy_integrate            | 11.4 ms                                                | 11.5 ms: 1.01x slower                                                     |
| xml_etree_iterparse        | 74.0 ms                                                | 74.8 ms: 1.01x slower                                                     |
| meteor_contest             | 71.7 ms                                                | 72.8 ms: 1.01x slower                                                     |
| spectral_norm              | 76.4 ms                                                | 78.0 ms: 1.02x slower                                                     |
| scimark_monte_carlo        | 45.0 ms                                                | 46.2 ms: 1.03x slower                                                     |
| xml_etree_process          | 39.7 ms                                                | 40.8 ms: 1.03x slower                                                     |
| sqlalchemy_imperative      | 6.68 ms                                                | 6.88 ms: 1.03x slower                                                     |
| regex_v8                   | 16.0 ms                                                | 16.5 ms: 1.04x slower                                                     |
| hexiom                     | 4.54 ms                                                | 4.72 ms: 1.04x slower                                                     |
| scimark_fft                | 195 ms                                                 | 203 ms: 1.04x slower                                                      |
| pickle_pure_python         | 200 us                                                 | 209 us: 1.04x slower                                                      |
| xml_etree_generate         | 55.8 ms                                                | 58.6 ms: 1.05x slower                                                     |
| typing_runtime_protocols   | 93.0 us                                                | 101 us: 1.08x slower                                                      |
| richards_super             | 36.0 ms                                                | 39.2 ms: 1.09x slower                                                     |
| richards                   | 32.1 ms                                                | 35.3 ms: 1.10x slower                                                     |
| pyflate                    | 316 ms                                                 | 347 ms: 1.10x slower                                                      |
| unpickle_pure_python       | 151 us                                                 | 167 us: 1.11x slower                                                      |
| crypto_pyaes               | 51.9 ms                                                | 58.6 ms: 1.13x slower                                                     |
| coverage                   | 38.9 ms                                                | 44.2 ms: 1.14x slower                                                     |
| fannkuch                   | 248 ms                                                 | 283 ms: 1.14x slower                                                      |
| tomli_loads                | 1.39 sec                                               | 1.64 sec: 1.17x slower                                                    |
| scimark_sor                | 87.4 ms                                                | 103 ms: 1.18x slower                                                      |
| json_dumps                 | 6.22 ms                                                | 7.37 ms: 1.18x slower                                                     |
| gc_traversal               | 2.40 ms                                                | 2.94 ms: 1.23x slower                                                     |
| tornado_http               | 69.3 ms                                                | 85.2 ms: 1.23x slower                                                     |
| telco                      | 3.68 ms                                                | 4.69 ms: 1.28x slower                                                     |
| bench_mp_pool              | 44.9 ms                                                | 59.1 ms: 1.32x slower                                                     |
| python_startup_no_site     | 9.37 ms                                                | 12.4 ms: 1.33x slower                                                     |
| python_startup             | 11.4 ms                                                | 17.1 ms: 1.50x slower                                                     |
| create_gc_cycles           | 701 us                                                 | 1.33 ms: 1.89x slower                                                     |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                              |

Benchmark hidden because not significant (3): scimark_sparse_mat_mult, xml_etree_parse, pycparser
Ignored benchmarks (13) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-darwin-arm64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, gunicorn, mypy2, pickle, pickle_dict, pickle_list, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (18) of results/bm-20241029-3.14.0a1+-f03f745/bm-20241029-darwin-arm64-faster%2dcpython-use_stackrefs-3.14.0a1+-f03f745.json: async_tree_eager, async_tree_eager_cpu_io_mixed, async_tree_eager_cpu_io_mixed_tg, async_tree_eager_io, async_tree_eager_io_tg, async_tree_eager_memoization, async_tree_eager_memoization_tg, async_tree_eager_tg, bpe_tokeniser, connected_components, genshi_text, genshi_xml, html5lib, k_core, pylint, shortest_path, sphinx, thrift

- Geometric mean (including insignificant results): 1.021x faster
# HPT report

- Reliability score: 98.20% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.17x