
# Results vs. 3.11.0

- fork: python
- ref: 22988c323ad621b9f47b
- machine: linux-x86_64
- commit hash: 22988c3
- commit date: 2023-07-10
- overall geometric mean: 1.04x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| docutils       | 2.86 sec                                                     | 2.91 sec: 1.02x slower                                                      |
| Geometric mean | (ref)                                                        | 1.01x slower                                                                |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 90.7 ms                                                      | 89.0 ms: 1.02x faster                                                       |
| pidigits       | 251 ms                                                       | 262 ms: 1.04x slower                                                        |
| float          | 74.2 ms                                                      | 82.5 ms: 1.11x slower                                                       |
| Geometric mean | (ref)                                                        | 1.04x slower                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 158 ms                                                       | 151 ms: 1.04x faster                                                        |
| regex_v8       | 23.9 ms                                                      | 23.7 ms: 1.01x faster                                                       |
| regex_effbot   | 3.50 ms                                                      | 3.54 ms: 1.01x slower                                                       |
| regex_dna      | 227 ms                                                       | 247 ms: 1.09x slower                                                        |
| Geometric mean | (ref)                                                        | 1.01x slower                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_dumps           | 13.4 ms                                                      | 10.1 ms: 1.32x faster                                                       |
| json_loads           | 28.7 us                                                      | 25.4 us: 1.13x faster                                                       |
| unpickle_pure_python | 241 us                                                       | 218 us: 1.10x faster                                                        |
| xml_etree_parse      | 158 ms                                                       | 151 ms: 1.05x faster                                                        |
| pickle_pure_python   | 319 us                                                       | 315 us: 1.01x faster                                                        |
| tomli_loads          | 2.26 sec                                                     | 2.33 sec: 1.03x slower                                                      |
| xml_etree_iterparse  | 104 ms                                                       | 108 ms: 1.03x slower                                                        |
| pickle_dict          | 30.8 us                                                      | 32.1 us: 1.04x slower                                                       |
| pickle               | 9.64 us                                                      | 10.1 us: 1.05x slower                                                       |
| xml_etree_process    | 56.5 ms                                                      | 59.5 ms: 1.05x slower                                                       |
| unpickle_list        | 4.53 us                                                      | 4.79 us: 1.06x slower                                                       |
| xml_etree_generate   | 80.5 ms                                                      | 87.0 ms: 1.08x slower                                                       |
| unpickle             | 13.4 us                                                      | 14.7 us: 1.10x slower                                                       |
| pickle_list          | 3.83 us                                                      | 4.47 us: 1.17x slower                                                       |
| Geometric mean       | (ref)                                                        | 1.00x slower                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 10.8 ms                                                      | 11.4 ms: 1.06x slower                                                       |
| python_startup_no_site | 7.76 ms                                                      | 8.41 ms: 1.08x slower                                                       |
| Geometric mean         | (ref)                                                        | 1.07x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|-----------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako      | 11.0 ms                                                      | 10.3 ms: 1.07x faster                                                       |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230710-pythonperf2-x86_64-python-22988c323ad621b9f47b-3.13.0a0-22988c3 |
|--------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| typing_runtime_protocols | 523 us                                                       | 154 us: 3.40x faster                                                        |
| asyncio_tcp              | 753 ms                                                       | 384 ms: 1.96x faster                                                        |
| asyncio_tcp_ssl          | 3.08 sec                                                     | 1.57 sec: 1.96x faster                                                      |
| generators               | 56.0 ms                                                      | 36.7 ms: 1.53x faster                                                       |
| chaos                    | 80.9 ms                                                      | 61.2 ms: 1.32x faster                                                       |
| json_dumps               | 13.4 ms                                                      | 10.1 ms: 1.32x faster                                                       |
| mypy2                    | 451 ms                                                       | 372 ms: 1.21x faster                                                        |
| scimark_lu               | 115 ms                                                       | 98.5 ms: 1.16x faster                                                       |
| json_loads               | 28.7 us                                                      | 25.4 us: 1.13x faster                                                       |
| coroutines               | 27.6 ms                                                      | 24.6 ms: 1.12x faster                                                       |
| nqueens                  | 103 ms                                                       | 92.1 ms: 1.12x faster                                                       |
| async_tree_memoization   | 630 ms                                                       | 566 ms: 1.11x faster                                                        |
| async_tree_none          | 519 ms                                                       | 467 ms: 1.11x faster                                                        |
| comprehensions           | 24.6 us                                                      | 22.2 us: 1.11x faster                                                       |
| raytrace                 | 317 ms                                                       | 285 ms: 1.11x faster                                                        |
| unpickle_pure_python     | 241 us                                                       | 218 us: 1.10x faster                                                        |
| crypto_pyaes             | 83.4 ms                                                      | 75.8 ms: 1.10x faster                                                       |
| json                     | 5.65 ms                                                      | 5.14 ms: 1.10x faster                                                       |
| deltablue                | 4.00 ms                                                      | 3.67 ms: 1.09x faster                                                       |
| logging_format           | 8.11 us                                                      | 7.47 us: 1.09x faster                                                       |
| hexiom                   | 7.13 ms                                                      | 6.57 ms: 1.09x faster                                                       |
| sqlglot_normalize        | 126 ms                                                       | 116 ms: 1.09x faster                                                        |
| mdp                      | 2.75 sec                                                     | 2.54 sec: 1.08x faster                                                      |
| async_tree_io            | 1.17 sec                                                     | 1.09 sec: 1.08x faster                                                      |
| fannkuch                 | 429 ms                                                       | 400 ms: 1.07x faster                                                        |
| sqlglot_parse            | 1.53 ms                                                      | 1.43 ms: 1.07x faster                                                       |
| mako                     | 11.0 ms                                                      | 10.3 ms: 1.07x faster                                                       |
| deepcopy                 | 399 us                                                       | 375 us: 1.06x faster                                                        |
| bench_thread_pool        | 1.01 ms                                                      | 960 us: 1.05x faster                                                        |
| xml_etree_parse          | 158 ms                                                       | 151 ms: 1.05x faster                                                        |
| regex_compile            | 158 ms                                                       | 151 ms: 1.04x faster                                                        |
| sqlglot_transpile        | 1.92 ms                                                      | 1.84 ms: 1.04x faster                                                       |
| async_tree_cpu_io_mixed  | 749 ms                                                       | 720 ms: 1.04x faster                                                        |
| richards_super           | 61.0 ms                                                      | 58.9 ms: 1.04x faster                                                       |
| logging_simple           | 7.19 us                                                      | 6.95 us: 1.03x faster                                                       |
| deepcopy_reduce          | 3.51 us                                                      | 3.41 us: 1.03x faster                                                       |
| logging_silent           | 101 ns                                                       | 98.2 ns: 1.03x faster                                                       |
| nbody                    | 90.7 ms                                                      | 89.0 ms: 1.02x faster                                                       |
| sqlglot_optimize         | 59.8 ms                                                      | 58.8 ms: 1.02x faster                                                       |
| pickle_pure_python       | 319 us                                                       | 315 us: 1.01x faster                                                        |
| regex_v8                 | 23.9 ms                                                      | 23.7 ms: 1.01x faster                                                       |
| deepcopy_memo            | 38.8 us                                                      | 38.4 us: 1.01x faster                                                       |
| meteor_contest           | 131 ms                                                       | 130 ms: 1.01x faster                                                        |
| regex_effbot             | 3.50 ms                                                      | 3.54 ms: 1.01x slower                                                       |
| pathlib                  | 19.1 ms                                                      | 19.3 ms: 1.01x slower                                                       |
| docutils                 | 2.86 sec                                                     | 2.91 sec: 1.02x slower                                                      |
| pycparser                | 1.32 sec                                                     | 1.36 sec: 1.03x slower                                                      |
| tomli_loads              | 2.26 sec                                                     | 2.33 sec: 1.03x slower                                                      |
| xml_etree_iterparse      | 104 ms                                                       | 108 ms: 1.03x slower                                                        |
| pidigits                 | 251 ms                                                       | 262 ms: 1.04x slower                                                        |
| pickle_dict              | 30.8 us                                                      | 32.1 us: 1.04x slower                                                       |
| coverage                 | 84.8 ms                                                      | 88.9 ms: 1.05x slower                                                       |
| gc_traversal             | 3.85 ms                                                      | 4.04 ms: 1.05x slower                                                       |
| pickle                   | 9.64 us                                                      | 10.1 us: 1.05x slower                                                       |
| xml_etree_process        | 56.5 ms                                                      | 59.5 ms: 1.05x slower                                                       |
| bench_mp_pool            | 4.62 ms                                                      | 4.88 ms: 1.06x slower                                                       |
| unpickle_list            | 4.53 us                                                      | 4.79 us: 1.06x slower                                                       |
| python_startup           | 10.8 ms                                                      | 11.4 ms: 1.06x slower                                                       |
| create_gc_cycles         | 1.61 ms                                                      | 1.70 ms: 1.06x slower                                                       |
| pprint_pformat           | 1.63 sec                                                     | 1.73 sec: 1.06x slower                                                      |
| go                       | 164 ms                                                       | 175 ms: 1.06x slower                                                        |
| spectral_norm            | 93.3 ms                                                      | 99.9 ms: 1.07x slower                                                       |
| xml_etree_generate       | 80.5 ms                                                      | 87.0 ms: 1.08x slower                                                       |
| scimark_monte_carlo      | 68.2 ms                                                      | 73.7 ms: 1.08x slower                                                       |
| scimark_sparse_mat_mult  | 4.05 ms                                                      | 4.38 ms: 1.08x slower                                                       |
| pprint_safe_repr         | 784 ms                                                       | 849 ms: 1.08x slower                                                        |
| python_startup_no_site   | 7.76 ms                                                      | 8.41 ms: 1.08x slower                                                       |
| regex_dna                | 227 ms                                                       | 247 ms: 1.09x slower                                                        |
| richards                 | 48.3 ms                                                      | 52.8 ms: 1.09x slower                                                       |
| unpickle                 | 13.4 us                                                      | 14.7 us: 1.10x slower                                                       |
| sqlite_synth             | 2.50 us                                                      | 2.74 us: 1.10x slower                                                       |
| telco                    | 6.86 ms                                                      | 7.62 ms: 1.11x slower                                                       |
| float                    | 74.2 ms                                                      | 82.5 ms: 1.11x slower                                                       |
| scimark_fft              | 285 ms                                                       | 318 ms: 1.12x slower                                                        |
| pyflate                  | 449 ms                                                       | 513 ms: 1.14x slower                                                        |
| pickle_list              | 3.83 us                                                      | 4.47 us: 1.17x slower                                                       |
| unpack_sequence          | 45.6 ns                                                      | 54.0 ns: 1.18x slower                                                       |
| async_generators         | 316 ms                                                       | 389 ms: 1.23x slower                                                        |
| scimark_sor              | 111 ms                                                       | 149 ms: 1.34x slower                                                        |
| dask                     | 410 ms                                                       | 592 ms: 1.44x slower                                                        |
| Geometric mean           | (ref)                                                        | 1.04x faster                                                                |

Benchmark hidden because not significant (2): dulwich_log, tornado_http
Ignored benchmarks (17) of results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
