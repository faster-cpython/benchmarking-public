# Results vs. 3.13.0b2

- fork: python
- ref: 5f6001130f8ada871193
- machine: linux-x86_64
- commit hash: 5f60011
- commit date: 2024-07-25
- overall geometric mean: 1.01x faster
- HPT reliability: 86.85%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.07x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 291 ms                                                           | 303 ms: 1.04x slower                                                        |
| docutils       | 2.98 sec                                                         | 3.14 sec: 1.05x slower                                                      |
| html5lib       | 74.7 ms                                                          | 70.4 ms: 1.06x faster                                                       |
| tornado_http   | 119 ms                                                           | 122 ms: 1.02x slower                                                        |
| Geometric mean | (ref)                                                            | 1.01x slower                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_io_tg           | 900 ms                                                           | 786 ms: 1.14x faster                                                        |
| async_tree_memoization     | 460 ms                                                           | 403 ms: 1.14x faster                                                        |
| async_tree_none_tg         | 340 ms                                                           | 304 ms: 1.12x faster                                                        |
| async_tree_memoization_tg  | 421 ms                                                           | 383 ms: 1.10x faster                                                        |
| async_tree_io              | 873 ms                                                           | 799 ms: 1.09x faster                                                        |
| async_tree_none            | 365 ms                                                           | 336 ms: 1.09x faster                                                        |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 539 ms: 1.06x faster                                                        |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 577 ms: 1.05x faster                                                        |
| Geometric mean             | (ref)                                                            | 1.10x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 87.8 ms                                                          | 81.4 ms: 1.08x faster                                                       |
| float          | 80.2 ms                                                          | 74.4 ms: 1.08x faster                                                       |
| pidigits       | 254 ms                                                           | 250 ms: 1.01x faster                                                        |
| Geometric mean | (ref)                                                            | 1.06x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 144 ms                                                           | 136 ms: 1.06x faster                                                        |
| regex_dna      | 249 ms                                                           | 236 ms: 1.05x faster                                                        |
| regex_v8       | 26.0 ms                                                          | 26.1 ms: 1.00x slower                                                       |
| regex_effbot   | 3.40 ms                                                          | 3.44 ms: 1.01x slower                                                       |
| Geometric mean | (ref)                                                            | 1.02x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| tomli_loads          | 2.40 sec                                                         | 2.12 sec: 1.13x faster                                                      |
| unpickle_pure_python | 224 us                                                           | 213 us: 1.05x faster                                                        |
| xml_etree_generate   | 85.7 ms                                                          | 81.9 ms: 1.05x faster                                                       |
| xml_etree_iterparse  | 103 ms                                                           | 98.1 ms: 1.04x faster                                                       |
| xml_etree_process    | 59.7 ms                                                          | 57.8 ms: 1.03x faster                                                       |
| xml_etree_parse      | 144 ms                                                           | 142 ms: 1.01x faster                                                        |
| json_loads           | 25.0 us                                                          | 25.8 us: 1.03x slower                                                       |
| pickle_pure_python   | 307 us                                                           | 317 us: 1.03x slower                                                        |
| Geometric mean       | (ref)                                                            | 1.03x faster                                                                |

Benchmark hidden because not significant (1): json_dumps

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 13.2 ms                                                          | 13.4 ms: 1.01x slower                                                       |
| python_startup_no_site | 8.85 ms                                                          | 9.05 ms: 1.02x slower                                                       |
| Geometric mean         | (ref)                                                            | 1.02x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|-----------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 10.4 ms                                                          | 9.19 ms: 1.13x faster                                                       |
| django_template | 39.0 ms                                                          | 43.2 ms: 1.11x slower                                                       |
| genshi_text     | 25.9 ms                                                          | 28.9 ms: 1.12x slower                                                       |
| genshi_xml      | 58.1 ms                                                          | 65.6 ms: 1.13x slower                                                       |
| Geometric mean  | (ref)                                                            | 1.06x slower                                                                |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240725-pythonperf2-x86_64-python-5f6001130f8ada871193-3.14.0a0-5f60011 |
|----------------------------|:----------------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deepcopy_memo              | 37.3 us                                                          | 29.6 us: 1.26x faster                                                       |
| spectral_norm              | 97.3 ms                                                          | 80.8 ms: 1.20x faster                                                       |
| deepcopy                   | 377 us                                                           | 315 us: 1.20x faster                                                        |
| richards                   | 53.4 ms                                                          | 46.4 ms: 1.15x faster                                                       |
| richards_super             | 61.2 ms                                                          | 53.4 ms: 1.15x faster                                                       |
| async_tree_io_tg           | 900 ms                                                           | 786 ms: 1.14x faster                                                        |
| async_tree_memoization     | 460 ms                                                           | 403 ms: 1.14x faster                                                        |
| tomli_loads                | 2.40 sec                                                         | 2.12 sec: 1.13x faster                                                      |
| mako                       | 10.4 ms                                                          | 9.19 ms: 1.13x faster                                                       |
| async_tree_none_tg         | 340 ms                                                           | 304 ms: 1.12x faster                                                        |
| scimark_fft                | 312 ms                                                           | 283 ms: 1.10x faster                                                        |
| async_tree_memoization_tg  | 421 ms                                                           | 383 ms: 1.10x faster                                                        |
| generators                 | 33.5 ms                                                          | 30.6 ms: 1.09x faster                                                       |
| async_tree_io              | 873 ms                                                           | 799 ms: 1.09x faster                                                        |
| deepcopy_reduce            | 3.39 us                                                          | 3.10 us: 1.09x faster                                                       |
| async_tree_none            | 365 ms                                                           | 336 ms: 1.09x faster                                                        |
| nbody                      | 87.8 ms                                                          | 81.4 ms: 1.08x faster                                                       |
| float                      | 80.2 ms                                                          | 74.4 ms: 1.08x faster                                                       |
| bpe_tokeniser              | 5.14 sec                                                         | 4.80 sec: 1.07x faster                                                      |
| telco                      | 8.40 ms                                                          | 7.84 ms: 1.07x faster                                                       |
| pyflate                    | 482 ms                                                           | 452 ms: 1.07x faster                                                        |
| async_tree_cpu_io_mixed_tg | 573 ms                                                           | 539 ms: 1.06x faster                                                        |
| regex_compile              | 144 ms                                                           | 136 ms: 1.06x faster                                                        |
| scimark_sparse_mat_mult    | 4.28 ms                                                          | 4.03 ms: 1.06x faster                                                       |
| html5lib                   | 74.7 ms                                                          | 70.4 ms: 1.06x faster                                                       |
| gc_traversal               | 4.69 ms                                                          | 4.43 ms: 1.06x faster                                                       |
| regex_dna                  | 249 ms                                                           | 236 ms: 1.05x faster                                                        |
| unpickle_pure_python       | 224 us                                                           | 213 us: 1.05x faster                                                        |
| crypto_pyaes               | 73.6 ms                                                          | 69.9 ms: 1.05x faster                                                       |
| async_tree_cpu_io_mixed    | 604 ms                                                           | 577 ms: 1.05x faster                                                        |
| xml_etree_generate         | 85.7 ms                                                          | 81.9 ms: 1.05x faster                                                       |
| xml_etree_iterparse        | 103 ms                                                           | 98.1 ms: 1.04x faster                                                       |
| pathlib                    | 17.1 ms                                                          | 16.4 ms: 1.04x faster                                                       |
| create_gc_cycles           | 2.00 ms                                                          | 1.93 ms: 1.04x faster                                                       |
| coroutines                 | 22.0 ms                                                          | 21.3 ms: 1.03x faster                                                       |
| xml_etree_process          | 59.7 ms                                                          | 57.8 ms: 1.03x faster                                                       |
| pidigits                   | 254 ms                                                           | 250 ms: 1.01x faster                                                        |
| go                         | 165 ms                                                           | 163 ms: 1.01x faster                                                        |
| xml_etree_parse            | 144 ms                                                           | 142 ms: 1.01x faster                                                        |
| fannkuch                   | 353 ms                                                           | 348 ms: 1.01x faster                                                        |
| coverage                   | 83.5 ms                                                          | 82.7 ms: 1.01x faster                                                       |
| logging_format             | 7.11 us                                                          | 7.06 us: 1.01x faster                                                       |
| pycparser                  | 1.22 sec                                                         | 1.21 sec: 1.01x faster                                                      |
| pprint_safe_repr           | 818 ms                                                           | 813 ms: 1.01x faster                                                        |
| meteor_contest             | 128 ms                                                           | 128 ms: 1.00x faster                                                        |
| asyncio_tcp_ssl            | 1.58 sec                                                         | 1.58 sec: 1.00x slower                                                      |
| regex_v8                   | 26.0 ms                                                          | 26.1 ms: 1.00x slower                                                       |
| logging_simple             | 6.40 us                                                          | 6.44 us: 1.01x slower                                                       |
| pprint_pformat             | 1.66 sec                                                         | 1.68 sec: 1.01x slower                                                      |
| python_startup             | 13.2 ms                                                          | 13.4 ms: 1.01x slower                                                       |
| regex_effbot               | 3.40 ms                                                          | 3.44 ms: 1.01x slower                                                       |
| sqlglot_parse              | 1.39 ms                                                          | 1.41 ms: 1.01x slower                                                       |
| thrift                     | 880 us                                                           | 894 us: 1.02x slower                                                        |
| tornado_http               | 119 ms                                                           | 122 ms: 1.02x slower                                                        |
| python_startup_no_site     | 8.85 ms                                                          | 9.05 ms: 1.02x slower                                                       |
| json                       | 5.35 ms                                                          | 5.48 ms: 1.02x slower                                                       |
| json_loads                 | 25.0 us                                                          | 25.8 us: 1.03x slower                                                       |
| pickle_pure_python         | 307 us                                                           | 317 us: 1.03x slower                                                        |
| 2to3                       | 291 ms                                                           | 303 ms: 1.04x slower                                                        |
| sqlglot_transpile          | 1.76 ms                                                          | 1.83 ms: 1.04x slower                                                       |
| logging_silent             | 96.3 ns                                                          | 101 ns: 1.05x slower                                                        |
| sympy_expand               | 501 ms                                                           | 526 ms: 1.05x slower                                                        |
| docutils                   | 2.98 sec                                                         | 3.14 sec: 1.05x slower                                                      |
| mdp                        | 2.46 sec                                                         | 2.59 sec: 1.05x slower                                                      |
| hexiom                     | 6.35 ms                                                          | 6.69 ms: 1.05x slower                                                       |
| sympy_str                  | 295 ms                                                           | 313 ms: 1.06x slower                                                        |
| async_generators           | 363 ms                                                           | 392 ms: 1.08x slower                                                        |
| deltablue                  | 3.37 ms                                                          | 3.66 ms: 1.08x slower                                                       |
| sympy_sum                  | 155 ms                                                           | 168 ms: 1.08x slower                                                        |
| comprehensions             | 17.0 us                                                          | 18.4 us: 1.09x slower                                                       |
| sqlglot_optimize           | 59.5 ms                                                          | 64.9 ms: 1.09x slower                                                       |
| typing_runtime_protocols   | 171 us                                                           | 187 us: 1.10x slower                                                        |
| nqueens                    | 88.4 ms                                                          | 97.6 ms: 1.10x slower                                                       |
| scimark_sor                | 119 ms                                                           | 131 ms: 1.11x slower                                                        |
| django_template            | 39.0 ms                                                          | 43.2 ms: 1.11x slower                                                       |
| sqlglot_normalize          | 120 ms                                                           | 134 ms: 1.12x slower                                                        |
| genshi_text                | 25.9 ms                                                          | 28.9 ms: 1.12x slower                                                       |
| sympy_integrate            | 23.2 ms                                                          | 26.0 ms: 1.12x slower                                                       |
| genshi_xml                 | 58.1 ms                                                          | 65.6 ms: 1.13x slower                                                       |
| pylint                     | 339 ms                                                           | 384 ms: 1.13x slower                                                        |
| chaos                      | 59.6 ms                                                          | 67.5 ms: 1.13x slower                                                       |
| raytrace                   | 260 ms                                                           | 301 ms: 1.16x slower                                                        |
| scimark_lu                 | 97.5 ms                                                          | 122 ms: 1.25x slower                                                        |
| Geometric mean             | (ref)                                                            | 1.01x faster                                                                |

Benchmark hidden because not significant (7): asyncio_websockets, json_dumps, asyncio_tcp, scimark_monte_carlo, bench_thread_pool, dask, bench_mp_pool
Ignored benchmarks (12) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf2-x86_64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 86.85% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.07x