# Results vs. 3.12.0

- fork: brandtbucher
- ref: justin_no_externs
- machine: windows-amd64
- commit hash: 5791853
- commit date: 2024-10-25
- overall geometric mean: 1.003x slower
- HPT reliability: 59.14%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 253 ms: 1.16x slower                                                           |
| docutils       | 1.66 sec                                                    | 1.94 sec: 1.17x slower                                                         |
| tornado_http   | 89.5 ms                                                     | 98.6 ms: 1.10x slower                                                          |
| Geometric mean | (ref)                                                       | 1.14x slower                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 367 ms                                                      | 262 ms: 1.40x faster                                                           |
| async_tree_none            | 291 ms                                                      | 219 ms: 1.33x faster                                                           |
| async_tree_io              | 731 ms                                                      | 550 ms: 1.33x faster                                                           |
| async_tree_none_tg         | 285 ms                                                      | 215 ms: 1.33x faster                                                           |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 390 ms: 1.25x faster                                                           |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 407 ms: 1.24x faster                                                           |
| async_tree_memoization     | 339 ms                                                      | 281 ms: 1.21x faster                                                           |
| async_tree_io_tg           | 771 ms                                                      | 643 ms: 1.20x faster                                                           |
| Geometric mean             | (ref)                                                       | 1.28x faster                                                                   |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| nbody          | 71.9 ms                                                     | 51.9 ms: 1.39x faster                                                          |
| float          | 56.8 ms                                                     | 48.1 ms: 1.18x faster                                                          |
| pidigits       | 152 ms                                                      | 148 ms: 1.03x faster                                                           |
| Geometric mean | (ref)                                                       | 1.19x faster                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_dna      | 126 ms                                                      | 122 ms: 1.04x faster                                                           |
| regex_effbot   | 1.62 ms                                                     | 1.63 ms: 1.01x slower                                                          |
| regex_compile  | 87.5 ms                                                     | 92.7 ms: 1.06x slower                                                          |
| regex_v8       | 14.2 ms                                                     | 15.4 ms: 1.08x slower                                                          |
| Geometric mean | (ref)                                                       | 1.03x slower                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| xml_etree_generate   | 55.8 ms                                                     | 51.2 ms: 1.09x faster                                                          |
| tomli_loads          | 1.37 sec                                                    | 1.31 sec: 1.04x faster                                                         |
| xml_etree_process    | 37.7 ms                                                     | 36.5 ms: 1.03x faster                                                          |
| xml_etree_iterparse  | 65.2 ms                                                     | 63.3 ms: 1.03x faster                                                          |
| pickle_pure_python   | 195 us                                                      | 199 us: 1.02x slower                                                           |
| xml_etree_parse      | 93.0 ms                                                     | 95.7 ms: 1.03x slower                                                          |
| json_loads           | 13.9 us                                                     | 14.3 us: 1.03x slower                                                          |
| unpickle_pure_python | 133 us                                                      | 145 us: 1.09x slower                                                           |
| json_dumps           | 5.70 ms                                                     | 6.47 ms: 1.14x slower                                                          |
| Geometric mean       | (ref)                                                       | 1.01x slower                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup_no_site | 16.2 ms                                                     | 18.4 ms: 1.13x slower                                                          |
| python_startup         | 19.5 ms                                                     | 24.2 ms: 1.24x slower                                                          |
| Geometric mean         | (ref)                                                       | 1.19x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|-----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| mako            | 7.09 ms                                                     | 5.20 ms: 1.36x faster                                                          |
| django_template | 22.9 ms                                                     | 27.0 ms: 1.18x slower                                                          |
| Geometric mean  | (ref)                                                       | 1.08x faster                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853 |
|----------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------------:|
| deepcopy_memo              | 23.7 us                                                     | 16.9 us: 1.41x faster                                                          |
| async_tree_memoization_tg  | 367 ms                                                      | 262 ms: 1.40x faster                                                           |
| nbody                      | 71.9 ms                                                     | 51.9 ms: 1.39x faster                                                          |
| mako                       | 7.09 ms                                                     | 5.20 ms: 1.36x faster                                                          |
| async_tree_none            | 291 ms                                                      | 219 ms: 1.33x faster                                                           |
| async_tree_io              | 731 ms                                                      | 550 ms: 1.33x faster                                                           |
| async_tree_none_tg         | 285 ms                                                      | 215 ms: 1.33x faster                                                           |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 390 ms: 1.25x faster                                                           |
| spectral_norm              | 66.9 ms                                                     | 53.9 ms: 1.24x faster                                                          |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 407 ms: 1.24x faster                                                           |
| deepcopy                   | 238 us                                                      | 195 us: 1.22x faster                                                           |
| async_tree_memoization     | 339 ms                                                      | 281 ms: 1.21x faster                                                           |
| async_tree_io_tg           | 771 ms                                                      | 643 ms: 1.20x faster                                                           |
| float                      | 56.8 ms                                                     | 48.1 ms: 1.18x faster                                                          |
| scimark_sor                | 78.8 ms                                                     | 67.0 ms: 1.18x faster                                                          |
| comprehensions             | 14.1 us                                                     | 12.1 us: 1.17x faster                                                          |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.20 ms: 1.16x faster                                                          |
| scimark_monte_carlo        | 43.7 ms                                                     | 38.0 ms: 1.15x faster                                                          |
| scimark_fft                | 184 ms                                                      | 161 ms: 1.14x faster                                                           |
| crypto_pyaes               | 48.5 ms                                                     | 42.5 ms: 1.14x faster                                                          |
| deepcopy_reduce            | 2.09 us                                                     | 1.87 us: 1.12x faster                                                          |
| pprint_pformat             | 1.05 sec                                                    | 941 ms: 1.11x faster                                                           |
| pprint_safe_repr           | 513 ms                                                      | 470 ms: 1.09x faster                                                           |
| xml_etree_generate         | 55.8 ms                                                     | 51.2 ms: 1.09x faster                                                          |
| dulwich_log                | 44.3 ms                                                     | 41.3 ms: 1.07x faster                                                          |
| logging_silent             | 60.5 ns                                                     | 56.4 ns: 1.07x faster                                                          |
| tomli_loads                | 1.37 sec                                                    | 1.31 sec: 1.04x faster                                                         |
| regex_dna                  | 126 ms                                                      | 122 ms: 1.04x faster                                                           |
| xml_etree_process          | 37.7 ms                                                     | 36.5 ms: 1.03x faster                                                          |
| xml_etree_iterparse        | 65.2 ms                                                     | 63.3 ms: 1.03x faster                                                          |
| pidigits                   | 152 ms                                                      | 148 ms: 1.03x faster                                                           |
| logging_simple             | 6.28 us                                                     | 6.12 us: 1.03x faster                                                          |
| bench_thread_pool          | 857 us                                                      | 836 us: 1.03x faster                                                           |
| logging_format             | 6.72 us                                                     | 6.59 us: 1.02x faster                                                          |
| json                       | 3.05 ms                                                     | 3.01 ms: 1.01x faster                                                          |
| regex_effbot               | 1.62 ms                                                     | 1.63 ms: 1.01x slower                                                          |
| fannkuch                   | 247 ms                                                      | 248 ms: 1.01x slower                                                           |
| pathlib                    | 80.5 ms                                                     | 81.1 ms: 1.01x slower                                                          |
| coroutines                 | 14.3 ms                                                     | 14.4 ms: 1.01x slower                                                          |
| pickle_pure_python         | 195 us                                                      | 199 us: 1.02x slower                                                           |
| go                         | 91.6 ms                                                     | 94.0 ms: 1.03x slower                                                          |
| xml_etree_parse            | 93.0 ms                                                     | 95.7 ms: 1.03x slower                                                          |
| pyflate                    | 295 ms                                                      | 303 ms: 1.03x slower                                                           |
| nqueens                    | 62.8 ms                                                     | 64.7 ms: 1.03x slower                                                          |
| json_loads                 | 13.9 us                                                     | 14.3 us: 1.03x slower                                                          |
| meteor_contest             | 74.6 ms                                                     | 77.5 ms: 1.04x slower                                                          |
| regex_compile              | 87.5 ms                                                     | 92.7 ms: 1.06x slower                                                          |
| pycparser                  | 699 ms                                                      | 744 ms: 1.06x slower                                                           |
| raytrace                   | 192 ms                                                      | 208 ms: 1.08x slower                                                           |
| regex_v8                   | 14.2 ms                                                     | 15.4 ms: 1.08x slower                                                          |
| unpickle_pure_python       | 133 us                                                      | 145 us: 1.09x slower                                                           |
| tornado_http               | 89.5 ms                                                     | 98.6 ms: 1.10x slower                                                          |
| deltablue                  | 2.16 ms                                                     | 2.38 ms: 1.10x slower                                                          |
| sympy_str                  | 175 ms                                                      | 195 ms: 1.12x slower                                                           |
| async_generators           | 239 ms                                                      | 267 ms: 1.12x slower                                                           |
| mdp                        | 1.37 sec                                                    | 1.54 sec: 1.12x slower                                                         |
| sqlglot_normalize          | 187 ms                                                      | 210 ms: 1.12x slower                                                           |
| python_startup_no_site     | 16.2 ms                                                     | 18.4 ms: 1.13x slower                                                          |
| json_dumps                 | 5.70 ms                                                     | 6.47 ms: 1.14x slower                                                          |
| telco                      | 4.13 ms                                                     | 4.72 ms: 1.14x slower                                                          |
| sympy_sum                  | 91.5 ms                                                     | 106 ms: 1.16x slower                                                           |
| sqlglot_parse              | 804 us                                                      | 931 us: 1.16x slower                                                           |
| sympy_expand               | 284 ms                                                      | 330 ms: 1.16x slower                                                           |
| 2to3                       | 218 ms                                                      | 253 ms: 1.16x slower                                                           |
| docutils                   | 1.66 sec                                                    | 1.94 sec: 1.17x slower                                                         |
| coverage                   | 40.8 ms                                                     | 47.7 ms: 1.17x slower                                                          |
| django_template            | 22.9 ms                                                     | 27.0 ms: 1.18x slower                                                          |
| sqlglot_transpile          | 1.02 ms                                                     | 1.21 ms: 1.19x slower                                                          |
| sympy_integrate            | 13.0 ms                                                     | 15.8 ms: 1.22x slower                                                          |
| richards_super             | 32.1 ms                                                     | 39.5 ms: 1.23x slower                                                          |
| typing_runtime_protocols   | 95.1 us                                                     | 118 us: 1.24x slower                                                           |
| python_startup             | 19.5 ms                                                     | 24.2 ms: 1.24x slower                                                          |
| richards                   | 28.4 ms                                                     | 35.5 ms: 1.25x slower                                                          |
| sqlglot_optimize           | 34.5 ms                                                     | 43.9 ms: 1.27x slower                                                          |
| gc_traversal               | 1.52 ms                                                     | 1.96 ms: 1.29x slower                                                          |
| bench_mp_pool              | 69.2 ms                                                     | 89.9 ms: 1.30x slower                                                          |
| hexiom                     | 4.10 ms                                                     | 5.43 ms: 1.32x slower                                                          |
| create_gc_cycles           | 752 us                                                      | 1.40 ms: 1.86x slower                                                          |
| Geometric mean             | (ref)                                                       | 1.00x slower                                                                   |

Benchmark hidden because not significant (3): generators, chaos, scimark_lu
Ignored benchmarks (15) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (6) of results/bm-20241025-3.14.0a1+-5791853-JIT/bm-20241025-pythonperf1-amd64-brandtbucher-justin_no_externs-3.14.0a1+-5791853.json: genshi_text, genshi_xml, html5lib, pylint, sphinx, thrift

- Geometric mean (including insignificant results): 1.003x slower
# HPT report

- Reliability score: 59.14% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown