
# Results vs. 3.11.0

- fork: brandtbucher
- ref: justin
- machine: linux-x86_64
- commit hash: ba47232
- commit date: 2023-07-17
- overall geometric mean: 1.01x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| docutils       | 2.63 sec                                               | 2.70 sec: 1.03x slower                                        |
| tornado_http   | 96.3 ms                                                | 98.5 ms: 1.02x slower                                         |
| Geometric mean | (ref)                                                  | 1.03x slower                                                  |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| pidigits       | 198 ms                                                 | 198 ms: 1.00x faster                                          |
| nbody          | 93.1 ms                                                | 94.2 ms: 1.01x slower                                         |
| float          | 77.2 ms                                                | 87.3 ms: 1.13x slower                                         |
| Geometric mean | (ref)                                                  | 1.04x slower                                                  |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|----------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| regex_effbot   | 3.99 ms                                                | 3.53 ms: 1.13x faster                                         |
| regex_v8       | 22.0 ms                                                | 22.4 ms: 1.02x slower                                         |
| regex_compile  | 138 ms                                                 | 141 ms: 1.02x slower                                          |
| regex_dna      | 204 ms                                                 | 210 ms: 1.03x slower                                          |
| Geometric mean | (ref)                                                  | 1.01x faster                                                  |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|----------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| json_dumps           | 12.6 ms                                                | 9.85 ms: 1.28x faster                                         |
| unpickle_pure_python | 228 us                                                 | 216 us: 1.05x faster                                          |
| json_loads           | 26.5 us                                                | 25.3 us: 1.05x faster                                         |
| xml_etree_parse      | 158 ms                                                 | 152 ms: 1.04x faster                                          |
| xml_etree_iterparse  | 104 ms                                                 | 103 ms: 1.01x faster                                          |
| unpickle_list        | 4.91 us                                                | 4.97 us: 1.01x slower                                         |
| pickle_dict          | 31.1 us                                                | 31.7 us: 1.02x slower                                         |
| pickle_pure_python   | 306 us                                                 | 313 us: 1.02x slower                                          |
| pickle               | 10.1 us                                                | 10.5 us: 1.04x slower                                         |
| tomli_loads          | 2.22 sec                                               | 2.37 sec: 1.07x slower                                        |
| pickle_list          | 4.11 us                                                | 4.44 us: 1.08x slower                                         |
| xml_etree_process    | 53.9 ms                                                | 58.5 ms: 1.09x slower                                         |
| unpickle             | 13.7 us                                                | 15.1 us: 1.11x slower                                         |
| xml_etree_generate   | 76.2 ms                                                | 85.2 ms: 1.12x slower                                         |
| Geometric mean       | (ref)                                                  | 1.01x slower                                                  |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|------------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| python_startup         | 8.52 ms                                                | 9.46 ms: 1.11x slower                                         |
| python_startup_no_site | 6.01 ms                                                | 6.88 ms: 1.15x slower                                         |
| Geometric mean         | (ref)                                                  | 1.13x slower                                                  |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|-----------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| mako      | 10.1 ms                                                | 12.3 ms: 1.22x slower                                         |

All benchmarks:
===============

| Benchmark                | bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230717-linux-x86_64-brandtbucher-justin-3.13.0a0-ba47232 |
|--------------------------|:------------------------------------------------------:|:-------------------------------------------------------------:|
| typing_runtime_protocols | 486 us                                                 | 147 us: 3.32x faster                                          |
| generators               | 73.5 ms                                                | 28.8 ms: 2.55x faster                                         |
| asyncio_tcp              | 922 ms                                                 | 505 ms: 1.82x faster                                          |
| asyncio_tcp_ssl          | 3.14 sec                                               | 1.79 sec: 1.76x faster                                        |
| json_dumps               | 12.6 ms                                                | 9.85 ms: 1.28x faster                                         |
| mypy2                    | 420 ms                                                 | 341 ms: 1.23x faster                                          |
| coroutines               | 25.5 ms                                                | 22.4 ms: 1.14x faster                                         |
| regex_effbot             | 3.99 ms                                                | 3.53 ms: 1.13x faster                                         |
| richards_super           | 56.8 ms                                                | 51.5 ms: 1.10x faster                                         |
| chaos                    | 69.2 ms                                                | 64.6 ms: 1.07x faster                                         |
| async_tree_io            | 1.30 sec                                               | 1.21 sec: 1.07x faster                                        |
| async_tree_none          | 526 ms                                                 | 492 ms: 1.07x faster                                          |
| unpickle_pure_python     | 228 us                                                 | 216 us: 1.05x faster                                          |
| coverage                 | 100 ms                                                 | 94.9 ms: 1.05x faster                                         |
| async_tree_memoization   | 627 ms                                                 | 598 ms: 1.05x faster                                          |
| json_loads               | 26.5 us                                                | 25.3 us: 1.05x faster                                         |
| xml_etree_parse          | 158 ms                                                 | 152 ms: 1.04x faster                                          |
| pycparser                | 1.18 sec                                               | 1.13 sec: 1.04x faster                                        |
| json                     | 4.94 ms                                                | 4.77 ms: 1.04x faster                                         |
| sqlglot_parse            | 1.40 ms                                                | 1.36 ms: 1.03x faster                                         |
| deltablue                | 3.67 ms                                                | 3.56 ms: 1.03x faster                                         |
| logging_format           | 6.68 us                                                | 6.55 us: 1.02x faster                                         |
| sqlglot_transpile        | 1.70 ms                                                | 1.68 ms: 1.01x faster                                         |
| xml_etree_iterparse      | 104 ms                                                 | 103 ms: 1.01x faster                                          |
| richards                 | 45.7 ms                                                | 45.2 ms: 1.01x faster                                         |
| meteor_contest           | 107 ms                                                 | 106 ms: 1.01x faster                                          |
| create_gc_cycles         | 1.49 ms                                                | 1.48 ms: 1.01x faster                                         |
| sqlglot_normalize        | 108 ms                                                 | 107 ms: 1.00x faster                                          |
| pidigits                 | 198 ms                                                 | 198 ms: 1.00x faster                                          |
| go                       | 140 ms                                                 | 141 ms: 1.00x slower                                          |
| sqlglot_optimize         | 53.1 ms                                                | 53.5 ms: 1.01x slower                                         |
| nbody                    | 93.1 ms                                                | 94.2 ms: 1.01x slower                                         |
| bench_thread_pool        | 819 us                                                 | 829 us: 1.01x slower                                          |
| gc_traversal             | 4.02 ms                                                | 4.07 ms: 1.01x slower                                         |
| unpickle_list            | 4.91 us                                                | 4.97 us: 1.01x slower                                         |
| regex_v8                 | 22.0 ms                                                | 22.4 ms: 1.02x slower                                         |
| pickle_dict              | 31.1 us                                                | 31.7 us: 1.02x slower                                         |
| mdp                      | 2.62 sec                                               | 2.67 sec: 1.02x slower                                        |
| nqueens                  | 83.4 ms                                                | 85.0 ms: 1.02x slower                                         |
| pickle_pure_python       | 306 us                                                 | 313 us: 1.02x slower                                          |
| pathlib                  | 18.2 ms                                                | 18.7 ms: 1.02x slower                                         |
| pprint_pformat           | 1.46 sec                                               | 1.49 sec: 1.02x slower                                        |
| tornado_http             | 96.3 ms                                                | 98.5 ms: 1.02x slower                                         |
| regex_compile            | 138 ms                                                 | 141 ms: 1.02x slower                                          |
| dulwich_log              | 63.7 ms                                                | 65.2 ms: 1.02x slower                                         |
| docutils                 | 2.63 sec                                               | 2.70 sec: 1.03x slower                                        |
| deepcopy                 | 342 us                                                 | 352 us: 1.03x slower                                          |
| regex_dna                | 204 ms                                                 | 210 ms: 1.03x slower                                          |
| comprehensions           | 22.4 us                                                | 23.2 us: 1.03x slower                                         |
| hexiom                   | 6.37 ms                                                | 6.58 ms: 1.03x slower                                         |
| telco                    | 6.58 ms                                                | 6.83 ms: 1.04x slower                                         |
| pickle                   | 10.1 us                                                | 10.5 us: 1.04x slower                                         |
| pprint_safe_repr         | 701 ms                                                 | 731 ms: 1.04x slower                                          |
| logging_silent           | 101 ns                                                 | 106 ns: 1.04x slower                                          |
| scimark_sor              | 118 ms                                                 | 124 ms: 1.05x slower                                          |
| tomli_loads              | 2.22 sec                                               | 2.37 sec: 1.07x slower                                        |
| scimark_monte_carlo      | 68.1 ms                                                | 72.7 ms: 1.07x slower                                         |
| deepcopy_reduce          | 2.94 us                                                | 3.14 us: 1.07x slower                                         |
| unpack_sequence          | 43.1 ns                                                | 46.1 ns: 1.07x slower                                         |
| pickle_list              | 4.11 us                                                | 4.44 us: 1.08x slower                                         |
| spectral_norm            | 100 ms                                                 | 108 ms: 1.08x slower                                          |
| xml_etree_process        | 53.9 ms                                                | 58.5 ms: 1.09x slower                                         |
| sqlite_synth             | 2.52 us                                                | 2.75 us: 1.09x slower                                         |
| unpickle                 | 13.7 us                                                | 15.1 us: 1.11x slower                                         |
| python_startup           | 8.52 ms                                                | 9.46 ms: 1.11x slower                                         |
| xml_etree_generate       | 76.2 ms                                                | 85.2 ms: 1.12x slower                                         |
| fannkuch                 | 388 ms                                                 | 437 ms: 1.13x slower                                          |
| float                    | 77.2 ms                                                | 87.3 ms: 1.13x slower                                         |
| scimark_fft              | 328 ms                                                 | 372 ms: 1.13x slower                                          |
| pyflate                  | 418 ms                                                 | 476 ms: 1.14x slower                                          |
| python_startup_no_site   | 6.01 ms                                                | 6.88 ms: 1.15x slower                                         |
| scimark_sparse_mat_mult  | 4.50 ms                                                | 5.19 ms: 1.15x slower                                         |
| crypto_pyaes             | 74.7 ms                                                | 87.2 ms: 1.17x slower                                         |
| scimark_lu               | 110 ms                                                 | 131 ms: 1.19x slower                                          |
| mako                     | 10.1 ms                                                | 12.3 ms: 1.22x slower                                         |
| async_generators         | 368 ms                                                 | 458 ms: 1.24x slower                                          |
| deepcopy_memo            | 37.0 us                                                | 48.6 us: 1.31x slower                                         |
| dask                     | 360 ms                                                 | 523 ms: 1.45x slower                                          |
| Geometric mean           | (ref)                                                  | 1.01x faster                                                  |

Benchmark hidden because not significant (4): async_tree_cpu_io_mixed, raytrace, bench_mp_pool, logging_simple
Ignored benchmarks (18) of results/bm-20221024-3.11.0-deaf509/bm-20221024-linux-x86_64-python-v3.11.0-3.11.0-deaf509.json: 2to3, aiohttp, chameleon, django_template, djangocms, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
