
# Results vs. 3.10.4

- fork: python
- ref: 2e343fc465ed0206340c
- machine: windows-amd64
- commit hash: 2e343fc
- commit date: 2022-11-10
- overall geometric mean: 1.16x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.11x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 198 ms: 1.20x faster                                                        |
| chameleon      | 5.92 ms                                                     | 4.89 ms: 1.21x faster                                                       |
| docutils       | 1.89 sec                                                    | 1.58 sec: 1.20x faster                                                      |
| html5lib       | 46.5 ms                                                     | 35.7 ms: 1.30x faster                                                       |
| tornado_http   | 109 ms                                                      | 93.4 ms: 1.17x faster                                                       |
| Geometric mean | (ref)                                                       | 1.21x faster                                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 54.1 ms: 1.11x faster                                                       |
| nbody          | 69.3 ms                                                     | 63.1 ms: 1.10x faster                                                       |
| pidigits       | 145 ms                                                      | 151 ms: 1.04x slower                                                        |
| Geometric mean | (ref)                                                       | 1.06x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 84.7 ms: 1.22x faster                                                       |
| regex_v8       | 15.0 ms                                                     | 13.5 ms: 1.11x faster                                                       |
| regex_dna      | 132 ms                                                      | 124 ms: 1.06x faster                                                        |
| regex_effbot   | 1.66 ms                                                     | 1.58 ms: 1.05x faster                                                       |
| Geometric mean | (ref)                                                       | 1.11x faster                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.38 ms: 1.58x faster                                                       |
| pickle_pure_python   | 257 us                                                      | 190 us: 1.35x faster                                                        |
| unpickle_pure_python | 171 us                                                      | 140 us: 1.22x faster                                                        |
| xml_etree_process    | 43.4 ms                                                     | 36.3 ms: 1.20x faster                                                       |
| unpickle             | 9.17 us                                                     | 7.75 us: 1.18x faster                                                       |
| xml_etree_parse      | 102 ms                                                      | 91.9 ms: 1.11x faster                                                       |
| unpickle_list        | 2.81 us                                                     | 2.56 us: 1.10x faster                                                       |
| json_loads           | 14.2 us                                                     | 13.1 us: 1.08x faster                                                       |
| xml_etree_generate   | 54.5 ms                                                     | 51.6 ms: 1.06x faster                                                       |
| xml_etree_iterparse  | 63.5 ms                                                     | 64.5 ms: 1.02x slower                                                       |
| pickle               | 6.80 us                                                     | 6.96 us: 1.02x slower                                                       |
| pickle_list          | 2.59 us                                                     | 2.80 us: 1.08x slower                                                       |
| pickle_dict          | 16.9 us                                                     | 19.3 us: 1.14x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.11x faster                                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.9 ms: 1.06x faster                                                       |
| python_startup_no_site | 15.5 ms                                                     | 16.1 ms: 1.04x slower                                                       |
| Geometric mean         | (ref)                                                       | 1.01x faster                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|-----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 6.55 ms: 1.34x faster                                                       |
| django_template | 28.2 ms                                                     | 21.7 ms: 1.30x faster                                                       |
| genshi_text     | 19.0 ms                                                     | 15.8 ms: 1.20x faster                                                       |
| genshi_xml      | 40.5 ms                                                     | 35.3 ms: 1.15x faster                                                       |
| Geometric mean  | (ref)                                                       | 1.25x faster                                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20221110-pythonperf1-amd64-python-2e343fc465ed0206340c-3.12.0a1+-2e343fc |
|-------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.35 ms: 1.77x faster                                                       |
| json_dumps              | 8.50 ms                                                     | 5.38 ms: 1.58x faster                                                       |
| mypy2                   | 352 ms                                                      | 225 ms: 1.57x faster                                                        |
| richards                | 41.2 ms                                                     | 27.2 ms: 1.51x faster                                                       |
| go                      | 136 ms                                                      | 91.2 ms: 1.49x faster                                                       |
| logging_silent          | 96.4 ns                                                     | 65.3 ns: 1.48x faster                                                       |
| raytrace                | 271 ms                                                      | 188 ms: 1.44x faster                                                        |
| scimark_sor             | 105 ms                                                      | 73.4 ms: 1.43x faster                                                       |
| sqlglot_parse           | 1.22 ms                                                     | 885 us: 1.38x faster                                                        |
| scimark_lu              | 85.4 ms                                                     | 62.4 ms: 1.37x faster                                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.07 ms: 1.37x faster                                                       |
| async_tree_io           | 1.07 sec                                                    | 784 ms: 1.36x faster                                                        |
| pickle_pure_python      | 257 us                                                      | 190 us: 1.35x faster                                                        |
| mako                    | 8.80 ms                                                     | 6.55 ms: 1.34x faster                                                       |
| pyflate                 | 387 ms                                                      | 291 ms: 1.33x faster                                                        |
| scimark_monte_carlo     | 55.9 ms                                                     | 42.3 ms: 1.32x faster                                                       |
| crypto_pyaes            | 62.3 ms                                                     | 47.5 ms: 1.31x faster                                                       |
| html5lib                | 46.5 ms                                                     | 35.7 ms: 1.30x faster                                                       |
| django_template         | 28.2 ms                                                     | 21.7 ms: 1.30x faster                                                       |
| pycparser               | 868 ms                                                      | 669 ms: 1.30x faster                                                        |
| chaos                   | 58.9 ms                                                     | 45.5 ms: 1.30x faster                                                       |
| hexiom                  | 5.52 ms                                                     | 4.28 ms: 1.29x faster                                                       |
| async_tree_memoization  | 497 ms                                                      | 387 ms: 1.29x faster                                                        |
| async_generators        | 224 ms                                                      | 175 ms: 1.28x faster                                                        |
| async_tree_none         | 420 ms                                                      | 330 ms: 1.27x faster                                                        |
| thrift                  | 615 us                                                      | 488 us: 1.26x faster                                                        |
| pprint_safe_repr        | 589 ms                                                      | 481 ms: 1.22x faster                                                        |
| regex_compile           | 103 ms                                                      | 84.7 ms: 1.22x faster                                                       |
| unpickle_pure_python    | 171 us                                                      | 140 us: 1.22x faster                                                        |
| chameleon               | 5.92 ms                                                     | 4.89 ms: 1.21x faster                                                       |
| genshi_text             | 19.0 ms                                                     | 15.8 ms: 1.20x faster                                                       |
| async_tree_cpu_io_mixed | 609 ms                                                      | 508 ms: 1.20x faster                                                        |
| xml_etree_process       | 43.4 ms                                                     | 36.3 ms: 1.20x faster                                                       |
| 2to3                    | 237 ms                                                      | 198 ms: 1.20x faster                                                        |
| spectral_norm           | 78.0 ms                                                     | 65.3 ms: 1.20x faster                                                       |
| docutils                | 1.89 sec                                                    | 1.58 sec: 1.20x faster                                                      |
| sqlglot_optimize        | 39.0 ms                                                     | 32.7 ms: 1.19x faster                                                       |
| pprint_pformat          | 1.21 sec                                                    | 1.01 sec: 1.19x faster                                                      |
| deepcopy_memo           | 28.5 us                                                     | 24.1 us: 1.18x faster                                                       |
| unpickle                | 9.17 us                                                     | 7.75 us: 1.18x faster                                                       |
| unpack_sequence         | 37.8 ns                                                     | 32.2 ns: 1.18x faster                                                       |
| tornado_http            | 109 ms                                                      | 93.4 ms: 1.17x faster                                                       |
| dask                    | 305 ms                                                      | 263 ms: 1.16x faster                                                        |
| create_gc_cycles        | 782 us                                                      | 676 us: 1.16x faster                                                        |
| genshi_xml              | 40.5 ms                                                     | 35.3 ms: 1.15x faster                                                       |
| coroutines              | 15.9 ms                                                     | 13.9 ms: 1.15x faster                                                       |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.32 ms: 1.14x faster                                                       |
| deepcopy_reduce         | 2.16 us                                                     | 1.91 us: 1.13x faster                                                       |
| deepcopy                | 255 us                                                      | 226 us: 1.13x faster                                                        |
| sympy_expand            | 315 ms                                                      | 280 ms: 1.12x faster                                                        |
| regex_v8                | 15.0 ms                                                     | 13.5 ms: 1.11x faster                                                       |
| float                   | 60.2 ms                                                     | 54.1 ms: 1.11x faster                                                       |
| sympy_integrate         | 14.8 ms                                                     | 13.3 ms: 1.11x faster                                                       |
| sqlglot_normalize       | 202 ms                                                      | 182 ms: 1.11x faster                                                        |
| bench_thread_pool       | 946 us                                                      | 852 us: 1.11x faster                                                        |
| xml_etree_parse         | 102 ms                                                      | 91.9 ms: 1.11x faster                                                       |
| json                    | 3.05 ms                                                     | 2.76 ms: 1.10x faster                                                       |
| mdp                     | 1.71 sec                                                    | 1.55 sec: 1.10x faster                                                      |
| unpickle_list           | 2.81 us                                                     | 2.56 us: 1.10x faster                                                       |
| nbody                   | 69.3 ms                                                     | 63.1 ms: 1.10x faster                                                       |
| fannkuch                | 258 ms                                                      | 236 ms: 1.09x faster                                                        |
| dulwich_log             | 47.6 ms                                                     | 43.9 ms: 1.09x faster                                                       |
| json_loads              | 14.2 us                                                     | 13.1 us: 1.08x faster                                                       |
| scimark_fft             | 193 ms                                                      | 179 ms: 1.08x faster                                                        |
| sympy_sum               | 104 ms                                                      | 97.3 ms: 1.07x faster                                                       |
| nqueens                 | 67.0 ms                                                     | 62.8 ms: 1.07x faster                                                       |
| sympy_str               | 188 ms                                                      | 177 ms: 1.06x faster                                                        |
| regex_dna               | 132 ms                                                      | 124 ms: 1.06x faster                                                        |
| xml_etree_generate      | 54.5 ms                                                     | 51.6 ms: 1.06x faster                                                       |
| python_startup          | 20.0 ms                                                     | 18.9 ms: 1.06x faster                                                       |
| regex_effbot            | 1.66 ms                                                     | 1.58 ms: 1.05x faster                                                       |
| pathlib                 | 77.4 ms                                                     | 74.4 ms: 1.04x faster                                                       |
| meteor_contest          | 72.5 ms                                                     | 70.4 ms: 1.03x faster                                                       |
| sqlite_synth            | 1.84 us                                                     | 1.79 us: 1.03x faster                                                       |
| comprehensions          | 16.0 us                                                     | 15.6 us: 1.03x faster                                                       |
| xml_etree_iterparse     | 63.5 ms                                                     | 64.5 ms: 1.02x slower                                                       |
| pickle                  | 6.80 us                                                     | 6.96 us: 1.02x slower                                                       |
| telco                   | 3.78 ms                                                     | 3.91 ms: 1.03x slower                                                       |
| python_startup_no_site  | 15.5 ms                                                     | 16.1 ms: 1.04x slower                                                       |
| pidigits                | 145 ms                                                      | 151 ms: 1.04x slower                                                        |
| bench_mp_pool           | 60.7 ms                                                     | 63.2 ms: 1.04x slower                                                       |
| asyncio_tcp             | 712 ms                                                      | 746 ms: 1.05x slower                                                        |
| logging_format          | 6.66 us                                                     | 6.99 us: 1.05x slower                                                       |
| logging_simple          | 6.20 us                                                     | 6.58 us: 1.06x slower                                                       |
| generators              | 31.6 ms                                                     | 33.9 ms: 1.07x slower                                                       |
| gc_traversal            | 1.34 ms                                                     | 1.45 ms: 1.08x slower                                                       |
| pickle_list             | 2.59 us                                                     | 2.80 us: 1.08x slower                                                       |
| pickle_dict             | 16.9 us                                                     | 19.3 us: 1.14x slower                                                       |
| coverage                | 40.0 ms                                                     | 53.4 ms: 1.33x slower                                                       |
| Geometric mean          | (ref)                                                       | 1.16x faster                                                                |
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.13x
- 95% likely to have a speedup of 1.12x
- 99% likely to have a speedup of 1.11x
