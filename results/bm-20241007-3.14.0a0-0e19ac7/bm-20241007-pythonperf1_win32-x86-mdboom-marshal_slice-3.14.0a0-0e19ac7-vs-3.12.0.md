# Results vs. 3.12.0

- fork: mdboom
- ref: marshal_slice
- machine: windows-x86
- commit hash: 0e19ac7
- commit date: 2024-10-07
- overall geometric mean: 1.10x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 280 ms                                                          | 255 ms: 1.10x faster                                                    |
| docutils       | 1.98 sec                                                        | 1.90 sec: 1.04x faster                                                  |
| Geometric mean | (ref)                                                           | 1.05x faster                                                            |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_none_tg         | 278 ms                                                          | 199 ms: 1.39x faster                                                    |
| async_tree_memoization_tg  | 350 ms                                                          | 254 ms: 1.38x faster                                                    |
| async_tree_none            | 298 ms                                                          | 218 ms: 1.37x faster                                                    |
| async_tree_memoization     | 364 ms                                                          | 274 ms: 1.33x faster                                                    |
| async_tree_io_tg           | 677 ms                                                          | 518 ms: 1.31x faster                                                    |
| async_tree_io              | 693 ms                                                          | 539 ms: 1.28x faster                                                    |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 473 ms: 1.19x faster                                                    |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 460 ms: 1.19x faster                                                    |
| Geometric mean             | (ref)                                                           | 1.30x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| nbody          | 127 ms                                                          | 96.4 ms: 1.32x faster                                                   |
| float          | 76.7 ms                                                         | 63.6 ms: 1.21x faster                                                   |
| pidigits       | 199 ms                                                          | 201 ms: 1.01x slower                                                    |
| Geometric mean | (ref)                                                           | 1.16x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_compile  | 129 ms                                                          | 111 ms: 1.17x faster                                                    |
| regex_dna      | 127 ms                                                          | 116 ms: 1.09x faster                                                    |
| regex_effbot   | 2.04 ms                                                         | 1.91 ms: 1.07x faster                                                   |
| regex_v8       | 15.0 ms                                                         | 16.1 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                           | 1.06x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| tomli_loads          | 2.20 sec                                                        | 1.91 sec: 1.15x faster                                                  |
| xml_etree_iterparse  | 77.6 ms                                                         | 67.8 ms: 1.15x faster                                                   |
| unpickle_pure_python | 210 us                                                          | 190 us: 1.10x faster                                                    |
| pickle_pure_python   | 286 us                                                          | 268 us: 1.06x faster                                                    |
| xml_etree_parse      | 113 ms                                                          | 107 ms: 1.06x faster                                                    |
| xml_etree_generate   | 72.1 ms                                                         | 69.3 ms: 1.04x faster                                                   |
| pickle_list          | 3.37 us                                                         | 3.31 us: 1.02x faster                                                   |
| xml_etree_process    | 53.2 ms                                                         | 52.5 ms: 1.01x faster                                                   |
| pickle_dict          | 19.9 us                                                         | 20.3 us: 1.02x slower                                                   |
| pickle               | 7.79 us                                                         | 7.95 us: 1.02x slower                                                   |
| unpickle_list        | 2.95 us                                                         | 3.02 us: 1.03x slower                                                   |
| json_loads           | 20.4 us                                                         | 20.9 us: 1.03x slower                                                   |
| json_dumps           | 7.40 ms                                                         | 7.75 ms: 1.05x slower                                                   |
| unpickle             | 9.71 us                                                         | 10.2 us: 1.05x slower                                                   |
| Geometric mean       | (ref)                                                           | 1.03x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 19.1 ms                                                         | 19.7 ms: 1.03x slower                                                   |
| python_startup         | 22.4 ms                                                         | 23.8 ms: 1.07x slower                                                   |
| Geometric mean         | (ref)                                                           | 1.05x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|-----------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 9.96 ms                                                         | 8.22 ms: 1.21x faster                                                   |
| django_template | 36.9 ms                                                         | 35.2 ms: 1.05x faster                                                   |
| Geometric mean  | (ref)                                                           | 1.13x faster                                                            |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0 | bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7 |
|----------------------------|:---------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| deepcopy_memo              | 38.4 us                                                         | 22.1 us: 1.74x faster                                                   |
| deepcopy                   | 360 us                                                          | 244 us: 1.48x faster                                                    |
| generators                 | 38.5 ms                                                         | 27.5 ms: 1.40x faster                                                   |
| async_tree_none_tg         | 278 ms                                                          | 199 ms: 1.39x faster                                                    |
| async_tree_memoization_tg  | 350 ms                                                          | 254 ms: 1.38x faster                                                    |
| unpack_sequence            | 62.5 ns                                                         | 45.4 ns: 1.38x faster                                                   |
| async_tree_none            | 298 ms                                                          | 218 ms: 1.37x faster                                                    |
| comprehensions             | 19.2 us                                                         | 14.1 us: 1.36x faster                                                   |
| logging_silent             | 101 ns                                                          | 75.6 ns: 1.34x faster                                                   |
| async_tree_memoization     | 364 ms                                                          | 274 ms: 1.33x faster                                                    |
| nbody                      | 127 ms                                                          | 96.4 ms: 1.32x faster                                                   |
| scimark_lu                 | 93.2 ms                                                         | 71.1 ms: 1.31x faster                                                   |
| async_tree_io_tg           | 677 ms                                                          | 518 ms: 1.31x faster                                                    |
| spectral_norm              | 104 ms                                                          | 80.5 ms: 1.29x faster                                                   |
| async_tree_io              | 693 ms                                                          | 539 ms: 1.28x faster                                                    |
| hexiom                     | 6.82 ms                                                         | 5.34 ms: 1.28x faster                                                   |
| deltablue                  | 3.58 ms                                                         | 2.82 ms: 1.27x faster                                                   |
| scimark_sor                | 130 ms                                                          | 103 ms: 1.26x faster                                                    |
| deepcopy_reduce            | 3.23 us                                                         | 2.58 us: 1.25x faster                                                   |
| go                         | 137 ms                                                          | 111 ms: 1.24x faster                                                    |
| chaos                      | 69.4 ms                                                         | 56.1 ms: 1.24x faster                                                   |
| mako                       | 9.96 ms                                                         | 8.22 ms: 1.21x faster                                                   |
| nqueens                    | 93.7 ms                                                         | 77.5 ms: 1.21x faster                                                   |
| float                      | 76.7 ms                                                         | 63.6 ms: 1.21x faster                                                   |
| async_tree_cpu_io_mixed    | 564 ms                                                          | 473 ms: 1.19x faster                                                    |
| raytrace                   | 308 ms                                                          | 259 ms: 1.19x faster                                                    |
| async_tree_cpu_io_mixed_tg | 546 ms                                                          | 460 ms: 1.19x faster                                                    |
| logging_simple             | 9.75 us                                                         | 8.23 us: 1.18x faster                                                   |
| crypto_pyaes               | 69.2 ms                                                         | 59.0 ms: 1.17x faster                                                   |
| scimark_sparse_mat_mult    | 3.86 ms                                                         | 3.29 ms: 1.17x faster                                                   |
| logging_format             | 10.4 us                                                         | 8.90 us: 1.17x faster                                                   |
| regex_compile              | 129 ms                                                          | 111 ms: 1.17x faster                                                    |
| tomli_loads                | 2.20 sec                                                        | 1.91 sec: 1.15x faster                                                  |
| pyflate                    | 424 ms                                                          | 369 ms: 1.15x faster                                                    |
| xml_etree_iterparse        | 77.6 ms                                                         | 67.8 ms: 1.15x faster                                                   |
| scimark_fft                | 271 ms                                                          | 237 ms: 1.14x faster                                                    |
| coroutines                 | 20.9 ms                                                         | 18.4 ms: 1.13x faster                                                   |
| scimark_monte_carlo        | 66.4 ms                                                         | 58.6 ms: 1.13x faster                                                   |
| mdp                        | 1.91 sec                                                        | 1.70 sec: 1.13x faster                                                  |
| sympy_sum                  | 123 ms                                                          | 109 ms: 1.12x faster                                                    |
| dulwich_log                | 58.5 ms                                                         | 52.1 ms: 1.12x faster                                                   |
| sqlglot_parse              | 1.25 ms                                                         | 1.12 ms: 1.12x faster                                                   |
| pycparser                  | 978 ms                                                          | 875 ms: 1.12x faster                                                    |
| sqlglot_transpile          | 1.53 ms                                                         | 1.37 ms: 1.12x faster                                                   |
| unpickle_pure_python       | 210 us                                                          | 190 us: 1.10x faster                                                    |
| bench_thread_pool          | 1.10 ms                                                         | 999 us: 1.10x faster                                                    |
| 2to3                       | 280 ms                                                          | 255 ms: 1.10x faster                                                    |
| sympy_integrate            | 17.5 ms                                                         | 16.0 ms: 1.10x faster                                                   |
| fannkuch                   | 354 ms                                                          | 324 ms: 1.09x faster                                                    |
| regex_dna                  | 127 ms                                                          | 116 ms: 1.09x faster                                                    |
| meteor_contest             | 86.9 ms                                                         | 80.1 ms: 1.08x faster                                                   |
| sympy_str                  | 240 ms                                                          | 222 ms: 1.08x faster                                                    |
| regex_effbot               | 2.04 ms                                                         | 1.91 ms: 1.07x faster                                                   |
| pickle_pure_python         | 286 us                                                          | 268 us: 1.06x faster                                                    |
| pprint_safe_repr           | 721 ms                                                          | 679 ms: 1.06x faster                                                    |
| sqlite_synth               | 2.07 us                                                         | 1.95 us: 1.06x faster                                                   |
| pathlib                    | 91.4 ms                                                         | 86.5 ms: 1.06x faster                                                   |
| xml_etree_parse            | 113 ms                                                          | 107 ms: 1.06x faster                                                    |
| django_template            | 36.9 ms                                                         | 35.2 ms: 1.05x faster                                                   |
| sqlglot_optimize           | 48.5 ms                                                         | 46.4 ms: 1.04x faster                                                   |
| docutils                   | 1.98 sec                                                        | 1.90 sec: 1.04x faster                                                  |
| pprint_pformat             | 1.50 sec                                                        | 1.44 sec: 1.04x faster                                                  |
| xml_etree_generate         | 72.1 ms                                                         | 69.3 ms: 1.04x faster                                                   |
| asyncio_tcp_ssl            | 17.7 sec                                                        | 17.1 sec: 1.03x faster                                                  |
| bench_mp_pool              | 75.4 ms                                                         | 73.1 ms: 1.03x faster                                                   |
| async_generators           | 313 ms                                                          | 305 ms: 1.03x faster                                                    |
| pickle_list                | 3.37 us                                                         | 3.31 us: 1.02x faster                                                   |
| xml_etree_process          | 53.2 ms                                                         | 52.5 ms: 1.01x faster                                                   |
| richards                   | 41.3 ms                                                         | 40.8 ms: 1.01x faster                                                   |
| sympy_expand               | 398 ms                                                          | 394 ms: 1.01x faster                                                    |
| gc_traversal               | 1.44 ms                                                         | 1.45 ms: 1.01x slower                                                   |
| pidigits                   | 199 ms                                                          | 201 ms: 1.01x slower                                                    |
| richards_super             | 46.5 ms                                                         | 47.0 ms: 1.01x slower                                                   |
| pickle_dict                | 19.9 us                                                         | 20.3 us: 1.02x slower                                                   |
| pickle                     | 7.79 us                                                         | 7.95 us: 1.02x slower                                                   |
| unpickle_list              | 2.95 us                                                         | 3.02 us: 1.03x slower                                                   |
| json_loads                 | 20.4 us                                                         | 20.9 us: 1.03x slower                                                   |
| python_startup_no_site     | 19.1 ms                                                         | 19.7 ms: 1.03x slower                                                   |
| json                       | 4.15 ms                                                         | 4.35 ms: 1.05x slower                                                   |
| json_dumps                 | 7.40 ms                                                         | 7.75 ms: 1.05x slower                                                   |
| unpickle                   | 9.71 us                                                         | 10.2 us: 1.05x slower                                                   |
| python_startup             | 22.4 ms                                                         | 23.8 ms: 1.07x slower                                                   |
| regex_v8                   | 15.0 ms                                                         | 16.1 ms: 1.07x slower                                                   |
| coverage                   | 48.4 ms                                                         | 54.0 ms: 1.12x slower                                                   |
| typing_runtime_protocols   | 126 us                                                          | 146 us: 1.16x slower                                                    |
| create_gc_cycles           | 652 us                                                          | 764 us: 1.17x slower                                                    |
| telco                      | 5.51 ms                                                         | 6.85 ms: 1.24x slower                                                   |
| sqlglot_normalize          | 100 ms                                                          | 240 ms: 2.39x slower                                                    |
| Geometric mean             | (ref)                                                           | 1.10x faster                                                            |

Benchmark hidden because not significant (2): tornado_http, asyncio_tcp
Ignored benchmarks (6) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1_win32-x86-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, chameleon, dask, mypy2, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (5) of results/bm-20241007-3.14.0a0-0e19ac7/bm-20241007-pythonperf1_win32-x86-mdboom-marshal_slice-3.14.0a0-0e19ac7.json: genshi_text, genshi_xml, html5lib, pylint, thrift

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.08x

# Memory
- memory change: unknown