# Results vs. 3.13.0b2

- fork: faster-cpython
- ref: monitoring_branch_ta
- machine: windows-amd64
- commit hash: 1b65c46
- commit date: 2024-08-01
- overall geometric mean: 1.01x slower
- HPT reliability: 99.74%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| 2to3           | 207 ms                                                          | 240 ms: 1.16x slower                                                                 |
| docutils       | 1.63 sec                                                        | 1.83 sec: 1.13x slower                                                               |
| html5lib       | 35.0 ms                                                         | 41.9 ms: 1.20x slower                                                                |
| tornado_http   | 81.8 ms                                                         | 96.0 ms: 1.17x slower                                                                |
| Geometric mean | (ref)                                                           | 1.16x slower                                                                         |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|-------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| async_tree_io_tg        | 605 ms                                                          | 541 ms: 1.12x faster                                                                 |
| async_tree_io           | 588 ms                                                          | 561 ms: 1.05x faster                                                                 |
| async_tree_none_tg      | 202 ms                                                          | 193 ms: 1.05x faster                                                                 |
| async_tree_cpu_io_mixed | 389 ms                                                          | 396 ms: 1.02x slower                                                                 |
| Geometric mean          | (ref)                                                           | 1.03x faster                                                                         |

Benchmark hidden because not significant (4): async_tree_memoization_tg, async_tree_none, async_tree_cpu_io_mixed_tg, async_tree_memoization

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| nbody          | 67.6 ms                                                         | 49.7 ms: 1.36x faster                                                                |
| float          | 49.7 ms                                                         | 45.3 ms: 1.10x faster                                                                |
| Geometric mean | (ref)                                                           | 1.14x faster                                                                         |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| regex_v8       | 15.8 ms                                                         | 14.4 ms: 1.10x faster                                                                |
| regex_dna      | 119 ms                                                          | 116 ms: 1.03x faster                                                                 |
| regex_effbot   | 1.58 ms                                                         | 1.56 ms: 1.01x faster                                                                |
| regex_compile  | 78.0 ms                                                         | 90.5 ms: 1.16x slower                                                                |
| Geometric mean | (ref)                                                           | 1.00x slower                                                                         |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|----------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| tomli_loads          | 1.35 sec                                                        | 1.28 sec: 1.06x faster                                                               |
| xml_etree_generate   | 53.2 ms                                                         | 51.4 ms: 1.03x faster                                                                |
| xml_etree_iterparse  | 62.3 ms                                                         | 61.2 ms: 1.02x faster                                                                |
| xml_etree_parse      | 90.9 ms                                                         | 92.8 ms: 1.02x slower                                                                |
| xml_etree_process    | 36.4 ms                                                         | 37.3 ms: 1.02x slower                                                                |
| pickle_pure_python   | 175 us                                                          | 187 us: 1.07x slower                                                                 |
| json_dumps           | 5.61 ms                                                         | 6.10 ms: 1.09x slower                                                                |
| unpickle_pure_python | 122 us                                                          | 135 us: 1.11x slower                                                                 |
| Geometric mean       | (ref)                                                           | 1.02x slower                                                                         |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| python_startup         | 20.3 ms                                                         | 22.7 ms: 1.12x slower                                                                |
| python_startup_no_site | 16.2 ms                                                         | 18.7 ms: 1.15x slower                                                                |
| Geometric mean         | (ref)                                                           | 1.13x slower                                                                         |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|-----------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| mako            | 6.36 ms                                                         | 5.10 ms: 1.25x faster                                                                |
| django_template | 21.7 ms                                                         | 25.6 ms: 1.18x slower                                                                |
| genshi_xml      | 31.6 ms                                                         | 38.6 ms: 1.22x slower                                                                |
| genshi_text     | 14.4 ms                                                         | 17.8 ms: 1.24x slower                                                                |
| Geometric mean  | (ref)                                                           | 1.09x slower                                                                         |

All benchmarks:
===============

| Benchmark                | bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240801-pythonperf1-amd64-faster%2dcpython-monitoring_branch_ta-3.14.0a0-1b65c46 |
|--------------------------|:---------------------------------------------------------------:|:------------------------------------------------------------------------------------:|
| thrift                   | 8.11 ms                                                         | 519 us: 15.63x faster                                                                |
| spectral_norm            | 63.7 ms                                                         | 45.7 ms: 1.40x faster                                                                |
| deepcopy_memo            | 22.1 us                                                         | 16.1 us: 1.37x faster                                                                |
| nbody                    | 67.6 ms                                                         | 49.7 ms: 1.36x faster                                                                |
| mako                     | 6.36 ms                                                         | 5.10 ms: 1.25x faster                                                                |
| scimark_sparse_mat_mult  | 2.50 ms                                                         | 2.04 ms: 1.23x faster                                                                |
| deepcopy                 | 220 us                                                          | 187 us: 1.17x faster                                                                 |
| scimark_fft              | 171 ms                                                          | 147 ms: 1.17x faster                                                                 |
| async_tree_io_tg         | 605 ms                                                          | 541 ms: 1.12x faster                                                                 |
| crypto_pyaes             | 45.5 ms                                                         | 40.8 ms: 1.11x faster                                                                |
| fannkuch                 | 243 ms                                                          | 219 ms: 1.11x faster                                                                 |
| pyflate                  | 279 ms                                                          | 253 ms: 1.10x faster                                                                 |
| regex_v8                 | 15.8 ms                                                         | 14.4 ms: 1.10x faster                                                                |
| float                    | 49.7 ms                                                         | 45.3 ms: 1.10x faster                                                                |
| pycparser                | 754 ms                                                          | 695 ms: 1.08x faster                                                                 |
| deepcopy_reduce          | 1.99 us                                                         | 1.85 us: 1.08x faster                                                                |
| tomli_loads              | 1.35 sec                                                        | 1.28 sec: 1.06x faster                                                               |
| asyncio_tcp_ssl          | 1.48 sec                                                        | 1.41 sec: 1.05x faster                                                               |
| async_tree_io            | 588 ms                                                          | 561 ms: 1.05x faster                                                                 |
| json                     | 3.19 ms                                                         | 3.04 ms: 1.05x faster                                                                |
| async_tree_none_tg       | 202 ms                                                          | 193 ms: 1.05x faster                                                                 |
| telco                    | 4.67 ms                                                         | 4.51 ms: 1.04x faster                                                                |
| xml_etree_generate       | 53.2 ms                                                         | 51.4 ms: 1.03x faster                                                                |
| regex_dna                | 119 ms                                                          | 116 ms: 1.03x faster                                                                 |
| scimark_monte_carlo      | 39.1 ms                                                         | 38.2 ms: 1.02x faster                                                                |
| xml_etree_iterparse      | 62.3 ms                                                         | 61.2 ms: 1.02x faster                                                                |
| regex_effbot             | 1.58 ms                                                         | 1.56 ms: 1.01x faster                                                                |
| gc_traversal             | 1.55 ms                                                         | 1.56 ms: 1.01x slower                                                                |
| pprint_safe_repr         | 474 ms                                                          | 477 ms: 1.01x slower                                                                 |
| pprint_pformat           | 966 ms                                                          | 973 ms: 1.01x slower                                                                 |
| async_tree_cpu_io_mixed  | 389 ms                                                          | 396 ms: 1.02x slower                                                                 |
| meteor_contest           | 69.9 ms                                                         | 71.1 ms: 1.02x slower                                                                |
| xml_etree_parse          | 90.9 ms                                                         | 92.8 ms: 1.02x slower                                                                |
| create_gc_cycles         | 888 us                                                          | 907 us: 1.02x slower                                                                 |
| xml_etree_process        | 36.4 ms                                                         | 37.3 ms: 1.02x slower                                                                |
| logging_format           | 6.22 us                                                         | 6.45 us: 1.04x slower                                                                |
| chaos                    | 38.4 ms                                                         | 39.9 ms: 1.04x slower                                                                |
| logging_simple           | 5.78 us                                                         | 6.03 us: 1.04x slower                                                                |
| bench_thread_pool        | 768 us                                                          | 807 us: 1.05x slower                                                                 |
| pickle_pure_python       | 175 us                                                          | 187 us: 1.07x slower                                                                 |
| pathlib                  | 75.9 ms                                                         | 81.1 ms: 1.07x slower                                                                |
| coroutines               | 12.8 ms                                                         | 13.8 ms: 1.08x slower                                                                |
| logging_silent           | 52.9 ns                                                         | 57.3 ns: 1.08x slower                                                                |
| json_dumps               | 5.61 ms                                                         | 6.10 ms: 1.09x slower                                                                |
| unpickle_pure_python     | 122 us                                                          | 135 us: 1.11x slower                                                                 |
| python_startup           | 20.3 ms                                                         | 22.7 ms: 1.12x slower                                                                |
| coverage                 | 42.1 ms                                                         | 47.0 ms: 1.12x slower                                                                |
| sqlglot_parse            | 748 us                                                          | 837 us: 1.12x slower                                                                 |
| nqueens                  | 56.7 ms                                                         | 63.5 ms: 1.12x slower                                                                |
| sqlglot_normalize        | 173 ms                                                          | 195 ms: 1.13x slower                                                                 |
| docutils                 | 1.63 sec                                                        | 1.83 sec: 1.13x slower                                                               |
| sqlglot_optimize         | 32.7 ms                                                         | 37.0 ms: 1.13x slower                                                                |
| sqlglot_transpile        | 955 us                                                          | 1.08 ms: 1.13x slower                                                                |
| dulwich_log              | 38.0 ms                                                         | 43.2 ms: 1.13x slower                                                                |
| generators               | 19.6 ms                                                         | 22.2 ms: 1.14x slower                                                                |
| typing_runtime_protocols | 101 us                                                          | 116 us: 1.15x slower                                                                 |
| asyncio_tcp              | 471 ms                                                          | 542 ms: 1.15x slower                                                                 |
| python_startup_no_site   | 16.2 ms                                                         | 18.7 ms: 1.15x slower                                                                |
| async_generators         | 223 ms                                                          | 258 ms: 1.16x slower                                                                 |
| regex_compile            | 78.0 ms                                                         | 90.5 ms: 1.16x slower                                                                |
| sympy_sum                | 84.4 ms                                                         | 98.0 ms: 1.16x slower                                                                |
| 2to3                     | 207 ms                                                          | 240 ms: 1.16x slower                                                                 |
| richards                 | 26.7 ms                                                         | 31.3 ms: 1.17x slower                                                                |
| tornado_http             | 81.8 ms                                                         | 96.0 ms: 1.17x slower                                                                |
| django_template          | 21.7 ms                                                         | 25.6 ms: 1.18x slower                                                                |
| richards_super           | 30.2 ms                                                         | 35.6 ms: 1.18x slower                                                                |
| mdp                      | 1.31 sec                                                        | 1.55 sec: 1.18x slower                                                               |
| bench_mp_pool            | 64.8 ms                                                         | 76.7 ms: 1.18x slower                                                                |
| sympy_str                | 159 ms                                                          | 190 ms: 1.19x slower                                                                 |
| html5lib                 | 35.0 ms                                                         | 41.9 ms: 1.20x slower                                                                |
| raytrace                 | 162 ms                                                          | 195 ms: 1.20x slower                                                                 |
| sympy_expand             | 271 ms                                                          | 329 ms: 1.21x slower                                                                 |
| sympy_integrate          | 12.2 ms                                                         | 14.9 ms: 1.21x slower                                                                |
| scimark_sor              | 75.3 ms                                                         | 92.0 ms: 1.22x slower                                                                |
| genshi_xml               | 31.6 ms                                                         | 38.6 ms: 1.22x slower                                                                |
| go                       | 82.1 ms                                                         | 101 ms: 1.24x slower                                                                 |
| pylint                   | 205 ms                                                          | 253 ms: 1.24x slower                                                                 |
| genshi_text              | 14.4 ms                                                         | 17.8 ms: 1.24x slower                                                                |
| deltablue                | 1.88 ms                                                         | 2.37 ms: 1.26x slower                                                                |
| scimark_lu               | 56.6 ms                                                         | 71.5 ms: 1.26x slower                                                                |
| hexiom                   | 3.72 ms                                                         | 4.77 ms: 1.28x slower                                                                |
| Geometric mean           | (ref)                                                           | 1.01x slower                                                                         |

Benchmark hidden because not significant (7): async_tree_memoization_tg, async_tree_none, async_tree_cpu_io_mixed_tg, json_loads, comprehensions, pidigits, async_tree_memoization
Ignored benchmarks (10) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf1-amd64-python-v3.13.0b2-3.13.0b2-3a83b17.json: aiohttp, chameleon, flaskblogging, mypy2, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list

# HPT report

- Reliability score: 99.74% likely to be slow
- 90% likely to have a slowdown of 1.02x
- 95% likely to have a slowdown of 1.01x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown