
# Results vs. 3.10.4

- fork: python
- ref: 878ead1ac16519651263
- machine: windows-amd64
- commit hash: 878ead1
- commit date: 2023-02-07
- overall geometric mean: 1.12x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 202 ms: 1.17x faster                                                     |
| chameleon      | 5.92 ms                                                     | 5.17 ms: 1.15x faster                                                    |
| docutils       | 1.89 sec                                                    | 1.57 sec: 1.21x faster                                                   |
| html5lib       | 46.5 ms                                                     | 38.5 ms: 1.21x faster                                                    |
| tornado_http   | 109 ms                                                      | 91.4 ms: 1.19x faster                                                    |
| Geometric mean | (ref)                                                       | 1.18x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 60.2 ms                                                     | 52.6 ms: 1.15x faster                                                    |
| pidigits       | 145 ms                                                      | 147 ms: 1.01x slower                                                     |
| Geometric mean | (ref)                                                       | 1.04x faster                                                             |

Benchmark hidden because not significant (1): nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 89.8 ms: 1.15x faster                                                    |
| regex_dna      | 132 ms                                                      | 118 ms: 1.12x faster                                                     |
| regex_v8       | 15.0 ms                                                     | 13.8 ms: 1.09x faster                                                    |
| regex_effbot   | 1.66 ms                                                     | 1.77 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                       | 1.07x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|----------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| pickle_pure_python   | 257 us                                                      | 196 us: 1.31x faster                                                     |
| xml_etree_process    | 43.4 ms                                                     | 36.7 ms: 1.18x faster                                                    |
| unpickle_pure_python | 171 us                                                      | 148 us: 1.16x faster                                                     |
| unpickle             | 9.17 us                                                     | 7.96 us: 1.15x faster                                                    |
| xml_etree_parse      | 102 ms                                                      | 92.2 ms: 1.10x faster                                                    |
| json_dumps           | 8.50 ms                                                     | 7.72 ms: 1.10x faster                                                    |
| unpickle_list        | 2.81 us                                                     | 2.67 us: 1.05x faster                                                    |
| pickle               | 6.80 us                                                     | 6.46 us: 1.05x faster                                                    |
| xml_etree_generate   | 54.5 ms                                                     | 52.2 ms: 1.04x faster                                                    |
| json_loads           | 14.2 us                                                     | 13.6 us: 1.04x faster                                                    |
| xml_etree_iterparse  | 63.5 ms                                                     | 61.6 ms: 1.03x faster                                                    |
| pickle_list          | 2.59 us                                                     | 2.67 us: 1.03x slower                                                    |
| pickle_dict          | 16.9 us                                                     | 18.2 us: 1.08x slower                                                    |
| Geometric mean       | (ref)                                                       | 1.08x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.7 ms: 1.07x faster                                                    |
| python_startup_no_site | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                                    |
| Geometric mean         | (ref)                                                       | 1.03x faster                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|-----------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 8.80 ms                                                     | 7.30 ms: 1.21x faster                                                    |
| django_template | 28.2 ms                                                     | 24.5 ms: 1.15x faster                                                    |
| genshi_text     | 19.0 ms                                                     | 17.2 ms: 1.11x faster                                                    |
| genshi_xml      | 40.5 ms                                                     | 38.1 ms: 1.07x faster                                                    |
| Geometric mean  | (ref)                                                       | 1.13x faster                                                             |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230207-pythonperf1-amd64-python-878ead1ac16519651263-3.11.2-878ead1 |
|-------------------------|:-----------------------------------------------------------:|:------------------------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 2.62 ms: 1.59x faster                                                    |
| async_tree_io           | 1.07 sec                                                    | 748 ms: 1.42x faster                                                     |
| go                      | 136 ms                                                      | 97.7 ms: 1.39x faster                                                    |
| scimark_sor             | 105 ms                                                      | 75.6 ms: 1.39x faster                                                    |
| logging_silent          | 96.4 ns                                                     | 70.2 ns: 1.37x faster                                                    |
| raytrace                | 271 ms                                                      | 199 ms: 1.36x faster                                                     |
| async_tree_none         | 420 ms                                                      | 311 ms: 1.35x faster                                                     |
| richards                | 41.2 ms                                                     | 30.8 ms: 1.33x faster                                                    |
| scimark_lu              | 85.4 ms                                                     | 64.2 ms: 1.33x faster                                                    |
| sqlglot_parse           | 1.22 ms                                                     | 921 us: 1.32x faster                                                     |
| sqlglot_transpile       | 1.46 ms                                                     | 1.12 ms: 1.31x faster                                                    |
| pickle_pure_python      | 257 us                                                      | 196 us: 1.31x faster                                                     |
| thrift                  | 615 us                                                      | 470 us: 1.31x faster                                                     |
| pyflate                 | 387 ms                                                      | 300 ms: 1.29x faster                                                     |
| async_tree_memoization  | 497 ms                                                      | 386 ms: 1.29x faster                                                     |
| async_generators        | 224 ms                                                      | 175 ms: 1.28x faster                                                     |
| crypto_pyaes            | 62.3 ms                                                     | 48.7 ms: 1.28x faster                                                    |
| mypy2                   | 352 ms                                                      | 275 ms: 1.28x faster                                                     |
| scimark_monte_carlo     | 55.9 ms                                                     | 44.3 ms: 1.26x faster                                                    |
| pycparser               | 868 ms                                                      | 697 ms: 1.25x faster                                                     |
| hexiom                  | 5.52 ms                                                     | 4.48 ms: 1.23x faster                                                    |
| async_tree_cpu_io_mixed | 609 ms                                                      | 496 ms: 1.23x faster                                                     |
| docutils                | 1.89 sec                                                    | 1.57 sec: 1.21x faster                                                   |
| mako                    | 8.80 ms                                                     | 7.30 ms: 1.21x faster                                                    |
| html5lib                | 46.5 ms                                                     | 38.5 ms: 1.21x faster                                                    |
| chaos                   | 58.9 ms                                                     | 49.0 ms: 1.20x faster                                                    |
| tornado_http            | 109 ms                                                      | 91.4 ms: 1.19x faster                                                    |
| xml_etree_process       | 43.4 ms                                                     | 36.7 ms: 1.18x faster                                                    |
| create_gc_cycles        | 782 us                                                      | 663 us: 1.18x faster                                                     |
| aiohttp                 | 1.01 ms                                                     | 857 us: 1.18x faster                                                     |
| 2to3                    | 237 ms                                                      | 202 ms: 1.17x faster                                                     |
| sqlalchemy_declarative  | 95.4 ms                                                     | 82.2 ms: 1.16x faster                                                    |
| unpickle_pure_python    | 171 us                                                      | 148 us: 1.16x faster                                                     |
| unpickle                | 9.17 us                                                     | 7.96 us: 1.15x faster                                                    |
| regex_compile           | 103 ms                                                      | 89.8 ms: 1.15x faster                                                    |
| django_template         | 28.2 ms                                                     | 24.5 ms: 1.15x faster                                                    |
| deepcopy_memo           | 28.5 us                                                     | 24.8 us: 1.15x faster                                                    |
| chameleon               | 5.92 ms                                                     | 5.17 ms: 1.15x faster                                                    |
| float                   | 60.2 ms                                                     | 52.6 ms: 1.15x faster                                                    |
| dask                    | 305 ms                                                      | 269 ms: 1.13x faster                                                     |
| pprint_pformat          | 1.21 sec                                                    | 1.07 sec: 1.13x faster                                                   |
| regex_dna               | 132 ms                                                      | 118 ms: 1.12x faster                                                     |
| pprint_safe_repr        | 589 ms                                                      | 525 ms: 1.12x faster                                                     |
| sqlglot_optimize        | 39.0 ms                                                     | 34.8 ms: 1.12x faster                                                    |
| genshi_text             | 19.0 ms                                                     | 17.2 ms: 1.11x faster                                                    |
| xml_etree_parse         | 102 ms                                                      | 92.2 ms: 1.10x faster                                                    |
| json_dumps              | 8.50 ms                                                     | 7.72 ms: 1.10x faster                                                    |
| bench_thread_pool       | 946 us                                                      | 861 us: 1.10x faster                                                     |
| spectral_norm           | 78.0 ms                                                     | 71.4 ms: 1.09x faster                                                    |
| pylint                  | 347 ms                                                      | 318 ms: 1.09x faster                                                     |
| sympy_integrate         | 14.8 ms                                                     | 13.6 ms: 1.09x faster                                                    |
| regex_v8                | 15.0 ms                                                     | 13.8 ms: 1.09x faster                                                    |
| sqlite_synth            | 1.84 us                                                     | 1.70 us: 1.08x faster                                                    |
| coroutines              | 15.9 ms                                                     | 14.7 ms: 1.08x faster                                                    |
| python_startup          | 20.0 ms                                                     | 18.7 ms: 1.07x faster                                                    |
| deepcopy                | 255 us                                                      | 239 us: 1.07x faster                                                     |
| genshi_xml              | 40.5 ms                                                     | 38.1 ms: 1.07x faster                                                    |
| deepcopy_reduce         | 2.16 us                                                     | 2.03 us: 1.06x faster                                                    |
| dulwich_log             | 47.6 ms                                                     | 44.8 ms: 1.06x faster                                                    |
| sympy_expand            | 315 ms                                                      | 297 ms: 1.06x faster                                                     |
| sqlalchemy_imperative   | 11.0 ms                                                     | 10.3 ms: 1.06x faster                                                    |
| sympy_sum               | 104 ms                                                      | 98.1 ms: 1.06x faster                                                    |
| pathlib                 | 77.4 ms                                                     | 73.2 ms: 1.06x faster                                                    |
| sqlglot_normalize       | 202 ms                                                      | 191 ms: 1.06x faster                                                     |
| unpickle_list           | 2.81 us                                                     | 2.67 us: 1.05x faster                                                    |
| pickle                  | 6.80 us                                                     | 6.46 us: 1.05x faster                                                    |
| json                    | 3.05 ms                                                     | 2.89 ms: 1.05x faster                                                    |
| comprehensions          | 16.0 us                                                     | 15.2 us: 1.05x faster                                                    |
| xml_etree_generate      | 54.5 ms                                                     | 52.2 ms: 1.04x faster                                                    |
| asyncio_tcp             | 712 ms                                                      | 685 ms: 1.04x faster                                                     |
| mdp                     | 1.71 sec                                                    | 1.65 sec: 1.04x faster                                                   |
| json_loads              | 14.2 us                                                     | 13.6 us: 1.04x faster                                                    |
| xml_etree_iterparse     | 63.5 ms                                                     | 61.6 ms: 1.03x faster                                                    |
| sympy_str               | 188 ms                                                      | 184 ms: 1.03x faster                                                     |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.61 ms: 1.02x faster                                                    |
| scimark_fft             | 193 ms                                                      | 191 ms: 1.01x faster                                                     |
| pidigits                | 145 ms                                                      | 147 ms: 1.01x slower                                                     |
| python_startup_no_site  | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                                    |
| meteor_contest          | 72.5 ms                                                     | 73.8 ms: 1.02x slower                                                    |
| pickle_list             | 2.59 us                                                     | 2.67 us: 1.03x slower                                                    |
| telco                   | 3.78 ms                                                     | 3.93 ms: 1.04x slower                                                    |
| generators              | 31.6 ms                                                     | 33.1 ms: 1.05x slower                                                    |
| gc_traversal            | 1.34 ms                                                     | 1.42 ms: 1.05x slower                                                    |
| logging_format          | 6.66 us                                                     | 7.06 us: 1.06x slower                                                    |
| regex_effbot            | 1.66 ms                                                     | 1.77 ms: 1.06x slower                                                    |
| bench_mp_pool           | 60.7 ms                                                     | 64.8 ms: 1.07x slower                                                    |
| logging_simple          | 6.20 us                                                     | 6.64 us: 1.07x slower                                                    |
| pickle_dict             | 16.9 us                                                     | 18.2 us: 1.08x slower                                                    |
| unpack_sequence         | 37.8 ns                                                     | 42.9 ns: 1.13x slower                                                    |
| coverage                | 40.0 ms                                                     | 53.3 ms: 1.33x slower                                                    |
| Geometric mean          | (ref)                                                       | 1.12x faster                                                             |

Benchmark hidden because not significant (4): fannkuch, flaskblogging, nbody, nqueens
Ignored benchmarks (4) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: asyncio_tcp_ssl, richards_super, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.08x
