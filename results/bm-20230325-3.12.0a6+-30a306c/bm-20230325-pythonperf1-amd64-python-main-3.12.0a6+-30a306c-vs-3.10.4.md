
# Results vs. 3.10.4

- fork: python
- ref: main
- machine: windows-amd64
- commit hash: 30a306c
- commit date: 2023-03-25
- overall geometric mean: 1.22x faster \*
- HPT reliability: 100.00%
- HPT 99th percentile: 1.17x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| 2to3           | 237 ms                                                      | 198 ms: 1.20x faster                                        |
| chameleon      | 5.92 ms                                                     | 4.57 ms: 1.30x faster                                       |
| docutils       | 1.89 sec                                                    | 1.52 sec: 1.25x faster                                      |
| html5lib       | 46.5 ms                                                     | 35.5 ms: 1.31x faster                                       |
| tornado_http   | 109 ms                                                      | 87.7 ms: 1.24x faster                                       |
| Geometric mean | (ref)                                                       | 1.26x faster                                                |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| float          | 60.2 ms                                                     | 48.6 ms: 1.24x faster                                       |
| nbody          | 69.3 ms                                                     | 56.6 ms: 1.23x faster                                       |
| pidigits       | 145 ms                                                      | 145 ms: 1.00x faster                                        |
| Geometric mean | (ref)                                                       | 1.15x faster                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| regex_compile  | 103 ms                                                      | 82.3 ms: 1.26x faster                                       |
| regex_dna      | 132 ms                                                      | 119 ms: 1.11x faster                                        |
| regex_v8       | 15.0 ms                                                     | 13.6 ms: 1.10x faster                                       |
| regex_effbot   | 1.66 ms                                                     | 1.77 ms: 1.06x slower                                       |
| Geometric mean | (ref)                                                       | 1.10x faster                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|----------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| json_dumps           | 8.50 ms                                                     | 5.58 ms: 1.52x faster                                       |
| pickle_pure_python   | 257 us                                                      | 176 us: 1.46x faster                                        |
| unpickle_pure_python | 171 us                                                      | 122 us: 1.40x faster                                        |
| xml_etree_process    | 43.4 ms                                                     | 35.3 ms: 1.23x faster                                       |
| unpickle             | 9.17 us                                                     | 7.97 us: 1.15x faster                                       |
| xml_etree_parse      | 102 ms                                                      | 89.5 ms: 1.14x faster                                       |
| json_loads           | 14.2 us                                                     | 13.4 us: 1.06x faster                                       |
| unpickle_list        | 2.81 us                                                     | 2.66 us: 1.06x faster                                       |
| xml_etree_generate   | 54.5 ms                                                     | 51.7 ms: 1.05x faster                                       |
| xml_etree_iterparse  | 63.5 ms                                                     | 60.4 ms: 1.05x faster                                       |
| pickle               | 6.80 us                                                     | 7.04 us: 1.04x slower                                       |
| pickle_dict          | 16.9 us                                                     | 18.7 us: 1.11x slower                                       |
| pickle_list          | 2.59 us                                                     | 2.91 us: 1.13x slower                                       |
| Geometric mean       | (ref)                                                       | 1.13x faster                                                |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|------------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| python_startup         | 20.0 ms                                                     | 18.6 ms: 1.07x faster                                       |
| python_startup_no_site | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                       |
| Geometric mean         | (ref)                                                       | 1.03x faster                                                |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|-----------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| genshi_text     | 19.0 ms                                                     | 13.6 ms: 1.40x faster                                       |
| mako            | 8.80 ms                                                     | 6.32 ms: 1.39x faster                                       |
| django_template | 28.2 ms                                                     | 20.6 ms: 1.37x faster                                       |
| genshi_xml      | 40.5 ms                                                     | 30.3 ms: 1.34x faster                                       |
| Geometric mean  | (ref)                                                       | 1.37x faster                                                |

All benchmarks:
===============

| Benchmark               | bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120 | bm-20230325-pythonperf1-amd64-python-main-3.12.0a6+-30a306c |
|-------------------------|:-----------------------------------------------------------:|:-----------------------------------------------------------:|
| deltablue               | 4.17 ms                                                     | 1.94 ms: 2.15x faster                                       |
| logging_silent          | 96.4 ns                                                     | 54.4 ns: 1.77x faster                                       |
| mypy2                   | 352 ms                                                      | 212 ms: 1.66x faster                                        |
| go                      | 136 ms                                                      | 82.6 ms: 1.65x faster                                       |
| richards                | 41.2 ms                                                     | 25.1 ms: 1.64x faster                                       |
| scimark_lu              | 85.4 ms                                                     | 52.6 ms: 1.63x faster                                       |
| raytrace                | 271 ms                                                      | 171 ms: 1.59x faster                                        |
| asyncio_tcp             | 712 ms                                                      | 467 ms: 1.52x faster                                        |
| json_dumps              | 8.50 ms                                                     | 5.58 ms: 1.52x faster                                       |
| generators              | 31.6 ms                                                     | 21.0 ms: 1.51x faster                                       |
| chaos                   | 58.9 ms                                                     | 40.2 ms: 1.47x faster                                       |
| pickle_pure_python      | 257 us                                                      | 176 us: 1.46x faster                                        |
| async_tree_io           | 1.07 sec                                                    | 731 ms: 1.46x faster                                        |
| hexiom                  | 5.52 ms                                                     | 3.80 ms: 1.45x faster                                       |
| async_tree_none         | 420 ms                                                      | 295 ms: 1.43x faster                                        |
| scimark_monte_carlo     | 55.9 ms                                                     | 39.5 ms: 1.42x faster                                       |
| sqlglot_parse           | 1.22 ms                                                     | 864 us: 1.41x faster                                        |
| async_tree_memoization  | 497 ms                                                      | 355 ms: 1.40x faster                                        |
| unpickle_pure_python    | 171 us                                                      | 122 us: 1.40x faster                                        |
| genshi_text             | 19.0 ms                                                     | 13.6 ms: 1.40x faster                                       |
| mako                    | 8.80 ms                                                     | 6.32 ms: 1.39x faster                                       |
| sqlglot_transpile       | 1.46 ms                                                     | 1.05 ms: 1.39x faster                                       |
| unpack_sequence         | 37.8 ns                                                     | 27.2 ns: 1.39x faster                                       |
| crypto_pyaes            | 62.3 ms                                                     | 45.0 ms: 1.39x faster                                       |
| pyflate                 | 387 ms                                                      | 281 ms: 1.38x faster                                        |
| scimark_sor             | 105 ms                                                      | 76.4 ms: 1.37x faster                                       |
| django_template         | 28.2 ms                                                     | 20.6 ms: 1.37x faster                                       |
| pycparser               | 868 ms                                                      | 641 ms: 1.35x faster                                        |
| genshi_xml              | 40.5 ms                                                     | 30.3 ms: 1.34x faster                                       |
| thrift                  | 615 us                                                      | 461 us: 1.34x faster                                        |
| deepcopy_memo           | 28.5 us                                                     | 21.5 us: 1.33x faster                                       |
| html5lib                | 46.5 ms                                                     | 35.5 ms: 1.31x faster                                       |
| chameleon               | 5.92 ms                                                     | 4.57 ms: 1.30x faster                                       |
| pprint_safe_repr        | 589 ms                                                      | 458 ms: 1.29x faster                                        |
| async_tree_cpu_io_mixed | 609 ms                                                      | 475 ms: 1.28x faster                                        |
| pprint_pformat          | 1.21 sec                                                    | 941 ms: 1.28x faster                                        |
| regex_compile           | 103 ms                                                      | 82.3 ms: 1.26x faster                                       |
| mdp                     | 1.71 sec                                                    | 1.37 sec: 1.25x faster                                      |
| docutils                | 1.89 sec                                                    | 1.52 sec: 1.25x faster                                      |
| tornado_http            | 109 ms                                                      | 87.7 ms: 1.24x faster                                       |
| float                   | 60.2 ms                                                     | 48.6 ms: 1.24x faster                                       |
| sqlglot_optimize        | 39.0 ms                                                     | 31.5 ms: 1.24x faster                                       |
| xml_etree_process       | 43.4 ms                                                     | 35.3 ms: 1.23x faster                                       |
| nbody                   | 69.3 ms                                                     | 56.6 ms: 1.23x faster                                       |
| deepcopy                | 255 us                                                      | 212 us: 1.20x faster                                        |
| 2to3                    | 237 ms                                                      | 198 ms: 1.20x faster                                        |
| spectral_norm           | 78.0 ms                                                     | 65.4 ms: 1.19x faster                                       |
| sqlglot_normalize       | 202 ms                                                      | 171 ms: 1.18x faster                                        |
| nqueens                 | 67.0 ms                                                     | 56.7 ms: 1.18x faster                                       |
| scimark_sparse_mat_mult | 2.66 ms                                                     | 2.25 ms: 1.18x faster                                       |
| deepcopy_reduce         | 2.16 us                                                     | 1.83 us: 1.18x faster                                       |
| scimark_fft             | 193 ms                                                      | 165 ms: 1.17x faster                                        |
| create_gc_cycles        | 782 us                                                      | 671 us: 1.17x faster                                        |
| sympy_integrate         | 14.8 ms                                                     | 12.8 ms: 1.15x faster                                       |
| unpickle                | 9.17 us                                                     | 7.97 us: 1.15x faster                                       |
| xml_etree_parse         | 102 ms                                                      | 89.5 ms: 1.14x faster                                       |
| fannkuch                | 258 ms                                                      | 227 ms: 1.14x faster                                        |
| bench_thread_pool       | 946 us                                                      | 833 us: 1.14x faster                                        |
| sympy_expand            | 315 ms                                                      | 278 ms: 1.13x faster                                        |
| json                    | 3.05 ms                                                     | 2.75 ms: 1.11x faster                                       |
| coroutines              | 15.9 ms                                                     | 14.4 ms: 1.11x faster                                       |
| dulwich_log             | 47.6 ms                                                     | 43.0 ms: 1.11x faster                                       |
| regex_dna               | 132 ms                                                      | 119 ms: 1.11x faster                                        |
| regex_v8                | 15.0 ms                                                     | 13.6 ms: 1.10x faster                                       |
| sympy_str               | 188 ms                                                      | 174 ms: 1.08x faster                                        |
| sympy_sum               | 104 ms                                                      | 96.8 ms: 1.08x faster                                       |
| python_startup          | 20.0 ms                                                     | 18.6 ms: 1.07x faster                                       |
| comprehensions          | 16.0 us                                                     | 15.0 us: 1.06x faster                                       |
| json_loads              | 14.2 us                                                     | 13.4 us: 1.06x faster                                       |
| unpickle_list           | 2.81 us                                                     | 2.66 us: 1.06x faster                                       |
| xml_etree_generate      | 54.5 ms                                                     | 51.7 ms: 1.05x faster                                       |
| xml_etree_iterparse     | 63.5 ms                                                     | 60.4 ms: 1.05x faster                                       |
| async_generators        | 224 ms                                                      | 213 ms: 1.05x faster                                        |
| logging_format          | 6.66 us                                                     | 6.35 us: 1.05x faster                                       |
| logging_simple          | 6.20 us                                                     | 5.99 us: 1.03x faster                                       |
| sqlite_synth            | 1.84 us                                                     | 1.78 us: 1.03x faster                                       |
| meteor_contest          | 72.5 ms                                                     | 70.9 ms: 1.02x faster                                       |
| pidigits                | 145 ms                                                      | 145 ms: 1.00x faster                                        |
| python_startup_no_site  | 15.5 ms                                                     | 15.7 ms: 1.01x slower                                       |
| telco                   | 3.78 ms                                                     | 3.83 ms: 1.01x slower                                       |
| pickle                  | 6.80 us                                                     | 7.04 us: 1.04x slower                                       |
| regex_effbot            | 1.66 ms                                                     | 1.77 ms: 1.06x slower                                       |
| bench_mp_pool           | 60.7 ms                                                     | 64.8 ms: 1.07x slower                                       |
| gc_traversal            | 1.34 ms                                                     | 1.45 ms: 1.08x slower                                       |
| pathlib                 | 77.4 ms                                                     | 84.2 ms: 1.09x slower                                       |
| pickle_dict             | 16.9 us                                                     | 18.7 us: 1.11x slower                                       |
| pickle_list             | 2.59 us                                                     | 2.91 us: 1.13x slower                                       |
| dask                    | 305 ms                                                      | 354 ms: 1.16x slower                                        |
| coverage                | 40.0 ms                                                     | 51.5 ms: 1.29x slower                                       |
| Geometric mean          | (ref)                                                       | 1.22x faster                                                |
Ignored benchmarks (9) of results/bm-20220323-3.10.4-9d38120/bm-20220323-pythonperf1-amd64-python-v3.10.4-3.10.4-9d38120.json: aiohttp, asyncio_tcp_ssl, flaskblogging, pylint, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols


# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.19x
- 95% likely to have a speedup of 1.19x
- 99% likely to have a speedup of 1.17x
