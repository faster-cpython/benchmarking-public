# Results vs. 3.13.0

- fork: brandtbucher
- ref: justin_unlikely
- machine: linux-aarch64
- commit hash: 15229e0
- commit date: 2024-10-17
- overall geometric mean: 1.18x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x slower
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|----------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| 2to3           | 306 ms                                                   | 385 ms: 1.26x slower                                                      |
| docutils       | 2.91 sec                                                 | 3.62 sec: 1.24x slower                                                    |
| html5lib       | 64.3 ms                                                  | 71.1 ms: 1.11x slower                                                     |
| tornado_http   | 131 ms                                                   | 146 ms: 1.11x slower                                                      |
| Geometric mean | (ref)                                                    | 1.18x slower                                                              |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|---------------------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| async_tree_memoization_tg | 649 ms                                                   | 539 ms: 1.21x faster                                                      |
| async_tree_none           | 493 ms                                                   | 458 ms: 1.08x faster                                                      |
| async_tree_none_tg        | 467 ms                                                   | 448 ms: 1.04x faster                                                      |
| async_tree_memoization    | 626 ms                                                   | 603 ms: 1.04x faster                                                      |
| async_tree_cpu_io_mixed   | 764 ms                                                   | 755 ms: 1.01x faster                                                      |
| asyncio_websockets        | 656 ms                                                   | 662 ms: 1.01x slower                                                      |
| coroutines                | 28.2 ms                                                  | 28.6 ms: 1.01x slower                                                     |
| async_generators          | 496 ms                                                   | 513 ms: 1.03x slower                                                      |
| async_tree_io             | 1.11 sec                                                 | 1.16 sec: 1.05x slower                                                    |
| asyncio_tcp_ssl           | 2.18 sec                                                 | 2.29 sec: 1.05x slower                                                    |
| asyncio_tcp               | 568 ms                                                   | 639 ms: 1.13x slower                                                      |
| async_tree_io_tg          | 1.09 sec                                                 | 1.25 sec: 1.14x slower                                                    |
| Geometric mean            | (ref)                                                    | 1.00x slower                                                              |

Benchmark hidden because not significant (1): async_tree_cpu_io_mixed_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|----------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| float          | 94.4 ms                                                  | 97.2 ms: 1.03x slower                                                     |
| nbody          | 114 ms                                                   | 119 ms: 1.04x slower                                                      |
| Geometric mean | (ref)                                                    | 1.02x slower                                                              |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|----------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| regex_compile  | 128 ms                                                   | 183 ms: 1.42x slower                                                      |
| Geometric mean | (ref)                                                    | 1.09x slower                                                              |

Benchmark hidden because not significant (3): regex_v8, regex_dna, regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|----------------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| unpickle             | 20.2 us                                                  | 19.0 us: 1.06x faster                                                     |
| unpickle_list        | 6.65 us                                                  | 6.50 us: 1.02x faster                                                     |
| pickle_list          | 5.34 us                                                  | 5.22 us: 1.02x faster                                                     |
| xml_etree_parse      | 188 ms                                                   | 185 ms: 1.01x faster                                                      |
| json_loads           | 31.4 us                                                  | 31.7 us: 1.01x slower                                                     |
| pickle               | 13.5 us                                                  | 13.7 us: 1.02x slower                                                     |
| xml_etree_iterparse  | 147 ms                                                   | 151 ms: 1.03x slower                                                      |
| unpickle_pure_python | 254 us                                                   | 266 us: 1.05x slower                                                      |
| tomli_loads          | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                    |
| json_dumps           | 13.4 ms                                                  | 14.3 ms: 1.07x slower                                                     |
| pickle_pure_python   | 359 us                                                   | 387 us: 1.08x slower                                                      |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                              |

Benchmark hidden because not significant (3): pickle_dict, xml_etree_generate, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|----------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| python_startup | 13.3 ms                                                  | 14.6 ms: 1.10x slower                                                     |
| Geometric mean | (ref)                                                    | 1.05x slower                                                              |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|-----------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| mako            | 13.3 ms                                                  | 13.4 ms: 1.00x slower                                                     |
| django_template | 42.3 ms                                                  | 50.9 ms: 1.20x slower                                                     |
| genshi_text     | 27.7 ms                                                  | 34.2 ms: 1.23x slower                                                     |
| genshi_xml      | 60.2 ms                                                  | 84.6 ms: 1.41x slower                                                     |
| Geometric mean  | (ref)                                                    | 1.20x slower                                                              |

All benchmarks:
===============

| Benchmark                 | bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5 | bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0 |
|---------------------------|:--------------------------------------------------------:|:-------------------------------------------------------------------------:|
| deepcopy_memo             | 51.0 us                                                  | 39.3 us: 1.30x faster                                                     |
| async_tree_memoization_tg | 649 ms                                                   | 539 ms: 1.21x faster                                                      |
| deepcopy                  | 451 us                                                   | 379 us: 1.19x faster                                                      |
| async_tree_none           | 493 ms                                                   | 458 ms: 1.08x faster                                                      |
| unpickle                  | 20.2 us                                                  | 19.0 us: 1.06x faster                                                     |
| async_tree_none_tg        | 467 ms                                                   | 448 ms: 1.04x faster                                                      |
| pathlib                   | 22.4 ms                                                  | 21.5 ms: 1.04x faster                                                     |
| async_tree_memoization    | 626 ms                                                   | 603 ms: 1.04x faster                                                      |
| unpickle_list             | 6.65 us                                                  | 6.50 us: 1.02x faster                                                     |
| pickle_list               | 5.34 us                                                  | 5.22 us: 1.02x faster                                                     |
| deepcopy_reduce           | 4.07 us                                                  | 3.99 us: 1.02x faster                                                     |
| async_tree_cpu_io_mixed   | 764 ms                                                   | 755 ms: 1.01x faster                                                      |
| xml_etree_parse           | 188 ms                                                   | 185 ms: 1.01x faster                                                      |
| mako                      | 13.3 ms                                                  | 13.4 ms: 1.00x slower                                                     |
| asyncio_websockets        | 656 ms                                                   | 662 ms: 1.01x slower                                                      |
| json_loads                | 31.4 us                                                  | 31.7 us: 1.01x slower                                                     |
| coroutines                | 28.2 ms                                                  | 28.6 ms: 1.01x slower                                                     |
| sqlite_synth              | 3.84 us                                                  | 3.91 us: 1.02x slower                                                     |
| pickle                    | 13.5 us                                                  | 13.7 us: 1.02x slower                                                     |
| json                      | 5.61 ms                                                  | 5.74 ms: 1.02x slower                                                     |
| bpe_tokeniser             | 5.90 sec                                                 | 6.04 sec: 1.02x slower                                                    |
| logging_format            | 7.83 us                                                  | 8.06 us: 1.03x slower                                                     |
| float                     | 94.4 ms                                                  | 97.2 ms: 1.03x slower                                                     |
| xml_etree_iterparse       | 147 ms                                                   | 151 ms: 1.03x slower                                                      |
| async_generators          | 496 ms                                                   | 513 ms: 1.03x slower                                                      |
| scimark_fft               | 447 ms                                                   | 463 ms: 1.03x slower                                                      |
| thrift                    | 946 us                                                   | 979 us: 1.04x slower                                                      |
| mdp                       | 3.36 sec                                                 | 3.49 sec: 1.04x slower                                                    |
| nbody                     | 114 ms                                                   | 119 ms: 1.04x slower                                                      |
| unpickle_pure_python      | 254 us                                                   | 266 us: 1.05x slower                                                      |
| async_tree_io             | 1.11 sec                                                 | 1.16 sec: 1.05x slower                                                    |
| asyncio_tcp_ssl           | 2.18 sec                                                 | 2.29 sec: 1.05x slower                                                    |
| tomli_loads               | 2.53 sec                                                 | 2.65 sec: 1.05x slower                                                    |
| logging_simple            | 7.04 us                                                  | 7.41 us: 1.05x slower                                                     |
| scimark_lu                | 139 ms                                                   | 148 ms: 1.06x slower                                                      |
| json_dumps                | 13.4 ms                                                  | 14.3 ms: 1.07x slower                                                     |
| bench_thread_pool         | 1.28 ms                                                  | 1.37 ms: 1.08x slower                                                     |
| pickle_pure_python        | 359 us                                                   | 387 us: 1.08x slower                                                      |
| scimark_monte_carlo       | 83.8 ms                                                  | 90.7 ms: 1.08x slower                                                     |
| scimark_sparse_mat_mult   | 6.58 ms                                                  | 7.16 ms: 1.09x slower                                                     |
| meteor_contest            | 113 ms                                                   | 124 ms: 1.09x slower                                                      |
| spectral_norm             | 141 ms                                                   | 155 ms: 1.10x slower                                                      |
| crypto_pyaes              | 82.7 ms                                                  | 90.7 ms: 1.10x slower                                                     |
| python_startup            | 13.3 ms                                                  | 14.6 ms: 1.10x slower                                                     |
| html5lib                  | 64.3 ms                                                  | 71.1 ms: 1.11x slower                                                     |
| tornado_http              | 131 ms                                                   | 146 ms: 1.11x slower                                                      |
| fannkuch                  | 452 ms                                                   | 508 ms: 1.12x slower                                                      |
| asyncio_tcp               | 568 ms                                                   | 639 ms: 1.13x slower                                                      |
| typing_runtime_protocols  | 192 us                                                   | 218 us: 1.14x slower                                                      |
| go                        | 163 ms                                                   | 185 ms: 1.14x slower                                                      |
| async_tree_io_tg          | 1.09 sec                                                 | 1.25 sec: 1.14x slower                                                    |
| pyflate                   | 556 ms                                                   | 643 ms: 1.16x slower                                                      |
| raytrace                  | 298 ms                                                   | 352 ms: 1.18x slower                                                      |
| richards_super            | 60.3 ms                                                  | 71.7 ms: 1.19x slower                                                     |
| richards                  | 53.5 ms                                                  | 63.8 ms: 1.19x slower                                                     |
| pycparser                 | 1.26 sec                                                 | 1.52 sec: 1.20x slower                                                    |
| deltablue                 | 3.85 ms                                                  | 4.63 ms: 1.20x slower                                                     |
| django_template           | 42.3 ms                                                  | 50.9 ms: 1.20x slower                                                     |
| sqlglot_normalize         | 128 ms                                                   | 156 ms: 1.22x slower                                                      |
| comprehensions            | 20.4 us                                                  | 24.9 us: 1.22x slower                                                     |
| genshi_text               | 27.7 ms                                                  | 34.2 ms: 1.23x slower                                                     |
| sqlglot_parse             | 1.38 ms                                                  | 1.71 ms: 1.24x slower                                                     |
| docutils                  | 2.91 sec                                                 | 3.62 sec: 1.24x slower                                                    |
| nqueens                   | 98.7 ms                                                  | 123 ms: 1.25x slower                                                      |
| 2to3                      | 306 ms                                                   | 385 ms: 1.26x slower                                                      |
| chaos                     | 68.8 ms                                                  | 87.2 ms: 1.27x slower                                                     |
| sqlglot_transpile         | 1.73 ms                                                  | 2.24 ms: 1.29x slower                                                     |
| sympy_expand              | 455 ms                                                   | 592 ms: 1.30x slower                                                      |
| sqlglot_optimize          | 62.4 ms                                                  | 82.9 ms: 1.33x slower                                                     |
| pprint_safe_repr          | 926 ms                                                   | 1.24 sec: 1.34x slower                                                    |
| gc_traversal              | 4.53 ms                                                  | 6.23 ms: 1.37x slower                                                     |
| pprint_pformat            | 1.90 sec                                                 | 2.61 sec: 1.38x slower                                                    |
| sympy_str                 | 264 ms                                                   | 363 ms: 1.38x slower                                                      |
| sympy_integrate           | 21.0 ms                                                  | 29.3 ms: 1.40x slower                                                     |
| genshi_xml                | 60.2 ms                                                  | 84.6 ms: 1.41x slower                                                     |
| regex_compile             | 128 ms                                                   | 183 ms: 1.42x slower                                                      |
| hexiom                    | 7.13 ms                                                  | 10.2 ms: 1.43x slower                                                     |
| pylint                    | 343 ms                                                   | 493 ms: 1.44x slower                                                      |
| sympy_sum                 | 143 ms                                                   | 216 ms: 1.51x slower                                                      |
| generators                | 37.8 ms                                                  | 58.9 ms: 1.56x slower                                                     |
| create_gc_cycles          | 2.12 ms                                                  | 3.68 ms: 1.74x slower                                                     |
| unpack_sequence           | 57.2 ns                                                  | 145 ns: 2.53x slower                                                      |
| bench_mp_pool             | 7.30 ms                                                  | 1.92 sec: 263.51x slower                                                  |
| Geometric mean            | (ref)                                                    | 1.18x slower                                                              |

Benchmark hidden because not significant (13): scimark_sor, pickle_dict, xml_etree_generate, regex_v8, regex_dna, python_startup_no_site, logging_silent, regex_effbot, coverage, pidigits, xml_etree_process, async_tree_cpu_io_mixed_tg, telco
Ignored benchmarks (6) of results/bm-20241007-3.13.0-60403a5/bm-20241007-arminc-aarch64-python-v3.13.0-3.13.0-60403a5.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2
Ignored benchmarks (2) of results/bm-20241017-3.14.0a1+-15229e0-JIT/bm-20241017-arminc-aarch64-brandtbucher-justin_unlikely-3.14.0a1+-15229e0.json: dulwich_log, sphinx

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.06x
- 95% likely to have a slowdown of 1.05x
- 99% likely to have a slowdown of 1.04x

# Memory
- memory change: 1.20x