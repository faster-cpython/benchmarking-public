# Results vs. 3.10.4

- fork: brandtbucher
- ref: tier_two_improvement
- machine: linux-aarch64
- commit hash: bbb07e8
- commit date: 2024-07-13
- overall geometric mean: 1.23x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 1.23x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 362 ms: 1.05x faster                                                          |
| docutils       | 3.53 sec                                                              | 3.59 sec: 1.02x slower                                                        |
| html5lib       | 86.5 ms                                                               | 70.8 ms: 1.22x faster                                                         |
| tornado_http   | 178 ms                                                                | 139 ms: 1.28x faster                                                          |
| Geometric mean | (ref)                                                                 | 1.13x faster                                                                  |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| async_tree_io           | 2.28 sec                                                              | 1.07 sec: 2.14x faster                                                        |
| async_tree_none         | 950 ms                                                                | 448 ms: 2.12x faster                                                          |
| async_tree_memoization  | 1.13 sec                                                              | 594 ms: 1.91x faster                                                          |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 734 ms: 1.73x faster                                                          |
| Geometric mean          | (ref)                                                                 | 1.97x faster                                                                  |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| float          | 135 ms                                                                | 91.9 ms: 1.47x faster                                                         |
| nbody          | 166 ms                                                                | 114 ms: 1.46x faster                                                          |
| Geometric mean | (ref)                                                                 | 1.29x faster                                                                  |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| regex_v8       | 32.2 ms                                                               | 30.5 ms: 1.06x faster                                                         |
| regex_dna      | 257 ms                                                                | 249 ms: 1.03x faster                                                          |
| regex_compile  | 177 ms                                                                | 178 ms: 1.01x slower                                                          |
| regex_effbot   | 4.25 ms                                                               | 4.88 ms: 1.15x slower                                                         |
| Geometric mean | (ref)                                                                 | 1.02x slower                                                                  |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| unpickle_pure_python | 366 us                                                                | 272 us: 1.34x faster                                                          |
| pickle_pure_python   | 529 us                                                                | 400 us: 1.32x faster                                                          |
| tomli_loads          | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                                        |
| xml_etree_process    | 99.5 ms                                                               | 80.0 ms: 1.24x faster                                                         |
| json_dumps           | 16.7 ms                                                               | 13.5 ms: 1.24x faster                                                         |
| xml_etree_parse      | 212 ms                                                                | 191 ms: 1.11x faster                                                          |
| xml_etree_generate   | 123 ms                                                                | 112 ms: 1.10x faster                                                          |
| xml_etree_iterparse  | 156 ms                                                                | 149 ms: 1.05x faster                                                          |
| json_loads           | 30.9 us                                                               | 33.2 us: 1.07x slower                                                         |
| Geometric mean       | (ref)                                                                 | 1.17x faster                                                                  |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 12.9 ms: 1.15x slower                                                         |
| python_startup_no_site | 6.89 ms                                                               | 8.72 ms: 1.27x slower                                                         |
| Geometric mean         | (ref)                                                                 | 1.21x slower                                                                  |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| mako            | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                         |
| django_template | 53.3 ms                                                               | 50.6 ms: 1.05x faster                                                         |
| genshi_text     | 35.2 ms                                                               | 34.5 ms: 1.02x faster                                                         |
| genshi_xml      | 69.8 ms                                                               | 78.7 ms: 1.13x slower                                                         |
| Geometric mean  | (ref)                                                                 | 1.08x faster                                                                  |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------------:|
| typing_runtime_protocols | 661 us                                                                | 210 us: 3.15x faster                                                          |
| async_tree_io            | 2.28 sec                                                              | 1.07 sec: 2.14x faster                                                        |
| async_tree_none          | 950 ms                                                                | 448 ms: 2.12x faster                                                          |
| deltablue                | 8.94 ms                                                               | 4.44 ms: 2.01x faster                                                         |
| async_tree_memoization   | 1.13 sec                                                              | 594 ms: 1.91x faster                                                          |
| bench_mp_pool            | 14.5 ms                                                               | 7.91 ms: 1.84x faster                                                         |
| generators               | 68.1 ms                                                               | 37.3 ms: 1.83x faster                                                         |
| richards_super           | 107 ms                                                                | 60.5 ms: 1.77x faster                                                         |
| raytrace                 | 573 ms                                                                | 329 ms: 1.74x faster                                                          |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 734 ms: 1.73x faster                                                          |
| richards                 | 91.7 ms                                                               | 53.6 ms: 1.71x faster                                                         |
| logging_silent           | 222 ns                                                                | 139 ns: 1.59x faster                                                          |
| deepcopy_memo            | 61.7 us                                                               | 38.8 us: 1.59x faster                                                         |
| asyncio_tcp              | 944 ms                                                                | 621 ms: 1.52x faster                                                          |
| crypto_pyaes             | 134 ms                                                                | 89.5 ms: 1.50x faster                                                         |
| sqlglot_parse            | 2.40 ms                                                               | 1.62 ms: 1.49x faster                                                         |
| go                       | 264 ms                                                                | 179 ms: 1.47x faster                                                          |
| float                    | 135 ms                                                                | 91.9 ms: 1.47x faster                                                         |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.25 sec: 1.46x faster                                                        |
| nbody                    | 166 ms                                                                | 114 ms: 1.46x faster                                                          |
| mako                     | 18.9 ms                                                               | 13.1 ms: 1.44x faster                                                         |
| scimark_monte_carlo      | 128 ms                                                                | 89.0 ms: 1.44x faster                                                         |
| comprehensions           | 33.1 us                                                               | 23.4 us: 1.41x faster                                                         |
| sqlglot_transpile        | 2.84 ms                                                               | 2.03 ms: 1.40x faster                                                         |
| scimark_sor              | 246 ms                                                                | 181 ms: 1.36x faster                                                          |
| logging_simple           | 9.78 us                                                               | 7.22 us: 1.35x faster                                                         |
| chaos                    | 121 ms                                                                | 89.9 ms: 1.35x faster                                                         |
| unpickle_pure_python     | 366 us                                                                | 272 us: 1.34x faster                                                          |
| deepcopy                 | 511 us                                                                | 383 us: 1.33x faster                                                          |
| pickle_pure_python       | 529 us                                                                | 400 us: 1.32x faster                                                          |
| logging_format           | 10.6 us                                                               | 8.06 us: 1.32x faster                                                         |
| spectral_norm            | 186 ms                                                                | 143 ms: 1.31x faster                                                          |
| thrift                   | 1.26 ms                                                               | 967 us: 1.30x faster                                                          |
| pyflate                  | 795 ms                                                                | 616 ms: 1.29x faster                                                          |
| tornado_http             | 178 ms                                                                | 139 ms: 1.28x faster                                                          |
| tomli_loads              | 3.36 sec                                                              | 2.62 sec: 1.28x faster                                                        |
| coroutines               | 37.2 ms                                                               | 29.0 ms: 1.28x faster                                                         |
| xml_etree_process        | 99.5 ms                                                               | 80.0 ms: 1.24x faster                                                         |
| json_dumps               | 16.7 ms                                                               | 13.5 ms: 1.24x faster                                                         |
| scimark_lu               | 227 ms                                                                | 184 ms: 1.23x faster                                                          |
| pycparser                | 1.69 sec                                                              | 1.38 sec: 1.22x faster                                                        |
| html5lib                 | 86.5 ms                                                               | 70.8 ms: 1.22x faster                                                         |
| pathlib                  | 26.3 ms                                                               | 21.8 ms: 1.20x faster                                                         |
| pylint                   | 485 ms                                                                | 404 ms: 1.20x faster                                                          |
| hexiom                   | 10.9 ms                                                               | 9.16 ms: 1.19x faster                                                         |
| bench_thread_pool        | 1.59 ms                                                               | 1.34 ms: 1.19x faster                                                         |
| fannkuch                 | 546 ms                                                                | 473 ms: 1.15x faster                                                          |
| dask                     | 450 ms                                                                | 393 ms: 1.14x faster                                                          |
| xml_etree_parse          | 212 ms                                                                | 191 ms: 1.11x faster                                                          |
| xml_etree_generate       | 123 ms                                                                | 112 ms: 1.10x faster                                                          |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 6.99 ms: 1.09x faster                                                         |
| scimark_fft              | 500 ms                                                                | 460 ms: 1.09x faster                                                          |
| meteor_contest           | 126 ms                                                                | 116 ms: 1.09x faster                                                          |
| sqlglot_normalize        | 156 ms                                                                | 144 ms: 1.08x faster                                                          |
| sqlglot_optimize         | 75.4 ms                                                               | 70.2 ms: 1.07x faster                                                         |
| deepcopy_reduce          | 4.60 us                                                               | 4.28 us: 1.07x faster                                                         |
| mdp                      | 3.70 sec                                                              | 3.47 sec: 1.06x faster                                                        |
| regex_v8                 | 32.2 ms                                                               | 30.5 ms: 1.06x faster                                                         |
| django_template          | 53.3 ms                                                               | 50.6 ms: 1.05x faster                                                         |
| 2to3                     | 381 ms                                                                | 362 ms: 1.05x faster                                                          |
| xml_etree_iterparse      | 156 ms                                                                | 149 ms: 1.05x faster                                                          |
| regex_dna                | 257 ms                                                                | 249 ms: 1.03x faster                                                          |
| genshi_text              | 35.2 ms                                                               | 34.5 ms: 1.02x faster                                                         |
| regex_compile            | 177 ms                                                                | 178 ms: 1.01x slower                                                          |
| pprint_safe_repr         | 1.15 sec                                                              | 1.16 sec: 1.01x slower                                                        |
| pprint_pformat           | 2.36 sec                                                              | 2.38 sec: 1.01x slower                                                        |
| docutils                 | 3.53 sec                                                              | 3.59 sec: 1.02x slower                                                        |
| sympy_str                | 329 ms                                                                | 339 ms: 1.03x slower                                                          |
| create_gc_cycles         | 2.26 ms                                                               | 2.34 ms: 1.03x slower                                                         |
| sympy_expand             | 543 ms                                                                | 565 ms: 1.04x slower                                                          |
| sympy_sum                | 184 ms                                                                | 195 ms: 1.06x slower                                                          |
| json_loads               | 30.9 us                                                               | 33.2 us: 1.07x slower                                                         |
| async_generators         | 452 ms                                                                | 509 ms: 1.12x slower                                                          |
| genshi_xml               | 69.8 ms                                                               | 78.7 ms: 1.13x slower                                                         |
| regex_effbot             | 4.25 ms                                                               | 4.88 ms: 1.15x slower                                                         |
| python_startup           | 11.2 ms                                                               | 12.9 ms: 1.15x slower                                                         |
| coverage                 | 83.6 ms                                                               | 101 ms: 1.21x slower                                                          |
| gc_traversal             | 4.15 ms                                                               | 5.02 ms: 1.21x slower                                                         |
| telco                    | 8.49 ms                                                               | 10.6 ms: 1.25x slower                                                         |
| python_startup_no_site   | 6.89 ms                                                               | 8.72 ms: 1.27x slower                                                         |
| Geometric mean           | (ref)                                                                 | 1.23x faster                                                                  |

Benchmark hidden because not significant (6): pidigits, nqueens, sympy_integrate, json, asyncio_websockets, dulwich_log
Ignored benchmarks (13) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240713-3.14.0a0-bbb07e8-JIT/bm-20240713-arminc-aarch64-brandtbucher-tier_two_improvement-3.14.0a0-bbb07e8.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 1.23x