
# Results vs. 3.11.0

- fork: python
- ref: dfb5d272e6b99c2c70c6
- machine: darwin-arm64
- commit hash: dfb5d27
- commit date: 2022-10-24
- overall geometric mean: 1.00x slower \*
- HPT reliability: 98.96%
- HPT 99th percentile: 1.00x slower

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 161 ms                                                 | 185 ms: 1.15x slower                                                  |
| chameleon      | 4.60 ms                                                | 4.61 ms: 1.00x slower                                                 |
| docutils       | 1.47 sec                                               | 1.47 sec: 1.00x slower                                                |
| html5lib       | 34.7 ms                                                | 36.5 ms: 1.05x slower                                                 |
| Geometric mean | (ref)                                                  | 1.04x slower                                                          |

Benchmark hidden because not significant (1): tornado_http

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 65.6 ms                                                | 64.6 ms: 1.02x faster                                                 |
| pidigits       | 281 ms                                                 | 282 ms: 1.01x slower                                                  |
| float          | 53.0 ms                                                | 56.2 ms: 1.06x slower                                                 |
| Geometric mean | (ref)                                                  | 1.02x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 76.7 ms                                                | 76.1 ms: 1.01x faster                                                 |
| regex_dna      | 152 ms                                                 | 152 ms: 1.00x slower                                                  |
| regex_v8       | 16.1 ms                                                | 16.4 ms: 1.02x slower                                                 |
| regex_effbot   | 2.63 ms                                                | 2.74 ms: 1.04x slower                                                 |
| Geometric mean | (ref)                                                  | 1.01x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 7.63 ms                                                | 6.13 ms: 1.24x faster                                                 |
| xml_etree_parse      | 108 ms                                                 | 91.0 ms: 1.19x faster                                                 |
| xml_etree_iterparse  | 70.1 ms                                                | 64.9 ms: 1.08x faster                                                 |
| unpickle_list        | 2.65 us                                                | 2.53 us: 1.05x faster                                                 |
| xml_etree_generate   | 48.3 ms                                                | 47.3 ms: 1.02x faster                                                 |
| xml_etree_process    | 35.1 ms                                                | 34.9 ms: 1.01x faster                                                 |
| pickle_dict          | 18.0 us                                                | 18.0 us: 1.00x slower                                                 |
| json_loads           | 16.1 us                                                | 16.2 us: 1.01x slower                                                 |
| unpickle             | 9.67 us                                                | 9.77 us: 1.01x slower                                                 |
| unpickle_pure_python | 159 us                                                 | 162 us: 1.02x slower                                                  |
| pickle_list          | 2.81 us                                                | 2.88 us: 1.02x slower                                                 |
| pickle_pure_python   | 201 us                                                 | 208 us: 1.04x slower                                                  |
| pickle               | 7.15 us                                                | 7.54 us: 1.06x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.03x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 10.2 ms                                                | 7.21 ms: 1.41x faster                                                 |
| python_startup         | 12.4 ms                                                | 9.19 ms: 1.35x faster                                                 |
| Geometric mean         | (ref)                                                  | 1.38x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 8.53 ms                                                | 8.13 ms: 1.05x faster                                                 |
| genshi_text     | 15.3 ms                                                | 15.4 ms: 1.01x slower                                                 |
| genshi_xml      | 29.8 ms                                                | 30.2 ms: 1.01x slower                                                 |
| django_template | 21.0 ms                                                | 21.7 ms: 1.03x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.00x slower                                                          |

All benchmarks:
===============

| Benchmark               | bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509 | bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27 |
|-------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site  | 10.2 ms                                                | 7.21 ms: 1.41x faster                                                 |
| python_startup          | 12.4 ms                                                | 9.19 ms: 1.35x faster                                                 |
| pathlib                 | 27.2 ms                                                | 20.7 ms: 1.31x faster                                                 |
| json_dumps              | 7.63 ms                                                | 6.13 ms: 1.24x faster                                                 |
| xml_etree_parse         | 108 ms                                                 | 91.0 ms: 1.19x faster                                                 |
| scimark_sparse_mat_mult | 3.19 ms                                                | 2.77 ms: 1.15x faster                                                 |
| coverage                | 58.4 ms                                                | 53.1 ms: 1.10x faster                                                 |
| xml_etree_iterparse     | 70.1 ms                                                | 64.9 ms: 1.08x faster                                                 |
| bench_mp_pool           | 43.9 ms                                                | 41.7 ms: 1.05x faster                                                 |
| bench_thread_pool       | 474 us                                                 | 452 us: 1.05x faster                                                  |
| mako                    | 8.53 ms                                                | 8.13 ms: 1.05x faster                                                 |
| unpickle_list           | 2.65 us                                                | 2.53 us: 1.05x faster                                                 |
| logging_silent          | 68.1 ns                                                | 65.8 ns: 1.04x faster                                                 |
| async_tree_memoization  | 336 ms                                                 | 325 ms: 1.03x faster                                                  |
| dulwich_log             | 30.3 ms                                                | 29.4 ms: 1.03x faster                                                 |
| pylint                  | 272 ms                                                 | 264 ms: 1.03x faster                                                  |
| unpack_sequence         | 34.1 ns                                                | 33.2 ns: 1.03x faster                                                 |
| deltablue               | 2.81 ms                                                | 2.74 ms: 1.03x faster                                                 |
| xml_etree_generate      | 48.3 ms                                                | 47.3 ms: 1.02x faster                                                 |
| scimark_lu              | 73.3 ms                                                | 71.9 ms: 1.02x faster                                                 |
| scimark_fft             | 200 ms                                                 | 196 ms: 1.02x faster                                                  |
| mdp                     | 1.55 sec                                               | 1.52 sec: 1.02x faster                                                |
| nbody                   | 65.6 ms                                                | 64.6 ms: 1.02x faster                                                 |
| telco                   | 3.41 ms                                                | 3.37 ms: 1.01x faster                                                 |
| crypto_pyaes            | 51.7 ms                                                | 51.3 ms: 1.01x faster                                                 |
| regex_compile           | 76.7 ms                                                | 76.1 ms: 1.01x faster                                                 |
| xml_etree_process       | 35.1 ms                                                | 34.9 ms: 1.01x faster                                                 |
| thrift                  | 442 us                                                 | 441 us: 1.00x faster                                                  |
| chameleon               | 4.60 ms                                                | 4.61 ms: 1.00x slower                                                 |
| docutils                | 1.47 sec                                               | 1.47 sec: 1.00x slower                                                |
| regex_dna               | 152 ms                                                 | 152 ms: 1.00x slower                                                  |
| pickle_dict             | 18.0 us                                                | 18.0 us: 1.00x slower                                                 |
| genshi_text             | 15.3 ms                                                | 15.4 ms: 1.01x slower                                                 |
| pidigits                | 281 ms                                                 | 282 ms: 1.01x slower                                                  |
| sympy_sum               | 85.5 ms                                                | 86.1 ms: 1.01x slower                                                 |
| json_loads              | 16.1 us                                                | 16.2 us: 1.01x slower                                                 |
| sympy_integrate         | 11.5 ms                                                | 11.6 ms: 1.01x slower                                                 |
| nqueens                 | 59.8 ms                                                | 60.3 ms: 1.01x slower                                                 |
| unpickle                | 9.67 us                                                | 9.77 us: 1.01x slower                                                 |
| genshi_xml              | 29.8 ms                                                | 30.2 ms: 1.01x slower                                                 |
| async_generators        | 196 ms                                                 | 199 ms: 1.01x slower                                                  |
| sqlglot_normalize       | 171 ms                                                 | 173 ms: 1.01x slower                                                  |
| regex_v8                | 16.1 ms                                                | 16.4 ms: 1.02x slower                                                 |
| sympy_str               | 151 ms                                                 | 154 ms: 1.02x slower                                                  |
| json                    | 2.78 ms                                                | 2.83 ms: 1.02x slower                                                 |
| chaos                   | 49.4 ms                                                | 50.3 ms: 1.02x slower                                                 |
| generators              | 28.8 ms                                                | 29.3 ms: 1.02x slower                                                 |
| async_tree_cpu_io_mixed | 533 ms                                                 | 544 ms: 1.02x slower                                                  |
| unpickle_pure_python    | 159 us                                                 | 162 us: 1.02x slower                                                  |
| sympy_expand            | 242 ms                                                 | 247 ms: 1.02x slower                                                  |
| sqlglot_optimize        | 31.1 ms                                                | 31.8 ms: 1.02x slower                                                 |
| pickle_list             | 2.81 us                                                | 2.88 us: 1.02x slower                                                 |
| fannkuch                | 261 ms                                                 | 268 ms: 1.03x slower                                                  |
| logging_simple          | 3.55 us                                                | 3.66 us: 1.03x slower                                                 |
| django_template         | 21.0 ms                                                | 21.7 ms: 1.03x slower                                                 |
| pickle_pure_python      | 201 us                                                 | 208 us: 1.04x slower                                                  |
| sqlglot_transpile       | 1.12 ms                                                | 1.17 ms: 1.04x slower                                                 |
| richards                | 32.2 ms                                                | 33.5 ms: 1.04x slower                                                 |
| regex_effbot            | 2.63 ms                                                | 2.74 ms: 1.04x slower                                                 |
| hexiom                  | 4.72 ms                                                | 4.92 ms: 1.04x slower                                                 |
| async_tree_io           | 704 ms                                                 | 736 ms: 1.04x slower                                                  |
| meteor_contest          | 73.5 ms                                                | 76.8 ms: 1.04x slower                                                 |
| sqlglot_parse           | 959 us                                                 | 1.00 ms: 1.04x slower                                                 |
| pprint_pformat          | 954 ms                                                 | 996 ms: 1.04x slower                                                  |
| pprint_safe_repr        | 466 ms                                                 | 488 ms: 1.05x slower                                                  |
| logging_format          | 3.78 us                                                | 3.97 us: 1.05x slower                                                 |
| html5lib                | 34.7 ms                                                | 36.5 ms: 1.05x slower                                                 |
| pickle                  | 7.15 us                                                | 7.54 us: 1.06x slower                                                 |
| float                   | 53.0 ms                                                | 56.2 ms: 1.06x slower                                                 |
| coroutines              | 17.8 ms                                                | 18.9 ms: 1.07x slower                                                 |
| deepcopy_reduce         | 1.91 us                                                | 2.05 us: 1.07x slower                                                 |
| deepcopy                | 223 us                                                 | 239 us: 1.08x slower                                                  |
| raytrace                | 207 ms                                                 | 227 ms: 1.10x slower                                                  |
| go                      | 109 ms                                                 | 119 ms: 1.10x slower                                                  |
| deepcopy_memo           | 26.3 us                                                | 29.0 us: 1.10x slower                                                 |
| sqlite_synth            | 1.27 us                                                | 1.45 us: 1.14x slower                                                 |
| pyflate                 | 310 ms                                                 | 355 ms: 1.14x slower                                                  |
| 2to3                    | 161 ms                                                 | 185 ms: 1.15x slower                                                  |
| scimark_monte_carlo     | 46.5 ms                                                | 55.4 ms: 1.19x slower                                                 |
| scimark_sor             | 82.6 ms                                                | 101 ms: 1.22x slower                                                  |
| Geometric mean          | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (4): tornado_http, spectral_norm, async_tree_none, pycparser
Ignored benchmarks (14) of results/bm-20221024-3.11.0-deaf509/bm-20221024-darwin-arm64-python-v3.11.0-3.11.0-deaf509.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, comprehensions, create_gc_cycles, flaskblogging, gc_traversal, gunicorn, mypy2, richards_super, sqlalchemy_declarative, sqlalchemy_imperative, tomli_loads, typing_runtime_protocols
Ignored benchmarks (1) of results/bm-20221024-3.12.0a0-dfb5d27/bm-20221024-darwin-arm64-python-dfb5d272e6b99c2c70c6-3.12.0a0-dfb5d27.json: mypy


# HPT report

- Reliability score: 98.96% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x
