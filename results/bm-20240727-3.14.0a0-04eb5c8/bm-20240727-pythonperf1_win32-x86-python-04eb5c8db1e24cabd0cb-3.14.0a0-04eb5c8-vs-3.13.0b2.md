# Results vs. 3.13.0b2

- fork: python
- ref: 04eb5c8db1e24cabd0cb
- machine: windows-x86
- commit hash: 04eb5c8
- commit date: 2024-07-27
- overall geometric mean: 1.01x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| 2to3           | 233 ms                                                              | 249 ms: 1.07x slower                                                           |
| docutils       | 1.81 sec                                                            | 1.89 sec: 1.04x slower                                                         |
| html5lib       | 45.4 ms                                                             | 48.5 ms: 1.07x slower                                                          |
| tornado_http   | 94.3 ms                                                             | 104 ms: 1.10x slower                                                           |
| Geometric mean | (ref)                                                               | 1.07x slower                                                                   |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| async_tree_io_tg           | 529 ms                                                              | 500 ms: 1.06x faster                                                           |
| async_tree_none_tg         | 203 ms                                                              | 193 ms: 1.05x faster                                                           |
| async_tree_memoization_tg  | 254 ms                                                              | 244 ms: 1.04x faster                                                           |
| async_tree_none            | 228 ms                                                              | 223 ms: 1.02x faster                                                           |
| async_tree_io              | 530 ms                                                              | 541 ms: 1.02x slower                                                           |
| async_tree_cpu_io_mixed    | 471 ms                                                              | 481 ms: 1.02x slower                                                           |
| async_tree_cpu_io_mixed_tg | 447 ms                                                              | 469 ms: 1.05x slower                                                           |
| Geometric mean             | (ref)                                                               | 1.01x faster                                                                   |

Benchmark hidden because not significant (1): async_tree_memoization

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| pidigits       | 199 ms                                                              | 197 ms: 1.01x faster                                                           |
| float          | 52.4 ms                                                             | 58.2 ms: 1.11x slower                                                          |
| nbody          | 76.9 ms                                                             | 90.9 ms: 1.18x slower                                                          |
| Geometric mean | (ref)                                                               | 1.09x slower                                                                   |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| regex_effbot   | 1.88 ms                                                             | 1.92 ms: 1.02x slower                                                          |
| regex_dna      | 118 ms                                                              | 122 ms: 1.03x slower                                                           |
| regex_v8       | 15.7 ms                                                             | 16.3 ms: 1.04x slower                                                          |
| regex_compile  | 99.9 ms                                                             | 104 ms: 1.05x slower                                                           |
| Geometric mean | (ref)                                                               | 1.03x slower                                                                   |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| json_loads           | 20.5 us                                                             | 20.2 us: 1.02x faster                                                          |
| xml_etree_parse      | 104 ms                                                              | 106 ms: 1.01x slower                                                           |
| xml_etree_iterparse  | 64.2 ms                                                             | 67.5 ms: 1.05x slower                                                          |
| tomli_loads          | 1.65 sec                                                            | 1.77 sec: 1.07x slower                                                         |
| json_dumps           | 6.84 ms                                                             | 7.46 ms: 1.09x slower                                                          |
| xml_etree_generate   | 59.6 ms                                                             | 65.5 ms: 1.10x slower                                                          |
| xml_etree_process    | 42.1 ms                                                             | 46.9 ms: 1.11x slower                                                          |
| pickle_pure_python   | 213 us                                                              | 239 us: 1.12x slower                                                           |
| unpickle_pure_python | 147 us                                                              | 170 us: 1.16x slower                                                           |
| Geometric mean       | (ref)                                                               | 1.08x slower                                                                   |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| python_startup_no_site | 18.2 ms                                                             | 19.4 ms: 1.07x slower                                                          |
| python_startup         | 22.2 ms                                                             | 23.8 ms: 1.07x slower                                                          |
| Geometric mean         | (ref)                                                               | 1.07x slower                                                                   |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|-----------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| genshi_xml      | 44.3 ms                                                             | 47.3 ms: 1.07x slower                                                          |
| genshi_text     | 20.1 ms                                                             | 21.5 ms: 1.07x slower                                                          |
| django_template | 30.1 ms                                                             | 33.3 ms: 1.11x slower                                                          |
| mako            | 6.94 ms                                                             | 8.06 ms: 1.16x slower                                                          |
| Geometric mean  | (ref)                                                               | 1.10x slower                                                                   |

All benchmarks:
===============

| Benchmark                  | bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17 | bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------------|:-------------------------------------------------------------------:|:------------------------------------------------------------------------------:|
| thrift                     | 9.73 ms                                                             | 770 us: 12.64x faster                                                          |
| coverage                   | 307 ms                                                              | 52.0 ms: 5.91x faster                                                          |
| deepcopy                   | 280 us                                                              | 242 us: 1.16x faster                                                           |
| deepcopy_memo              | 23.5 us                                                             | 20.9 us: 1.12x faster                                                          |
| async_tree_io_tg           | 529 ms                                                              | 500 ms: 1.06x faster                                                           |
| async_tree_none_tg         | 203 ms                                                              | 193 ms: 1.05x faster                                                           |
| async_tree_memoization_tg  | 254 ms                                                              | 244 ms: 1.04x faster                                                           |
| deepcopy_reduce            | 2.59 us                                                             | 2.51 us: 1.03x faster                                                          |
| async_tree_none            | 228 ms                                                              | 223 ms: 1.02x faster                                                           |
| json_loads                 | 20.5 us                                                             | 20.2 us: 1.02x faster                                                          |
| create_gc_cycles           | 756 us                                                              | 748 us: 1.01x faster                                                           |
| pidigits                   | 199 ms                                                              | 197 ms: 1.01x faster                                                           |
| asyncio_tcp_ssl            | 16.9 sec                                                            | 17.1 sec: 1.01x slower                                                         |
| json                       | 4.10 ms                                                             | 4.15 ms: 1.01x slower                                                          |
| xml_etree_parse            | 104 ms                                                              | 106 ms: 1.01x slower                                                           |
| crypto_pyaes               | 55.8 ms                                                             | 56.8 ms: 1.02x slower                                                          |
| regex_effbot               | 1.88 ms                                                             | 1.92 ms: 1.02x slower                                                          |
| async_tree_io              | 530 ms                                                              | 541 ms: 1.02x slower                                                           |
| async_tree_cpu_io_mixed    | 471 ms                                                              | 481 ms: 1.02x slower                                                           |
| sympy_expand               | 375 ms                                                              | 385 ms: 1.03x slower                                                           |
| sympy_str                  | 211 ms                                                              | 217 ms: 1.03x slower                                                           |
| regex_dna                  | 118 ms                                                              | 122 ms: 1.03x slower                                                           |
| sympy_sum                  | 105 ms                                                              | 109 ms: 1.03x slower                                                           |
| telco                      | 6.03 ms                                                             | 6.24 ms: 1.03x slower                                                          |
| regex_v8                   | 15.7 ms                                                             | 16.3 ms: 1.04x slower                                                          |
| sympy_integrate            | 14.6 ms                                                             | 15.2 ms: 1.04x slower                                                          |
| logging_simple             | 7.47 us                                                             | 7.78 us: 1.04x slower                                                          |
| docutils                   | 1.81 sec                                                            | 1.89 sec: 1.04x slower                                                         |
| mdp                        | 1.60 sec                                                            | 1.67 sec: 1.04x slower                                                         |
| regex_compile              | 99.9 ms                                                             | 104 ms: 1.05x slower                                                           |
| nqueens                    | 68.7 ms                                                             | 71.9 ms: 1.05x slower                                                          |
| async_tree_cpu_io_mixed_tg | 447 ms                                                              | 469 ms: 1.05x slower                                                           |
| pathlib                    | 83.9 ms                                                             | 88.1 ms: 1.05x slower                                                          |
| xml_etree_iterparse        | 64.2 ms                                                             | 67.5 ms: 1.05x slower                                                          |
| logging_format             | 8.13 us                                                             | 8.58 us: 1.06x slower                                                          |
| async_generators           | 266 ms                                                              | 281 ms: 1.06x slower                                                           |
| bench_mp_pool              | 69.4 ms                                                             | 73.4 ms: 1.06x slower                                                          |
| meteor_contest             | 74.1 ms                                                             | 78.3 ms: 1.06x slower                                                          |
| sqlglot_parse              | 964 us                                                              | 1.02 ms: 1.06x slower                                                          |
| pprint_safe_repr           | 607 ms                                                              | 646 ms: 1.07x slower                                                           |
| python_startup_no_site     | 18.2 ms                                                             | 19.4 ms: 1.07x slower                                                          |
| html5lib                   | 45.4 ms                                                             | 48.5 ms: 1.07x slower                                                          |
| 2to3                       | 233 ms                                                              | 249 ms: 1.07x slower                                                           |
| genshi_xml                 | 44.3 ms                                                             | 47.3 ms: 1.07x slower                                                          |
| genshi_text                | 20.1 ms                                                             | 21.5 ms: 1.07x slower                                                          |
| pprint_pformat             | 1.24 sec                                                            | 1.33 sec: 1.07x slower                                                         |
| sqlglot_transpile          | 1.19 ms                                                             | 1.28 ms: 1.07x slower                                                          |
| tomli_loads                | 1.65 sec                                                            | 1.77 sec: 1.07x slower                                                         |
| python_startup             | 22.2 ms                                                             | 23.8 ms: 1.07x slower                                                          |
| pylint                     | 217 ms                                                              | 233 ms: 1.07x slower                                                           |
| scimark_sparse_mat_mult    | 2.87 ms                                                             | 3.10 ms: 1.08x slower                                                          |
| asyncio_tcp                | 662 ms                                                              | 722 ms: 1.09x slower                                                           |
| json_dumps                 | 6.84 ms                                                             | 7.46 ms: 1.09x slower                                                          |
| sqlglot_normalize          | 206 ms                                                              | 225 ms: 1.09x slower                                                           |
| typing_runtime_protocols   | 136 us                                                              | 148 us: 1.09x slower                                                           |
| go                         | 101 ms                                                              | 110 ms: 1.09x slower                                                           |
| sqlglot_optimize           | 39.7 ms                                                             | 43.5 ms: 1.09x slower                                                          |
| xml_etree_generate         | 59.6 ms                                                             | 65.5 ms: 1.10x slower                                                          |
| tornado_http               | 94.3 ms                                                             | 104 ms: 1.10x slower                                                           |
| pycparser                  | 777 ms                                                              | 858 ms: 1.10x slower                                                           |
| pyflate                    | 308 ms                                                              | 341 ms: 1.11x slower                                                           |
| django_template            | 30.1 ms                                                             | 33.3 ms: 1.11x slower                                                          |
| float                      | 52.4 ms                                                             | 58.2 ms: 1.11x slower                                                          |
| xml_etree_process          | 42.1 ms                                                             | 46.9 ms: 1.11x slower                                                          |
| coroutines                 | 15.5 ms                                                             | 17.3 ms: 1.12x slower                                                          |
| scimark_fft                | 198 ms                                                              | 221 ms: 1.12x slower                                                           |
| chaos                      | 48.0 ms                                                             | 53.6 ms: 1.12x slower                                                          |
| scimark_monte_carlo        | 45.2 ms                                                             | 50.6 ms: 1.12x slower                                                          |
| pickle_pure_python         | 213 us                                                              | 239 us: 1.12x slower                                                           |
| comprehensions             | 11.9 us                                                             | 13.3 us: 1.12x slower                                                          |
| richards_super             | 35.5 ms                                                             | 39.9 ms: 1.12x slower                                                          |
| spectral_norm              | 68.0 ms                                                             | 78.1 ms: 1.15x slower                                                          |
| fannkuch                   | 270 ms                                                              | 311 ms: 1.15x slower                                                           |
| scimark_lu                 | 59.4 ms                                                             | 68.4 ms: 1.15x slower                                                          |
| deltablue                  | 2.23 ms                                                             | 2.58 ms: 1.16x slower                                                          |
| unpickle_pure_python       | 147 us                                                              | 170 us: 1.16x slower                                                           |
| mako                       | 6.94 ms                                                             | 8.06 ms: 1.16x slower                                                          |
| hexiom                     | 4.23 ms                                                             | 4.95 ms: 1.17x slower                                                          |
| richards                   | 31.2 ms                                                             | 36.7 ms: 1.18x slower                                                          |
| raytrace                   | 189 ms                                                              | 222 ms: 1.18x slower                                                           |
| nbody                      | 76.9 ms                                                             | 90.9 ms: 1.18x slower                                                          |
| generators                 | 21.2 ms                                                             | 25.7 ms: 1.21x slower                                                          |
| scimark_sor                | 81.0 ms                                                             | 98.4 ms: 1.21x slower                                                          |
| logging_silent             | 57.9 ns                                                             | 72.6 ns: 1.25x slower                                                          |
| Geometric mean             | (ref)                                                               | 1.01x slower                                                                   |

Benchmark hidden because not significant (3): async_tree_memoization, gc_traversal, bench_thread_pool
Ignored benchmarks (8) of results/bm-20240605-3.13.0b2-3a83b17/bm-20240605-pythonperf1_win32-x86-python-v3.13.0b2-3.13.0b2-3a83b17.json: chameleon, flaskblogging, pickle, pickle_dict, pickle_list, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (1) of results/bm-20240727-3.14.0a0-04eb5c8/bm-20240727-pythonperf1_win32-x86-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8.json: dulwich_log

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.05x
- 95% likely to have a slowdown of 1.05x
- 99% likely to have a slowdown of 1.04x

# Memory
- memory change: unknown