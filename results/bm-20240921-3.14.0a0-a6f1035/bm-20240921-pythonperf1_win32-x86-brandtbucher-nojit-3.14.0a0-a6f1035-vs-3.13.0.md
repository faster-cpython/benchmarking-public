# Results vs. 3.13.0

- fork: brandtbucher
- ref: nojit
- machine: windows-x86
- commit hash: a6f1035
- commit date: 2024-09-21
- overall geometric mean: 1.02x faster
- HPT reliability: 99.26%
- HPT 99th percentile: 1.00x slower
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|----------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 1.82 sec                                                        | 1.86 sec: 1.03x slower                                                |
| html5lib       | 48.3 ms                                                         | 46.9 ms: 1.03x faster                                                 |
| tornado_http   | 104 ms                                                          | 95.1 ms: 1.10x faster                                                 |
| Geometric mean | (ref)                                                           | 1.02x faster                                                          |

Benchmark hidden because not significant (1): 2to3

Benchmarks with tag 'asyncio':
==============================

| Benchmark                 | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|---------------------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| asyncio_tcp               | 842 ms                                                          | 701 ms: 1.20x faster                                                  |
| async_tree_memoization_tg | 287 ms                                                          | 250 ms: 1.15x faster                                                  |
| async_tree_none           | 246 ms                                                          | 217 ms: 1.13x faster                                                  |
| async_tree_memoization    | 302 ms                                                          | 270 ms: 1.12x faster                                                  |
| async_tree_none_tg        | 219 ms                                                          | 200 ms: 1.09x faster                                                  |
| async_tree_cpu_io_mixed   | 494 ms                                                          | 470 ms: 1.05x faster                                                  |
| asyncio_tcp_ssl           | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                |
| async_tree_io_tg          | 509 ms                                                          | 520 ms: 1.02x slower                                                  |
| async_generators          | 274 ms                                                          | 307 ms: 1.12x slower                                                  |
| coroutines                | 15.7 ms                                                         | 18.7 ms: 1.19x slower                                                 |
| Geometric mean            | (ref)                                                           | 1.03x faster                                                          |

Benchmark hidden because not significant (2): async_tree_cpu_io_mixed_tg, async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|----------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 202 ms                                                          | 197 ms: 1.03x faster                                                  |
| float          | 57.8 ms                                                         | 63.2 ms: 1.09x slower                                                 |
| nbody          | 81.9 ms                                                         | 90.2 ms: 1.10x slower                                                 |
| Geometric mean | (ref)                                                           | 1.05x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|----------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_dna      | 117 ms                                                          | 119 ms: 1.02x slower                                                  |
| regex_v8       | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                                 |
| regex_compile  | 103 ms                                                          | 108 ms: 1.05x slower                                                  |
| regex_effbot   | 1.81 ms                                                         | 1.91 ms: 1.05x slower                                                 |
| Geometric mean | (ref)                                                           | 1.04x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|----------------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| unpickle_list        | 3.09 us                                                         | 2.85 us: 1.09x faster                                                 |
| pickle_dict          | 21.7 us                                                         | 20.6 us: 1.05x faster                                                 |
| pickle               | 8.42 us                                                         | 8.03 us: 1.05x faster                                                 |
| unpickle             | 10.5 us                                                         | 10.2 us: 1.03x faster                                                 |
| json_loads           | 21.0 us                                                         | 20.6 us: 1.02x faster                                                 |
| xml_etree_parse      | 109 ms                                                          | 108 ms: 1.01x faster                                                  |
| json_dumps           | 7.40 ms                                                         | 7.57 ms: 1.02x slower                                                 |
| xml_etree_iterparse  | 65.1 ms                                                         | 68.9 ms: 1.06x slower                                                 |
| tomli_loads          | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                |
| xml_etree_generate   | 62.6 ms                                                         | 67.0 ms: 1.07x slower                                                 |
| xml_etree_process    | 45.0 ms                                                         | 48.6 ms: 1.08x slower                                                 |
| pickle_pure_python   | 238 us                                                          | 260 us: 1.09x slower                                                  |
| unpickle_pure_python | 162 us                                                          | 182 us: 1.13x slower                                                  |
| Geometric mean       | (ref)                                                           | 1.02x slower                                                          |

Benchmark hidden because not significant (1): pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|------------------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup         | 24.3 ms                                                         | 23.7 ms: 1.03x faster                                                 |
| python_startup_no_site | 19.9 ms                                                         | 19.6 ms: 1.02x faster                                                 |
| Geometric mean         | (ref)                                                           | 1.02x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|-----------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| genshi_xml      | 49.5 ms                                                         | 47.6 ms: 1.04x faster                                                 |
| django_template | 32.7 ms                                                         | 32.1 ms: 1.02x faster                                                 |
| genshi_text     | 22.4 ms                                                         | 22.2 ms: 1.01x faster                                                 |
| mako            | 7.31 ms                                                         | 8.30 ms: 1.14x slower                                                 |
| Geometric mean  | (ref)                                                           | 1.02x slower                                                          |

All benchmarks:
===============

| Benchmark                 | bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5 | bm-20240921-pythonperf1_win32-x86-brandtbucher-nojit-3.14.0a0-a6f1035 |
|---------------------------|:---------------------------------------------------------------:|:---------------------------------------------------------------------:|
| thrift                    | 10.1 ms                                                         | 766 us: 13.24x faster                                                 |
| coverage                  | 333 ms                                                          | 53.2 ms: 6.26x faster                                                 |
| deepcopy                  | 307 us                                                          | 249 us: 1.23x faster                                                  |
| asyncio_tcp               | 842 ms                                                          | 701 ms: 1.20x faster                                                  |
| deepcopy_reduce           | 2.85 us                                                         | 2.42 us: 1.18x faster                                                 |
| async_tree_memoization_tg | 287 ms                                                          | 250 ms: 1.15x faster                                                  |
| deepcopy_memo             | 26.2 us                                                         | 23.1 us: 1.13x faster                                                 |
| async_tree_none           | 246 ms                                                          | 217 ms: 1.13x faster                                                  |
| async_tree_memoization    | 302 ms                                                          | 270 ms: 1.12x faster                                                  |
| tornado_http              | 104 ms                                                          | 95.1 ms: 1.10x faster                                                 |
| async_tree_none_tg        | 219 ms                                                          | 200 ms: 1.09x faster                                                  |
| unpickle_list             | 3.09 us                                                         | 2.85 us: 1.09x faster                                                 |
| pathlib                   | 89.4 ms                                                         | 83.8 ms: 1.07x faster                                                 |
| pickle_dict               | 21.7 us                                                         | 20.6 us: 1.05x faster                                                 |
| async_tree_cpu_io_mixed   | 494 ms                                                          | 470 ms: 1.05x faster                                                  |
| pickle                    | 8.42 us                                                         | 8.03 us: 1.05x faster                                                 |
| genshi_xml                | 49.5 ms                                                         | 47.6 ms: 1.04x faster                                                 |
| unpickle                  | 10.5 us                                                         | 10.2 us: 1.03x faster                                                 |
| bench_mp_pool             | 74.3 ms                                                         | 72.2 ms: 1.03x faster                                                 |
| gc_traversal              | 1.45 ms                                                         | 1.41 ms: 1.03x faster                                                 |
| html5lib                  | 48.3 ms                                                         | 46.9 ms: 1.03x faster                                                 |
| pidigits                  | 202 ms                                                          | 197 ms: 1.03x faster                                                  |
| go                        | 111 ms                                                          | 109 ms: 1.03x faster                                                  |
| python_startup            | 24.3 ms                                                         | 23.7 ms: 1.03x faster                                                 |
| telco                     | 6.67 ms                                                         | 6.54 ms: 1.02x faster                                                 |
| asyncio_tcp_ssl           | 17.4 sec                                                        | 17.1 sec: 1.02x faster                                                |
| django_template           | 32.7 ms                                                         | 32.1 ms: 1.02x faster                                                 |
| json_loads                | 21.0 us                                                         | 20.6 us: 1.02x faster                                                 |
| pycparser                 | 869 ms                                                          | 854 ms: 1.02x faster                                                  |
| python_startup_no_site    | 19.9 ms                                                         | 19.6 ms: 1.02x faster                                                 |
| pprint_safe_repr          | 644 ms                                                          | 636 ms: 1.01x faster                                                  |
| genshi_text               | 22.4 ms                                                         | 22.2 ms: 1.01x faster                                                 |
| xml_etree_parse           | 109 ms                                                          | 108 ms: 1.01x faster                                                  |
| sqlite_synth              | 1.97 us                                                         | 1.95 us: 1.01x faster                                                 |
| sympy_str                 | 215 ms                                                          | 217 ms: 1.01x slower                                                  |
| sympy_expand              | 375 ms                                                          | 380 ms: 1.01x slower                                                  |
| pprint_pformat            | 1.30 sec                                                        | 1.32 sec: 1.02x slower                                                |
| sympy_integrate           | 15.2 ms                                                         | 15.5 ms: 1.02x slower                                                 |
| async_tree_io_tg          | 509 ms                                                          | 520 ms: 1.02x slower                                                  |
| sqlglot_optimize          | 42.5 ms                                                         | 43.4 ms: 1.02x slower                                                 |
| logging_format            | 8.57 us                                                         | 8.76 us: 1.02x slower                                                 |
| regex_dna                 | 117 ms                                                          | 119 ms: 1.02x slower                                                  |
| json_dumps                | 7.40 ms                                                         | 7.57 ms: 1.02x slower                                                 |
| dulwich_log               | 49.7 ms                                                         | 50.8 ms: 1.02x slower                                                 |
| docutils                  | 1.82 sec                                                        | 1.86 sec: 1.03x slower                                                |
| logging_simple            | 7.87 us                                                         | 8.09 us: 1.03x slower                                                 |
| create_gc_cycles          | 713 us                                                          | 735 us: 1.03x slower                                                  |
| regex_v8                  | 15.6 ms                                                         | 16.2 ms: 1.04x slower                                                 |
| typing_runtime_protocols  | 136 us                                                          | 142 us: 1.04x slower                                                  |
| nqueens                   | 75.1 ms                                                         | 78.2 ms: 1.04x slower                                                 |
| sqlglot_normalize         | 220 ms                                                          | 230 ms: 1.04x slower                                                  |
| regex_compile             | 103 ms                                                          | 108 ms: 1.05x slower                                                  |
| sqlglot_parse             | 1.05 ms                                                         | 1.10 ms: 1.05x slower                                                 |
| sqlglot_transpile         | 1.29 ms                                                         | 1.36 ms: 1.05x slower                                                 |
| regex_effbot              | 1.81 ms                                                         | 1.91 ms: 1.05x slower                                                 |
| xml_etree_iterparse       | 65.1 ms                                                         | 68.9 ms: 1.06x slower                                                 |
| mdp                       | 1.67 sec                                                        | 1.77 sec: 1.06x slower                                                |
| meteor_contest            | 77.0 ms                                                         | 81.8 ms: 1.06x slower                                                 |
| tomli_loads               | 1.73 sec                                                        | 1.84 sec: 1.06x slower                                                |
| chaos                     | 51.2 ms                                                         | 54.6 ms: 1.07x slower                                                 |
| xml_etree_generate        | 62.6 ms                                                         | 67.0 ms: 1.07x slower                                                 |
| unpack_sequence           | 42.9 ns                                                         | 46.1 ns: 1.07x slower                                                 |
| scimark_sparse_mat_mult   | 2.90 ms                                                         | 3.13 ms: 1.08x slower                                                 |
| xml_etree_process         | 45.0 ms                                                         | 48.6 ms: 1.08x slower                                                 |
| pickle_pure_python        | 238 us                                                          | 260 us: 1.09x slower                                                  |
| float                     | 57.8 ms                                                         | 63.2 ms: 1.09x slower                                                 |
| nbody                     | 81.9 ms                                                         | 90.2 ms: 1.10x slower                                                 |
| pyflate                   | 326 ms                                                          | 364 ms: 1.12x slower                                                  |
| async_generators          | 274 ms                                                          | 307 ms: 1.12x slower                                                  |
| unpickle_pure_python      | 162 us                                                          | 182 us: 1.13x slower                                                  |
| scimark_lu                | 63.5 ms                                                         | 71.7 ms: 1.13x slower                                                 |
| mako                      | 7.31 ms                                                         | 8.30 ms: 1.14x slower                                                 |
| scimark_sor               | 91.8 ms                                                         | 105 ms: 1.14x slower                                                  |
| scimark_fft               | 206 ms                                                          | 236 ms: 1.15x slower                                                  |
| comprehensions            | 12.7 us                                                         | 14.6 us: 1.15x slower                                                 |
| hexiom                    | 4.64 ms                                                         | 5.33 ms: 1.15x slower                                                 |
| spectral_norm             | 71.3 ms                                                         | 82.1 ms: 1.15x slower                                                 |
| raytrace                  | 205 ms                                                          | 237 ms: 1.15x slower                                                  |
| scimark_monte_carlo       | 50.3 ms                                                         | 58.6 ms: 1.16x slower                                                 |
| fannkuch                  | 293 ms                                                          | 341 ms: 1.17x slower                                                  |
| deltablue                 | 2.41 ms                                                         | 2.82 ms: 1.17x slower                                                 |
| coroutines                | 15.7 ms                                                         | 18.7 ms: 1.19x slower                                                 |
| richards                  | 33.8 ms                                                         | 40.5 ms: 1.20x slower                                                 |
| richards_super            | 38.0 ms                                                         | 45.6 ms: 1.20x slower                                                 |
| logging_silent            | 61.6 ns                                                         | 75.2 ns: 1.22x slower                                                 |
| generators                | 22.1 ms                                                         | 27.2 ms: 1.23x slower                                                 |
| Geometric mean            | (ref)                                                           | 1.02x faster                                                          |

Benchmark hidden because not significant (9): json, crypto_pyaes, 2to3, async_tree_cpu_io_mixed_tg, bench_thread_pool, sympy_sum, async_tree_io, pickle_list, pylint
Ignored benchmarks (2) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf1_win32-x86-python-v3.13.0-3.13.0-60403a5.json: chameleon, flaskblogging

# HPT report

- Reliability score: 99.26% likely to be slow
- 90% likely to have a slowdown of 1.01x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: unknown