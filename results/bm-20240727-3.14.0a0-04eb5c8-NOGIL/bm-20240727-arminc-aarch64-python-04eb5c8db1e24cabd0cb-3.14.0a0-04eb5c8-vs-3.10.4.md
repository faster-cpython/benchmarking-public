# Results vs. 3.10.4

- fork: python
- ref: 04eb5c8db1e24cabd0cb
- machine: linux-aarch64
- commit hash: 04eb5c8
- commit date: 2024-07-27
- overall geometric mean: 1.21x slower
- HPT reliability: 100.00%
- HPT 99th percentile: 1.17x slower
- Memory change: 1.32x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 381 ms                                                                | 516 ms: 1.36x slower                                                    |
| docutils       | 3.53 sec                                                              | 4.15 sec: 1.18x slower                                                  |
| html5lib       | 86.5 ms                                                               | 121 ms: 1.41x slower                                                    |
| tornado_http   | 178 ms                                                                | 200 ms: 1.12x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.26x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark               | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|-------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_io           | 2.28 sec                                                              | 1.44 sec: 1.59x faster                                                  |
| async_tree_none         | 950 ms                                                                | 631 ms: 1.51x faster                                                    |
| async_tree_memoization  | 1.13 sec                                                              | 767 ms: 1.48x faster                                                    |
| async_tree_cpu_io_mixed | 1.27 sec                                                              | 942 ms: 1.35x faster                                                    |
| Geometric mean          | (ref)                                                                 | 1.48x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 135 ms                                                                | 207 ms: 1.54x slower                                                    |
| nbody          | 166 ms                                                                | 285 ms: 1.72x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.38x slower                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_dna      | 257 ms                                                                | 253 ms: 1.02x faster                                                    |
| regex_v8       | 32.2 ms                                                               | 32.8 ms: 1.02x slower                                                   |
| regex_effbot   | 4.25 ms                                                               | 4.41 ms: 1.04x slower                                                   |
| regex_compile  | 177 ms                                                                | 256 ms: 1.45x slower                                                    |
| Geometric mean | (ref)                                                                 | 1.11x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|----------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_parse      | 212 ms                                                                | 177 ms: 1.20x faster                                                    |
| json_dumps           | 16.7 ms                                                               | 17.9 ms: 1.07x slower                                                   |
| tomli_loads          | 3.36 sec                                                              | 4.19 sec: 1.25x slower                                                  |
| json_loads           | 30.9 us                                                               | 39.1 us: 1.26x slower                                                   |
| xml_etree_process    | 99.5 ms                                                               | 130 ms: 1.31x slower                                                    |
| xml_etree_generate   | 123 ms                                                                | 162 ms: 1.32x slower                                                    |
| pickle_pure_python   | 529 us                                                                | 781 us: 1.48x slower                                                    |
| unpickle_pure_python | 366 us                                                                | 550 us: 1.50x slower                                                    |
| Geometric mean       | (ref)                                                                 | 1.21x slower                                                            |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 11.2 ms                                                               | 17.6 ms: 1.57x slower                                                   |
| python_startup_no_site | 6.89 ms                                                               | 11.7 ms: 1.70x slower                                                   |
| Geometric mean         | (ref)                                                                 | 1.63x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|-----------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_xml      | 69.8 ms                                                               | 105 ms: 1.50x slower                                                    |
| genshi_text     | 35.2 ms                                                               | 52.8 ms: 1.50x slower                                                   |
| mako            | 18.9 ms                                                               | 29.1 ms: 1.54x slower                                                   |
| django_template | 53.3 ms                                                               | 82.3 ms: 1.54x slower                                                   |
| Geometric mean  | (ref)                                                                 | 1.52x slower                                                            |

All benchmarks:
===============

| Benchmark                | bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120 | bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8 |
|--------------------------|:---------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| bench_mp_pool            | 14.5 ms                                                               | 6.14 ms: 2.37x faster                                                   |
| typing_runtime_protocols | 661 us                                                                | 334 us: 1.98x faster                                                    |
| async_tree_io            | 2.28 sec                                                              | 1.44 sec: 1.59x faster                                                  |
| async_tree_none          | 950 ms                                                                | 631 ms: 1.51x faster                                                    |
| async_tree_memoization   | 1.13 sec                                                              | 767 ms: 1.48x faster                                                    |
| asyncio_tcp              | 944 ms                                                                | 660 ms: 1.43x faster                                                    |
| create_gc_cycles         | 2.26 ms                                                               | 1.60 ms: 1.41x faster                                                   |
| async_tree_cpu_io_mixed  | 1.27 sec                                                              | 942 ms: 1.35x faster                                                    |
| asyncio_tcp_ssl          | 3.28 sec                                                              | 2.46 sec: 1.33x faster                                                  |
| gc_traversal             | 4.15 ms                                                               | 3.45 ms: 1.20x faster                                                   |
| xml_etree_parse          | 212 ms                                                                | 177 ms: 1.20x faster                                                    |
| generators               | 68.1 ms                                                               | 57.0 ms: 1.20x faster                                                   |
| regex_dna                | 257 ms                                                                | 253 ms: 1.02x faster                                                    |
| pathlib                  | 26.3 ms                                                               | 26.0 ms: 1.01x faster                                                   |
| regex_v8                 | 32.2 ms                                                               | 32.8 ms: 1.02x slower                                                   |
| asyncio_websockets       | 657 ms                                                                | 672 ms: 1.02x slower                                                    |
| regex_effbot             | 4.25 ms                                                               | 4.41 ms: 1.04x slower                                                   |
| pylint                   | 485 ms                                                                | 508 ms: 1.05x slower                                                    |
| coroutines               | 37.2 ms                                                               | 39.0 ms: 1.05x slower                                                   |
| crypto_pyaes             | 134 ms                                                                | 141 ms: 1.05x slower                                                    |
| json_dumps               | 16.7 ms                                                               | 17.9 ms: 1.07x slower                                                   |
| deepcopy                 | 511 us                                                                | 566 us: 1.11x slower                                                    |
| tornado_http             | 178 ms                                                                | 200 ms: 1.12x slower                                                    |
| scimark_sparse_mat_mult  | 7.62 ms                                                               | 8.82 ms: 1.16x slower                                                   |
| deepcopy_memo            | 61.7 us                                                               | 71.9 us: 1.17x slower                                                   |
| mdp                      | 3.70 sec                                                              | 4.33 sec: 1.17x slower                                                  |
| bench_thread_pool        | 1.59 ms                                                               | 1.87 ms: 1.18x slower                                                   |
| docutils                 | 3.53 sec                                                              | 4.15 sec: 1.18x slower                                                  |
| json                     | 5.88 ms                                                               | 6.96 ms: 1.18x slower                                                   |
| scimark_fft              | 500 ms                                                                | 593 ms: 1.19x slower                                                    |
| pycparser                | 1.69 sec                                                              | 2.06 sec: 1.22x slower                                                  |
| comprehensions           | 33.1 us                                                               | 41.3 us: 1.25x slower                                                   |
| tomli_loads              | 3.36 sec                                                              | 4.19 sec: 1.25x slower                                                  |
| spectral_norm            | 186 ms                                                                | 233 ms: 1.25x slower                                                    |
| json_loads               | 30.9 us                                                               | 39.1 us: 1.26x slower                                                   |
| logging_silent           | 222 ns                                                                | 285 ns: 1.28x slower                                                    |
| pyflate                  | 795 ms                                                                | 1.02 sec: 1.29x slower                                                  |
| nqueens                  | 117 ms                                                                | 152 ms: 1.29x slower                                                    |
| richards_super           | 107 ms                                                                | 139 ms: 1.30x slower                                                    |
| xml_etree_process        | 99.5 ms                                                               | 130 ms: 1.31x slower                                                    |
| sympy_integrate          | 26.5 ms                                                               | 34.9 ms: 1.32x slower                                                   |
| xml_etree_generate       | 123 ms                                                                | 162 ms: 1.32x slower                                                    |
| deepcopy_reduce          | 4.60 us                                                               | 6.07 us: 1.32x slower                                                   |
| richards                 | 91.7 ms                                                               | 121 ms: 1.32x slower                                                    |
| chaos                    | 121 ms                                                                | 162 ms: 1.34x slower                                                    |
| thrift                   | 1.26 ms                                                               | 1.69 ms: 1.34x slower                                                   |
| 2to3                     | 381 ms                                                                | 516 ms: 1.36x slower                                                    |
| meteor_contest           | 126 ms                                                                | 173 ms: 1.37x slower                                                    |
| fannkuch                 | 546 ms                                                                | 758 ms: 1.39x slower                                                    |
| scimark_sor              | 246 ms                                                                | 345 ms: 1.40x slower                                                    |
| scimark_monte_carlo      | 128 ms                                                                | 179 ms: 1.40x slower                                                    |
| html5lib                 | 86.5 ms                                                               | 121 ms: 1.41x slower                                                    |
| deltablue                | 8.94 ms                                                               | 12.8 ms: 1.44x slower                                                   |
| raytrace                 | 573 ms                                                                | 826 ms: 1.44x slower                                                    |
| hexiom                   | 10.9 ms                                                               | 15.8 ms: 1.45x slower                                                   |
| regex_compile            | 177 ms                                                                | 256 ms: 1.45x slower                                                    |
| telco                    | 8.49 ms                                                               | 12.4 ms: 1.46x slower                                                   |
| pickle_pure_python       | 529 us                                                                | 781 us: 1.48x slower                                                    |
| sqlglot_normalize        | 156 ms                                                                | 231 ms: 1.48x slower                                                    |
| logging_format           | 10.6 us                                                               | 15.8 us: 1.49x slower                                                   |
| genshi_xml               | 69.8 ms                                                               | 105 ms: 1.50x slower                                                    |
| genshi_text              | 35.2 ms                                                               | 52.8 ms: 1.50x slower                                                   |
| logging_simple           | 9.78 us                                                               | 14.7 us: 1.50x slower                                                   |
| unpickle_pure_python     | 366 us                                                                | 550 us: 1.50x slower                                                    |
| scimark_lu               | 227 ms                                                                | 343 ms: 1.51x slower                                                    |
| sqlglot_transpile        | 2.84 ms                                                               | 4.29 ms: 1.51x slower                                                   |
| sqlglot_optimize         | 75.4 ms                                                               | 114 ms: 1.52x slower                                                    |
| async_generators         | 452 ms                                                                | 692 ms: 1.53x slower                                                    |
| mako                     | 18.9 ms                                                               | 29.1 ms: 1.54x slower                                                   |
| sqlglot_parse            | 2.40 ms                                                               | 3.70 ms: 1.54x slower                                                   |
| float                    | 135 ms                                                                | 207 ms: 1.54x slower                                                    |
| pprint_pformat           | 2.36 sec                                                              | 3.64 sec: 1.54x slower                                                  |
| django_template          | 53.3 ms                                                               | 82.3 ms: 1.54x slower                                                   |
| pprint_safe_repr         | 1.15 sec                                                              | 1.77 sec: 1.55x slower                                                  |
| sympy_str                | 329 ms                                                                | 514 ms: 1.56x slower                                                    |
| python_startup           | 11.2 ms                                                               | 17.6 ms: 1.57x slower                                                   |
| coverage                 | 83.6 ms                                                               | 135 ms: 1.62x slower                                                    |
| go                       | 264 ms                                                                | 448 ms: 1.70x slower                                                    |
| python_startup_no_site   | 6.89 ms                                                               | 11.7 ms: 1.70x slower                                                   |
| sympy_sum                | 184 ms                                                                | 313 ms: 1.70x slower                                                    |
| nbody                    | 166 ms                                                                | 285 ms: 1.72x slower                                                    |
| sympy_expand             | 543 ms                                                                | 951 ms: 1.75x slower                                                    |
| Geometric mean           | (ref)                                                                 | 1.21x slower                                                            |

Benchmark hidden because not significant (2): xml_etree_iterparse, pidigits
Ignored benchmarks (15) of results/bm-20220323-3.10.4-9d38120/bm-20220323-arminc-aarch64-python-9d38120e335357a3b294-3.10.4-9d38120.json: aiohttp, chameleon, dask, dulwich_log, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlite_synth, unpickle, unpickle_list
Ignored benchmarks (5) of results/bm-20240727-3.14.0a0-04eb5c8-NOGIL/bm-20240727-arminc-aarch64-python-04eb5c8db1e24cabd0cb-3.14.0a0-04eb5c8.json: async_tree_cpu_io_mixed_tg, async_tree_io_tg, async_tree_memoization_tg, async_tree_none_tg, bpe_tokeniser

# HPT report

- Reliability score: 100.00% likely to be slow
- 90% likely to have a slowdown of 1.22x
- 95% likely to have a slowdown of 1.19x
- 99% likely to have a slowdown of 1.17x

# Memory
- memory change: 1.32x