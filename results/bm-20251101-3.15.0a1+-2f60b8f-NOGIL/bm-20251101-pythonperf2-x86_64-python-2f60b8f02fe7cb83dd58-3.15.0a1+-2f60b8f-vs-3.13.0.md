# Results vs. 3.13.0

- fork: python
- ref: 2f60b8f02fe7cb83dd58
- machine: linux-x86_64
- commit hash: 2f60b8f
- commit date: 2025-11-01
- overall geometric mean: 1.048x slower
- HPT reliability: 91.75%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.33x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| 2to3           | 293 ms                                                       | 310 ms: 1.06x slower                                                         |
| docutils       | 2.83 sec                                                     | 2.84 sec: 1.00x slower                                                       |
| html5lib       | 73.5 ms                                                      | 69.7 ms: 1.05x faster                                                        |
| sphinx         | 1.12 sec                                                     | 1.14 sec: 1.02x slower                                                       |
| Geometric mean | (ref)                                                        | 1.01x slower                                                                 |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| async_tree_memoization_tg  | 466 ms                                                       | 293 ms: 1.59x faster                                                         |
| async_tree_io_tg           | 831 ms                                                       | 527 ms: 1.58x faster                                                         |
| async_tree_io              | 843 ms                                                       | 559 ms: 1.51x faster                                                         |
| async_tree_none_tg         | 346 ms                                                       | 231 ms: 1.50x faster                                                         |
| async_tree_none            | 376 ms                                                       | 262 ms: 1.43x faster                                                         |
| async_tree_memoization     | 453 ms                                                       | 319 ms: 1.42x faster                                                         |
| async_tree_cpu_io_mixed_tg | 581 ms                                                       | 467 ms: 1.24x faster                                                         |
| async_tree_cpu_io_mixed    | 604 ms                                                       | 498 ms: 1.21x faster                                                         |
| asyncio_websockets         | 388 ms                                                       | 375 ms: 1.04x faster                                                         |
| async_generators           | 457 ms                                                       | 466 ms: 1.02x slower                                                         |
| coroutines                 | 21.6 ms                                                      | 22.6 ms: 1.05x slower                                                        |
| Geometric mean             | (ref)                                                        | 1.29x faster                                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| float          | 81.3 ms                                                      | 71.4 ms: 1.14x faster                                                        |
| pidigits       | 252 ms                                                       | 253 ms: 1.00x slower                                                         |
| nbody          | 89.3 ms                                                      | 125 ms: 1.40x slower                                                         |
| Geometric mean | (ref)                                                        | 1.07x slower                                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| regex_v8       | 26.1 ms                                                      | 23.3 ms: 1.12x faster                                                        |
| regex_effbot   | 3.67 ms                                                      | 3.40 ms: 1.08x faster                                                        |
| regex_dna      | 247 ms                                                       | 248 ms: 1.01x slower                                                         |
| regex_compile  | 143 ms                                                       | 154 ms: 1.08x slower                                                         |
| Geometric mean | (ref)                                                        | 1.03x faster                                                                 |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| xml_etree_iterparse  | 103 ms                                                       | 88.2 ms: 1.17x faster                                                        |
| tomli_loads          | 2.46 sec                                                     | 2.14 sec: 1.15x faster                                                       |
| xml_etree_parse      | 150 ms                                                       | 137 ms: 1.10x faster                                                         |
| json_dumps           | 11.1 ms                                                      | 11.9 ms: 1.07x slower                                                        |
| unpickle_pure_python | 217 us                                                       | 240 us: 1.11x slower                                                         |
| json_loads           | 24.7 us                                                      | 27.9 us: 1.13x slower                                                        |
| xml_etree_generate   | 86.5 ms                                                      | 97.9 ms: 1.13x slower                                                        |
| pickle_pure_python   | 323 us                                                       | 366 us: 1.13x slower                                                         |
| xml_etree_process    | 61.2 ms                                                      | 69.4 ms: 1.13x slower                                                        |
| Geometric mean       | (ref)                                                        | 1.03x slower                                                                 |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| python_startup         | 15.9 ms                                                      | 19.2 ms: 1.21x slower                                                        |
| python_startup_no_site | 8.96 ms                                                      | 11.8 ms: 1.31x slower                                                        |
| Geometric mean         | (ref)                                                        | 1.26x slower                                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| genshi_text     | 26.2 ms                                                      | 30.1 ms: 1.15x slower                                                        |
| genshi_xml      | 57.1 ms                                                      | 66.4 ms: 1.16x slower                                                        |
| django_template | 36.4 ms                                                      | 43.8 ms: 1.20x slower                                                        |
| mako            | 10.4 ms                                                      | 17.3 ms: 1.67x slower                                                        |
| Geometric mean  | (ref)                                                        | 1.28x slower                                                                 |

All benchmarks:
===============

| Benchmark                  | bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| gc_traversal               | 4.74 ms                                                      | 2.21 ms: 2.14x faster                                                        |
| mdp                        | 2.54 sec                                                     | 1.43 sec: 1.77x faster                                                       |
| async_tree_memoization_tg  | 466 ms                                                       | 293 ms: 1.59x faster                                                         |
| async_tree_io_tg           | 831 ms                                                       | 527 ms: 1.58x faster                                                         |
| async_tree_io              | 843 ms                                                       | 559 ms: 1.51x faster                                                         |
| async_tree_none_tg         | 346 ms                                                       | 231 ms: 1.50x faster                                                         |
| async_tree_none            | 376 ms                                                       | 262 ms: 1.43x faster                                                         |
| async_tree_memoization     | 453 ms                                                       | 319 ms: 1.42x faster                                                         |
| create_gc_cycles           | 2.68 ms                                                      | 1.92 ms: 1.40x faster                                                        |
| deepcopy                   | 392 us                                                       | 301 us: 1.30x faster                                                         |
| deepcopy_memo              | 38.6 us                                                      | 30.5 us: 1.27x faster                                                        |
| async_tree_cpu_io_mixed_tg | 581 ms                                                       | 467 ms: 1.24x faster                                                         |
| async_tree_cpu_io_mixed    | 604 ms                                                       | 498 ms: 1.21x faster                                                         |
| xml_etree_iterparse        | 103 ms                                                       | 88.2 ms: 1.17x faster                                                        |
| go                         | 162 ms                                                       | 140 ms: 1.16x faster                                                         |
| tomli_loads                | 2.46 sec                                                     | 2.14 sec: 1.15x faster                                                       |
| pathlib                    | 17.5 ms                                                      | 15.3 ms: 1.15x faster                                                        |
| float                      | 81.3 ms                                                      | 71.4 ms: 1.14x faster                                                        |
| generators                 | 33.6 ms                                                      | 29.8 ms: 1.13x faster                                                        |
| regex_v8                   | 26.1 ms                                                      | 23.3 ms: 1.12x faster                                                        |
| sqlite_synth               | 2.91 us                                                      | 2.60 us: 1.12x faster                                                        |
| dulwich_log                | 68.2 ms                                                      | 61.9 ms: 1.10x faster                                                        |
| xml_etree_parse            | 150 ms                                                       | 137 ms: 1.10x faster                                                         |
| regex_effbot               | 3.67 ms                                                      | 3.40 ms: 1.08x faster                                                        |
| pyflate                    | 503 ms                                                       | 471 ms: 1.07x faster                                                         |
| scimark_sor                | 123 ms                                                       | 117 ms: 1.06x faster                                                         |
| html5lib                   | 73.5 ms                                                      | 69.7 ms: 1.05x faster                                                        |
| deepcopy_reduce            | 3.54 us                                                      | 3.38 us: 1.05x faster                                                        |
| asyncio_websockets         | 388 ms                                                       | 375 ms: 1.04x faster                                                         |
| pycparser                  | 1.24 sec                                                     | 1.21 sec: 1.03x faster                                                       |
| json                       | 5.69 ms                                                      | 5.53 ms: 1.03x faster                                                        |
| scimark_fft                | 328 ms                                                       | 322 ms: 1.02x faster                                                         |
| hexiom                     | 6.55 ms                                                      | 6.48 ms: 1.01x faster                                                        |
| logging_silent             | 97.9 ns                                                      | 97.0 ns: 1.01x faster                                                        |
| bpe_tokeniser              | 5.09 sec                                                     | 5.06 sec: 1.01x faster                                                       |
| pidigits                   | 252 ms                                                       | 253 ms: 1.00x slower                                                         |
| docutils                   | 2.83 sec                                                     | 2.84 sec: 1.00x slower                                                       |
| regex_dna                  | 247 ms                                                       | 248 ms: 1.01x slower                                                         |
| sphinx                     | 1.12 sec                                                     | 1.14 sec: 1.02x slower                                                       |
| async_generators           | 457 ms                                                       | 466 ms: 1.02x slower                                                         |
| coroutines                 | 21.6 ms                                                      | 22.6 ms: 1.05x slower                                                        |
| pprint_safe_repr           | 843 ms                                                       | 883 ms: 1.05x slower                                                         |
| spectral_norm              | 97.0 ms                                                      | 102 ms: 1.05x slower                                                         |
| 2to3                       | 293 ms                                                       | 310 ms: 1.06x slower                                                         |
| sympy_integrate            | 23.6 ms                                                      | 25.0 ms: 1.06x slower                                                        |
| pprint_pformat             | 1.72 sec                                                     | 1.83 sec: 1.06x slower                                                       |
| deltablue                  | 3.42 ms                                                      | 3.64 ms: 1.06x slower                                                        |
| richards                   | 52.9 ms                                                      | 56.4 ms: 1.07x slower                                                        |
| logging_simple             | 6.39 us                                                      | 6.83 us: 1.07x slower                                                        |
| json_dumps                 | 11.1 ms                                                      | 11.9 ms: 1.07x slower                                                        |
| thrift                     | 901 us                                                       | 965 us: 1.07x slower                                                         |
| k_core                     | 2.17 sec                                                     | 2.33 sec: 1.07x slower                                                       |
| comprehensions             | 17.0 us                                                      | 18.3 us: 1.08x slower                                                        |
| regex_compile              | 143 ms                                                       | 154 ms: 1.08x slower                                                         |
| richards_super             | 59.6 ms                                                      | 64.5 ms: 1.08x slower                                                        |
| sympy_expand               | 509 ms                                                       | 552 ms: 1.08x slower                                                         |
| sympy_str                  | 298 ms                                                       | 325 ms: 1.09x slower                                                         |
| sympy_sum                  | 155 ms                                                       | 170 ms: 1.10x slower                                                         |
| chaos                      | 60.2 ms                                                      | 66.5 ms: 1.10x slower                                                        |
| meteor_contest             | 130 ms                                                       | 143 ms: 1.11x slower                                                         |
| unpickle_pure_python       | 217 us                                                       | 240 us: 1.11x slower                                                         |
| logging_format             | 6.94 us                                                      | 7.75 us: 1.12x slower                                                        |
| shortest_path              | 460 ms                                                       | 516 ms: 1.12x slower                                                         |
| connected_components       | 432 ms                                                       | 486 ms: 1.12x slower                                                         |
| json_loads                 | 24.7 us                                                      | 27.9 us: 1.13x slower                                                        |
| xml_etree_generate         | 86.5 ms                                                      | 97.9 ms: 1.13x slower                                                        |
| pickle_pure_python         | 323 us                                                       | 366 us: 1.13x slower                                                         |
| xml_etree_process          | 61.2 ms                                                      | 69.4 ms: 1.13x slower                                                        |
| genshi_text                | 26.2 ms                                                      | 30.1 ms: 1.15x slower                                                        |
| nqueens                    | 90.7 ms                                                      | 105 ms: 1.16x slower                                                         |
| genshi_xml                 | 57.1 ms                                                      | 66.4 ms: 1.16x slower                                                        |
| raytrace                   | 273 ms                                                       | 322 ms: 1.18x slower                                                         |
| scimark_sparse_mat_mult    | 4.77 ms                                                      | 5.69 ms: 1.19x slower                                                        |
| scimark_lu                 | 98.7 ms                                                      | 118 ms: 1.19x slower                                                         |
| django_template            | 36.4 ms                                                      | 43.8 ms: 1.20x slower                                                        |
| python_startup             | 15.9 ms                                                      | 19.2 ms: 1.21x slower                                                        |
| fannkuch                   | 363 ms                                                       | 445 ms: 1.23x slower                                                         |
| scimark_monte_carlo        | 66.1 ms                                                      | 81.5 ms: 1.23x slower                                                        |
| bench_mp_pool              | 5.12 ms                                                      | 6.39 ms: 1.25x slower                                                        |
| typing_runtime_protocols   | 169 us                                                       | 213 us: 1.26x slower                                                         |
| crypto_pyaes               | 73.3 ms                                                      | 93.5 ms: 1.28x slower                                                        |
| python_startup_no_site     | 8.96 ms                                                      | 11.8 ms: 1.31x slower                                                        |
| many_optionals             | 930 us                                                       | 1.28 ms: 1.37x slower                                                        |
| nbody                      | 89.3 ms                                                      | 125 ms: 1.40x slower                                                         |
| bench_thread_pool          | 942 us                                                       | 1.35 ms: 1.44x slower                                                        |
| coverage                   | 80.0 ms                                                      | 121 ms: 1.51x slower                                                         |
| mako                       | 10.4 ms                                                      | 17.3 ms: 1.67x slower                                                        |
| subparsers                 | 17.5 ms                                                      | 50.4 ms: 2.88x slower                                                        |
| telco                      | 8.79 ms                                                      | 174 ms: 19.82x slower                                                        |
| Geometric mean             | (ref)                                                        | 1.05x slower                                                                 |

Benchmark hidden because not significant (1): pylint
Ignored benchmarks (11) of results/bm-20241007-3.13.0-60403a5/bm-20241007-pythonperf2-x86_64-python-v3.13.0-3.13.0-60403a5.json: chameleon, djangocms, gevent_hub, gunicorn, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20251101-3.15.0a1+-2f60b8f-NOGIL/bm-20251101-pythonperf2-x86_64-python-2f60b8f02fe7cb83dd58-3.15.0a1+-2f60b8f.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.048x slower

# HPT report

- Reliability score: 91.75% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.33x