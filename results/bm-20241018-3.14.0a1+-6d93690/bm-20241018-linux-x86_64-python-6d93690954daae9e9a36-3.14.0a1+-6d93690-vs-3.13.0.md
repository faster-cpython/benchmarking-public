# Results vs. 3.13.0

- fork: python
- ref: 6d93690954daae9e9a36
- machine: linux-x86_64
- commit hash: 6d93690
- commit date: 2024-10-18
- overall geometric mean: 1.019x faster
- HPT reliability: 96.70%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.02x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 267 ms                                                 | 254 ms: 1.05x faster                                                   |
| docutils       | 2.59 sec                                               | 2.66 sec: 1.03x slower                                                 |
| html5lib       | 64.2 ms                                                | 63.0 ms: 1.02x faster                                                  |
| tornado_http   | 92.4 ms                                                | 89.7 ms: 1.03x faster                                                  |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (1): sphinx

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_memoization_tg  | 464 ms                                                 | 374 ms: 1.24x faster                                                   |
| async_tree_none            | 351 ms                                                 | 325 ms: 1.08x faster                                                   |
| async_tree_memoization     | 442 ms                                                 | 410 ms: 1.08x faster                                                   |
| async_tree_cpu_io_mixed    | 577 ms                                                 | 565 ms: 1.02x faster                                                   |
| async_generators           | 436 ms                                                 | 433 ms: 1.01x faster                                                   |
| asyncio_websockets         | 552 ms                                                 | 555 ms: 1.01x slower                                                   |
| coroutines                 | 22.7 ms                                                | 23.0 ms: 1.01x slower                                                  |
| async_tree_cpu_io_mixed_tg | 546 ms                                                 | 555 ms: 1.02x slower                                                   |
| async_tree_io_tg           | 857 ms                                                 | 960 ms: 1.12x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (2): async_tree_none_tg, async_tree_io

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| pidigits       | 186 ms                                                 | 187 ms: 1.01x slower                                                   |
| nbody          | 87.0 ms                                                | 91.2 ms: 1.05x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x slower                                                           |

Benchmark hidden because not significant (1): float

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_v8       | 26.2 ms                                                | 25.3 ms: 1.03x faster                                                  |
| regex_compile  | 132 ms                                                 | 128 ms: 1.03x faster                                                   |
| regex_dna      | 218 ms                                                 | 213 ms: 1.03x faster                                                   |
| regex_effbot   | 3.73 ms                                                | 3.75 ms: 1.00x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_loads           | 27.2 us                                                | 26.3 us: 1.04x faster                                                  |
| xml_etree_process    | 60.6 ms                                                | 59.5 ms: 1.02x faster                                                  |
| tomli_loads          | 2.14 sec                                               | 2.10 sec: 1.02x faster                                                 |
| xml_etree_generate   | 86.7 ms                                                | 85.2 ms: 1.02x faster                                                  |
| unpickle_pure_python | 216 us                                                 | 217 us: 1.01x slower                                                   |
| xml_etree_iterparse  | 101 ms                                                 | 105 ms: 1.04x slower                                                   |
| json_dumps           | 10.6 ms                                                | 11.3 ms: 1.07x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.00x slower                                                           |

Benchmark hidden because not significant (2): pickle_pure_python, xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup         | 12.5 ms                                                | 11.8 ms: 1.06x faster                                                  |
| python_startup_no_site | 6.96 ms                                                | 7.02 ms: 1.01x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 23.5 ms                                                | 22.8 ms: 1.03x faster                                                  |
| genshi_xml      | 50.9 ms                                                | 49.9 ms: 1.02x faster                                                  |
| django_template | 35.2 ms                                                | 34.6 ms: 1.02x faster                                                  |
| mako            | 11.1 ms                                                | 11.5 ms: 1.04x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.01x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5 | bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| deepcopy                   | 358 us                                                 | 262 us: 1.37x faster                                                   |
| deepcopy_memo              | 39.1 us                                                | 31.1 us: 1.26x faster                                                  |
| async_tree_memoization_tg  | 464 ms                                                 | 374 ms: 1.24x faster                                                   |
| go                         | 144 ms                                                 | 121 ms: 1.19x faster                                                   |
| deepcopy_reduce            | 3.20 us                                                | 2.72 us: 1.18x faster                                                  |
| json                       | 5.36 ms                                                | 4.83 ms: 1.11x faster                                                  |
| pathlib                    | 17.5 ms                                                | 16.1 ms: 1.09x faster                                                  |
| async_tree_none            | 351 ms                                                 | 325 ms: 1.08x faster                                                   |
| async_tree_memoization     | 442 ms                                                 | 410 ms: 1.08x faster                                                   |
| telco                      | 8.54 ms                                                | 7.97 ms: 1.07x faster                                                  |
| pycparser                  | 1.20 sec                                               | 1.13 sec: 1.07x faster                                                 |
| mdp                        | 2.72 sec                                               | 2.56 sec: 1.06x faster                                                 |
| logging_format             | 6.40 us                                                | 6.04 us: 1.06x faster                                                  |
| generators                 | 29.0 ms                                                | 27.4 ms: 1.06x faster                                                  |
| python_startup             | 12.5 ms                                                | 11.8 ms: 1.06x faster                                                  |
| crypto_pyaes               | 75.3 ms                                                | 71.3 ms: 1.06x faster                                                  |
| 2to3                       | 267 ms                                                 | 254 ms: 1.05x faster                                                   |
| logging_simple             | 5.72 us                                                | 5.48 us: 1.04x faster                                                  |
| richards_super             | 54.9 ms                                                | 52.7 ms: 1.04x faster                                                  |
| richards                   | 48.7 ms                                                | 46.8 ms: 1.04x faster                                                  |
| json_loads                 | 27.2 us                                                | 26.3 us: 1.04x faster                                                  |
| regex_v8                   | 26.2 ms                                                | 25.3 ms: 1.03x faster                                                  |
| spectral_norm              | 115 ms                                                 | 111 ms: 1.03x faster                                                   |
| regex_compile              | 132 ms                                                 | 128 ms: 1.03x faster                                                   |
| genshi_text                | 23.5 ms                                                | 22.8 ms: 1.03x faster                                                  |
| scimark_sparse_mat_mult    | 5.04 ms                                                | 4.89 ms: 1.03x faster                                                  |
| sympy_sum                  | 150 ms                                                 | 146 ms: 1.03x faster                                                   |
| sympy_str                  | 275 ms                                                 | 267 ms: 1.03x faster                                                   |
| tornado_http               | 92.4 ms                                                | 89.7 ms: 1.03x faster                                                  |
| regex_dna                  | 218 ms                                                 | 213 ms: 1.03x faster                                                   |
| typing_runtime_protocols   | 165 us                                                 | 161 us: 1.02x faster                                                   |
| sympy_expand               | 463 ms                                                 | 454 ms: 1.02x faster                                                   |
| sqlglot_normalize          | 108 ms                                                 | 105 ms: 1.02x faster                                                   |
| genshi_xml                 | 50.9 ms                                                | 49.9 ms: 1.02x faster                                                  |
| async_tree_cpu_io_mixed    | 577 ms                                                 | 565 ms: 1.02x faster                                                   |
| xml_etree_process          | 60.6 ms                                                | 59.5 ms: 1.02x faster                                                  |
| tomli_loads                | 2.14 sec                                               | 2.10 sec: 1.02x faster                                                 |
| html5lib                   | 64.2 ms                                                | 63.0 ms: 1.02x faster                                                  |
| thrift                     | 809 us                                                 | 795 us: 1.02x faster                                                   |
| xml_etree_generate         | 86.7 ms                                                | 85.2 ms: 1.02x faster                                                  |
| django_template            | 35.2 ms                                                | 34.6 ms: 1.02x faster                                                  |
| sqlglot_optimize           | 53.7 ms                                                | 53.0 ms: 1.01x faster                                                  |
| dulwich_log                | 64.3 ms                                                | 63.5 ms: 1.01x faster                                                  |
| async_generators           | 436 ms                                                 | 433 ms: 1.01x faster                                                   |
| meteor_contest             | 109 ms                                                 | 108 ms: 1.01x faster                                                   |
| sympy_integrate            | 19.9 ms                                                | 19.8 ms: 1.01x faster                                                  |
| coverage                   | 84.0 ms                                                | 83.6 ms: 1.00x faster                                                  |
| sqlglot_parse              | 1.27 ms                                                | 1.28 ms: 1.00x slower                                                  |
| regex_effbot               | 3.73 ms                                                | 3.75 ms: 1.00x slower                                                  |
| sqlglot_transpile          | 1.58 ms                                                | 1.59 ms: 1.00x slower                                                  |
| pprint_pformat             | 1.49 sec                                               | 1.50 sec: 1.01x slower                                                 |
| pidigits                   | 186 ms                                                 | 187 ms: 1.01x slower                                                   |
| asyncio_websockets         | 552 ms                                                 | 555 ms: 1.01x slower                                                   |
| scimark_fft                | 364 ms                                                 | 367 ms: 1.01x slower                                                   |
| unpickle_pure_python       | 216 us                                                 | 217 us: 1.01x slower                                                   |
| bpe_tokeniser              | 4.75 sec                                               | 4.78 sec: 1.01x slower                                                 |
| python_startup_no_site     | 6.96 ms                                                | 7.02 ms: 1.01x slower                                                  |
| coroutines                 | 22.7 ms                                                | 23.0 ms: 1.01x slower                                                  |
| scimark_monte_carlo        | 67.4 ms                                                | 68.4 ms: 1.01x slower                                                  |
| async_tree_cpu_io_mixed_tg | 546 ms                                                 | 555 ms: 1.02x slower                                                   |
| scimark_lu                 | 113 ms                                                 | 115 ms: 1.02x slower                                                   |
| pprint_safe_repr           | 728 ms                                                 | 742 ms: 1.02x slower                                                   |
| comprehensions             | 16.5 us                                                | 16.8 us: 1.02x slower                                                  |
| hexiom                     | 6.16 ms                                                | 6.28 ms: 1.02x slower                                                  |
| deltablue                  | 3.23 ms                                                | 3.30 ms: 1.02x slower                                                  |
| bench_thread_pool          | 822 us                                                 | 841 us: 1.02x slower                                                   |
| logging_silent             | 102 ns                                                 | 104 ns: 1.03x slower                                                   |
| docutils                   | 2.59 sec                                               | 2.66 sec: 1.03x slower                                                 |
| nqueens                    | 78.4 ms                                                | 81.2 ms: 1.04x slower                                                  |
| scimark_sor                | 124 ms                                                 | 128 ms: 1.04x slower                                                   |
| mako                       | 11.1 ms                                                | 11.5 ms: 1.04x slower                                                  |
| xml_etree_iterparse        | 101 ms                                                 | 105 ms: 1.04x slower                                                   |
| chaos                      | 58.1 ms                                                | 60.6 ms: 1.04x slower                                                  |
| nbody                      | 87.0 ms                                                | 91.2 ms: 1.05x slower                                                  |
| json_dumps                 | 10.6 ms                                                | 11.3 ms: 1.07x slower                                                  |
| gc_traversal               | 4.37 ms                                                | 4.80 ms: 1.10x slower                                                  |
| create_gc_cycles           | 2.41 ms                                                | 2.69 ms: 1.12x slower                                                  |
| async_tree_io_tg           | 857 ms                                                 | 960 ms: 1.12x slower                                                   |
| bench_mp_pool              | 24.0 ms                                                | 77.8 ms: 3.24x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (10): async_tree_none_tg, pyflate, raytrace, float, pickle_pure_python, xml_etree_parse, async_tree_io, sphinx, fannkuch, pylint
Ignored benchmarks (8) of results/bm-20241007-3.13.0-60403a5/bm-20241007-linux-x86_64-python-v3.13.0-3.13.0-60403a5.json: chameleon, connected_components, gevent_hub, k_core, mypy2, shortest_path, sqlalchemy_declarative, sqlalchemy_imperative
Ignored benchmarks (9) of results/bm-20241018-3.14.0a1+-6d93690/bm-20241018-linux-x86_64-python-6d93690954daae9e9a36-3.14.0a1+-6d93690.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlite_synth, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.019x faster
# HPT report

- Reliability score: 96.70% likely to be faster
- 90% likely to have a speedup of 1.00x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.02x