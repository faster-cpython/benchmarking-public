
# Results vs. 3.11.0

- fork: python
- ref: 3.12
- machine: linux-x86_64
- commit hash: 0327106
- commit date: 2023-07-24
- overall geometric mean: 1.07x faster

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| 2to3           | 288 ms                                                       | 285 ms: 1.01x faster                                         |
| docutils       | 2.86 sec                                                     | 2.89 sec: 1.01x slower                                       |
| tornado_http   | 122 ms                                                       | 120 ms: 1.01x faster                                         |
| Geometric mean | (ref)                                                        | 1.00x faster                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| nbody          | 90.7 ms                                                      | 87.1 ms: 1.04x faster                                        |
| pidigits       | 251 ms                                                       | 260 ms: 1.03x slower                                         |
| float          | 74.2 ms                                                      | 79.2 ms: 1.07x slower                                        |
| Geometric mean | (ref)                                                        | 1.02x slower                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|----------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| regex_compile  | 158 ms                                                       | 145 ms: 1.09x faster                                         |
| regex_v8       | 23.9 ms                                                      | 23.7 ms: 1.01x faster                                        |
| regex_dna      | 227 ms                                                       | 246 ms: 1.09x slower                                         |
| Geometric mean | (ref)                                                        | 1.00x faster                                                 |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|----------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| json_dumps           | 13.4 ms                                                      | 10.3 ms: 1.31x faster                                        |
| json_loads           | 28.7 us                                                      | 25.2 us: 1.14x faster                                        |
| unpickle_pure_python | 241 us                                                       | 213 us: 1.13x faster                                         |
| xml_etree_parse      | 158 ms                                                       | 146 ms: 1.08x faster                                         |
| tomli_loads          | 2.26 sec                                                     | 2.17 sec: 1.04x faster                                       |
| unpickle_list        | 4.53 us                                                      | 4.72 us: 1.04x slower                                        |
| xml_etree_process    | 56.5 ms                                                      | 58.9 ms: 1.04x slower                                        |
| xml_etree_generate   | 80.5 ms                                                      | 85.3 ms: 1.06x slower                                        |
| pickle               | 9.64 us                                                      | 10.4 us: 1.08x slower                                        |
| pickle_dict          | 30.8 us                                                      | 33.4 us: 1.09x slower                                        |
| unpickle             | 13.4 us                                                      | 14.7 us: 1.09x slower                                        |
| pickle_list          | 3.83 us                                                      | 4.44 us: 1.16x slower                                        |
| Geometric mean       | (ref)                                                        | 1.01x faster                                                 |

Benchmark hidden because not significant (2): xml_etree_iterparse, pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|------------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| python_startup         | 10.8 ms                                                      | 11.5 ms: 1.07x slower                                        |
| python_startup_no_site | 7.76 ms                                                      | 8.51 ms: 1.10x slower                                        |
| Geometric mean         | (ref)                                                        | 1.08x slower                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|-----------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| mako      | 11.0 ms                                                      | 9.96 ms: 1.10x faster                                        |

All benchmarks:
===============

| Benchmark                | bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509 | bm-20230724-pythonperf2-x86_64-python-3.12-3.12.0b4+-0327106 |
|--------------------------|:------------------------------------------------------------:|:------------------------------------------------------------:|
| typing_runtime_protocols | 523 us                                                       | 154 us: 3.39x faster                                         |
| asyncio_tcp              | 753 ms                                                       | 380 ms: 1.98x faster                                         |
| asyncio_tcp_ssl          | 3.08 sec                                                     | 1.57 sec: 1.96x faster                                       |
| generators               | 56.0 ms                                                      | 36.1 ms: 1.55x faster                                        |
| json_dumps               | 13.4 ms                                                      | 10.3 ms: 1.31x faster                                        |
| chaos                    | 80.9 ms                                                      | 62.6 ms: 1.29x faster                                        |
| fannkuch                 | 429 ms                                                       | 344 ms: 1.25x faster                                         |
| deltablue                | 4.00 ms                                                      | 3.21 ms: 1.25x faster                                        |
| coroutines               | 27.6 ms                                                      | 22.2 ms: 1.24x faster                                        |
| mypy2                    | 451 ms                                                       | 368 ms: 1.23x faster                                         |
| richards_super           | 61.0 ms                                                      | 50.5 ms: 1.21x faster                                        |
| hexiom                   | 7.13 ms                                                      | 5.90 ms: 1.21x faster                                        |
| scimark_lu               | 115 ms                                                       | 96.9 ms: 1.18x faster                                        |
| comprehensions           | 24.6 us                                                      | 21.6 us: 1.14x faster                                        |
| json_loads               | 28.7 us                                                      | 25.2 us: 1.14x faster                                        |
| go                       | 164 ms                                                       | 145 ms: 1.13x faster                                         |
| unpickle_pure_python     | 241 us                                                       | 213 us: 1.13x faster                                         |
| async_tree_memoization   | 630 ms                                                       | 557 ms: 1.13x faster                                         |
| nqueens                  | 103 ms                                                       | 91.0 ms: 1.13x faster                                        |
| async_tree_none          | 519 ms                                                       | 466 ms: 1.11x faster                                         |
| mako                     | 11.0 ms                                                      | 9.96 ms: 1.10x faster                                        |
| async_tree_io            | 1.17 sec                                                     | 1.07 sec: 1.10x faster                                       |
| sqlglot_parse            | 1.53 ms                                                      | 1.39 ms: 1.10x faster                                        |
| regex_compile            | 158 ms                                                       | 145 ms: 1.09x faster                                         |
| mdp                      | 2.75 sec                                                     | 2.52 sec: 1.09x faster                                       |
| richards                 | 48.3 ms                                                      | 44.5 ms: 1.09x faster                                        |
| json                     | 5.65 ms                                                      | 5.21 ms: 1.08x faster                                        |
| xml_etree_parse          | 158 ms                                                       | 146 ms: 1.08x faster                                         |
| logging_format           | 8.11 us                                                      | 7.54 us: 1.08x faster                                        |
| deepcopy                 | 399 us                                                       | 372 us: 1.07x faster                                         |
| sqlglot_normalize        | 126 ms                                                       | 117 ms: 1.07x faster                                         |
| sqlglot_transpile        | 1.92 ms                                                      | 1.80 ms: 1.07x faster                                        |
| pycparser                | 1.32 sec                                                     | 1.24 sec: 1.07x faster                                       |
| logging_simple           | 7.19 us                                                      | 6.80 us: 1.06x faster                                        |
| deepcopy_memo            | 38.8 us                                                      | 36.7 us: 1.06x faster                                        |
| async_tree_cpu_io_mixed  | 749 ms                                                       | 711 ms: 1.05x faster                                         |
| bench_thread_pool        | 1.01 ms                                                      | 971 us: 1.04x faster                                         |
| tomli_loads              | 2.26 sec                                                     | 2.17 sec: 1.04x faster                                       |
| nbody                    | 90.7 ms                                                      | 87.1 ms: 1.04x faster                                        |
| sqlglot_optimize         | 59.8 ms                                                      | 57.6 ms: 1.04x faster                                        |
| deepcopy_reduce          | 3.51 us                                                      | 3.40 us: 1.03x faster                                        |
| dulwich_log              | 68.4 ms                                                      | 66.3 ms: 1.03x faster                                        |
| crypto_pyaes             | 83.4 ms                                                      | 81.0 ms: 1.03x faster                                        |
| raytrace                 | 317 ms                                                       | 308 ms: 1.03x faster                                         |
| meteor_contest           | 131 ms                                                       | 127 ms: 1.03x faster                                         |
| logging_silent           | 101 ns                                                       | 98.3 ns: 1.03x faster                                        |
| pyflate                  | 449 ms                                                       | 441 ms: 1.02x faster                                         |
| spectral_norm            | 93.3 ms                                                      | 91.9 ms: 1.02x faster                                        |
| tornado_http             | 122 ms                                                       | 120 ms: 1.01x faster                                         |
| scimark_sor              | 111 ms                                                       | 110 ms: 1.01x faster                                         |
| regex_v8                 | 23.9 ms                                                      | 23.7 ms: 1.01x faster                                        |
| 2to3                     | 288 ms                                                       | 285 ms: 1.01x faster                                         |
| scimark_monte_carlo      | 68.2 ms                                                      | 68.0 ms: 1.00x faster                                        |
| docutils                 | 2.86 sec                                                     | 2.89 sec: 1.01x slower                                       |
| pathlib                  | 19.1 ms                                                      | 19.3 ms: 1.01x slower                                        |
| pprint_pformat           | 1.63 sec                                                     | 1.65 sec: 1.01x slower                                       |
| create_gc_cycles         | 1.61 ms                                                      | 1.64 ms: 1.02x slower                                        |
| gc_traversal             | 3.85 ms                                                      | 3.93 ms: 1.02x slower                                        |
| scimark_fft              | 285 ms                                                       | 293 ms: 1.03x slower                                         |
| pprint_safe_repr         | 784 ms                                                       | 809 ms: 1.03x slower                                         |
| telco                    | 6.86 ms                                                      | 7.08 ms: 1.03x slower                                        |
| pidigits                 | 251 ms                                                       | 260 ms: 1.03x slower                                         |
| unpickle_list            | 4.53 us                                                      | 4.72 us: 1.04x slower                                        |
| xml_etree_process        | 56.5 ms                                                      | 58.9 ms: 1.04x slower                                        |
| coverage                 | 84.8 ms                                                      | 89.4 ms: 1.05x slower                                        |
| scimark_sparse_mat_mult  | 4.05 ms                                                      | 4.28 ms: 1.06x slower                                        |
| xml_etree_generate       | 80.5 ms                                                      | 85.3 ms: 1.06x slower                                        |
| float                    | 74.2 ms                                                      | 79.2 ms: 1.07x slower                                        |
| python_startup           | 10.8 ms                                                      | 11.5 ms: 1.07x slower                                        |
| unpack_sequence          | 45.6 ns                                                      | 49.1 ns: 1.08x slower                                        |
| pickle                   | 9.64 us                                                      | 10.4 us: 1.08x slower                                        |
| regex_dna                | 227 ms                                                       | 246 ms: 1.09x slower                                         |
| pickle_dict              | 30.8 us                                                      | 33.4 us: 1.09x slower                                        |
| unpickle                 | 13.4 us                                                      | 14.7 us: 1.09x slower                                        |
| python_startup_no_site   | 7.76 ms                                                      | 8.51 ms: 1.10x slower                                        |
| sqlite_synth             | 2.50 us                                                      | 2.75 us: 1.10x slower                                        |
| pickle_list              | 3.83 us                                                      | 4.44 us: 1.16x slower                                        |
| async_generators         | 316 ms                                                       | 396 ms: 1.25x slower                                         |
| dask                     | 410 ms                                                       | 568 ms: 1.39x slower                                         |
| Geometric mean           | (ref)                                                        | 1.07x faster                                                 |

Benchmark hidden because not significant (4): xml_etree_iterparse, pickle_pure_python, regex_effbot, bench_mp_pool
Ignored benchmarks (16) of results/bm-20221024-3.11.0-deaf509/bm-20221024-pythonperf2-x86_64-python-v3.11.0-3.11.0-deaf509.json: aiohttp, chameleon, django_template, flaskblogging, genshi_text, genshi_xml, gunicorn, html5lib, pylint, sqlalchemy_declarative, sqlalchemy_imperative, sympy_expand, sympy_integrate, sympy_str, sympy_sum, thrift
